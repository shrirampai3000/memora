...
for (let i = 0; i < args.length; i++) {
  ...
  } else if (args[i] === "--MEMORA-host" && args[i + 1]) {
    host = args[i + 1];
  } else if (
    (args[i] === "--MEMORA-url" || args[i] === "--MEMORA-api-url") &&
    args[i + 1]
  ) {
...
// Resolve the MEMORA API base URL so this MCP can target a REMOTE
// MEMORA (e.g. an agent on a VPS reading a synced copy of your data),
// not just localhost. Priority:
//   1. --MEMORA-url / --MEMORA-api-url flag
//   2. MEMORA_API_URL env (set by `MEMORA agent setup --api-url`)
//   3. --MEMORA-host (+ --port) â†’ http://host:port
...
//   2. CLI via bundled `bun` from MEMORA.app at a deterministic absolute
//      path. Runs `bun x MEMORA@latest auth token` â†’ goes through the
...
//      contract as `MEMORA auth token` in a terminal, no PATH needed.
//   3. CLI via node-adjacent npx â€” for dev environments that have node but
//      not the desktop app.
//   4. CLI via PATH-based npx â€” last CLI fallback.
//   5. Direct sqlite3 read of ~/.MEMORA/db.sqlite â€” plaintext entries
//      only (encrypted entries need the keychain, which only the CLI can
//      reach). Kept as a final last-resort for users who have MEMORA
...
async function discoverApiKey(): Promise<string> {
  const envKey = process.env.MEMORA_LOCAL_API_KEY || process.env.MEMORA_API_KEY;
  ...
  // gates the MCP handshake â€” the cap only stops a hung CLI from making the
  ...
  //    deterministic install path on each OS, so we don't need PATH â€”
  ...
  //    Node process â€” so we approximate it with the sources below, ordered
  //    most- to least-reliable:
  const bunExe = process.platform === "win32" ? "bun.exe" : "bun";
  const bunCandidates: string[] = [];

  // 2a. Explicit override â€” the app (or a user) can point us straight at the
  //     bundled bun, bypassing every guess below. Cheapest + most reliable.
  if (process.env.MEMORA_BUN_PATH) bunCandidates.push(process.env.MEMORA_BUN_PATH);

  // 2b. The bun that is running THIS process, if any. When Claude launches us
  //     via the config we write (`<abs>/bun x MEMORA-mcp@latest`), the
  //     bundled bun is our own runner â€” so its path is knowable without
  ...
  if (process.platform === "darwin") {
    for (const appName of ["MEMORA", "MEMORA beta", "MEMORA enterprise"]) {
    ...
    for (const dir of ["MEMORA", "MEMORA beta", "MEMORA enterprise", "MEMORA-app"]) {
      bunCandidates.push(path.join(home, "AppData", "Local", dir, "bun.exe"));
    }
    bunCandidates.push("C:\\Program Files\\MEMORA\\bun.exe");
  } else {
    // Linux: the .deb/.rpm install dir and common AppImage/manual locations.
    bunCandidates.push(
      "/opt/MEMORA/bun",
      "/usr/lib/MEMORA/bun",
      "/usr/lib/MEMORA-app/bun",
      "/usr/local/lib/MEMORA/bun",
      "/usr/bin/bun",
      path.join(home, ".local", "share", "MEMORA", "bun"),
    );
  }
  ...
  for (const bunPath of Array.from(new Set(bunCandidates))) {
    ...
    try {
      // first run downloads the package; subsequent runs are cached â€” clamp to
      // the remaining overall budget, capped per attempt.
      const { stdout } = await execFileAsync(bunPath, ["x", "MEMORA@latest", "auth", "token"], {
  ...
  try {
    ...
    if (fs.existsSync(npxPath) && budgetLeft() > 0) {
      const { stdout } = await execFileAsync(npxPath, ["MEMORA@latest", "auth", "token"], {
        timeout: Math.min(PER_CANDIDATE_MS, budgetLeft()),
        encoding: "utf-8",
      });
      const token = String(stdout).trim();
  ...
  try {
    if (budgetLeft() > 0) {
      const { stdout } = await execAsync("npx MEMORA@latest auth token", {
        timeout: Math.min(PER_CANDIDATE_MS, budgetLeft()),
        encoding: "utf-8",
      });
      const token = String(stdout).trim();
  ...
  //    entries only â€” encrypted ones live behind the keychain, which the
  //    CLI paths above already cover. Used when the user has MEMORA
  ...
  try {
    const dbPath = path.join(home, ".MEMORA", "db.sqlite");
    if (fs.existsSync(dbPath)) {
      ...
      if (row) {
        ...
        // Encrypted â€” only the CLI paths above can decrypt this; we
        // already tried them.
      }
    }
  } catch {}
  ...
  // the MEMORA server on every tool call.
  process.stderr.write(
    [
      "[MEMORA-mcp] could not discover MEMORA_LOCAL_API_KEY from any source.",
      "  - env vars (MEMORA_LOCAL_API_KEY / MEMORA_API_KEY) not set",
      "  - bundled `bun` from MEMORA.app not found at any known install path",
      "  - npx fallback unavailable",
      "  - direct sqlite3 read of ~/.MEMORA/db.sqlite failed",
      "Fix: set MEMORA_LOCAL_API_KEY in your MCP launcher's env block,",
      "or install the MEMORA desktop app (https://github.com/shrirampai3000/memora).",
      "",
    ].join("\n"),
  );
  // This is a user-side misconfiguration (no key set + no desktop app / CLI /
  // local DB), not a MEMORA defect â€” the stderr hint above tells the user
...
// scope used to block the entire module body from finishing â€” which meant
...
// Claude Desktop reported "Could not attach to MCP server MEMORA".
//
...
let API_KEY = process.env.MEMORA_LOCAL_API_KEY || process.env.MEMORA_API_KEY || "";
...
// Enterprise team token â€” when present, this MCP additionally registers
...
// MEMORA-mcp inside Claude Desktop / Cursor / Windsurf wants to ask "what
...
//
//   TOKEN (discoverTeamToken) â€” `sk_ent_â€¦`:
//     1. MEMORA_ENTERPRISE_TOKEN env var (Claude config, terminal)
//     2. team_api_token in ~/.MEMORA/enterprise.json (written by the
//        desktop app's Settings â†’ Privacy â†’ Admin Team API Token)
//
//   BASE URL (discoverTeamApiBase) â€” where those tools send their requests:
//     1. --team-api-url flag
//     2. MEMORA_TEAM_API_URL env var
//     3. gateway_url in ~/.MEMORA/enterprise.json
//     4. the hosted default, https://github.com/shrirampai3000/memora/api/enterprise/v1
//
...
// read path to their data â€” pointing there returns 401. The bearer token is
// the same `sk_ent_` either way; only the base moves.
//
// Empty / missing token â†’ team tools are not registered; non-admin users of
// MEMORA-mcp see exactly what they see today.
...
const TOOLS: Tool[] = [
  {
    ...
    description:
      ...
      "USE WHEN: you need the actual text/content of a moment â€” quotes, screen text, transcript lines, or compact parsed messages, emails, tasks, documents, and code review â€” or want to filter by speaker/window. " +
    ...
    inputSchema: {
      ...
      properties: {
        q: {
          type: "string",
          description: "Full-text search query. Omit to return all content in time range. Avoid for audio â€” transcriptions are noisy, q filters too aggressively.",
        },
        content_type: {
          type: "string",
          enum: ["all", "ocr", "audio", "input", "accessibility", "memory", "parsed"],
          description:
            "Filter by content type. Use 'parsed' for compact app-specific records such as messages, emails, tasks, documents, and code review; it is experimental and may be empty when parsing is disabled or unsupported. NOTE on screen text: 'ocr' is a legacy label â€” it returns ALL screen-text rows, which are accessibility-derived for most apps (the result tag [ScreenÂ·a11y] vs [ScreenÂ·ocr] tells you which). Use 'ocr' for screen text (covers both paths), 'audio' for transcriptions, 'input' for keyboard/mouse events, 'memory' for stored facts. Default: 'all'.",
          default: "all",
        },
  ...
  },
  {
    ...
    description:
      ...
      "Pass `q` to substring-match title, attendee names/emails, and notes â€” `q` searches ALL meeting history, so when " +
    ...
    inputSchema: {
      type: "object",
      properties: {
        start_time: { type: "string", description: "ISO 8601 UTC or relative (e.g. '1d ago'). Omit when searching by q â€” it filters all history." },
  ...
  },
  {
    ...
    description:
      ...
      "USE WHEN: any broad question about what the user did â€” 'what was I doing?', 'how long on X?', 'which apps?', 'recap my morning'. " +
      "This is almost always the right first call for time-range questions â€” usually sufficient without follow-up searches. " +
  ...
  },
  {
    ...
    description:
      ...
      "USE WHEN: you want a specific UI control or page-structure question â€” 'find every Submit button I saw', 'list the links in that page'. " +
      ...
      properties: {
        ...
        output_path: {
          ...
          description:
            ...
            "Defaults to the MEMORA data dir's exports/ folder.",
        },
      },
      required: ["start_time", "end_time"],
    },
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "integer", description: "Memory ID â€” omit to create new, provide to update/delete" },
  ...
  },
  {
    ...
    description:
      "Send a notification to the MEMORA desktop UI. " +
        ...
        actions: {
          ...
          items: {
            ...
            properties: {
              ...
              pipe: { type: "string", description: "Target pipe to run when clicked (type=pipe). Set this explicitly â€” e.g. a 'share-data' pipe. If omitted it falls back to the sending pipe, which usually does nothing useful." },
              prompt: { type: "string", description: "Instruction to run in a fresh chat session when clicked (type=chat). Write the whole task inline â€” no pre-installed pipe required." },
              ...
              context: { type: "object", description: "Data passed to the action (type=pipe â†’ injected into the pipe prompt; type=chat â†’ included as background context)" },
  ...
  },
  {
    name: "health-check",
    description:
      "Check if MEMORA is running and healthy. Returns recording status, frame/audio stats, timestamps.",
  ...
  },
  {
    ...
    description:
      ...
      "Pass include_transcript=true to also get the speaker-attributed transcript segments â€” do this when the note is " +
  ...
  },
  {
    ...
    description:
      ...
      "others stay as-is. Use this to save an AI-generated summary into the meeting note â€” read the current note first via get-meeting " +
  ...
  },
  {
    ...
    description:
      ...
      "DO NOT USE for: structured filters by content_type / speaker / window â€” this endpoint ignores those (use search-content instead). " +
  ...
  // presets, how a pipe should query MEMORA) lives in the
  // MEMORA://guide/pipes resource â€” keep these descriptions short and
  ...
  },
  {
    name: "create-pipe",
    description:
      "Create a pipe â€” a scheduled AI automation that runs a markdown prompt on a schedule (e.g. 'every day at 9am'). " +
      "Writes ~/.MEMORA/pipes/<name>/pipe.md, installs it, enables it, and (by default) runs it once to test. " +
      "USE WHEN: the user wants to automate a recurring task (daily summary, reminder, report, monitor, sync). " +
      "IMPORTANT: read the MEMORA://guide/pipes resource FIRST â€” it documents the prompt format, schedule syntax, presets, and how the pipe prompt should query MEMORA. After creating, check pipe-logs to confirm the test run worked.",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        prompt: {
          type: "string",
          description:
            "The markdown instructions the AI agent runs every scheduled execution. Be specific: what to query (which MEMORA endpoints + time range), how to process it, and what to output (write a note, send a notification, push to a connection). MEMORA prepends a context header (time range, timezone, OS, API base + key) before each run, so no template variables are needed. See MEMORA://guide/pipes.",
        },
        schedule: {
          type: "string",
          description:
...
// ---------------------------------------------------------------------------
// Enterprise team tools â€” registered only when a team API token is present.
// Same endpoint surface as the desktop `MEMORA-team` pi-agent skill:
...
const TEAM_TOOLS: Tool[] = [
  {
    ...
    description:
      ...
      "Auth: enterprise admin token (sk_ent_â€¦). " +
  ...
  },
  {
    name: "team-devices",
    description:
      "List all devices enrolled under this org's license â€” hostname, OS, " +
  ...
  },
  {
    name: "team-records",
    description:
      "Chronological dump of the org's data for a time window â€” both raw " +
      ...
      "Raw kinds return oldest â†’ newest (vs team-search which is recency-ranked). " +
      ...
      "we extract from my team's work\" â€” each item carries evidence-cited " +
...
// ---------------------------------------------------------------------------
// Resources â€” dynamic context only (no duplicated reference docs)
// ---------------------------------------------------------------------------
const RESOURCES = [
  {
    uri: "MEMORA://context",
  ...
  },
  {
    uri: "MEMORA://guide",
    name: "Usage Guide",
    description: "How to use MEMORA tools effectively â€” search strategy, progressive disclosure, and common patterns",
    mimeType: "text/markdown",
  },
  {
    uri: "MEMORA://guide/pipes",
    name: "Creating Pipes",
    description:
      "How to create pipes (scheduled AI automations): the pipe.md prompt format, schedule syntax, presets, how the prompt should query MEMORA, and the createâ†’runâ†’logs lifecycle. Read before using create-pipe.",
    mimeType: "text/markdown",
  },
];
      ...
      contents: [
        {
          uri,
          mimeType: "text/markdown",
          text: `# MEMORA Usage Guide

## Progressive Disclosure â€” start light, escalate only when needed
...
- **Always provide start_time** â€” without it, search scans the entire history
- **Start with limit=5** â€” increase only if you need more results
- **Use max_content_length=500** to keep responses compact
- **Don't use q for audio** â€” transcriptions are noisy, q filters too aggressively. Search audio by time range and speaker instead
- **app_name is case-sensitive** â€” use exact names: "Google Chrome" not "chrome"
- **Screen text is mostly accessibility-derived, not OCR.** MEMORA walks the OS accessibility tree first; OCR is only a fallback (terminals, canvas-rendered apps, games). \`content_type=ocr\` returns both paths â€” the result label \`[ScreenÂ·a11y]\` vs \`[ScreenÂ·ocr]\` tells you which produced the row. Don't pre-filter to a11y/ocr unless you specifically need one or the other

## Common Patterns

- "What was I doing for the last 2 hours?" â†’ activity-summary with start_time='2h ago'
- "What did I discuss in my meeting?" â†’ list-meetings to find it, then get-meeting with include_transcript=true
- "When did I last talk to <person>?" â†’ list-meetings with q=<name or email>, NO start_time (q searches all history)
- "Find when I was on Twitter" â†’ search-content with app_name='Arc' (or the browser name), q='twitter'
- "Remember that I prefer X" â†’ update-memory with content describing the preference
- "What do you remember about X?" â†’ search-content with content_type='memory', q='X'
- "Automate X every day / on a schedule" â†’ read the MEMORA://guide/pipes resource, then create-pipe (a scheduled AI automation)

## Deep Links

When referencing specific moments in results, create clickable links:
- Frame: [10:30 AM â€” Chrome](MEMORA://frame/{frame_id}) â€” use frame_id from search results
- Timeline: [meeting at 3pm](MEMORA://timeline?timestamp=2024-01-15T15:00:00Z) â€” use exact timestamp from results
Never fabricate IDs or timestamps â€” only use values from actual results.
`,
        },
      ],
    };
  ...
  if (uri === "MEMORA://guide/pipes") {
    return {
      contents: [
        {
          uri,
          mimeType: "text/markdown",
          text: `# Creating Pipes â€” scheduled AI automations

A **pipe** is a markdown prompt that an AI agent runs on a schedule. Each pipe is a folder \`~/.MEMORA/pipes/<name>/pipe.md\` with YAML frontmatter + a prompt body. Use the **create-pipe** tool â€” it writes the file, installs, enables, and (by default) runs it once to test. Manage with **list-pipes**, **run-pipe**, and **pipe-logs**.
...
**schedule** (required): \`every 30m\` Â· \`every 1h\` Â· \`every day at 9am\` Â· \`every monday at 9am\` Â· or cron \`0 9 * * *\`.

MEMORA **prepends a context header** before every run (current time range, timezone, OS, API base URL + auth). So the prompt does NOT need template variables or to hardcode the key â€” it just says what to do.
...
1. **Query** the relevant window of activity. Prefer the same endpoints these MCP tools wrap:
   - \`GET /activity-summary?start_time=...&end_time=now\` â€” apps/windows/durations. **Let this endpoint own all time math; never sum minutes in the prompt (the model drifts).**
   - \`GET /search?q=...&content_type=all&start_time=...\` â€” specific screen text, audio transcripts, memories.
   - \`GET /memories?...\`, \`GET /meetings?...\` for curated facts / meetings.
   Always pass \`start_time\` â€” never scan the whole history.
2. **Process / summarize** the results.
3. **Output** somewhere: write a note/file, send a desktop notification (\`POST\` the Tauri sidecar on port 11435 \`/notify\`), or push to a configured connection (Telegram/Slack/Discord/Email â€” see the CLI \`connection\` commands).

Keep each pipe to **one bounded job**. A focused "summarize my day and write it to a note" beats a vague "monitor everything".

## Lifecycle

- **create-pipe** â†’ writes pipe.md + installs + enables (+ optional \`run_now\` test).
- **run-pipe** â†’ run once now to test, independent of schedule.
- **pipe-logs** â†’ read the output / debug.
...
class BackendDownError extends Error {
  constructor(public readonly cause: unknown) {
    super(
      `MEMORA backend not running on ${MEMORA_API}. ` +
        `Start it with \`MEMORA\` in a terminal, or open the MEMORA desktop app.`,
    );
...
class BackendHttpError extends Error {
  constructor(
    ...
    if (status === 401 || status === 403) {
      hint =
        " â€” API key not accepted. Set MEMORA_LOCAL_API_KEY in your MCP " +
        "launcher env, or install the MEMORA desktop app so the MCP can " +
        "discover the key automatically.";
    } else if (status === 404) {
      hint =
        " â€” endpoint not found. The backend may be on a different version than this MCP.";
    } else if (status === 400) {
      hint = " â€” bad request. Check argument names and types against the tool schema.";
    } else if (status >= 500) {
      hint = " â€” backend error. Check MEMORA logs.";
    }
...
async function fetchAPI(
  ...
  const url = `${MEMORA_API}${endpoint}`;
  // Resolve the key lazily on the first request â€” never at module load, so the
...
// ("2026-05-17") â€” normalize those here so the request doesn't 400.
function normalizeTime(input: string | undefined): string | undefined {
  ...
  // Bare YYYY-MM-DD â†’ start of day UTC
...
// its LOCAL timezone (e.g. "...T09:03:44+05:30"), so the HH:MM is already local â€”
...
function truncateMiddle(text: string | null | undefined, max: number): string {
  ...
  return (
    ...
    `â€¦[${cut} chars truncated â€” pass max_content_length=0 for full text]â€¦` +
...
// OCR in the engine but most captures are accessibility-derived â€” surface
// the actual source so the model picks filters correctly.
function screenTag(textSource: unknown): string {
  if (textSource === "accessibility") return "[ScreenÂ·a11y]";
  if (textSource === "ocr") return "[ScreenÂ·ocr]";
  return "[Screen]";
}
    ...
    switch (name) {
        ...
        if (!pipes.length) {
          return {
            content: [
              {
                type: "text",
                text: "No pipes yet. Use create-pipe to add a scheduled automation (read the MEMORA://guide/pipes resource first).",
              },
            ],
          };
        }
      ...
      case "create-pipe": {
        ...
        if (!/^[a-z0-9][a-z0-9-_]*$/i.test(pipeName)) {
          throw new Error(
            "invalid pipe name â€” use kebab-case letters/numbers/dashes, e.g. 'daily-time-report'"
          );
        }
        ...
        const dir = path.join(os.homedir(), ".MEMORA", "pipes", pipeName);
        ...
        if (runNow) {
          try {
            ...
            runNote = `\n\nA test run was started â€” read it with pipe-logs (name="${pipeId}").`;
          } catch (e) {
          ...
          content: [
            {
              type: "text",
              text:
                `Created pipe "${pipeId}" â€” schedule: ${schedule}, ${enabled ? "enabled" : "disabled"}.\n` +
                steps.map((s) => `- ${s}`).join("\n") +
                runNote,
            },
          ],
      ...
      case "pipe-logs": {
        ...
        const trimmed = text.length > 6000 ? `â€¦${text.slice(-6000)}` : text;
      ...
      case "search-content": {
        ...
        for (const result of results) {
          ...
          if (result.type === "OCR") {
            ...
            // frame_id links a memory back to the exact moment â€” jump there with
      ...
      case "list-meetings": {
        ...
        if (!Array.isArray(meetings) || meetings.length === 0) {
          ...
          const hint = normalized.q
            ? hadTimeFilter
              ? " Retry the same q WITHOUT start_time/end_time â€” q searches all meeting history."
              : " Try a shorter substring (single first name, email fragment) â€” matching is exact-substring, not fuzzy."
        ...
        const formatted = meetings.map((m: Record<string, unknown>) => {
          ...
          const note = noteStr
            ? `\nNote: ${noteStr.length > 200 ? `${noteStr.slice(0, 200)}â€¦` : noteStr}`
            : "\nNote: (none â€” use get-meeting with include_transcript to reconstruct)";
          return `[id ${m.id}] [${m.detection_source}] ${app}${title}\n  ${start} â†’ ${end}${attendees}${note}`;
        });
      ...
      case "activity-summary": {
          ...
          (a: {
            ...
            const timeSpan =
              a.first_seen && a.last_seen
                ? `, ${a.first_seen.slice(11, 16)}â€“${a.last_seen.slice(11, 16)}${zoneSuffix(a.first_seen)}`
                : "";
            return `  ${a.name}: ${a.minutes} min (${a.frame_count} frames${timeSpan})`;
          }
        );

        // Window/tab activity â€” what pages/documents were open
        const windowLines = (data.windows || []).map(
          (w: {
            ...
            return `  [${w.app_name}] ${w.window_name}${url} â€” ${w.minutes} min`;
          }
        );
      ...
      case "export-video": {
        ...
        // engine export core (the `MEMORA export` CLI's HTTP twin). MCP runs on the
      ...
      case "send-notification": {
        ...
        // (11435), not the MEMORA API. Use 127.0.0.1 because the daemon is
        ...
        try {
          ...
          if (e instanceof Error && e.name === "AbortError") {
            throw new Error(
              "notification daemon accepted the request but did not respond within 3s â€” the desktop notification UI may be stuck",
            );
          }
          throw new Error(
            "notification daemon not reachable on 127.0.0.1:11435 â€” is the MEMORA desktop app running?",
          );
        } finally {
      ...
      case "search-speakers": {
        ...
        const formatted = speakers.map(
          (s: { id: number; name: string; metadata?: string }) =>
            `#${s.id} ${s.name}${s.metadata ? ` â€” ${s.metadata}` : ""}`
        );
        return {
      ...
      case "get-meeting": {
        ...
        if (args.include_transcript) {
          ...
          if (!Array.isArray(segments) || segments.length === 0) {
            ...
            const more =
              remaining > 0
                ? `\nâ€¦ ${remaining} more segments â€” call again with transcript_offset=${offset + shown}.`
                : "";
      ...
      case "update-meeting": {
        ...
        // Build partial body â€” only forward fields the caller provided.
        const body: Record<string, unknown> = {};
      ...
      case "keyword-search": {
        ...
        //   content_type: dropped â€” the keyword endpoint doesn't filter by type.
        ...
        const formatted = results.map((r) => {
          ...
          // or "ocr" (fallback) â€” show it so the model knows which path hit.
      ...
      // ---------------------------------------------------------------------
      // Enterprise team tools â€” only callable when TEAM_TOKEN is set at boot.
      ...
      case "team-records": {
            ...
            content: [
              {
                type: "text",
                text:
                  `team-* tools require an enterprise admin token (sk_ent_â€¦). ` +
                  `Set MEMORA_ENTERPRISE_TOKEN in your MCP env, or paste ` +
                  `the token into Settings â†’ Privacy â†’ Admin Team API Token in ` +
                  `the MEMORA desktop app.\n` +
                  `Where to get it: hosted orgs mint one at ` +
                  `https://github.com/shrirampai3000/memora/enterprise â†’ API Tokens. Orgs running ` +
                  `their own query gateway use the token their gateway accepts, ` +
                  `and must ALSO point this MCP at that gateway â€” ` +
                  `MEMORA_TEAM_API_URL=https://<gateway>/api/enterprise/v1 ` +
                  `(or --team-api-url). Requests currently go to ${TEAM_API}.`,
              },
            ],
          };
        }
        // Map MCP tool name â†’ /api/enterprise/v1 path. team-records also
        ...
        if (!response.ok) {
          throw new Error(
            `${name} failed: HTTP ${response.status} ${response.statusText} â€” ${body.slice(0, 300)}`
          );
        }
...
async function main() {
  ...
  // shows how far startup got. The transport is connected FIRST â€” before any
  // key discovery â€” so attach never depends on (possibly slow) auth.
  console.error(`[MEMORA-mcp] v${PKG_VERSION} phase=connect target=${MEMORA_API}`);
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("[MEMORA-mcp] phase=connected transport=stdio");
  // Warm the API key in the background so the first tool call doesn't pay the
  // discovery latency. Never awaited here â€” key discovery must not gate attach.
  void ensureApiKey();
}
