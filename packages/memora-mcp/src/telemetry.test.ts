...
describe("MCP telemetry privacy", () => {
  it("honors MEMORA telemetry opt-out env vars", () => {
    expect(isMcpTelemetryDisabled({ MEMORA_MCP_SENTRY_DISABLED: "1" })).toBe(true);
    expect(isMcpTelemetryDisabled({ MEMORA_TELEMETRY_DISABLED: "true" })).toBe(true);
    expect(isMcpTelemetryDisabled({ MEMORA_DISABLE_TELEMETRY: "yes" })).toBe(true);
    expect(isMcpTelemetryDisabled({ MEMORA_MCP_SENTRY_DISABLED: "0" })).toBe(false);
  });

  it("redacts bearer tokens, MEMORA tokens, enterprise tokens, and home paths", () => {
    const home = process.env.HOME || "";
    const value = [
      "Bearer abc.def-123",
      "sp-secret-token",
      "sk_ent_secret-token",
      "MEMORA_LOCAL_API_KEY=sp-local-secret",
      home ? `${home}/.MEMORA/db.sqlite` : "",
    ].join(" ");
    ...
    expect(scrubbed).toContain("MEMORA_LOCAL_API_KEY=[redacted]");
    if (home) expect(scrubbed).toContain("~/.MEMORA/db.sqlite");
  });
  ...
  it("removes request, user, extra payloads, and disallowed contexts from Sentry events", () => {
    ...
    const event = {
      ...
      exception: {
        values: [{ value: `failed with sp-secret-token at ${home}/.MEMORA/db.sqlite` }],
      },
    };
...
describe("MCP telemetry throttle", () => {
  const cleanup = (key: string) => {
    const marker = path.join(os.tmpdir(), `MEMORA-mcp-throttle-${key}`);
