...
// â”€â”€ CLI parsing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 *
 * Mirrors the MEMORA-engine CLI: --listen-on-lan flips bind to 0.0.0.0
...
export function parseArgs(argv: string[]): CliConfig {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    } else if (a === "--MEMORA-port" && argv[i + 1]) {
      MEMORAPort = parseInt(argv[++i], 10);
  ...
  if (Number.isNaN(MEMORAPort) || MEMORAPort <= 0 || MEMORAPort > 65535) {
    throw new CliError(`invalid --MEMORA-port: ${MEMORAPort}`);
  }
  if (listenOnLan && !apiKey) {
    throw new CliError(
      "--listen-on-lan requires --api-key <secret> â€” refusing to expose " +
...
function usage(): string {
  return [
    "MEMORA-mcp http server",
    "",
    "  --port <n>             listen port (default 3031)",
    "  --MEMORA-port <n>  upstream MEMORA API port (default 3030)",
 ...
 * (loopback-only deployment), non-loopback shouldn't even be reachable â€”
...
const TOOLS = [
  {
    name: "search_content",
    description:
      "Search MEMORA's recorded content: screen text, audio transcriptions, input events, and parsed app data. " +
...
function makeFetchAPI(MEMORAPort: number) {
  // Honor MEMORA_API_URL so the HTTP MCP can also front a remote MEMORA
  // (set by `MEMORA agent setup --api-url`); falls back to the local port.
  const base = (process.env.MEMORA_API_URL || `http://localhost:${MEMORAPort}`).replace(
    /\/+$/,
    "",
  );
...
// â”€â”€ MCP server factory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

// Each HTTP session gets its own Server â€” the MCP SDK requires a 1:1
...
export function buildHttpServer(config: CliConfig) {
  const fetchAPI = makeFetchAPI(config.MEMORAPort);
  ...
  return createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    // Health check is unauthenticated â€” monitors / load balancers need it.
 ...
 * when invoked as `MEMORA-mcp --http â€¦`, in addition to the direct
 * `MEMORA-mcp-http` bin path which auto-starts via the `isMain` check
 * below.
 */
export function runFromArgv(argv: string[]): void {
  ...
  server.listen(config.mcpPort, config.host, () => {
    ...
    console.log(`MEMORA MCP HTTP server listening on ${printable}:${config.mcpPort}`);
    ...
    if (config.apiKey) {
      console.log("  Auth required for non-loopback requests (Authorization: Bearer â€¦)");
    }
  });
}
