...
// Regression guard for "Could not attach to MCP server MEMORA": the stdio
...
// attached. Discovery is now lazy + off the connect path â€” these tests spawn the
 ...
 * silently passed the whole suite â€” including the serverInfo.version assertion
 ...
 * @param stripPath when true, drop PATH so `npx`/node lookups can't resolve â€”
...
function initializeHandshake(
  ...
  return new Promise((resolve, reject) => {
    const baseEnv: Record<string, string> = {
      ...
      // Point at a port with no backend so no real MEMORA is required; the
      // handshake must not depend on the backend being up.
      MEMORA_API_URL: "http://127.0.0.1:59999",
      ...env,
    };
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [CLI], {
      env: {
        ...
        MEMORA_DISABLE_TELEMETRY: "1",
        MEMORA_LOCAL_API_KEY: "sp-smoke-test-key",
        MEMORA_API_URL: "http://127.0.0.1:59999",
      },
      stdio: ["pipe", "pipe", "pipe"],
    });
...
describe("stdio startup handshake", () => {
  ...
  it("completes initialize with the API key present (fast env path)", async () => {
    ...
    expect(response.result?.serverInfo?.name).toBe("MEMORA");
