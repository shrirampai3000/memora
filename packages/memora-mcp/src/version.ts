...
/**
 * The published package version â€” ONE source of truth for every surface that
 ...
 * npm's `MEMORA-mcp@0.18.15` was built from a tree that predated
 * `team-config.ts`, so its `team-*` tools could not be repointed at a
 * customer gateway â€” and because nobody bumped `package.json`, the registry's
