...
describe("parseArgs", () => {
  it("defaults to localhost on 3031 with no api key", () => {
    ...
    expect(c.MEMORAPort).toBe(3030);
    expect(c.host).toBe("127.0.0.1");
    expect(c.apiKey).toBeUndefined();
  });

  it("--port and --MEMORA-port override defaults", () => {
    const c = parseArgs(["--port", "9000", "--MEMORA-port", "8080"]);
    expect(c.mcpPort).toBe(9000);
    expect(c.MEMORAPort).toBe(8080);
  });
  ...
  it("ignores unknown flags rather than crashing", () => {
    // Lenient parser â€” random extra args don't break startup.
...
describe("runFromArgv", () => {
  ...
  // bind a port here â€” just confirm the export exists and that invalid
...
describe("buildHttpServer", () => {
  it("persists initialized sessions so tools/list works on the next request", async () => {
    ...
    try {
      ...
      // unrelated to the installed build â€” the same ambiguity that made the
      // npm-0.18.15-vs-repo-0.18.15 mixup (SCR-352) undiagnosable from a
      // support transcript.
      const initBody = await initResponse.text();
      expect(initBody).toContain('"name":"MEMORA-http"');
