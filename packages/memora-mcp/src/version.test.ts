 ...
 *
 * The incident: npm's `MEMORA-mcp@0.18.15` was built from a tree that
 ...
 * wearing one version string â€” and `release-mcp.yml`'s publish step SKIPS
...
/** Parse "1.2.3" â†’ [1,2,3]. Rejects anything that is not plain x.y.z. */
 ...
 * no dist/team-config.js and zero occurrences of MEMORA_TEAM_API_URL).
...
describe("package version identity", () => {
  ...
  it("is what src/version.ts exports â€” the single source of truth", () => {
    const { version } = readJson("package.json");
...
describe("no surface reports a hardcoded version", () => {
  ...
  it("no non-test source file holds a version literal in a `version:` field", () => {
    ...
    for (const file of fs.readdirSync(SRC)) {
      ...
      // `version: "1.2.3"` â€” a literal where PKG_VERSION belongs.
