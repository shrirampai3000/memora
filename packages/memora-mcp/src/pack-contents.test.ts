 ...
 * another `MEMORA-mcp@0.18.15` â€” a tarball published with no
 ...
 * matches `npm view MEMORA-mcp@0.18.15 dist.shasum`), and the snippet in
...
/** Verbatim file list of the published (broken) MEMORA-mcp@0.18.15. */
...
describe("pack-contents gate â€” file list", () => {
  it("rejects the tarball npm actually published as 0.18.15", () => {
    ...
    // so only the two modules that carry gateway support were absent.
    expect(missing).toEqual(["dist/team-config.js", "dist/version.js"]);
  });
  ...
  it("requires dist/team-config.js â€” the file whose absence caused the incident", () => {
    expect(gate.REQUIRED_PATHS).toContain("dist/team-config.js");
  });
});

describe("pack-contents gate â€” built-file contents", () => {
  /** A dist/ built from THIS tree: every marker present, no hardcoded base. */
  const goodDist: Record<string, string> = {
    "dist/team-config.js":
      'exports.HOSTED_TEAM_API = "https://github.com/shrirampai3000/memora/api/enterprise/v1";\n' +
      "const base = flagOverride || env.MEMORA_TEAM_API_URL || fromFile;\n" +
  ...
  it("fails when team-config.js is present but predates the env override", () => {
    const stale = {
      ...goodDist,
      "dist/team-config.js": 'exports.HOSTED_TEAM_API = "https://github.com/shrirampai3000/memora/api/enterprise/v1";\n',
    };
    const failures: string[] = gate.markerFailures(reader(stale));
    expect(failures).toHaveLength(2);
    expect(failures[0]).toContain("MEMORA_TEAM_API_URL");
  ...
  it("fails on the hardcoded base that 0.18.15 shipped", () => {
    ...
    const reverted = {
      ...goodDist,
      "dist/index.js": 'const TEAM_API = "https://github.com/shrirampai3000/memora/api/enterprise/v1";\n',
    };
