...
function homeWithEnterpriseJson(contents: Record<string, unknown>): string {
  ...
  tmpHomes.push(home);
  fs.mkdirSync(path.join(home, ".MEMORA"), { recursive: true });
  fs.writeFileSync(
    path.join(home, ".MEMORA", "enterprise.json"),
...
describe("discoverTeamApiBase", () => {
  ...
  it("env overrides file; flag overrides env", () => {
    ...
    const env = { MEMORA_TEAM_API_URL: "https://from-env.example" };
...
describe("discoverTeamToken", () => {
  it("prefers env, falls back to enterprise.json, requires sk_ent_ prefix", () => {
    ...
    expect(
      discoverTeamToken({ MEMORA_ENTERPRISE_TOKEN: "sk_ent_env" }, home)
    ).toBe("sk_ent_env");
