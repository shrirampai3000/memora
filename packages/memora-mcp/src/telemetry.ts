...
const DISABLE_ENV_VARS = [
  "MEMORA_DISABLE_TELEMETRY",
  "MEMORA_TELEMETRY_DISABLED",
  "MEMORA_MCP_SENTRY_DISABLED",
  "SENTRY_DISABLED",
];
...
type TelemetryContext = {
  ...
  // sent per machine for this key â€” dedupes noise from repeated respawns.
  throttleKey?: string;
  throttleMs?: number;
};
...
function sentryDsn(env: NodeJS.ProcessEnv = process.env): string {
  return env.MEMORA_MCP_SENTRY_DSN || env.SENTRY_DSN || DEFAULT_MCP_SENTRY_DSN;
}
...
export function scrubSensitiveValue(value: string): string {
  let scrubbed = value
    ...
    .replace(/(MEMORA_(?:LOCAL_API_KEY|API_KEY|ENTERPRISE_TOKEN)=)[^\s]+/g, "$1[redacted]");
...
export function initMcpTelemetry(context: TelemetryContext = {}): boolean {
  ...
  Sentry.init({
    dsn,
    release: `MEMORA-mcp@${MCP_VERSION}`,
  ...
  Sentry.setTag("component", "MEMORA-mcp");
  Sentry.setTag("package", "MEMORA-mcp");
...
export function captureMcpException(error: unknown, context: TelemetryContext = {}): void {
  ...
  Sentry.withScope((scope) => {
    scope.setTag("component", "MEMORA-mcp");
...
// server on every session/reconnect â€” so a single misconfigured machine can
...
export function throttleAllows(throttleKey: string, throttleMs: number): boolean {
  try {
    ...
    const marker = path.join(os.tmpdir(), `MEMORA-mcp-throttle-${safeKey}`);
    ...
    // On any FS error, fail open â€” better to send than to silently swallow.
    return true;
  }
}
