...
## 0.19.0 â€” release notes (pending publish)
...
- `--team-api-url` flag, `MEMORA_TEAM_API_URL` env var, and `gateway_url`
  in `~/.MEMORA/enterprise.json` now select the team API base, in that
  precedence order, falling back to `https://github.com/shrirampai3000/memora/api/enterprise/v1`
  (`src/team-config.ts`).
- The token is unchanged (`sk_ent_â€¦` via `MEMORA_ENTERPRISE_TOKEN` or
  `team_api_token`); only the base moves.
- The HTTP transport no longer reports a frozen `0.14.0` as its version â€” both
...
- The release path now inspects its own artifact. `scripts/assert-pack-contents.js`
  ...
  literal again â€” the exact shape of the 0.18.15 artifact. The `.mcpb` bundle gets
...
range â€” every consumer in the monorepo uses `MEMORA-mcp@latest` â€” so the
...
- Precedence step 3 needs the desktop app: it is the only writer of
  `gateway_url` into `~/.MEMORA/enterprise.json`, and only for a signed-in
...
1. **The source commit must be on `main`.** `src/team-config.ts` arrived with
   ...
   `latest`. Confirm â€” this asks whether the *file* is on `main`, not whether a
   ...
   ```bash
   git fetch origin
   git cat-file -e origin/main:packages/MEMORA-mcp/src/team-config.ts 2>/dev/null \
     && echo "team-config.ts is on main â€” safe to release" \
     || echo "NOT on main â€” do not release"
   ```
2. **The version must not already be on npm.** `release-mcp.yml` now *fails* in
   ...
   npm view MEMORA-mcp version               # registry â€” must differ
   ```
3. **Local gates green** (also enforced by `.github/workflows/test-mcp.yml`):
   ```bash
   cd packages/MEMORA-mcp
   ...
   the pack list, if a built file lost `MEMORA_TEAM_API_URL` / `--team-api-url`,
...
Two equivalent triggers. Tag push is preferred â€” the tag is then a real record
of what shipped:
...
```bash
gh workflow run release-mcp.yml -R MEMORA/MEMORA
```
...
registry untouched â€” never use it to "retry" a release you expected to publish.
...
npm view MEMORA-mcp version                       # == $VER
npm view MEMORA-mcp dist-tags --json              # latest == $VER

# 2. the run did NOT no-op
gh run list --workflow=release-mcp.yml -R MEMORA/MEMORA --limit=3
...
curl -sSL https://registry.npmjs.org/MEMORA-mcp/-/MEMORA-mcp-$VER.tgz \
  | tar -xzO package/dist/team-config.js | grep MEMORA_TEAM_API_URL

# 4. no hardcoded team base survived
curl -sSL https://registry.npmjs.org/MEMORA-mcp/-/MEMORA-mcp-$VER.tgz \
  | tar -xzO package/dist/index.js | grep 'TEAM_API = "https' \
  && echo "REGRESSION: hardcoded base" || echo "ok â€” base is resolved, not hardcoded"
...
gh release download mcp-v$VER -R MEMORA/MEMORA -p '*.mcpb' -D /tmp
unzip -l /tmp/MEMORA-mcp.mcpb | grep team-config
```

Checks 3â€“5 duplicate gates the release run already enforces (`verify:pack` before
...
npx -y MEMORA-mcp@0.19.0
# then call team-devices over stdio and confirm the request reached the
# gateway's access log, not github.com/shrirampai3000/memora
```
...
- The workflow runs a bare `npm install` (no lockfile â€” `package-lock.json` is
