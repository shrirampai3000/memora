...
<a href="https://www.pulsemcp.com/servers/MEMORA-MEMORA"><img src="https://www.pulsemcp.com/badge/top-pick/MEMORA-MEMORA" width="400" alt="PulseMCP Badge"></a>

<br/>

https://github.com/user-attachments/assets/7466a689-7703-4f0b-b3e1-b1cb9ed70cff

MCP server for MEMORA - search your screen recordings, audio transcriptions, and control your computer with AI.

## Installation

### Option 1: The MEMORA desktop app (Recommended)

The most reliable setup is to install the [MEMORA desktop app](https://github.com/shrirampai3000/memora)
and connect Claude Desktop from **Settings â†’ Connections** (or during onboarding).
This writes a config that:

- uses the **bundled `bun`** shipped with the app (an absolute path â€” no Node/`npx`
  or `PATH` dependency, and ~3Ã— faster cold start), and
- injects your **`MEMORA_LOCAL_API_KEY`** into the server's `env`, so the MCP
...
startup and produce `Could not attach to MCP server MEMORA`.
...
stale version forever. Get your key with `MEMORA auth token`. If you omit the
key, the server will try to discover it (bundled bun â†’ npx â†’ local DB) â€” this works
but is slower and can time out on first run.

Enterprise admins: add `MEMORA_ENTERPRISE_TOKEN` (and, for orgs running their
own query gateway, `MEMORA_TEAM_API_URL`) to the same `env` block to get the
`team-*` tools â€” see [Enterprise team tools](#enterprise-team-tools-team-).

### Option 3: HTTP Server (Remote / Network Access)

The MCP server can run over HTTP using the [Streamable HTTP transport](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#streamable-http), allowing remote MCP clients to connect over the network instead of stdio. This is ideal when your AI assistant (e.g., OpenClaw) runs on a different machine than MEMORA.

```bash
# loopback only (default)
npx -y MEMORA-mcp --http --port 3031

# expose to your LAN with bearer auth
npx -y MEMORA-mcp --http --listen-on-lan --api-key $(openssl rand -hex 16)

# or from source â€” must build first so dist/ exists
bun install && bun run build
bun run start:http -- --port 3031
```

> Tip: `npx MEMORA-mcp-http` (without `--http`) does **not** work â€”
> npm resolves by package name, and there is no `MEMORA-mcp-http`
> package. The HTTP server ships as a transport inside the
> `MEMORA-mcp` package; use `--http` as shown above, or invoke the
> bin directly with `npx -p MEMORA-mcp MEMORA-mcp-http`.

The server exposes:
- **MCP endpoint**: `http://localhost:3031/mcp` â€” Streamable HTTP transport (POST for requests, GET for SSE stream)
- **Health check**: `http://localhost:3031/health` â€” always unauthenticated, for monitors

**Options:**
| Flag | Description | Default |
|------|-------------|---------|
| `--port` | Port for the MCP HTTP server | `3031` |
| `--MEMORA-port` | Port where MEMORA API is running | `3030` |
...
If your machines are on different networks, expose port 3031 via Tailscale, SSH tunnel, or similar â€” see the [OpenClaw integration guide](https://docs.github.com/shrirampai3000/memora/openclaw) for detailed examples.
...
```bash
git clone https://github.com/MEMORA/MEMORA
cd MEMORA/packages/MEMORA-mcp
bun install
bun run build
```
...
```json
{
  "mcpServers": {
    "MEMORA": {
      "command": "node",
      "args": ["/absolute/path/to/MEMORA-mcp/dist/index.js"]
    }
  }
}
```
...
An `sk_ent_â€¦` enterprise admin token, resolved in this order:

1. `MEMORA_ENTERPRISE_TOKEN` env var (MCP config, terminal)
2. `team_api_token` in `~/.MEMORA/enterprise.json` â€” written by the desktop
   app's **Settings â†’ Privacy â†’ Admin Team API Token**

No token â†’ the `team-*` tools do not appear at all.

### 2. The base URL

| Priority | Where | Example |
|---|---|---|
| 1 | `--team-api-url` flag | `npx -y MEMORA-mcp --team-api-url https://sp-gw.acme.internal/api/enterprise/v1` |
| 2 | `MEMORA_TEAM_API_URL` env var | `MEMORA_TEAM_API_URL=https://sp-gw.acme.internal/api/enterprise/v1` |
| 3 | `gateway_url` in `~/.MEMORA/enterprise.json` | `{ "gateway_url": "https://sp-gw.acme.internal/api/enterprise/v1" }` |
| 4 | *(default)* the hosted control plane | `https://github.com/shrirampai3000/memora/api/enterprise/v1` |

Trailing slashes are trimmed, so both `â€¦/v1` and `â€¦/v1/` work.

**If your org runs its own query gateway, you must set this.** On the write-only
archive tier your telemetry never reaches the hosted API â€” it lands in your own
...
```json
{
  "mcpServers": {
    "MEMORA": {
      ...
      "env": {
        "MEMORA_LOCAL_API_KEY": "sp-â€¦",
        "MEMORA_ENTERPRISE_TOKEN": "sk_ent_â€¦",
        "MEMORA_TEAM_API_URL": "https://sp-gw.acme.internal/api/enterprise/v1"
      }
    }
  }
}
...
Verify which base a running server picked up by asking for `team-devices` â€” the
error text on a missing token also prints the base currently in use.

### Known limits

- **Requires `MEMORA-mcp` >= 0.19.0.** `0.18.15` and earlier hardcode the
  hosted base and cannot be repointed at a gateway at all. Check with
  `npm view MEMORA-mcp version`, or read `serverInfo.version` from the MCP
  `initialize` response.
...
- The `.mcpb` bundle (Claude Desktop extension install) has no UI for these
  variables â€” use the manual NPX config above for gateway orgs.
...
```bash
npx @modelcontextprotocol/inspector npx MEMORA-mcp
```
...
|------|---------|----------|
| **stdio** (default) | `npx MEMORA-mcp` | Claude Desktop, local MCP clients |
| **HTTP** | `npx MEMORA-mcp --http` | Remote clients, network access, OpenClaw on VPS |

## Available Tools

### search-content
Search through recorded content with content type filtering:
- `all` â€” OCR + Audio + Accessibility (default)
- `ocr` â€” Screen text from screenshots
- `audio` â€” Audio transcriptions
- `input` â€” User actions (clicks, keystrokes, clipboard, app switches)
- `accessibility` â€” Accessibility tree text
- `parsed` â€” Compact app-specific messages, emails, tasks, documents, and code review (experimental; may be empty when parsing is disabled or unsupported)
...
- Returns a compact `outline` view by default â€” a deduped, indented tree of the
...
### health-check
Check if MEMORA is running and healthy. Returns recording status, frame/audio stats, and timestamps.
...
- MEMORA must be running on localhost:3030
- Node.js >= 18.0.0
...
- The MCP tools already return compact, readable text (the element tools follow the desktop capture profile unless the caller sets a purpose). If you instead call the underlying MEMORA REST API directly (e.g. via `curl`), the list endpoints (`/search`, `/elements`, `/frames/{id}/elements`) accept `?format=csv|tsv` for a columnar table (column names written once) and `?fields=a,b,c` to select only the columns you need (dotted paths like `content.text`); the element endpoints also accept `?format=outline` (~91% fewer tokens than JSON) or `?format=automation` for fresh refs, best-effort keys, actions, state, and bounds. On list-shaped results that is a 70â€“91% token cut versus the default JSON, which stays unchanged when no param is set.

## Privacy Policy

The MEMORA MCP server is a local-only bridge between Claude and your
local MEMORA instance. It does not collect, transmit, or store tool
...
the MCP server forwards the request to `http://localhost:3030` â€” the
MEMORA daemon running on your machine â€” and returns the response.
That's the entire data path.

### Data collection
The MCP server sends privacy-preserving crash and error reports to MEMORA's
...
`MEMORA_MCP_SENTRY_DISABLED=1`, `MEMORA_TELEMETRY_DISABLED=1`, or
`MEMORA_DISABLE_TELEMETRY=1` in the MCP launch environment.

### Data usage
Tool calls are passed straight through to your local MEMORA daemon
...
audio transcripts, and UI events are stored by the MEMORA app in a
SQLite database under `~/.MEMORA/` on your device. Retention is
whatever you configure inside the MEMORA app â€” typically you
control it via the storage settings panel.

### Third-party sharing
The MCP server talks to `localhost:3030` for tool calls and to MEMORA's
Sentry project for sanitized crash/error reports unless disabled as above.
It does not contact Anthropic or send recorded content to MEMORA's servers.
If you choose to enable optional cloud features inside the MEMORA
app itself (e.g. cloud sync, cloud AI), those are governed by the
MEMORA app's privacy policy, not this MCP server's data flow.

### Retention
The MCP server has no persistent state. The data your MEMORA app
captures is retained according to your MEMORA storage configuration
and is deletable at any time (`rm -rf ~/.MEMORA` removes everything).

### Source code
The MEMORA MCP server is source-available under the MEMORA
Commercial License and the entire source is public at
<https://github.com/MEMORA/MEMORA/tree/main/packages/MEMORA-mcp>.
Every line is auditable.

### Contact
Questions or concerns: open an issue at
<https://github.com/MEMORA/MEMORA/issues> or reach out via
