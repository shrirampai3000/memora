 ...
 *
 * The incident this exists to prevent: `MEMORA-mcp@0.18.15` was published
 ...
 * alone and then failed with HTTP 401 against every customer query gateway â€”
 ...
 * differently:
 *   - REQUIRED_PATHS  â€” the file is absent from the tarball (what 0.18.15 did).
 *     Note how load-bearing that is here: `.gitignore` lists `dist/`, and the
 *     only reason the build output ships at all is that npm â‰¥ 8
 *     (npm-packlist â‰¥ 5) dropped the `.gitignore`-as-`.npmignore` fallback â€”
 ...
 *     disappear â€” precisely the two files 0.18.15 was missing â€” with no error
 *     anywhere. This list is what turns that into a red build.
 *   - REQUIRED_MARKERS / FORBIDDEN_MARKERS â€” the file is present but was built
 ...
 *
 * Usage (both `test-mcp.yml` and `release-mcp.yml` call this â€” the release job
...
const REQUIRED_PATHS = [
  ...
  "README.md",
  "dist/cli.js", // bin: MEMORA-mcp
  "dist/http-server.js", // bin: MEMORA-mcp-http
  "dist/index.js", // main â€” the stdio server
...
const REQUIRED_MARKERS = [
  {
    ...
    marker: "MEMORA_TEAM_API_URL",
    why: "the env-var override is how gateway orgs repoint the team-* tools",
  },
  {
    file: "dist/team-config.js",
    marker: "gateway_url",
    why: "the ~/.MEMORA/enterprise.json fallback in the precedence ladder",
  },
  {
    file: "dist/index.js",
    marker: "--team-api-url",
...
const FORBIDDEN_MARKERS = [
  {
    ...
    why: "the team API base must be resolved at runtime (discoverTeamApiBase), never a literal â€” a hardcoded base is exactly what 401'd for gateway orgs",
  },
];
...
function markerFailures(read) {
  ...
  for (const { file, marker, why } of REQUIRED_MARKERS) {
    ...
    if (text === null) {
      failures.push(`${file}: unreadable â€” cannot verify "${marker}" (${why})`);
    } else if (!text.includes(marker)) {
      failures.push(`${file}: missing "${marker}" â€” ${why}`);
    }
  }
  for (const { file, pattern, why } of FORBIDDEN_MARKERS) {
    const text = read(file);
    if (text !== null && pattern.test(text)) {
      failures.push(`${file}: matches forbidden ${pattern} â€” ${why}`);
    }
  }
  return failures;
}
...
function main() {
  ...
  if (missing.length) {
    console.error(
      `missing from the npm tarball: ${missing.join(", ")}\n` +
        `  packed ${files.length} files. Did the build run? Or did an .npmignore appear â€” ` +
  ...
  if (missing.length || failures.length) {
    console.error(
      `\nREFUSING MEMORA-mcp@${version}: this tarball is not a gateway-capable build. ` +
        `Run \`npm run build\` and re-check; see packages/MEMORA-mcp/RELEASE.md.`,
    );
    process.exit(1);
  }

  console.log(
    `pack contents OK â€” MEMORA-mcp@${version}, ${files.length} files, ` +
