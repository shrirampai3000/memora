 ...
 *
 * Usage:
 *   bun run local/cli.ts setup        â€” install tart + create VM
 *   bun run local/cli.ts test         â€” run all tests (setup if needed)
 *   bun run local/cli.ts test --target cli   â€” CLI tests only
 *   bun run local/cli.ts test --target e2e   â€” API e2e tests only
 *   bun run local/cli.ts ssh          â€” open SSH session to VM
 *   bun run local/cli.ts status       â€” show VM status
 *   bun run local/cli.ts teardown     â€” stop + delete VM + remove tart
 */
...
async function main() {
  switch (command) {
    ...
    default: {
      console.log(`MEMORA e2e local â€” macOS VM testing via Tart
