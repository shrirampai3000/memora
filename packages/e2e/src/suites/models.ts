...
/**
 * Model loading test suite â€” verifies transcription engines start correctly.
 *
...
// â”€â”€ Engine test definitions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export async function runModelTests(
  ...
  for (const t of engineTests) {
    if (t.skipReason) {
      if (!opts?.quiet)
        console.log(`  âŠ˜ ${t.name} (skipped: ${t.skipReason})`);
      continue;
    }
    ...
    try {
      ...
      await exec("pkill -f 'MEMORA record' 2>/dev/null || true");
      ...
      const cmd = [
        ...
        `--port ${port}`,
        t.flags ?? "",
        `> /tmp/MEMORA-model-test.log 2>&1 & echo $!`,
      ].join(" ");
      ...
      const { stdout: logs } = await exec(
        "grep -i 'error\\|panic\\|fatal' /tmp/MEMORA-model-test.log | head -5",
        { timeout: 5_000 }
      );
