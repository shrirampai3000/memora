 ...
 * Changing a `STORAGE_KEY_*` requires a migration path â€” avoid it.
 */

export const DEFAULT_BASE_URL = "http://127.0.0.1:3030";

/** chrome.storage.local keys. Namespaced to avoid collisions. */
export const STORAGE_KEY_TOKEN = "MEMORA_token";
export const STORAGE_KEY_BASE_URL = "MEMORA_base_url";
 ...
 * same route family as every other integration (Slack, Notion, Google Calendar, â€¦).
 */
const BROWSER_BASE_PATH = "/connections/browser";

/**
 * Build the WebSocket URL for the browser bridge.
 * Handles http â†’ ws / https â†’ wss rewriting and appends the token as a query
...
/** HTTP endpoint used to probe server liveness â€” exempt from auth. */
 ...
 * Returns 401 when api_auth is enabled and no token is provided â€” that's the
