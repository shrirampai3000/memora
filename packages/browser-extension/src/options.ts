 ...
 * own â€” this page does not need to message the worker directly.
 */
...
const MEMORA_FOCUS_URL = "http://127.0.0.1:11435/focus";
...
function setPairCode(code: string | null): void {
  ...
  details.open = false;
  el.textContent = code
    ? `match code: ${code}. approve only if the same code appears in MEMORA.`
    : "";
}
...
function setConnectedUi(connected: boolean): void {
  ...
  title.textContent = connected ? "MEMORA connected" : "connect MEMORA";
  help.textContent = connected
    ? "you can close this tab. agents can now use your browser when needed."
    : "click connect, then approve the request in the MEMORA desktop app. no API token copy/paste needed.";
}
...
async function openMEMORAForApproval(): Promise<boolean> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 2_000);
  try {
    const res = await fetch(MEMORA_FOCUS_URL, {
      method: "POST",
 ...
 *   1. /health â€” always exempt from auth. If this fails, the server is down.
 *   2. /browser/status â€” requires auth when api_auth=true. 401 means the
...
async function probeConnection(
  ...
  try {
    ...
    return {
      status: "server_down",
      message: `can't reach MEMORA at ${baseUrl} â€” is the app running?`,
    };
  }
  ...
  try {
    ...
    if (auth.status === 401 || auth.status === 403) {
      return {
        ...
        message: token
          ? "token was rejected â€” copy a fresh one from MEMORA Settings"
          : "this server requires a token â€” paste one above",
      };
    }
...
async function onConnectClick(): Promise<void> {
  ...
  setPairCode(null);
  const baseUrl = $<HTMLInputElement>("baseUrl").value.trim() || DEFAULT_BASE_URL;
  setStatus("saving", "checking for MEMORAâ€¦");

  const liveness = await probeConnection("", baseUrl);
  if (liveness.status === "server_down") {
    setStatus("server_down", `MEMORA is not running at ${baseUrl}`);
  ...
  try {
    setStatus("pairing", "opening approval request in MEMORAâ€¦");
    const pair = await startPairing(baseUrl);
    setPairCode(pair.code);
    const focused = await openMEMORAForApproval();
    setStatus(
      "pairing",
      focused
        ? "MEMORA should come to front. click Allow there"
        : "approve in MEMORA. if it did not come forward, open the app manually"
    );
    ...
    if (approval.status === "approved") {
      ...
      setStatus(status, status === "ok" ? "connected to MEMORA" : message);
      setConnectedUi(status === "ok");
      return;
    }

    if (approval.status === "denied") {
      setStatus("denied", "connection denied in MEMORA");
      return;
    }

    setStatus("error", "approval expired â€” try connecting again");
  } catch (e: any) {
    setPairCode(null);
...
async function onSaveClick(): Promise<void> {
  ...
  await saveSettings(token, baseUrl);
  const { status, message } = await probeConnection(token, baseUrl);
  setStatus(status, status === "ok" ? `settings saved Â· ${message}` : message);
  setConnectedUi(status === "ok");
}

async function onTestClick(): Promise<void> {
  setStatus("saving", "testingâ€¦");
  const { token, baseUrl } = getFormValues();
  const { status, message } = await probeConnection(token, baseUrl);
...
async function recheckSavedConnection(): Promise<void> {
  ...
  setStatus(
    status,
    status === "auth_required"
      ? "MEMORA auth changed â€” connect again"
      : message
  );
}
...
async function init(): Promise<void> {
  ...
  if (token || baseUrl !== DEFAULT_BASE_URL) {
    ...
    setStatus("idle", "click connect, then approve in the MEMORA app");
