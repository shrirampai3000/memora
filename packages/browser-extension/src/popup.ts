...
const SESSION_PAIR_KEY = "MEMORA_pending_pair";
...
async function tryFocusMEMORA(): Promise<void> {
  try {
    ...
    clearTimeout(t);
  } catch { /* app may not be listening â€” ignore */ }
}
...
// ---------------------------------------------------------------------------
// Pairing flow (runs entirely in the popup â€” no tab switch needed)
// ---------------------------------------------------------------------------
...
async function runPairingFlow(baseUrl: string, existingPair?: PairState): Promise<void> {
  ...
  try {
    ...
    if (existingPair) {
      ...
      // Persist so polling can resume if popup closes when user switches to MEMORA.
    ...
    // Try to bring MEMORA to front. The popup will close as soon as the
    // OS focuses another window â€” that's fine, session storage keeps state.
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        continue; // transient network blip â€” keep polling
      }
...
// ---------------------------------------------------------------------------
// Open MEMORA flow (server_down state)
// ---------------------------------------------------------------------------

async function tryOpenMEMORA(baseUrl: string): Promise<void> {
...
async function init(): Promise<void> {
  ...
  if (pendingPair) {
    // Resume polling â€” show pairing UI immediately without re-probing.
