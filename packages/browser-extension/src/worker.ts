...
/**
 * Service worker â€” maintains a single WebSocket to MEMORA's /browser/ws
...
/** How long to stay silent after alerting â€” avoids notification spam. */
const ALERT_COOLDOWN_MS = 10 * 60_000;
/** Heartbeat â€” must be < server READ_IDLE_TIMEOUT (50s). */
...
/** Consecutive closes with no successful OPEN â€” smells like auth rejection. */
...
/** Last time we received a frame from the server (any frame: pong, eval, â€¦). */
let lastFrameAt = 0;
/** Heartbeat interval handle â€” cleared on disconnect. */
...
function notifyOnce(title: string, message: string): void {
  ...
  try {
    ...
    // notifications permission may be missing on some forks â€” fail silent.
  }
}
...
async function connect(): Promise<void> {
  // Honor the backoff timer â€” alarms and tab events shouldn't bypass it.
  ...
  socket.onclose = () => {
    ...
    if (!openedThisAttempt) {
      ...
      if (closeWithoutOpen >= AUTH_FAIL_THRESHOLD) {
        setBadge("!", "#dc2626");
        notifyOnce(
          "MEMORA extension needs a token",
          "Open the extension options to paste your MEMORA API token."
        );
      }
    }
    scheduleReconnect();
...
function forceReconnect(): void {
  ...
  // half-dead socket) â€” blow away state so the next attempt starts fresh
...
// ---------------------------------------------------------------------------
// Heartbeat â€” detects half-dead WS sockets where onclose never fires.
//
...
// would show "connected" while the server saw a stale dead socket â€” exactly
...
function startHeartbeat(): void {
  ...
  heartbeatTimer = setInterval(() => {
    ...
    // App-level ping â€” the server replies via its own ping frame on a 20s
    ...
    if (Date.now() - lastFrameAt > HEARTBEAT_DEAD_MS) {
      console.warn("[MEMORA] no server traffic for 50s â€” reconnecting");
      forceReconnect();
      return;
    }
    send({ type: "ping" });
 ...
 * picking one would surface as an opaque "cannot execute scripts on â€¦" error.
 */
...
async function findTab(urlPattern?: string): Promise<number> {
  ...
  // Prefer the focused active tab when it's eligible â€” that's almost always
  ...
  throw new Error(
    "no eligible tab found â€” open a regular web page (not chrome://, chrome-extension://, or the chrome web store)",
  );
}
 ...
 *  is 50s. With a 1-minute alarm the SW spends T+30..T+60 dormant â€” the
...
chrome.alarms.create("MEMORA_keepalive", { periodInMinutes: 0.5 });
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name !== "MEMORA_keepalive") return;
