...
function setPairCode(code) {
  ...
  details.hidden = !code;
  details.open = false;
  el.textContent = code ? `match code: ${code}. approve only if the same code appears in MEMORA.` : "";
}
function setConnectedUi(connected) {
  ...
  help.textContent = connected ? "you can close this tab. agents can now use your browser when needed." : "click connect, then approve the request in the MEMORA desktop app. no API token copy/paste needed.";
}
...
async function openMEMORAForApproval() {
  ...
  try {
    ...
    if (auth.status === 401 || auth.status === 403) {
      return {
        ...
        message: token ? "token was rejected â€” copy a fresh one from MEMORA Settings" : "this server requires a token â€” paste one above"
      };
    }
...
async function onConnectClick() {
  ...
  try {
    ...
    setStatus("pairing", focused ? "MEMORA should come to front. click Allow there" : "approve in MEMORA. if it did not come forward, open the app manually");
...
async function recheckSavedConnection() {
  ...
  setStatus(status, status === "auth_required" ? "MEMORA auth changed â€” connect again" : message);
}
async function init() {
  const { token, baseUrl } = await loadSettings();
  $("token").value = token;
