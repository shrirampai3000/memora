  ...
  socket.onclose = () => {
    ...
    if (!openedThisAttempt) {
      ...
      if (closeWithoutOpen >= AUTH_FAIL_THRESHOLD) {
        setBadge("!", "#dc2626");
        notifyOnce("MEMORA extension needs a token", "Open the extension options to paste your MEMORA API token.");
      }
    }
    scheduleReconnect();
  };
...
async function findTab(urlPattern) {
  ...
  if (eligible?.id != null)
    return eligible.id;
  throw new Error("no eligible tab found â€” open a regular web page (not chrome://, chrome-extension://, or the chrome web store)");
}
...
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name !== "MEMORA_keepalive")
    return;
  if (socket?.readyState === WebSocket.OPEN) {
    send({ type: "ping" });
  } else {
