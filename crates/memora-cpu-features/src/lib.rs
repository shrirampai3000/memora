...
//! else is baseline. Gate any AVX2-compiled code path on `has_avx2()` â€”
//! executing it on a CPU without AVX2 dies with STATUS_ILLEGAL_INSTRUCTION
//! (0xc000001d), which no panic hook can catch.
//!
//! This is a dependency-free leaf crate because both `memora-core` and
//! `memora-config` need it and config cannot depend on core (package
//! cycle via memora-secrets). `memora-core` re-exports it as
//! `memora_core::cpu_features`, which is the path most callers use.
...
///
/// `is_x86_feature_detected!` uses raw CPUID+XGETBV â€” works on every Windows
