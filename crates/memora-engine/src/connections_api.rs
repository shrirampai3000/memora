...
use memora_connect::connections::{bee, build_default_client, telegram, ConnectionManager};
use memora_connect::oauth::{self as oauth_store, OAuthCallbackResult, PENDING_OAUTH};
use memora_connect::whatsapp::WhatsAppGateway;
...
use memora_connect::connections::browser::{BrowserRegistry, BrowserSummary, EvalError};
...
pub struct SlackHistoryQuery {
    /// Conversation id (channel `Câ€¦`, DM `Dâ€¦`, group `Gâ€¦`).
    pub channel: String,
    #[serde(default)]
    pub limit: Option<u32>,
    #[serde(default)]
...
/// `apps/memora-app-tauri/components/settings/connections-section.tsx`.
///
...
/// â€” not the connector secret store â€” so the connector's own `connected` flag
...
async fn mcp_oauth_connected_ids(
    MEMORA_dir: &std::path::Path,
    secret_store: Option<Arc<SecretStore>>,
) -> std::collections::HashSet<String> {
    use memora_connect::mcp_servers::McpServerStore;
    let mut connected = std::collections::HashSet::new();
    let store = McpServerStore::new(MEMORA_dir.to_path_buf(), secret_store);
...
/// GET /connections â€” list all integrations with connection status.
async fn list_connections(State(state): State<ConnectionsState>) -> Json<Value> {
    ...
    let wa_connected = matches!(
        wa_status,
        memora_connect::whatsapp::WhatsAppStatus::Connected { .. }
    );
    let wa_desc = if let Some(port) = wa_port {
        format!(
            "WhatsApp messaging gateway on port {}. Endpoints: \
            GET /contacts â€” list saved contacts (may be empty, ask user for phone number if needed). \
            GET /chats â€” list recent chats with last message. \
            GET /messages?phone=+PHONE&limit=50 â€” read recent messages from a chat. \
            POST /send {{\"to\":\"+PHONE\",\"text\":\"MSG\"}} â€” send a message. \
            GET /status â€” connection info. \
    ...
    // store â€” so the base entry above reports connected=false even after the
    ...
    let mcp_connected_ids =
        mcp_oauth_connected_ids(&state.MEMORA_dir, state.secret_store.clone()).await;
    ...
    if let Some(arr) = data.as_array_mut() {
        ...
        // Native calendar â€” macOS only (EventKit). Windows/Linux have no equivalent.
        #[cfg(target_os = "macos")]
        {
        ...
        let (ics_feed_count, ics_enabled_count, ics_error) =
            match memora_connect::ics_calendar::load_ics_calendar_settings_from_store(
                &state.MEMORA_dir,
            ) {
                Ok(settings) => {
        ...
        // Browsers â€” every kind of browser the agent can drive (user's
...
/// GET /connections/:id â€” get connection status and non-secret settings.
async fn get_connection(
    ...
    // Browsers live in the registry, not in the integration table â€” so a
    ...
    let (status, body) = match mgr.get_credentials(&id).await {
        Ok(Some(creds)) => {
            let safe = mgr
                .find_def(&id)
                .map(|def| memora_connect::connections::safe_connection_config(def, &creds))
                .unwrap_or_default();
            (
                StatusCode::OK,
...
/// PUT/POST /connections/:id â€” save credentials.
...
/// DELETE /connections/:id â€” disconnect.
...
/// POST /connections/:id/test â€” verify credentials without saving.
...
/// GET /connections/:id/instances â€” list all saved instances for an integration.
async fn list_instances(
    State(state): State<ConnectionsState>,
    Path(id): Path<String>,
) -> (StatusCode, Json<Value>) {
    let mgr = state.cm.lock().await;
    let is_oauth = memora_connect::connections::all_integrations()
        .iter()
        .find(|i| i.def().id == id)
        ...
        Ok(instances) => {
            if let Some(def) = mgr.find_def(&id) {
                for (inst, conn) in instances {
                    let safe = memora_connect::connections::safe_connection_config(
                        def,
                        &conn.credentials,
                    );
...
/// PUT /connections/:id/instances/:instance â€” save credentials for a named instance.
...
/// DELETE /connections/:id/instances/:instance â€” disconnect a named instance.
...
/// POST /connections/whatsapp/pair â€” start QR pairing.
...
/// GET /connections/whatsapp/status â€” get current pairing/connection status.
...
/// POST /connections/whatsapp/disconnect â€” stop gateway and wipe session.
...
// ---------------------------------------------------------------------------
// Calendar routes â€” exposes native OS calendar (EventKit / WinRT)
// ---------------------------------------------------------------------------
...
enum NativeCalendarError {
    ...
    /// Calendar exists but MEMORA lacks permission (macOS TCC).
...
fn native_calendar_error_response(e: NativeCalendarError) -> (StatusCode, Json<Value>) {
    match e {
        // Nothing to fix and nothing failed â€” same convention as the ICS
...
/// GET /connections/calendar/events â€” fetch native OS calendar events.
...
/// GET /connections/calendar/status â€” check native calendar access.
...
fn get_native_calendar_events(
    ...
    use memora_connect::calendar::{CalendarError, MEMORACalendar};
    let cal = MEMORACalendar::new();
    let events = cal
        .get_events(hours_back, hours_ahead)
        .map_err(|e| match e {
            CalendarError::AuthorizationDenied => NativeCalendarError::AuthRequired(
                "calendar access not granted â€” allow MEMORA under System Settings > \
...
fn get_native_calendar_events(
    ...
    use memora_connect::calendar_windows::MEMORACalendar;
    ...
    let cal = MEMORACalendar::new().map_err(NativeCalendarError::StoreUnavailable)?;
    let events = cal
        .get_events(hours_back, hours_ahead)
...
fn get_native_calendar_events(
    ...
    // state of the world, not a 500 â€” the app polls this route every 60s
...
fn is_native_calendar_available() -> bool {
    use memora_connect::calendar::MEMORACalendar;
    // Try fetching a tiny window â€” if it succeeds, we have access
    let cal = MEMORACalendar::new();
...
#[cfg(target_os = "windows")]
fn is_native_calendar_available() -> bool {
    memora_connect::calendar_windows::MEMORACalendar::is_available()
}
...
fn ics_feed_summaries(
    settings: &memora_connect::ics_calendar::IcsCalendarSettings,
) -> Vec<Value> {
    settings
        .entries
        .iter()
...
/// GET /connections/ics-calendar/status â€” check subscribed ICS feeds.
...
/// GET /connections/ics-calendar/events â€” fetch subscribed ICS feed events.
async fn ics_calendar_events(
    State(state): State<ConnectionsState>,
    Query(params): Query<CalendarEventsQuery>,
) -> (StatusCode, Json<Value>) {
    let settings = match memora_connect::ics_calendar::load_ics_calendar_settings_from_store(
        &state.MEMORA_dir,
    ) {
    ...
    let events = memora_connect::ics_calendar::fetch_ics_calendar_events(
        &client,
        &enabled,
...
/// a machine-readable body â€” `reason: "auth_required"` mirrors the native
/// calendar route â€” so pollers can back off instead of retrying a state that
...
/// GET /connections/google-calendar/status â€” check connection + email.
async fn gcal_status(
    ...
    // and reports false even though every account is healthy â€” mirror the
...
/// GET /connections/google-calendar/events â€” fetch Google Calendar events.
...
async fn gcal_events_inner(
    ...
    // all pass no instance â€” refusing turned "user connected a 2nd Google
            ...
            for inst in &connected {
                ...
                match events {
                    ...
                    Err(e) => {
                        ...
                        // the healthy ones â€” keep going, report only if all fail.
...
/// DELETE /connections/google-calendar/disconnect â€” remove stored tokens.
...
// ---------------------------------------------------------------------------
// IMAP inbox routes â€” IMAP is not HTTP, so the generic credential proxy
...
/// GET /connections/imap/messages â€” list recent messages (headers only).
async fn imap_messages(
    ...
    match memora_connect::connections::imap::list_messages(
        &creds,
        mailbox,
        limit,
        q.query.as_deref(),
...
/// GET /connections/imap/messages/:uid â€” fetch one full message.
async fn imap_message(
    ...
    let mailbox = q.mailbox.as_deref().unwrap_or("INBOX");
    match memora_connect::connections::imap::get_message(&creds, mailbox, uid).await {
...
/// GET /connections/imap/mailboxes â€” list folder names.
async fn imap_mailboxes(State(state): State<ConnectionsState>) -> (StatusCode, Json<Value>) {
    let creds = match imap_creds(&state).await {
        Ok(c) => c,
        Err(e) => return e,
    };
    match memora_connect::connections::imap::list_mailboxes(&creds).await {
...
pub struct OAuthCallbackQuery {
    ...
    // Optional human-readable error detail (RFC 6749 Â§4.1.2.1).
...
/// GET /connections/oauth/callback â€” receives the provider redirect.
///
/// The `state` parameter is used to look up the waiting `oauth_connect` Tauri command
/// via the `PENDING_OAUTH` channel map, then delivers the outcome â€” success with the
/// authorization `code`, or the provider's error â€” as a typed `OAuthCallbackResult`.
...
async fn oauth_callback(Query(params): Query<OAuthCallbackQuery>) -> (StatusCode, Html<String>) {
    if let Some(err) = params.error {
        ...
        match pending {
            Some(pending) => {
                tracing::warn!(
                    "oauth callback: provider returned error '{}' for {} â€” resolving pending flow",
                    err,
            ...
            None => tracing::warn!(
                "oauth callback: provider returned error '{}' with {} state â€” no pending flow to resolve",
                err,
        ...
        return oauth_callback_page(
            StatusCode::BAD_REQUEST,
            "Connection failed",
            "MEMORA could not finish the OAuth flow.",
            &err,
        );
    }
    ...
    let (code, state) = match (params.code, params.state) {
        ...
        _ => {
            ...
            return oauth_callback_page(
                StatusCode::BAD_REQUEST,
                "Invalid callback",
                "MEMORA could not verify this authorization response.",
        ...
        Some(pending) => {
            ...
            if delivered {
                ...
                oauth_callback_page(
                    StatusCode::OK,
                    "Connected",
                    "MEMORA can now use this connection.",
                    "You can close this tab and return to MEMORA.",
                )
            } else {
                ...
                oauth_callback_page(
                    StatusCode::BAD_REQUEST,
                    "Sign-in expired",
                    "This sign-in took a while, so MEMORA stopped waiting.",
                    "Open MEMORA and click connect again â€” a fresh sign-in stays valid for 10 minutes.",
                )
            }
        }
        None => {
            tracing::warn!("oauth callback: unknown or stale state â€” no pending flow");
            oauth_callback_page(
                StatusCode::BAD_REQUEST,
                "Link already used",
                "This authorization link was already used or has expired.",
                "If MEMORA already shows the connection, you can close this tab. Otherwise click connect in the app to try again.",
            )
        }
    }
}
...
// ---------------------------------------------------------------------------
// Credential proxy â€” forward requests to third-party APIs with auth injected
// ---------------------------------------------------------------------------

/// Resolved authentication â€” extracted from credentials before the request.
enum ResolvedAuth {
...
/// placeholder (tokens and lifecycle metadata). Everything else â€” `realmId`,
/// `email`, `workspace_name`, etc. â€” is fair game.
...
fn resolve_base_url(
    ...
    while let Some(open) = rest.find('{') {
        ...
        match (value, default) {
            ...
            (None, None) => {
                return Err(format!(
                    "unresolved placeholder {{{}}} in base_url â€” credential field missing",
                    name
                ));
            }
        }
...
fn resolve_auth(
    proxy_auth: &memora_connect::connections::ProxyAuth,
    creds: Option<&Map<String, Value>>,
    oauth_token: Option<&str>,
    oauth_extras: Option<&Value>,
) -> ResolvedAuth {
    use memora_connect::connections::ProxyAuth;
    match proxy_auth {
...
/// - Only safe HTTP methods allowed (GET, POST, PUT, PATCH) â€” DELETE blocked by default
...
async fn connection_proxy(
    ...
    // Block destructive methods â€” pipes should not delete external resources
    if method == axum::http::Method::DELETE {
        tracing::warn!(
            "proxy: blocked DELETE request to {}/{} â€” destructive methods not allowed",
            id,
            api_path
        );
        return (
            StatusCode::METHOD_NOT_ALLOWED,
            Json(json!({ "error": "DELETE method not allowed through proxy â€” use the API directly if you need to delete resources" })),
        )
            .into_response();
    }
    ...
    let creds = mgr
        ...
        .ok()
        .flatten();
    let oauth_json = memora_connect::oauth::load_oauth_json(
        state.secret_store.as_deref(),
        &id,
        instance_ref,
    )
    ...
    let oauth_token = memora_connect::oauth::get_valid_token_instance(
        state.secret_store.as_deref(),
        &http_client,
        &id,
        instance_ref,
        ...
        )
    {
        ...
        let error = if has_oauth_state {
            ...
            format!(
                "connection '{}' has no stored credentials â€” connect it first in Settings",
                id
            )
        };
    ...
    // Build the target URL. Path-prefix routes (e.g. Google Docs "docs/" â†’
    // docs.googleapis.com) override base_url for specific path prefixes.
    // Query params from the caller must be forwarded verbatim â€” without this,
    ...
    // Forward the request â€” use a client that trusts any extra root CA the
    ...
    //     UA at the edge with 403 â€” and reqwest's default Client sends no UA.
    ...
    // override on key collision â€” integration policy still wins over caller hint.
...
/// GET /connections/:id/config â€” return non-secret connection config.
...
/// POST /connections/slack/send â€” send a Slack message.
///
...
/// POST /connections/telegram/send â€” send through stored credentials without
    ...
    match resp {
        Ok(r) => {
            ...
            if body["ok"].as_bool().unwrap_or(false) {
                ...
                let hint = match err {
                    "missing_scope" => " â€” reconnect Slack and choose \"Send + read\".",
                    "not_in_channel" => " â€” you must be a member of that channel.",
                    _ => "",
                };
                (
                    StatusCode::BAD_GATEWAY,
...
/// GET /connections/slack/search â€” search the user's accessible messages
...
/// GET /connections/slack/conversations â€” list the channels, DMs and groups the
...
/// GET /connections/slack/history â€” read recent messages in one conversation
...
// ---------------------------------------------------------------------------
// Browser extension pairing â€” lets the extension receive the local API token
...
// ---------------------------------------------------------------------------
// Browser extension bridge wrappers â€” re-extract the bridge from ConnectionsState
...
// ---------------------------------------------------------------------------
// Browser registry â€” uniform API for every kind of browser the agent can
...
/// `GET /connections` listing reaches for them before /eval â€” the
/// transcript-eaten failure mode of a model writing JS by hand.
fn format_browser_description(natural_desc: &str, id: &str) -> String {
    format!(
        "{natural_desc}\n\n\
         Control:\n\
         - POST /connections/browsers/{id}/navigate {{\"url\": \"https://...\"}}  â†’ open a URL.\n\
         - GET  /connections/browsers/{id}/snapshot                              â†’ compact, token-efficient page outline. Interactive elements carry a stable ref like #e7; headings/landmarks give structure. Read the page AND get refs to act on, here.\n\
         - POST /connections/browsers/{id}/act      {{\"ref\": \"e7\", \"action\": \"click\"}}  â†’ act on a snapshot element by ref. action âˆˆ click | fill (+\"value\") | clear | check | uncheck | select (+\"value\") | hover | focus. Re-snapshot first if refs may be stale. Prefer this over hand-written JS.\n\
         - GET  /connections/browsers/{id}/status                                â†’ ready check.\n\
         - POST /connections/browsers/{id}/eval     {{\"code\": \"...\"}}            â†’ escape hatch: run JS when navigate + snapshot + act aren't enough."
    )
}
...
/// GET /connections/browsers â€” list every registered browser with its
...
async fn list_browsers(State(state): State<ConnectionsState>) -> Json<Value> {
    ...
    // Stable ordering â€” browsers should appear in the same order across
    // calls so the agent's prompt doesn't shuffle.
    summaries.sort_by(|a, b| a.id.cmp(&b.id));
    Json(json!({ "data": summaries }))
}

/// GET /connections/browsers/:id/status â€” single-browser readiness probe.
...
/// POST /connections/browsers/:id/navigate â€” open `url` in the named browser.
///
/// Fire-and-forget: returns `{ok: true, dispatched: true, url}` as soon as
/// the navigation has been kicked off (and committed â€” see the 150ms wait
...
/// `location.href` â€” that information is now obtained via `/snapshot`.
#[derive(Deserialize)]
struct BrowserNavigateBody {
    url: String,
    /// Accepted but ignored â€” kept so existing pipes that pass it don't
...
async fn browser_run_navigate(
    ...
    // Owner of this navigation â€” the chat/session id the calling agent or pipe
    // runs under. Injected by the agent's curl shim (see
    // `memora-core::agents::bash_env`) as `x-MEMORA-session`. It rides
...
/// `browser_scripts/snapshot.js` â€” a real file so it can be linted and run in
...
/// GET /connections/browsers/:id/snapshot â€” return a compact accessibility
...
/// POST /connections/browsers/:id/act â€” perform one deterministic action on a
...
/// executes. No model calls, no heuristics â€” keep the smarts in the pipe.
async fn browser_run_act(
    ...
    if !ACT_ACTIONS.contains(&action.as_str()) {
        return (
            ...
            Json(json!({
                "ok": false,
                "error": format!("unknown action '{}' â€” use one of {:?}", body.action, ACT_ACTIONS),
            })),
        );
    }
    ...
    if ref_id.is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            Json(
                json!({ "ok": false, "error": "missing 'ref' â€” get one from /snapshot (e.g. \"e7\")" }),
            ),
        );
    }
    ...
    match browser.eval_with_owner(&script, None, timeout, owner).await {
        Ok(r) if r.ok => {
            ...
            // matching option â€” surface that as 422, success as 200.
            let inner_ok = r
                .result
                .as_ref()
                .and_then(|v| v.get("ok"))
...
/// POST /connections/browsers/:id/eval â€” run JS in the named browser.
///
...
/// `x-MEMORA-session` owner tag as the dedicated `/navigate` endpoint, or
...
/// `browser_run_navigate` and `memora-core::agents::bash_env`.
...
// (protocol lives in memora_connect::connections::bee)
// ---------------------------------------------------------------------------

/// Server-side state for an in-flight Bee pairing. The ephemeral secret key
/// never leaves the engine â€” it's held here keyed by `request_id` until the
...
/// POST /connections/bee/pair/start â€” begin a one-click Bee pairing.
///
...
/// POST /connections/bee/pair/poll {request_id} â€” poll a pending pairing.
///
/// Returns `{status: pending|completed|expired|unknown}`. On `completed` it
/// decrypts the sealed token and stores it as the Bee `api_key` credential â€”
/// the same slot the manual "Developer Token" field uses â€” so the proxy and
    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    Router::new()
        ...
        // Browser registry â€” canonical multi-instance API.
        ...
        // Browser extension pairing â€” unauthenticated start/status are still
        ...
        // Legacy single-instance browser routes â€” deployed extensions
        ...
        // Credential proxy â€” pipes call this instead of external APIs directly
...
mod mcp_oauth_connector_tests {
    ...
    fn enriches_only_matching_connector_entries() {
        ...
        // notion (not in the MCP set) is untouched â€” no spurious mcp marker
...
mod gcal_merge_tests {
    ...
    fn sorts_as_instants_across_mixed_utc_offsets() {
        // 10:00-07:00 is 17:00Z â€” a lexicographic sort would wrongly place it
        // before 16:30Z.
...
mod calendar_error_response_tests {
    ...
    fn gcal_auth_failure_maps_to_structured_401() {
        let err = anyhow::Error::new(GcalAuthError {
            message: "Google Calendar not connected â€” use 'Connect Google Calendar' from the Connections page in the desktop app".to_string(),
        });
...
mod tests {
    use super::*;
    use memora_connect::connections::{lexi::Lexi, mochi::Mochi, Integration, ProxyAuth};
    use serde_json::json;

    use axum::body::{to_bytes, Body};
    use axum::http::{header, Request};
    use memora_connect::connections::ConnectionManager;
    use memora_connect::whatsapp::WhatsAppGateway;
    ...
    fn ics_test_router(dir: &TempDir) -> Router<()> {
        let MEMORA_dir = dir.path().to_path_buf();
        let cm = Arc::new(Mutex::new(ConnectionManager::new(
            MEMORA_dir.clone(),
            None,
        )));
        let wa = Arc::new(Mutex::new(WhatsAppGateway::new(MEMORA_dir.clone())));
        router(
            cm,
            wa,
            MEMORA_dir,
    ...
    /// `?query=` is agent-reachable and carries whatever the caller sends â€”
    ...
    fn ics_feed_summaries_never_include_urls() {
        let settings = memora_connect::ics_calendar::IcsCalendarSettings {
            entries: vec![memora_connect::ics_calendar::IcsCalendarEntry {
                name: "Work".to_string(),
    ...
    fn test_resolve_base_url_unresolved_placeholder_fails() {
        let creds = Map::new(); // empty â€” no "domain" field
    ...
    fn test_resolve_auth_basic_oauth_only_no_creds() {
        // An OAuth-only Zendesk connection has no manual Basic creds â€” the OAuth
    ...
    fn test_all_proxy_configs_have_valid_base_urls() {
        use memora_connect::connections::all_integrations;
    ...
    fn browser_description_leads_with_navigate() {
        ...
        assert!(
            ...
            "navigate â†’ snapshot â†’ eval order regressed: {s}"
        );
    }
    ...
    fn snapshot_script_skips_aria_hidden() {
        ...
        // tree â€” surfacing them defeats the point of the snapshot.
    ...
    /// can prove the `x-MEMORA-session` header reaches the browser.
    struct OwnerRecordingBrowser {
        last_owner: Arc<Mutex<Option<String>>>,
    }

    #[async_trait::async_trait]
    impl memora_connect::connections::browser::Browser for OwnerRecordingBrowser {
        fn id(&self) -> &str {
            "owned-default"
        }
        fn name(&self) -> &str {
        ...
        async fn eval(
            ...
            memora_connect::connections::browser::EvalResult,
            memora_connect::connections::browser::EvalError,
        > {
            ...
            *self.last_owner.lock().await = None;
            Ok(memora_connect::connections::browser::EvalResult {
                ok: true,
    ...
    /// the `x-MEMORA-session` owner header the agent's curl shim injects.
    ...
    /// owned-browser navigate event was emitted with owner=None â€” honored in
    /// every chat.
    #[tokio::test]
        ...
        let response = app
            .oneshot(
                Request::builder()
                    ...
                    .header(header::CONTENT_TYPE, "application/json")
                    .header("x-MEMORA-session", "pipe:reddit-poster")
                    .body(Body::from(
    ...
    // -----------------------------------------------------------------------
    // OAuth callback â€” success and provider-error delivery (#5092)
    // -----------------------------------------------------------------------

    use memora_connect::oauth::PendingOAuth;
    ...
    /// page â€” not a success page, and not the unknown-state one.
    #[tokio::test]
