...
/// Kept in sync with `MEMORA_audio`'s `AUDIO_RECENCY_WINDOW_MS`.
...
pub async fn run_meeting_detection_loop(
    ...
    // If the calendar isn't connected, this stream simply never yields â€” safe no-op.
    ...
    // which is fine â€” the DB filter takes over from there.
    ...
    loop {
        ...
        let audio_onset = std::future::pending::<()>();
        tokio::select! {
        ...
        // â€” if we were Active, we resume Active on unlock; the next scan
        ...
        if running_apps.is_empty() {
            // No meeting apps running â€” end (or keep alive) the current meeting
