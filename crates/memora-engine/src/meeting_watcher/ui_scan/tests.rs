...
fn output_audio_keepalive_requires_real_voice_not_just_a_chunk() {
    ...
    // during silence â€” that alone must NOT keep an ended call alive, or a
...
// â”€â”€ audio-gated scan cadence tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ ignoredMeetingApps filter tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Webex messaging-vs-meeting discrimination (#4145/#4337) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
fn real_webex_meeting_window_is_not_guarded() {
    ...
    // messaging window â€” exact-equality (not substring) keeps it detectable.
...
fn webex_messaging_leave_menuitem_produces_no_signal() {
    ...
    // `appMenuLeaveTeam` â€” NOT an AXButton. The old `AutomationIdContains
    // ("leave")` matched its id and started a phantom meeting. With the signal
    // narrowed to `callControl`, it produces NO signal â€” and note this holds
...
fn real_webex_meeting_call_controls_are_detected() {
    ...
    // must still fire â€” the fix must not silence real meetings.
    let p = webex_profile();
...
fn webex_messaging_open_during_meeting_still_detects_meeting() {
    ...
    // excluded at the signal level), while the meeting's call control fires â€”
...
// â”€â”€ AttrNeeds tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn attr_needs_title_and_desc_signals() {
    let signals = [
        ...
        CallSignal::KeyboardShortcut("âŒ˜â‡§M"),
...
// â”€â”€ Profile tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn test_no_standalone_mute_signals() {
    ...
    for (i, p) in profiles.iter().enumerate() {
        ...
        assert!(
            has_non_mute,
            "profile {} only has mute signals â€” would cause false positives",
            i
        );
    }
}
...
fn test_zoom_screen_share_starts_detection() {
    ...
    // The idle home-screen "Share Screen" button must NOT trigger detection â€”
...
fn test_calendar_event_keep_alive() {
    ...
    // Event in progress now â†’ keep the meeting alive.
    ...
    // All-day event â†’ must NOT pin a meeting open all day.
    assert!(!has_active_calendar_event(
        &[ev(
    ...
    // Past event and no events â†’ no keep-alive.
    assert!(!has_active_calendar_event(
        &[ev(
            now - chrono::Duration::hours(2),
...
// â”€â”€ Signal matching tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn test_mute_button_alone_does_not_match_profiles() {
    ...
    for profile in &profiles {
        ...
        // leave/hangup, not mute alone â€” verified by test_no_standalone_mute_signals
        assert!(
...
fn test_generic_profile_url_patterns_are_path_qualified_for_known_lookalikes() {
    ...
    // already narrowed â€” re-broadening them in the patterns list should
    ...
    for host in must_be_path_qualified {
        ...
        assert!(
            ...
            "url pattern {host:?} must be path-qualified (e.g. {host}/<call-route>), \
                 not a bare host â€” otherwise dashboard/marketing URLs match"
        );
    }
}
...
fn test_meeting_url_in_query_does_not_match_after_stripping() {
    ...
    // count as being on that meeting page â€” this mirrors the AXDocument
...
fn test_4246_real_browsing_titles_do_not_trigger_meetings() {
    // The exact (window_title, browser_url) pairs MEMORA captured for
    // Safari when the phantom fired (#4246). The titles are full of meeting
    // keywords â€” an Amazon conference-camera shopping spree and the
    // jitsi-meet GitHub repo â€” but NONE of these pages is a meeting.
    let real_browsing: &[(&str, Option<&str>)] = &[
            (
                "Amazon.com: Owl Labs Meeting Owl 3 - 360Â° 1080p HD Conference Room Camera, \
            ...
            // tabs â€” title says "meet â€¦" but the URL is a GitHub page. URL-first
            // matching must trust the URL, not the stray title.
            (
                "meet - App on Amazon Appstore",
                Some("https://github.com/MEMORA/MEMORA/issues"),
            ),
            ...
            (
                "Issues Â· MEMORA/MEMORA Â· GitHub",
                Some("https://github.com/MEMORA/MEMORA/issues"),
            ),
        ];
...
fn test_real_browser_meetings_still_detected() {
    ...
    // Arc exposes no tab URL â€” the title "Meet" is the only signal and must
...
// â”€â”€ State machine tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn test_active_no_controls_with_audio_stays_active() {
    // Regression guard for the Activeâ‡ŒEnding flap on audio-sustained
    ...
    // And re-running many times must never leave Active or end the meeting â€”
...
fn test_native_ending_no_audio_times_out() {
    ...
    // No audio + timeout elapsed â†’ should end
    assert!(matches!(new_state, MeetingState::Idle));
    assert!(matches!(
        action,
        Some(StateAction::EndMeeting { meeting_id: 42 })
...
// â”€â”€ Edge case tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn test_handle_no_apps_active_kept_alive_by_audio() {
    ...
    // app relaunch) but output audio is still playing â€” the call is live,
...
fn test_handle_no_apps_active_kept_alive_preserves_identity() {
    ...
    // start time, same browser-ness â€” only last_seen is refreshed.
    let started = Utc::now();
    let state = MeetingState::Active {
        meeting_id: 99,
        app: "Arc".to_string(),
...
fn test_handle_no_apps_confirming_ignores_audio() {
    ...
    // ambient output audio â€” audio liveness only applies once a meeting is
    // established (Active/Ending). Confirming + no process â†’ Idle, always.
    let state = MeetingState::Confirming {
        since: Instant::now(),
        app: "Zoom".to_string(),
        profile_index: 0,
...
fn test_handle_no_apps_ending_revived_before_timeout_by_audio() {
    // Audio short-circuits the timeout entirely â€” even a brand-new Ending
...
fn test_trajectory_transient_misses_with_audio_keep_meeting() {
    ...
    // Audio stops â†’ begins ending (but is not ended on this tick).
...
fn test_trajectory_advance_to_ending_then_no_apps_audio_revives() {
    ...
    // app process disappears entirely (no_apps path) but audio is live â€”
    ...
    // Controls gone, no audio yet â†’ Ending (advance_state path).
    let (state, action) = advance_state(state, &[], false);
    assert!(matches!(state, MeetingState::Ending { meeting_id: 12, .. }));
    assert!(action.is_none());
    // Now the process scan comes up empty AND audio is flowing â†’ revive.
...
// â”€â”€ Multiple results tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Zoom menu bar signal tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ WindowTitle signal tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Zoom false positive prevention tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn test_zoom_no_mute_menu_item_signals() {
    ...
    assert!(
        !has_mute_menu,
        "Zoom profile must NOT have onMuteAudio:/onMuteVideo: signals â€” \
...
fn test_zoom_no_bare_end_button_signal() {
    // "end" alone is too broad â€” matches "Send", "Append", "Calendar End".
    ...
    assert!(
        !has_bare_end,
        "Zoom profile must NOT have bare 'end' signal â€” too broad. Use 'end meeting' instead."
    );
}
...
fn test_zoom_idle_single_signal_no_detection() {
    ...
    let state = MeetingState::Idle;
    // Only 1 signal found â€” below the threshold of 2
...
// â”€â”€ Google Meet false positive prevention tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

#[test]
fn test_google_meet_no_cmd_d_signal() {
    // Cmd+D and Ctrl+D are browser bookmark shortcuts â€” they must NOT be
    // used as meeting signals because they cause false positives when any
    // browser element exposes âŒ˜D in its accessibility description.
    let profiles = load_detection_profiles();
    let meet = profiles
        .iter()
        .find(|p| {
...
fn test_google_meet_url_patterns_no_broad_match() {
    // "google meet" as a URL pattern is too broad â€” it matches Google Calendar
    ...
    assert!(
        !has_broad_pattern,
        "Google Meet browser_url_patterns must NOT include bare 'google meet' â€” \
...
fn test_actual_meet_url_still_detected() {
    ...
    // AXDocument (page URL) â€” primary detection path
    let meet_urls = [
        "https://meet.google.com/abc-defg-hij",
...
fn test_bookmark_shortcut_not_a_meeting_signal() {
    ...
    // A random AXButton with âŒ˜D in its description (e.g. bookmark button)
    ...
    assert!(
        check_signal_match(
            ...
            "AXButton",
            None,
            Some("Add bookmark âŒ˜D"),
            None
        ),
        "KeyboardShortcut signal itself matches â€” this test verifies the signal \
    ...
    for signal in &meet.call_signals {
        if let CallSignal::KeyboardShortcut(k) = signal {
            assert!(
                *k != "\u{2318}D" && *k != "Ctrl+D",
                "Google Meet profile must not contain keyboard shortcut '{}' â€” \
...
// â”€â”€ browser_title_matches_pattern + Google Meet on Windows â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
fn browser_title_pattern_rejects_non_prefix_and_word_continuations() {
    ...
    for title in [
        // Pattern at position 0 but continues into a larger word â€”
        // separator check fails.
        "meeting reminders - gmail - google chrome",
        "meetup.com - upcoming events - firefox",
        // Pattern appears mid-string, never at the start â€” the actual
...
// â”€â”€ ax_window_matches_meeting: per-window decision of the macOS AX sweep â”€â”€â”€â”€
//
...
fn ax_window_popout_title_matches_without_document_url() {
    ...
    // only evidence ("Meet â€“ abc-defg-hij"). The anchored title patterns must
    // catch it â€” this was the secondary-window "never detected" gap.
    let profiles = load_detection_profiles();
    let meet = google_meet_profile(&profiles);

    let popout_titles = ["Meet â€“ abc-defg-hij", "Meet - abc-defg-hij - Google Chrome"];
...
fn windows_title_sweep_ignores_meet_end_and_landing_pages() {
    ...
    // the meeting") page and the Meet landing page identically â€” `Meet - Google
    // Chrome` â€” so ungated they became meeting candidates. A browser candidate
...
fn windows_title_sweep_still_matches_real_meet_windows() {
    ...
    // or without a meeting code â€” this branch is unchanged by the code gate.
    for title in &[
...
fn windows_title_sweep_ignores_calendar_named_calls_as_before() {
    ...
    for title in &[
        ...
        "MEMORA troubleshooting with Rajat - Google Chrome",
...
fn title_contains_meeting_code_requires_standalone_lowercase_token() {
    // Real pop-out shapes.
    assert!(title_contains_meeting_code("Meet â€“ abc-defg-hij"));
    ...
    assert!(!title_contains_meeting_code("Meet â€“ ABC-DEFG-HIJ"));
    assert!(!title_contains_meeting_code("Meet â€“ ab1-defg-hij"));
    assert!(!title_contains_meeting_code("Meet â€“ ab-defg-hij"));
    assert!(!title_contains_meeting_code("Meet â€“ abc-def-hij"));
...
fn ax_window_title_patterns_not_consulted_when_url_known() {
    ...
    assert!(!ax_window_matches_meeting(
        ...
        Some("Meet â€“ planning notes"),
        meet
    ));
    // Whitespace-only doc counts as "no URL" â€” the title fallback applies.
    assert!(ax_window_matches_meeting(
        Some("  "),
        Some("Meet â€“ abc-defg-hij"),
        meet
    ));
}
...
fn ax_window_calendar_page_text_does_not_match() {
    ...
    // resolve â€” the anchored matcher requires the pattern at position 0.
    let profiles = load_detection_profiles();
    let meet = google_meet_profile(&profiles);
...
// â”€â”€ role_matches cross-platform tolerance â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
// every Windows match â€” only `NameContains` signals were actually firing,
...
fn test_role_matches_macos_pascalcase_identity() {
    // macOS path: cidre returns exact `AXButton` â€” strict equality must work.
...
fn test_role_matches_windows_ax_prefix_mash() {
    ...
    // macOS naming â€” that produced "AXbutton" (lowercase tail). role_matches
...
fn test_role_matches_rejects_unrelated_roles() {
    ...
    // requires `AXButton`-shaped signals â€” false positives here would let
...
fn test_check_signal_match_role_with_name_matches_windows_button() {
    ...
    // Wrong role must still be rejected â€” a Text element named "Leave call"
