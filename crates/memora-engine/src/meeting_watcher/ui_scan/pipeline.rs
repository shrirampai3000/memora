...
//! Scanâ†’candidate plumbing for the UI/AX-scan meeting detector: discovering
...
/// in-progress meeting is tracked at full fidelity â€” the audio gate can never
...
pub(crate) async fn db_find_browser_meetings(
    ...
    for (app_name, window_name, browser_url) in &rows {
        for (idx, profile) in profiles.iter().enumerate() {
            ...
            // listing "Meeting Owl â€¦ Certified for Microsoft Teams â€¦ Zoom,
            ...
            if browser_window_matches_meeting(browser_url.as_deref(), Some(window_name), profile) {
                ...
                if pid > 0 {
                    debug!(
                        "meeting v2: DB hint â€” {} window {:?} matches profile {}",
...
pub(crate) fn get_active_tracking(
    ...
    // This is intentional â€” on the first scan after state change, we rediscover via
