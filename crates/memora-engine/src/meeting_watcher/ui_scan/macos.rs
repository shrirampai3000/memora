...
pub(crate) fn walk_for_signals(
    ...
    // Only fetch attributes the current signal set actually consults â€” each
...
/// call â€” the URL disappears from the active tab title, but call controls remain
/// in the AX tree.
    ...
    cidre::objc::ar_pool(|| {
        ...
        for i in 0..apps.len() {
            ...
            for (idx, profile) in profiles.iter().enumerate() {
                ...
                // Check browser URL patterns â€” only if this is a browser
...
/// anchored `browser_title_patterns` â€” the last only for windows exposing no
/// page URL, so a Meet pop-out titled "Meet â€“ abc-defg-hij" (Chrome exposes no
...
/// neither AXDocument nor an AppleScript `windows` entry â€” their only AX
...
impl MeetingUiScanner {
    ...
    /// focused one â€” this handles PiP/floating windows and background tabs).
    ///
...
pub(crate) fn discord_profile() -> Option<MeetingDetectionProfile> {
    ...
    // against: 2026-04-14 fe669f5b6 â€” Mute alone fired even when
...
mod live_tests {
    ...
    /// Run with: cargo test -p memora-engine --lib -- live_tests::test_live_meeting_detection --nocapture --ignored
    #[test]
    #[ignore]
...
mod live_tests2 {
    ...
    /// Run with: cargo test -p memora-engine --lib -- live_tests2::test_arc_deep_window_check --nocapture --ignored
    #[test]
    #[ignore]
