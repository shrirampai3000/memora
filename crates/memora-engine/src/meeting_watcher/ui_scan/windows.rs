...
pub(crate) fn windows_enumerate_window_titles() -> Vec<(i32, String)> {
    ...
    // empty Vec â€” killing every browser meeting detection on Windows.
    ...
    unsafe {
        ...
        for signal in signals {
            match signal {
                ...
                // can't be expressed as simple PropertyConditions â€” handled by tree walk below.
        ...
        for hwnd in window_handles {
            ...
            // named descendant buttons â€” so we must check the window itself.
                ...
                if let Ok(results) = element.FindAll(TreeScope_Descendants, &search_condition) {
                    ...
                    for i in 0..len {
                        ...
                        if let Ok(el) = results.GetElement(i) {
                            ...
                            // Single pass â€” check_signal_match (via role_matches)
...
pub fn find_running_meeting_apps(
    ...
    //       â€” if exe is missing or not in the allow-list, that's the bug.
    ...
    if tracing::enabled!(tracing::Level::DEBUG) {
        ...
        // (b) meet-shaped titles, unfiltered by browser â€” surfaces broken
        // exe lookups.
    ...
    for (idx, profile) in profiles.iter().enumerate() {
        ...
        for (pid, title) in &window_titles {
            ...
            // Pure decision, unit-tested in `shared::ignore` â€” see
            ...
            if matched.is_some() {
                // Confirms MEMORA saw the meeting window; pairs with the
            ...
            {
                // Per (profile Ã— window) miss. Cardinality is high â€” keep at
...
pub(crate) fn discord_profile() -> Option<MeetingDetectionProfile> {
    ...
    // "Disconnect") â€” and the per-element matching loop short-
...
mod windows_live_tests {
    ...
    /// Run with: cargo test -p memora-engine --no-default-features --lib -- windows_live_tests::test_live_zoom_detection --nocapture --ignored
    #[test]
    #[ignore]
