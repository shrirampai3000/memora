...
/// Sync the in-meeting flag to the shared AtomicBool.
...
/// change, AX reflow) must not end a live call â€” this mirrors the audio-liveness
/// guard in `advance_state`'s Endingâ†’Active path.
///
...
pub(crate) fn handle_no_apps_running(
    ...
    match state {
        MeetingState::Active {
            ...
            // Output audio still flowing â†’ the process scan missed transiently;
            // keep the meeting alive rather than ending an in-progress call.
            if has_output_audio {
                info!(
                    "meeting v2: no meeting app process found but output audio active â€” keeping meeting alive (app={}, id={})",
                    app, meeting_id
                );
                return (
                    MeetingState::Active {
            ...
            // for the whole window â€” the call is genuinely over, not a tab
            ...
            (
                MeetingState::Ending {
                    ...
                    started_at,
                    since: Instant::now(),
                    is_browser: false, // gone + silent â†’ use short timeout
        ...
        MeetingState::Ending {
            ...
            // Audio still flowing â†’ return to Active instead of ending, even
    ...
    *state = new_state;
    ...
    if matches!(*state, MeetingState::Idle) {
        *idle_scan_count += 1;
        ...
        // Ending/Confirming state â€” keep scanning at active rate
        base_interval
    }
}
    ...
    match action {
        StateAction::StartMeeting { app } => {
            ...
            *flap_count = 0;
            ...
            // meeting â€” one calendar event describes one meeting.
            ...
            let (meeting_id, decision_trigger) = match merge_candidate {
                Ok(Some(recent)) => match db.reopen_meeting(recent.id).await {
                    Ok(()) => {
                        ...
                        // none â€” but only once this meeting owns the event.
