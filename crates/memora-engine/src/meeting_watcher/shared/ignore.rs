...
/// (`"Meet - abc - Google Chrome"`, `"Meet â€” Mozilla Firefox"`). The anchor +
/// separator rule is what keeps `"Meeting reminders"` and
/// `"Join with Google Meet - Calendar"` from triggering detection.
///
/// `title_lower` must already be lowercased â€” hot path, called per window.
...
///   1. the running app's localized name / process name â€” what the user sees
...
/// searched for URL patterns â€” titles carry arbitrary text (an Amazon listing
/// for a "Meeting Owl â€¦ Certified for Microsoft Teams â€¦ Works with Zoom, Google
...
/// a fallback used ONLY when no URL is available â€” e.g. Arc, which titles its
...
///
/// - `browser_url_patterns` must appear verbatim with boundary matching â€” this
///   keeps a real meeting host matching while closing the `daily.co` âŠ‚
...
/// `Meet â€“ abc-defg-hij`. Ungated, the bare `Meet` anchor therefore fired almost
...
/// AXDocument (the page URL, when the browser exposes one â€” Safari does,
...
///   title ("meet.google.com") counts â€” bare text like "Google Meet" appears
...
///   pop-out titled "Meet â€“ abc-defg-hij" (no AXDocument, no domain in the
...
///
/// A real Chrome/Edge Meet pop-out is titled "Meet â€“ abc-defg-hij"; this is
...
///   * `daily.co`      â†’ matches `daily.co`, `app.daily.co/room`; NOT
///                       `thedaily.com`, `daily.com`, `daily.co.uk`.
///   * `cal.com/video` â†’ matches `app.cal.com/video/uid`; NOT `cal.com/videos`,
///                       `cal.com/pricing`.
///
/// Non-domain patterns (containing a space, or with no `.`) keep the previous
/// case-insensitive substring behavior â€” they are free-text markers
