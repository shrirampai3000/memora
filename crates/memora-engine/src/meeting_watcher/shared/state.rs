...
pub enum MeetingState {
    /// No meeting detected.
    Idle,
    /// Possible meeting â€” waiting for a second scan to confirm.
    Confirming {
        since: Instant,
        app: String,
        profile_index: usize,
    ...
    /// Meeting controls disappeared â€” waiting before marking ended.
    Ending {
        meeting_id: i64,
        app: String,
        started_at: DateTime<Utc>,
        ...
        /// Ending â†’ Active. See `REENTRY_HYSTERESIS_SCANS`.
...
/// Longer timeout for browser-based meetings â€” tab switching hides AX controls,
...
/// single transient blip (one scan that happens to find controls â€” AX tree
/// reflow, brief toolbar peek) can no longer flip the state. Two consecutive
/// visible scans (â‰¥5s of sustained presence) are needed. This cuts log noise
/// from the Activeâ‡ŒEnding oscillation observed in Arc/Meet (Meeting 72,
...
/// matches *any* `(output)` audio chunk in the window â€” but the system-audio tap
...
///
/// Shared by BOTH keep-alive sites â€” the apps-present (Ending-only) path and the
/// no-apps (Active|Ending) path â€” so the silence-gating can never drift between
...
/// True when a transition is an Active<->Ending oscillation (a "flap") â€” the
...
/// This function is pure â€” it does not perform side effects, making it easy to test.
///
/// `has_output_audio`: when true, audio output device has recent data â€” keeps
    ...
    match state {
        ...
        MeetingState::Active {
            ...
            if let Some(result) = best_active {
                ...
                // Controls vanished but output audio is still flowing â€” the user
                ...
                debug!(
                    "meeting v2: Active (no controls but output audio active â€” staying Active, app={}, id={})",
                    app, meeting_id
                );
                (
                    MeetingState::Active {
        ...
        MeetingState::Ending {
            ...
            if let Some(result) = best_active {
                ...
                // Keep the grace clock ticking â€” a single transient blip
            ...
            if has_output_audio {
                // Audio output is still active â€” the user likely just switched
