...
pub(crate) struct CalendarEventSignal {
    ...
    /// don't supply one â€” such events are never bound, only matched.
    #[serde(default)]
    pub id: String,
    pub title: String,
    pub start: String,
...
impl CalendarBinding {
    fn from_event(event: &CalendarEventSignal) -> Self {
        ...
        // event's natural identity â€” an event is the same event when its
...
/// break toward the event whose start is nearest `now`. Deterministic â€” it
...
pub(crate) async fn resolve_calendar_binding(
    ...
    match db.meeting_id_for_calendar_event(&binding.key).await {
        ...
        Ok(Some(owner)) => {
            info!(
                "calendar event {:?} already belongs to meeting {} â€” leaving this meeting to be named from its own content",
                binding.title, owner
            );
            None
        }
