...
/// `"low"`, while `"high"` flags sustained `Active<->Ending` oscillation â€” the
/// failure mode where the call's controls vanished but the meeting kept getting
/// revived and never finalized. No titles/attendees/notes â€” buckets only.
