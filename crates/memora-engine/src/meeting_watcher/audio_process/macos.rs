...
/// pipeline to happen to capture a frame. Enumerating ALL windows â€” not just
/// the focused one â€” is what catches a call in a pop-out or secondary window
...
pub(crate) async fn active_tab_url_candidates(
    ...
    let probe = tokio::task::spawn_blocking(move || {
        let detector = memora_screen::browser_utils::create_url_detector();
        browser_apps
            .iter()
            .filter_map(|app| {
