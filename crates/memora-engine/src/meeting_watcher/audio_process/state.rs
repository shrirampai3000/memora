...
pub(crate) fn advance_audio_process_state(
    ...
    // Live evidence â€” a native app identity, the active-tab URL probe, or the
    // AX window sweep, all observed on THIS poll â€” starts a meeting from any
...
/// `Idle` â†’ promote a resolved session to `Candidate`, an unresolved browser to
...
/// `Candidate` â†’ start the meeting once the same resolved session has been seen
...
/// `CandidateUnresolvedBrowser` â†’ promote to `Candidate` once the browser
...
/// `Active` â†’ stay active while the session is present, otherwise begin winding
/// down via `Ending`.
#[allow(clippy::too_many_arguments)]
...
/// `Ending` â†’ revive to `Active` if the session reappears, end the meeting once
