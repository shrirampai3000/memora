...
async fn setup_db() -> (tempfile::TempDir, DatabaseManager) {
    ...
    sqlx::migrate!("../memora-db/src/migrations")
        .run(writer.pool())
        .await
        .unwrap();
    drop(writer);
...
/// separate process â€” see `facetime_daemon_is_not_an_identity_signal`.)
#[test]
...
/// Documents (pre-existing, unchanged) behavior: `avconferenced` â€” FaceTime's
/// audio IO daemon â€” is NOT a platform identity for auto-detection; the
...
fn active_tab_join_url_with_query_params_resolves_candidate() {
    // Green-room join URLs carry query params (?ijlm=â€¦&adhoc=1). Matching
...
fn active_tab_unrelated_url_does_not_resolve() {
    ...
    assert!(resolve_active_tab_url_candidate(
        "Arc",
        "https://github.com/MEMORA/MEMORA/pull/4772",
        &profiles
    )
    .is_none());
...
fn meet_code_title_shape_is_strict() {
    ...
    for title in [
        ...
        "\u{0430}bc-defg-hij", // non-ASCII lookalike (Cyrillic Ð°)
    ] {
...
fn little_arc_candidate_starts_meeting_as_live_evidence() {
    ...
    // must behave exactly like a URL-probe candidate â€” live evidence, Google
...
fn resolved_browser_meeting_stays_alive_when_url_evidence_lost() {
    ...
    // because screen-capture URL evidence lapsed â€” e.g. the user switched to
...
fn sticky_process_absent_from_live_snapshot_cannot_start() {
    ...
    let key = ProcessKey::from_process(&process).unwrap();
    let start = Instant::now();
    // live_evidence: true on purpose â€” even live evidence must not start a
...
fn live_resolution_starts_meeting_on_first_sighting() {
    ...
    // native identity) starts the meeting immediately â€” no confirm window,
    // even a generous one.
    let process = arc_process();
    let key = ProcessKey::from_process(&process).unwrap();
    let start = Instant::now();
...
fn live_resolution_starts_immediately_from_unresolved_browser() {
    ...
    // a fresh confirm window â€” that wait exists for stored evidence only.
    let process = arc_process();
    let key = ProcessKey::from_process(&process).unwrap();
    let start = Instant::now();
...
fn browser_meeting_ends_only_after_audio_session_disappears() {
    ...
    // evidence lapsed â€” degrading the live candidate to `UnresolvedBrowser`
    // with the SAME audio session â€” must stay Active: the mic is still held,
    ...
    // Mic released (audio session gone) â†’ transition toward Ending.
    ...
    // Past the ending grace with the session still gone â†’ EndMeeting.
...
fn reattached_meeting_ends_when_no_session_is_live() {
    ...
    // No live meeting candidates at all â€” the reattached meeting must still
...
fn ax_resolution_only_runs_before_a_meeting_is_active() {
    ...
    // settled â€” re-walking the AX tree every 1s for the whole call is the
...
// ---------------------------------------------------------------------------
// Call signal gate tests (#4776) â€” WhatsApp/Signal/Telegram voice note phantom
// meeting prevention.
// ---------------------------------------------------------------------------
...
fn unicode_ltr_mark_stripped_from_whatsapp_identity() {
    // macOS returns "\u{200e}WhatsApp" â€” verify process_identity_fields strips it.
...
fn signal_resolves_to_native_without_gate() {
    ...
    // because its Electron AX tree is opaque â€” requires_call_signal is
...
fn zoom_unaffected_by_call_signal_gate() {
    ...
    // is fine â€” it should never be gated.
}
...
/// Windows-shaped Signal process: same shape as above (a real call â€” Windows
...
fn native_meeting_survives_mic_switch_rekey() {
    ...
    // EndMeeting) just because the key rotated â€” the same native platform
...
fn end_after_mic_switch_rekey_suppresses_adopted_session_key() {
    ...
    // The call winds down: no candidates â†’ Ending.
    let (ending, action) = advance_audio_process_state(
        active,
        &[],
        &[],
    ...
    // Past the ending grace â†’ EndMeeting, whose suppression must carry the
...
// piggyback sweep computes an empty pid set and disengages entirely â€” no
...
fn ax_resolution_keeps_running_for_reattached_meeting() {
    ...
    // until a live candidate re-attributes it â€” otherwise a browser meeting
...
fn reattached_meeting_keeps_synthetic_key_when_resolution_is_other_platform() {
    ...
    // identity â€” the synthetic keep-alive stays in force instead.
    let start = Instant::now();
...
// â”€â”€ Calendar event binding â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
/// Reproduces the 2026-08-13 incident. An 11:30â€“12:00 calendar event was still
/// running when a second, unrelated Google Meet started at 11:40 â€” 2m13s after
...
async fn calendar_event_binds_to_a_single_meeting() {
    ...
    // adopt it â€” the second Meet becomes its own row, as it did in production.
...
/// bind silently discards a legitimate re-enrichment â€” and a meeting whose
...
async fn manually_started_meeting_claims_its_calendar_event() {
    ...
    assert!(
        ...
        "a manually claimed event must not also name the next meeting â€” got {:?}",
        row.title
    );
}
