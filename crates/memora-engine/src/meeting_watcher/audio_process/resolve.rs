...
/// meeting platform. Once a meeting is `Active` â€” or already winding down in
/// `Ending` â€” the platform is settled, so re-walking the tree every
...
/// `reattached:` key and a published `ActiveMeeting { pid: None }` â€” the
...
/// only a candidate RESOLVED to the meeting's own platform counts â€” an
...
pub(crate) async fn should_use_ax_fallback(
    ...
    // the browser directly for its active-tab URL â€” this works with the URL
...
/// â€” the title (the bare meeting code) is their entire AX footprint. Gated to
...
pub(crate) fn resolve_process_candidate(
    ...
    if let Some(browser_app) = browser_app_name(process) {
        ...
        // up to 10s stale â€” a stale frame must never outrank a live answer.
    ...
    if let Some((platform, profile_index)) = resolve_native_platform(process, profiles) {
        ...
        // "renderer", it's a voice note â€” block it.
        //
        // `bundle_id`/`owner_bundle_id` are macOS-only fields (always `None` on
        // Windows â€” see `MEMORA_audio::meeting_processes::platform`), so
        ...
        // discriminating field is structurally absent, fail open instead â€”
        ...
        {
            debug!(
                "audio-process meeting detector: Signal blocked â€” voice note helper \
...
pub(crate) fn known_native_bundle_platform(field_lower: &str) -> Option<&'static str> {
    ...
    // â€” we can't distinguish calls from voice notes, so requires_call_signal
...
/// Acquire the current audio-input process snapshot, dropping MEMORA's own
...
pub(crate) fn acquire_input_processes(
    ...
    if !snapshot.supported {
        ...
        // This is a flag resync, not a fresh transition â€” preserve whatever
...
pub(crate) async fn build_candidates(
    ...
    // call UI (e.g. Calling_Window). Only run during pre-active states â€”
                ...
                if requires_gate {
                    ...
                    match call_evidence.iter().find(|e| e.platform == platform_lower) {
                        ...
                        None => {
                            debug!(
                                "audio-process meeting detector: {} blocked by call signal gate \
                                 (no evidence produced â€” scan may have timed out)",
                                platform
                            );
                            return false;
                        }
