...
use MEMORA_audio::meeting_detector::MeetingDetector;
use MEMORA_audio::meeting_processes::{self, AudioInputProcess};
use memora_db::{DatabaseManager, MEETING_END_REASON_AUTO_END, MEETING_END_REASON_SHUTDOWN};
use memora_events::subscribe_to_event;
...
/// stale) must persist before a meeting starts â€” two consecutive sightings at
...
/// filter voice notes on messaging platforms (real voice notes run 5â€“60s and
/// outlast any sane value) â€” that is #4776's call-signal gate, not this
/// constant.
...
/// browser has stayed unattributed this long â€” the "my call was never
...
pub async fn run_audio_process_meeting_detection_loop(
    ...
    if let Ok(Some(meeting)) = db.get_most_recent_active_meeting().await {
        ...
        // (which publishes ActiveMeeting{manual: true} for manual meetings â€”
        ...
        sync_meeting_flag(
            true,
            Some(MEMORA_audio::meeting_detector::ActiveMeeting {
                pid: None,
                bundle_id: None,
    ...
    loop {
...

        {
            ...
            if manual.is_some() {
                ...
                // detector has no process identity for it and never will â€”
        ...
        // only `StartMeeting` ever published a pid â€” so without this the
                        ...
                        resolved_platform_identity(&live_candidates, platform)
                    {
                        info!(
                            "audio-process meeting detector: re-resolved meeting process \
                             (pid={}, app={}) â€” per-process capture re-engages",
                            pid, platform
                        );
