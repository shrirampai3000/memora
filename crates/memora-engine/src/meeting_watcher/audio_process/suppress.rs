...
pub(crate) fn matching_session_key(
    ...
    // session key and no real process/url to reconcile against â€” and a browser
    ...
    let exact_match = candidates.iter().any(|candidate| match candidate {
        ...
        // proof the call is ongoing â€” so keep it alive on the `session_key` alone.
    ...
    // different mic mints a new GUID. Ending a live call over that rotation â€”
    // Active â†’ Ending â†’ 20s grace â†’ EndMeeting, then an instant "new" meeting
    // under the new key â€” tears capture down for ~20s per mic switch. The same
