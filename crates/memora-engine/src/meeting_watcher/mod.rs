...
//! Meeting watcher â€” spawns the selected meeting-detection backend.
//!
//! Two backends live under this module:
//! - [`audio_process`] â€” mic-capture sensor (default on macOS/Windows).
//! - [`ui_scan`] â€” periodic accessibility/UI-Automation tree scanning.
//!
...
// Public surface consumed by other crates via `memora_engine::meeting_watcher::â€¦`.
...
fn selected_detector_mode() -> MeetingDetectorMode {
    ...
    // back to UI scanning. Override on any platform with MEMORA_MEETING_DETECTOR.
    let audio_process_is_default = cfg!(target_os = "macos") || cfg!(target_os = "windows");
    selected_detector_mode_from(
        std::env::var("MEMORA_MEETING_DETECTOR").ok().as_deref(),
...
mod tests {
    ...
    fn detector_override_accepts_hyphenated_and_mixed_case_spellings() {
        ...
        // `MEMORA_MEETING_DETECTOR=ui-scan`. Both hyphen and underscore
