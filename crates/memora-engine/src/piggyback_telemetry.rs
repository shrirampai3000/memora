...
//! Lives engine-side because MEMORA-audio cannot depend on the analytics
...
//! opt-out, kill switches) is inherited from `analytics::capture_event_*` â€”
...
/// Allowlist: bus event name â†’ PostHog event name. `capture_event_nonblocking`
...
/// emitters in MEMORA-audio and to the PostHog dashboard insights.
...
pub fn spawn_piggyback_telemetry_forwarder() {
    tokio::spawn(async move {
        let mut subscription = memora_events::subscribe_to_all_events();
