...
//! Export a recording â€” a meeting, or any wall-clock time range â€” to a single self-contained
...
//! then exists on both devices, so combining them â€” summed to mono or panned to separate
//! channels â€” plays it twice = echo. The loopback also never contains your microphone, so it
...
use memora_core::{ffmpeg_cmd_async, find_ffmpeg_path};
...
/// meeting export and the `MEMORA export` CLI. Returns a summary on success.
///
...
async fn export_range_to_mp4_with_audio(
    ...
    if frames.is_empty() {
        return Err(anyhow!(
            "no screen frames indexed for this range ({start} .. {end}) â€” the recording may be missing from the database or not yet indexed"
        ));
    }
    ...
    //    skipped â€” the previous surviving frame is simply held longer, preserving sync.
    let mut frame_out: Vec<Option<PathBuf>> = vec![None; frames.len()];

    // 4a. Snapshots (direct JPEGs) â€” one decode each.
    ...
    //     ends up on both devices â€” confirmed on real meetings, where the two devices transcribe
    ...
    //     microphone in it, so it never contains your own voice â€” only the mic does. So when both
    ...
    if !audio.is_empty() {
        ...
        if devices.len() > 1 {
            // Prefer the microphone (input) â€” the only device that carries your voice â€” then the
    ...
    // 8. Single ffmpeg mux: concat (video) + N audio inputs â†’ re-timed video + mixed audio.
...
/// Parse the device name out of a MEMORA audio chunk filename and report whether it is a
...
/// 0-based decoded frame index (`offset_index`). MEMORA's "compact" chunks are variable frame
/// rate (a nominal `r_frame_rate` of e.g. 25 but only a fraction of a real fps), so seeking by
/// `offset_index / fps` lands on the wrong frame. `select='eq(n,N)+â€¦'` is exact regardless of VFR.
///
...
async fn run_mux(
    ...
    if audio.is_empty() {
        // No audio in range â€” emit a silent video.
        cmd.args([
            "-map",
            "0:v:0",
            "-c:v",
    ...
    // overlap â€” the mix is just stitching one continuous stream back together, with no echo.
...
mod tests {
    ...
    fn concat_list_holds_first_frame_from_origin() {
        ...
        let durations: Vec<f64> = body
            ...
            .collect();
        // frame 0 held from origin â†’ 7s (covers the 5s lead-in), then 3s, then tail to 15s = 5s
    ...
    fn device_key_parses_name_and_output_flag() {
        let (k, out) = audio_device_key(
            "/Users/x/.memora/data/System Audio (output)_2026-05-28_18-05-16.mp4",
        );
        assert_eq!(k, "System Audio (output)");
        assert!(out, "system audio should be detected as output");

        let (k, out) = audio_device_key(
            "/Users/x/.memora/data/MacBook Pro Microphone (input)_2026-05-28_18-05-22.mp4",
        );
    ...
    fn device_key_without_timestamp_suffix_is_whole_stem() {
        // No recognizable timestamp suffix â†’ fall back to the full stem (never panic / mis-slice).
    ...
    /// Real-data smoke test: exports an actual meeting from `~/.memora/db.sqlite` and opens the
    ...
    ///   SP_EXPORT_TEST_MEETING_ID=185 cargo test -p memora-engine \
    ///     export_real_meeting_opens_mp4 -- --ignored --nocapture
    #[tokio::test]
    #[ignore]
    async fn export_real_meeting_opens_mp4() {
        let home = std::env::var("HOME").expect("HOME not set");
        let base = PathBuf::from(home).join(".memora");
