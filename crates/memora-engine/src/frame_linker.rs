...
//! Frame linker â€” pairs `ui_events` rows with the `frames` they triggered.
//!
//! ## Why this exists
//!
...
//! counter) and forwards it both into the `CaptureTrigger` and â€” after
//! the batch flush returns row ids â€” into the linker. The capture loop
...
//!   correlation ids (debounced triggers) â€” all matching rows get the
//!   same frame_id.
...
/// that the capture loop consumed since its last successful capture â€”
...
pub enum DropReason {
    ...
    Paused,
    /// `broadcast::Receiver::recv` returned `Lagged(n)` â€” the ring
...
/// trip from "trigger sent" â†’ "frame captured + row flushed."
...
impl Default for FrameLinkerConfig {
    fn default() -> Self {
        Self {
            ...
            // (a noisy app firing 100s of clicks/sec) without OOM â€”
...
impl FrameLinker {
    ...
    pub fn on_event_persisted(&mut self, e: EventPersisted, now: Instant) -> Option<LinkUpdate> {
        ...
        // No match yet â€” stash and wait for the frame.
    ...
    /// from the host actor. Returns the number of entries evicted â€”
    ...
    fn evict_if_full_events(&mut self, _now: Instant) {
        ...
        // O(capacity) at worst â€” fine at the volumes we see.
...
mod tests {
    ...
    fn late_frame_for_already_paired_event_is_noop() {
        ...
        // but if it does we don't double-update â€” the corr id is gone.
    ...
    fn unrelated_correlation_ids_dont_match() {
        ...
        assert!(updates.is_empty());
        // Both sides remain pending â€” neither has been matched.
    ...
    fn pending_frame_with_mixed_matched_and_unmatched_corr_ids() {
        ...
        // Row 2 lands â†’ pair with the same frame.
        let u2 = linker.on_event_persisted(
            EventPersisted {
                correlation_id: 2,
                row_id: 101,
