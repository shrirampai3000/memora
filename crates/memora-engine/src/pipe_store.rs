...
use memora_core::pipes::{PipeExecution, PipeExecutionActivity, PipeStore, SchedulerState};
use memora_db::DatabaseManager;
use std::sync::Arc;

/// SQLite-backed pipe store using the main MEMORA database.
/// Reads go through the read pool (27 connections), writes go through
/// the write coalescing queue for proper serialization â€” avoiding
...
impl PipeStore for SqlitePipeStore {
    ...
    async fn set_execution_running(&self, id: i64, pid: Option<u32>) -> Result<()> {
        use memora_db::write_queue::PipeBindValue;
    ...
    async fn get_all_executions(
        ...
        // List view: skip stdout/stderr â€” they're MB-scale per row (observed avg
        ...
        // each call (ran every ~30s from the UI poll, observed 1.3â€“4.7s per
        ...
        // do `WHERE pipe_name = ? ORDER BY id DESC LIMIT ?` â€” that one is a
...
mod tests {
    ...
    async fn setup_test_store() -> (SqlitePipeStore, tempfile::TempDir) {
        ...
        // Retry up to 3 times â€” when many tests start simultaneously the
    ...
    // Legacy setup kept for reference â€” the old approach used raw pool
    ...
    async fn test_create_execution() {
        let (store, _tmp) = setup_test_store().await;
        let id = store
            .create_execution("my-pipe", "manual", "haiku", Some("MEMORA"))
            .await
            .unwrap();
        assert!(id > 0);
    }
    ...
    async fn test_mark_orphaned_running() {
        ...
        // Mark orphans â€” should catch id1 (running) and id2 (queued).
    ...
    async fn test_cleanup_old_executions() {
        ...
        // Keep only 2 per pipe â€” should delete 3 from pipe-a and 1 from pipe-b.
        // The write queue implementation can't return rows_affected, so
