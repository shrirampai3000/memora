...
//! MEMORA cloud classifier. When a high-confidence workflow event is detected,
...
use reqwest::Client;
use memora_events::{ActivityEntry, ClassifierResult, WorkflowEvent};
use serde_json::json;
...
/// Default endpoint for the event classifier â€” routes through the MEMORA
/// AI gateway, which forwards the `MEMORA-event-classifier` model to the
/// self-hosted vLLM. Going through the gateway means infra moves only need a
/// `wrangler secret put EVENT_CLASSIFIER_URL` â€” no client release.
/// Override with the `MEMORA_EVENT_CLASSIFIER_URL` env var for self-host.
pub const DEFAULT_CLASSIFIER_URL: &str = "https://api.memora.com";
...
/// Query the local MEMORA `/search` API for recent activity across all content types.
...
/// Call the classifier through the MEMORA gateway (default) or directly
/// against a self-hosted vLLM (`MEMORA_EVENT_CLASSIFIER_URL` override).
/// The model name `MEMORA-event-classifier` is recognized by both: vLLM
