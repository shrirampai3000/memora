...
//! Local data retention â€” auto-deletes old data after a configurable number of
...
/// - `Lean`: also reclaims media AND strips the heavy text a frame carries â€”
///   the per-node accessibility/OCR `elements`, the raw accessibility tree
///   JSON, and the `ui_events` stream â€” while keeping `full_text`, transcripts,
...
/// POST /retention/configure â€” enable/disable local retention, set days.
#[oasgen]
...
/// GET /retention/status â€” return current retention state.
#[oasgen]
...
/// POST /retention/run â€” trigger an immediate cleanup run.
#[oasgen]
...
fn spawn_retention_loop(
    ...
    tokio::spawn(async move {
        ...
        // process â€” without this, every single cycle would re-walk the full
        ...
        loop {
            ...
            if batch_start >= cutoff {
                // Already caught up to the retention window â€” nothing ages
...
/// subtraction would underflow chrono's representable date range â€” in which
...
/// is already met â€” usually because it was compacted away, deleted by a prior
...
/// nothing to process) â€” the caller persists this as the next cycle's
...
async fn do_local_cleanup(
    ...
    while batch_start < cutoff {
        ...
        // â€” so a failure must stop the walk here instead of advancing past
        ...
        if batch_failed {
            // Stop here without advancing `batch_start` past this range â€”
    ...
    if any_deleted {
        ...
        // without a full VACUUM (intentionally not run here â€” it takes an
...
mod tests {
    ...
    /// Regression test for MEMORA/MEMORA#4843: the retention loop used
    ...
    async fn do_local_cleanup_resume_from_watermark_skips_reprocessed_range() {
        ...
        // inter-batch yield sleep â€” this is what every single 5-minute cycle
        ...
        // first â€” and would stay identical forever, every 5 minutes.
    ...
    /// cycles two ways â€” restarting from `oldest` every time (the pre-#4843
    ...
    async fn simulated_retention_cycles_old_vs_new_calling_convention() {
        ...
        println!(
            "retention: {} simulated 5-min cycles over 6h of history â€” \
        ...
        // Speed alone doesn't prove correctness â€” a buggy resume that
