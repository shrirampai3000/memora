...
const SUPPORT_ID_ENV_VARS: &[&str] = &["MEMORA_SUPPORT_ID", "MEMORA_TELEMETRY_ID"];
const CUSTOMER_ID_ENV_VARS: &[&str] = &[
    "MEMORA_CUSTOMER_ID",
    "MEMORA_ORG_ID",
    "MEMORA_TELEMETRY_CUSTOMER_ID",
];
const DEPLOYMENT_ID_ENV_VARS: &[&str] = &[
    "MEMORA_DEPLOYMENT_ID",
    "MEMORA_TELEMETRY_DEPLOYMENT_ID",
];
const EMBEDDER_ENV_VARS: &[&str] = &[
    "MEMORA_EMBEDDER",
    "MEMORA_HOST_APP",
    "MEMORA_TELEMETRY_HOST_APP",
];
const EMBEDDER_VERSION_ENV_VARS: &[&str] = &[
    "MEMORA_EMBEDDER_VERSION",
    "MEMORA_HOST_VERSION",
    "MEMORA_TELEMETRY_HOST_VERSION",
];
const DISTRIBUTION_ENV_VARS: &[&str] = &["MEMORA_DISTRIBUTION", "MEMORA_DIST"];
const ENTERPRISE_LICENSE_HASH_ENV_VARS: &[&str] = &["MEMORA_ENTERPRISE_LICENSE_HASH"];
const ENTERPRISE_DEVICE_ID_ENV_VARS: &[&str] = &["MEMORA_ENTERPRISE_DEVICE_ID"];

/// How this engine was launched: "desktop-app" (Tauri app), "cli" (npm/bunx),
/// or "source"/"source-dev" (built locally). The app and CLI set
/// `MEMORA_DISTRIBUTION` explicitly; without it we infer a source build
...
impl TelemetryContext {
    ...
    /// enterprise support/telemetry id). For a bare `MEMORA` CLI run â€” none
    /// of those set â€” it falls back to the persistent per-machine id instead of
    ...
    pub fn distinct_id() -> String {
        Self::distinct_id_from_env().unwrap_or_else(memora_core::sync::get_or_create_machine_id)
    }
    ...
    pub fn pairs(&self) -> Vec<(&'static str, &str)> {
        ...
        push_if_some(&mut pairs, "MEMORA_support_id", &self.support_id);
        push_if_some(&mut pairs, "MEMORA_customer_id", &self.customer_id);
        push_if_some(&mut pairs, "MEMORA_deployment_id", &self.deployment_id);
        push_if_some(&mut pairs, "MEMORA_embedder", &self.embedder);
        push_if_some(
            &mut pairs,
            "MEMORA_embedder_version",
        ...
        push_if_some(
            &mut pairs,
            "MEMORA_enterprise_license_hash",
            &self.enterprise_license_hash,
        );
        push_if_some(
            &mut pairs,
            "MEMORA_enterprise_device_id",
    ...
    pub fn insert_posthog_properties(&self, properties: &mut Map<String, Value>) {
        // Distribution channel (desktop-app / cli / source) is ALWAYS tagged â€”
        // even with no enterprise context â€” so every engine event and person can
        // be split by how MEMORA was installed (separates the free OSS engine
        // WAU from the signed app + paid users).
        let distribution = resolve_distribution();
        properties.insert(
            "MEMORA_distribution".to_string(),
        ...
        if let Some(set_obj) = set.as_object_mut() {
            set_obj.insert("MEMORA_distribution".to_string(), json!(distribution));
...
mod tests {
    ...
    fn distinct_id_prefers_existing_analytics_id() {
        with_env(
            &[
                ("MEMORA_ANALYTICS_ID", "analytics-user"),
                ("MEMORA_SUPPORT_ID", "support-user"),
            ],
            || {
                assert_eq!(
    ...
    fn support_id_can_supply_standalone_distinct_id() {
        with_env(&[("MEMORA_SUPPORT_ID", "spcust_acme_123")], || {
            assert_eq!(
                TelemetryContext::distinct_id_from_env(),
    ...
    fn distinct_id_falls_back_to_stable_machine_id() {
        with_env(&[], || {
            ...
            // per-machine id, so it's identical across calls â€” i.e. across
            // process restarts â€” instead of a fresh UUID each time (which made
    ...
    fn distinct_id_prefers_launcher_id_over_machine_id() {
        with_env(&[("MEMORA_ANALYTICS_ID", "analytics-user")], || {
    ...
    fn posthog_properties_include_person_set_values() {
        with_env(
            &[
                ("MEMORA_SUPPORT_ID", "spcust_acme_123"),
                ("MEMORA_ORG_ID", "acme"),
                ("MEMORA_DEPLOYMENT_ID", "prod-fleet"),
                ("MEMORA_EMBEDDER", "acme-agent"),
            ],
            || {
                ...
                assert_eq!(
                    properties.get("MEMORA_support_id"),
                    Some(&json!("spcust_acme_123"))
                );
                assert_eq!(
                    properties.get("MEMORA_customer_id"),
                    Some(&json!("acme"))
                );
                assert_eq!(
                    properties.get("MEMORA_deployment_id"),
                    Some(&json!("prod-fleet"))
                );

                let set = properties.get("$set").and_then(|value| value.as_object());
                assert_eq!(
                    set.and_then(|value| value.get("MEMORA_embedder")),
    ...
    fn distribution_prefers_env_then_falls_back_to_source() {
        with_env(&[("MEMORA_DISTRIBUTION", "desktop-app")], || {
    ...
    fn distribution_always_tagged_even_without_context() {
        with_env(&[("MEMORA_DISTRIBUTION", "cli")], || {
            let context = TelemetryContext::from_env();
            ...
            assert_eq!(
                properties.get("MEMORA_distribution"),
                Some(&json!("cli"))
            );
            let set = properties.get("$set").and_then(|value| value.as_object());
            assert_eq!(
                set.and_then(|value| value.get("MEMORA_distribution")),
                Some(&json!("cli"))
            );
        });
    }
    ...
    fn posthog_properties_include_enterprise_tags_and_group() {
        with_env(
            &[
                ("MEMORA_ENTERPRISE_LICENSE_HASH", "ent_abc123"),
                ("MEMORA_ENTERPRISE_DEVICE_ID", "device-1"),
            ],
            || {
                let context = TelemetryContext::from_env();
                let mut properties = Map::new();
                ...
                assert_eq!(
                    properties.get("MEMORA_enterprise_license_hash"),
                    Some(&json!("ent_abc123"))
                );
                assert_eq!(
                    properties.get("MEMORA_enterprise_device_id"),
                    Some(&json!("device-1"))
                );
                ...
                let set = properties.get("$set").and_then(|value| value.as_object());
                assert_eq!(
                    set.and_then(|value| value.get("MEMORA_enterprise_device_id")),
                    Some(&json!("device-1"))
                );
            },
        );
