...
// Injected by GET /connections/browsers/:id/snapshot. Walks the live DOM â€”
// piercing open shadow roots and same-origin iframes â€” and returns a compact,
...
// below-the-fold content is kept. State is inlined â€” `(disabled)`, `(checked)`,
// `(expanded)` â€” so the model needn't probe with follow-up evals.
//
// Limits (documented, not silently wrong): cross-origin iframes are opaque to
// the page by browser security â€” we surface them as one `iframe (cross-origin)`
...
const MAX_DEPTH = 12;
const out = [];
// Non-interactive tags kept for structure/context â€” but only when they carry
...
// Associated <label> text â€” the accessible name a form control gets from a
...
// check yet be covered by a modal, cookie wall, or sticky header â€” offering it
...
function isOccluded(el) {
    ...
    if (!hit) return false; // can't determine â†’ assume visible
...
// div/span "buttons" SPA frameworks love) â€” but guard cursor:pointer so a
...
function isInteractive(el, tag, role, style) {
    // pointer-events:none means clicks pass through the element â€” not actionable
...
function walk(el, depth) {
    ...
    // Structural tags only earn a line if they're named â€” keeps the tree dense.
    ...
    if (include) {
        ...
        // Covered by an overlay/modal/sticky header â€” keep the ref (the agent
