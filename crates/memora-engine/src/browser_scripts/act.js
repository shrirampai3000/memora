...
// (`browser_act_script`) prepends three JSON-encoded consts â€” REF, ACTION,
// VALUE â€” so a page value like `"); evil()` can't break out of a string
...
// stamped â€” i.e. the model acts on exactly what it just saw, with no selector
...
if (!el) {
    return { ok: false, error: "ref '" + REF + "' not found â€” call /snapshot again to get fresh refs" };
}
...
function fillValue(node, val) {
    ...
    return { ok: false, error: "cannot fill <" + t + "> â€” not a text field, select, or contenteditable" };
}
