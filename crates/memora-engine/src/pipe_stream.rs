...
//!
//! * `GET  /pipes/:id/stream`      â€” the raw agent events, one SSE frame per line
//!
...
/// `GET /pipes/:id/stream` â€” raw agent events for the named pipe.
///
...
/// `POST /v1/chat/completions` â€” run a pipe through the OpenAI wire format.
///
...
pub async fn openai_chat_completions(
    ...
    if !req.stream {
        ...
        return Json(json!({
            "error": {
                "message": "set \"stream\": true â€” non-streaming pipe runs should use POST /pipes/{id}/run",
