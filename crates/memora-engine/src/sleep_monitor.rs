...
/// if no waiter is currently parked â€” the next `.notified().await` returns
...
/// Fired on every locked â†’ unlocked transition. Lets the monitor watcher retry
...
/// notify â€” otherwise they'd never wake on topology changes.
#[cfg(target_os = "macos")]
...
/// Also updates the shared flag in memora-config so other crates (e.g. audio) can read it.
pub fn set_screen_locked(locked: bool) {
    SCREEN_IS_LOCKED.store(locked, Ordering::SeqCst);
    memora_config::set_screen_locked(locked);
}
...
fn mark_recently_woke(platform: &'static str) {
    ...
    // Suppress permission-change emissions briefly â€” TCC / preflight APIs
...
/// `CGSSessionScreenIsLocked` key â€” this catches ALL lock methods
...
/// telemetry â€” users lock/unlock several times a day. System *wake* already
...
fn handle_screen_unlock_transition() {
    memora_screen::stream_invalidation::request();
    // Displays are routinely attached, detached or rearranged while locked.
    memora_screen::monitor::invalidate_monitor_lookup_cache();
    crate::permission_monitor::notify_wake();
...
pub fn start_sleep_monitor() {
    ...
    if initial_locked {
        info!("Screen is locked at startup â€” setting SCREEN_IS_LOCKED");
    ...
    // Uses com.apple.screenIsLocked / com.apple.screenIsUnlocked â€” event-driven, no polling.
    ...
    std::thread::spawn(|| {
        ...
        unsafe extern "C" fn on_screen_unlocked(
            ...
            if was_locked {
                ...
                // C boundary â€” this runs in a CFNotificationCenter callback.
        ...
        unsafe {
            ...
            // Run the CF run loop â€” blocks forever, delivers notifications.
            CFRunLoopRun();
        }
    });
    ...
    // Uses CGDisplayRegisterReconfigurationCallback â€” fires BEFORE and AFTER
    ...
    std::thread::spawn(|| {
        ...
        unsafe extern "C" fn on_display_reconfigured(
            ...
            // Display topology changed â€” invalidate cached SCStream handles
            ...
            #[cfg(target_os = "macos")]
            memora_screen::monitor::invalidate_monitor_lookup_cache();
            MEMORA_audio::stream_invalidation::request();
        ...
        unsafe {
            ...
            if err != 0 {
                // CGError != kCGErrorSuccess â€” log and continue without this watcher.
...
fn on_did_wake(handle: &tokio::runtime::Handle) {
    ...
    if was_locked && !locked {
        // CFNotification missed the unlock â€” we're fixing it here
    }
    ...
    handle.spawn(async {
        ...
        memora_db::request_write_resume();
...
mod tests {
    ...
    fn test_screen_is_locked_flag() {
        ...
        // suite while the screen is actually locked leaves it armed â€” reset
    ...
    async fn test_notify_one_semantics() {
        ...
        // Case 1: permit buffers when no waiter is parked â€” next notified()
        // returns immediately.
        notify.notify_one();
    ...
    async fn test_stale_permit_drain_then_block() {
        ...
        // no permit left â€” next wait must block
        let fresh =
    ...
    fn test_display_reconfig_callback_registered_default_false() {
        ...
        // stay false â€” forces monitor_watcher into the 5s fallback backstop.
    ...
    /// This is the core of the fix â€” when macOS fires the wake notification,
    ...
    async fn test_on_did_wake_sets_audio_invalidation() {
        let _guard = RECENTLY_WOKE_TEST_LOCK
            ...
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        // Clear stale flags
        let _ = MEMORA_audio::stream_invalidation::take();
        let _ = memora_screen::stream_invalidation::take();
        reset_recently_woke();
        ...
        assert!(
            MEMORA_audio::stream_invalidation::take(),
            "Audio stream invalidation flag must be set after wake"
        );
        assert!(
            memora_screen::stream_invalidation::take(),
    ...
    /// Run with: cargo test -p memora-engine --lib -- test_screen_lock_unlock --ignored --nocapture
    #[cfg(target_os = "macos")]
    #[tokio::test]
    ...
    async fn test_screen_lock_unlock() {
        ...
        assert!(
            saw_locked,
            "Screen lock was NOT detected â€” did you lock the screen?"
        );
        assert!(
