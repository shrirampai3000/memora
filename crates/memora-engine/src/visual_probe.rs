...
//! loop â€” no idle snapshots, no capture-attempt heartbeat â€” until user input
...
/// capture did not complete within `timeout` â€” the caller skips this tick
...
/// E2E fault injection: `MEMORA_E2E_SEED=visual-check-hang-once` makes the
...
/// probe timed out (SCK unresponsive) â€” callers skip this tick and keep the
...
pub(crate) async fn bounded_visual_probe(
    ...
    if matches!(result, Ok(None)) {
        ...
        if now_secs.saturating_sub(prev) >= 60 {
            ...
            warn!(
                "visual-change probe timed out after {:?} on monitor {} (ScreenCaptureKit unresponsive) â€” skipping check, capture loop stays live",
