...
//! the user's other AI assistants â€” Claude Code (`~/.claude/CLAUDE.md`),
//! the Codex CLI (`~/.codex/AGENTS.md`), and an Obsidian vault
//! (`<vault>/<folder>/MEMORA-memories.md`).
//!
//! ## Layering
//!
//! - The *rendering* + *file write* layer lives in
//!   `memora-core::memories::external_sync`. Pure, no DB, easy to
//!   unit-test.
//! - The two *destination definitions* (Claude Code, Codex) live in
//!   `memora-connect::connections::{claude_code, codex}`. They're
...
//!
//! Mirrors the shape of `memora_connect::oauth_refresh_scheduler`
//! deliberately â€” same start/stop/metrics/snapshot contract â€” so an
...
use anyhow::Result;
use memora_connect::connections::{load_connection, SavedConnection};
use memora_core::memories::external_sync::{
...
impl ExternalMemorySyncScheduler {
    ...
    /// Idempotent â€” calling twice keeps the first task running.
    pub fn start(
        &self,
        ...
        if self
            ...
            .is_err()
        {
            debug!("external memory sync: start called while already running â€” no-op");
            return;
        }
        ...
        let dir_clone = MEMORA_dir.clone();
    ...
    /// Cooperative shutdown â€” the spawned task exits at the next sleep
...
pub async fn run_once(
    ...
    // Load memories once and reuse across destinations â€” cheaper than
...
async fn sync_destination(
    ...
    let conn = load_connection(secret_store, MEMORA_dir, dest.id).await;
...
fn apply(
    ...
    // Destinations MEMORA owns end-to-end (e.g. an Obsidian vault note)
    // have no hand-edited content to preserve â€” write the full digest and
    ...
    if dest.owns_target {
        ...
        let entries_used = entries
            .len()
            .min(memora_core::memories::external_sync::MAX_ENTRIES_PER_DIGEST);
    ...
    // MEMORA-owned â€” no hand-edited content to preserve â€” so we
    // rewrite it whole.
...
fn resolve_claude_code_path(creds: &serde_json::Map<String, Value>) -> Result<PathBuf> {
    memora_connect::connections::claude_code::resolve_home_path(creds)
}

fn resolve_codex_path(creds: &serde_json::Map<String, Value>) -> Result<PathBuf> {
    memora_connect::connections::codex::resolve_home_path(creds)
}
...
fn resolve_obsidian_path(creds: &serde_json::Map<String, Value>) -> Result<PathBuf> {
    memora_connect::connections::obsidian_memories::resolve_target_dir(creds)
}
...
mod tests {
    use super::*;
    use memora_core::memories::external_sync::{marker_end, marker_start};
    use serde_json::json;
    ...
    fn apply_writes_block_into_destination_file() {
        ...
        match outcome {
            SyncOutcome::Wrote { path, entries: n } => {
                ...
                assert!(
                    outer.contains("@MEMORA-memories.md"),
                ...
                let sidecar = dir.path().join("MEMORA-memories.md");
    ...
    fn apply_reports_change_when_entries_shift() {
        ...
        // regardless of entry count â€” but the sidecar body changes when
        ...
        let sidecar = std::fs::read_to_string(dir.path().join("MEMORA-memories.md")).unwrap();
    ...
    fn apply_writes_owned_note_for_obsidian() {
        ...
        match outcome {
            SyncOutcome::Wrote { path, entries: n } => {
                assert_eq!(n, 1);
                assert!(path.ends_with("MEMORA-memories.md"));
                ...
                assert!(!dir.path().join("MEMORA-memories.md.sidecar").exists());
            }
            other => panic!("expected Wrote, got {:?}", other),
        }
    }
    ...
    async fn run_once_covers_obsidian_destination() {
        ...
        // just Claude Code + Codex â€” otherwise the app's per-destination UI
    ...
    async fn sync_destination_skips_when_connection_absent() {
        ...
        // No secret store, no legacy connections.json â€” the integration
    ...
    async fn sync_destination_skips_when_disabled() {
        ...
        // off â€” the scheduler should respect that and not write.
    ...
    async fn sync_destination_writes_when_enabled() {
        ...
        match result.outcome {
            Ok(SyncOutcome::Wrote { path, entries: n }) => {
                ...
                let outer = std::fs::read_to_string(&path).unwrap();
                assert!(outer.contains("@MEMORA-memories.md"));
                assert!(!outer.contains("first fact"));
                // The memory bodies land in the sidecar.
                let sidecar =
                    std::fs::read_to_string(target_dir.join("MEMORA-memories.md")).unwrap();
