...
//!   input within [`ACTIVE_WINDOW_SECS`]) â€” idle and asleep time are excluded.
...
//!
//! The non-recording active time splits into `active_stalled` (our fault â€” the
//! loop should have been capturing but wasn't) vs `active_paused` (the user, a
//! work-hours schedule, or DRM chose to stop capture â€” not a reliability
...
pub enum CoverageState {
    /// User active and capture healthy â€” the time we want to maximize.
    Recording,
    /// User active but capture stalled (DB writes stopped) â€” our fault.
    ActiveStalled,
    /// User active but capture intentionally paused (schedule / DRM / user) â€”
    /// not a reliability failure, excluded from "stalled".
    ActivePaused,
    /// No recent input â€” user away. Excluded from the coverage denominator.
    Idle,
}
...
/// Point-in-time snapshot of recording coverage (no private data â€” only
...
/// Current unix seconds (0 on a clock that predates the epoch â€” never in
/// practice).
fn now_unix() -> u64 {
    SystemTime::now()
...
fn classify_state(vision_metrics: &memora_screen::PipelineMetrics, now: u64) -> CoverageState {
    ...
    // advances last_db_write_ts) means the loop is alive. During warmup â€”
    // before the first frame lands â€” give the recorder a grace window so a
...
pub fn start_coverage_sampler(
    vision_metrics: std::sync::Arc<memora_screen::PipelineMetrics>,
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            // suspended between ticks. Don't attribute that gap to anything â€”
...
mod tests {
    ...
    fn paused_counts_toward_active_but_not_recording() {
        ...
        // paused is active time but not recording â†’ ratio = 30 / 100 = 0.3
        assert_eq!(s.active_secs, 100);
