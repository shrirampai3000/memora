...
//! URL, key) â€” the same ones the desktop app uses â€” so this works with a local
...
use axum::Json;
use memora_core::pipes::ResolvedPreset;
...
///
/// A preset's explicit `url` always wins â€” that is how custom gateways and a
...
pub fn base_url_for(preset: &ResolvedPreset) -> Option<String> {
    ...
    let provider = preset.provider.as_deref().unwrap_or("MEMORA-cloud");
    let base = match provider {
        "openai" | "openai-chatgpt" => "https://api.openai.com/v1",
        "native-ollama" | "ollama" => "http://localhost:11434/v1",
        // MEMORA-cloud and anything unknown have no local default: the
...
/// `POST /v1/chat/completions` â€” plain chat, no pipe.
///
...
pub async fn local_chat_completions(
    ...
    let Some(preset) = preset else {
        return error(
            ...
            "no ai preset configured â€” set one in the app's AI settings",
        );
    };
...
pub type SharedManager = Arc<Mutex<memora_core::pipes::PipeManager>>;
...
mod tests {
    ...
    fn unknown_provider_without_url_is_refused_rather_than_guessed() {
        ...
        assert!(base_url_for(&preset(Some("MEMORA-cloud"), None)).is_none());
