...
use memora_screen::monitor::{get_monitor_by_id, list_monitors};
use memora_screen::PipelineMetrics;
...
/// displays". Deliberately says nothing about the cause â€” see the emit site.
...
pub struct VisionManagerConfig {
    ...
    /// Empty means no explicit selection â€” honour `use_all_monitors` instead.
    ...
    /// snapshot max width via `memora_core::video::*`. Values: "low",
...
pub(crate) struct MonitorLivenessSnapshot {
    ...
    /// ago" (see [`memora_screen::CaptureLoopStage`]).
    pub loop_stage: memora_screen::CaptureLoopStage,
...
pub struct VisionManager {
    ...
    /// Broadcast sender for capture triggers â€” shared with UI recorder.
    /// Each monitor subscribes via `trigger_tx.subscribe()`.
    trigger_tx: TriggerSender,
    /// Sender for the frame-linker actor â€” shared with UI recorder and
    ...
    /// Hot frame cache â€” capture pushes frames here for zero-DB timeline reads.
    hot_frame_cache: Option<Arc<HotFrameCache>>,
    /// Power profile receiver â€” each monitor gets a clone.
    power_profile_rx: Option<watch::Receiver<PowerProfile>>,
    /// Focus-aware capture controller â€” always constructed.
    ...
    /// sessions only â€” see `HighFpsController`). `None` means the
    ...
    /// resumes it â€” mirrors the audio manager's user-disabled device set.
    user_disabled: Arc<DashSet<u32>>,
    /// Connected displays selected for recording, including user-paused ones.
    /// Keeping intent separate from active tasks lets health distinguish â€œall
    /// displays intentionally pausedâ€ from â€œone paused display plus one dead
    /// capture task.â€
    expected_monitors: Arc<StdRwLock<HashSet<u32>>>,
    /// Watcher-owned answer to â€œshould any selected display be producing
    /// frames now?â€ False for all-user-paused or all-displays-asleep states.
...
impl VisionManager {
    ...
    pub fn new(
        ...
        // Focus-aware capture is always on. `new_tracker()` always succeeds â€”
    ...
    /// disable frame buffering (e.g. when the timeline feature is disabled â€” the
    ...
    pub fn is_monitor_allowed(&self, monitor: &memora_screen::monitor::SafeMonitor) -> bool {
        if self
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        *status = VisionManagerStatus::Running;
        ...
        if task_count == 0 && total_monitors > 0 && !self.config.use_all_monitors {
            warn!(
                "VisionManager: allowlist {:?} matched 0/{} display(s) â€” \
        ...
        if task_count == 0 {
            ...
            *self.status.write().await = VisionManagerStatus::Stopped;
            ...
            // vec â€” enumeration timeout, retry budget exhausted, a genuinely
            // asleep/clamshell display â€” so a revoked grant is only ONE of the
            ...
            if total_monitors == 0 {
                warn!(
                    "VisionManager: {} â€” display asleep/clamshell, enumeration failure, or \
            ...
            warn!(
                "VisionManager: no monitors matched the allowed list \
                 ({} enumerated, 0 started) â€” stale monitor_ids?",
                total_monitors
            );
            return Err(anyhow::anyhow!(
    ...
    pub async fn start_monitor(&self, monitor_id: u32) -> Result<()> {
        ...
        // enumeration admission budget in memora-screen.
    ...
    pub(crate) async fn start_monitor_handle(
        &self,
        monitor: memora_screen::monitor::SafeMonitor,
    ) -> Result<()> {
        let monitor_id = monitor.id();
    ...
    async fn start_event_driven_monitor(
        ...
        // single `videoQuality` setting via `memora_core::video` â€” that
        ...
        #[cfg(target_os = "macos")]
        memora_screen::monitor::set_sck_capture_max_width(max_snapshot_width);

        // Create activity feed for this monitor
        let activity_feed = ActivityFeed::new();

        // Tree walker config â€” pass through user's ignored/included window filters
        ...
        // Event-driven capture config â€” seed jpeg_quality from the user's
    ...
    pub async fn stop_monitor(&self, monitor_id: u32) -> Result<()> {
        ...
        if let Some((_, handle)) = self.recording_tasks.remove(&monitor_id) {
            ...
            // Wait for it to finish with a timeout â€” if the capture task is stuck
    ...
    /// (the recording popover). Records the intent first â€” so the monitor
    /// watcher won't auto-restart it on the next reconcile tick â€” then tears
...
mod tests {
    ...
    async fn start_with_stale_allowlist_falls_back_to_all_monitors() {
        ...
        if monitors.is_empty() {
            // Headless CI â€” nothing to fall back to.
            return;
        }
    ...
    async fn start_with_no_connected_monitors_returns_err() {
        let monitors = list_monitors().await;
        if !monitors.is_empty() {
            // Needs a headless environment â€” skip on dev machines with displays.
            return;
        }
    ...
    async fn user_pause_blocks_start_until_resumed() {
        ...
        // start_monitor is a no-op while paused â€” returns Ok, starts no task.
        ...
        // fake id has no monitor â€” the flag clear is what we assert here).
    ...
    /// not spawn capture â€” mirrors audio `resume_device` when capture is off.
    #[tokio::test]
    ...
    /// Verify that stop_monitor doesn't hang on a slow task â€” the 3s timeout fires.
    #[tokio::test]
    async fn test_stop_monitor_timeout_on_slow_task() {
        ...
        if let Some((_, handle)) = tasks.remove(&1) {
            ...
            // The result is either Ok (abort completed) or Err (timeout). Either is acceptable â€”
