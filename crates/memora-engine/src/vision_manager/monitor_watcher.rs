...
use memora_screen::monitor::{list_monitors_detailed, MonitorListError};
...
// â”€â”€ Silent-wedge watchdog (#3939) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
// Symptom: VisionManager status stays `Running` and the tray shows "Recording",
// but the capture loop has stopped reaching terminal outcomes â€” for ~1h in the
// original report â€” with no auto-recovery. The existing recovery paths all miss
...
// long time" cannot happen on a healthy idle screen â€” it only happens when
...
/// attempts stay fresh â†’ wedged. Deliberately looser than the /health stall
...
// â”€â”€ Gone-silent / dead-loop watchdog â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
// even attempting â€” the macOS ScreenCaptureKit stream froze and the loop is
...
// falls through all of them â€” the app would show "recording" for hours with no
...
/// stale â†’ gone-silent. Higher than `WEDGE_DB_STALE_SECS` because we cannot
...
/// machine â€” shortened so recovery beats the desktop's own incident alert
...
/// the db-stale clock while landing far below the 150s alert â€” a loop that
...
/// reaching a single terminal outcome â†’ started-but-never-produced. Generous so
...
fn vision_watchdog_config() -> VisionWatchdogConfig {
    ...
    if crate::event_driven_capture::e2e_capture_loop_silent_fault_started() {
        // Fast enough to exercise the complete failure â†’ visible incident â†’
...
/// Pure decision: has vision capture gone fully silent â€” status would be
...
fn vision_capture_silent(
    ...
    // No presence signal â†’ idle tier, the conservative window. Attended
...
///
/// Reuses the UI recorder's event clock â€” the same source
/// `recording_coverage` classifies active/idle from â€” so the watchdog needs no
...
struct DetectedVisionStall {
    ...
    last_capture_loop_heartbeat_ts: u64,
    last_db_write_ts: u64,
    loop_stage: memora_screen::CaptureLoopStage,
...
/// Pure step function for the anomalous-empty-enumeration counter â€” how many
...
///
/// - `screen_locked` â†’ a locked session yields an empty SCK list with the
...
/// - `Some(non-empty)` capture-ready set â†’ awake displays exist yet SCK sees
///   none: genuinely anomalous, count it.
/// - `Some(empty)` â†’ every display is asleep/inactive (clamshell, Energy-Saver
...
/// - `None` â†’ CG topology unreadable = unknown; callers MUST NOT infer a
///   degraded topology from it (same discipline as the audio SCK watchdog's
///   `usable_display_ids`), so hold the count â€” neither escalate toward the
...
/// the steady-state arm at all â€” `start()` fails, the loop parks in the retry
...
fn classify_empty_enumeration(counter: &mut u32, denied_logged: &mut bool, site: &str) -> bool {
    ...
    let capture_ready = if cfg!(target_os = "macos") {
        memora_core::display_topology::capture_ready_display_ids()
    } else {
        None
    };
    *counter = next_anomalous_empty_count(*counter, capture_ready.as_ref(), screen_locked);
    ...
    if !*denied_logged {
        warn!(
            "SCK enumerated no displays {} times in a row ({}) while CoreGraphics reports {} \
             awake display(s) â€” screen recording permission is likely revoked or awaiting \
    ...
    // a delivered report â€” so repeating is what makes the episode survive.
...
/// At the surrounding back-off cadence this is ~15s â€” long enough that a
...
/// Canonical JSON snapshot of the display arrangement â€” displays sorted by
/// runtime id, stable field order â€” so change detection is a string compare
...
/// reported by the capture backend (SCK reports pixels, xcap logical points â€”
...
/// (and thus to per-monitor-normalized element-tree bounds) â€” historically
/// unrecorded, which capped clickâ†”tree coordinate matching at ~25% accuracy.
pub(crate) fn canonical_display_layout_json(
    displays: &mut [(u32, String, memora_screen::monitor::MonitorData)],
) -> String {
...
pub async fn start_monitor_watcher(
    ...
    let handle = tokio::spawn(async move {
        ...
        // permission problem â€” display sleep, clamshell and screen lock all
        // produce it legitimately (empirically verified on macOS 26.4: granted
        // + display asleep â†’ SCShareableContent OK with 0 displays; see PR
        // #5393 review). But CG topology is readable without the
        // screen-recording grant, so "CG sees awake displays, SCK persistently
        // sees none" is the anomalous state â€” the silent capture-loss shape
        ...
        // arrangement writes nothing. Diffed on the FULL geometry â€” not the
        // id set â€” so re-arrangements and resolution changes (same ids) are
        ...
        match list_monitors_detailed().await {
            Ok(monitors) => {
                ...
                // it must be reported from here too â€” not just from the loop.
        ...
        loop {
            // â”€â”€ DRM pause handling â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            if drm_detector::drm_content_paused() {
                if !drm_stopped {
                    info!(
                        "DRM content focused â€” stopping all vision monitors to release SCK handles"
                    );
                ...
                // DRM cleared â€” fall through to restart below
            }

            if drm_stopped {
                info!("DRM content no longer focused â€” restarting vision monitors");
            ...
            // â”€â”€ Schedule pause handling â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            if crate::schedule_monitor::schedule_paused() {
                if !schedule_stopped {
                    info!("outside work-hours schedule â€” stopping all capture");
                    if let Err(e) = vision_manager.stop().await {
                        warn!("failed to stop vision manager for schedule pause: {:?}", e);
                    }
                    schedule_stopped = true;
                }
                // Check every 30 s â€” matches the schedule monitor's own cadence.
                tokio::time::sleep(Duration::from_secs(30)).await;
                continue;
            }

            if schedule_stopped {
                info!("within work-hours schedule â€” resuming capture");
                if let Err(e) = vision_manager.start().await {
                    warn!(
            ...
            // â”€â”€ Normal monitor polling â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            if vision_manager.status().await != VisionManagerStatus::Running {
                #[cfg(target_os = "macos")]
                {
                    ...
                    if tokio::time::timeout(Duration::from_secs(5), unlock.notified())
                        ...
                        .is_ok()
                    {
                        info!("screen unlocked â€” retrying VisionManager start immediately");
                    }
                }
                #[cfg(not(target_os = "macos"))]
            ...
            let current_monitors = match list_monitors_detailed().await {
                Ok(monitors) => {
                    ...
                    // latched â€” permanently denying screen recording for the
            ...
            // â”€â”€ Capture stall watchdog (#3939) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                if let Some(stall) = stall.filter(|_| cooldown_ok) {
                    ...
                    warn!(
                        "vision capture stalled on monitor {} ({}): status=Running, {}, loop heartbeat {}s ago, \
                         last attempt {}s ago, no terminal capture outcome for {}s â€” restarting VisionManager (#3939)",
                        stall.monitor_id, reason, stage_detail, loop_heartbeat_age, attempt_age, db_stale
                    );
                    let _ = memora_events::send_event(
            ...
            // (origins/sizes/primary â€” not just the id set). Cheap when
            // unchanged: one string compare against the last persisted value.
            {
                let mut geo: Vec<(u32, String, memora_screen::monitor::MonitorData)> =
                    current_monitors
                        .iter()
            ...
            // boot, not a hot-plug â€” suppress the notification so the user doesn't
            ...
            for monitor in &current_monitors {
                ...
                if !active_ids.contains(&monitor_id) {
                    if !vision_manager.is_monitor_allowed(monitor) {
                        debug!(
                            "Skipping monitor {} ({}) â€” not in allowed list",
                            monitor_id,
                            monitor.stable_id()
                        );
                        continue;
                    ...
                    if vision_manager.is_monitor_user_disabled(monitor_id) {
                        debug!(
                            "Skipping monitor {} â€” user-paused from recording popover",
                            monitor_id
                        );
                        continue;
                    }
            ...
            for monitor_id in &active_ids {
                if !current_ids.contains(monitor_id) {
                    ...
                    match vision_manager.stop_monitor(*monitor_id).await {
                        Ok(()) => {
                            // Use the last-known name for this id â€” the OS no
...
mod tests {
    use super::*;
    use memora_screen::monitor::MonitorData;
    ...
    fn anomalous_empty_counter_only_advances_with_awake_unlocked_displays() {
        ...
        // threshold â€” the state that ran silent for whole days in the wild.
        ...
        // review's macOS 26.4 probe demonstrated (granted + display asleep â†’
        // SCShareableContent OK with 0 displays) â€” it must NEVER escalate.
        ...
        // Non-macOS always passes None â†’ counter can never leave 0.
        assert_eq!(next_anomalous_empty_count(0, None, false), 0);
    }

    #[test]
    fn healthy_recent_write_is_not_wedged() {
        // Wrote 1s ago, attempting 1s ago â†’ clearly healthy.
    ...
    fn gone_silent_or_idle_is_not_restartable_here() {
        // Attempts stale (loop idle, or blocked in sync FFI) â†’ NOT the
    ...
    fn thresholds_are_respected() {
        // db stale 119s (< 120s) â†’ hold off a little longer.
        assert!(!vision_capture_wedged(600.0, NOW - 3, NOW - 119, NOW));
        // db stale 121s (> 120s) â†’ trip.
        assert!(vision_capture_wedged(600.0, NOW - 3, NOW - 121, NOW));
        // attempt 60s ago is not "< 60s fresh" â†’ treat as not actively attempting.
        assert!(!vision_capture_wedged(600.0, NOW - 60, NOW - 200, NOW));
    }

    // â”€â”€ vision_capture_silent (gone-silent / never-produced) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    #[test]
    fn healthy_or_static_screen_is_not_silent() {
        // Wrote + attempted 1s ago â†’ healthy.
        ...
        // Static screen: dedup-skip keeps last_db_write_ts fresh â†’ not silent
        // even after an hour.
        assert!(!vision_capture_silent(
            3600.0,
            NOW - 2,
    ...
    fn silent_respects_warmup_and_thresholds() {
        // Below the warm-up uptime floor â†’ never silent.
        assert!(!vision_capture_silent(
            30.0,
            NOW - 100,
            NOW - 100,
        ...
        // Wrote once; gone 239s (< 240s) with attempts stopped â†’ hold off.
        assert!(!vision_capture_silent(
            600.0,
            NOW - 100,
            NOW - 239,
        ...
        // never-produced but only 200s uptime (< 240s) â†’ still warming up.
    ...
    fn silent_requires_the_loop_to_have_attempted() {
        ...
        // could restart-loop â†’ not silent.
    ...
    fn detected_stall_reports_the_stage_the_loop_froze_in() {
        let snapshots = [MonitorLivenessSnapshot {
            ...
            loop_stage: memora_screen::CaptureLoopStage::VisualProbe,
    ...
    fn one_healthy_monitor_cannot_mask_a_silent_sibling() {
        let snapshots = [
            MonitorLivenessSnapshot {
                ...
                loop_stage: memora_screen::CaptureLoopStage::TriggerWait,
    ...
    fn layout_json_is_canonical_and_sorted_by_id() {
        // deliberately out of id order â€” canonical form must sort
        let mut a = vec![
            (
                7,
