...
use chrono::Local;
use reqwest::Client;
use memora_resource::{ProcessBreakdown, ResourceSampler, ResourceSnapshot};
...
// A leak that users actually feel ("MEMORA ate 10GB overnight") is slow
// and steady â€” hundreds of MB per hour for hours â€” which per-sample telemetry
...
// numbers (slope, rÂ², window) â€” no user content, no new infra. The current
// slope/rÂ² also ride on every `resource_usage` event so PostHog can trend
...
/// ...and has enough points for the fit to mean anything (30s cadence â†’ 3h â‰ˆ 360).
const LEAK_MIN_SAMPLES: usize = 30;
/// Alert thresholds: steady (rÂ²) climb of â‰¥150 MB/h that has already added
/// â‰¥0.5 GB and pushed us past 1.5 GB â€” i.e. on its way to a user-visible blowup.
...
impl LeakStats {
    /// Growth the *fit* predicts over the window â€” robust to endpoint noise,
...
/// Pure function â€” unit-tested against leak/flat/spike/sawtooth shapes.
fn analyze_memory_trend(samples: &[(f64, f64)]) -> Option<LeakStats> {
    ...
    // Perfectly flat memory â†’ syyâ‰ˆ0 â†’ define rÂ²=0 (slope is ~0 anyway).
...
/// governance live in `memora-resource`; this type owns only reporting,
...
pub struct ResourceTelemetryReporter {
    ...
    /// One leak alert per process lifetime â€” a leaking process keeps leaking;
...
/// governance in `memora-resource`.
pub type ResourceMonitor = ResourceTelemetryReporter;

/// Static host info collected once at startup.
/// Only contains general OS/model names â€” no serial numbers, UUIDs, or PII.
#[derive(Clone, Debug)]
struct HardwareInfo {
    cpu_brand: String,
    cpu_arch: String,
...
impl HardwareInfo {
    fn collect() -> Self {
        ...
        // single `refresh_cpu()` â€” that flake reported cpu_count=0 on ~99% of
...
/// Never panics â€” returns an empty vec on any failure.
...
impl ResourceTelemetryReporter {
    pub fn new(telemetry_enabled: bool) -> Arc<Self> {
        ...
        // Launcher-provided id when present, else the stable per-machine id â€”
        // a fresh UUID per process start counted each run as a new user.
        let distinct_id = TelemetryContext::distinct_id();

        // Collect host info once (OS, CPU, GPU names) â€” never panics
    ...
    /// the regression over â‰¤`LEAK_MAX_SAMPLES` points is microseconds.
    fn record_leak_sample(&self, runtime_secs: f64, mem_gb: f64) -> Option<LeakStats> {
                ...
                .swap(true, std::sync::atomic::Ordering::SeqCst)
        {
            ...
            warn!(
                "memory leak suspected: +{:.0} MB/h over {:.1}h (rÂ²={:.2}, {:.2}â†’{:.2} GB)",
                stats.slope_mb_per_h, stats.span_hours, stats.r2, stats.start_gb, stats.end_gb
            );
            warn!(
                "memory leak suspected: +{:.0} MB/h over {:.1}h (rÂ²={:.2}, {:.2} GB â†’ {:.2} GB)",
                stats.slope_mb_per_h, stats.span_hours, stats.r2, stats.start_gb, stats.end_gb
            );
        }
        Some(stats)
    ...
    async fn send_to_posthog(
        ...
        // Activity-Monitor "Memory" â€” surfaces cold/compressed/swapped leaks
        // that resident set size (`total_memory_gb`) hides. See
        // `memora-resource` obtains this from `proc_pid_rusage` on macOS.
        properties.insert(
        ...
        // Memory-trend fit from the leak sentinel â€” lets PostHog chart fleet
        // leak-slope per release (p90 of mem_slope_mb_per_h regressing =
        // a leak shipped). Absent until the rolling window spans â‰¥3h.
    ...
    fn log_process_breakdown(breakdown: &ProcessBreakdown) {
        ...
        } else if env::var("memora_resource_BREAKDOWN").is_ok()
    ...
    async fn log_status_local(&self, snapshot: &ResourceSnapshot) {
        ...
        // Every tick feeds the leak sentinel â€” alerting must not depend on
...
mod tests {
    ...
    /// Steady +200 MB/h from 1 GB over 4h â€” the textbook leak. Must trigger,
    ...
    fn linear_leak_is_detected() {
        ...
        assert!(stats.r2 > 0.99, "perfect line should fit, rÂ²={}", stats.r2);
        assert!(stats.is_leak());
    }

    /// Flat memory with sampling jitter â€” healthy steady state. No alert.
    #[test]
    ...
    /// Slope over the window is ~0 â€” must not alert.
    #[test]
    ...
    /// Sawtooth (grow, GC/flush back down, repeat) â€” high churn, no net leak.
    /// The regression sees a poor linear fit (low rÂ²) â€” must not alert.
    #[test]
    ...
    /// Too little data to judge: short span or too few points â†’ None.
    #[test]
    ...
    /// Growth that is steady but tiny (30 MB/h) â€” below the alert bar even
