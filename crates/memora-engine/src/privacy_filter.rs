...
//!   github.com/MEMORA/privacy-filter and pins TLS to the attested key,
...
//!   tabs, IDE panes, the same email thread) â€” caching typically cuts the
//!   Tinfoil round-trip count by 5-10Ã— during an active session.
...
/// Cache size â€” tuned for ~20 results per search Ã— ~50 recent queries.
...
impl PrivacyFilter {
    ...
    /// Any single failure fails the whole batch â€” caller should surface
...
/// `MEMORA_PRIVACY_FILTER_ENCLAVE` / `MEMORA_PRIVACY_FILTER_REPO`,
/// defaulting to the MEMORA-published enclave + its source repo so that
/// Tinfoil's measurement comparison ties this client to the exact open
/// source code at github.com/MEMORA/privacy-filter.
pub fn global() -> Arc<PrivacyFilter> {
    INSTANCE
        .get_or_init(|| {
            let enclave = std::env::var("MEMORA_PRIVACY_FILTER_ENCLAVE")
                .unwrap_or_else(|_| DEFAULT_ENCLAVE.to_string());
            let repo = std::env::var("MEMORA_PRIVACY_FILTER_REPO")
...
mod tests {
    ...
    async fn short_text_bypasses_round_trip() {
        // Using a bogus URL â€” the assertion is that the call never happens
        // for under-threshold input, so it doesn't matter that the URL is
        // unreachable.
        // These tests never reach the network â€” they exercise the cache /
