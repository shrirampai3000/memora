...
//! The local daemon: the `MEMORA` binary, the localhost HTTP API, and the
...
//!
//! - `routes/` â€” the localhost HTTP surface (search, health, pipes, meetings).
//!   Everything the desktop app and MCP server call lands here.
//! - `cli/` â€” argument parsing and startup for the `MEMORA` binary.
//! - `meeting_watcher/` â€” meeting start/stop detection and the UI scan behind it.
//! - `vision_manager/` â€” owns the screen-capture lifecycle and its watchdog.
//! - `power/`, `focus_tracker/` â€” sleep, lock, and foreground-app signals that
//!   gate whether capture should be running at all.
//!
//! The flat files at `src/` root are single-concern supervisors and detectors
//! (`disk_pressure`, `drm_detector`, `frame_linker`, `hd_recorder`, â€¦). Grep by
...
//! - All database writes go through `memora-db`, which serializes through
//!   `memora-sqlite-coordinator`. Do not open your own pool.
//!
...
pub mod ui_recorder;
// Exposed publicly so the commercial `MEMORA-sdk` (MEMORA/sdk repo)
