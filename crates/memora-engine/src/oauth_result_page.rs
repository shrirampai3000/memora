...
pub(crate) fn render_oauth_result_page(
    ...
    let hint = if ok {
        "This tab will try to close automatically. You can return to MEMORA."
    } else {
        "Return to MEMORA and try connecting again."
    };
...
<body data-tone="{tone}" data-auto-close="{auto_close}">
  <main class="stage">
    <div class="brand"><span class="brand-mark" aria-hidden="true">{brand_icon}</span><span>MEMORA</span></div>
...
fn MEMORA_brand_icon() -> &'static str {
