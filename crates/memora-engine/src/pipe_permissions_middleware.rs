...
//!
//!   **deny â†’ allow â†’ default allowlist â†’ reject**
//!
//! This is a backstop layer â€” the primary enforcement happens in the Pi
//! extension (`MEMORA-permissions.ts`). The middleware validates pipe
...
use dashmap::DashMap;
use memora_core::pipes::permissions::{PipePermissions, PipeTokenRegistry};
use std::sync::Arc;

use crate::server::AppState;

/// Wrapper that implements PipeTokenRegistry for the server's DashMap.
/// Passed to PipeManager so it can register/remove tokens from memora-core.
...
pub async fn pipe_permissions_layer(
    ...
    if let Some(ref token) = token {
        ...
        match state.pipe_permissions.get(token) {
            Some(entry) => {
                ...
                if !perms.is_endpoint_allowed(&method, &path) {
                    return (
                        ...
                        format!(
                            "pipe \"{}\" is not permitted to call {} {} â€” \
            ...
            None => {
                // Token not in registry â€” likely hallucinated by the model.
    ...
    // No token = user/app request â€” no restrictions (backwards compatible)
    next.run(req).await
}
...
pub async fn pipe_backpressure_layer(
    ...
    // Try to acquire a permit â€” don't block, fail fast
        ...
        Err(_) => {
            ...
            return (
                ...
                axum::Json(serde_json::json!({
                    "error": "too many concurrent pipe queries â€” recording takes priority, retry later",
                    "retry_after_ms": 1000
                })),
            )
                .into_response();
    ...
    match response {
        ...
        Err(_) => {
            tracing::warn!(
                pipe = %pipe_name,
                "pipe query timed out after 30s â€” killed to protect recording"
            );
            (
