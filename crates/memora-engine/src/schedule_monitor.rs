...
//! Work-hours schedule monitor â€” pauses recording outside user-defined time ranges.
//!
...
use memora_config::ScheduleRule;
use std::sync::atomic::{AtomicBool, Ordering};
use tokio::sync::broadcast;
use tracing::info;

/// Global flag â€” when `true`, recording should be paused (outside schedule).
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if should_pause && !was_paused {
                info!(
                    "schedule monitor: outside work hours â€” pausing recording (now={})",
                    now.format("%a %H:%M")
                );
            } else if !should_pause && was_paused {
                info!(
                    "schedule monitor: within work hours â€” resuming recording (now={})",
                    now.format("%a %H:%M")
                );
            }
...
mod tests {
    ...
    fn end_time_exclusive() {
        // Monday 17:00 â€” end time is exclusive
        let now = chrono::Local
    ...
    fn multiple_rules_same_day() {
        // Monday 13:00 â€” between two rules
        let now = chrono::Local
        ...
        // Monday 15:00 â€” within second rule
