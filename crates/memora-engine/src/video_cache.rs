...
use memora_db::{DatabaseManager, FrameData, OCREntry};
...
impl From<memora_db::AudioEntry> for AudioEntry {
    fn from(db_entry: memora_db::AudioEntry) -> Self {
        Self {
...
impl FrameCache {
    pub async fn new(MEMORA_dir: PathBuf, db: Arc<DatabaseManager>) -> Result<Self> {
        let cache_config = CacheConfig {
            cache_dir: cache_dir().unwrap().join("MEMORA").join("frames"),
            ..Default::default()
        };
...
/// requested ordinal â€” but that only holds when FFmpeg produced exactly one
...
async fn is_video_file_complete(ffmpeg_path: &PathBuf, file_path: &str) -> Result<bool> {
    ...
    let mut cmd = memora_core::ffmpeg_cmd_async(ffmpeg_path);
