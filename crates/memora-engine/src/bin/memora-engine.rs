...
use memora_core::agents::AgentExecutor;
use memora_core::find_ffmpeg_path;
use memora_core::paths;
...
use memora_screen::monitor::list_monitors;
use serde_json::json;
use std::{
    env, fs,
...
async fn doctor_check_system_audio_capture() -> bool {
    ...
    match get_cpal_device_and_config(&device).await {
        ...
        Err(e) => {
            let terminal = memora_core::permissions::detect_terminal();
            eprintln!("  system audio capture: failed");
            eprintln!("    ScreenCaptureKit could not open System Audio for this CLI session.");
            eprintln!(
                "    macOS grants CLI capture through the terminal app, not the MEMORA app."
            );
            eprintln!(
            ...
            eprintln!(
                "    if you only need a workaround: MEMORA record --experimental-coreaudio-system-audio"
            );
...
fn get_base_dir(custom_path: &Option<String>) -> anyhow::Result<PathBuf> {
    let default_path = paths::default_MEMORA_data_dir();
    ...
    // starting â€” capture is the job, the exclusion is hardening around it.
...
fn setup_logging(
    ...
    let file_appender = memora_engine::logging::SizedRollingWriter::builder()
    ...
    let make_env_filter = || {
        let filter = EnvFilter::from_default_env()
            ...
            // ORT (ONNX Runtime) is extremely chatty at INFO â€” emits hundreds
        ...
        // Silence on every platform â€” was previously windows-only, but the
        ...
        if debug {
            filter.add_directive("MEMORA=debug".parse().unwrap())
        } else {
            filter
        }
    };
...
async fn main() -> anyhow::Result<()> {
    // dhat heap profiler â€” must be the first thing in main.
    ...
    debug!("starting MEMORA server");
    let matches = Cli::command().get_matches();
    let record_arg_sources = RecordArgSources::from_cli_matches(&matches);
    let cli = Cli::from_arg_matches(&matches)?;

    // Dispatch subcommands â€” non-recording commands return early
    let record_args = match cli.command {
        ...
        Command::Audio { .. } => {
            eprintln!("Audio capture and management are removed in Memora.");
            return Ok(());
        }
        ...
        Command::Agent { ref subcommand } => {
            memora_engine::cli::agent::handle_agent_command(subcommand).await?;
            return Ok(());
        }
        Command::Setup { ref api_url } => {
            memora_engine::cli::agent::setup_all_detected(api_url)?;
            return Ok(());
        }
        Command::Service { ref subcommand } => {
            memora_engine::cli::service::handle_service_command(subcommand).await?;
            return Ok(());
        }
        Command::Connection { ref subcommand } => {
            memora_engine::cli::connection::handle_connection_command(subcommand).await?;
            return Ok(());
        }
        Command::Vault { ref subcommand } => {
            memora_engine::cli::vault::handle_vault_command(subcommand).await?;
            return Ok(());
        }
        Command::Install {
            ref url,
            ...
            memora_engine::cli::install::handle_install(url, allow_untrusted).await?;
            return Ok(());
        }
        Command::Login => {
            memora_engine::cli::login::handle_login_command().await?;
            return Ok(());
        }
        Command::Logout => {
            memora_engine::cli::login::handle_logout_command().await?;
            return Ok(());
        }
        Command::Whoami => {
            memora_engine::cli::login::handle_whoami_command().await?;
            return Ok(());
        }
        Command::Survey => {
            memora_engine::cli::survey::handle_survey_command().await?;
            return Ok(());
        }
        Command::Auth { ref subcommand } => {
            memora_engine::cli::auth::handle_auth_command(subcommand).await?;
            return Ok(());
        }
        Command::Db { ref subcommand } => {
            memora_engine::cli::db::handle_db_command(subcommand).await?;
            return Ok(());
        }
        Command::Backup {
            ref subcommand,
            ...
            memora_engine::cli::backup::handle_backup_command(subcommand, data_dir).await?;
            return Ok(());
        }
        Command::Export(ref args) => {
            memora_engine::cli::export::handle_export_command(args).await?;
            return Ok(());
        }
        Command::Doctor => {
            eprintln!("MEMORA doctor");
            eprintln!("=================");
            eprintln!();

            // permissions
            eprintln!("permissions:");
            let perms_ok = memora_core::permissions::preflight_check(true, true);
            ...
            if port_ok {
                ...
                eprintln!("  port 3030: in use (MEMORA may already be running)");
                eprintln!("    stop the existing process or run record with --port <PORT>");
            }
            eprintln!();

            if perms_ok && system_audio_ok && ffmpeg_ok && port_ok {
                eprintln!("all checks passed â€” ready to record");
            } else {
                eprintln!("some checks failed â€” fix the issues above before running MEMORA");
                std::process::exit(1);
            }
            return Ok(());
        }
        Command::Diagnose {
            ...
            let local_data_dir = get_base_dir(data_dir)?;
            memora_engine::cli::diagnose::handle_diagnose_command(
                &local_data_dir,
                message.as_deref(),
                port,
                dry_run,
    ...
    memora_engine::cli::agent::maybe_prompt_connect_detected(&local_data_dir);
    ...
    if config.analytics_enabled && memora_engine::analytics::telemetry_disabled_by_env() {
    ...
    memora_connect::mdns::set_enabled(record_args.enable_mdns);
    ...
    if let Err(e) = memora_engine::power::set_keep_awake(config.keep_computer_awake) {
        warn!("failed to apply keep-awake setting: {}", e);
    }

    // Yield CPU to foreground apps (Windows: BELOW_NORMAL priority class).
    // CLI-record only â€” the desktop app gets targeted lowering of background
    // work instead; see the process_priority module doc. Placed before any
    // child spawns so ffmpeg/pipe children inherit the class.
    memora_engine::process_priority::set_background_priority();

    // Non-blocking update check â€” runs in background, prints banner if outdated
    tokio::spawn(async {
        memora_engine::cli_reminder::check_for_updates().await;
    });

    // Periodic terminal nudge to install the desktop app (CLI-only).
    memora_engine::cli_reminder::spawn();

    // Telemetry / Sentry disabled for Memora local-first architecture
    ...
    // only the exit code â€” never *why*. Install a panic hook that writes the
    ...
    // apps/memora-app-tauri/src-tauri/src/main.rs.
    {
        // Write to the resolved data dir (honors --data-dir) so the crash log
        // sits next to MEMORA.log, and an embedder running with its own
        // --data-dir doesn't collide with the desktop app's
        // ~/.memora/last-panic.log (the app runs its engine in-process and
        // owns that file).
        ...
        // Reuse the existing embedder attribution (MEMORA_EMBEDDER /
        // MEMORA_CUSTOMER_ID / ...) so the local crash record is identifiable
        ...
        let attribution = {
            use memora_engine::telemetry_context::TelemetryContext;
        ...
        std::panic::set_hook(Box::new(move |info| {
            // stderr first â€” the embedding parent usually pipes the child's
            // stderr, and unwinding into an extern "C" frame can turn into
            // panic_cannot_unwind â†’ abort() and drop everything after this.
            eprintln!("PANIC: {}", info);
            ...
            // crash â€” don't record it where it would skew crash dashboards or
            ...
            // force_capture ignores RUST_BACKTRACE â€” we always want the trace.
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            loop {
                ...
                if !printed_waiting {
                    ...
                    if !screen_ok {
                        eprintln!(
                            "  screen recording: waiting â€” grant access to \"{}\"",
                            terminal
                        );
                    }
                    if !audio_ok {
                        eprintln!("  microphone: waiting â€” grant access to \"{}\"", terminal);
                    }
    ...
    if !is_local_ipv4_port_free(config.port) {
        error!(
            "you're likely already running MEMORA instance in a different environment, e.g. terminal/ide, close it and restart or use different port"
        );
        return Err(anyhow::anyhow!("port already in use"));
    }

    // Only enumerate monitors when vision is enabled â€” on macOS, calling
    ...
    // NOTE: must be started AFTER database init â€” the monitor spawns background
    ...
    // Start the permission monitor â€” polls OS permission state and emits
    ...
    let _permission_monitor_handle = memora_engine::permission_monitor::start();
    ...
    let monitor_ids: Vec<u32> = if config.use_all_monitors || config.monitor_ids.is_empty() {
        ...
        // "default" means primary monitor only â€” show all for display, VisionManager filters
        all_monitors.iter().map(|m| m.id()).collect::<Vec<_>>()
    } else {
        config
            .monitor_ids
    ...
    memora_engine::schedule_monitor::reset_schedule_paused();
    ...
    let meeting_detector: Option<()> = if config.disable_meeting_detector {
    ...
    let vision_metrics = Arc::new(memora_screen::PipelineMetrics::new());

    // Start power manager â€” polls battery/thermal state and broadcasts profile changes
    let power_manager = start_power_manager();

    // Start background snapshot compaction (JPEG â†’ MP4) unless explicitly disabled.
    ...
    // Wire `meeting_ended` â†’ controller.handle_meeting_ended so a
    ...
    // the only safety net is the 4-hour hard cap.
    {
        ...
        tokio::spawn(async move {
            ...
            let mut sub =
                memora_events::subscribe_to_event::<serde_json::Value>("meeting_ended");
    ...
    //      Safe regardless of default_mode â€” only PrewarmPending sessions
    ...
    //   to the existing notification).
    {
        ...
        tokio::spawn(async move {
            use futures::StreamExt;
            let mut sub =
                memora_events::subscribe_to_event::<serde_json::Value>("meeting_started");
            while let Some(event) = sub.next().await {
                let meeting_id = event
                    .data
                    .get("meeting_id")
                ...
                if !matches!(
                    snap.default_mode,
                    memora_engine::high_fps_controller::DefaultMode::Always
                ) {
                    continue;
                }
                let already_bound = matches!(
                    snap.kind,
                    Some(memora_engine::high_fps_controller::SessionKind::Meeting { .. })
                );
                if !already_bound {
        ...
        .disable_vision
    {
        ...
        // Vision disabled â€” spawn a pending task so `handle` never completes
    ...
    // Shared manual meeting lock â€” bridges the HTTP meeting routes and the meeting persister
    ...
    if config.listen_address.is_loopback() {
        ...
        warn!(
            "API server listening on {}:{} â€” accessible from the network",
            config.listen_address, config.port
        );
    }
    if config.api_auth {
        info!("API auth enabled â€” run `MEMORA auth token` to view your key");
    }

    // Standalone CLI users miss out on the chat/timeline UI â€” point them at the
    // desktop app. MEMORA_ANALYTICS_ID is only set when the Tauri app spawns
    // the engine, so its absence is a reliable "this is a bare CLI run" signal.
    if std::env::var("MEMORA_ANALYTICS_ID").is_err() {
        eprintln!();
        eprintln!("  tip: get the desktop app for chat, timeline, and search UI");
        eprintln!("       â†’ https://github.com/shrirampai3000/memora/onboarding");
        eprintln!();
    }
    ...
    // MEMORA_API_KEY directly; desktop path overrides via
    // SCServer::cloud_token_handle after spawn.
    if let Ok(t) = std::env::var("MEMORA_API_KEY") {
        if !t.is_empty() {
    ...
    let encryption_requested =
        config.encrypt_secrets || memora_secrets::is_encryption_requested(&local_data_dir);
    {
        ...
        let secret_store_result = if config.encrypt_secrets {
            let secret_key = match memora_secrets::keychain::get_or_create_key() {
                Some(k) => {
                    info!("keychain: encryption key ready (--encrypt-secrets)");
                    if let Err(e) = memora_secrets::mark_encryption_enabled(&local_data_dir) {
                        warn!("keychain: failed to persist encryption opt-in flag: {}", e);
                    }
                    Some(k)
                }
                None => {
                    warn!("keychain: failed to create encryption key â€” secrets will be stored unencrypted");
                    None
                }
            };
            memora_secrets::SecretStore::open_for_data_dir(&local_data_dir, secret_key).await
        } else {
            memora_secrets::SecretStore::open_for_data_dir_with_vault_key(&local_data_dir).await
        };
        match secret_store_result {
            Ok(store) => {
                // Run startup permission sweep
                let fixed = memora_secrets::fix_secret_file_permissions(&local_data_dir);
                if fixed > 0 {
                    info!("fixed permissions on {} credential files", fixed);
                }

                // Run legacy migration
                match memora_secrets::migrate_legacy_secrets(&store, &local_data_dir).await {
                    Ok(report) => {
    ...
    let user_token = std::env::var("MEMORA_API_KEY").ok();
    let pi_executor = std::sync::Arc::new(
        memora_core::agents::pi::PiExecutor::new(user_token.clone())
            .with_api_auth_key(config.api_auth_key.clone()),
    );

    // Workflow event classifier â€” opt-in cloud feature. Polls recent activity
    ...
    // can override with MEMORA_EVENT_CLASSIFIER_URL.
    if config.enable_workflow_events {
        let classifier_url =
            std::env::var("MEMORA_EVENT_CLASSIFIER_URL").unwrap_or_else(|_| {
                memora_engine::workflow_classifier::DEFAULT_CLASSIFIER_URL.to_string()
            });
        let token = user_token.clone().unwrap_or_default();
        let port = config.port;
        tokio::spawn(async move {
            memora_engine::workflow_classifier::start_workflow_classifier(
                classifier_url,
                token,
                port,
    ...
    let mut agent_executors: std::collections::HashMap<
        String,
        std::sync::Arc<dyn memora_core::agents::AgentExecutor>,
    > = std::collections::HashMap::new();
    agent_executors.insert("pi".to_string(), pi_executor.clone());

    // Create pipe store backed by the main SQLite DB
    let pipe_store: Option<std::sync::Arc<dyn memora_core::pipes::PipeStore>> =
        Some(std::sync::Arc::new(
            memora_engine::pipe_store::SqlitePipeStore::new(db.clone()),
        ));

    let mut pipe_manager = memora_core::pipes::PipeManager::new(
        pipes_dir,
        agent_executors,
        pipe_store,
        config.port,
    );
    let mcp_session_access = memora_core::pipes::mcp_access::McpSessionAccessRegistry::new();
    pipe_manager.set_mcp_session_access(mcp_session_access.clone());
    // Wire pipe permission token registry (bridges PipeManager â†” server middleware)
    pipe_manager.set_token_registry(std::sync::Arc::new(
        memora_engine::pipe_permissions_middleware::DashMapTokenRegistry::new(
    ...
    // Gate scheduled pipe runs on connection readiness â€” same predicate the
    ...
    // pipes that are still in "setup mode" (declared connections not paired).
    {
        ...
        let MEMORA_dir_for_check = local_data_dir.clone();
        pipe_manager.set_connection_check(std::sync::Arc::new(move |required| {
            let ss = secret_store_for_check.clone();
            let dir = MEMORA_dir_for_check.clone();
            Box::pin(async move {
                memora_connect::missing_pipe_connections(ss.as_deref(), &dir, &required).await
            })
        }));
    }
    ...
    {
        let MEMORA_dir_for_live_views = local_data_dir.clone();
        pipe_manager.set_scheduled_run_context(std::sync::Arc::new(move |pipe_name: &str| {
            memora_engine::live_views::scheduled_live_view_run_context_for_dir(
                &MEMORA_dir_for_live_views,
                pipe_name,
                chrono::Local::now(),
            )
            .map(|context| memora_engine::pipes_api::format_run_context(&context))
        }));
    }
    ...
    println!("â”‚ port                   â”‚ {:<34} â”‚", config.port);
    println!("â”‚ audio disabled         â”‚ {:<34} â”‚", config.disable_audio);
    println!("â”‚ vision disabled        â”‚ {:<34} â”‚", config.disable_vision);
    println!(
        "â”‚ pause on DRM content   â”‚ {:<34} â”‚",
    ...
    println!("â”‚ vad engine             â”‚ {:<34} â”‚", "Silero");
    println!(
        "â”‚ data directory         â”‚ {:<34} â”‚",
        local_data_dir_clone.display()
    );
    println!("â”‚ debug mode             â”‚ {:<34} â”‚", record_args.debug);
    println!(
        "â”‚ telemetry              â”‚ {:<34} â”‚",
        config.analytics_enabled
    );
    println!(
        "â”‚ use pii removal        â”‚ {:<34} â”‚",
        config.use_pii_removal
    );
    println!(
        "â”‚ use all monitors       â”‚ {:<34} â”‚",
    ...
    println!(
        "â”‚ retention mode         â”‚ {:<34} â”‚",
        if record_args.retention_days == 0 {
            "n/a".to_string()
        } else {
            match record_args.retention_mode {
                memora_engine::retention::RetentionMode::Media => {
                    "media-only (keep transcripts)".to_string()
                }
                memora_engine::retention::RetentionMode::Lean => {
                    "lean (keep text+memories)".to_string()
                }
                memora_engine::retention::RetentionMode::All => "all (full delete)".to_string(),
            }
        }
    );
    ...
    println!(
        "\n{}",
        "check latest changes here: https://github.com/MEMORA/MEMORA/releases"
            .bright_blue()
            .italic()
    );
    ...
    let _meeting_watcher_handle = Some(start_meeting_watcher(
        db.clone(),
        v2_in_meeting,
    ...
    // every caller (CLI + in-process desktop app) gets it â€” no explicit call here.
    ...
    let redact_database_error_hook: memora_redact::DatabaseErrorHook = {
        let db = Arc::clone(&db);
        Arc::new(move |error| {
    ...
    if config.redact_agent_session_secrets {
        if let Ok(pi_dir) = memora_core::agents::pi::pi_config_dir() {
            use memora_redact::worker::{Worker, WorkerConfig};
            use std::sync::Arc;
            // A sessions-only worker (empty tables) never uses this redactor; the
            // session scrub runs its own secrets-only regex pipeline.
            let placeholder = Arc::new(memora_redact::Pipeline::regex_only())
                as Arc<dyn memora_redact::Redactor>;
    ...
    // Off by default â€” only runs when `--async-pii-redaction` is set.
    // The capture path is unaffected either way.
    if !config.async_pii_redaction {
        info!(
            "text-PII worker skipped at startup â€” async_pii_redaction=false. \
             OPF model (~2.8 GB) will NOT be downloaded or loaded. \
             Toggle via Settings â†’ Privacy â†’ AI PII removal."
        );
    }
    if config.async_pii_redaction {
        use memora_redact::{
        ...
        //      from huggingface.co/MEMORA/pii-redactor under
        ...
        //   4. Regex-only otherwise (still destructive â€” overwrites
        ...
        tokio::spawn(async move {
            ...
            info!(
                "fetching v45 phase 3 ONNX text redactor (~278 MB INT8 on first run, \
                 cached at ~/.memora/models/v45_phase3_onnx/)"
            );
            let pipeline = match OnnxRedactor::load_or_download(OnnxConfig::default()).await {
                Ok(adapter) => {
                    info!(
                        "text-PII AI step: local v45_phase3 ONNX â€” same checkpoint as the \
                ...
                Err(onnx_err) => {
                    ...
                    info!(
                        "fetching local OPF v6 checkpoint (~2.8 GB on first run, cached at \
                         ~/.memora/models/opf-v6/)"
                    );
                    match OpfAdapter::load_or_download(OpfConfig::default()).await {
                        Ok(adapter) => {
                            info!(
                                "text-PII AI step: local opf-rs (candle) fallback â€” lazy load \
                        ...
                        Err(e) => {
                                ...
                                || std::env::var("TINFOIL_BASE_URL").is_ok()
                            {
                                ...
                                tracing::warn!(
                                    "text-PII AI step disabled â€” both v45 ONNX and opf-rs \
            ...
            // off by default â€” opt-in). Orthogonal to the category policy above.
            ...
            // join its handle â€” when the process exits the runtime
    ...
    // Independent of the text worker â€” users can toggle either one
    ...
    if !config.async_image_pii_redaction {
        info!(
            "image-PII worker skipped at startup â€” async_image_pii_redaction=false. \
    ...
    if config.async_image_pii_redaction {
        ...
        use memora_redact::image::worker::{ImageWorker, ImageWorkerConfig};
        use memora_redact::{ImageRedactionPolicy, ImageRedactor};
        use std::sync::Arc;
        ...
        {
            if std::env::var_os("MEMORA_ENABLE_EXPERIMENTAL_RFDETR_MLX").is_some() {
                use memora_redact::adapters::rfdetr_mlx::{RfdetrMlxConfig, RfdetrMlxRedactor};
                let mlx_cfg = RfdetrMlxConfig::default();
                // Mirrors the ONNX adapter: download once, verify SHA-256,
                // cache at ~/.memora/models/rfdetr_v9.safetensors.
                if let Err(e) = mlx_cfg.ensure_model_present().await {
                    ...
                    match RfdetrMlxRedactor::load(mlx_cfg) {
                        Ok(d) => {
                            ...
                            // Lazy-load + 60 s idle-unload â€” frees the
                            // ~150â€“200 MB MLX resident footprint when the
                ...
                tracing::info!(
                    "rfdetr-mlx disabled by default for CLI stability; \
                     set MEMORA_ENABLE_EXPERIMENTAL_RFDETR_MLX=1 to opt in"
                );
            }
        }
                ...
                Err(e) => {
                    ...
                    tracing::warn!(
                        "image-PII redaction enabled but couldn't load model; skipping: {e}. \
                         ...
                         rfdetr_v9.onnx at ~/.memora/models/."
                    );
                }
            }
        }
