...
/// so the saving is 2 Ã— 8 allocs = 16 heap allocs avoided per captured frame.
#[derive(Debug, Clone)]
pub struct HotFrame {
    pub frame_id: i64,
...
///
/// String fields are `Arc<str>` for the same reason as `HotFrame` â€” cheap clone
...
pub struct HotFrameCache {
    ...
    /// Ordinal day number â€” cache is cleared on day rollover.
...
impl HotFrameCache {
    ...
    pub async fn push_frame(&self, frame: HotFrame) {
        ...
        // Broadcast to WS handlers â€” ignore errors (no subscribers = fine)
    ...
    pub async fn warm_from_db(&self, db: &memora_db::DatabaseManager, hours: i64) {
        let end = Utc::now();
            ...
            Ok(chunks) => {
                ...
                for frame_data in chunks.frames {
                    ...
                    for ocr_entry in &frame_data.ocr_entries {
                        // Skip MEMORA's own frames
                        if is_MEMORA_app(&ocr_entry.app_name) {
                            continue;
                        }
                        let hot = HotFrame {
                ...
                // NOT claim coverage â€” otherwise the streaming handler skips
        ...
        // Signal that warm is complete (even on failure â€” callers should not block forever)
...
/// Find audio entries within Â±60s of a frame timestamp.
/// Wider than the DB's Â±15s window because smart/batch mode can delay
/// transcription by minutes â€” audio is keyed by capture_timestamp which
...
mod tests {
    ...
    async fn test_audio_association() {
        ...
        // Push audio within Â±15s window
        cache
            .push_audio(HotAudio {
                audio_chunk_id: 10,
