...
/// events without back-pressuring the recorder. At 50â€“100 events/sec
...
pub fn spawn_frame_linker(
    ...
    tokio::spawn(async move {
        ...
        // First `interval.tick().await` returns immediately â€” skip it so
                        ...
                        Some(LinkerMessage::EventPersisted(e)) => {
                                ...
                                linker.on_event_persisted(e, Instant::now())
                            {
                                ...
                                debug!(
                                    ...
                                    "frame_linker: paired eventâ†’frame (frame arrived first)"
                                );
                        ...
                        Some(LinkerMessage::FrameCaptured(c)) => {
                            ...
                            if !updates.is_empty() {
                                ...
                                debug!(
                                    ...
                                    "frame_linker: paired frameâ†’events (events arrived first)"
                                );
                            } else {
                                let (pe, pf) = linker.pending_len();
                                debug!(
                ...
                _ = tick.tick() => {
                    ...
                    if evicted > 0 {
                        ...
                        warn!(
                            ...
                            dropped_other,
                            "frame_linker: stale entries expired without pairing â€” these slipped past every instrumented drop site (DRM/paused/lagged/capture_error); investigate the residual"
                        );
                    } else {
                        debug!(
                            pending_events = pe,
...
async fn apply_update(db: &Arc<DatabaseManager>, row_id: i64, frame_id: i64) {
    match db.update_ui_event_frame_id(row_id, frame_id).await {
        ...
        Err(e) => {
            ...
            // NULL) but very rare in practice â€” log and move on. We don't
