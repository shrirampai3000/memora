...
//!
//! Called ONLY from the CLI binary's `record` path â€” the inverse of the
...
//! (MEMORA-audio `SttPriorityDip`), and the snapshot-compaction ffmpeg
...
//! follow it down (desired â€” they are background work), and the LL input-hook
//! threads are pinned HIGHEST in memora-a11y so the system-wide input
...
//! only changes CPU scheduling â€” the safe first step per #4849.
//!
//! Escape hatch: set `MEMORA_KEEP_NORMAL_PRIORITY=1` to skip the drop â€”
//! e.g. an embedder that spawns `MEMORA record` as a child and manages
//! priority itself (our call would otherwise override its choice).

/// True when `MEMORA_KEEP_NORMAL_PRIORITY` opts out of the priority drop
/// (shared lenient truthiness â€” see `analytics::env_value_truthy`).
...
/// continues on failure â€” recording at Normal priority beats not recording.
#[cfg(windows)]
pub fn set_background_priority() {
    ...
    let env_value = std::env::var("MEMORA_KEEP_NORMAL_PRIORITY").ok();
    if keep_normal_requested(env_value.as_deref()) {
        info!("keeping Normal process priority (MEMORA_KEEP_NORMAL_PRIORITY set)");
        return;
    }
    ...
    unsafe {
        match SetPriorityClass(GetCurrentProcess(), BELOW_NORMAL_PRIORITY_CLASS) {
            Ok(()) => info!(
                "process priority set to BelowNormal â€” engine yields CPU to foreground apps \
                 (opt out with MEMORA_KEEP_NORMAL_PRIORITY=1)"
            ),
            Err(e) => warn!("SetPriorityClass(BELOW_NORMAL) failed: {e:?} â€” staying at Normal"),
        }
    }
}

/// Windows-only for now: macOS/Linux schedulers already treat the engine
/// acceptably and have their own mechanisms (nice/QoS) â€” see #4849.
#[cfg(not(windows))]
pub fn set_background_priority() {}
...
mod tests {
    ...
    fn drops_process_to_below_normal() {
        ...
        let saved = std::env::var("MEMORA_KEEP_NORMAL_PRIORITY").ok();
        std::env::remove_var("MEMORA_KEEP_NORMAL_PRIORITY");
        ...
        if let Some(v) = saved {
            std::env::set_var("MEMORA_KEEP_NORMAL_PRIORITY", v);
        }
    }
}
