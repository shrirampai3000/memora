...
use crate::{
    ...
    routes::{
        ...
        health::{
            api_list_monitors, api_vision_status, health_check, vision_metrics_handler,
        },
        meetings::{
...
pub struct AppState {
    ...
    /// Active pipe permission tokens â€” maps token string to resolved permissions.
    pub pipe_permissions:
        Arc<DashMap<String, Arc<memora_core::pipes::permissions::PipePermissions>>>,
    /// Hot frame cache â€” in-memory cache for today's frames.
    ...
    /// Vault lock manager â€” encrypts data at rest when locked
    pub vault: memora_vault::VaultManager,
    ...
    /// Browser extension bridge â€” relays JS eval requests to the connected extension
    pub browser_bridge: Arc<crate::routes::browser::BrowserBridge>,
    /// Registry of every browser the agent can drive â€” user's real browser via
    /// the extension, the app-managed owned webview, future remote-CDP backends.
    /// `GET /connections/browsers` lists what's here.
    pub browser_registry: Arc<memora_connect::connections::browser::BrowserRegistry>,
    ...
    pub owned_browser: Arc<memora_connect::connections::browser::OwnedBrowser>,
    /// When true, non-localhost requests require Authorization: Bearer <api_key>
    pub api_auth: bool,
    /// The API key to validate against (from MEMORA_API_KEY or auth.json)
    pub api_auth_key: Option<String>,
    /// Cloud JWT (Clerk) used to authenticate proxied requests to api.memora.com.
    ...
    /// this token â€” agent calls localhost/v1/chat/completions and the server
    ...
    pub secret_store: Option<Arc<memora_secrets::SecretStore>>,
...
pub struct SCServer {
    ...
    pub vision_metrics: Arc<memora_screen::PipelineMetrics>,
    /// Shared hot frame cache â€” set this before starting the server so AppState uses it.
    pub hot_frame_cache: Option<Arc<HotFrameCache>>,
    /// Power manager handle â€” set this before starting to enable /power endpoints.
    pub power_manager: Option<Arc<crate::power::PowerManagerHandle>>,
    /// Shared pipe permission token registry â€” set before starting so PipeManager can use it.
    pub pipe_permissions:
        Arc<DashMap<String, Arc<memora_core::pipes::permissions::PipePermissions>>>,
    /// Per-pipe MCP allowlist registry shared by PipeManager and /mcp-servers.
    pub mcp_session_access: Option<memora_core::pipes::mcp_access::McpSessionAccessRegistry>,
    /// Shared manual meeting lock â€” pass in from binary so persister and server share the same state.
    pub manual_meeting: Option<Arc<tokio::sync::RwLock<Option<i64>>>>,
    /// Owned browser instance â€” set by the desktop shell so it can attach an
    ...
    pub owned_browser: Option<Arc<memora_connect::connections::browser::OwnedBrowser>>,
    ...
    /// same reasons as `oauth_refresher` â€” keeps the JoinHandle alive
...
fn is_api_auth_token_authorized(
    ...
    pipe_permissions: &DashMap<String, Arc<memora_core::pipes::permissions::PipePermissions>>,
    token: Option<&str>,
) -> bool {
...
impl SCServer {
    ...
    pub fn new(
        ...
        SCServer {
            ...
            video_quality,
            pipe_manager: None,
            vision_metrics: Arc::new(memora_screen::PipelineMetrics::new()),
    ...
    /// to api.memora.com. Safe to call before or after `start()` â€” the route
    ...
    pub fn with_mcp_session_access(
        mut self,
        registry: memora_core::pipes::mcp_access::McpSessionAccessRegistry,
    ) -> Self {
    ...
    pub async fn start(self) -> Result<(), std::io::Error> {
        ...
        if self.advertise_mdns {
            if let Err(e) = memora_connect::mdns::advertise(self.addr.port()) {
    ...
    /// connection â€” each keeps a clone of the router state (DB pool + secret
                ...
                accepted = listener.accept() => {
                    let (stream, remote_addr) = match accepted {
                        ...
                        Err(e) => {
                            ...
                            // ECONNABORTED, â€¦) must not kill the server. The
    ...
    async fn create_router_inner(&self) -> Router {
        ...
        // Spawn warm_from_db in the background â€” the cache starts empty but fills
        ...
        let app_state = Arc::new(AppState {
            ...
            // Each entry is just a file path (~100 bytes) + Instant, so 1000 entries â‰ˆ 100KB.
            ...
            // Limit pipe queries to 3 concurrent â€” protects recording from pipe overload.
            ...
            vault: memora_vault::VaultManager::new(self.MEMORA_dir.clone()),
            manual_meeting: self
                .manual_meeting
                .clone()
                .unwrap_or_else(|| Arc::new(tokio::sync::RwLock::new(None))),
            browser_bridge: crate::routes::browser::BrowserBridge::new(),
            browser_registry: memora_connect::connections::browser::BrowserRegistry::new(),
            ...
            // to a default unattached instance â€” useful for CLI / tests /
        ...
        // OwnedWebviewHandle.
        {
            use memora_connect::connections::browser::UserBrowser;
        ...
        let server = Server::axum()
            ...
            // General export: meeting_id XOR start/end â†’ MP4 (frames + synced audio).
            // HTTP twin of the `MEMORA export` CLI; used by the MCP export-video tool.
        ...
        let router = Router::new()
            ...
            // Private MCP transport adapter (not in OpenAPI spec â€” a plain
            ...
            // Vision status endpoint (not in OpenAPI spec â€” no State param)
            .route("/vision/status", get(api_vision_status))
            // Vision pipeline metrics (not in OpenAPI spec â€” external types)
            .route("/vision/metrics", get(vision_metrics_handler))
            // HD recording â€” bound sessions (meeting / timer / prewarm-pending),
            // no indefinite mode. Every session has a natural end condition.
            // GET    /capture/hd            â†’ current snapshot
            // POST   /capture/hd/start      â†’ { boundTo: "meeting"|"timer"|"prewarm_pending", meetingId?, durationSecs? }
            // POST   /capture/hd/stop       â†’ clear active session
            // POST   /capture/hd/extend     â†’ { additionalSecs } push expires_at back
            // POST   /capture/hd/settings   â†’ { defaultMode?, intervalMs? }
            ...
            // Local proxy â†’ api.memora.com/v1/chat/completions. Lets the
            ...
            // Local proxy â†’ api.memora.com/v1/web-search. Lets coding-agent
            ...
            // Blocking connect broker for the harness MCP `memora_connect_app`
        ...
        let router = if let Some(ref pm) = self.pipe_manager {
            ...
            let pipe_routes = Router::new()
                ...
                // Favorites â€” register before `/:id` so axum doesn't match
                // it as a pipe id.
                .route(
                    "/favorites",
                    axum::routing::get(crate::pipes_api::list_favorites),
                )
                // Stateless schedule preview â€” register before `/:id` so the
        ...
        let cm: crate::connections_api::SharedConnectionManager = Arc::new(Mutex::new(
            memora_connect::connections::ConnectionManager::new(
                self.MEMORA_dir.clone(),
                self.secret_store.clone(),
            ),
        ));
        let wa: crate::connections_api::SharedWhatsAppGateway = Arc::new(Mutex::new(
            memora_connect::whatsapp::WhatsAppGateway::new(self.MEMORA_dir.clone()),
        ));
        ...
        // (bundled sidecar â†’ install dirs â†’ PATH).
        {
        ...
        let mcp_store: crate::mcp_servers_api::SharedMcpServerStore =
            Arc::new(memora_connect::mcp_servers::McpServerStore::new(
                self.MEMORA_dir.clone(),
                self.secret_store.clone(),
            ));
        let router = router.nest(
        ...
        router
            ...
            // Browser extension bridge â€” DEPRECATED top-level paths.
            ...
            .layer({
                // API auth middleware â€” when api_auth is enabled, ALL requests
                ...
                axum::middleware::from_fn(
                    move |req: axum::extract::Request, next: axum::middleware::Next| {
                        ...
                        async move {
                            ...
                            // 2. MEMORA_auth=<token> cookie (img src, WebSocket)
                            ...
                            let cookie_token = req
                                ...
                                .and_then(|cookies| {
                                    cookies.split(';')
                                        .map(|c| c.trim())
                                        .find_map(|c| c.strip_prefix("MEMORA_auth="))
                                        .map(|s| s.to_string())
                                });
                            ...
                            if authorized {
                                ...
                                if upgrade {
                                    ...
                                    // ~2 880Ã—/day in steady state. The 403 response already
                                    // tells the caller what's wrong â€” keep the log line
                                    // available for debugging but not at WARN.
                                    debug!(
                                        path = %path,
                                        "api auth: rejected WebSocket upgrade (missing/invalid token; use Cookie MEMORA_auth, Authorization Bearer, or ?token=)"
                                    );
                                }
                                axum::response::Response::builder()
                                    ...
                                    .body(axum::body::Body::from(
                                        ...
                                        // Settings > Privacy â€” surface the CLI path
                                        ...
                                        r#"{"error":"unauthorized: API access requires authentication. Pass `Authorization: Bearer <your-api-key>`. Get the key with `MEMORA auth token`, or set the `MEMORA_API_KEY` env var before starting MEMORA. (Desktop app users: Settings > Privacy.)"}"#,
                                    ))
                                    .unwrap()
                            }
                        }
...
mod tests {
    ...
    use dashmap::DashMap;
    use memora_core::pipes::permissions::PipePermissions;
    ...
    async fn graceful_shutdown_tears_down_live_connections_and_frees_port() {
        ...
        // that will never finish on its own â€” the exact kind that used to
        ...
        // for 60s â€” proving connection tasks are aborted and awaited, not
