...
//! by the standalone `memora-engine` binary and the Tauri desktop app.
//!
...
//! soft `RLIMIT_NOFILE` of 256 â€” while opening a large SQLite pool (high-tier:
...
//!
//! Override the target with the `MEMORA_FD_LIMIT` env var.

/// Raise the soft `RLIMIT_NOFILE` toward `MEMORA_FD_LIMIT` (default 8192),
...
pub fn set_fd_limit() {
    ...
    let desired_limit: u64 = env::var("MEMORA_FD_LIMIT")
        .ok()
    ...
    unsafe {
        if libc::getrlimit(libc::RLIMIT_NOFILE, &mut rlim) == 0 {
            ...
            if current_soft < desired_limit {
                ...
                // already sufficient â€” no need to log
            }
        } else {
