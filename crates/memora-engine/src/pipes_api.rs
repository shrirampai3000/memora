...
use axum::Json;
use memora_connect::{connections, mcp_servers};
use memora_core::pipes::{
...
/// GET /pipes â€” list all pipes with status.
...
/// GET /pipes/activity â€” compact, cursor-paginated execution-backed inventory.
///
...
/// GET /pipes/:id â€” single pipe detail.
...
/// POST /pipes/:id/enable â€” enable or disable a pipe.
...
/// POST /pipes/:id/run â€” trigger a manual pipe run.
...
pub async fn run_pipe_now(
    ...
    if !required_connections.is_empty() {
        ...
        let missing = memora_connect::missing_pipe_connections(
            ss,
            &MEMORA_dir,
        ...
        if !missing.is_empty() {
            return Json(json!({
                "error": format!(
                    "pipe '{}' requires unconfigured connections: {} â€” set them up from the Connections page in the desktop app",
                    id,
                    missing.join(", ")
                )
            }));
    ...
    let conn_ctx = join_context_blocks([
        connections::render_context(&MEMORA_dir, api_port, ss).await,
        mcp_servers::render_context(&MEMORA_dir, api_port).await,
    ]);
...
/// POST /pipes/:id/stop â€” stop a running pipe.
pub async fn stop_pipe(State(pm): State<SharedPipeManager>, Path(id): Path<String>) -> Json<Value> {
    ...
    match mgr.stop_pipe(&id).await {
        Ok(status) => Json(json!({
            "success": matches!(
                status,
                memora_core::pipes::PipeStopStatus::Stopping
                    | memora_core::pipes::PipeStopStatus::StopPending
            ),
            "status": status,
        })),
        Err(e) => Json(json!({ "error": e.to_string() })),
...
/// GET /pipes/:id/logs â€” recent run logs (in-memory).
...
/// GET /pipes/:id/executions â€” execution history from DB.
...
/// GET /pipes/:id/executions/:exec_id â€” one exact persisted execution.
...
/// GET /pipes/:id/session/:exec_id â€” read Pi session file for an execution.
...
/// POST /pipes/:id/config â€” update pipe config fields.
...
/// POST /pipes/schedule/preview â€” stateless preview for the schedule builder.
/// Body is a `ScheduleConfig`; returns its human summary and the next few fire
/// times so the UI can show "Next occurrence â€¦" without duplicating timezone /
...
/// POST /pipes/install â€” install a pipe from URL or local path.
...
/// DELETE /pipes/:id/history â€” clear a pipe's chat history (Pi session files).
...
/// DELETE /pipes/:id â€” delete a pipe.
...
/// GET /pipes/favorites â€” list pipe names the user has starred.
///
...
pub async fn list_favorites(State(pm): State<SharedPipeManager>) -> Json<Value> {
    ...
    let favorites = memora_core::pipes::favorites::load(&dir);
    Json(json!({ "data": favorites }))
}

/// POST /pipes/:id/favorite â€” mark or unmark a pipe as favorite.
...
pub async fn set_pipe_favorite(
    ...
    let dir = mgr.pipes_dir().to_path_buf();
    drop(mgr);
    match memora_core::pipes::favorites::set(&dir, &id, body.favorite) {
...
mod tests {
    ...
    async fn run_api_returns_and_reads_the_exact_new_execution() {
        ...
        let db = Arc::new(
            memora_db::DatabaseManager::new(db_path.to_str().unwrap(), Default::default())
                .await
                .unwrap(),
        );
    ...
    async fn schedule_preview_returns_summary_and_next() {
        // Stateless endpoint â€” no pipe manager needed.
