...
fn cli_engine_to_str(_engine: &CliAudioTranscriptionEngine) -> &'static str {
    "disabled"
}
...
impl CliAppContext {
    fn resolved(self) -> (bool, memora_config::SemanticContextMode) {
        use memora_config::SemanticContextMode;
...
#[command(
    author,
    version,
    about = "MEMORA: power AI by everything you've seen, said or heard",
    long_about = None,
    name = "MEMORA"
)]
pub struct Cli {
...
pub enum Command {
    ...
    /// Show MEMORA status (running state, data stats)
    Status {
        /// Output format
        #[arg(long, default_value_t = false)]
        json: bool,
        /// Data directory. Default to $HOME/.memora
    ...
    /// (no daemon required â€” opens `~/.memora/db.sqlite` read-side
    /// via WAL while sp may be writing).
    Search(SearchArgs),

    /// Enterprise: query teammates' screen + audio history via
    /// `github.com/shrirampai3000/memora/api/enterprise/v1/*`. Admin-only â€” needs a
    /// `team_api_token` minted at https://github.com/shrirampai3000/memora/enterprise?tab=tokens.
    Team {
        #[command(subcommand)]
    ...
    /// Wire an external agent (OpenClaw, Hermes, Claude) to MEMORA
    Agent {
        #[command(subcommand)]
        subcommand: agent::AgentCommand,
    },

    /// Connect MEMORA to every supported AI tool detected on this computer
    Setup {
        /// MEMORA REST API base URL the skills + MCP should target.
        #[arg(long, default_value = "http://localhost:3030")]
        api_url: String,
    },

    /// Run MEMORA as a background service that starts at boot + stays up
    ...
    /// Vault: encrypt/lock all MEMORA data at rest
    Vault {
        #[command(subcommand)]
    ...
    Install {
        /// Manifest URL (HTTPS, JSON). Defaults to the MEMORA starter bundle.
        #[arg(default_value = "https://github.com/shrirampai3000/memora/start.json")]
        url: String,
    ...
    /// Authenticate with MEMORA cloud
    Login,

    /// Sign out of MEMORA cloud
    Logout,

    /// Show current auth status
    Whoami,

    /// Open the MEMORA survey in your browser
    Survey,

    /// Check system readiness (permissions, ffmpeg, etc.)
    Doctor,

    /// Collect recent logs + system info and send them to MEMORA support,
    /// returning a ticket id. The headless twin of the app's "send logs &
    /// feedback" button â€” for VPS/boxes with no UI, or for an agent to
    ...
    /// `~/.memora/db.sqlite` directly â€” no daemon required.
...
pub struct ExportArgs {
    /// Meeting id to export â€” resolves the meeting's start/end automatically
    /// (see `MEMORA search` or the app's meetings list). Mutually
...
pub struct RecordArgs {
    ...
    /// Request MEMORA's software Acoustic Echo Cancellation (via sonora WebRTC AEC3).
    #[arg(long, default_value_t = false)]
    pub MEMORA_aec_enabled: bool,

    /// Data directory. Default to $HOME/.memora
    #[arg(long, value_hint = ValueHint::DirPath)]
    pub data_dir: Option<String>,

    /// Enable debug logging for MEMORA modules
    #[arg(long)]
    pub debug: bool,
    ...
    /// gone after the worker processes the row. Off by default â€”
    ...
    /// `rfdetr_v11.onnx` at `~/.memora/models/` and the binary
    /// built with one of the `onnx-*` cargo features. Off by default.
    #[arg(long, default_value_t = false)]
    pub async_image_pii_redaction: bool,

    /// Backend for the AI PII workers â€” `local` (on-device ONNX,
    /// privacy by construction, slower on weak hardware) or
    /// `tinfoil` (MEMORA-hosted confidential-compute enclave on
    /// H200, fast everywhere, requires network). Single flag for
    /// both text + image â€” flipping it swaps the inner adapter for
    /// both worker types.
    ...
    /// a11y_url_field. The list is exact (key present â†’ on, absent â†’ off);
    /// `full_text` is always redacted, and so is `frames.text_json` (the
    /// per-word OCR boxes â€” a structured copy of the same on-screen text,
    /// scrubbed alongside full_text, issue #4117). Default scrubs the clear
    /// surfaces plus element_properties (form-field values â€” the real PII
    ...
    #[arg(
        long,
        env = "MEMORA_ENABLE_SEMANTIC_CONTEXT",
    ...
    /// Keep the computer awake while MEMORA is running.
    #[arg(long, default_value_t = false)]
    ...
    /// the app's "Capture frequency" setting â€” use it when the default
    /// event-driven cadence feels too sparse (e.g. long reading / watching with
    /// few clicks). Overrides the active PowerProfile's idle interval (AC 30s â€¦
    ...
    /// Mitsukeru fork: override `EventDrivenCaptureConfig::visual_change_threshold` (0.0â€“1.0).
    ...
    /// Override `UiRecorderConfig::capture_scroll` â€” record scroll wheel
    ...
    ///   1. mouse/keyboard hook locks switch to try_lock (contended â†’ app_name/window=None)
    ...
    #[arg(long, env = "MEMORA_ENABLE_MDNS", default_value_t = false)]
    pub enable_mdns: bool,

    /// API token for cloud sync
    #[arg(long, env = "MEMORA_SYNC_TOKEN")]
    pub sync_token: Option<String>,

    /// Password for encrypting synced data
    #[arg(long, env = "MEMORA_SYNC_PASSWORD")]
    ...
    /// Horizon) is focused â€” these blank their windows while any app is
    /// recording the screen.
    #[arg(long, default_value_t = false)]
    pub pause_on_drm_content: bool,

    /// Disable clipboard capture entirely. The UI recorder will not record
    /// clipboard copy/paste events or contents â€” useful when piping
    /// ~/.memora data into a remote LLM (passwords, keys, secrets often
    ...
    /// capture on-screen text. Useful when piping ~/.memora data into a
    ...
    /// capture; only the `ui_events` click rows are skipped. On by default â€”
    ...
    /// requests must include Authorization: Bearer <MEMORA_API_KEY>.
    /// Localhost requests are always allowed.
    #[arg(long, action = ArgAction::Set, num_args = 0..=1, default_value_t = true, default_missing_value = "true")]
    pub api_auth: bool,

    /// Bind the HTTP server to 0.0.0.0 so other devices on the LAN can
    /// reach it. Off by default â€” the server binds 127.0.0.1 only.
    ...
    /// CLI deployments must never silently delete data â€” the desktop app opts new users in
    ...
    /// Use when meeting detection is not consumed (task-mining, headless analysis) â€”
    ...
    pub schedule_rules: Vec<memora_config::ScheduleRule>,
}
...
impl RecordArgSources {
    pub fn from_cli_matches(matches: &ArgMatches) -> Self {
        ...
        Self {
            ...
            MEMORA_aec_enabled: from_command_line(record, "MEMORA_aec_enabled"),
...
fn parse_schedule_rule(s: &str) -> Result<memora_config::ScheduleRule, String> {
...
impl RecordArgs {
    ...
    pub fn to_recording_settings(&self) -> memora_config::RecordingSettings {
        ...
        let aec_mode = if self.MEMORA_aec_enabled {
            memora_config::AecMode::MEMORA
        } else if self.windows_input_aec_enabled {
            memora_config::AecMode::Windows
        } else if self.macos_input_vpio_enabled {
            memora_config::AecMode::Macos
        } else {
            memora_config::AecMode::Off
        };
        let (enable_semantic_context, semantic_context_mode) =
            self.app_context.map(CliAppContext::resolved).unwrap_or((
                self.enable_semantic_context,
                memora_config::SemanticContextMode::Memory,
            ));

        memora_config::RecordingSettings {
            ...
            windows_input_aec_enabled: aec_mode == memora_config::AecMode::Windows,
            macos_input_vpio_enabled: aec_mode == memora_config::AecMode::Macos,
            MEMORA_aec_enabled: aec_mode == memora_config::AecMode::MEMORA,
            aec_mode,
    ...
    pub async fn into_recording_config(
        ...
        let cli_encryption_ready = if self.encrypt_secrets {
            match memora_secrets::keychain::get_or_create_key() {
                Some(_) => {
                    memora_secrets::mark_encryption_enabled(&data_dir)?;
                    true
                }
                None => {
                    tracing::warn!(
                        "keychain: failed to create encryption key â€” secrets will be stored unencrypted"
                    );
                    false
                }
            }
        ...
        // or cloud features (STT, MEMORA-cloud pipes) silently lose auth
        ...
        if settings.device_tier.is_none() {
            ...
            let existing = memora_config::load_toml(&config_path).ok();
            ...
            if has_tier {
                // Existing config with tier â€” just use it
                if let Some(existing) = existing {
                    settings.device_tier = existing.device_tier;
                }
            } else {
                let tier = memora_config::detect_tier();
                eprintln!("detected hardware tier: {:?}", tier);
                // Only apply capture defaults (video_quality, power_mode) for truly fresh installs.
                // Existing config without tier = upgrade â€” just set the tier for DB/channel tuning.
                let is_fresh = !config_path.exists() && !loaded_from_store;
                if is_fresh {
                    memora_config::apply_tier_defaults(&mut settings, tier);

                    // Restore CLI audio engine â€” user's explicit -a/--audio-transcription-engine
                    ...
                    // Restore CLI monitor flags â€” user's explicit --use-all-monitors or -m
        ...
        let tier = settings
            ...
            .unwrap_or_else(memora_config::detect_tier);
        if memora_config::is_engine_unsafe(&settings.audio_transcription_engine, tier) {
            let safe = memora_config::best_engine_for_platform(tier);
            eprintln!(
        ...
        // the API is bound to the LAN â€” that would publish an unauthenticated
        ...
        if self.listen_on_lan && !self.api_auth {
            tracing::warn!(
                "--listen-on-lan was set but --api-auth=false â€” forcing api_auth on for safety. Use `MEMORA auth token` to view your key."
            );
        }
        if config.api_auth {
    ...
    fn apply_explicit_overrides(
        &self,
        settings: &mut memora_config::RecordingSettings,
        sources: &RecordArgSources,
    ) {
        ...
        if sources.MEMORA_aec_enabled && self.MEMORA_aec_enabled {
            settings.aec_mode = memora_config::AecMode::MEMORA;
            settings.MEMORA_aec_enabled = true;
            ...
            settings.aec_mode = memora_config::AecMode::Windows;
            ...
            settings.aec_mode = memora_config::AecMode::Macos;
            settings.MEMORA_aec_enabled = false;
            settings.windows_input_aec_enabled = false;
...
fn load_recording_settings_from_store(
    data_dir: &Path,
) -> anyhow::Result<Option<memora_config::RecordingSettings>> {
...
fn persist_recording_settings_to_store(
    data_dir: &Path,
    settings: &memora_config::RecordingSettings,
...
pub enum PipeCommand {
    ...
    Publish {
        /// Pipe name (directory name under ~/.memora/pipes/)
        name: String,
    },
    ...
    SetPreset {
        /// Pipe name
        name: String,
        /// Preset id(s) â€” multiple ids form a fallback chain (first works wins)
...
pub enum ModelCommand {
    ...
    Show {
        /// Preset id
        id: String,
        /// Output as JSON (returns raw api key â€” for scripting / backup)
        #[arg(long, default_value_t = false)]
        json: bool,
    },
    /// Create a new preset
    Create {
        ...
        id: String,
        /// Provider: openai | anthropic | native-ollama | custom | MEMORA-cloud
        #[arg(long)]
        provider: String,
        ...
        /// Max input context characters (1000â€“2_000_000)
        #[arg(long)]
        max_context_chars: Option<i64>,
        /// Max output tokens (1â€“200000)
        #[arg(long)]
...
pub enum VaultCommand {
    ...
    Setup {},
    /// Lock the vault (encrypt all data; stop MEMORA first)
    Lock {
        /// Port MEMORA listens on. `vault lock` refuses to run while a
        /// daemon is detected here â€” locking encrypts the DB in place and would
        /// corrupt it underneath a live daemon, so stop MEMORA first.
        #[arg(short = 'p', long, default_value_t = 3030)]
        port: u16,
    },
...
pub enum DbCommand {
    ...
    /// Refuses to run while MEMORA is open.
    Recover {
        /// Deprecated compatibility flag. Recovery still refuses to race a
        /// reachable MEMORA server because that cannot preserve one exact
    ...
    Cleanup {
        ...
        #[arg(long)]
        apply: bool,
        /// Run even if the MEMORA HTTP server is reachable.
        #[arg(long)]
        force: bool,
    },
    /// Force-clear the .db_recovery.lock file (escape hatch if a previous
    /// `MEMORA db ...` run was killed without releasing the lock).
    Unlock {
        /// Remove the lock even if it appears alive â€” only do this if you're
...
/// shape the API returns â€” `MEMORA search` and `curl /search` are
/// interchangeable for downstream consumers.
#[derive(Parser, Clone, Debug)]
pub struct SearchArgs {
    /// Search query (omit for a time-only browse â€” pair with `--start`).
    ...
    /// Data directory. Default `$HOME/.memora`.
    #[arg(long, value_hint = ValueHint::DirPath)]
    pub data_dir: Option<String>,
...
/// Mirrors the `MEMORA-team` skill â€” same endpoints, same vocabulary.
/// All variants hit `https://github.com/shrirampai3000/memora/api/enterprise/v1/*` directly
/// with the admin's `team_api_token` from `~/.memora/enterprise.json`
/// (or `MEMORA_TEAM_API_TOKEN` env override). No daemon needed.
...
pub enum TeamCommand {
    ...
    /// Chronological dump for one device â€” use after `devices` + `search`
...
pub struct TeamSearchArgs {
    ...
    /// Restrict to one device â€” get the id from `MEMORA team devices`.
    #[arg(long)]
    ...
    /// Relative time window â€” accepts `24h`, `2d`, `30m`, `1w`. Default 24h.
    #[arg(long)]
...
pub struct TeamRecordsArgs {
    /// Device id to dump records for (required â€” without it you'd get the
    ...
    /// Relative time window â€” `4h`, `1d`, `30m`. Default 4h.
    #[arg(long)]
    pub since: Option<String>,
...
pub enum McpCommand {
    ...
    Setup {
        /// Directory to save MCP files (default: $HOME/.memora/mcp)
        #[arg(long, value_hint = ValueHint::DirPath)]
...
pub enum SyncCommand {
    ...
    /// Sync ~/.memora to a remote SSH server (SFTP, no cloud account)
    Remote {
        #[command(subcommand)]
...
/// SSH/SFTP-based sync of `~/.memora` to a remote server.
///
/// No cloud account or MEMORA-cloud dependency â€” pushes the entire data
...
pub enum RemoteSyncCommand {
    ...
    /// Push `~/.memora/` to the remote once
    Now {
        #[command(flatten)]
        cfg: RemoteSyncArgs,
        /// Override the local data directory (default: $HOME/.memora)
        #[arg(long)]
        data_dir: Option<String>,
    },
...
/// Shared connection flags for `MEMORA sync remote {test,now}`.
///
/// All values can also come from env vars: MEMORA_REMOTE_HOST,
/// MEMORA_REMOTE_USER, MEMORA_REMOTE_KEY, MEMORA_REMOTE_PATH.
#[derive(clap::Args, Debug)]
pub struct RemoteSyncArgs {
    /// Remote host (IP or DNS, e.g. "myserver.tail-scale.ts.net")
    #[arg(long, env = "MEMORA_REMOTE_HOST")]
    pub host: String,
    /// SSH user
    #[arg(long, env = "MEMORA_REMOTE_USER")]
    pub user: String,
    /// Path to SSH private key (e.g. ~/.ssh/id_ed25519)
    #[arg(long, env = "MEMORA_REMOTE_KEY")]
    pub key_path: String,
    /// Absolute path on the remote where ~/.memora/ should land
    #[arg(long, env = "MEMORA_REMOTE_PATH")]
    pub remote_path: String,
    /// SSH port
    #[arg(long, default_value_t = 22)]
    pub port: u16,
    /// Skip recordings: the `data/` + `exports/` dirs and media files
    /// (mp4/wav/jpg/...). Syncs the searchable db + config only â€” the fast
    /// way to put a huge ~/.memora on a small remote.
    #[arg(long, default_value_t = false)]
    pub no_media: bool,
    /// Exclude paths matching a glob (repeatable, `*` wildcards). Also reads
    /// `<data-dir>/.memoraignore` (one glob per line, `#` comments).
    #[arg(long = "exclude")]
...
pub fn get_or_create_machine_id(override_id: Option<String>) -> String {
    ...
    memora_core::sync::get_or_create_machine_id()
}
...
mod tests {
    ...
    fn test_incognito_flags_default_to_basic_filtering() {
        let cli = Cli::try_parse_from(["MEMORA", "record"]).unwrap();
        match cli.command {
            Command::Record(args) => {
    ...
    fn test_incognito_filter_can_be_disabled() {
        let cli = Cli::try_parse_from(["MEMORA", "record", "--ignore-incognito-windows=false"])
            .unwrap();
        match cli.command {
            Command::Record(args) => {
                ...
                assert!(
                    record_sources(["MEMORA", "record", "--ignore-incognito-windows=false",])
    ...
    fn test_enhanced_incognito_detection_is_explicit() {
        let cli = Cli::try_parse_from(["MEMORA", "record", "--enhanced-incognito-detection"])
            .unwrap();
        match cli.command {
            Command::Record(args) => {
                let settings = args.to_recording_settings();
    ...
    fn test_enable_semantic_context_flag_flows_to_recording_settings() {
        let cli =
            Cli::try_parse_from(["MEMORA", "record", "--enable-semantic-context"]).unwrap();
        match cli.command {
            Command::Record(args) => {
    ...
    fn test_app_context_modes_flow_to_recording_settings() {
        ...
        for (value, enabled, mode) in cases {
            let cli =
                Cli::try_parse_from(["MEMORA", "record", "--app-context", value]).unwrap();
            match cli.command {
                Command::Record(args) => {
                    let settings = args.to_recording_settings();
                    ...
                    assert!(
                        record_sources(["MEMORA", "record", "--app-context", value,])
                            .app_context,
    ...
    fn test_pause_on_drm_content_flag_sets_true() {
        let cli = Cli::try_parse_from(["MEMORA", "record", "--pause-on-drm-content"]).unwrap();
        match cli.command {
            Command::Record(args) => {
    ...
    fn test_keep_computer_awake_flag_flows_to_recording_settings() {
        let cli = Cli::try_parse_from(["MEMORA", "record", "--keep-computer-awake"]).unwrap();
        match cli.command {
            Command::Record(args) => {
                let settings = args.to_recording_settings();
                assert!(
    ...
    fn test_schedule_rule_rejects_bad_input() {
        ...
        assert!(
            Cli::try_parse_from(["MEMORA", "record", "--schedule-rule", "0,9am,18:00,all"])
                .is_err(),
    ...
    fn test_survey_command_parses() {
        let cli = Cli::try_parse_from(["MEMORA", "survey"]).unwrap();
        match cli.command {
            Command::Survey => {}
    ...
    fn test_setup_command_parses_with_default_api_url() {
        let cli = Cli::try_parse_from(["MEMORA", "setup"]).unwrap();
        match cli.command {
    ...
    fn test_setup_command_accepts_remote_api_url() {
        ...
        match cli.command {
            Command::Setup { api_url } => {
                assert_eq!(api_url, "https://MEMORA.example");
            }
            _ => panic!("expected Setup command"),
        }
    }
    ...
    fn test_profile_command_parses_with_defaults() {
        let cli = Cli::try_parse_from(["MEMORA", "profile"]).unwrap();
        match cli.command {
    ...
    fn test_profile_command_parses_port_and_json() {
        let cli =
            Cli::try_parse_from(["MEMORA", "profile", "--port", "4040", "--json"]).unwrap();
        match cli.command {
            Command::Profile { json, port } => {
                assert!(json);
                assert_eq!(port, 4040);
    ...
    fn test_monitor_id_disables_use_all_monitors_default() {
        ...
        match cli.command {
            Command::Record(args) => {
                ...
                // default for the bool arg â€” but the resolved settings must
    ...
    fn test_record_arg_sources_only_mark_explicit_cli_args() {
        let matches = Cli::command()
            .try_get_matches_from(["MEMORA", "record", "--port", "4040"])
            .unwrap();
    ...
    fn test_explicit_cli_overrides_preserve_app_settings_for_absent_defaults() {
        let cli =
            Cli::try_parse_from(["MEMORA", "record", "--port", "4040", "--disable-audio"])
                .unwrap();
        let sources = record_sources(["MEMORA", "record", "--port", "4040", "--disable-audio"]);
        let mut settings = memora_config::RecordingSettings {
            port: 3030,
            disable_audio: false,
    ...
    fn test_app_context_off_overrides_persisted_enabled_mode() {
        let args = ["MEMORA", "record", "--app-context", "off"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        let mut settings = memora_config::RecordingSettings {
            enable_semantic_context: true,
            semantic_context_mode: memora_config::SemanticContextMode::Both,
            ..Default::default()
        };
    ...
    fn test_app_context_automation_overrides_persisted_disabled_mode() {
        let args = ["MEMORA", "record", "--app-context", "automation"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        let mut settings = memora_config::RecordingSettings {
            enable_semantic_context: false,
            semantic_context_mode: memora_config::SemanticContextMode::Memory,
            ..Default::default()
        };
        ...
        assert_eq!(
            settings.semantic_context_mode,
            memora_config::SemanticContextMode::ComputerUse
        );
    }

    #[test]
    fn test_absent_app_context_arguments_preserve_persisted_settings() {
        let args = ["MEMORA", "record", "--port", "4040"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        let mut settings = memora_config::RecordingSettings {
            enable_semantic_context: true,
        ...
        assert_eq!(
            settings.semantic_context_mode,
            memora_config::SemanticContextMode::Both
        );
    ...
    fn test_monitor_id_flag_enables_vision_over_persisted_disable() {
        ...
        let args = ["MEMORA", "record", "--monitor-id", "5"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        let mut settings = memora_config::RecordingSettings {
            disable_vision: true,
    ...
    fn test_default_true_bools_default_to_true_without_flag() {
        let args = record_args(["MEMORA", "record"]);
    ...
    fn test_default_true_bools_accept_space_separated_value() {
        let args = record_args(["MEMORA", "record", "--use-pii-removal", "false"]);
        assert!(
            !args.use_pii_removal,
            "--use-pii-removal false must disable"
        );
    ...
    fn test_eq_false_registers_as_cli_source_and_overrides_persisted_settings() {
        let args = ["MEMORA", "record", "--use-pii-removal=false"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        assert!(
