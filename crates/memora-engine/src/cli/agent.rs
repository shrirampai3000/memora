...
//! `MEMORA agent setup` â€” make an external agent (OpenClaw, Hermes, Claude)
//! aware of MEMORA in one command: install the MEMORA skill(s) into the
//! agent's skills directory and register the MEMORA MCP server in its config.
//!
//! The skills are host-aware: pass `--api-url` (e.g. when the agent runs on a
//! VPS that holds a synced copy of your MEMORA data) and the written
...
// Canonical skill sources â€” single source of truth lives in memora-core
...
const API_SKILL_MD: &str =
    include_str!("../../../memora-core/assets/skills/MEMORA-api/SKILL.md");
const CLI_SKILL_MD: &str =
    include_str!("../../../memora-core/assets/skills/MEMORA-cli/SKILL.md");

#[derive(clap::Subcommand, Debug)]
pub enum AgentCommand {
    /// Install the MEMORA skills + MCP server into one agent or every
    ...
    Setup {
        ...
        all: bool,
        /// MEMORA REST API base URL the skill + MCP should target.
        /// Default `http://localhost:3030` (agent co-located with the engine).
        /// Set this when the agent runs elsewhere â€” e.g. a VPS holding a synced
        /// copy of your data â€” so the skill points at the right host.
        #[arg(long, default_value = "http://localhost:3030")]
        api_url: String,
    },
    /// Remove the MEMORA skills + MCP server entry from an agent.
...
pub fn maybe_prompt_connect_detected(data_dir: &Path) {
    if std::env::var("MEMORA_NO_REMINDERS").is_ok()
        || std::env::var("MEMORA_NO_AGENT_SETUP_PROMPT").is_ok()
    ...
    eprintln!(
        "  add MEMORA MCP + supported skills to every detected tool? existing settings are preserved."
    );
    ...
    if accepted {
        ...
        if let Err(error) = setup_all_detected("http://localhost:3030") {
            ...
            eprintln!("  retry with {}", "MEMORA setup".green().bold());
        }
    } else {
        eprintln!(
            "  not now â€” run {} whenever you're ready.\n",
            "MEMORA setup".green().bold()
        );
    }
}
...
fn is_agent_setup_in(target: &str, home: &Path) -> bool {
    ...
    skills_ready(&layout) && has_MEMORA_mcp(&layout)
}

fn has_MEMORA_mcp(layout: &AgentLayout) -> bool {
    ...
    match layout.mcp_format {
        McpFormat::Json => serde_json::from_str::<serde_json::Value>(&existing)
            .ok()
            .and_then(|root| root.get("mcpServers")?.get("MEMORA").cloned())
            .is_some_and(|entry| !entry.is_null()),
        McpFormat::Toml => existing.lines().any(|line| {
            line.trim() == "[mcp_servers.memora]"
                || line.trim() == "[mcp_servers.\"MEMORA\"]"
        }),
        McpFormat::Yaml => existing.lines().any(|line| {
            let line = line.trim_start();
            !line.starts_with('#')
                && (line.starts_with("MEMORA:") || line.contains("MEMORA-mcp"))
        }),
    }
}
...
pub fn setup_all_detected(api_url: &str) -> Result<()> {
    ...
    let pending = detected
        ...
        .collect::<Vec<_>>();
    if pending.is_empty() {
        println!("all detected AI tools are already connected to MEMORA");
        return Ok(());
    }
    ...
    if failures.is_empty() {
        println!(
            "\nconnected MEMORA to {} detected AI tool(s)",
            pending.len()
        );
        Ok(())
    } else {
...
fn desktop_launch_config(
    ...
    let mut env = BTreeMap::from([
        ("MEMORA_API_URL".to_string(), api_url.to_string()),
        ("MEMORA_MCP_CLIENT".to_string(), agent.id.to_string()),
    ]);
    if let Some(api_key) = api_key {
        env.insert("MEMORA_LOCAL_API_KEY".to_string(), api_key.to_string());
    }
    McpLaunchConfig {
        command: bun_path.to_string_lossy().to_string(),
        args: vec!["x".to_string(), "MEMORA-mcp@latest".to_string()],
        env,
...
fn install_missing_desktop_skills(layout: &AgentLayout) -> Result<Vec<DesktopSkillChange>> {
    ...
    for (name, markdown) in [
        ("MEMORA-api", API_SKILL_MD),
        ("MEMORA-cli", CLI_SKILL_MD),
    ] {
...
fn layout_in(target: &str, h: &Path) -> Result<AgentLayout> {
    Ok(match target {
        ...
        // and, for compat, ~/.claude/skills + ~/.codex/skills) â€” see
...
/// Read a config file. Missing â†’ None (caller starts fresh). Present but
/// unreadable (permissions, IO) â†’ error â€” never treated as empty, which is
...
fn read_config_text(path: &Path) -> Result<Option<String>> {
    match std::fs::read_to_string(path) {
        ...
        Err(e) => anyhow::bail!(
            "could not read {} ({e}) â€” fix its permissions and retry",
            path.display()
        ),
    }
}

/// How many `.memora-backup-*` siblings to keep per config file.
const MAX_CONFIG_BACKUPS: usize = 2;

fn config_lock_path(path: &Path) -> PathBuf {
    PathBuf::from(format!("{}.memora.lock", path.display()))
}
...
fn replace_config(path: &Path, expected: Option<&str>, contents: &str) -> Result<()> {
    ...
    anyhow::ensure!(
        ...
        "{} changed while MEMORA was preparing its MCP entry; left the newer file untouched",
        path.display()
    );
    ...
    if current.is_some() {
        let ts = std::time::SystemTime::now()
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let backup = PathBuf::from(format!("{}.memora-backup-{ts}", path.display()));
...
fn prune_config_backups(path: &Path) {
    ...
    let prefix = format!("{name}.memora-backup-");
...
fn write_skill(skills_dir: &Path, name: &str, md: &str, api_url: &str) -> Result<PathBuf> {
    ...
    // target host so an off-box agent hits the right MEMORA.
...
fn setup(target: &str, api_url: &str) -> Result<()> {
    ...
    println!(
        "\ndone â€” restart {} so it loads the MEMORA {}, then ask it:\n  \"what was i doing yesterday afternoon?\"",
        l.name,
        if has_skills { "skills + MCP" } else { "MCP" }
    );
    if remote {
        println!(
            "\nnote: the agent will query MEMORA at {api_url}. make sure that host\n      is reachable from the agent (run `MEMORA record` there over your\n      synced data, or expose the API via a tunnel)."
        );
    }
    Ok(())
}

/// Install the canonical MEMORA API and CLI skills for an external agent.
///
...
/// the exact same skill installation behavior as `MEMORA agent setup`.
...
fn install_skills_in(target: &str, api_url: &str, home: &Path) -> Result<Vec<PathBuf>> {
    ...
    Ok(vec![
        write_skill(skills_dir, "MEMORA-api", API_SKILL_MD, api_url)?,
        write_skill(skills_dir, "MEMORA-cli", CLI_SKILL_MD, api_url)?,
    ])
}

/// Remove the two built-in MEMORA skills from an external agent.
///
/// Mirror of [`install_skills`]: deletes only `<skills_dir>/MEMORA-api`
/// and `<skills_dir>/MEMORA-cli`, never the parent skills directory or any
...
fn remove_skills_from(skills_dir: &Path) -> Result<Vec<PathBuf>> {
    ...
    for name in ["MEMORA-api", "MEMORA-cli"] {
        let dir = skills_dir.join(name);
        if dir.exists() {
...
/// `MEMORA agent remove <target>` â€” undo `setup`. Removes the two
/// MEMORA skills and the MEMORA MCP entry; idempotent, missing
/// files/entries are a no-op.
fn remove(target: &str) -> Result<()> {
    let l = layout(target)?;
    println!("removing MEMORA from {}", l.name);

    let removed = remove_skills(target)?;
    if l.skills_dir.is_none() {
        println!("  Â· {} is MCP-only (no skills dir)", l.name);
    } else if removed.is_empty() {
        println!("  Â· no MEMORA skills installed");
    } else {
        for path in removed {
            println!("  âœ“ removed skill {}", path.display());
        }
    }
    ...
    println!(
        "\ndone â€” restart {} so it drops the MEMORA tools.",
        l.name
    );
    Ok(())
}

/// Remove `mcpServers.memora` from a JSON config, preserving everything
...
fn remove_mcp_json(path: &Path) -> Result<()> {
    ...
    let existing = match read_config_text(path)? {
        ...
        _ => {
            println!("  Â· no MEMORA mcp entry in {}", path.display());
            return Ok(());
        }
    };
    ...
    println!("  âœ“ mcp removed from {}", path.display());
    Ok(())
}

/// Strip the `[mcp_servers.memora]` table and its `.env` subtable from a
...
fn remove_mcp_toml(path: &Path) -> Result<()> {
    ...
    if !existing.contains("[mcp_servers.memora]") {
        println!("  Â· no MEMORA mcp entry in {}", path.display());
        return Ok(());
    }
    let mut out: Vec<&str> = Vec::new();
    let mut in_MEMORA = false;
    for line in existing.lines() {
        let trimmed = line.trim();
        if trimmed == "[mcp_servers.memora]" || trimmed == "[mcp_servers.memora.env]" {
            in_MEMORA = true;
            continue;
        }
        if in_MEMORA && trimmed.starts_with('[') {
            in_MEMORA = false;
        }
        if !in_MEMORA {
            out.push(line);
...
/// Remove only the exact `MEMORA:` child `merge_mcp_yaml` writes. Other
...
fn remove_mcp_yaml(path: &Path) -> Result<()> {
    ...
    // Comment-aware: a commented `# MEMORA` mention isn't an entry.
    let has_uncommented_MEMORA = existing
        .lines()
        .any(|l| !l.trim_start().starts_with('#') && l.contains("MEMORA"));
    if !has_uncommented_MEMORA {
        println!("  Â· no MEMORA mcp entry in {}", path.display());
        return Ok(());
    }
    ...
    let Some(block_start) = lines.iter().position(|line| line == "mcp_servers:") else {
        println!(
            "  â€¢ {} references MEMORA outside an mcp_servers block â€” remove it manually",
            path.display()
        );
        return Ok(());
    };
    ...
    let Some(server_start) = (block_start + 1..block_end)
        .find(|index| yaml_indent(&lines[*index]) == 2 && lines[*index].trim() == "MEMORA:")
    else {
        println!(
            "  â€¢ {} has a customized MEMORA MCP entry â€” remove it manually",
            path.display()
        );
        return Ok(());
    };
...
fn cli_launch_config(remote: bool, api_url: &str) -> McpLaunchConfig {
    let env = if remote {
        BTreeMap::from([("MEMORA_API_URL".to_string(), api_url.to_string())])
    } else {
        BTreeMap::new()
    };
    McpLaunchConfig {
        command: "npx".to_string(),
        args: vec!["-y".to_string(), "MEMORA-mcp@latest".to_string()],
        env,
        transport: None,
    }
}
...
/// Idempotently add the `MEMORA` server to a JSON MCP config (OpenClaw,
...
fn merge_mcp_json_launch(path: &Path, launch: &McpLaunchConfig) -> Result<()> {
    ...
    servers.insert("MEMORA".to_string(), entry);
    replace_config(
        path,
        existing.as_deref(),
        &(serde_json::to_string_pretty(&root)? + "\n"),
    )?;
    println!("  âœ“ mcp   {}", path.display());
    Ok(())
}

/// Add the `MEMORA` server to a YAML MCP config (Hermes). We don't pull a
...
fn merge_mcp_yaml_launch(path: &Path, launch: &McpLaunchConfig) -> Result<()> {
    ...
    if uncommented("MEMORA-mcp") {
        anyhow::bail!(
            "{} references MEMORA-mcp outside a standard mcp_servers block; left it untouched",
            path.display()
        );
    }
...
/// Add or repair the `MEMORA` server in a TOML MCP config (Codex),
...
fn strip_MEMORA_toml_tables(existing: &str) -> String {
    let mut out = Vec::new();
    let mut in_MEMORA = false;
    for line in existing.lines() {
        let trimmed = line.trim();
        let MEMORA_header = matches!(
            trimmed,
            "[mcp_servers.memora]"
                | "[mcp_servers.memora.env]"
                | "[mcp_servers.\"MEMORA\"]"
...
fn merge_mcp_toml_launch(path: &Path, launch: &McpLaunchConfig) -> Result<()> {
    ...
    let existing_value = read_config_text(path)?;
    let mut out = strip_MEMORA_toml_tables(existing_value.as_deref().unwrap_or_default());
    if !out.is_empty() && !out.ends_with('\n') {
        out.push('\n');
    }
    if !out.is_empty() {
...
mod tests {
    ...
    fn test_api_skill_routes_learning_to_user_owned_skills() {
        ...
        assert!(API_SKILL_MD.contains("Never modify this `MEMORA-api` skill"));
    ...
    #[test]
    fn test_remove_skills_deletes_only_MEMORA_dirs() {
        let dir = std::env::temp_dir().join(format!("sp-agent-remove-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);

        // Two MEMORA skills plus a user skill that must survive.
        write_skill(&dir, "MEMORA-api", "api", "http://localhost:3030").unwrap();
        write_skill(&dir, "MEMORA-cli", "cli", "http://localhost:3030").unwrap();
        write_skill(&dir, "my-own-skill", "mine", "http://localhost:3030").unwrap();

        let removed = remove_skills_from(&dir).unwrap();
        assert_eq!(
            removed,
            vec![dir.join("MEMORA-api"), dir.join("MEMORA-cli")]
        );
        assert!(!dir.join("MEMORA-api").exists());
        assert!(!dir.join("MEMORA-cli").exists());
    ...
    fn test_merge_mcp_json_fresh_and_idempotent() {
        ...
        assert_eq!(v["mcpServers"]["MEMORA"]["command"], "npx");
        assert!(v["mcpServers"]["MEMORA"]["env"].is_null());
        ...
        assert_eq!(
            v["mcpServers"]["MEMORA"]["env"]["MEMORA_API_URL"],
            "http://box:3030"
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn test_replace_config_backs_up_prunes_and_leaves_no_tmp() {
        ...
        let backups = names
            .iter()
            .filter(|n| n.contains(".memora-backup-"))
            .count();
        assert!(
    ...
    fn test_replace_config_rejects_a_concurrent_edit() {
        ...
        let error = replace_config(&path, Some("stale edit"), "MEMORA edit").unwrap_err();
        assert!(error.to_string().contains("changed while MEMORA"));
    ...
    fn test_authenticated_config_replacement_is_owner_only() {
        ...
        let launch = McpLaunchConfig {
            ...
            args: vec!["x".to_string(), "MEMORA-mcp@latest".to_string()],
            env: BTreeMap::from([(
                "MEMORA_LOCAL_API_KEY".to_string(),
                "sp-secret".to_string(),
            )]),
            transport: None,
        };
    ...
    fn test_remove_mcp_json_preserves_other_servers() {
        ...
        std::fs::write(
            &path,
            serde_json::json!({
                "mcpServers": {"other": {"command": "x"}, "MEMORA": {"command": "bun"}},
                "theme": "dark"
            })
            .to_string(),
        )
        ...
        assert!(v["mcpServers"]["MEMORA"].is_null());
    ...
    fn test_remove_mcp_toml_strips_table_and_env() {
        ...
        assert!(!s.contains("mcp_servers.memora"));
        assert!(!s.contains("MEMORA_API_URL"));
    ...
    fn test_merge_mcp_yaml_ignores_commented_example_block() {
        ...
        // example â€” merge must append a real block, not bail to manual mode.
    ...
    fn test_merge_and_remove_mcp_yaml_preserves_existing_servers() {
        ...
        assert!(merged.contains("  MEMORA:\n    command: \"npx\""));
        assert!(merged.contains("MEMORA_API_URL: \"http://box:3030\""));
    ...
    fn test_remove_mcp_yaml_leaves_foreign_block() {
        ...
        // Hand-authored: MEMORA alongside another server â€” must not touch.
        let content =
            "mcp_servers:\n  MEMORA:\n    url: http://custom\n  other:\n    command: x\n";
    ...
    fn test_merge_mcp_toml() {
        ...
        assert!(s.contains("model = \"o3\""));
        assert!(s.contains("[mcp_servers.memora]"));
        assert!(s.contains("MEMORA_API_URL = \"http://box:3030\""));

        // idempotent: no duplicate table.
        merge_mcp_toml(&path, true, "http://box:3030").unwrap();
        let s2 = std::fs::read_to_string(&path).unwrap();
        assert_eq!(s2.matches("[mcp_servers.memora]").count(), 1);
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn test_claude_skills_directory_alone_does_not_invent_claude_code() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(dir.path().join(".claude/skills/MEMORA-api")).unwrap();
    ...
    fn test_desktop_background_setup_connects_detected_tools_end_to_end() {
        ...
        let bun = home.join("MEMORA-runtime/bun");
        ...
        assert!(codex.contains("MEMORA_LOCAL_API_KEY = \"sp-test-key\""));
        assert!(codex.contains("MEMORA_MCP_CLIENT = \"codex\""));
        assert!(codex.contains("MEMORA_API_URL = \"http://localhost:31337\""));
        assert!(home.join(".codex/skills/MEMORA-api/SKILL.md").is_file());
        assert!(home.join(".codex/skills/MEMORA-cli/SKILL.md").is_file());
        ...
        assert_eq!(
            cursor["mcpServers"]["MEMORA"]["command"],
            bun.to_string_lossy().as_ref()
        );
        assert_eq!(
            cursor["mcpServers"]["MEMORA"]["env"]["MEMORA_LOCAL_API_KEY"],
            "sp-test-key"
        );
        ...
        assert_eq!(openclaw["mcpServers"]["MEMORA"]["transport"], "stdio");

        let hermes = std::fs::read_to_string(home.join(".hermes/config.yaml")).unwrap();
        assert!(hermes.contains("model: test"));
        assert!(hermes.contains("MEMORA_MCP_CLIENT: \"hermes\""));
        assert!(hermes.contains("MEMORA_LOCAL_API_KEY: \"sp-test-key\""));
    ...
    fn test_desktop_background_setup_repairs_a_stale_mcp_entry() {
        ...
        std::fs::write(
            home.join(".codex/config.toml"),
            "model = \"gpt-5\"\n\n[mcp_servers.memora]\ncommand = \"npx\"\nargs = [\"stale-package\"]\n\n[mcp_servers.memora.env]\nMEMORA_LOCAL_API_KEY = \"stale-key\"\n",
        )
        .unwrap();
        write_skill(
            &home.join(".codex/skills"),
        ...
        assert!(config.contains("MEMORA_LOCAL_API_KEY = \"current-key\""));
        assert!(config.contains("MEMORA_API_URL = \"http://localhost:4242\""));
        assert!(!config.contains("stale-package"));
        assert!(!config.contains("stale-key"));
        assert_eq!(config.matches("[mcp_servers.memora]").count(), 1);
    }
    ...
    fn test_desktop_background_setup_removes_stale_key_when_auth_is_disabled() {
        ...
        assert!(!config.contains("MEMORA_LOCAL_API_KEY"));
        assert!(config.contains("MEMORA_API_URL = \"http://localhost:3030\""));
        assert_eq!(config.matches("[mcp_servers.memora]").count(), 1);
    }
    ...
    fn test_desktop_background_setup_isolates_invalid_config_failure() {
        ...
        std::fs::create_dir_all(home.join(".cursor/skills/MEMORA-api")).unwrap();
        std::fs::write(
            home.join(".cursor/skills/MEMORA-api/SKILL.md"),
        ...
        assert_eq!(
            std::fs::read_to_string(home.join(".cursor/skills/MEMORA-api/SKILL.md")).unwrap(),
            "custom pre-existing skill"
        );
        assert!(!home.join(".cursor/skills/MEMORA-cli").exists());
        assert!(home.join(".codex/skills/MEMORA-api/SKILL.md").is_file());
    }
    ...
    fn test_setup_all_cli_flag_is_explicit_and_conflicts_with_target() {
        ...
        let cli =
            crate::cli::Cli::try_parse_from(["MEMORA", "agent", "setup", "--all"]).unwrap();
        assert!(matches!(
            cli.command,
        ...
        assert!(crate::cli::Cli::try_parse_from(["MEMORA", "agent", "setup"]).is_err());
