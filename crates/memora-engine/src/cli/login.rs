...
/// Handle `MEMORA login` â€” opens browser, polls server for token.
pub async fn handle_login_command() -> anyhow::Result<()> {
    ...
    if current_cloud_token(&data_dir).await.is_some() {
        println!("already logged in. use `MEMORA whoami` to check status.");
        return Ok(());
    }
    ...
    // machine, sign in, approve) routinely takes longer than 5 min â€” the old
    ...
    loop {
        ...
        match status {
            "complete" => {
                ...
                if token.is_empty() {
                    println!();
                    anyhow::bail!("authentication failed â€” no token received");
                }
...
/// Handle `MEMORA logout` â€” clear cloud auth from the shared SecretStore.
///
...
pub async fn handle_logout_command() -> anyhow::Result<()> {
    ...
    println!();
    println!("  run `MEMORA login` to sign back in");
    println!();

    Ok(())
}

/// Handle `MEMORA whoami` â€” show current auth status.
    ...
    match token {
        Some(t) if !t.is_empty() => {
            ...
            let source = if explicit_is_cloud {
                "MEMORA_API_KEY env var"
            } else {
                "MEMORA cloud session store"
            };
        ...
        _ => {
            ...
            println!();
            println!("  run `MEMORA login` to authenticate");
            println!();
        }
    }
