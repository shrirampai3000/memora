...
/// Handle connection subcommands (standalone â€” does NOT require a running server).
pub async fn handle_connection_command(command: &ConnectionCommand) -> anyhow::Result<()> {
    let MEMORA_dir = memora_core::paths::default_MEMORA_data_dir();
    ...
    let legacy_or_dedicated_exists = MEMORA_dir.join("db.sqlite").exists()
        || memora_secrets::secrets_database_path(&MEMORA_dir).exists();
    let secret_store = if legacy_or_dedicated_exists {
        match memora_secrets::SecretStore::open_for_data_dir_with_vault_key(&MEMORA_dir)
            .await
        {
    ...
    let cm = ConnectionManager::new(MEMORA_dir.clone(), secret_store);

    // Fetch browsers from the running engine â€” they live in the in-memory
    ...
    match command {
        ConnectionCommand::List { json: use_json } => {
            ...
            // Add WhatsApp â€” check session on disk AND whether gateway is actually reachable
            ...
            // Connected entries lead â€” pi-cli's bash tool truncates large
        ...
        ConnectionCommand::Get { id, json: use_json } => {
            ...
            // Browser registry â€” `connection get <browser-id>` should describe
            ...
            if let Some(creds) = has_creds {
                ...
                println!("\nhint: MEMORA connection set {} key=value ...", id);
            }
        }
        ...
        ConnectionCommand::Set { id, credentials } => {
            ...
            for pair in credentials {
                let (key, value) = pair
                    .split_once('=')
                    .ok_or_else(|| anyhow::anyhow!("invalid format '{}' â€” use key=value", pair))?;
        ...
        ConnectionCommand::Test { id } => {
            // For OAuth integrations, test with empty creds â€” the test()
...
// ---------------------------------------------------------------------------
// Browser registry â€” the engine's in-memory browser list. Lives only in the
...
async fn fetch_running_browsers() -> Vec<BrowserSummaryWire> {
    // Auth lookup goes through `crate::auth_key::find_api_auth_key` â€”
