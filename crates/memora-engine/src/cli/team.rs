...
//! `MEMORA team` â€” enterprise admin queries against the org's team
//! API: `https://github.com/shrirampai3000/memora/api/enterprise/v1/*` for hosted orgs, or the
//! customer-run query gateway for write-only-archive orgs.
//!
//! Authoritative spec for parameters + permissions is the
//! `MEMORA-team` skill at
//! `crates/memora-core/assets/skills/MEMORA-team/SKILL.md` â€” this
...
//!
//! Auth: `team_api_token` from `~/.memora/enterprise.json` (admin
//! mints it once at <https://github.com/shrirampai3000/memora/enterprise?tab=tokens>). Override
//! via `MEMORA_TEAM_API_TOKEN` env var for scripts/CI. Base URL
//! resolution (same contract as `packages/MEMORA-mcp`):
//! `MEMORA_TEAM_API_URL` env (full v1 base) â†’ legacy
//! `MEMORA_CLOUD_BASE_URL` env (origin) â†’ `gateway_url` in
//! enterprise.json (gateway orgs) â†’ hosted default.
//!
//! Skips the local sp daemon entirely â€” calls go straight to the cloud,
//! so this works on any machine the admin has signed into (CI, a fresh
//! laptop, a server), not just one running MEMORA locally.
//!
...
/// Hosted default. github.com/shrirampai3000/memora, matching the MCP + bundled skill â€” this
/// file used to say github.com/shrirampai3000/memora while every other client said
/// github.com/shrirampai3000/memora; the drift is fixed on the github.com/shrirampai3000/memora side.
const DEFAULT_V1_BASE: &str = "https://github.com/shrirampai3000/memora/api/enterprise/v1";
const ENV_TOKEN: &str = "MEMORA_TEAM_API_TOKEN";
const ENV_TEAM_API_URL: &str = "MEMORA_TEAM_API_URL";
const ENV_BASE_URL: &str = "MEMORA_CLOUD_BASE_URL";

const READ_TOKEN_HELP: &str = "no team_api_token found.

Open https://github.com/shrirampai3000/memora/enterprise?tab=tokens, mint a token with scopes
`read:devices`, `read:search`, `read:records`, then either:

  - paste it into desktop Settings â†’ Privacy â†’ Admin Team API Token
    (writes ~/.memora/enterprise.json), or
  - export MEMORA_TEAM_API_TOKEN=<token> for this shell.";
...
pub(crate) struct TeamEnv {
    ...
    /// Full v1 base, e.g. `https://github.com/shrirampai3000/memora/api/enterprise/v1` or a
...
/// Base resolution, same contract as packages/MEMORA-mcp: explicit
/// full-v1-base env â†’ legacy origin env â†’ enterprise.json gateway_url
/// (written by the desktop app from the org's storage-binding gateway
/// URL) â†’ hosted default.
fn resolve_v1_base(
...
fn read_enterprise_json() -> Option<Value> {
    ...
    let path: PathBuf = home.join(".memora").join("enterprise.json");
...
async fn search(
    ...
    {
        eprintln!(
            "warning: response truncated â€” narrow `--since`/`--app` and re-run rather than raising `--limit`"
        );
    }
...
fn parse_iso(s: &str) -> Result<DateTime<Utc>, String> {
    s.parse::<DateTime<Utc>>().map_err(|_| {
        format!(
            "invalid ISO 8601 timestamp '{}' â€” expected e.g. 2026-01-15T10:00:00Z",
            s
        )
    })
}
...
/// malformed input â€” callers map that to a clap-style error.
...
async fn get_json(
    ...
    let resp = client
        ...
        .send()
        .await
        .with_context(|| format!("GET {} â€” couldn't reach the team API (offline?)", url))?;
    ...
    let hint = match status {
        StatusCode::UNAUTHORIZED => {
            ...
            Re-mint at https://github.com/shrirampai3000/memora/enterprise?tab=tokens."
        }
        ...
        StatusCode::TOO_MANY_REQUESTS => {
            "rate limited â€” narrow your query \
...
mod tests {
    ...
    fn parse_duration_units() {
        ...
        // The unit token is trimmed, so internal whitespace is fine â€”
        // this mirrors the MEMORA-api skill's lenient parsing.
    ...
    fn push_time_params_since() {
        ...
        // value is rfc3339 â€” just sanity check shape
    ...
    fn parses_team_devices() {
        let cli = Cli::try_parse_from(["MEMORA", "team", "devices", "--raw"]).unwrap();
        match cli.command {
            Command::Team {
    ...
    fn parses_team_records_requires_device_id() {
        ...
        let res = Cli::try_parse_from(["MEMORA", "team", "records"]);
    ...
    fn trim_caps_long_strings() {
        ...
        let t = trim(&long);
        // 500 ASCII chars + 3 UTF-8 bytes for "â€¦"
    ...
    fn v1_base_resolution_covers_env_gateway_and_default() {
        // Default: hosted github.com/shrirampai3000/memora (github.com/shrirampai3000/memora drift fixed).
        assert_eq!(
            resolve_v1_base(None, None, None),
            "https://github.com/shrirampai3000/memora/api/enterprise/v1"
        );
        // enterprise.json gateway_url â†’ gateway orgs, trailing slash trimmed.
        ...
        assert_eq!(
            resolve_v1_base(None, Some("https://staging.github.com/shrirampai3000/memora"), Some(&ent)),
            "https://staging.github.com/shrirampai3000/memora/api/enterprise/v1"
        );
        // Explicit full-base env beats everything.
        assert_eq!(
            resolve_v1_base(
                Some("http://127.0.0.1:3041/api/enterprise/v1"),
                Some("https://staging.github.com/shrirampai3000/memora"),
                Some(&ent)
            ),
