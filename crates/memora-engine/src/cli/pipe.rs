...
use memora_core::pipes::PipeManager;
use serde_json::{json, Value};
use std::collections::HashMap;
use std::sync::Arc;

/// Handle pipe subcommands (standalone â€” does NOT require a running server).
pub async fn handle_pipe_command(command: &PipeCommand) -> anyhow::Result<()> {
    ...
    // MEMORA_API_KEY is also the local HTTP bearer returned by `auth token`.
    ...
    let user_token = if matches!(command, PipeCommand::Run { .. }) {
        crate::auth_key::resolve_cloud_token(&data_dir, std::env::var("MEMORA_API_KEY").ok())
            .await
    } else {
        None
    };
    ...
    match command {
        PipeCommand::List { json } => {
            ...
            if *json {
                ...
                println!("\nhint: create one at ~/.memora/pipes/my-pipe/pipe.md");
            } else {
                println!(
                    "{:<20} {:<10} {:<12} {:<20}",
        ...
        PipeCommand::Logs { name, follow: _ } => {
            ...
            if manager.get_pipe(name).await.is_none() {
                ...
                if !pipe_dir.exists() {
                    anyhow::bail!(
                        "pipe '{}' not found â€” directory does not exist: {}\nhint: install it first with `MEMORA pipe install <source>`",
                        name,
                        pipe_dir.display()
                    );
                }
                ...
                for log in &logs {
                    ...
                    println!(
                        ...
                        if !log.success && !log.stderr.is_empty() {
                            format!("â€” {}", log.stderr.lines().next().unwrap_or(""))
                        } else {
                            String::new()
                        }
                    );
...
pub fn api_base_url() -> String {
    std::env::var("MEMORA_API_BASE_URL")
...
pub async fn get_cloud_auth_token() -> Option<String> {
    let data_dir = memora_core::paths::default_MEMORA_data_dir();
    crate::auth_key::resolve_cloud_token(&data_dir, std::env::var("MEMORA_API_KEY").ok()).await
}
...
async fn handle_publish_command(name: &str, pipes_dir: &std::path::Path) -> anyhow::Result<()> {
    ...
    if !pipe_md_path.exists() {
        anyhow::bail!(
            "pipe.md not found at {}\nhint: make sure the pipe '{}' exists in ~/.memora/pipes/",
    ...
    let token = get_cloud_auth_token()
        .await
        .ok_or_else(|| anyhow::anyhow!("no cloud session found. run `MEMORA login`"))?;
...
async fn handle_info_command(slug: &str) -> anyhow::Result<()> {
    ...
    println!();
    println!("install with: MEMORA pipe install {}", slug);
...
/// Handle `pipe models â€¦` subcommands. Pure presentation â€” store IO and
...
fn model_list(json: bool) -> anyhow::Result<()> {
    ...
    if presets.is_empty() {
        ...
        println!(
            "\nhint: create one with `MEMORA pipe models create <id> --provider â€¦ --model â€¦`"
        );
        return Ok(());
    }
...
fn model_create(
    ...
    println!(
        ...
        model,
        if set_default { " â€” now default" } else { "" }
    );
    Ok(())
}
...
fn model_delete(id: &str, force: bool) -> anyhow::Result<()> {
    ...
    if !report.deleted {
        eprintln!(
            "refusing to delete '{}' â€” referenced by {} pipe(s):",
            id,
    ...
    if !report.referencing_pipes.is_empty() {
        println!(
            "deleted preset '{}' (was referenced by {} pipe(s) â€” they will fall back to the default):",
            id,
            report.referencing_pipes.len()
        );
        for name in &report.referencing_pipes {
    ...
    if report.was_default {
        println!("warning: deleted preset was the default â€” set a new one with `pipe models set-default <id>`");
    }
    Ok(())
}
...
/// â€” useful when chaining with a preset that's about to be created.
async fn handle_set_preset_command(
    ...
    if manager.get_pipe(name).await.is_none() {
        anyhow::bail!(
            "pipe '{}' not found â€” install it first with `MEMORA pipe install <source>`",
            name
        );
    }
