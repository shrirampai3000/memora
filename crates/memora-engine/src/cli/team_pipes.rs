...
Open https://github.com/shrirampai3000/memora/enterprise?tab=tokens, mint an admin token with the
...
async fn list_pipes(client: &Client, env: &TeamEnv, args: &TeamPipeListArgs) -> anyhow::Result<()> {
    ...
    if pipes.is_empty() {
        ...
        println!("\nhint: MEMORA team pipes deploy ./my-pipe/pipe.md");
        return Ok(());
    }
...
async fn schedule_pipe(
    ...
    let pipe = find_pipe(&current, &args.name).ok_or_else(|| {
        anyhow::anyhow!(
            "managed Pipe '{}' not found\nhint: run `MEMORA team pipes list`",
            args.name
        )
    })?;
...
fn payload_from_source(args: &TeamPipeSpecArgs) -> anyhow::Result<Value> {
    ...
    let (config, prompt_body) = memora_core::pipes::parse_frontmatter(&source)
...
async fn get_pipes(client: &Client, env: &TeamEnv) -> anyhow::Result<Value> {
    ...
    let response = client
        ...
        .send()
        .await
        .with_context(|| format!("GET {} â€” couldn't reach the Enterprise API", url))?;
...
async fn post_pipe(client: &Client, env: &TeamEnv, payload: &Value) -> anyhow::Result<Value> {
    ...
    let response = client
        ...
        .send()
        .await
        .with_context(|| format!("POST {} â€” couldn't reach the Enterprise API", url))?;
    parse_pipe_response(response, &url).await
}
...
async fn parse_pipe_response(response: reqwest::Response, url: &str) -> anyhow::Result<Value> {
    ...
    let hint = match status {
        ...
        StatusCode::NOT_IMPLEMENTED => {
            "managed Pipe writes are hosted-only; use the github.com/shrirampai3000/memora control-plane base"
        }
