...
//! `MEMORA profile` â€” per-stage timing for the capture pipeline.
//!
...
pub async fn handle_profile_command(json: bool, port: u16) -> anyhow::Result<()> {
    ...
    let resp = match resp {
        Ok(r) => r,
        Err(_) => {
            eprintln!("MEMORA does not appear to be running on port {port}.");
            eprintln!(
                "start it (`MEMORA`) â€” or pass --port â€” then run `MEMORA profile` again."
            );
            std::process::exit(1);
        }
    };
...
/// fields render as `n/a` rather than failing â€” the report degrades gracefully
/// against an older/partial server response.
fn format_profile_report(health: &Value) -> String {
    let mut out = String::new();
    out.push_str("MEMORA profile â€” per-stage timing (since server start)\n\n");
    ...
    match health.get("pipeline").filter(|v| !v.is_null()) {
        ...
        None => out.push_str("vision pipeline   (no data â€” vision disabled or not started)\n"),
    }
    ...
    match health.get("audio_pipeline").filter(|v| !v.is_null()) {
        ...
        None => out.push_str("audio pipeline    (no data â€” audio disabled or not started)\n"),
    }
