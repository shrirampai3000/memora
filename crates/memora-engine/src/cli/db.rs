...
//! `MEMORA db ...` â€” corruption recovery + storage cleanup.
//!
//! ## Safety model
//!
//! These commands rewrite or delete files inside `~/.memora/`. A racing
...
//!
//!   * `~/.memora/.db_recovery.lock` â€” JSON `{pid, host, started_at, op}`
...
//!     (`apps/memora-app-tauri/src-tauri/src/main.rs`); env var
//!     `MEMORA_IGNORE_DB_LOCK=1` is the escape hatch.
//!
//! ## When the lock is "stale"
//!
//! 1. Foreign host (lock written from another machine over a shared `$HOME`):
//!    we **refuse to clear** automatically â€” print the path and tell the user.
//! 2. Same host, PID gone: clear and proceed.
//! 3. Same host, PID alive: refuse, point at `MEMORA db unlock`.
//! 4. Same host, PID unknown, mtime > 1 h: clear (heartbeat would have kept it
//!    fresh; older means the heartbeat thread is dead too).
//!
//! ## Recovery pre-flight
//!
//!  * SQLite's official recovery extension is compiled into MEMORA
//!  * free disk â‰¥ 2Ã— the DB/WAL/SHM generation size
...
struct RecoveryManifest {
    ...
    phase: RecoveryPhase,
    live_path: PathBuf,
    original_identity: memora_db::SqliteFileIdentity,
    candidate_identity: Option<memora_db::SqliteFileIdentity>,
    started_at_unix_ms: u64,
}

// â”€â”€ lock file payload â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ lock guard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

struct DbLock {
    path: PathBuf,
    /// Set to true on Drop / signal â€” heartbeat thread observes and exits.
    stop: Arc<AtomicBool>,
}

#[derive(Debug)]
enum LockState {
    /// No lock file present â€” free to acquire.
    Free,
    /// Lock is held by `pid` on the same host and that PID is alive.
    HeldHere { pid: u32, op: String },
    /// Lock is held by another machine â€” never auto-clear.
    ...
    Stale,
    /// Lock file exists but is unparseable â€” treat as stale after mtime check.
    Unreadable,
}
...
if you're sure no `MEMORA db ...` is running, run `MEMORA db unlock --force`.",
                        path.display(),
                    );
                }
                let _ = fs::remove_file(&path);
            ...
            LockState::HeldHere { pid, op: other } => {
                bail!(
                    "another db op is running: {} (pid {}). wait for it, or `MEMORA db unlock --force` if you're sure it's stuck.",
                    other,
                    pid,
                );
            }
...
will not auto-clear cross-host locks. resolve manually if needed.",
        ...
        // Atomic exclusive create â€” fails if another process raced us between
        ...
        let mut file = OpenOptions::new()
            ...
            .with_context(|| {
                format!(
                    "couldn't acquire lock at {}. another `MEMORA db ...` may have raced us.",
                    path.display()
                )
            })?;
    ...
    fn inspect(path: &Path) -> LockState {
        ...
        // PID dead â€” might still be a fresh-but-orphaned lock. Heartbeat keeps
    ...
    fn install_signal_handlers(path: &Path, stop: Arc<AtomicBool>) {
        ...
        tokio::spawn(async move {
            ...
            let _ = fs::remove_file(&path);
            eprintln!("\nMEMORA: caught signal, lock released. exiting.");
...
// â”€â”€ runtime checks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn ensure_app_quit(force: bool) -> Result<()> {
    if !force && server_is_running() {
        bail!(
            "MEMORA is currently running (port 3030 is open). quit the app from the menubar first, or pass --force to override (only safe if you know nothing is writing to the db)."
        );
    }
    Ok(())
}
...
fn ensure_disk_space(data_dir: &Path, source_size: u64) -> Result<()> {
    // Need â‰¥ 2Ã— source: the .recovered sidecar plus the snapshot copy.
    let needed = source_size.saturating_mul(2);
    let available = available_disk_bytes(data_dir);
    if let Some(avail) = available {
        if avail < needed {
            bail!(
                "not enough disk space: need ~{} MB free, have {} MB. free space outside the active database recovery evidence; `MEMORA db cleanup` can reclaim old artifacts only when quarantine is not active.",
                needed / 1_048_576,
                avail / 1_048_576,
            );
        }
...
// â”€â”€ command entry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

fn integrity_check(db_path: &Path) -> Result<()> {
    if !db_path.exists() {
        bail!("no database at {}", db_path.display());
    }
    if memora_db::sqlite_quarantine_exists(db_path) {
        bail!(
            "database is durably quarantined; refusing to open or checkpoint its DB/WAL/SHM generation. quit MEMORA and run `MEMORA db recover`"
        );
    }
    println!("running PRAGMA quick_check on {} â€¦", db_path.display());
    ...
    if results.as_slice() == ["ok"] {
        ...
        bail!(
            "corruption detected:\n{}\n\nnext step: quit the MEMORA app, then run `MEMORA db recover`",
            results.join("\n")
        )
    }
}

// â”€â”€ recover â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
struct GenerationComponentFingerprint {
    name: String,
    identity: memora_db::SqliteFileIdentity,
    length: u64,
    ...
    for path in [
        ...
        fingerprint.push(GenerationComponentFingerprint {
            name: path
                ...
                .to_string_lossy()
                .into_owned(),
            identity: memora_db::sqlite_file_identity(&path)
...
fn restore_interrupted_swap(data_dir: &Path, live: &Path) -> Result<()> {
    for directory in newest_recovery_directories(data_dir)? {
        ...
        let live_identity = memora_db::sqlite_file_identity(live).ok();
...
pub async fn recover_quarantined_database(data_dir: &Path) -> Result<()> {
    ...
    if !memora_db::sqlite_quarantine_exists(&live) {
        bail!(
...
async fn recover_offline(data_dir: &Path) -> Result<()> {
    ...
    let original_identity = memora_db::sqlite_file_identity(&live)
    ...
    let marker = match memora_db::read_sqlite_quarantine(&live) {
        Ok(marker) => marker,
        Err(error) => {
            eprintln!(
    ...
    if !memora_db::sqlite_quarantine_exists(&live) {
        memora_db::persist_sqlite_quarantine(
            &live,
            None,
    ...
    memora_sqlite_recovery::recover_database(&work, &candidate)
    ...
    let rebuilt_fts = memora_db::rebuild_recovered_fts5_indexes(&candidate)
        .await
    ...
    let candidate_verification =
        memora_db::verify_fresh_sqlite_recovery_candidate(&candidate, &forbidden_identities)
            .await
            .context(
    ...
    {
        bail!(
            "recovered db is missing too many tables ({} â†’ {}); refusing to swap. artifacts kept at {}.",
            source_table_count,
            recovered_table_count,
    ...
    let installed_verification =
        memora_db::verify_fresh_sqlite_recovery_candidate(&live, &forbidden_identities).await;
    ...
    memora_db::archive_resolved_sqlite_quarantine(
        &live,
        &resolved_marker,
        &installed_verification.file_identity,
    ...
    println!();
    println!("    next: start MEMORA â€” it will rebuild WAL/SHM on first open.");
    println!("    once you've confirmed everything works, run:");
    println!("        MEMORA db cleanup --apply");
...
async fn cleanup(data_dir: &Path, apply: bool, force: bool) -> Result<()> {
    ...
    let live = data_dir.join("db.sqlite");
    if apply && memora_db::sqlite_quarantine_exists(&live) {
        bail!(
    ...
    if targets.is_empty() {
        println!("nothing to clean up â€” no stale recovery/backup artifacts found.");
        return Ok(());
    }
    ...
    if !apply {
        println!("\n(dry-run by default â€” pass `--apply` to actually delete)");
        return Ok(());
    }
...
// â”€â”€ unlock â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn unlock(data_dir: &Path, force: bool) -> Result<()> {
    ...
    if !safe && !force {
        bail!(
            "lock looks live ({state:?}). pass --force to remove anyway. you should only do this if you're certain no `MEMORA db ...` is actually running."
        );
    }
    fs::remove_file(&path).context("failed to delete lock file")?;
    println!("âœ“ lock file removed.");
    Ok(())
}
...
mod recovery_tests {
    ...
    fn test_manifest(live: &Path) -> RecoveryManifest {
        RecoveryManifest {
            ...
            original_identity: memora_db::sqlite_file_identity(live)
    ...
    async fn end_to_end_recovery_salvages_corruption_installs_fresh_inode_and_archives_original() {
        ...
        let original_identity = memora_db::sqlite_file_identity(&live).expect("old identity");
        memora_db::prepare_sqlite_quarantine_reserve(&live).expect("prepare marker reserve");
        memora_db::persist_sqlite_quarantine(&live, Some(11), "test corruption")
        ...
        let replacement_identity =
            memora_db::sqlite_file_identity(&live).expect("replacement identity");
        assert_ne!(replacement_identity, original_identity);
        assert!(!memora_db::sqlite_quarantine_exists(&live));
    ...
    async fn cleanup_cannot_delete_evidence_while_quarantine_is_active() {
        ...
        memora_db::prepare_sqlite_quarantine_reserve(&live).expect("prepare reserve");
        memora_db::persist_sqlite_quarantine(&live, Some(522), "test I/O fault")
            .expect("persist quarantine");
        let check_error =
