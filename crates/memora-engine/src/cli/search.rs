...
//! `MEMORA search` â€” query the local SQLite history without the daemon.
//!
//! Mirrors `GET /search` so AI / shell / pipe callers don't have to choose
//! between two vocabularies. Opens `~/.memora/db.sqlite` directly; WAL
...
use crate::routes::search::{is_MEMORA_app, search_result_to_content_item};
...
pub async fn handle_search_command(args: &SearchArgs) -> anyhow::Result<()> {
    let base_dir = match &args.data_dir {
        ...
        None => paths::default_MEMORA_data_dir(),
    };
    let db_path = base_dir.join("db.sqlite");
    if !db_path.exists() {
        anyhow::bail!(
            "no MEMORA database at {}. run `MEMORA record` first.",
            db_path.display()
        );
    }
    ...
    let results: Vec<SearchResult> = db
        .search(
            ...
            args.min_length,
            args.max_length,
            None, // speaker_ids â€” exposed via --speaker (name) only
...
fn parse_content_type(s: &str) -> Result<ContentType, String> {
    match s.to_lowercase().as_str() {
        ...
        other => Err(format!(
            "unknown content type '{}' â€” expected one of: all, ocr, audio, accessibility, input, memory",
            other
        )),
    }
}
...
mod tests {
    ...
    fn empty_query_is_allowed_for_time_only_browse() {
        let cli = Cli::try_parse_from(["MEMORA", "search", "--start", "30m ago"]).unwrap();
        match cli.command {
            Command::Search(args) => {
