...
//! `MEMORA service {install,uninstall,status}` â€” run MEMORA as a
//! background service that starts at boot and stays up (Restart=always). This
//! is what makes a headless box (a VPS, a Mac mini) keep MEMORA running
...
pub enum ServiceCommand {
    /// Install + enable a service that runs MEMORA at boot, always-on.
    Install {
        /// Args appended to `MEMORA record`. Default is serve-only (no
        /// capture) â€” right for a headless box that just serves synced/cloud
...
const SERVICE_NAME: &str = "MEMORA";
const LAUNCHD_LABEL: &str = "com.memora.agent";

/// Build the systemd user-unit contents (pure â€” unit-tested).
#[allow(dead_code)] // used by the linux install() + tests
fn systemd_unit(exec_start: &str, path_env: &str) -> String {
    format!(
        "[Unit]\n\
         Description=MEMORA â€” always-on capture/serve\n\
...
/// Build the launchd plist contents (pure â€” unit-tested).
...
fn MEMORA_exe() -> Result<String> {
    Ok(std::env::current_exe()
        .context("could not resolve the MEMORA binary path")?
        .to_string_lossy()
        .into_owned())
}
...
fn install(record_args: &str) -> Result<()> {
    ...
    println!("  âœ“ wrote {}", unit_path.display());
    ...
    println!("  âœ“ MEMORA service enabled + started â€” runs at boot, restarts on crash");
...
fn install(record_args: &str) -> Result<()> {
    ...
    println!("  âœ“ wrote {}", plist_path.display());

    let p = plist_path.to_string_lossy().into_owned();
    run_quiet("launchctl", &["unload", &p]);
    run("launchctl", &["load", &p])?;
    println!("  âœ“ MEMORA launch agent loaded â€” runs at login, restarts on crash");
    Ok(())
}
...
fn uninstall() -> Result<()> {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        println!("  âœ“ removed {}", unit_path.display());
    }
    let _ = run("systemctl", &["--user", "daemon-reload"]);
    println!("  âœ“ MEMORA service removed");
    Ok(())
}
...
fn uninstall() -> Result<()> {
    if let Some(home) = dirs::home_dir() {
        ...
        println!("  âœ“ removed {}", plist_path.display());
    }
    println!("  âœ“ MEMORA launch agent removed");
    Ok(())
}
...
mod tests {
    ...
    fn test_systemd_unit() {
        let u = systemd_unit(
            "/usr/local/bin/MEMORA record --disable-vision",
            "/usr/bin",
        );
        assert!(u.contains("ExecStart=/usr/local/bin/MEMORA record --disable-vision"));
    ...
    fn test_launchd_plist() {
        let p = launchd_plist(&[
            "/Applications/x/MEMORA".to_string(),
            "record".to_string(),
            "--disable-audio".to_string(),
        ]);
        assert!(p.contains("<key>Label</key>"));
        assert!(p.contains("com.memora.agent"));
