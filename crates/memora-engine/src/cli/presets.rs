...
//!
//! Reads/writes presets in `~/.memora/store.bin` (plain JSON or the
...
//! at the file level â€” same trade-off as `MEMORA login`.
...
/// stored on disk and consumed by `resolve_preset` in `memora-core`.
...
pub enum Provider {
    ...
    Custom,
    MEMORACloud,
    /// CLI refuses to write this â€” token lives in the encrypted secrets store
...
impl Provider {
    pub fn parse(s: &str) -> Result<Self> {
        match s {
            ...
            "MEMORA-cloud" | "pi" => Ok(Provider::MEMORACloud),
            "openai-chatgpt" => Ok(Provider::OpenAiChatGpt),
            other => bail!(
                "unknown provider '{}'. Valid: openai, anthropic, native-ollama, custom, MEMORA-cloud",
                other
            ),
        }
    }
    ...
    pub fn as_canonical(self) -> &'static str {
        match self {
            ...
            Provider::MEMORACloud => "MEMORA-cloud",
    ...
    fn forbids_url(self) -> bool {
        matches!(self, Provider::MEMORACloud)
    }

    fn forbids_api_key(self) -> bool {
        matches!(
            self,
            Provider::NativeOllama | Provider::MEMORACloud | Provider::OpenAiChatGpt
        )
    }
}
...
/// Validate the preset id alone â€” used for create, update, set-default, delete.
...
fn validate_provider_combo(
    ...
    if provider == Provider::OpenAiChatGpt {
        bail!(
            "provider 'openai-chatgpt' uses OAuth and cannot be configured via CLI â€” \
    ...
    if provider.forbids_api_key() && api_key.map(|s| !s.is_empty()).unwrap_or(false) {
        let reason = match provider {
            ...
            Provider::MEMORACloud => "cloud uses your login token (run `MEMORA login`)",
            _ => "this provider does not accept --api-key",
        };
        bail!(
            "provider '{}' does not accept --api-key â€” {}",
            provider.as_canonical(),
            reason
        );
    }
...
pub fn create(input: PresetInput) -> Result<()> {
    ...
    {
        bail!(
            "preset '{}' already exists â€” use `pipe models update {}` to modify",
            input.id,
            input.id
        );
    }
...
/// patch are left untouched â€” including unknown keys the UI may have set.
...
/// Result of a `delete` call â€” lists the pipes that referenced this preset so
...
/// Pipes that referenced it are NOT auto-rewritten â€” caller surfaces the list.
...
/// Scan `~/.memora/pipes/*/pipe.md` and return the names of pipes whose
/// frontmatter `preset:` contains the given id. Best-effort â€” failure to read
/// or parse a pipe is logged and skipped.
fn pipes_referencing_preset(id: &str) -> Result<Vec<String>> {
    let pipes_dir = memora_core::paths::default_MEMORA_data_dir().join("pipes");
    if !pipes_dir.exists() {
        return Ok(vec![]);
    }
...
/// Conservative â€” false positives are tolerable (over-warn), false negatives
...
mod tests {
    ...
    fn provider_combos() {
        ...
        assert!(validate_provider_combo(Provider::MEMORACloud, None, None).is_ok());
        assert!(
            validate_provider_combo(Provider::MEMORACloud, Some("http://x"), None).is_err()
        );
        assert!(validate_provider_combo(Provider::MEMORACloud, None, Some("k")).is_err());
