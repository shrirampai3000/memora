...
pub async fn handle_status_command(
    ...
    // db.sqlite lives at the top of the MEMORA dir; the `data/` subdir
    ...
    let (frame_count, audio_count, last_timestamp) = if db_path.exists() {
        match memora_db::DatabaseManager::new(&db_path.to_string_lossy(), Default::default())
            .await
        {
            Ok(db) => {
    ...
    if json {
        ...
        let status_icon = if running { "â—" } else { "â—‹" };
        println!("MEMORA: {} {} (port {})", status_icon, status, port);
