...
//! `MEMORA debug` â€” one-shot diagnostic subcommands used to reproduce bugs
...
//! once via System Settings â†’ Privacy & Security â†’ Accessibility, then
...
async fn a11y_walk(app_filter: Option<&str>, settle_ms: u64) -> anyhow::Result<()> {
    #[cfg(not(target_os = "macos"))]
    {
        ...
        anyhow::bail!("debug a11y-walk is macOS-only â€” the probe targets AXUIElement APIs");
    }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        //    binary does not have Accessibility permission â€” surface an
        //    actionable message and bail out before doing more work.
        let (pid, app_name) = match focused_app_via_ax() {
            Ok(x) => x,
            Err(e) => {
                println!("\nâŒ AX sys-wide query failed: {e}");
                println!("   Most likely cause: the binary at this path was not granted");
                println!("   Accessibility permission. Drag the binary into System Settings â†’");
                println!("   Privacy & Security â†’ Accessibility and toggle it on.");
        ...
        // No focus-stealing here â€” the user is expected to bring the target
        ...
        if let Some(filter) = app_filter {
            if !app_name.eq_ignore_ascii_case(filter) {
                println!("\nâš   focused app is {app_name:?}, not {filter:?} â€” bring {filter} to the front and rerun.");
                return Ok(());
            }
        }

        // 2. Walk the tree 3Ã—: immediately after a brief settle, then at
        ...
        for (i, (elapsed, result)) in results.iter().enumerate() {
            ...
            match result {
                TreeWalkResult::Found(snap) => {
                    ...
                    for node in &snap.nodes {
                        ...
                        if let Some(b) = &node.bounds {
                            ...
                            if node.on_screen == Some(true)
                                && memora_a11y::tree::node_looks_multiline(&node.text, b, 1.5)
                            {
                ...
                TreeWalkResult::NotFound => {
                    println!(
                        "{label:<25}  NotFound â€” focused_window AX query failed (app has no focused window or denied)"
                    );
                }
                TreeWalkResult::Skipped(reason) => {
                    println!("{label:<25}  Skipped â€” {reason:?}");
                }
            }
        }
        ...
        if lens.iter().all(|&l| l == 0) {
            println!("all three walks empty â†’ async-settle is NOT the cause.");
            ...
            println!(
                "text grew across retries ({} â†’ {} â†’ {}) â†’ async-settle IS helping.",
            ...
            println!(
                "text populated on the first walk ({}) â†’ AX works fine for this app.",
                lens[0]
            );
        }
