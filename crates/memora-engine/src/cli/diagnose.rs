...
//! `MEMORA diagnose` â€” collect recent logs + system info and ship them to
//! MEMORA support, returning a ticket id. This is the headless/CLI twin of
//! the desktop app's "send logs & feedback" button
//! (apps/memora-app-tauri/components/share-logs-button.tsx). It hits the
//! exact same public contract:
//!
//!   1. `POST /api/logs`         â†’ signed upload url + storage path
//!   2. `PUT  <signedUrl>`       â†’ the combined diagnostics bundle (text/plain)
//!   3. `POST /api/logs/confirm` â†’ persists the ticket, returns its id
//!
...
/// Per-file cap â€” we keep the *last* N bytes since failures are at the tail.
const MAX_LOG_BYTES: usize = 100 * 1024;

/// Handle `MEMORA diagnose`.
///
/// * `data_dir`  â€” resolved MEMORA data dir (`~/.memora` by default).
/// * `message`   â€” optional free-text describing the problem (the "feedback").
/// * `port`      â€” local API port to probe for a live `/health` snapshot.
/// * `dry_run`   â€” when true, write the bundle to a temp file and upload nothing.
pub async fn handle_diagnose_command(
    ...
    if log_files.is_empty() {
        anyhow::bail!(
            "no .log files found in {} â€” has MEMORA recorded anything yet?\n\
             hint: run `MEMORA record` (or start the app) at least once first.",
            data_dir.display()
        );
    }
    ...
    if dry_run {
        ...
        println!("  drop it in the MEMORA discord. drop --dry-run to send it for you.");
        return Ok(());
    }

    let (identifier, kind) = resolve_identifier(data_dir);
    let MEMORA_id = read_pointer(
        data_dir,
    ...
    // 3. confirm â€” this is what actually files the support ticket.
    ...
    if let Some(id) = MEMORA_id.filter(|s| !s.is_empty()) {
        confirm_body["MEMORA_id"] = json!(id);
    }
    ...
    println!();
    println!("  âœ“ logs sent to MEMORA support");
    if let Some(id) = ticket {
        println!("  ticket: #{id}");
        match follow_up {
            "email" => println!("  we emailed you a receipt and will reply there."),
            _ => println!("  mention #{id} in the MEMORA discord if you need an update."),
        }
    } else {
...
async fn build_bundle(
    ...
    out.push_str("=== MEMORA diagnostics ===\n");
    out.push_str(&format!(
        "generated: {}\n",
...
async fn probe_health(port: u16) -> String {
    ...
    match client.get(&url).send().await {
        ...
        Err(_) => format!("[server not reachable on :{port} â€” MEMORA may not be running]\n"),
    }
}
...
fn list_pipes(data_dir: &Path) -> String {
    ...
    let mut names: Vec<String> = match std::fs::read_dir(&pipes_dir) {
        Ok(rd) => rd
            ...
            // skip internal infra dirs (.pi, _shared, etc.) â€” not user pipes
            .filter(|e| {
                let n = e.file_name();
