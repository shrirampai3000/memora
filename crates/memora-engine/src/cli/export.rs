...
//! `MEMORA export` â€” render a recording to a single MP4 (screen frames +
//! synced audio) by reading `~/.memora/db.sqlite` directly. No daemon
...
pub async fn handle_export_command(args: &ExportArgs) -> anyhow::Result<()> {
    ...
    let summary = match mode {
        Mode::Meeting(id) => {
            ...
            println!("exporting meeting {id} â†’ {}", output_path.display());
...
/// Best-effort "open with default app". Failure is non-fatal â€” the path is already printed.
