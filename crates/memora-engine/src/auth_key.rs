...
//!
//! Every component that needs the key â€” the running server, the Tauri app,
//! the `MEMORA` CLI, `MEMORA auth token`, the MCP bridge â€” must go
...
pub async fn resolve_api_auth_key(data_dir: &Path, settings_key: Option<&str>) -> Result<String> {
    ...
    // Read the existing secret-store value once â€” used both as a fallback
    // source and to avoid a no-op write when nothing has changed.
    //
    // CRITICAL: distinguish "no row" from "row exists but unreadable". If
    // `get()` errors (decrypt failure, IO error), we MUST log loudly â€” the
    ...
    // for `github.com/shrirampai3000/memora`. Result: rotation, mismatched in-memory caches,
    // 401 storms â€” observed for chris@lovephoenixhomes.com 2026-05-06.
    ...
    let stored_key: Option<String> = if let Some(ref s) = store {
        match s.get("api_auth_key").await {
            ...
            Err(e) => {
                ...
                tracing::error!(
                    "api auth: failed to read api_auth_key from secret store â€” \
                     ...
                     (devâ†”prod bundle id, recent encryption toggle, or revoked \
                     keychain item). Error: {}",
                    e
                );
                None
    ...
    let (key, source) = if let Ok(k) = std::env::var("MEMORA_API_KEY") {
        if !k.is_empty() {
            (k, "MEMORA_API_KEY env")
        } else {
    ...
    // reader (running server, MCP, `MEMORA auth token` CLI) agrees on
    ...
    // rotating the user's API key (MEMORA-APP-9Z: 25 events / 18 users,
...
/// The running server will keep using its in-memory key until restart â€” caller
...
async fn open_secret_store(data_dir: &Path) -> Result<memora_secrets::SecretStore> {
    ...
    memora_secrets::SecretStore::open_for_data_dir_with_vault_key(data_dir).await
}

/// Path to the pre-SecretStore `~/.memora/auth.json`, but **only** when this
/// process is actually running against `~/.memora`.
///
/// The legacy file belongs to whichever instance owns the legacy data dir. An
/// instance pointed elsewhere by `MEMORA_DATA_DIR` (a dev build, a relocated
/// profile, the E2E harness) must neither read that key â€” it would authenticate
/// against a foreign instance's credential â€” nor delete the file, which is what
/// the `set`/`regenerate` paths below do as legacy cleanup.
fn legacy_auth_json_path() -> Option<PathBuf> {
    let legacy_dir = dirs::home_dir()?.join(".memora");
...
/// be tested without mutating process-global `MEMORA_DATA_DIR`.
fn legacy_auth_json_path_for(legacy_dir: &Path, active_dir: &Path) -> Option<PathBuf> {
    // Compare canonically where possible so `~/.memora` and a symlinked or
...
/// (env â†’ encrypted SecretStore â†’ legacy file) but does NOT auto-generate
/// or persist anything when no key is found â€” returns `None` instead.
///
...
pub async fn find_api_auth_key() -> Option<String> {
    if let Ok(k) = std::env::var("MEMORA_API_KEY") {
        if !k.is_empty() {
            return Some(k);
        }
    }
    // Tauri sidecar processes (pi-agent shelling into bash) inherit the
    // app's env under different names. Honor those too â€” without this the
    // agent's `connection list` couldn't authenticate even though the key
    // was right there.
    for var in ["MEMORA_LOCAL_API_KEY", "MEMORA_API_AUTH_KEY"] {
...
// ---------------------------------------------------------------------------
// Cloud auth token (Clerk JWT â€” the Bearer for api.memora.com).
//
...
// `db.sqlite`) must read it there too â€” otherwise a standalone `MEMORA`
// run would lose cloud auth (STT, MEMORA-cloud pipes) once the app has
// migrated the token out of store.bin.
// ---------------------------------------------------------------------------

/// SecretStore key for the cloud auth token. MUST match the desktop app's
/// `auth_token` module (`apps/memora-app-tauri/src-tauri/src/auth_token.rs`).
const CLOUD_AUTH_TOKEN_KEY: &str = "cloud.auth_token";

/// Clerk cloud-session tokens are JWTs. The local HTTP bearer returned by
/// `MEMORA auth token` is deliberately not accepted here: both credentials
/// historically used `MEMORA_API_KEY`, and treating the local `sp-*` key as
...
pub(crate) fn is_cloud_session_token(value: &str) -> bool {
    (value.starts_with("eyJ") && value.matches('.').count() == 2)
        || (std::env::var_os("MEMORA_E2E_SEED").is_some()
...
/// app. This is the durable write path for `MEMORA login`; writing the JWT
...
pub async fn set_cloud_token(data_dir: &Path, token: &str) -> Result<()> {
    anyhow::ensure!(
        ...
        "MEMORA cloud login returned an invalid session token"
    );

    let key = if memora_secrets::is_encryption_requested(data_dir) {
        match memora_secrets::keychain::get_key() {
            memora_secrets::keychain::KeyResult::Found(key) => Some(key),
            memora_secrets::keychain::KeyResult::AccessDenied => {
                anyhow::bail!(
                    "keychain access denied; refusing to persist the cloud session unencrypted"
                )
            }
            memora_secrets::keychain::KeyResult::NotFound
            | memora_secrets::keychain::KeyResult::Unavailable => None,
        }
    } else {
        None
    };
    ...
    std::fs::create_dir_all(data_dir).map_err(|error| {
        anyhow::anyhow!(
            "could not create MEMORA data directory {}: {error}",
            data_dir.display()
        )
    })?;
    let store = memora_secrets::SecretStore::open_for_data_dir(data_dir, key)
        .await
...
pub async fn clear_cloud_token(data_dir: &Path) -> Result<()> {
    if !data_dir.join("db.sqlite").exists()
        && !memora_secrets::secrets_database_path(data_dir).exists()
    {
        return Ok(());
    }
    let store = open_secret_store(data_dir)
...
///   1. The shared encrypted SecretStore key `cloud.auth_token` â€” written by
///      the desktop app (`auth_token.rs`).
///   2. Legacy fallback: `store.bin` `settings.user.token` â€” where older
///      versions of `MEMORA login` persisted the token. Current CLI and
...
/// older `MEMORA login` versions wrote). Skips app-managed encrypted
/// stores (SPSTORE1 magic) â€” those resolve via the SecretStore path instead.
...
mod cloud_token_tests {
    ...
    async fn resolve_cloud_token_reads_desktop_secret_store() {
        ...
        let store = memora_secrets::SecretStore::open(&db_path.to_string_lossy(), None)
            .await
            .unwrap();
        store
    ...
    fn test_cloud_token_from_store_json() {
        ...
        // CLI-login style plaintext store.bin â†’ token resolves.
        std::fs::write(
            dir.join("store.bin"),
    ...
    fn legacy_auth_json_is_scoped_to_the_instance_that_owns_it() {
        ...
        let legacy = root.join(".memora");
        let other = root.join(".memora-dev");
