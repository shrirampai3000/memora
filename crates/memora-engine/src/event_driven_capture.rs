...
use anyhow::Result;
use chrono::Utc;
use memora_a11y::tree::TreeWalkerConfig;
use memora_a11y::ActivityFeed;
use memora_capture::ocr_gate::OcrGate;
use memora_capture::paired_capture::{paired_capture, CaptureContext, PairedCaptureResult};
use memora_core::window_pattern::{self, WindowPattern};
use memora_db::DatabaseManager;
use memora_screen::capture_screenshot_by_window::WindowFilters;
use memora_screen::frame_comparison::{FrameComparer, FrameComparisonConfig};
use memora_screen::monitor::{list_monitors, SafeMonitor};
use memora_screen::snapshot_writer::SnapshotWriter;
use memora_screen::utils::capture_monitor_image;
...
fn e2e_force_focus_cold(monitor_id: u32) -> bool {
    let enabled = std::env::var("MEMORA_E2E_SEED")
        .ok()
            ...
            .is_ok()
    {
        ...
        if let Ok(dir) = std::env::var("MEMORA_DATA_DIR") {
            let _ = std::fs::write(
...
async fn e2e_park_capture_loop_once(
    vision_metrics: &memora_screen::PipelineMetrics,
    monitor_id: u32,
) {
...
pub(crate) struct CaptureParams<'a> {
    ...
    pub languages: &'a [memora_core::Language],
...
pub enum CaptureTrigger {
    ...
    VisualChange,
    /// No activity for a while â€” periodic fallback capture
    Idle,
...
/// pending entries will TTL-evict after 60s. Returns immediately â€”
    ...
    loop {
        match trigger_rx.try_recv() {
            ...
            Err(broadcast::error::TryRecvError::Lagged(_)) => {
                // Lagged inside drain â€” keep trying; the receiver auto-
...
pub struct EventDrivenCaptureConfig {
    ...
    /// Frame difference threshold (0.0â€“1.0) above which a VisualChange trigger fires.
    ...
    /// survive PowerProfile transitions â€” a battery-saver switch (whose
...
/// writes through an unchanged AX-text hash. ~12 frames/min â€” bounded, and only
/// during meetings.
...
/// turned continuous typing into a back-to-back capture storm â€” each
/// keystroke chaining a full screenshot + tree walk + OCR-if-thin + DB write
/// (#4844). A fixed const rather than a config field â€” no plumbing needed,
...
impl EventDrivenCapture {
    ...
    /// must keep that floor regardless of power state â€” otherwise plugging
    ...
    /// `min_capture_interval_ms` â€” that value can be *longer* than
    ...
    /// timing, but that path was untraceable â€” the resulting frame
...
/// permanently â€” the `ui_events` rows then stay `frame_id = NULL`.
...
impl HighFpsBookkeeping {
    ...
    pub(crate) fn on_controller_state(
        ...
        match (self.active_ms, controller_ms) {
            (None, None) => None,
            (None, Some(new_ms)) => {
                // false â†’ true: stash the current live value as the
                // restore target (defense-in-depth â€” even if the baseline
            ...
            (Some(_), None) => {
                // true â†’ false: restore the tracked baseline.
                self.active_ms = None;
            ...
            (Some(prev_ms), Some(new_ms)) => {
                if prev_ms == new_ms {
                    ...
                    // underneath us â€” install the new value, keep baseline.
                    self.active_ms = Some(new_ms);
                    Some(new_ms)
                }
            }
...
///
/// Edge-triggered: fires exactly once on the non-paused â†’ paused transition.
...
/// the capture path is about to use. Regression for perf(macos) e47f53fc4 â€”
...
fn record_capture_attempt(
    aggregate: &memora_screen::PipelineMetrics,
    monitor: &memora_screen::PipelineMetrics,
) {
...
/// [`memora_screen::CaptureLoopStage`]). Per-monitor is what the watchdog
/// reads: the aggregate is overwritten by whichever display iterated last.
fn record_loop_stage(
    aggregate: &memora_screen::PipelineMetrics,
    monitor: &memora_screen::PipelineMetrics,
    stage: memora_screen::CaptureLoopStage,
) {
...
fn record_screenshot_capture_state(
    aggregate: &memora_screen::PipelineMetrics,
    monitor: &memora_screen::PipelineMetrics,
    state: memora_screen::ScreenshotCaptureState,
) {
...
pub(crate) async fn event_driven_capture_loop(
    ...
    vision_metrics: Arc<memora_screen::PipelineMetrics>,
    monitor_liveness: Arc<memora_screen::PipelineMetrics>,
    hot_frame_cache: Option<Arc<HotFrameCache>>,
    use_pii_removal: bool,
    pause_on_drm_content: bool,
    languages: Vec<memora_core::Language>,
    ...
    record_screenshot_capture_state(
        &vision_metrics,
        &monitor_liveness,
        memora_screen::ScreenshotCaptureState::resolve(screenshots_disabled_by_config, false),
    );
    ...
    let mut walk_budget = memora_a11y::budget::AppWalkBudget::new();
    ...
    // merged into whatever capture happens next â€” see the trigger-handling
    ...
    // events to a frame even when content-dedup skips the capture â€” the
    // screen looks the same, so reusing the last frame is semantically correct.
    let mut last_frame_id: Option<i64> = None;
    // Track last successful DB write time â€” dedup is bypassed after 30s
    // to guarantee the timeline always has periodic entries
    let mut last_db_write = Instant::now();
    // Per-device elements dedup cache: device_name â†’ (last_frame_id, last_content_hash)
    ...
    // Debounce consecutive capture errors â€” log error! once on first failure,
    ...
    // the HD interval even when the accessibility-tree hash is unchanged â€”
    ...
    // Skip if screen is locked â€” avoids storing black frames from sleep/lock.
        ...
        && !crate::schedule_monitor::schedule_paused()
    {
        ...
        match capture_with_timeout(
            ...
            do_capture(
                ...
                None, // first capture â€” no previous hash
                last_db_write,
                None, // first capture â€” no elements ref
                &mut walk_budget,
                &mut ocr_gate,
                screenshot_disabled,
                false, // hd not active at startup (Manual is dedup-exempt anyway)
                false, // not in a meeting at startup
                true,  // focus unknown at startup â€” controller defaults to Active
            ),
        )
        .await
        {
            Ok(Ok(output)) => {
                ...
                if let Some(ref result) = output.result {
                    ...
                    // Frame rejected as corrupt (black / green band) â€” distinct
                    ...
                    // Symmetry with the live loop â€” startup capture rarely
    ...
    // its OS-level capture session exactly once per Active/Warm â†’ Cold edge.
    ...
    // at 2fps on macOS, WGC on Windows) â€” measurable share of a core per
    ...
    // reader for the entire pause window â€” defeating the whole point of
    ...
    loop {
        ...
        record_loop_stage(
            &vision_metrics,
            &monitor_liveness,
            memora_screen::CaptureLoopStage::Heartbeat,
        );

        #[cfg(debug_assertions)]
        e2e_park_capture_loop_once(&vision_metrics, monitor_id).await;

        // Focus-aware gating â€” always on. Skips or pauses capture on
        ...
        // iteration) or setting `warm_trigger_override` â€” which falls through
        ...
        record_loop_stage(
            &vision_metrics,
            &monitor_liveness,
            memora_screen::CaptureLoopStage::FocusGate,
        );
        let mut warm_trigger_override: Option<CaptureTrigger> = None;
        {
            ...
            if is_cold && !was_cold {
                record_loop_stage(
                    &vision_metrics,
                    &monitor_liveness,
                    memora_screen::CaptureLoopStage::ReleaseStream,
                );
            ...
            match capture_state {
                ...
                CaptureState::Warm => {
                    ...
                    // a11y tree walk) â€” Warm does a screenshot + 15Ã—15 sample
                    ...
                    // we can't cheaply detect change â€” idle.
                    ...
                    // is still correct â€” it just might include pixels from
                    ...
                    match snap {
                        Ok(None) => {
                            // Probe timed out â€” treat like a failed check and
                ...
                CaptureState::Cold => {
                    // Drain any triggers that arrived while we were Cold â€”
                    ...
                    record_loop_stage(
                        &vision_metrics,
                        &monitor_liveness,
                        memora_screen::CaptureLoopStage::ColdWait,
                    );
                    let notify = focus_controller.notify_for(monitor_id);
                    tokio::select! {
                        _ = notify.notified() => {}
        ...
        // sleeping reader for the entire pause window â€” the exact cost the
        // user expected `capture_paused` to eliminate.
        record_loop_stage(
            &vision_metrics,
            &monitor_liveness,
            memora_screen::CaptureLoopStage::PauseGate,
        );
        ...
        if let Some(ref mut rx) = power_profile_rx {
            if rx.has_changed().unwrap_or(false) {
                ...
                // never raise it â€” picking "max" in settings shouldn't be silently
                ...
                record_screenshot_capture_state(
                    &vision_metrics,
                    &monitor_liveness,
                    memora_screen::ScreenshotCaptureState::resolve(
                ...
                if screenshot_disabled {
                    info!(
                        "screenshots disabled for monitor {} (power profile {:?}, managed/config={}) â€” a11y walk continues",
        ...
        // directly to VisualChange â€” the regular trigger sources (external
        ...
        if let Some(warm) = warm_trigger_override.take() {
            ...
            record_loop_stage(
                &vision_metrics,
                &monitor_liveness,
                memora_screen::CaptureLoopStage::TriggerWait,
            );
            match tokio::time::timeout(poll_interval, trigger_rx.recv()).await {
                ...
                Ok(Err(broadcast::error::RecvError::Lagged(n))) => {
                    ...
                    // Missed broadcast msgs â€” their correlation_ids are
                ...
                Err(_elapsed) => {
                    // No trigger this poll_interval â€” fall through to
                    // poll_activity below.
                }
            }
            ...
            record_loop_stage(
                &vision_metrics,
                &monitor_liveness,
                memora_screen::CaptureLoopStage::TriggerDrain,
            );
        ...
        // `trigger` is still `None` â€” a promoted checkpoint should suppress
        ...
        // Re-check DRM pause before touching SCK â€” the flag may have been set
        // between the top-of-loop check and here.
        // Use the same window exclusions as the full capture so the diff image
        // matches what we'd actually store â€” avoids triggering on excluded
        ...
        ) || force_visual_check_for_e2e
        {
            ...
            if let Some(ref mut comparer) = frame_comparer {
                record_loop_stage(
                    &vision_metrics,
                    &monitor_liveness,
                    memora_screen::CaptureLoopStage::VisualProbe,
                );
                match bounded_visual_probe(&monitor, &cached_excluded_ids, monitor_id).await {
                    ...
                    Ok(None) => {
                        // Probe timed out â€” skip this check; the idle fallback
        ...
        if let Some(trigger) = trigger {
            ...
            // Clipboard fire from a raw, unspaced event stream â€” continuous
            ...
            // bypasses the floor entirely â€” HD wants dense capture.
            ...
            if can_capture || debounce_exempt {
                ...
                // Uses AX APIs only â€” prevents even a single leaked frame.
                {
                ...
                record_loop_stage(
                    &vision_metrics,
                    &monitor_liveness,
                    memora_screen::CaptureLoopStage::Capture,
                );
                ...
                match capture_result {
                    Ok(Ok(output)) => {
                        ...
                        // image â€” no extra screenshot needed)
                        if let Some(ref mut comparer) = frame_comparer {
                            let _ = comparer.compare(&output.image);
                        }
                        ...
                        // buffer. Deterministic â€” idle content, look-alike
                        ...
                        {
                            warn!(
                                "monitor {}: capture stream frozen â€” no new frame delivered for \
                        ...
                        if let Some(ref result) = output.result {
                            // Full capture â€” update hash, metrics, cache
                            last_content_hash = result.content_hash;
                            last_frame_id = Some(result.frame_id);
                            last_db_write = Instant::now();
                            ...
                            // (skip / crop_ocr â€” the fast-path ratio) plus the
                            ...
                            if let Some(ref cache) = hot_frame_cache {
                                record_loop_stage(
                                    &vision_metrics,
                                    &monitor_liveness,
                                    memora_screen::CaptureLoopStage::HotCachePush,
                                );
                            ...
                            // correlation ids attached â€” internal-only
                            ...
                            // Capture skipped â€” content dedup, window filter, or
                            ...
                            // so link them to that frame â€” semantically correct
                            ...
                            if !correlation_ids.is_empty() {
                                if let (Some(ref linker), Some(fid)) = (&linker_tx, last_frame_id) {
                                    ...
                                    // No frame ever captured yet â€” just drop the ids.
                    ...
                    Ok(Err(e)) => {
                        ...
                        // Mark captured on failure to reset idle timer â€” without
                        ...
                        // Release corr_ids the linker is waiting on â€”
                        ...
                        // Exponential backoff for persistent failures â€” avoids
                    ...
                    Err(_timeout) => {
                        ...
                        // JPEG may already be on disk (orphaned, no DB row) â€”
                        ...
                        warn!(
                            "event capture timed out after {:?} (trigger={}, monitor={}) â€” frame dropped; likely a stuck DB write / saturated write pool or a slow a11y/OCR step",
                            CAPTURE_OPERATION_TIMEOUT,
                            trigger.as_str(),
                            monitor_id
                        );
...
struct CaptureOutput {
    ...
    /// The captured image â€” reused for frame comparer update to avoid taking
...
fn resolve_capture_metadata(
    tree_snapshot: Option<&memora_a11y::tree::TreeSnapshot>,
    trigger: &CaptureTrigger,
...
fn resolved_window_matches_privacy_filters(
    ...
    if resolved_window_hosts_focus
        ...
        && window_name.is_some_and(memora_a11y::incognito::is_title_private)
    {
        return true;
    }
...
///
/// **Terminals** (wezterm/alacritty/â€¦): bypass accessibility entirely and
...
/// CPU ~30Ã— â€” still captures ~2 frames/min of what the user is writing.
/// See issue #3002.
///
...
fn terminal_ocr_throttled(app_name: &str) -> bool {
    ...
    // `is_ocr_heavy_app` allow-list â€” but it's a process-lifetime static, so
...
fn is_lock_screen_app(app_name: &str) -> bool {
    app_name.eq_ignore_ascii_case("loginwindow")
        ...
        // credential prompt. Both must be recognized here â€” an unrecognized
...
/// after a rebuild â€” or a rare spurious detection â€” can't thrash the capture
...
///
/// A live persistent stream advances the sequence continuously â€” even on a
...
/// none of them can cause a false positive â€” the failure mode the earlier
...
///
/// `None` (no persistent stream cached â€” e.g. just after a release, or on a
...
impl StreamLivenessWatch {
    ...
    fn observe(&mut self, seq: Option<u64>, now: Instant) -> bool {
        let Some(seq) = seq else {
            ...
            // unsupported platform) â€” nothing to judge.
            self.last_seq = None;
            self.last_advance = now;
            return false;
        };
        // `seq != prev` covers both a normal advance and a post-rebuild reset
        // to 0 (seq < prev) â€” both mean "the stream is delivering", so we treat
...
///   moves pixels while leaving the AX-tree text unchanged â€” exactly what the
...
///   must stay dedup-exempt â€” the meeting idle floor
...
///   same text twice, â€¦) â€” exempting them just chained redundant
...
///   though the AX-tree text is unchanged â€” exactly the content the meeting
...
/// stream. Continuous typing/editing fires these with no natural spacing â€”
...
enum CheckpointGate {
    ...
    /// â€” let the trigger through to a normal capture attempt.
    Proceed,
    /// The floor hasn't elapsed â€” stash it via `defer_checkpoint` instead of
...
/// Stash a deferred soft checkpoint. The latest trigger kind wins â€” it's the
/// best proxy for the state a capture right now would actually show â€” while
...
/// â€” when a drain batch mixes a checkpoint (of either kind) with trailing
...
/// `CaptureOutput.result` will be `None` in that case â€” the caller should still
...
/// bypasses it only for visual-change triggers â€” see [`dedup_applies`].
#[allow(clippy::too_many_arguments)]
async fn do_capture(
    ...
    walk_budget: &mut memora_a11y::budget::AppWalkBudget,
    ...
    //   1. Near-all-black â€” an ignored window covering the monitor; SCK fills
    //      its pixels black.
    //   2. A flat green "garbage" band anchored to the bottom of the frame â€”
    ...
    if !screenshot_disabled && !skip_pixels_for_unknown_exclusions {
        if let Some(kind) = frame_corruption(&image) {
            match kind {
                // Green is the notable, rarer signal â€” surface it at warn so it
                // shows up in shared logs. Black is common (DRM / excluded window).
                CorruptKind::GreenBand => warn!(
                "captured frame has a green decode-garbage band on monitor {} â€” skipping DB write",
                params.monitor_id
            ),
                CorruptKind::Black => debug!(
                    "captured frame is mostly black on monitor {} â€” skipping DB write",
                    params.monitor_id
                ),
            }
    ...
    use memora_a11y::tree::TreeWalkResult;
    if let Some(ref app) = trigger_app {
        let decision = walk_budget.should_walk(app);
        if !decision.walk && !bypass_capture_throttles {
            debug!(
                "walk budget: throttling tree walk for {} (tier={:?}) â€” skipping capture",
                app, decision.tier
            );
            // Skip the entire capture. Previously this fell through to a
            // TreeWalkResult::NotFound which triggered OCR fallback â€” but the
    ...
    let tree_walk_result = if monitor_hosts_focus {
        Some(
            tokio::task::spawn_blocking(move || {
                memora_capture::paired_capture::walk_accessibility_tree(&config)
            })
            .await?,
        )
    } else {
    ...
    // bail out entirely â€” don't OCR the screenshot.
    ...
    // walk outcome. Found â†’ stored/deduped/empty (deduped when the content
    // hash matches the previous frame under the same dedup gate used below);
    // NotFound â†’ error. Skipped windows are intentionally NOT counted as walk
    // attempts â€” they're user/incognito filters, not real walks.
    match tree_walk_result.as_ref() {
        Some(TreeWalkResult::Found(snap)) => {
            ...
            if snap.walk_duration > std::time::Duration::from_millis(100) {
                ...
                debug!(
                    "walk budget: {}ms for {} â†’ tier={:?} (next: max_nodes={}, timeout={}ms)",
            ...
            use memora_a11y::tree::TruncationReason;
            if snap.text_content.is_empty() {
                ...
                // UTF-8 byte length (O(1)) as a cheap text-volume proxy â€”
                ...
                // eligible) won't be stored â€” count it as deduped.
                let is_dedup =
        ...
        // Skipped: user filter / incognito â€” not a walk attempt, don't count.
    ...
    // force a write every 30s even if the hash matches â€” see `dedup_applies`.
    ...
    // Skip lock screen / screensaver â€” these waste disk and pollute timeline.
    ...
    if let Some(ref app) = app_name_owned {
        if is_lock_screen_app(app) {
            ...
            // Screen was marked locked but now a real app is focused â€” unlock
            debug!(
        ...
        // This is the common case when locked via Cmd+Ctrl+Q â€” the tree walker
    ...
    // Guard on the walked app matching the resolved app â€” for focus-change
    ...
    // frame is the safe fallback. Floatâ†’int casts saturate, and
    ...
    let focused_window_bounds = {
        ...
        tree_snapshot
            ...
            .map(
                |b| memora_capture::paired_capture::FocusedWindowBounds {
...
/// works for Electron apps (Obsidian, Discord, â€¦) where the AX sys-wide
/// query returns empty â€” see issue #3002. Falls back to AX only for
...
/// (50â€“200 entries on a typical mac) and the iteration plus `is_active()`
/// check costs a few ms. Capture triggers fire on every click / typing
/// pause / visual change â€” paying that cost on every trigger is wasteful
...
fn get_focused_metadata_lightweight() -> Option<LightweightFocusedMetadata> {
    let (app_name, window_name, _) =
        memora_a11y::platform::linux::get_active_window_info_fresh()?;
...
/// Windows: `GetForegroundWindow` + cached process-name lookup â€” no UIA/COM.
...
/// case to a single atomic load â€” same rationale as the macOS implementation.
...
fn get_focused_metadata_lightweight() -> Option<LightweightFocusedMetadata> {
    ...
    let cache = CACHE.get_or_init(|| {
        ...
        // (autostart) â€” fall back to "fresh", costing one stale-second at most.
    ...
    let fresh = memora_a11y::platform::windows::get_focused_app_window_lightweight().map(
...
fn query_frontmost_app_name_uncached() -> Option<String> {
    ...
    // AX fallback â€” the pre-#3002 path. Kept for the edge cases where
...
/// Grid dimension for the corruption sampler: a ~15Ã—15 grid â‰ˆ 225 points.
const CORRUPT_GRID: usize = 15;

/// True if `px` is the green a zeroed-chroma / truncated-JPEG fill takes.
///
/// Zeroed YUV420 chroma decodes to roughly `(0, 135..255, 0)` in RGB â€” a
...
///
/// Reads ~225 pixels straight from the `DynamicImage` via `get_pixel` â€” there
...
///
/// Two signals:
///   * **black** â€” when ScreenCaptureKit excludes an ignored window (or a
///     monitor is asleep / shows DRM-protected content), the pixels come back
///     `(0,0,0)`. >95% near-black â‡’ skip. Real content, even dark mode, has
///     variation (scrollbars, text, status bar) and won't trip it.
///   * **green band** â€” a truncated MJPEG frame / partially written capture
...
///     decode fill, not textured content), (c) â‰¥20% of the height and â‰¥3 rows,
...
///     never shows the contentâ†’green transition a partial decode does, and is
...
fn sample_frame_corruption(image: &image::DynamicImage) -> FrameCorruption {
    ...
    if w == 0 || h == 0 {
        // Degenerate frame â€” treat as black so it's skipped.
    ...
    // Contiguous run of â‰¥90%-green rows anchored at the bottom, accumulating
    ...
    let spread = (0..3)
        ...
        .max()
        .unwrap_or(255);
    // (d) real content must sit above the band â€” the top sampled row is not
...
enum CorruptKind {
    ...
    Black,
    /// A flat green decode-garbage band â€” a truncated / partially-written
...
mod tests {
    ...
    fn aggregate_and_per_monitor_liveness_advance_together() {
        let aggregate = memora_screen::PipelineMetrics::new();
        let monitor = memora_screen::PipelineMetrics::new();
    ...
    fn resolve_capture_metadata_uses_lightweight_metadata_when_tree_values_are_blank() {
        let snapshot = memora_a11y::tree::TreeSnapshot {
            app_name: "  ".into(),
            app_id: None,
            executable: None,
            app_version: None,
            ...
            simhash: 0,
            truncated: false,
            truncation_reason: memora_a11y::tree::TruncationReason::None,
            max_depth_reached: 0,
            window_bounds: None,
        };
        let metadata = LightweightFocusedMetadata {
    ...
    fn resolve_capture_metadata_prefers_lightweight_metadata_over_stale_tree_values() {
        ...
        let metadata = LightweightFocusedMetadata {
            ...
            window_name: Some("MEMORA-wayland-fix".into()),
        };
        ...
        assert_eq!(window_name.as_deref(), Some("MEMORA-wayland-fix"));
    }
    ...
    fn workflow_checkpoint_trigger_classification() {
        // The union still covers everything the old single class did â€” only
    ...
    fn hard_vs_soft_checkpoint_trigger_split() {
        // Hard checkpoints (#4844): rare, context-defining â€” unconditional
        ...
        // Soft checkpoints (#4844): raw, unspaced keystroke/paste stream â€”
    ...
    fn checkpoint_gate_proceeds_on_floor_or_hd_active() {
        // Neither condition met â€” defer.
        assert_eq!(checkpoint_gate(false, false), CheckpointGate::Defer);
        // Floor elapsed â€” proceed regardless of HD.
        assert_eq!(checkpoint_gate(true, false), CheckpointGate::Proceed);
        // HD active â€” proceed even if the floor hasn't elapsed; HD wants
    ...
    fn defer_checkpoint_keeps_latest_trigger_and_accumulates_corr_ids() {
        ...
        // accumulate â€” every triggering row must still reach a frame.
        defer_checkpoint(
            &mut pending,
            &mut pending_corr_ids,
            CaptureTrigger::Clipboard,
        ...
        // would have none) â€” kind still updates, ids list is untouched.
        defer_checkpoint(
            &mut pending,
            &mut pending_corr_ids,
            CaptureTrigger::TypingPause,
    ...
    fn checkpoint_debounce_floor_is_independent_of_min_capture_interval() {
        ...
        // checkpoint floor is its own clock â€” a soft checkpoint shouldn't wait
    ...
    fn test_dedup_applies() {
        ...
        // dedup-eligible like any content-driven trigger â€” unlike hard
        ...
        // HD active â†’ dedup is bypassed even for an otherwise-eligible trigger.
        ...
        // â€¦but the meeting bypass is scoped to visual changes only â€” a Click in
    ...
    fn capture_throttle_bypass_classification() {
        ...
        // or the walk budget â€” that's the storm this issue is about.
    ...
    fn test_debounce() {
        ...
        // Just created â€” should not be able to capture yet (within debounce window)
    ...
    fn meeting_caps_the_idle_capture_interval() {
        ...
        // ~6s since the last capture â€” past the meeting cap, well under 30s.
    ...
    fn user_idle_pin_survives_power_profile_change() {
        ...
        // (e.g. unplugging â†’ battery-saver wants 300s) must NOT relax it.
        let config = EventDrivenCaptureConfig {
        ...
        // override the user's explicit choice â€” the pin is authoritative.
    ...
    fn reduce_drained_gates_off_pass_through() {
        ...
        // Both gates off (capture_on_X=true) â†’ both pass.
        let (trigger, corrs) = reduce_drained_triggers(drained, false, false);
        assert_eq!(trigger, Some(CaptureTrigger::KeyPress));
        assert_eq!(corrs, vec![1, 2]);
    }
    ...
    fn test_frame_with_visible_menubar_not_skipped() {
        // A menu bar at y=0 gets sampled by the grid â†’ enough non-black
        ...
        // so the frame is NOT detected as mostly black â€” correct, it has content.
        assert!(!is_frame_corrupt(&img));
    }
    ...
    fn test_dark_mode_app_not_falsely_detected() {
        // Dark mode: dark grey background (30, 30, 30) â€” NOT pure black
        let mut buf = image::RgbImage::new(1920, 1080);
        for px in buf.pixels_mut() {
    ...
    fn test_bottom_green_band_quarter_height_is_corrupt() {
        // ~25% bottom band still trips the â‰¥20% / â‰¥3-row threshold.
    ...
    fn test_high_truncation_leaves_thin_top_content_still_corrupt() {
        ...
        // Still has the contentâ†’green transition, so it's flagged.
    ...
    fn test_full_frame_green_not_flagged() {
        ...
        // green-screen backdrop is green TOP-TO-BOTTOM â€” there's no
        // contentâ†’green transition, so we deliberately leave it alone rather
    ...
    fn test_thin_green_dock_not_corrupt() {
        // A ~7% bottom dock/taskbar in green is real content, not a fill â€”
    ...
    fn test_textured_green_bottom_not_corrupt() {
        ...
        // is far > 24 â†’ NOT a flat decode fill, so it's left alone. This
        // specifically exercises the uniformity guard.
        let mut buf = image::RgbImage::new(1920, 1080);
        for y in 0..1080 {
            for x in 0..1920 {
                let color = if y >= 540 {
                    // g in 110..=229 â€” always garbage-green, spread ~119 â‰« 24
    ...
    // come back from ScreenCaptureKit as black, never green â€” so the green path
    // never fires on them, and only a *whole-frame* black trips the skip.

    #[test]
    fn test_drm_fullscreen_black_skipped() {
        // Full-screen DRM playback / asleep display â‡’ all-black â‡’ skipped.
    ...
    fn test_drm_black_region_with_content_kept() {
        ...
        // NOT drop the whole frame â€” only the protected region is black, the
        // rest is indexable content.
        let img = frame_with_bottom_band(1920, 1080, 648, [120, 130, 140], [0, 0, 0]);
        assert!(!sample_frame_corruption(&img).green_band); // black band â‰  green band
    ...
    fn test_rgba_frame_sampled_correctly() {
        ...
        // format) must be handled identically to RGB â€” green band still caught.
    ...
    fn should_release_only_on_pause_entry_edge() {
        ...
        // the entire pause window â€” the exact cost e47f53fc4 eliminated.
        assert!(
            should_release_on_pause_entry(false, true),
            "non-paused â†’ paused: must release the OS handle"
        );
        assert!(
        ...
        assert!(
            ...
            "paused â†’ resumed: must NOT release (capture is about to need it)"
        );
        assert!(
    ...
    fn high_fps_no_change_returns_none() {
        ...
        // No state transition â€” caller must do nothing.
    ...
    fn high_fps_baseline_change_during_override_persists_on_exit() {
        ...
        // not be dropped â€” exiting must restore the NEW baseline.
        let mut o = HighFpsBookkeeping::new(500);
        assert_eq!(o.on_controller_state(Some(100), 500), Some(100));
        ...
        // Exit â€” must restore the updated baseline, not 500.
    ...
    fn high_fps_redundant_calls_are_noops() {
        let mut o = HighFpsBookkeeping::new(500);
        assert_eq!(o.on_controller_state(Some(100), 500), Some(100));
        // Same active interval â€” no transition, no write.
        assert_eq!(o.on_controller_state(Some(100), 100), None);
        assert_eq!(o.on_controller_state(Some(100), 100), None);
        assert_eq!(o.on_controller_state(None, 100), Some(500));
        // Same idle state â€” no transition, no write.
        assert_eq!(o.on_controller_state(None, 500), None);
    }

    #[test]
    fn high_fps_runtime_interval_change_while_active() {
        // User toggles 10 fps â†’ 30 fps via the tray slider mid-override.
    ...
    fn high_fps_power_profile_then_interval_change_then_exit() {
        ...
        // Enter override.
        assert_eq!(o.on_controller_state(Some(100), 500), Some(100));
        // Power profile drops baseline to 1000 (saver) â€” live config stays at 100.
        assert_eq!(o.on_baseline_change(1000), 100);
        // User cranks override to 30 fps via tray slider.
        assert_eq!(o.on_controller_state(Some(33), 100), Some(33));
        // Exit override â€” restore the saver baseline.
    ...
    fn liveness_none_seq_never_recovers() {
        // No cached stream â†’ nothing to judge, even across a long span.
        let mut w = StreamLivenessWatch::new(Instant::now());
        let t0 = Instant::now();
        for i in 0..100u64 {
    ...
    fn liveness_recovers_when_seq_flat_past_timeout() {
        ...
        // Flat past the stall window â†’ frozen.
        assert!(w.observe(
            Some(42),
            t0 + STREAM_STALL_TIMEOUT + Duration::from_millis(1)
        ));
    ...
    fn liveness_post_rebuild_seq_reset_is_healthy() {
        // After a rebuild the counter restarts at 0 (seq < prev) â€” that's the
    ...
    fn liveness_cooldown_prevents_thrash() {
        ...
        assert!(w.observe(Some(7), t1)); // first recovery
                                         ...
                                         // recovery cooldown â†’ no second teardown.
        assert!(!w.observe(
            Some(7),
