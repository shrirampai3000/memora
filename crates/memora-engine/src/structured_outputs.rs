...
// Live Views alone may define 12 dashboards Ã— 24 Blocks. Keep headroom for
...
fn load_unlocked(MEMORA_dir: &Path) -> Result<OutputTargetStore, StructuredOutputError> {
...
fn with_store<T>(
    ...
    let mut store = load_unlocked(MEMORA_dir)?;
    let (result, changed) = operation(&mut store)?;
    if changed {
        write_unlocked(MEMORA_dir, &store)?;
    }
    Ok(result)
}
...
pub fn list_output_targets(
    ...
    with_store(MEMORA_dir, |store| Ok((store.targets.clone(), false)))
}
