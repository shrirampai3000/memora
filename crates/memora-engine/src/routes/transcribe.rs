...
//! transcription engine (whisper, deepgram, MEMORA-cloud, etc.).
//!
//! Compatible with:
...
//!     -F model=whisper-1
//!
//! The `model` field is accepted for compatibility but ignored â€” the user's
...
use MEMORA_audio::utils::ffmpeg::read_audio_from_file;
use serde::Serialize;
use serde_json::json;
use std::sync::Arc;
...
pub async fn transcribe_handler(
    ...
    // Decode audio via ffmpeg (any format â†’ f32 PCM 16kHz mono)
    ...
    let transcription_engine = if let Some(ref engine_str) = engine_override {
        // Explicit engine override â€” must create a new instance
