...
use memora_db::{DatabaseManager, FrameData, Order};
...
/// returned them). Down-sampling preserves the full time span â€” just sparser
...
pub(crate) fn create_time_series_frame(chunk: FrameData) -> TimeSeriesFrame {
    ...
    let mut device_frames: Vec<DeviceFrame> = chunk
        .ocr_entries
        .into_iter()
        // Filter out MEMORA frames at display time
        .filter(|device_data| !is_MEMORA_app(&device_data.app_name))
            ...
            if let Message::Text(text) = msg {
                match serde_json::from_str::<StreamFramesRequest>(&text) {
                    Ok(request) => {
                        ...
                        // use database â€” hot_cache only has recent in-memory frames.
                        let now = Utc::now();
                        ...
                        *live_sub_clone.lock().await = Some(is_today);
                        ...
                        if is_today {
                            ...
                            // Read from hot cache â€” pure in-memory, <1ms
                            let frames =
                            ...
                            if backfill_needed && backfill_limit > 0 {
                                ...
                                info!(
                                    "skipping DB backfill â€” hot cache covers full range or stream limit reached"
                                );
                            }
                        } else {
                            // Past day â€” one-shot DB query (acceptable, rare)
                ...
                result = frame_rx_cache.recv() => {
                    match result {
                        Ok(hot_frame) => {
                            ...
                            if is_MEMORA_app(&hot_frame.app_name) {
                                continue;
                            }
...
mod tests {
    use super::*;
    use memora_db::{AudioEntry as DbAudioEntry, FrameData, OCREntry, OcrEngine};
    ...
    // the tail like truncate did â€” otherwise the morning vanishes on dense days.
    #[test]
    ...
    #[test]
    fn test_create_time_series_frame_filters_MEMORA_app_names() {
        let mut frame_data = create_test_frame_data(2, 0);
        frame_data.ocr_entries[0].app_name = "MEMORA Desktop".to_string();
