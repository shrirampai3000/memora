...
use memora_events::{send_event, subscribe_to_all_events, Event as MEMORAEvent};
    ...
    let incoming = lifecycle.spawn(async move {
        while let Some(Ok(msg)) = receiver.next().await {
            if let Message::Text(t) = msg {
                if let Ok(event) = serde_json::from_str::<MEMORAEvent>(&t) {
...
/// Lightweight real-time metrics WS â€” reads atomic counters every 500ms.
...
async fn handle_meeting_overlay_socket(
    ...
    let mut status_stream =
        memora_events::subscribe_to_event::<MeetingStatusResponse>("meeting_status_changed");
    let mut delta_stream =
        memora_events::subscribe_to_event::<serde_json::Value>("meeting_transcript_delta");
    let mut final_stream =
        memora_events::subscribe_to_event::<serde_json::Value>("meeting_transcript_final");
        ...
        tokio::select! {
            ...
            _ = interval.tick() => {
                ...
                let payload = serde_json::json!({
                    "audio": {
                        "mic_capture_active": false,
                        "vad_passed": 0,
                        "vad_rejected": 0,
                        "chunks_sent": 0,
                        "total_words": 0,
                        "audio_level_rms": 0.0,
                        "device_levels": serde_json::json!({}),
                    },
                    "vision": {
