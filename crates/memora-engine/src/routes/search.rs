...
use memora_db::{ContentType, Order, SearchResult, SemanticContextQuery, SemanticFrameContext};
use memora_semantic::{IdentityQuality, SemanticKind};
...
pub(crate) struct SearchQuery {
    ...
    /// scroll buffers, scrolled-off IDE editor lines) â€” passing
    ...
    /// namespace (`people`, `projects`, `workflows`, â€¦) and ordered
...
/// Same but for `Option<bool>` â€” empty string deserializes to `None` so a
...
pub struct SearchResponse {
    ...
    /// (`people`, `projects`, `workflows`, â€¦) and ordered most-frequent
...
/// full-scans (no tag index) and a hot tag fans out to itemsÃ—tags rows, so on a
...
/// real CPU each), so running one per result item concurrently â€” `limit` has no
/// upper clamp â€” turns `include_frames=true` into a process-spawn storm (e.g.
/// `limit=500` â†’ 500 simultaneous ffmpeg processes). Four keeps the wall-clock
    ...
    Fut: Future<Output = Result<String, anyhow::Error>>,
{
    ...
    // frame that can't be extracted â€” e.g. an offset in the still-being-recorded
    // chunk, or a corrupt file â€” must not discard the frames for the whole
...
/// callers expect (`person:` â†’ `people`, `project:` â†’ `projects`,
/// `workflow:` â†’ `workflows`); unknown namespaces just get a trailing `s`.
...
/// `namespace â†’ values` map for the `related` block. `person:louis` lands as
...
pub fn truncate_middle(text: &str, max_chars: usize) -> String {
    ...
    // documented `max_content_length=0` â‡’ full text contract honored by the
    ...
    // this guard a 0 cap returned only the "...(truncated N chars)..." marker â€”
...
/// because it belongs to MEMORA itself.
pub fn is_MEMORA_app(app_name: &str) -> bool {
    const MEMORA_APP_NEEDLE: &[u8] = b"MEMORA";
    app_name
        .as_bytes()
        .windows(MEMORA_APP_NEEDLE.len())
        .any(|window| window.eq_ignore_ascii_case(MEMORA_APP_NEEDLE))
}
...
/// text-bearing fields.
///
/// Shared with the `MEMORA search` CLI so terminal output matches the
/// API exactly â€” `jq` filters written against one work against the other.
...
fn deduplicate_ocr_and_ui(content_items: &mut Vec<ContentItem>) {
    ...
    // Build a map of timestamp (rounded to 1s) + app_name â†’ index of the OCR entry.
...
pub(crate) fn compute_search_cache_key(query: &SearchQuery) -> u64 {
    ...
    // on_screen changes the result set materially â€” must be in the
    ...
    // Tags change the result set materially â€” must be in the cache key so a
...
fn search_overloaded_response() -> Response<Body> {
    Response::builder()
        ...
        .body(Body::from(
            json!({
                "error": "search is busy â€” retry shortly",
...
fn search_timeout_response() -> Response<Body> {
    Response::builder()
        ...
        .body(Body::from(
            json!({
                "error": "search query timed out after 30s â€” try a narrower time range or add filters",
            })
            .to_string(),
        ))
        .unwrap_or_else(|_| StatusCode::INTERNAL_SERVER_ERROR.into_response())
...
fn classified_search_database_response(error: &sqlx::Error) -> Option<Response<Body>> {
    ...
    } else if memora_db::is_sqlite_interrupt(error) {
...
pub(crate) async fn search(
    ...
    // LLM agent has no schema-level way to bypass this â€” the permissions
    ...
    let mut content_items: Vec<ContentItem> = match results {
        SearchPage::Standard(results) => results
            .iter()
            // Filter out MEMORA results at display time
            .filter(|result| match result {
                SearchResult::OCR(ocr) => !is_MEMORA_app(&ocr.app_name),
                SearchResult::Audio(_) => true, // Audio doesn't have app_name
                SearchResult::UI(ui) => !is_MEMORA_app(&ui.app_name),
                SearchResult::Input(input) => input
                    .app_name
                    .as_ref()
                    .is_none_or(|app| !is_MEMORA_app(app)),
        ...
        SearchPage::Parsed(contexts) => contexts
            .into_iter()
            .filter(|context| !is_MEMORA_app(&context.app_name))
            .map(|context| {
    ...
    // binary frame data. Fail closed â€” return 503 rather than leak raw text.
    if query.filter_pii {
...
pub(crate) async fn keyword_search_handler(
    ...
    if query.group {
        ...
        let filtered: Vec<_> = groups
            .into_iter()
            .filter(|group| !is_MEMORA_app(&group.representative.app_name))
            .collect();
        ...
        let filtered: Vec<_> = matches
            .into_iter()
            .filter(|m| !is_MEMORA_app(&m.app_name))
            .collect();
...
/// `?tags=person:ada,project:atlas` â†’ `["person:ada", "project:atlas"]`.
...
mod tests {
    use super::*;
    use memora_core::pipes::permissions::PermissionRule;
    ...
    fn ocr_text_json_is_not_observable_in_search_response() {
        ...
        let make_result = |text_json: &str| {
            SearchResult::OCR(memora_db::OCRResult {
                frame_id: 42,
    ...
    fn accessibility_event_source_is_observable_in_search_response() {
        let result = SearchResult::UI(memora_db::UiContent {
            id: 42,
    ...
    /// (`limit` has no upper clamp â€” `limit=500` meant 500 simultaneous ffmpeg
    ...
    async fn include_frames_bounds_concurrent_extractions() {
        ...
        attach_frames_to_ocr_items(&mut items, move |file_path, _offset_index| {
            ...
            async move {
                ...
                // future â€” an unbounded join_all would drive all 32 here.
    ...
    /// Issue #2436: changing `on_screen` must invalidate the cache â€”
    ...
    fn test_group_related_tags_buckets_by_namespace() {
        ...
        let rows = vec![
            ...
            ("project:MEMORA".to_string(), 7),
            ("person:connor".to_string(), 5),
            ("workflow:ai-coding".to_string(), 4),
            ("project:MEMORA-finance".to_string(), 3),
    ...
    #[test]
    fn test_is_MEMORA_app_case_insensitive() {
        assert!(is_MEMORA_app("MEMORA"));
        assert!(is_MEMORA_app("MEMORA Desktop"));
        assert!(is_MEMORA_app("com.memora.capture"));
        assert!(!is_MEMORA_app("pipe viewer"));
    }
    ...
    fn test_truncate_middle_unicode() {
        let text = "hello ðŸŒ world ðŸŽ‰ end";
        let result = truncate_middle(text, 10);
