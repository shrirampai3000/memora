...
//!
//! A harness reaches MEMORA only through an MCP server that speaks HTTP to
//! localhost:3030 â€” it cannot open the in-chat "connect app" card the way a
...
//!
//!   1. The `memora_connect_app` MCP tool POSTs `/v1/connect-request`.
//!   2. We mint a request id, emit an `acp_connect_request` event, and await a
//!      matching `acp_connect_response` (with a timeout) â€” the HTTP request
...
pub struct ConnectRequestBody {
    ...
    /// The chat/conversation id (MEMORA_CHAT_SESSION_ID) so the desktop can
...
async fn next_response(
    responses: &mut (impl StreamExt<Item = memora_events::Event<Value>> + Unpin),
    request_id: &str,
    terminal_only: bool,
) -> Option<String> {
...
/// rendered anywhere â€” return "timeout" fast. Once acked, wait `wait` for the
...
async fn run_connect_request(body: ConnectRequestBody, grace: Duration, wait: Duration) -> Value {
    ...
    let mut responses = memora_events::subscribe_to_event::<Value>(RESPONSE_EVENT);
    ...
    let status = match acked {
        ...
        // up fast â€” the tool falls back to the async, tool-result-driven card.
        Ok(None) | Err(_) => "timeout",
        Ok(Some(status)) if status == "connected" => "connected",
        Ok(Some(status)) if status == "declined" => "declined",
        // "shown": the card is up. Phase 2 â€” wait for the user's real answer.
        Ok(Some(_)) => {
...
mod tests {
    ...
    fn answer_next_request(
        ...
        let mut requests = memora_events::subscribe_to_event::<Value>(REQUEST_EVENT);
    ...
    // event bus â€” parallel tests would consume each other's request events.
    #[tokio::test]
    async fn connect_broker_round_trip() {
        ...
        // 4. No ack at all â†’ fast timeout after the grace window (chat not open
        ...
        // 5. Shown but never answered â†’ timeout after the (short) wait phase.
