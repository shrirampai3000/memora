...
// (see memora-db OutputRecord); everything HTTP-facing says "artifacts".
...
pub fn spawn_artifact_search_backfill(db: Arc<memora_db::DatabaseManager>) {
...
// ---------------------------------------------------------------------------
// SAF (MEMORA Artifact Format) detection
//
...
// file ends with `.saf.json` â€” or parses as a JSON object containing the
// key "saf_version" â€” we validate the envelope and, on success, mark the
...
// apps/memora-app-tauri/lib/saf.ts â€” keep them in sync.
// ---------------------------------------------------------------------------
...
pub(crate) enum SafDetection {
    /// Not a SAF candidate (or malformed JSON) â€” register as a plain file,
    /// no warning.
    NotSaf,
    /// Looked like SAF but failed envelope validation â€” register as a plain
...
/// (any non-empty string â‰¤32 chars â€” forward compatibility with new kinds);
...
pub(crate) fn detect_saf(content: &str, filename: &str) -> SafDetection {
    ...
    if !is_saf_name && !has_saf_key {
        // Ordinary JSON output â€” not claiming to be an artifact.
...
/// returns `None`) when a SAF candidate fails validation â€” registration
...
/// POST /artifacts/register â€” copy a file into the canonical outputs dir
...
pub(crate) async fn register_artifact_handler(
    ...
    // SAF detection â€” a valid envelope upgrades the row to kind="saf";
...
/// DELETE /artifacts/:id â€” remove a registered artifact row and its copied
/// file under `~/.memora/outputs/`. Filesystem-derived pipe artifacts
...
pub(crate) struct ListArtifactsQuery {
    ...
    /// Exact SAF kind match (e.g. "sop") â€” only SAF-enveloped artifacts.
    pub saf_kind: Option<String>,
...
fn artifact_matches_query(item: &ArtifactItem, q: &str) -> bool {
    ...
    // never matches because the haystack is lowercased but the needle isn't â€”
...
/// GET /artifacts â€” unified listing of AI-generated artifacts.
///
/// Merges registered outputs (DB rows whose files live under
/// `~/.memora/outputs/`) with artifacts derived from pipe directories
...
pub(crate) async fn list_artifacts_handler(
        ...
        for (pipe_name, decls) in declarations {
            for (decl, abs_path) in decls {
                ...
                items.push(ArtifactItem {
                    ...
                    preview,
                    // Filesystem-derived items are never SAF â€” only the
    ...
    // Chat outputs carry a per-session id as source â€” group them under the
    ...
    // Newest first by parsed instant â€” sources emit different UTC offsets,
...
/// centralized `~/.memora/outputs/` directory and the DB.
///
...
pub async fn auto_register_pipe_artifacts(
    db: &memora_db::DatabaseManager,
    items: Vec<(
        memora_core::pipes::ArtifactDeclaration,
        std::path::PathBuf,
    )>,
    pipe_name: &str,
    ...
    for (decl, abs_path) in items {
        ...
        // Create dirs + copy (always â€” file may have changed)
        if let Some(parent) = dest.parent() {
        ...
        // artifact_id) first â€” a re-emit under a new filename updates the
...
mod tests {
    ...
    fn build_output_path_rejects_traversal() {
        let root = std::path::Path::new("/fake/MEMORA");
    ...
    fn build_output_path_normal() {
        ...
        assert_eq!(
            ...
            Some(std::path::PathBuf::from(
                "/fake/MEMORA/outputs/pipe/my-pipe/report.txt"
            ))
        );
    }
    ...
    fn artifact_query_matches_path_and_original_path() {
        let item = ArtifactItem {
            ...
            kind: "html".to_string(),
            path: "/Users/test/.memora/outputs/pipe/focus-pipe/relatorio-foco-offline.html"
                .to_string(),
            original_path: Some(
                "/Users/test/.memora/pipes/focus-pipe/output/relatorio-foco.html".to_string(),
            ),
            size_bytes: 1024,
    ...
    // â”€â”€ SAF envelope validator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Mirrored by apps/memora-app-tauri/lib/saf.ts (parseSafArtifact);
    ...
    fn saf_outputs_validator_rejects_bad_slug() {
        for bad in [
            ...
            assert!(err.contains("artifact_id"), "slug {:?} â†’ {}", bad, err);
        }
    }
    ...
    fn saf_outputs_validator_rejects_version_zero_and_non_integer() {
        for bad in [json!(0), json!(-3), json!(1.5), json!("2")] {
            ...
            let err = validate_saf_envelope(&env).unwrap_err();
            assert!(err.contains("version"), "version {:?} â†’ {}", bad, err);
        }
    }
    ...
    // â”€â”€ SAF detection (filename + content classification) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn saf_outputs_detect_marker_key_in_any_filename() {
        ...
        // marker present but envelope invalid â†’ Invalid (warn + plain file)
    ...
    // â”€â”€ auto-register integration (in-memory db + real files) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn decl(path: &str) -> memora_core::pipes::ArtifactDeclaration {
        memora_core::pipes::ArtifactDeclaration {
            path: path.to_string(),
            title: None,
            kind: None,
    ...
    async fn auto_register_outputs_saf_reemit_updates_not_duplicates() {
        ...
        // higher version, same file â†’ same row, bumped version
        ...
        // same version again â†’ idempotent
        auto_register_pipe_artifacts(&db, items(), "my-pipe", None, &sp_dir).await;
        let rows = db
            .list_outputs(Some("my-pipe"), None, None, 100, 0)
            .await
    ...
    async fn auto_register_outputs_saf_invalid_falls_back_to_plain() {
        ...
        // valid JSON, invalid envelope (version 0) â†’ plain file, no SAF cols
    ...
    async fn auto_register_outputs_saf_artifact_id_scoped_per_pipe() {
        ...
        // same artifact_id from two pipes â†’ two distinct rows (source scoping)
        let a = db
    ...
    // â”€â”€ Chat artifact visibility â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async fn chat_artifacts_visible_in_unified_listing() {
        ...
        // Verify display_source grouping: source_type="chat" â†’ "display: chat"
        let item = ArtifactItem {
            registered: true,
            id: Some(row.id),
            source: row.source,
