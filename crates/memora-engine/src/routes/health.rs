...
///    (`intentionally_deferring`) â€” expected, not broken.
...
fn suspected_stall_cause(read_idle: u32, write_idle: u32) -> &'static str {
    if write_idle == 0 && read_idle == 0 {
        ...
        "pools idle â€” cause upstream (writer path, lock contention, or missing metrics)"
    }
}
...
/// wrong â€” the database was idle (119ms average write latency, every pool
...
pub(crate) enum VisionStallCause {
    ...
    SilentLoss,
    /// Attempts stopped entirely â€” capture is paused or the backend is
...
impl VisionStallCause {
    ...
    /// [`memora_screen::monitor::secs_since_capture_backend_fallback`].
    ...
    pub(crate) fn detail(
        ...
        match self {
            Self::SilentLoss => format!(
                "no vision frame written for {stalled_secs}s â€” capture is attempting but frames \
            ...
            Self::CapturePaused => match backend_fallback_secs {
                Some(ago) if ago <= CAPTURE_BACKEND_FALLBACK_RECENT_SECS => format!(
                    "no vision frame written for {stalled_secs}s â€” the capture backend has \
            ...
            Self::DbWritesNotLanding => format!(
                "vision DB writes stalled for {stalled_secs}s â€” capture running but DB writes not \
                 landing"
            ),
        }
    }
...
/// state with "check if screen recording permissions are enabled" â€” without
/// ever consulting a permission result. A user whose pixels were off because
/// MEMORA itself turned them off was sent to macOS System Settings to fix
...
impl VisionReason {
    ...
    /// Permission advice appears here for exactly one reason â€” the one backed
    ...
    pub(crate) fn instruction(self) -> Option<&'static str> {
        match self {
            ...
            Self::DisabledBySetting => {
                Some("Screen capture is turned off in MEMORA. Turn it back on in Settings â†’ Recording.")
            }
            Self::NoDisplaysExpected => {
                Some("No display is being recorded â€” every selected monitor is paused or asleep. Resume one in Settings â†’ Recording.")
            }
            Self::ScreenshotsDisabledByConfig => {
                Some("Screenshots are turned off in MEMORA, so only on-screen text is being captured. Re-enable them in Settings â†’ Recording. This is not a permission problem.")
            }
            ...
            Self::PermissionDenied => {
                Some("macOS is blocking screen capture. Grant Screen Recording to MEMORA in System Settings â†’ Privacy & Security, then restart the app.")
            }
            Self::CaptureStalled => {
                Some("Screen capture stopped producing frames. Restarting MEMORA usually clears it; if it returns, please send logs from the Help section.")
            }
            Self::NotStarted => {
...
/// fault, because a user who turned pixels off does not have a problem â€” and
...
pub(crate) fn classify_vision_reason(
    ...
    screenshot_state: memora_screen::ScreenshotCaptureState,
    permission_granted: bool,
    frame_status: &str,
) -> VisionReason {
    use memora_screen::ScreenshotCaptureState as S;
    ...
    // Only now can a missing frame be a real fault â€” and permission is only
...
fn classify_audio_status(
    ...
    if audio_disabled {
        ...
        // Audio is on but there is no microphone to capture from â€” expected
...
fn capture_status(
    ...
    let (status, severity, reason) = if audio_disabled {
        ...
        (
            "no_input_device",
            "ok",
            "no microphone detected â€” audio capture idle, screen recording continues",
        )
...
pub struct HealthCheckResponse {
    ...
    ///
    /// `frame_status` alone collapses "MEMORA turned pixels off" and "the
...
pub struct PipelineHealthInfo {
    ...
    /// Capture cycles skipped by content dedup (static screen â€” expected/benign).
    pub dedup_skips: u64,
...
/// launchd watchdogs â€” none of which expect it to stall. If
        ...
        || async {
            match tokio::time::timeout(HEALTH_RESPONSE_BUDGET, health_check_inner(&state)).await {
                ...
                Err(_) => {
                    ...
                    warn!(
                        "health_check: inner computation exceeded {:?} budget â€” serving last cached snapshot",
...
async fn health_check_inner(state: &Arc<AppState>) -> HealthCheckResponse {
    ...
    let device_statuses: Vec<(String, bool, u64)> = Vec::new();
    let current_device_capture_at = HashMap::<String, u64>::new();
    let global_audio_active = false;
    let most_recent_audio_timestamp = 0u64;
    ...
    let last_audio_ts = 0u64;

    let now = Utc::now();
    let now_ts = now.timestamp() as u64;

    // 60 seconds â€” tight enough to detect real stalls, loose enough to
    ...
    let threshold_secs = {
        #[cfg(debug_assertions)]
        {
            if crate::event_driven_capture::e2e_capture_loop_silent_fault_started() {
                ...
                match std::env::var("MEMORA_E2E_SEED").ok() {
    ...
    // â€” data is being captured but silently lost.
        ...
        && vision_snap.uptime_secs > 120.0
    {
        ...
        // not "writes stopped" â€” same fix as audio side.
        ...
        // "climbing or flat" â€” the question the triage rule below turns on.
        ...
        if stalled {
            ...
            if now_ts.saturating_sub(prev) >= 60 {
                ...
                // (idle user, slide deck, video call, IDE waiting) â€” NOT a
                ...
                // failed â€” without us having to email the user back asking
                ...
                //
                //   attempts climbing, persisted climbing, dedup â‰ˆ 0
                //     â†’ healthy active screen
                //   attempts climbing, persisted â‰ˆ flat, dedup climbing
                //     â†’ static screen / idle user (false alarm)
                //   attempts climbing, persisted â‰ˆ flat, dedup flat
                //     â†’ real silent loss between attempt and writer
                //   attempts flat too
                //     â†’ capture itself paused (TCC revoke, display sleep)
                let silent_loss = vision_snap
                    .capture_attempts
                    .saturating_sub(vision_snap.frames_db_written)
                    .saturating_sub(vision_snap.dedup_skips);
                warn!(
                    "health_check: no unique vision frame in {}s (capture heartbeat {}s ago â€” {}) | lifetime: attempts={}, persisted={}, dedup={}, silent_loss={} | pool: read={}/{} idle, write={}/{} idle | suspected: {}",
                    ...
                    match cause {
                        ...
                        VisionStallCause::CapturePaused =>
                            "capture stopped attempting â€” backend paused or refusing frames",
                        VisionStallCause::DbWritesNotLanding =>
    ...
    let audio_db_write_stalled = false;
    ...
    let frame_status = if state.vision_disabled {
        ...
        "ok" // screen locked â€” no captures expected, not a real stall
    ...
    let audio_status = "disabled".to_string();
    let transcription_paused = false;
    let active_audio_devices = 0;
    let active_input_devices = 0;
    let paused_audio_devices = 0;
    let paused_input_devices = 0;
    let capture_status = "disabled".to_string();
    ...
    // poll (5s interval Ã— multiple WebSocket clients = many calls/second).
    ...
    // â€” deep + old + not draining â€” so a healthy catch-up stays 200 and only a
    // truly stuck queue is surfaced as degraded.
    let audio_degraded = false;
    ...
    // Read cached monitor list â€” updated every 5s by the monitor watcher.
    ...
    HealthCheckResponse {
        ...
        capture_status: CaptureStatusInfo {
            ...
            reason: "audio capture disabled".to_string(),
            audio_disabled: true,
            active_audio_devices: 0,
            paused_audio_devices: 0,
            pending_transcription_segments: None,
        ...
        ui_recorder: {
            ...
            // Only attach when start_ui_recording has touched the atomics â€”
        ...
        recording_coverage: {
            ...
            // Only attach once the sampler has observed any wall-clock time â€”
...
) -> JsonResponse<memora_screen::MetricsSnapshot> {
...
pub(crate) fn get_verbose_instructions(
    ...
    if unhealthy_systems.contains(&"vision") {
        ...
        // System Settings even when MEMORA had turned pixels off itself.
...
mod vision_stall_classification_tests {
    ...
    /// Both shipped messages were wrong â€” the overlay blamed the database and
    ...
    /// explanation once this classifier runs â€” the removed "idle user"
...
/// state, including the ones MEMORA itself caused. These pin the reason
...
mod vision_reason_tests {
    ...
    /// The reported case: permission is fine, MEMORA disabled screenshots
    ...
    fn config_disabled_screenshots_is_not_a_permission_problem() {
        ...
        assert!(instruction.contains("Settings â†’ Recording"));
    ...
    fn verbose_instructions_name_the_cause_instead_of_permission() {
        ...
        assert!(config.contains("Screenshots are turned off in MEMORA"));
...
mod tests {
    use super::*;
    use crate::recording_config::TranscriptionMode;
    ...
    fn transcription_mode_reports_configuration_not_activity() {
        ...
        assert_eq!(
            transcription_mode_label(Some(()), 0, 0),
            "disabled"
        );
        assert_eq!(
            transcription_mode_label(Some(()), 5, 3),
            "disabled"
        );
    }
    ...
    fn transcription_mode_falls_back_to_activity_when_contended() {
        assert_eq!(transcription_mode_label(None, 0, 0), "disabled");
        assert_eq!(transcription_mode_label(None, 1, 0), "disabled");
        assert_eq!(transcription_mode_label(None, 0, 1), "disabled");
    }

    #[test]
    fn disabled_transcription_engine_suppresses_backlog_health_query() {
        assert!(
            !should_query_audio_transcription_backlog(false),
            "audio transcription is disabled in Memora"
        );
        assert!(
            !should_query_audio_transcription_backlog(true),
    ...
    fn audio_backlog_stall_gate() {
        ...
        let progressing = 30; // a write landed 30s ago â€” sweep is draining
        ...
        // Same deep+old backlog but the sweep IS draining it (fresh write) â€”
        ...
        // Deep+old+not-draining but a live session is in flight â€” must NOT flag.
        ...
        // Small backlog within the freshness window â€” never a stall, even if no
        ...
        // Big count but young enough â€” not a stall yet.
        assert!(!audio_backlog_is_stalled(
            200,
            freshness,
            no_progress,
        ...
        // Never-written engine (last_db_write_ts == 0 â†’ u64::MAX age) with a
    ...
    fn audio_status_recovers_to_ok_after_timeout_goes_stale() {
        ...
        // (cumulative) check made a single past timeout stick forever â€” a
    ...
    /// input devices â€” e.g. RDP loopback "Remote Audio (output)") reports the
