...
use memora_db::{OcrTextBlock, TextPosition};
...
fn thumbnail_response(
    ...
    Response::builder()
        ...
        .header("x-MEMORA-thumbnail-cache", result.cache_status)
        .header("x-MEMORA-thumbnail-width", result.value.width)
        .header("x-MEMORA-thumbnail-height", result.value.height)
...
pub async fn get_frame_data(
    ...
    match timeout(Duration::from_secs(5), async {
        ...
        // (rfdetr â†’ black boxes, atomic overwrite) when "AI PII removal" is
        // on, so serving the file already serves the redacted pixels â€” no
                    ...
                    if let Some((file_path, timestamp)) = cache.get(&frame_id) {
                        ...
                        if timestamp.elapsed() < Duration::from_secs(1800) {
                            ...
                            match serve_file(&path_owned).await {
                                ...
                                Err(_) => {
                                    // File was deleted (e.g. by compaction) â€” evict stale entry
        ...
        match state.db.get_frame(frame_id).await {
            Ok(Some((file_path, offset_index, is_snapshot))) => {
                // Synced frame from another device â€” no local file exists.
                ...
                if is_snapshot {
                    // Snapshot frame â€” serve JPEG directly (no ffmpeg needed)
                    match serve_file(&file_path).await {
                        ...
                        Err(_) => {
                            // Snapshot file missing (compacted/deleted) â€” try nearest frame
                            debug!(
                ...
                // Legacy video-chunk frame â€” extract via ffmpeg
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        // Extraction failed â€” try the nearest valid frame as fallback
                        debug!(
                        ...
                        if err_str.contains("FFPROBE_NOT_FOUND") {
                            error!("ffprobe not found â€” frame extraction will fail for all compacted frames: {}", err_str);
                            Err((
                                StatusCode::INTERNAL_SERVER_ERROR,
                                JsonResponse(json!({
...
async fn try_nearest_frame(state: &Arc<AppState>, frame_id: i64) -> Option<Response<Body>> {
    ...
    // direction is enough â€” the bad frame is almost always at a chunk boundary
    // so the very next frame in the other chunk works.
    const SEARCH_LIMIT: i32 = 3;
    const MIN_VIDEO_SIZE: u64 = 1024; // 1KB â€” below this the file is certainly corrupt
    ...
    for (forward, dir_name) in directions {
        ...
        for (candidate_id, file_path, offset_index, _timestamp, is_snapshot) in candidates {
            if is_snapshot {
                // Snapshot frame â€” only serve regular files.
            ...
            // Legacy frame â€” skip non-files and tiny files before spawning ffmpeg.
...
/// Get frame metadata (timestamp) for deep link navigation. MEMORA://frame/123 â†’ resolve to timestamp.
#[oasgen]
...
pub struct AccessibilityNode {
    ...
    /// Automation properties (stable ID, class, state, etc.) â€” present for newer captures.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub async fn get_frame_text_data(
    ...
    // Merge accessibility tree link nodes â€” they have complete URLs with proper
    ...
    // Pure a11y fallback for frames with no OCR â€” use accessibility tree bounding boxes.
    ...
    if text_positions.is_empty() {
        if let Ok((_, Some(tree_json))) = state.db.get_frame_accessibility_data(frame_id).await {
            if let Some(query) = &params.query {
                ...
                text_positions = memora_db::find_matching_a11y_positions(&tree_json, query);
...
/// the closure this should never exceed 1 â€” a higher value means the permit
...
pub async fn run_frame_ocr(
    ...
    // Check if OCR data already exists â€” avoid redundant work
    ...
    let ocr_permit = memora_capture::ocr_semaphore()
        .acquire()
        .await
    ...
    let image = if is_snapshot {
        ...
        // Legacy video-chunk frame â€” extract via ffmpeg. The quality arg is
        // mjpeg's -q:v scale (2-31, LOWER is better) â€” "2" is best quality
    ...
    // task is not cancellable and runs to completion â€” releasing the permit on
    ...
    let ocr_result = tokio::task::spawn_blocking(move || {
        ...
        let (text, json, _confidence) = memora_screen::perform_ocr_apple(&image, &[]);
        (text, json)
    })
    .await
    ...
    let ocr_result = {
        ...
        let _permit = ocr_permit;
        let _watch = OcrInflightGuard::enter(frame_id);
        match memora_screen::perform_ocr_windows(&image, &[]).await {
    ...
    let ocr_result = tokio::task::spawn_blocking(move || {
        let _permit = ocr_permit;
        let _watch = OcrInflightGuard::enter(frame_id);
        let (text, json, _confidence) = memora_screen::perform_ocr_tesseract(&image, vec![]);
        (text, json)
    })
    .await
    .unwrap_or_else(|_| (String::new(), "[]".to_string()));
    ...
    let text_positions = memora_db::parse_all_text_positions(&blocks);
...
async fn persist_on_demand_ocr(
    ...
    #[cfg(target_os = "macos")]
    let engine = Arc::new(memora_db::OcrEngine::AppleNative);
    #[cfg(target_os = "windows")]
    let engine = Arc::new(memora_db::OcrEngine::WindowsNative);
    #[cfg(not(any(target_os = "macos", target_os = "windows")))]
    let engine = Arc::new(memora_db::OcrEngine::Tesseract);
    if let Err(e) = db
...
mod thumbnail_tests {
    ...
    ///
    /// Run with:
    /// MEMORA_BENCH_DB=/path/to/db.sqlite cargo test -p memora-engine
    ...
    async fn benchmark_real_frame_thumbnail_workload() {
        let source_db = PathBuf::from(
            std::env::var_os("MEMORA_BENCH_DB")
                .expect("set MEMORA_BENCH_DB to a local MEMORA db.sqlite"),
        );
