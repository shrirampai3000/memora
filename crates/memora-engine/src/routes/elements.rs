...
use memora_db::{Element, ElementSource};
...
pub(crate) struct ElementsQuery {
    ...
    /// that the user never saw â€” passing `on_screen=true` filters those
...
// ---------------------------------------------------------------------------
// `format=outline` â€” a compact, element-specific tree view
//
...
// confidence) are dropped â€” ask for `format=json` when you need them.
//
...
// 1â€“4, 6â€“7 above) are the target â€” structural noise to drop, repeated
// cells/rows to dedup, hierarchy, and state â€” a 6â€“75Ã— cut. OCR is the floor
// (flat text blocks: generic role, no hierarchy/state, nothing to dedup, so the
// outline â‰ˆ the text) â€” same caveat as CSV/TSV; for pure OCR `fields=text` is
// about as good.
// ---------------------------------------------------------------------------
...
/// identical `role`+`text` into `Ã—N`, hoists per-frame context into a header,
...
fn elements_outline_text(elements: &[ElementResponse], total: i64) -> String {
    ...
    'frames: for fid in &frame_order {
        ...
        out.push_str(&format!(
            "frame {fid} Â· {src} Â· {} text elements\n",
            els.len()
        ));
        ...
        while i < els.len() {
            ...
            // Collapse a run of identical (role, clipped-text) siblings â€” AX
            ...
            // Inline state â€” off-screen (issue #2436) + interaction flags parsed
    ...
    if truncated || (total as usize) > emitted {
        out.push_str(&format!(
            "â€¦ showing {emitted} of {total} elements â€” narrow with ?q=, ?role=, ?on_screen=true, or ?limit=\n"
        ));
    }
    out
}
...
// ---------------------------------------------------------------------------
// `format=automation` â€” targeting context for downstream automation agents
//
...
fn automation_context_text(elements: &[Element], total: i64) -> String {
    ...
    if candidates.len() > shown || total as usize > elements.len() {
        out.push_str(&format!(
            "â€¦ showing {shown} automation targets; narrow to one frame or app before acting\n"
        ));
    }
    out
}
...
mod tests {
    ...
    fn automation_el(
        ...
        Element {
            ...
            bounds: Some(memora_db::ElementBounds {
                left: 0.1,
                top: 0.2,
                width: 0.3,
                height: 0.4,
    ...
    fn parse_element_state_extracts_flags_and_inverts_enabled() {
        ...
        .unwrap();
        assert_eq!(s.disabled, Some(true)); // is_enabled:false â†’ disabled
        assert_eq!(s.selected, Some(true));
        assert_eq!(s.focused, None);
        // value/placeholder only â†’ no interaction state
    ...
    fn outline_collapses_identical_runs_into_count() {
        ...
        assert!(out.contains("AXCell \"Active\" #1 Ã—3"), "got:\n{out}");
    ...
    fn outline_groups_by_frame_with_headers_in_first_seen_order() {
        ...
        assert!(
            out.contains("frame 100 Â· accessibility Â· 1 text elements"),
            "got:\n{out}"
        );
        assert!(out.contains("frame 200 Â· accessibility Â· 1 text elements"));
    ...
    fn outline_is_char_safe_with_unicode_and_clips_by_char() {
        ...
        // â€¦ marker must appear without slicing a code point.
        let emoji = "ðŸŽ‰".repeat(OUTLINE_TEXT_CLIP + 30);
        let els = vec![
            el(
                1,
                9,
                "AXStaticText",
                Some("cafÃ© â€” naÃ¯ve æ—¥æœ¬èªž ðŸ˜€"),
                1,
                None,
            ),
    ...
    fn outline_caps_indent_for_deeply_nested() {
        ...
        assert_eq!(leading, 12, "indent should cap at 6 levels Ã— 2 spaces"); // not 60
    }

    #[test]
    fn outline_dedup_is_adjacent_only_preserving_structure() {
        // A A B A â€” the trailing A must NOT merge with the leading run.
        let els = vec![
        ...
        let out = elements_outline_text(&els, 4);
        assert!(out.contains("AXTab \"A\" #1 Ã—2"), "got:\n{out}");
        assert!(out.contains("AXTab \"B\" #3"));
        assert!(out.contains("AXTab \"A\" #4"));
        assert!(!out.contains("#4 Ã—"));
    }
    ...
    fn outline_buckets_interleaved_frames() {
        ...
        let out = elements_outline_text(&els, 3);
        assert!(
            out.contains("frame 100 Â· accessibility Â· 2 text elements"),
            "got:\n{out}"
        );
        assert!(out.contains("frame 200 Â· accessibility Â· 1 text elements"));
    ...
    //   cargo test -p memora-engine dump_token_samples -- --ignored
    ...
    // Measured (o200k_base): 91% fewer tokens aggregate (11Ã—); per shape 67%
    // (OCR text-heavy) â€¦ 99% (tabular dedup). #[ignore] so CI never does the IO.
    #[test]
    #[ignore]
