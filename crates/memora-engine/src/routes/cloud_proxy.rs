...
//! `unset MEMORA_API_KEY` (see memora-core::agents::bash_env) so the
//! agent never sees the cloud JWT. Agent-driven media analysis goes through
//! this route instead: agent â†’ localhost:3030/v1/chat/completions (auth'd
//! with the local sp-key the wrapper auto-injects) â†’ server forwards to
//! api.memora.com with the user's cloud JWT held in AppState.
...
pub async fn chat_completions(
    ...
    let Some(token) = (**token).clone().filter(|t| !t.is_empty()) else {
        return (
            ...
            r#"{"error":"cloud_token_missing","message":"sign in to MEMORA to use cloud media analysis"}"#,
        )
            .into_response();
    };
