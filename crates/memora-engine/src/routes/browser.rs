...
//! [`memora_connect::connections::browser`]; this file is a thin shell
...
use memora_connect::connections::browser::{
...
pub use memora_connect::connections::browser::BrowserBridge;

// ---------------------------------------------------------------------------
// Tunables â€” keep both halves in lockstep with the extension's heartbeat.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Request / response types â€” engine-facing JSON shapes.
// ---------------------------------------------------------------------------
...
pub struct EvalRequestBody {
    ...
    pub code: String,
    /// Optional URL pattern â€” execute in a tab whose URL contains this string
...
// ---------------------------------------------------------------------------
// Transport adapter â€” wraps the axum WS sink so the bridge can write to it
...
/// GET /browser/status â€” check if the extension is connected
...
/// POST /browser/eval â€” execute JS in the browser via the extension
...
/// GET /browser/ws â€” WebSocket endpoint for the extension
...
async fn handle_extension_socket(
    ...
    // pending requests are now stale â€” cancel them so callers don't hang.
    ...
    let ping_task = lifecycle.spawn(async move {
        ...
        loop {
            ...
            // text-only by design â€” sending an actual WS Ping frame would
            // require leaking axum types into memora-connect.
            if ping_transport
                .send_text(r#"{"type":"ping"}"#.to_string())
                .await
                .is_err()
            {
                debug!("browser ws ping failed â€” peer is gone");
                break;
            }
        }
    });
    ...
    loop {
        let next = tokio::select! {
            ...
            result = tokio::time::timeout(READ_IDLE_TIMEOUT, stream.next()) => {
                match result {
                    ...
                    Err(_) => {
                        warn!(
                            "browser ws idle for {}s â€” assuming peer is dead",
        ...
        match parsed {
            // Eval response â€” has both `id` and `ok` fields
            ...
            // Other messages (hello, pong) â€” log and continue
            _ => {
