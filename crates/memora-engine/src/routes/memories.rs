...
fn memory_to_response(m: memora_db::MemoryRecord) -> MemoryResponse {
    MemoryResponse {
        id: m.id,
        content: m.content,
        source: m.source,
...
pub(crate) async fn delete_memory_handler(
    ...
    // tombstone. Reads should never block the delete on failure â€” a
...
///
///   * `None` / `Value::Null` â†’ `{"_device":"<machine_id>"}` (no original
///     context to merge with).
///   * `Value::Object(map)` â†’ original keys preserved, `_device` added.
///     If the caller already provided a `_device` key (rare â€” typically
///     pipes don't), we overwrite it: trust the local server, not the
///     client.
///   * Anything else (string, number, array, bool) â†’ wrap as
...
/// a stable id, which in practice never happens â€” the helper falls
...
fn enrich_source_context_with_device(ctx: Option<Value>) -> Option<Value> {
    let machine_id = memora_core::sync::get_or_create_machine_id();
...
mod tests {
    ...
    fn enrich_overwrites_caller_provided_device() {
        // A malicious or buggy caller can't impersonate another machine â€”
