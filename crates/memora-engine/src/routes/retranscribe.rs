...
use MEMORA_audio::core::engine::AudioTranscriptionEngine;
use MEMORA_audio::transcription::engine::TranscriptionEngine;
use MEMORA_audio::transcription::VocabularyEntry;
use memora_db::{AudioChunkInfo, NewMeetingTranscriptSegment};
...
pub struct RetranscribeRequest {
    /// Explicit chunk IDs to retranscribe (preferred â€” avoids timestamp mismatches)
...
pub struct MeetingRetranscribeRequest {
    /// Optional engine override: "deepgram", "MEMORA-cloud", "parakeet", etc.
    pub engine: Option<String>,
    /// Custom vocabulary for this re-transcription.
...
pub async fn retranscribe_handler(
    ...
    // 1. Query audio chunks â€” by explicit IDs (preferred) or time range (fallback)
    ...
    for chunk in &chunks {
        ...
        if !file_path.exists() {
            info!(
                "skipping chunk {} â€” file not found: {}",
        ...
        let (samples, sample_rate) =
            match MEMORA_audio::utils::ffmpeg::read_audio_from_file(file_path) {
                Ok(result) => result,
                Err(e) => {
        ...
        // Update DB â€” replace all existing transcription rows for this chunk
...
pub async fn retranscribe_meeting_handler(
    ...
    for batch in batches {
        ...
        for chunk in batch {
            ...
            match tokio::task::spawn_blocking(move || {
                MEMORA_audio::utils::ffmpeg::read_audio_from_file(StdPath::new(&path_owned))
            })
            .await
            {
                Ok(Ok((samples, sr))) => {
    ...
    let summary_refresh_event_emitted = match memora_events::send_event(
