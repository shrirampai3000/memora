...
use memora_db::{MeetingRecord, MeetingTranscriptSegment, MEETING_END_REASON_EXPLICIT_STOP};
...
pub fn emit_meeting_status_changed(status: &MeetingStatusResponse) {
    ...
    if let Err(e) = memora_events::send_event("meeting_status_changed", status.clone()) {
...
pub(crate) async fn start_meeting_handler(
    ...
    // from being inserted while one already exists â€” the historical bug that
    ...
    let id = if let Some(id) = body.id {
        ...
        //   1) The active meeting is already the manual one â€” idempotent
        ...
        //      user's mental model â€” "start meeting" on a call that's
        ...
        state
            ...
            .map_err(|e| {
                ...
                // keeps working if columns change â€” `meetings.meeting_end`
...
pub(crate) async fn stop_meeting_handler(
    ...
    // intent â€” the audio batch-deferral flag and detector tracking must release
    ...
    // never clears â€” leaving the meeting permanently unstoppable with hundreds
    ...
    // even if this times out (pool wedged), the deferral drains and a retry â€”
    // or the detector's own grace-timeout end â€” completes persistence.
...
/// `meeting_id` XOR `start`/`end` contract as the `MEMORA export` CLI and the
...
/// the `MEMORA export` CLI â€” same `meeting_id` XOR `start`/`end` contract.
#[oasgen]
pub(crate) async fn export_handler(
    ...
    // meeting_id XOR start/end, same contract as the `MEMORA export` CLI.
