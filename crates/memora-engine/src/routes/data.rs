...
pub(crate) async fn delete_time_range_handler(
    ...
    // mp4/jpeg files we just unlinked â€” which made the timeline "jump
...
/// POST /data/evict-media â€” reclaim mp4/wav/jpeg files in a time range
...
/// GET /data/storage-preview â€” estimate disk reclaimable by media eviction
...
// â”€â”€ Database checkpoint & backup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/// MEMORA is running â€” the backup is a consistent snapshot.
#[oasgen]
...
/// POST /data/compact â€” rebuild the database with a full `VACUUM` to return
...
pub(crate) async fn compact_handler(
    ...
    let bytes_before: u64 = database_files
        .iter()
        .map(|file_name| size_of(&state.MEMORA_dir.join(file_name)))
        .sum();
    let required_free_space = compact_required_free_space(bytes_before);
    let available_free_space = available_space_for_path(&state.MEMORA_dir);
