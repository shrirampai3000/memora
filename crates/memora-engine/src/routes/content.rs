...
use memora_db::{DeviceType, Speaker, TagAutocompleteItem, TagContentType};
...
pub struct OCRContent {
    ...
    /// â€” read this field to know which path produced the text.
    ...
    if let Some(ocr_results) = &frame.ocr_results {
        for ocr in ocr_results {
            db.insert_ocr_text(
                ...
                Arc::new(OcrEngine::default().into()), // Ideally could pass any str as ocr_engine since can be run outside of MEMORA
            )
            .await?;
        }
    }
    ...
    match payload.content.content_type.as_str() {
        "frames" => {
            if let ContentData::Frames(frames) = &payload.content.data {
                if !frames.is_empty() {
                    let output_dir = state.MEMORA_dir.join("data");
                    let time = Utc::now();
...
/// as a real token â€” not as a substring of an identifier such as `ACCOUNT(`.
...
fn validate_raw_sql(query: &str) -> Result<(), String> {
    ...
    // Only allow read-only queries â€” block writes via /raw_sql
    ...
    // EXPLAIN queries and single-row aggregations (COUNT/MAX/â€¦ with no GROUP BY)
...
pub(crate) async fn execute_raw_sql(
    ...
    match state.db.query_raw_sql(&payload.query).await {
        ...
        // A database-level error means SQLite rejected the *query* itself â€”
        ...
        // 500 for genuine infrastructure failures (pool timeout, IO, â€¦).
...
pub(crate) async fn merge_frames_handler(
    ...
    let output_dir = state.MEMORA_dir.join("videos");
...
mod raw_sql_validation_tests {
    ...
    fn still_requires_limit_for_group_by() {
        // GROUP BY can return many rows â€” keep requiring a LIMIT there.
        assert!(
