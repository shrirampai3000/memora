...
//!
//! This is the default broad-context call for chat agents â€” preferred over
...
pub struct ActivitySummaryQuery {
    ...
    /// unaffected â€” they still sample screen text internally.
    #[serde(default = "default_true")]
...
pub struct ActivitySummaryResponse {
    ...
    /// Authoritative total active screen time (minutes) over the WHOLE range â€”
...
/// heaviest field) â€” each omits its field from the response when disabled.
#[oasgen]
pub async fn get_activity_summary(
    ...
    // Run optional sidecars in parallel â€” each is best-effort; failures
...
async fn collect_summary_core(
    ...
    // Cap at 50 paths â€” a 1000-file workspace would be noise anyway.
...
fn next_best_query(
    ...
    match data_status {
        ...
        "not_recording" => Some(
            "no local MEMORA captures exist yet. check /health or start recording before concluding the user was inactive.".to_string(),
        ),
...
/// â€” this is the canonical definition of "active time" the SQL `minutes`
...
mod tests {
    ...
    fn truncate_text_unicode_safe() {
        // Multibyte chars â€” slicing by bytes would panic; chars() doesn't.
        let s = "ðŸŽ‰".repeat(100);
...
mod db_tests {
    ...
    /// Real MEMORA timestamps are RFC3339 with microseconds and a `+00:00`
...
mod include_flag_tests {
    ...
    /// Parse a query string through axum's real `Query` extractor â€” the exact
    /// path the live handler uses â€” so we test deserialization, not a hand-built
    /// struct.
