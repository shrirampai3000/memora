...
//! Codex, â€¦) run behind an MCP server that talks only to localhost:3030 and
//! never holds the cloud JWT (it is scrubbed from third-party adapter process
//! trees). A web search therefore goes agent â†’ MCP tool â†’
//! localhost:3030/v1/web-search (auth'd with the local sp-key) â†’ this route
//! forwards to api.memora.com with the user's cloud JWT held in AppState.
...
const CLOUD_BASE_URL: &str = "https://api.memora.com";

/// Shared, timeout-bounded client. Without an explicit timeout reqwest waits
/// forever, so a stalled/half-open upstream would hang the agent's tool call â€”
/// and thus its whole turn â€” with no recovery. Built once and reused.
    ...
    let Some(token) = (**token).clone().filter(|t| !t.is_empty()) else {
        return (
            ...
            Json(json!({
                ...
                "message": "sign in to MEMORA to use web search",
            })),
        )
            .into_response();
    };
