...
//! 1512x949 snapshots at matched quality it loses on every axis â€” 2.25 CPU-s
...
//!
//! Escape hatch: `MEMORA_COMPACTION_ENCODER=software` forces libx264;
...
impl CompactionEncoder {
    ...
    /// selection demotes to software for the rest of the process lifetime â€”
    ...
    pub fn record_encode_outcome(self, success: bool) {
        ...
        if failures >= MAX_HW_ENCODE_FAILURES && !HW_DEMOTED.swap(true, Ordering::Relaxed) {
            warn!(
                "hardware compaction encoder {} failed {} consecutive encodes â€” \
...
async fn select_uncached(ffmpeg: &Path) -> CompactionEncoder {
    ...
    if let Ok(v) = std::env::var("MEMORA_COMPACTION_ENCODER") {
        match CompactionEncoder::from_env_name(v.trim()) {
            Some(CompactionEncoder::X264) => {
                debug!("MEMORA_COMPACTION_ENCODER={v}: forcing software encode");
                return CompactionEncoder::X264;
            }
            Some(enc) => {
                debug!("MEMORA_COMPACTION_ENCODER={v}: probing it first");
                candidates.push(enc);
            }
            None => {
                warn!("MEMORA_COMPACTION_ENCODER={v} not recognized; using default order");
            }
        }
    }
...
async fn probe_encoder(ffmpeg: &Path, enc: CompactionEncoder) -> bool {
    let mut cmd = memora_core::ffmpeg_cmd_async(PathBuf::from(ffmpeg));
    cmd.args([
        "-hide_banner",
        "-loglevel",
        "error",
...
mod tests {
    ...
    fn hardware_demotes_after_consecutive_failures() {
        ...
        // A success resets the streak â€” transient blips must not demote.
    ...
    async fn unspawnable_ffmpeg_falls_back_to_software() {
        let enc = select_from_candidates(
            Path::new("C:/MEMORA-does-not-exist/ffmpeg.exe"),
