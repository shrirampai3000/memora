...
/// Don't bother spawning an encoder for fewer frames than this â€” tiny chunks
/// compress poorly (a chunk is mostly its keyframe) and litter the data dir
/// with sub-100KB MP4s. Small batches are deferred until they growâ€¦
const MIN_FRAMES_PER_CHUNK: usize = 25;

/// â€¦unless the oldest frame has waited this long. Caps how long a JPEG can
...
pub fn start_snapshot_compaction(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if let Some(reason) = current_pause_reason(&power_manager).await {
                debug!("snapshot compaction: skipping â€” {}", reason.as_str());
                tokio::select! {
            ...
            let chunk_size = match state.thermal_state {
                ...
                ThermalState::Serious | ThermalState::Critical => {
                    debug!(
                        "snapshot compaction: throttled â€” thermal state {:?}",
                        state.thermal_state
                    );
                ...
                result = run_compaction_cycle(
                    ...
                    match result {
                        ...
                        Ok(CompactionCycleOutcome::Paused(reason)) => {
                            debug!("snapshot compaction: paused â€” {}", reason.as_str());
                            0
                        }
                        Err(e) => {
...
async fn compact_chunk(
    ...
    // the log line). Header-only read â€” ffmpeg does the actual decoding, so
    ...
    let (frame_w, frame_h) = {
        ...
        match dims {
            Some(d) => d,
            None => {
                // All JPEGs gone/unreadable â€” clear stale DB pointers so we don't retry
    ...
    // Encode JPEGs â†’ MP4 via ffmpeg (low-priority, capped threads, JPEG passthrough)
    ...
    for (frame_id, snapshot_path, _) in frames {
        ...
        match read_result {
            Ok(jpeg_bytes) => {
                ...
                video_position += 1;
                // Pace writes to avoid CPU spikes â€” spread encoding over time
    ...
    if let Some(error) = write_error {
        return Err(anyhow::anyhow!(
            "failed while feeding snapshots to ffmpeg: {error} â€” keeping source JPEGs for retry"
        ));
    }
    if !encode_ok {
        return Err(anyhow::anyhow!(
            "ffmpeg ({}) exited with failure â€” keeping source JPEGs for retry",
            encoder.codec_name()
        ));
    }
    ...
    // Step 3: Delete source JPEGs â€” only for frames actually encoded into
    ...
    info!(
        "snapshot compaction: {} frames, {:.1}MB â†’ {:.1}MB ({:.1}x), {} JPEGs deleted",
        encoded_frames.len(),
        jpeg_total_bytes as f64 / 1_048_576.0,
...
async fn start_ffmpeg_lowpri(
    ...
    let ffmpeg_path =
        memora_core::find_ffmpeg_path().ok_or_else(|| anyhow::anyhow!("ffmpeg not found"))?;
    ...
    #[cfg(not(unix))]
    let mut command = memora_core::ffmpeg_cmd_async(ffmpeg_path);
...
mod tests {
    ...
    fn test_calculate_fps_ten_frames_ten_seconds() {
        let frames: Vec<_> = (0..10)
            ...
            .collect();
        // 10 frames / 9 seconds â‰ˆ 1.11 fps
        let fps = calculate_fps(&frames);
    ...
    fn test_calculate_fps_clamped_high() {
        // 100 frames in 1 second â†’ 100 fps, should clamp to 30.0
    ...
    fn test_calculate_fps_clamped_low() {
        // 2 frames over 1000 seconds â†’ 0.002 fps, should clamp to 0.1
        let frames = vec![
            make_frame(1, "/tmp/a.jpg", "2025-01-01T00:00:00Z"),
