...
//! with automatic cleanup â€” preventing unbounded log growth that time-based
...
//!
//! e.g. `MEMORA.2026-02-17.0.log`, `MEMORA.2026-02-17.1.log`
//!
//! # Example
//!
//! ```no_run
//! use memora_engine::logging::SizedRollingWriter;
//!
...
impl Write for SizedRollingWriter {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        // 1. Date rotation (cheap SystemTime comparison â€” no allocation).
...
impl SizedRollingWriter {
    ...
    fn cleanup(&mut self) {
        ...
        // Remove the active file from the candidate list â€” never delete it.
...
mod tests {
    ...
    fn rotates_on_size_limit() {
        ...
        // Write 30 more â€” pushes over 50, triggers rotation
        w.write_all(&[b'B'; 30]).unwrap();
        assert_eq!(count_log_files(tmp.path()), 2);

        // Write 30 more into second file â€” pushes it over 50 too
    ...
    fn cleanup_deletes_oldest_files() {
        ...
        // Write 6 files worth of data â€” well over the cap.
        for _ in 0..6 {
        ...
        // At least some old files must have been deleted â€” we wrote 6 files
    ...
    fn resumes_from_existing_files() {
        ...
        // Build writer â€” should resume at index 1 (latest for today).
    ...
    #[test]
    fn filename_format() {
        let name = SizedRollingWriter::filename("MEMORA", "log", "2026-02-17", 3);
        assert_eq!(name, "MEMORA.2026-02-17.3.log");
    }
    ...
    fn works_with_fresh_directory() {
        ...
        // Directory doesn't exist yet â€” builder should create it.
    ...
    fn many_rotations_stay_under_cap() {
        ...
        // Write 50 chunks of 25 bytes each â†’ 50 rotations.
        for _ in 0..50 {
