...
/// automation can set `MEMORA_DISABLE_TELEMETRY` to opt out explicitly.
/// This is the single chokepoint that keeps CI runs â€” the `MEMORA` CLI
/// (e2e-macos, npm-e2e) and the desktop-app e2e suite (which embeds the engine
/// in-process and calls [`init`] on every server start) â€” from inflating
...
pub async fn capture_event(event: &str, properties: Value) {
    ...
    if let Some(obj) = props.as_object_mut() {
        ...
        obj.insert("$lib".to_string(), json!("memora-engine"));
...
mod tests {
    ...
    const VARS: &[&str] = &["MEMORA_DISABLE_TELEMETRY", "GITHUB_ACTIONS", "CI"];
    ...
    fn telemetry_disabled_by_explicit_optout() {
        with_env(&[("MEMORA_DISABLE_TELEMETRY", "1")], || {
            assert!(telemetry_disabled_by_env())
        });
    }
