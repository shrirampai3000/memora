...
//! Focus-aware capture controller â€” maintains per-monitor state (Active /
...
//!   â”‚
//!   â–¼
//! â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”  lost focus     â”Œâ”€â”€â”€â”€â”€â”€â”  cold_cutoff   â”Œâ”€â”€â”€â”€â”€â”€â”
//! â”‚ Active â”‚â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–ºâ”‚ Warm â”‚â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–ºâ”‚ Cold â”‚
//! â””â”€â”€â”€â”€â”€â”€â”€â”€â”˜  < warm_cutoff  â””â”€â”€â”€â”€â”€â”€â”˜                â””â”€â”€â”€â”€â”€â”€â”˜
//!   â–²              â”‚                                   â”‚
//!   â”‚ focus returnsâ”‚                                   â”‚
...
//! and skip OCR/DB work â€” this keeps the SCK stream alive (no WindowServer
...
/// `current_focus` as if it were `Unknown` â€” this guards against a silent
...
impl FocusAwareController {
    ...
    fn spawn_subscriber(self: &Arc<Self>) {
        ...
        // subscriber â€” the controller will still work in "all Active"
        // fallback (state() returns Active when current_focus is None).
        let Ok(handle) = tokio::runtime::Handle::try_current() else {
            warn!("no tokio runtime â€” focus-aware subscriber task skipped");
            return;
        };
            ...
            loop {
                ...
                match rx.recv().await {
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                        debug!("focus-aware subscriber lagged by {} events â€” continuing", n);
                    }
    ...
    /// Query state for a monitor. Must be cheap â€” called on every capture
    /// loop iteration.
    ...
    fn hosts_focus_for_identity(&self, identity: &MonitorIdentity) -> bool {
        ...
        // the AX walk from every capture â€” frames kept flowing but with OCR
        ...
        // display gap, platform tracker wedged) this stays fail-closed â€”
        // "unknown monitor â†’ OCR + unknown identity" is the deliberate
    ...
    fn state_for_identity(&self, identity: &MonitorIdentity) -> CaptureState {
        ...
        // If focus is Unknown (no data yet), everything is Active â€” safest
        ...
        match last {
            // Never focused since controller start â†’ Cold. The loop will block
...
mod tests {
    ...
    /// focus stopped *changing* â€” the normal single-monitor condition.
    ...
    async fn focused_monitor_is_active_and_never_focused_is_cold() {
        ...
        assert_eq!(ctrl.state(1), CaptureState::Active);
        assert!(ctrl.hosts_focus(1));
        // Monitor 2 has never been focused â€” should be Cold immediately.
    ...
    async fn warm_window_hysteresis() {
        ...
        // warm-cutoff hysteresis window â†’ Active.
    ...
    async fn stale_focus_falls_back_to_active() {
        ...
        assert_eq!(ctrl.state(2), CaptureState::Cold);
        // Backdate the last event past the stale cutoff â€” simulates a
    ...
    /// 30 quiet seconds `last_event_time` is stale â€” but the tracker itself
    ...
    async fn hosts_focus_survives_quiet_event_stream_via_tracker_pull() {
        ...
        // 60 quiet seconds â€” no focus change, so no events. Not a fault.
        ctrl.backdate_last_event_for_test(Instant::now() - Duration::from_secs(60));
