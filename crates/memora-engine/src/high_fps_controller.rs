...
//! ## Design
//!
//! Every HD session is *bound* to a natural end condition â€” there is no
//! indefinite "stay on until I remember to turn it off" mode. The two
//! supported bindings are:
//!
//! - [`SessionKind::Meeting`] â€” tied to a specific `meeting_id`. The
...
//! - [`SessionKind::Timer`] â€” fires for a user-chosen duration (15 min to
...
//!
//! - `Ask` (default) â€” the existing meeting-started notification grows a
//!   "+ HD" action. Click â†’ starts a meeting-bound session.
//! - `Always` â€” every detected meeting auto-starts a session.
//! - `Never` â€” no auto-action; only the manual tray timer can start one.
//!
//! ## Persistence
//!
...
//! Active sessions are NOT persisted â€” they're explicitly ephemeral, the
...
/// 30 fps ceiling â€” anything tighter buries the snapshot compactor in JPEGs
/// without meaningfully helping human-eye replay.
pub const MIN_INTERVAL_MS: u64 = 33;

/// Default interval (10 fps) â€” smooth enough for slide flips and demo
...
pub enum SessionKind {
    ...
    /// has actually started â€” no `meeting_id` exists yet). Recording fires
    ...
    /// arrives â€” the user probably forgot they clicked, or the call was
    /// cancelled.
    PrewarmPending,
}
...
/// Public snapshot returned to readers â€” exposes the live state plus the
...
pub struct HighFpsController {
    ...
    /// Event-tracked meeting state. Set from `meeting_started`/`meeting_ended` via
...
impl HighFpsController {
    ...
    /// its deadline has passed â€” putting this in the read path means the
    ...
    pub fn snapshot(&self) -> HighFpsSnapshot {
        ...
        let meeting = Some(self.event_in_meeting.load(Ordering::Relaxed));

        let mut guard = self
            .inner
            .lock()
            .expect("HighFpsController mutex poisoned â€” fatal");
    ...
    pub fn start_meeting_session(&self, meeting_id: i64) -> HighFpsSnapshot {
        ...
        // timer â€” so HD stopped 30 min into any longer call instead of staying
        ...
        let in_meeting = self.event_in_meeting.load(Ordering::Relaxed);
    ...
    /// Start a `PrewarmPending` session â€” used by the prewarm notification's
    ...
    pub fn start_prewarm_pending_session(&self) -> HighFpsSnapshot {
        ...
        *guard = Some(Session {
        ...
        tracing::info!(
            target: "capture::high_fps",
            "HD session started (prewarm-pending â€” awaiting meeting_started)",
        );
        self.snapshot()
    }
            ...
            if matches!(s.kind, SessionKind::PrewarmPending) {
                *guard = Some(Session {
                ...
                tracing::info!(
                    ...
                    meeting_id,
                    elapsed_secs = (now - s.started_at).as_secs(),
                    "HD session upgraded (prewarm-pending â†’ meeting-bound)",
                );
            }
        }
    }
    ...
    /// alone â€” the user may have started a timer mid-call and shouldn't
    ...
    /// session), so this is what makes `snapshot().meeting` meaningful there â€”
...
mod tests {
    ...
    fn meeting_session_active_until_meeting_ends() {
        ...
        // Different meeting id ending â€” should NOT stop our session.
        c.handle_meeting_ended(99);
        assert!(c.snapshot().active);

        // Matching meeting id ending â€” should stop.
    ...
    fn seed_interval_is_clamped() {
        let c = Arc::new(HighFpsController::new(DefaultMode::Ask, 0));
        assert_eq!(c.snapshot().interval_ms, MIN_INTERVAL_MS);
    }
    ...
    fn app_meeting_session_is_meeting_bound_via_event_flag() {
        let c = Arc::new(HighFpsController::new(DefaultMode::Always, 100));
        c.set_in_meeting(true);
    ...
    fn no_detector_works_for_timer_sessions() {
        ...
        c.start_timer_session(Duration::from_secs(600));
        assert!(c.snapshot().active);
        // No detector + no event signal yet â†’ not in a meeting (Some(false),
    ...
    fn event_in_meeting_drives_meeting_without_detector() {
        ...
        let c = Arc::new(HighFpsController::new(DefaultMode::Ask, 100));
        assert_eq!(c.snapshot().meeting, Some(false));
        c.set_in_meeting(true);
