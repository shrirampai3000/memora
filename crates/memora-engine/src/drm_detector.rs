...
//! DRM and content-protection detection â€” pauses screen capture when an app
...
//!
//! Simply skipping captures isn't enough â€” we must fully release all SCK
...
/// Global flag â€” when `true`, all monitors skip screen capture.
...
pub fn set_drm_paused(paused: bool) {
    ...
    if paused && !was_paused {
        info!("DRM content detected â€” pausing screen capture");
    } else if !paused && was_paused {
        info!("DRM content no longer focused â€” resuming screen capture");
    }
}
...
pub fn is_drm_app(app_name: &str) -> bool {
    ...
    for &drm in DRM_APPS {
        // "max" and "tv" are short, generic names â€” match them exactly so we
...
/// (e.g. "Netflix - Comet" â†’ synthesizes "https://netflix.com").
#[cfg(target_os = "macos")]
...
fn get_drm_url_from_window_title(app: &cidre::ax::UiElement) -> Option<String> {
    ...
    // e.g. "disneyplus.com" â†’ base "disneyplus" won't match title "Disney+".
    ...
    for &(keyword, domain) in TITLE_ALIASES {
        if title_lower.contains(keyword) {
            debug!(
                "DRM URL from window title alias: title='{}', matched keyword='{}' â†’ domain={}",
    ...
    // "netflix.com" â†’ check for "netflix", "disneyplus.com" â†’ "disneyplus", etc.
    for &domain in DRM_DOMAINS {
...
/// Pre-capture DRM gate â€” called BEFORE any ScreenCaptureKit call.
///
...
/// queried via AX APIs.
///
/// Does NOT clear the DRM pause â€” that is `poll_drm_clear()`'s job.
#[cfg(target_os = "macos")]
pub fn pre_capture_drm_check(pause_on_drm_content: bool, trigger_app_name: Option<&str>) -> bool {
    ...
    // Already paused â€” stay paused, no need to re-query
    if drm_content_paused() {
        return true;
    }
    ...
    let result = std::panic::catch_unwind(|| -> bool {
        ...
        if let Some(app) = trigger_app_name {
            if is_drm_app(app) {
                info!("pre-capture DRM check: native DRM app '{}' â€” blocking", app);
                set_drm_paused(true);
                return true;
            }
            if is_browser(app) {
                // Browser switch â€” need the URL to decide
                match get_focused_app_info() {
                    Some((ref name, ref url)) => {
                        ...
                        if let Some(ref u) = url {
                            if is_drm_url(u) {
                                info!(
                                    "pre-capture DRM check: browser '{}' on DRM URL {} â€” blocking",
                                    app, u
                                );
                                set_drm_paused(true);
                                return true;
        ...
        // No trigger app name (Idle, Click, etc.) â€” query the focused app
        if let Some((app_name, url)) = get_focused_app_info() {
            if is_drm_content(&app_name, url.as_deref()) {
                info!(
                    "pre-capture DRM check: focused app='{}', url={:?} â€” blocking",
                    app_name, url
                );
                set_drm_paused(true);
                return true;
            ...
            debug!(
                "pre-capture DRM check: focused app='{}', url={:?} â€” no DRM",
                app_name, url
            );
        } else {
...
///
/// Uses `CGWindowListCopyWindowInfo` (CoreGraphics) â€” does NOT touch ScreenCaptureKit.
        ...
        Ok(Some((app_name, url))) => {
            ...
            if is_browser(&app_name) {
                if let Some(ref u) = url {
                    if is_drm_url(u) {
                        debug!(
                            "browser '{}' still on DRM URL: {} â€” keeping pause",
                            app_name, u
                        );
                        return true;
                    }
            ...
            // Focused app is not DRM â€” but a DRM window may still be visible
            ...
            if any_drm_window_on_screen() {
                debug!(
                    "focused app '{}' is not DRM, but a DRM window is still visible â€” keeping pause",
                    app_name
                );
                return true;
            }
            ...
            info!(
                "focused app '{}' is not DRM and no DRM windows visible â€” clearing pause",
                app_name
            );
            set_drm_paused(false);
            false
...
mod tests {
    ...
    fn test_is_drm_app_negative() {
        ...
        // "tv" is matched exactly â€” apps merely containing "tv" must not be flagged.
    ...
    fn test_is_drm_url_negative() {
        ...
        // subdomain anchor â€” must not match lookalike domains
    ...
    // â”€â”€ pre_capture_drm_check unit tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ live integration tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    ///
    /// Run: `cargo test -p memora-engine --lib -- test_pre_capture_blocks_netflix_in_comet --ignored`
    #[test]
    #[ignore]
    #[cfg(target_os = "macos")]
    fn test_pre_capture_blocks_netflix_in_comet() {
        ...
        let _ = Command::new("open")
            ...
            .spawn()
            .expect("failed to open Comet â€” is it installed?")
            .wait();
        ...
        if has_ax {
            ...
            println!("No direct AX â€” verifying via osascript fallback");
            ...
            println!("âœ“ Window title contains Netflix â€” DRM detection would fire");

            // Also verify the pure logic: if we had the URL, would it be detected?
            assert!(is_drm_content("Comet", Some("https://netflix.com")));
            println!("âœ“ is_drm_content('Comet', 'https://netflix.com') = true");
        }
    ...
    // â”€â”€ check_and_update_drm_state tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
