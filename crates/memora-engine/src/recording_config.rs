...
use memora_config::{ChannelConfig, DbConfig, SemanticContextMode};
...
pub struct RecordingConfig {
    ...
    /// Opt in to the bounded semantic projection worker. False preserves the
    /// historical capture and database behavior.
    ...
    /// downloads ~108 MB from huggingface.co/MEMORA/pii-image-redactor.
    pub async_image_pii_redaction: bool,
    /// "local" or "tinfoil" â€” flips both async-PII workers between
    /// on-device ONNX and the MEMORA-hosted Tinfoil enclave.
    ...
    /// rationale â€” SCO/A2DP downgrade, issue #3750).
    ...
    /// Fallback chain: settings userName â†’ cloud auth name â†’ cloud auth email.
    ...
    /// `Authorization: Bearer <MEMORA_API_KEY>`.
    pub api_auth: bool,

    /// The API key for this instance (from MEMORA_API_KEY env or auth.json).
...
impl RecordingConfig {
    ...
    pub fn from_settings(
        ...
        memora_config::set_record_while_locked(settings.record_while_locked);
        let (MEMORA_aec_enabled, windows_input_aec_enabled, macos_input_vpio_enabled) =
        ...
        Self {
            ...
            audio_transcription_engine: AudioTranscriptionEngine::Disabled,
            transcription_mode: TranscriptionMode::Batch,
            audio_capture_mode: AudioCaptureMode::Disabled,
            ...
            listen_address: if settings.listen_on_lan {
                std::net::Ipv4Addr::UNSPECIFIED // 0.0.0.0 â€” all interfaces
            } else {
...
mod tests {
    ...
    fn settings_with(lan: bool, api_auth: bool) -> memora_config::RecordingSettings {
    ...
    fn build(s: &memora_config::RecordingSettings) -> RecordingConfig {
        RecordingConfig::from_settings(s, std::path::PathBuf::from("/tmp/sp_test"), None)
    }

    #[test]
    fn defaults_to_loopback() {
        let c = build(&memora_config::RecordingSettings::default());
    ...
    fn semantic_context_setting_is_preserved_when_enabled() {
        let settings = memora_config::RecordingSettings {
    ...
    fn listen_on_lan_forces_api_auth_on_even_if_disabled() {
        ...
        // while listen_on_lan is on â€” we refuse to let that combo ship.
    ...
    fn listen_on_lan_off_respects_api_auth_off() {
        ...
        // loopback, leave them alone â€” localhost-only is already safe.
    ...
    fn aec_mode_produces_one_effective_backend() {
        let legacy_conflict_without_mode = memora_config::RecordingSettings {
        ...
        assert!(!c.MEMORA_aec_enabled);
        assert!(!c.windows_input_aec_enabled);
        assert!(!c.macos_input_vpio_enabled);

        let explicit_MEMORA = memora_config::RecordingSettings {
            aec_mode: memora_config::AecMode::MEMORA,
            MEMORA_aec_enabled: false,
            windows_input_aec_enabled: true,
            macos_input_vpio_enabled: true,
            ..Default::default()
        };
        let c = build(&explicit_MEMORA);
        assert!(c.MEMORA_aec_enabled);
        assert!(!c.windows_input_aec_enabled);
        assert!(!c.macos_input_vpio_enabled);

        let explicit_macos = memora_config::RecordingSettings {
            aec_mode: memora_config::AecMode::Macos,
            MEMORA_aec_enabled: true,
            windows_input_aec_enabled: true,
    ...
    fn privacy_capture_toggles_flow_to_ui_recorder_config() {
        ...
        assert_eq!(
            ...
            memora_a11y::ExtractionThreadPriority::Lowest
        );
    ...
    fn click_capture_defaults_on_and_rows_follow_setting() {
        ...
        let ui = build(&memora_config::RecordingSettings::default()).to_ui_recorder_config();
        assert!(ui.capture_clicks);
        assert!(ui.record_click_events);
