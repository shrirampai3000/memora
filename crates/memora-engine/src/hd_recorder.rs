...
//! High-fps HD video recorder â€” decoupled from OCR/indexing.
//!
...
//! frame â€” ~250-800ms of work each. That's the searchable index, and it must
...
//! constant-frame-rate H.264 chunk with ffmpeg â€” no OCR, no a11y. The event-
...
//! - a **bounded writer pacer** targets `elapsed_wall_time * fps` frames â€” the
...
use memora_screen::monitor::SafeMonitor;

/// Per-monitor configuration for the HD recorder.
///
/// The capture rate is NOT here â€” it's read live from the `HighFpsController`
...
pub struct HdRecorderConfig {
    /// Ignored-window patterns â€” excluded from HD capture at the OS level so
    ...
    /// Ignored-URL patterns â€” windows showing a blocked URL are excluded from
...
/// the chunk hits its max length, or capture is paused â€” then repeats.
...
mod macos {
    ...
    async fn e2e_stall_hd_writer_once() {
        let armed = std::env::var("MEMORA_E2E_SEED")
            .ok()
    ...
    async fn record_one_chunk(
        ...
        use memora_screen::capture_screenshot_by_window::{
            get_excluded_sck_window_ids, WindowFilters,
        };

        // Read the HD rate live from the controller so tray changes (10â†”30fps)
    ...
    /// Spawn ffmpeg: a stream of JPEGs on stdin â†’ constant-frame-rate H.264.
    ...
    fn start_hd_ffmpeg(out: &Path, fps: u32) -> Result<tokio::process::Child> {
        let ffmpeg = memora_core::find_ffmpeg_path().context("ffmpeg not found")?;
        let mut cmd = memora_core::ffmpeg_cmd_async(&ffmpeg);
        ...
        cmd.args([
            ...
            "-i",
            "-",
            // Output: CFR H.264 so decode-frame N is at time N/fps â€” the
    ...
    /// mjpegâ†’libx264 pipe is what produces the green-band corruption users
    ...
    mod tests {
        ...
        fn encode_jpeg_downscales_oversized_frame_to_target() {
            ...
            // resolution change) â€” must be pinned to the stream size so the
            // mjpegâ†’libx264 pipe stays constant-dimension.
