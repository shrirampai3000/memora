...
use memora_connect::mcp_servers::{
    McpAuthMode, McpHeader, McpServerConfig, McpServerStore, McpTransport,
};
use memora_core::pipes::mcp_access::McpSessionAccessRegistry;
...
/// GET /mcp-servers â€” list all registered servers (no header values).
...
/// GET /mcp-servers/:id â€” single server detail (no header values).
async fn get_server(
...
/// PUT /mcp-servers/:id â€” create or replace a server.
...
/// DELETE /mcp-servers/:id â€” remove a server.
...
/// *name* (per RFC 7230 names are tokens â€” letters/digits/specials â€”
...
fn validate_headers(headers: &[McpHeader]) -> Result<(), String> {
    for h in headers {
        if h.name.bytes().any(|b| b == b'\r' || b == b'\n' || b == 0) {
            return Err("header name contains a CR/LF/NUL byte â€” refusing to send".to_string());
        }
        if h.value.bytes().any(|b| b == b'\r' || b == b'\n' || b == 0) {
            return Err(format!(
                "header `{}` value contains a CR/LF/NUL byte â€” refusing to send",
                h.name
            ));
        }
        // RFC 7230 token rule for header names â€” letters, digits and
...
fn merge_headers(existing: &[McpHeader], supplied: &[McpHeader]) -> Vec<McpHeader> {
    ...
    supplied
        ...
        .filter_map(|h| {
            if !h.value.is_empty() {
                // New value supplied â€” use it and forget the old one
...
/// POST /mcp-servers/:id/test â€” probe stored server.
...
/// POST /mcp-servers/test â€” probe an unsaved server config.
...
/// GET /mcp-servers/:id/tools â€” cached tools list (same wire format as
...
/// POST /mcp-servers/:id/call â€” forward a tool call.
async fn call_tool(
    State(state): State<McpServersState>,
    headers: HeaderMap,
    Path(id): Path<String>,
...
/// GET /mcp-servers/:id/oauth/status â€” whether a token is stored.
...
/// POST /mcp-servers/:id/oauth/start â€” create PKCE state and return provider URL.
...
/// GET /mcp-servers/:id/oauth/callback â€” browser redirect target.
async fn oauth_callback(
    ...
    if let Some(error) = query.get("error") {
        return html_response(
            ...
            &format!("MEMORA MCP OAuth failed: {}", error),
        );
    }
    let Some(state_value) = query.get("state") else {
        return html_response(
            StatusCode::BAD_REQUEST,
            "MEMORA MCP OAuth failed: missing state",
        );
    };
    let Some(code) = query.get("code") else {
        return html_response(
            StatusCode::BAD_REQUEST,
            "MEMORA MCP OAuth failed: missing code",
        );
    };
    match state.store.complete_oauth(state_value, code).await {
        Ok(server_id) if server_id == id => html_response(
            StatusCode::OK,
            "MEMORA MCP OAuth connected. You can close this tab.",
        ),
        Ok(_) => html_response(
            StatusCode::BAD_REQUEST,
            "MEMORA MCP OAuth failed: callback server mismatch",
        ),
        Err(e) => html_response(
            StatusCode::BAD_REQUEST,
            &format!("MEMORA MCP OAuth failed: {}", e),
        ),
    }
}

/// POST /mcp-servers/:id/oauth/disconnect â€” wipe stored token.
...
/// unauthenticated â€” we proxy it so the desktop UI can browse/search
...
/// GET /mcp-servers/registry â€” search the official registry.
///
...
/// as-is â€” we deliberately do NOT model the full schema so new fields
...
/// whole page â€” same defensive instinct as the json_each tag guard.
...
fn html_response(status: StatusCode, message: &str) -> Response {
    ...
    let detail = if ok {
        "MEMORA can now use this MCP server."
    } else {
        "MEMORA could not finish the MCP OAuth flow."
    };
    (
        status,
        [("content-type", "text/html; charset=utf-8")],
        render_oauth_result_page("MEMORA MCP OAuth", title, detail, message, ok),
    )
        .into_response()
}
...
fn mcp_oauth_redirect_uri(
    ...
    match app_scheme {
        ...
        Some(scheme @ ("MEMORA" | "MEMORA-enterprise")) => Ok(format!(
            "https://github.com/shrirampai3000/memora/api/oauth/mcp/{server_id}/callback?app_scheme={scheme}"
        )),
        Some(_) => Err("app_scheme must be MEMORA or MEMORA-enterprise"),
    }
}
...
mod tests {
    ...
    fn merge_drops_placeholder_with_no_existing_value() {
        // Name in supplied with empty value and nothing stored â€” drop.
    ...
    fn merge_handles_duplicate_supplied_names() {
        // Same name supplied twice â€” the second non-empty wins.
        ...
        // really want to â€” and matches what reqwest does with
    ...
    fn normalise_preserves_value_whitespace() {
        // Trim names, not values â€” some tokens are space-sensitive
    ...
    fn oauth_redirect_allows_only_exact_app_schemes() {
        assert_eq!(
            mcp_oauth_redirect_uri("linear", Some("MEMORA")).unwrap(),
            "https://github.com/shrirampai3000/memora/api/oauth/mcp/linear/callback?app_scheme=MEMORA"
        );
        assert_eq!(
            mcp_oauth_redirect_uri("linear", Some("MEMORA-enterprise")).unwrap(),
            "https://github.com/shrirampai3000/memora/api/oauth/mcp/linear/callback?app_scheme=MEMORA-enterprise"
        );
        assert!(mcp_oauth_redirect_uri("linear", Some("https")).is_err());
        assert!(mcp_oauth_redirect_uri("linear", Some("MEMORA://evil")).is_err());
    }
    ...
    fn flatten_registry_skips_malformed_entry() {
        // No `server` object â€” one bad row must not poison the page.
