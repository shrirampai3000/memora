...
//! Runtime permission monitor â€” single source of truth for permission events.
//!
//! Emits `permission_lost` and `permission_restored` events on the shared
//! `memora-events` bus when the OS permission state changes.
//!
//! ## Design
//!
...
//!    revocation as a specific error â€” the stream just goes silent.
...
//! lostâ†’restored flash.

use std::sync::Mutex;
use std::time::{Duration, Instant};

use once_cell::sync::Lazy;
use memora_core::permissions::{check_permissions, PermissionStatus};
...
struct State {
    ...
    /// TCC preflight still (stale-ly) reports granted â€” the macOS 15+/26
...
static STATE: Lazy<Mutex<State>> = Lazy::new(|| {
    Mutex::new(State {
        // Initialize all as "granted" â€” first poll will emit `lost` for any
...
/// Start the monitor. Idempotent â€” calling twice returns early; the first
...
/// lostâ†’restored flashes after sleep/wake.
...
fn report_state_inner(kind: PermissionKind, now_granted: bool, reason: Option<&str>) -> bool {
    ...
    // caller, so this counts as delivered â€” a repeat report must not be read as
    // "swallowed".
...
/// enumeration â€” the ground truth for screen-recording permission.
///
...
/// returns zero displays, so the poll alone never notices the loss â€” the
...
/// emissions are currently suppressed). Diagnostic/test accessor â€” lets the
...
async fn run() {
    ...
    // First tick fires immediately â€” skip it, we already seeded state.
    ticker.tick().await;
    ...
    loop {
        ...
        // `report_screen_enumeration`) â€” don't let it flap us back.
...
/// True while an enumeration-reported screen-recording loss is active â€”
...
/// alone â€” in the lapsed-grant state the preflight keeps answering granted,
...
/// permitted â€” either the poll saw the grant revoked, or enumeration proved
...
/// `true` â€” the permission doesn't apply there, so "no transition" is the
/// correct semantic.
fn keychain_accessible() -> bool {
    use memora_secrets::keychain::{get_key, is_keychain_available, KeyResult};
    ...
    if !memora_secrets::is_encryption_requested(
...
mod tests {
    ...
    /// report (the preflight is stale in exactly this state â€” it kept saying
    ...
    async fn enumeration_loss_suppresses_poll_until_enumeration_recovers() {
        ...
        // Otherwise the app reads "denied" â€” refusing to start capture â€” while
