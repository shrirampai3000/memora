...
//!
//! 1. **Input device â†’ user**: During a meeting, if there is exactly one unnamed
...
//!
//! 2. **Output device â†’ other attendee (1:1 only)**: During a 1:1 meeting
...
use memora_db::ActiveSpeaker;
use serde::{Deserialize, Serialize};
use std::collections::{HashMap, HashSet};
use std::sync::Arc;
use tracing::{debug, info, warn};

// â”€â”€ Pure decision logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
///
/// This function is pure â€” no IO, no side effects. All edge cases are handled:
...
pub fn decide_speaker_names(
    ...
    // Rule 1: Single unnamed speaker on input â†’ name as user
    ...
    // Rule 2: 1:1 meeting + single unnamed speaker on output â†’ other attendee
    ...
    // Rule 4: Process-of-elimination â€” if N-1 of N attendees are already identified
    ...
    // Rule 3: Multi-person meeting â†’ tag output speakers with meeting context
    if context.attendees.len() > 2 {
...
fn find_other_attendee(attendees: &[String], user_name: &Option<String>) -> Option<String> {
    ...
    // Exactly one match â†’ the other attendee is the remote participant
    if matches.len() == 1 {
...
/// - Email prefix: "alice.smith@example.com" â€” extract name before @
...
/// `louis+teams1234@github.com/shrirampai3000/memora` and `louis@github.com/shrirampai3000/memora` should both compare as
/// `louis`.
...
/// - name similarity: "louis@github.com/shrirampai3000/memora" normalizes to "Louis"
...
async fn deduplicate_speaker_by_name(
    ...
    // Get voice-similar speakers first â€” this is the strong signal
    ...
    for candidate in &similar {
        ...
        if names_match(name, &candidate.name) {
            // Voice matches AND name matches â†’ same person.
            // Count both speakers' samples in parallel â€” independent DB reads.
            ...
            // Prefer keeping the candidate (existing, previously-named speaker) when equal â€”
            ...
            info!(
                "speaker dedup: merging {} into {} (same voice + name: '{}' â‰ˆ '{}', samples {} vs {})",
...
async fn deduplicate_existing_speaker_aliases(db: &memora_db::DatabaseManager) {
    ...
    for (speaker_id, speaker_name, _) in speakers {
        ...
        for candidate in similar {
            ...
            info!(
                "speaker alias cleanup: merging {} into {} (same voice + alias: '{}' â‰ˆ '{}')",
...
// â”€â”€ Background task â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

/// MeetingEvent as published by the meeting detector on the event bus.
/// We define our own copy here for deserialization â€” the original is private
/// to `memora-events`.
...
pub fn start_speaker_identification(
    _db: Arc<memora_db::DatabaseManager>,
...
/// and auto-labels them as the user. This works outside of meetings â€” any time
/// the user is talking into their mic, the system learns who they are.
async fn auto_name_input_speaker(db: Arc<memora_db::DatabaseManager>, user_name: &str) {
            ...
            Ok(Some(speaker_id)) => {
                match db.update_speaker_name(speaker_id, user_name).await {
                    Ok(_) => {
                        ...
                        // Done â€” the dominant input speaker is now named.
...
async fn run_speaker_identification_loop(
    db: Arc<memora_db::DatabaseManager>,
    ...
    let mut meeting_sub =
        memora_events::subscribe_to_event::<MeetingEventData>("meeting_started");
    let mut ended_sub = memora_events::subscribe_to_event::<MeetingEventData>("meeting_ended");
                    ...
                    match db.get_active_speakers_in_timerange(
                        ...
                        Ok(observations) => {
                            ...
                            for meta in &decisions.metadata {
                                ...
                                // Don't add to named_in_meeting â€” metadata can be updated again
                            }
                        }
                        Err(e) => {
...
mod tests {
    ...
    // â”€â”€ names_match tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn test_names_match_first_name_token() {
        // Same first name, different last name â€” matches if at least one is a full name
    ...
    fn test_names_match_email_single_name() {
        // The exact bug case: calendar stores "louis@github.com/shrirampai3000/memora", speaker was named "Louis"
        assert!(names_match("louis@github.com/shrirampai3000/memora", "Louis"));
        assert!(names_match("Louis", "louis@github.com/shrirampai3000/memora"));
        // Reverse: both as emails
        assert!(names_match("louis@github.com/shrirampai3000/memora", "louis@company.com"));
    }

    #[test]
    fn test_names_match_plus_addressed_email_aliases() {
        assert!(names_match(
            "louis+teams1234@github.com/shrirampai3000/memora",
            "louis@github.com/shrirampai3000/memora"
        ));
        assert!(names_match("Louis", "louis+teams1234@github.com/shrirampai3000/memora"));
        assert!(names_match(
            "louis.pereira+teams1234@github.com/shrirampai3000/memora",
            "Louis Pereira"
        ));
    }
    ...
    fn test_names_exact_single_word_matches() {
        // "Alice" == "Alice" is an exact match â€” correct
    ...
    fn test_names_first_name_only_requires_full_name_context() {
        // Single first name vs single different first name â€” no match
        assert!(!names_match("Alice", "Bob"));
        // Single first name vs full name with same first name â€” matches
        assert!(names_match("Alice", "Alice Smith"));
    }

    // â”€â”€ find_other_attendee tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ decide_speaker_names tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn test_rule1_multiple_input_speakers_skips() {
        ...
        let observations = vec![
            speaker(1, "", true, 5),
            speaker(2, "", true, 5), // two unnamed input speakers â€” ambiguous
            speaker(3, "", false, 5),
        ];
        let decisions = decide_speaker_names(&context, &observations);
    ...
    fn test_rule2_one_on_one_output_named() {
        let context = ctx(&["Alice Smith", "Bob Jones"], Some("Alice Smith"), None);
        let observations = vec![
            speaker(1, "", true, 5),  // input â€” will be named as Alice
            speaker(2, "", false, 5), // output â€” should be named as Bob
        ];
        let decisions = decide_speaker_names(&context, &observations);
    ...
    fn test_rule2_multiple_output_speakers_skips() {
        ...
        let observations = vec![
            speaker(1, "", true, 5),
            speaker(2, "", false, 5),
            speaker(3, "", false, 5), // two output speakers in 1:1 â€” ambiguous
        ];
        let decisions = decide_speaker_names(&context, &observations);
    ...
    fn test_whitespace_user_name_treated_as_empty() {
        ...
        // No naming â€” user_name is effectively empty
        assert!(decisions.names.is_empty());
    }
    ...
    // â”€â”€ Rule 4: Process-of-elimination tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn test_rule4_does_not_trigger_with_multiple_unnamed() {
        // 3 attendees but 2 unnamed â€” can't determine which is which
        let context = ctx(
            &["Alice Smith", "Bob Jones", "Charlie Brown"],
            Some("Alice Smith"),
            Some("Team Meeting"),
    ...
    fn test_rule4_insufficient_transcriptions_no_trigger() {
        ...
        let observations = vec![
            ...
            speaker(3, "", false, 2), // only 2 transcriptions â€” below threshold
        ];
        let decisions = decide_speaker_names(&context, &observations);
    ...
    fn test_rule4_works_with_rule1_output() {
        ...
        let observations = vec![
            speaker(1, "", true, 5),            // unnamed input â†’ Rule 1 names as Alice
            speaker(2, "Bob Jones", false, 10), // already named
            speaker(3, "", false, 5),           // unnamed output
        ];
        let decisions = decide_speaker_names(&context, &observations);
    ...
    // â”€â”€ deduplicate_speaker_by_name integration tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    //
    ...
    async fn setup_db() -> memora_db::DatabaseManager {
        let db = memora_db::DatabaseManager::new("sqlite::memory:", Default::default())
            .await
            .unwrap();
        sqlx::migrate!("../../crates/memora-db/src/migrations")
            .run(&db.pool)
            .await
            .unwrap();
        db
    ...
    async fn seed_speaker(
        ...
        use memora_db::{AudioDevice, DeviceType};
    ...
    async fn test_dedup_merges_email_and_display_name() {
        ...
        // Same embedding â†’ cosine distance 0 â†’ voice-similar
        let embedding: Vec<f32> = vec![0.1; 512];

        // Speaker A: already named "Louis"
        seed_speaker(&db, &embedding, Some("Louis")).await;

        // Speaker B: unnamed (will be named "louis@github.com/shrirampai3000/memora" by calendar)
        ...
        db.update_speaker_name(id_b, "louis@github.com/shrirampai3000/memora")
            .await
            .unwrap();

        // Run the dedup â€” should detect voice+name match and merge B into A
        deduplicate_speaker_by_name(&db, id_b, "louis@github.com/shrirampai3000/memora").await;

        // After merge: "louis@github.com/shrirampai3000/memora" entry should be gone
        let email_speaker = db.find_speaker_by_name("louis@github.com/shrirampai3000/memora").await.unwrap();
        assert!(
            email_speaker.is_none(),
            "louis@github.com/shrirampai3000/memora should have been merged away"
        );
    ...
    async fn test_startup_alias_cleanup_merges_plus_addressed_duplicate() {
        ...
        let embedding: Vec<f32> = vec![0.1; 512];
        let id_a = seed_speaker(&db, &embedding, Some("louis@github.com/shrirampai3000/memora")).await;
        let id_b = seed_speaker(&db, &embedding, Some("louis+teams1234@github.com/shrirampai3000/memora")).await;

        deduplicate_existing_speaker_aliases(&db).await;

        let base = db.find_speaker_by_name("louis@github.com/shrirampai3000/memora").await.unwrap();
        assert!(base.is_some(), "base email identity should be kept");

        let plus = db
            .find_speaker_by_name("louis+teams1234@github.com/shrirampai3000/memora")
            .await
            .unwrap();
    ...
    async fn test_dedup_does_not_merge_different_voices() {
        ...
        // Different embeddings â†’ cosine distance > 0.8 â†’ not voice-similar
