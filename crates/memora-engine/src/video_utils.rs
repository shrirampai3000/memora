...
use memora_db::VideoMetadata as DBVideoMetadata;
...
pub fn get_ffprobe_path(ffmpeg_path: &Path) -> PathBuf {
    ...
    // ffprobe not alongside ffmpeg â€” try PATH
    #[cfg(unix)]
    ...
    warn!(
        "ffprobe not found alongside ffmpeg ({}) or in PATH â€” frame extraction from MP4 will fail",
        ffmpeg_path.display()
    );
...
async fn get_video_fps_and_duration_uncached(
    ...
    let fps = parsed
        ...
        // NaN means "ffprobe answered but no usable frame rate" â€” distinct from
...
pub async fn extract_frame_from_video(
    ...
    // than merely awkward â€” the route maps that to 410 instead of 500.
    ...
    let frames_dir = PathBuf::from("/tmp/MEMORA_frames");
