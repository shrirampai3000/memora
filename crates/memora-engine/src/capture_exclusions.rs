...
//!   configured â€” the caller must skip the frame's pixels rather than risk
//!   storing a window the user asked to hide.

use memora_screen::capture_screenshot_by_window::{get_excluded_sck_window_ids, WindowFilters};
...
/// "last known good" â€” it must never satisfy a storage capture).
...
async fn refresh_excluded_sck_window_ids(window_filters: &WindowFilters) -> Option<Vec<u32>> {
    ...
    match tokio::time::timeout(EXCLUSION_REFRESH_TIMEOUT, handle).await {
        Ok(Ok(ids)) => Some(ids),
        Ok(Err(join_err)) => {
            // The task never ran its closure tail â€” clear the flag here.
            EXCLUSION_REFRESH_IN_FLIGHT.store(false, Ordering::Release);
...
/// Exclusion ids for the visual-change PROBE (frame diffing only â€” nothing is
...
/// `EXCLUSION_STORAGE_MAX_STALENESS`) while exclusions ARE configured â€” the
...
mod tests {
    ...
    fn storage_exclusion_cache_cold_or_stale_fails_closed() {
        ...
        // Stale (or cold on a <120s-uptime host â€” both fail closed).
    ...
    fn storage_cache_freshness_is_inclusive_at_the_boundary() {
        ...
        // `<= max_staleness` â€” allow a hair of scheduling slack and assert
    ...
    async fn storage_tier_falls_back_to_cache_rule_when_refresh_unavailable() {
        ...
        // Now make the cache stale (or cold on a <600s-uptime host â€” the
