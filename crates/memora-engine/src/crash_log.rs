...
//!
//! Integrators embed the `MEMORA` binary as a child process inside their
//! own wrapper (e.g. an Electron app) and, when it dies, observe only its exit
//! code â€” which doesn't say *why*. These helpers persist the panic message and
...
//! who disable analytics â€” for them it's the *only* crash signal that never
//! leaves the machine. See `bin/memora-engine.rs` for the panic hook that
//! calls into here.
...
/// is preserved in `.prev` while the new run starts a fresh log. Best-effort â€”
...
mod tests {
    ...
    fn write_then_append_accumulates_records() {
        ...
        // Append, not overwrite: two records â†’ two lines.
