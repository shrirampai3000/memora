...
//!
//! Integrates memora-a11y capture with the server's recording loop.

use anyhow::Result;
use memora_a11y::{ExtractionThreadPriority, UiCaptureConfig, UiRecorder};
use memora_core::window_pattern::{self, WindowPattern};
use memora_db::{DatabaseManager, InsertUiEvent};
...
/// `correlation_id` as `None` â€” those rows stay `frame_id = NULL`.
///
...
fn is_click_context_enrichment(event: &InsertUiEvent) -> bool {
    event.event_type == memora_db::UiEventType::Click
...
fn click_context_merge_candidate(candidate: &InsertUiEvent, enrichment: &InsertUiEvent) -> bool {
    candidate.event_type == memora_db::UiEventType::Click
...
pub struct UiRecorderConfig {
    ...
    /// User-configured ignored windows (for tree walker â€” substring match)
...
impl Default for UiRecorderConfig {
    fn default() -> Self {
        Self {
            ...
            // (macOS: memora_a11y::scroll; Windows: ScrollAggregator).
...
/// from `memora_a11y::tree::TreeWalkResult` â€” callers translate their
...
pub enum TreeWalkOutcome {
    ...
    /// (content dedup). Still a real walk attempt â€” counts toward totals.
    Deduped {
        duration_ms: u64,
        node_count: u64,
        max_depth: u64,
    ...
    Empty,
    /// Walk failed â€” no focused window / AX error (`TreeWalkResult::NotFound`).
    Error,
}
...
/// Global shared tree walker metrics â€” a cumulative accumulator updated on
...
/// Coarse-grained UI-recorder state â€” the one-field summary the UI cares
...
pub enum UiRecorderMode {
    ...
    /// granted â€” keystrokes, clicks, clipboard, app/window events all
    /// captured.
    Full,
    /// Recorder is running with Accessibility only â€” clipboard and
...
/// can tell whether input/clipboard capture is actually running â€” distinct
...
pub struct UiRecorderStatus {
    ...
    /// CGEventTap thread is alive â€” keystrokes and clicks are being
    ...
    /// NSWorkspace observer is alive â€” app switches and window focus
...
pub async fn start_ui_recording(
    ...
    //   and AX click-target enrichment). Missing â†’ fail entirely.
    // - Input Monitoring is OPTIONAL. Missing â†’ the recorder runs in
    ...
    if !perms.accessibility {
        ...
        // (macOS).
        #[cfg(target_os = "macos")]
        let hint = "Grant Accessibility in System Settings â†’ Privacy & Security â†’ Accessibility, then relaunch.";
        #[cfg(target_os = "linux")]
        let hint =
        ...
        error!(
            "Accessibility unavailable â€” UI event recording disabled \
    ...
    // accessibility is granted (which it is, here â€” we'd have bailed
    // otherwise).
    ...
    // Parse user-configured ignore patterns once â€” the spawned task fires on
    ...
    // patterns (see `memora-core::window_pattern`).
    ...
    let task_handle = tokio::spawn(async move {
        ...
        // captured events (MEMORA-CLI-FZ); this cap stops that retention from
        // growing without limit â€” and keeps the retried INSERT cheap â€” when the
        ...
        // exceed the a11y coalescer's max-burst split interval (2s â€” see
        // ScrollBuffer::new in memora_a11y::scroll): a long sustained
        ...
        loop {
            ...
            match handle.recv_timeout(recv_timeout) {
                Some(event) => {
                    ...
                    // with the eventual EventPersisted notification â€” that's
                    ...
                    let is_scroll =
                        matches!(db_event.event_type, memora_db::UiEventType::Scroll);
                    let trigger_kind =
                    ...
                    // explicitly so the recorder doesn't bother â€” there's
                        ...
                        (&capture_trigger_tx, trigger_kind)
                    {
                        ...
                        // receivers between the count check above and now â€”
                    ...
                    // Flush when the batch is full â€” but only while writes are
...
// Tree walker is disabled â€” paired_capture.rs handles accessibility capture.
...
fn should_record_input_event(
    ...
    match db_event.event_type {
        memora_db::UiEventType::Key | memora_db::UiEventType::Text => {
            record_keyboard_events
        }
        memora_db::UiEventType::Clipboard => record_clipboard_events,
        ...
        // with the pointer" â€” scroll rows appearing anyway (now that
        // capture_scroll defaults on) would silently widen that surface.
        memora_db::UiEventType::Click | memora_db::UiEventType::Scroll => {
            record_click_events
        }
        _ => true,
    }
...
/// flushes are being retained for retry during a DB stall â€” see `flush_batch`.
...
async fn flush_batch(
    ...
    // The DB call borrows the events slice directly â€” no clones.
    ...
    match db.insert_ui_events_batch(&batch.events).await {
        Ok(row_ids) => {
            ...
            *consecutive_failures = 0;
            ...
            if let Some(linker) = linker_tx {
                for (row_id, corr_id_opt) in row_ids.iter().zip(batch.correlation_ids.iter()) {
                    if let Some(corr_id) = corr_id_opt {
                        ...
                        // the recorder. Frame linkage is best-effort â€”
            ...
            // the next flush retries it â€” otherwise a transient write-pool stall
            // (e.g. PoolTimedOut) silently drops captured events and leaves a
            // hole in the timeline. See MEMORA-CLI-FZ. Growth across a
        ...
        Err(e) => {
            *consecutive_failures += 1;
            ...
            // as MEMORA-CLI-SQ. A genuinely stuck coordinator is escalated
            // by the write queue itself, not by counting log lines here.
            if memora_db::is_retryable_write_stall(&e) {
...
/// (Move, Idle) and for Scroll events â€” Scroll triggers are deferred
...
fn capture_trigger_kind_with_ignored(
    ...
    match &db_event.event_type {
        memora_db::UiEventType::AppSwitch => {
            if is_ignored {
                None
            } else {
        ...
        memora_db::UiEventType::WindowFocus => {
            if is_ignored {
                None
            } else {
        ...
        memora_db::UiEventType::Click => Some(
            target
        ...
        memora_db::UiEventType::Clipboard => Some(CaptureTrigger::Clipboard),
        // Text events are already burst-end-debounced by the a11y layer
        // (`text_timeout_ms`, default 300ms) â€” one row per typing burst,
        // so one TypingPause trigger per row is the correct semantic.
        memora_db::UiEventType::Text => Some(CaptureTrigger::TypingPause),
...
/// row every ~2s, and each row resets this timer â€” a shorter delay
...
mod event_batch_tests {
    ...
    fn click_at(x: i32, y: i32) -> InsertUiEvent {
        ...
        e.window_title = Some("MEMORA".to_string());
        e
    }
    ...
    fn enriched_click_merges_into_pending_raw_click_and_keeps_corr_id() {
        ...
        enriched.element_ancestors =
            Some(r#"[{"role":"AXWindow","name":"MEMORA"}]"#.to_string());
        ...
        assert_eq!(merged.window_title.as_deref(), Some("MEMORA"));
        ...
        assert_eq!(
            ...
            Some(r#"[{"role":"AXWindow","name":"MEMORA"}]"#)
        );
    }
    ...
    fn enriched_click_with_missing_context_keeps_raw_app_window() {
        ...
        assert_eq!(b.events[0].window_title.as_deref(), Some("MEMORA"));
    ...
    fn enriched_click_replaces_approximate_context_without_losing_linkage() {
        ...
        enriched.element_ancestors = Some(r#"[{"role":"Window","name":"MEMORA"}]"#.to_string());
        ...
        assert_eq!(
            ...
            Some(r#"[{"role":"Window","name":"MEMORA"}]"#)
        );
    }
    ...
    /// Regression for MEMORA-CLI-FZ: a failed insert must NOT discard the
    ...
    /// MEMORA-CLI-SQ: a batch that lost a race for the SQLite write path
    /// is an expected stall â€” `compact()` and the RESTART checkpoint hold that
    /// same process-wide lock by design â€” so it must not be reported as a
    ...
    fn write_path_contention_is_classified_as_a_retryable_stall() {
        let contention = sqlx::Error::Protocol(memora_db::WRITE_LOCK_HELD_MESSAGE.to_string());
        let starvation =
            sqlx::Error::Protocol(memora_db::WRITE_POOL_STARVED_MESSAGE.to_string());
        assert!(memora_db::is_retryable_write_stall(&contention));
        assert!(memora_db::is_retryable_write_stall(&starvation));
        ...
        assert!(
            !memora_db::is_retryable_write_stall(&real_fault),
            "a real schema/IO fault must not be treated as contention"
        );
        assert!(!memora_db::is_retryable_write_stall(
...
mod tests {
    ...
    fn tree_walks_disabled_by_default_through_to_ui_config() {
        ...
        // and freeze it (TESTING.md Â§15), and their snapshots have no consumers â€”
    ...
    fn ui_recorder_status_reflects_state_and_flush() {
        ...
        // Full mode: both perms granted â†’ input_tap + app_events both up.
        ...
        // Reduced mode: input monitoring missing â€” input_tap_running flips
        ...
        // Disabled path: everything off â†’ Off, regardless of bool combos.
        set_ui_recorder_state(false, false, false, false, false);
        ...
        // Counter and timestamp persist across state transitions â€” they're
    ...
    async fn test_spawn_channel_pattern_clean_shutdown() {
        ...
        // Drop the sender without sending â€” simulates runtime dropping the task
        drop(result_tx);
    ...
    async fn test_spawn_channel_runtime_shutdown_no_panic() {
        ...
        // runtime. The recv should return Err cleanly â€” no panic.
        let (result_tx, result_rx) = std::sync::mpsc::sync_channel::<Result<i64, String>>(1);
        ...
        // Shutdown the runtime â€” the task is cancelled, sender is dropped
