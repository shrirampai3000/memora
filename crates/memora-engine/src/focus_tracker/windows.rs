...
//! Windows focus tracker â€” Phase 2 event-driven WinEvent hook observer.
//!
...
/// Plain rect used for monitor resolution â€” the Win32 `RECT` type is not
...
fn foreground_window_anchor() -> Option<(i32, i32)> {
    ...
    // foreground window â€” check `is_invalid()`.
    ...
    // Use the window's center â€” not its top-left â€” so a window straddling
...
fn run_win_event_observer() {
    ...
    unsafe extern "system" fn on_foreground_change(
        ...
        // on the Win32 message-pump thread, which has no tokio context â€” driving the
        ...
        let monitors = inner
            .runtime
            .block_on(memora_screen::monitor::list_monitors());
        let rects = monitor_rects_from_monitors(&monitors);
    ...
    // loop â€” thread must pump messages (GetMessageW) for delivery.
    let hook = unsafe {
        SetWinEventHook(
    ...
    unsafe {
        loop {
            ...
            if bret.0 <= 0 {
                // 0 = WM_QUIT, -1 = error â€” either way we exit.
                break;
            }
