...
//! macOS focus tracker â€” Phase 2 event-driven NSWorkspace observer.
//!
...
//! than probing NSWindow â†’ NSScreen. Window-level resolution requires AX
...
//! is robust, permission-free, and correct in the common case â€” users glance
...
//! bounds check `[x, x+width) Ã— [y, y+height)` works.
//!
//! # Lifetime
//! The CF run-loop thread is a `std::thread` (no tokio); it outlives the
//! tracker on purpose â€” CFRunLoop teardown mid-observation is brittle. The
...
//! for the process lifetime (one per feature-enable cycle â€” bounded in
...
extern "C" {
    // Must match the signature in `sleep_monitor.rs` â€” both take `*const c_void`
...
/// Pure bounds check â€” returns the monitor whose rect contains (x, y).
///
...
fn monitor_for_point(
    monitors: &[memora_screen::monitor::SafeMonitor],
    x: f64,
    y: f64,
    current: Option<&MonitorIdentity>,
) -> Option<MonitorIdentity> {
...
impl Inner {
    ...
    fn resolve_and_emit(&self, monitors: &[memora_screen::monitor::SafeMonitor]) {
...
impl DarwinFocusTracker {
    pub fn start() -> Result<Self> {
        ...
        // lifetime â€” see module docstring for rationale.
        let observer_inner = Arc::clone(&inner);
        std::thread::Builder::new()
            .name("MEMORA-focus-tracker".into())
            .spawn(move || {
        ...
        let _poll_handle = handle.spawn(async move {
            ...
            loop {
                ...
                let monitors = memora_screen::monitor::list_monitors_cached().await;
...
fn run_workspace_observer(inner: Arc<Inner>) {
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        // cursor position. We don't use `running_apps()` inside the block â€”
        ...
        let activate_cb = move |_note: &ns::Notification| {
            // Wrap in autorelease pool â€” this callback runs on a custom
            ...
            cidre::objc::ar_pool(|| {
                ...
                let monitors =
                    futures::executor::block_on(memora_screen::monitor::list_monitors_cached());
        ...
        // Run the run loop â€” blocks forever, delivers notifications.
...
impl FocusTracker for DarwinFocusTracker {
    ...
    fn stop(&self) {
        // Idempotent â€” multiple calls are fine.
