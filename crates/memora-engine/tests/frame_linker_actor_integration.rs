...
use chrono::Utc;
use memora_db::{DatabaseManager, InsertUiEvent, UiEventType};
use memora_engine::frame_linker::{DropReason, EventPersisted, FrameCaptured};
use memora_engine::frame_linker_actor::{
...
/// asynchrony â€” but it should land within a handful of ms in practice.
...
async fn reverse_order_frame_then_event_links() {
    ...
    linker_tx
        ...
        .await
        .unwrap();
    // Brief pause â€” actor must hold the unmatched corr id.
    tokio::time::sleep(Duration::from_millis(20)).await;
    assert!(
        read_frame_id(&db, row_id).await.is_none(),
        "no link yet â€” event not persisted"
    );
...
async fn unmatched_event_stays_null_when_no_frame_arrives() {
    ...
    assert!(
        read_frame_id(&db, row_id).await.is_none(),
        "no frame ever arrived â€” row stays NULL"
    );
...
///   T2  recorder batch flush â†’ EventPersisted{K, row}
///   T3  monitor B (Cold) drains corr=K, sends TriggerDropped
///   T4  monitor A finishes â†’ FrameCaptured{K, frame}
///
/// The actor MUST treat TriggerDropped as a metrics signal only â€” TTL
...
async fn trigger_dropped_does_not_unpair_a_racing_capture() {
    ...
    // as dropped. This MUST NOT remove the pending event â€” another
    ...
    assert_eq!(
        observed,
        Some(frame_id),
        "pending event must survive a racy TriggerDropped â€” the Active monitor's frame still pairs it"
    );
...
/// TriggerDropped for K must not clear that unmatched slot â€” the
...
async fn trigger_dropped_does_not_clear_pending_frame_waiters() {
    ...
    linker_tx
        ...
        .await
        .unwrap();
    // Event row arrives last â€” should still pair.
    linker_tx
        .send(LinkerMessage::EventPersisted(EventPersisted {
            correlation_id: corr_id,
            row_id,
