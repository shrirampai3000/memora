...
use memora_engine::video_cache::FrameCache;
...
async fn setup_test_env() -> Result<(FrameCache, Arc<DatabaseManager>)> {
    ...
    debug!("using real MEMORA data dir: {:?}", MEMORA_dir);

    let db = Arc::new(
        DatabaseManager::new(
            paths::default_MEMORA_data_dir()
                .join("db.sqlite")
                .to_str()
                .unwrap(),
            Default::default(),
    ...
    let cache = FrameCache::new(MEMORA_dir, db.clone()).await?;
    Ok((cache, db))
}
    ...
    for (i, frame) in frames.iter().enumerate() {
        if let Some(prev) = prev_timestamp {
            if frame.timestamp > prev {
                is_ordered = false;
                println!(
                    "âŒ ordering violation at index {}: {} > {} (should be descending)",
