...
//! Tests for `strip_heavy_text_in_range` â€” the DB primitive behind the "lean"
...
async fn strip_heavy_text_preserves_elements_of_kept_frame_referencing_old_anchor() {
    ...
    // The element must survive â€” migrated onto the kept frame 2, not deleted.
    assert_eq!(
        count(&db, "SELECT COUNT(*) FROM elements WHERE frame_id = 2").await,
        1,
...
/// the database runs with `auto_vacuum = NONE` (SQLite default â€” nothing in the
...
async fn database_auto_vacuum_is_none_so_file_does_not_self_shrink() {
    ...
    assert_eq!(
        ...
        "auto_vacuum is NONE: lean halts growth + reuses freed pages, but the \
         file only shrinks on a full VACUUM â€” keep the UI copy honest about this"
    );
}

/// `compact()` (the `POST /data/compact` action) must actually return freed
/// pages to the OS via a full VACUUM â€” the reclaim that the no-op
...
async fn compact_returns_free_pages_to_the_os_after_deletion() {
    ...
    // Delete everything â†’ pages go on the free list (auto_vacuum=NONE).
