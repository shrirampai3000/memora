...
use memora_engine::video_utils::{extract_frame, extract_frames_from_video};
use memora_screen::capture_screenshot_by_window::CapturedWindow;
#[cfg(target_os = "macos")]
use memora_screen::perform_ocr_apple;
...
async fn create_test_video() -> Result<PathBuf> {
    let MEMORA_dir = paths::default_MEMORA_data_dir().join("data");

    info!("looking for monitor video in {}", MEMORA_dir.display());

    // Read directory and find first video containing "monitor"
    let mut entries = tokio::fs::read_dir(&MEMORA_dir).await?;
    ...
    Err(anyhow::anyhow!(
        "no monitor video found in MEMORA data dir"
    ))
}
...
#[tokio::test]
#[ignore = "requires local MEMORA data"]
...
async fn test_get_video_metadata() -> Result<()> {
    ...
    let metadata =
        memora_engine::video_utils::get_video_metadata(video_path.to_str().unwrap()).await?;
