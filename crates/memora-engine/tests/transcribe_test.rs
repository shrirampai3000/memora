...
mod tests {
    ...
    async fn setup_test_app() -> Router {
        ...
        let audio_manager = Arc::new(
            AudioManagerBuilder::new()
                .output_path("/tmp/MEMORA-test".into())
                .build(db.clone())
                .await
                .unwrap(),
        );
    ...
    async fn test_transcribe_real_audio_returns_text() {
        ...
        // Use the test WAV file from MEMORA-audio
        let test_file = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .unwrap()
            .join("MEMORA-audio/test_data/poetic_kapil_gupta.wav");
    ...
    async fn test_transcribe_accepts_openai_compat_fields() {
        ...
        let response = app.oneshot(request).await.unwrap();
        // Should not fail due to unknown fields â€” either 200 or 500 (no engine configured)
        // but NOT 400
