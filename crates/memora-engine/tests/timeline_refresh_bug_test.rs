...
mod tests {
    ...
    /// TEST 9: Test MEMORA app filtering logic
    ///
    /// Frames from MEMORA app should be filtered out.
    /// This tests the filtering function directly.
    #[tokio::test]
    async fn test_MEMORA_app_filtering() {
        // Simulate the filtering logic from create_time_series_frame
        fn should_include_entry(app_name: &str) -> bool {
            !app_name.to_lowercase().contains("MEMORA")
        }
        ...
        // MEMORA variants should be filtered out
        assert!(
            !should_include_entry("MEMORA"),
            "MEMORA should be filtered"
        );
        assert!(
            !should_include_entry("MEMORA"),
            "MEMORA should be filtered"
        ...
        assert!(
            !should_include_entry("MEMORA-app"),
            "MEMORA-app should be filtered"
        );
    }

    /// TEST 10: Test that frames with all-MEMORA entries result in empty frame_data
    ///
    /// When all OCR entries in a frame are from MEMORA, after filtering,
    /// the frame_data should be empty and the frame should NOT be sent.
    #[tokio::test]
    async fn test_all_MEMORA_entries_results_in_empty_frame() {
        ...
        let filtered: Vec<_> = entries
            .into_iter()
            .filter(|(app_name, _)| !app_name.to_lowercase().contains("MEMORA"))
            .collect();

        assert!(
            filtered.is_empty(),
            "All MEMORA entries should be filtered out, resulting in empty frame_data"
        );
    ...
    /// TEST 11: Test mixed entries (some MEMORA, some other apps)
    ///
    /// When a frame has mixed entries, only non-MEMORA entries should remain.
    #[tokio::test]
    async fn test_mixed_entries_partial_filtering() {
        ...
        assert_eq!(filtered.len(), 2, "Should have 2 non-MEMORA entries");
    ...
    async fn test_empty_frame_data_should_be_skipped() {
        ...
        // Frame with empty data (all MEMORA filtered) - should NOT be sent
