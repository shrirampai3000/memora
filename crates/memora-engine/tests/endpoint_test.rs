...
mod tests {
    ...
    use MEMORA_audio::audio_manager::AudioManagerBuilder;
    use memora_db::{ContentType, DatabaseManager, SearchResult};
    use memora_engine::routes::search::SearchResponse;
    use memora_engine::SCServer;
    use memora_engine::{ContentItem, PaginatedResponse};
    use memora_screen::OcrEngine; // Adjust this import based on your actual module structure
    use memora_semantic::{
    ...
    async fn setup_test_app() -> (Router, Arc<DatabaseManager>) {
        let unique_suffix = std::time::SystemTime::now()
            ...
            .unwrap_or_default();
        let MEMORA_dir = std::env::temp_dir().join(format!(
            "MEMORA-endpoint-test-{}-{unique_suffix}",
            std::process::id()
        ));
        ...
        let audio_manager = Arc::new(
            AudioManagerBuilder::new()
                .is_disabled(true)
                .output_path(MEMORA_dir.join("audio"))
                .build(db.clone())
                .await
                .unwrap(),
        );
    ...
    async fn frame_thumbnail_endpoint_resizes_caches_and_invalidates_snapshot() {
        ...
        assert_eq!(first.headers()["x-MEMORA-thumbnail-cache"], "miss");
        assert_eq!(first.headers()["x-MEMORA-thumbnail-width"], "384");
        assert_eq!(first.headers()["x-MEMORA-thumbnail-height"], "216");
        ...
        assert_eq!(second.headers()["x-MEMORA-thumbnail-cache"], "hit");
        ...
        assert_eq!(replaced.headers()["x-MEMORA-thumbnail-cache"], "miss");
        assert_eq!(replaced.headers()["x-MEMORA-thumbnail-width"], "384");
        assert_eq!(replaced.headers()["x-MEMORA-thumbnail-height"], "288");
    ...
    async fn frame_thumbnail_endpoint_caches_legacy_video_frame() {
        ...
        let ffmpeg = memora_core::find_ffmpeg_path().expect("ffmpeg is required by the engine");
        let output = memora_core::ffmpeg_cmd(ffmpeg)
            .args([
                "-f",
                "lavfi",
                "-i",
    ...
    async fn test_search_audio_with_length_constraints() {
        ...
        let _ = db
            .insert_audio_transcription(
                ...
                0,
                "",
                &memora_db::AudioDevice {
                    name: "test1".to_string(),
                    device_type: memora_db::DeviceType::Input,
                },
                None,
                None,
                None,
    ...
    async fn test_recent_tasks_no_bleeding_production_db() {
        ...
        let db_path = format!("{}/.memora/db.sqlite", home);
    ...
    async fn on_demand_ocr_waits_for_the_shared_capture_permit() {
        ...
        let permit = memora_capture::ocr_semaphore().acquire().await.unwrap();
        ...
        // empty text â€” the sharper proof that the flag itself gates the write is
