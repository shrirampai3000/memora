...
use memora_engine::compaction_encoder::CompactionEncoder;
use memora_engine::video_utils::extract_frame_from_video;
use tokio::io::AsyncWriteExt;

/// Locate an already-installed ffmpeg without invoking MEMORA's production
...
fn find_installed_ffmpeg() -> Option<PathBuf> {
    ...
    candidates.into_iter().find(|candidate| {
        memora_core::ffmpeg_cmd(candidate)
            .arg("-version")
            .output()
...
/// Full decode to the null muxer â€” proves the produced file is a playable
/// video, exactly what frame extraction relies on.
async fn assert_decodable(ffmpeg: &Path, file: &Path) {
    let status = memora_core::ffmpeg_cmd_async(ffmpeg)
        .args(["-v", "error", "-i"])
        .arg(file)
        .args(["-f", "null", "-"])
