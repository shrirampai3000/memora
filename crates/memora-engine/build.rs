...
// `MEMORA-audio` configures `ort` with `load-dynamic` on Windows (both x86_64 and
// aarch64), so onnxruntime.dll is opened via LoadLibrary at runtime â€” no `.lib`
// import library is needed when invoking link.exe. The DLL is staged next to the
// binary by the workflow (and by `install_onnxruntime()` in MEMORA-audio's
// build.rs for local Windows devs).

fn main() {
    #[cfg(target_os = "macos")]
    {
        // Swift runtime rpath â€” Swift dylibs (libswift_Concurrency etc.) used by
    ...
    // future for the ~2300-line async main in src/bin/memora-engine.rs sits
