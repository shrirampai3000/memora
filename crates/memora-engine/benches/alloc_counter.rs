...
//!
//! Run with:
//!   cargo bench -p memora-engine --bench alloc_counter

use chrono::Utc;
use criterion::{black_box, criterion_group, criterion_main, Criterion};
use memora_engine::hot_frame_cache::{HotAudio, HotFrame};
...
unsafe impl GlobalAlloc for CountingAllocator {
    ...
    unsafe fn realloc(&self, ptr: *mut u8, layout: Layout, new_size: usize) -> *mut u8 {
        // realloc is effectively a dealloc + alloc â€” count it
        ALLOC_COUNT.fetch_add(1, Ordering::Relaxed);
...
fn bench_hotframe_clone_allocs(c: &mut Criterion) {
    ...
    c.bench_function("alloc_count/HotFrame::clone", |b| {
        b.iter_custom(|iters| {
            ...
            eprintln!(
                "\n  HotFrame::clone Ã— {} â†’ {} allocs, {} bytes ({} allocs/call, {} bytes/call)",
                iters,
                count,
                bytes,
...
fn bench_hotframe_construct_allocs(c: &mut Criterion) {
    c.bench_function("alloc_count/HotFrame::construct", |b| {
        b.iter_custom(|iters| {
            ...
            let (count, bytes) = read_counters();
            eprintln!(
                "\n  HotFrame::construct Ã— {} â†’ {} allocs, {} bytes ({} allocs/call, {} bytes/call)",
                iters,
                count,
                bytes,
                count / iters as usize,
...
fn bench_hotaudio_clone_allocs(c: &mut Criterion) {
    let audio = HotAudio {
        ...
        is_input: true,
        audio_file_path: "/Users/user/Library/Application Support/MEMORA/data/input_1.mp4".into(),
        duration_secs: 30.0,
        start_time: Some(0.0),
        end_time: Some(30.0),
        speaker_id: Some(1),
    ...
    c.bench_function("alloc_count/HotAudio::clone", |b| {
        b.iter_custom(|iters| {
            ...
            let (count, bytes) = read_counters();
            eprintln!(
                "\n  HotAudio::clone Ã— {} â†’ {} allocs, {} bytes ({} allocs/call, {} bytes/call)",
                iters,
                count,
                bytes,
                count / iters as usize,
