...
//!
//! Run with:
//!   cargo bench -p memora-engine --bench hot_frame_cache

use chrono::{Duration, Utc};
use criterion::{black_box, criterion_group, criterion_main, BenchmarkId, Criterion};
use memora_engine::hot_frame_cache::{HotAudio, HotFrame, HotFrameCache};
...
fn make_frame(id: i64) -> HotFrame {
    HotFrame {
        ...
        window_name: "MEMORA â€” Memory Optimization - Google Chrome".into(),
        ocr_text_preview: "The quick brown fox jumps over the lazy dog. "
            .repeat(4)
            .into(),
        snapshot_path: format!(
            "/Users/user/Library/Application Support/MEMORA/data/monitor_0_{}.jpg",
            id
        )
        .into(),
        browser_url: Some("https://github.com/mediar-ai/MEMORA".into()),
...
fn make_audio(id: i64, frame_ts: chrono::DateTime<Utc>) -> HotAudio {
    HotAudio {
        ...
        transcription: "Hello, this is a sample transcription for benchmarking purposes. It represents real-world audio captured by MEMORA.".into(),
        device_name: "MacBook Pro Microphone".into(),
        is_input: true,
        audio_file_path: format!("/Users/user/Library/Application Support/MEMORA/data/input_{}.mp4", id).into(),
        duration_secs: 30.0,
        start_time: Some(0.0),
        end_time: Some(30.0),
        speaker_id: Some(1),
...
// ---------------------------------------------------------------------------
// 2. push_frame â€” full insert path (acquire write lock + BTreeMap insert +
...
// ---------------------------------------------------------------------------
// 3. get_frames_in_range â€” BTreeMap range query at various cache sizes.
...
// ---------------------------------------------------------------------------
// 4. push_audio â€” measures string clone cost on every transcription insert.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 5. find_audio_near â€” string clone cost of the Â±60s audio lookup that
...
// ---------------------------------------------------------------------------
// 6. evict_range â€” retention / delete path.
// ---------------------------------------------------------------------------
