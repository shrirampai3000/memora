...
//   --depth N            max AX tree walk depth (default 25 â€” matches prod)
//   --scan-timeout-ms N  per-process timeout (default 5000 â€” matches prod)
//   --interval-ms N      delay between scan rounds (default 5000 â€” matches prod)
...
use memora_engine::meeting_watcher::{
...
fn resolve_apps(patterns: &[String]) -> Vec<(i32, String)> {
    ...
    // dedupe patterns â€” keep first match per pattern
...
fn pick_profile<'a>(
    ...
    // default: Teams profile (profile 0) â€” gives the scanner the heaviest signal set to look for
    &profiles[0]
}
...
fn main() {
    ...
    // AX permission probe â€” without this, the scanner returns empty results fast and the
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        println!(
            ...
            if trusted {
                ...
                "NOT GRANTED â€” scans will return empty"
            }
        );
    }
    ...
    println!(
        "WindowServer CPU â€” avg: {:.1}%  p95: {:.1}%  max: {:.1}%  samples: {}",
        ws_avg,
        ws_p95,
        ws_max,
        ws.len()
