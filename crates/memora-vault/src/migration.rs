...
/// Encrypt all files in the MEMORA data directory.
///
/// Uses a journal for crash recovery â€” if interrupted, the next call
...
pub async fn encrypt_data_dir(
    ...
    let journal_path = MEMORA_dir.join(".vault_journal");
        ...
        for file in &remaining {
            ...
            match crypto::encrypt_file(file, &key) {
                ...
                Err(e) => {
                    ...
                    // Don't journal failures â€” they'll be retried next time
...
/// Decrypt all files in the MEMORA data directory.
