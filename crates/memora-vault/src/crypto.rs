...
//!   + file nonce (12 bytes) â€” base nonce for chunk counter
...
pub fn encrypt_file(path: &std::path::Path, key: &[u8; KEY_SIZE]) -> VaultResult<bool> {
    ...
    match result {
        ...
        Err(e) => {
            // Clean up temp file on failure â€” original file is untouched
...
mod tests {
    ...
    fn test_small_encrypt_decrypt_roundtrip() {
        ...
        let plaintext = b"hello MEMORA vault";
    ...
    fn test_file_encrypt_decrypt_roundtrip() {
        ...
        // Encrypt again â€” should skip (idempotent)
        assert!(!encrypt_file(&path, &key).unwrap());
