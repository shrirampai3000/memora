...
//! VaultManager â€” owns lock/unlock lifecycle and encryption state.
...
fn database_files(MEMORA_dir: &std::path::Path) -> impl Iterator<Item = PathBuf> + '_ {
    VAULT_DATABASE_FILES
        .iter()
        .map(|name| MEMORA_dir.join(name))
}
...
pub enum VaultState {
    ...
    None,
    /// Vault is locked â€” all data encrypted, no recording.
    Locked,
    /// Vault is unlocked â€” data accessible, recording active.
    Unlocked,
...
pub struct VaultManager {
    ...
    /// Master key â€” only present while unlocked. Zeroized on lock/drop.
    master_key: Arc<RwLock<Option<Zeroizing<[u8; KEY_SIZE]>>>>,
}

impl VaultManager {
    pub fn new(MEMORA_dir: PathBuf) -> Self {
        let initial_state = if !MEMORA_dir.join("vault.meta").exists() {
            VaultState::None
        } else if MEMORA_dir.join(".vault_locked").exists() {
            // Sentinel file present â†’ vault is locked
            VaultState::Locked
        } else {
            // A crash can happen after either database was encrypted but before
            // the sentinel was written.
            if database_files(&MEMORA_dir)
                .any(|path| path.exists() && crypto::is_encrypted_file(&path).unwrap_or(false))
            {
                VaultState::Locked
            } else {
                // Vault set up, data not encrypted â†’ unlocked (key not in memory,
    ...
    pub fn is_set_up(&self) -> bool {
        self.MEMORA_dir.join("vault.meta").exists()
    }
    ...
    /// Does NOT encrypt existing data â€” call `lock()` after to do that.
    pub async fn setup(&self, password: &str) -> VaultResult<()> {
        ...
        let meta_path = self.MEMORA_dir.join("vault.meta");
    ...
    pub async fn lock(&self, password: &str) -> VaultResult<watch::Receiver<MigrationProgress>> {
        ...
        let data_dir = self.MEMORA_dir.join("data");
        let database_paths: Vec<_> = database_files(&self.MEMORA_dir).collect();
        let sentinel = self.MEMORA_dir.join(".vault_locked");
        let vault_state = self.state.clone();

        *self.state.write().await = VaultState::Migrating { progress: 0.0 };

        tokio::spawn(async move {
            // Write sentinel FIRST â€” if we crash mid-encryption, startup
            ...
            if data_dir.exists() {
                if let Err(e) =
                    migration::encrypt_data_dir(&MEMORA_dir, &data_dir, key_bytes, progress_tx)
                        .await
                {
    ...
    pub async fn unlock(&self, password: &str) -> VaultResult<watch::Receiver<MigrationProgress>> {
        ...
        *self.state.write().await = VaultState::Migrating { progress: 0.0 };
        ...
        tokio::spawn(async move {
            ...
            if data_dir.exists() {
                if let Err(e) =
                    migration::decrypt_data_dir(&MEMORA_dir, &data_dir, key_bytes, progress_tx)
                        .await
                {
