...
//! (`memora_db::OcrTextBlock`). Each block carries the recognized word in
//! a single free-text field â€” `text` â€” plus a pile of geometry / structural
...
//! ## What we redact
//!
//! Only the `text` field of each block â€” the recognized word. Every geometry
//! field is left **byte-for-byte** untouched, because:
//!   * the text-overlay rendering needs the boxes to position highlights, and
//!   * the image-PII (blur) worker is unaffected â€” it detects regions directly
...
//! ## How
//!
//! `text_json` is structurally identical to `accessibility_tree_json` â€” an
//! array of objects where a single allowlisted key holds free text â€” so this
...
//!
//! ## Known limitation â€” multi-word PII leaks (issue #4117 follow-up)
//!
...
//! (apple-native: `memora-screen/src/apple.rs` splits each Vision
...
//!
//!   * **Single-token values redact correctly** â€” `alice@example.com`,
//!     URLs, `sk-proj-â€¦` keys, `4111111111111111` have no internal
//!     whitespace, so they sit in one block and the map matches them.
//!   * **Whitespace-separated values survive as fragments** â€” a name
...
//!     â€” i.e. reconstructible, NOT merely loose boxes.
//!
...
//! over-redaction risk, or a direct redactor pass per frame) â€” tracked as a
...
/// image-blur worker never reads â€” so the allowlist is exactly this one key.
...
mod tests {
    ...
    /// Only the `text` KEY is scrubbed â€” a geometry field whose string value
    ...
    /// Malformed JSON returns Err (worker leaves the row pending) â€” never a
    /// panic.
    #[test]
    ...
    impl Redactor for StubEnclaveRedactor {
        ...
        // redact_with_map left as the trait default (None) â†’ enclave arm.
    }
