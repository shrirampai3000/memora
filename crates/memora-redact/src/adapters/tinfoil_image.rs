...
//! attestation document â€” so a compromised CA can't MITM the bytes
...
//! either consumer-CPU OPF or M-series-CoreML rfdetr â€” and avoids
...
//!
//! 200 OK
//! { "detections": [ {"bbox":[x,y,w,h],"label":"private_person","score":0.95}, â€¦ ],
//!   "latency_ms": 8.2,
//!   "model": "rfdetr_v8" }
//! ```
...
// pass per frame, no per-row tokenization) â€” but allow generous
...
/// Same rationale as the text adapter â€” see
...
pub struct TinfoilImageConfig {
    ...
    /// `MEMORA_PRIVACY_FILTER_ENCLAVE`, then `MEMORA_PRIVACY_FILTER_URL`,
    ...
    /// to `MEMORA_PRIVACY_FILTER_REPO`, then [`DEFAULT_REPO`].
    pub repo: Option<String>,
    /// Bearer token. Falls back through `MEMORA_PRIVACY_FILTER_API_KEY`,
    /// `MEMORA_LOCAL_API_KEY`, then `MEMORA_API_AUTH_KEY` (deprecated alias).
...
impl TinfoilImageRedactor {
    pub fn new(cfg: TinfoilImageConfig) -> Self {
        ...
        let (bearer, has_auth) = match api_key.as_deref() {
            ...
            None => {
                tracing::info!(
                    "tinfoil_image redactor has no api key â€” requests will be un-authenticated. \
                     Set MEMORA_PRIVACY_FILTER_API_KEY or pass via TinfoilImageConfig.api_key."
                );
                (None, false)
            }
        };
    ...
    /// [`crate::adapters::tinfoil::TinfoilRedactor::http`] â€” re-attests
        ...
        let resp = match req.send().await {
            ...
            Err(e) => {
                if looks_like_attestation_drift(&e) {
                    tracing::warn!(
                        error = %e,
                        "tinfoil_image: attestation drift detected â€” invalidating cached client; \
