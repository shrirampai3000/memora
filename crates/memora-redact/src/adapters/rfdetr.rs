...
//! [MEMORA-pii-bench-image](https://github.com/MEMORA/MEMORA-pii-bench-image)
//! training pipeline. Same Execution-Provider chain as
//! [`crate::adapters::onnx`] (CoreML on Mac, DirectML on Windows, CPU
//! baseline) â€” no CUDA / Vulkan / GPU-vendor SDKs.
//!
//! ## Status
//!
...
//!
//! `rfdetr_v38` (512Ã—512 input, FP16 ONNX with fp32 I/O wrapper, ~60 MB,
...
//!   larger backbone â€” accuracy here is data-limited, not
...
//! and OCR'd to judge it â€” the eval that actually predicts field
//! behaviour):
//!
//! ```text
...
//! ```
//!
//! Every model before v38 â€” including what shipped â€” fired hundreds of
...
//! our own generator â€” held out, but not a real screen, and it flatters
...
//! training weights (`last.ckpt` â€” NOT `checkpoint_best_ema.pth`,
//! whose best-val selection can freeze at epoch 0), then wrap I/O back
//! to fp32 Casts â€” this adapter feeds f32 and extracts f32.
...
/// Class index â†’ [`SpanLabel`]. Order is fixed by the training pipeline:
/// `MEMORA-pii-bench-image/src/score.py::CANONICAL_LABELS`.
#[cfg(feature = "onnx-cpu")]
...
/// detections score ~0.5â€“0.7 while zero secret false fires were
/// observed at conf 0.50 across every zero-PII suite â€” a high floor
...
pub struct RfdetrConfig {
    /// Path to `rfdetr_vN.onnx`. We default to `~/.memora/models/rfdetr_v38.onnx`
    ...
    /// v9, 512 for v11). Must match the model â€” auto-detected if 0.
    pub input_size: u32,
    ...
    /// â€” keep this one permissive (default 0.10) and tighten via the
    /// policy's `min_score`.
    pub conf_threshold: f32,
    /// Additionally run inference on a 2Ã—2 grid of overlapping tiles, on top
    /// of the whole-frame pass, when the frame is much larger than
    /// `input_size`.
    ///
    /// Squeezing a 1512Ã—948 desktop into 512Ã—512 shrinks a 14 px line of
    ...
    /// unique real PII values, recall on email/phone/secret went 62.5% â†’
    /// 81.2% (email 75â†’92%, secret 1/2â†’2/2) at no cost in precision: on the
    ...
    /// that survive an OCR-and-judge audit nearly double (7 â†’ 13 verified
    /// true positives). Costs 5 forward passes instead of 1 (whole frame +
    /// 4 tiles), 99 â†’ 487 ms/frame at the 2-thread setting this adapter
    /// pins. Gated on frame size and can be turned off.
    ///
    /// Two variants were measured and are worse. A 3Ã—3 grid: core 75%, it
...
impl Default for RfdetrConfig {
    fn default() -> Self {
        Self {
            ...
            // tighten further (e.g. 0.70 paranoid mode) â€” that just
...
impl RfdetrConfig {
    /// `~/.memora/models/rfdetr_v38.onnx`. Created lazily by
    ...
    pub const HF_DOWNLOAD_URL: &'static str =
        "https://huggingface.co/MEMORA/pii-image-redactor/resolve/main/rfdetr_v38.onnx";
    ...
    /// Make sure the ONNX is present on disk. Idempotent â€” does
    ...
    pub async fn ensure_model_present(&self) -> Result<(), RedactError> {
        ...
        tracing::info!(
            ...
            "downloading rfdetr_v38.onnx (~60 MB) â€” first-run only"
        );
...
// â”€â”€â”€ feature-on path: the real implementation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
mod imp {
        ...
        match std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            || -> Result<Session, ort::Error> {
                ...
                let mut builder = Session::builder()?
                    ...
                    // Background batch worker â€” never busy-spin between ops. With
                    ...
                    // 365 CPU-ms at 2 threads â€” 2.7x less CPU â€” while wall
    ...
    impl RfdetrRedactor {
        ...
        /// Recommended call site for production â€” single round trip.
        ...
        /// [`RedactError::Unavailable`] if the file is missing â€” call
        ...
        fn infer(&self, image_path: &Path) -> Result<Vec<ImageRegion>, RedactError> {
            ...
            // nothing, so 4Ã— the compute would buy nothing.
            ...
            // UNION of the whole frame and 2Ã—2 tiles â€” the tiles ADD to the
        ...
        fn infer_window(
            ...
            // ImageNet mean/std â†’ NCHW float32.
    ...
    /// 56.1 % â€” the model has a narrow trained scale band, and content already
    /// inside it is magnified OUT of band by the tile zoom.
    ///
    /// Tiles are 2Ã—2 with ~20 % overlap so a PII string on a seam is still
    ...
    ///
    /// Tiles overlap by design, so the same string is often detected twice â€”
    ...
    impl ImageRedactor for RfdetrRedactor {
        ...
        async fn detect(&self, image_path: &Path) -> Result<Vec<ImageRegion>, RedactError> {
            ...
            // any `&self` without 'static gymnastics â€” the runtime
    ...
    fn detect_input_size(session: &Session) -> Option<u32> {
        ...
        // fall back to 384 and feed a wrong-sized tensor â†’ ort "invalid
...
// â”€â”€â”€ feature-off path: stub so the trait wires without ort â”€â”€â”€â”€â”€â”€â”€â”€
...
mod tests {
    ...
    ///
    /// Dropping the whole-frame pass looks like a free optimisation â€” one
    /// fewer forward pass, and real-frame recall does not move â€” but it
    ...
    /// The 2Ã—2 grid must cover every pixel of the frame. A gap between
        ...
        for (w, h) in [(1512u32, 948u32), (1920, 1080), (2560, 1440), (1023, 767)] {
            ...
            for (px, py) in [
                ...
                assert!(
                    ...
                    "{w}x{h}: pixel ({px},{py}) is covered by no tile â€” blind spot"
                );
            }
    ...
    #[test]
    fn default_path_lives_under_MEMORA_dir() {
        let p = RfdetrConfig::default_model_path();
        let expected_suffix = Path::new(".memora")
            .join("models")
            .join("rfdetr_v38.onnx");
        assert!(
    ...
    async fn ensure_model_present_passes_through_when_file_already_correct() {
        ...
        // Wrong-checksum file â†’ ensure_model_present tries to
