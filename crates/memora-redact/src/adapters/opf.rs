...
//! `MEMORA-pii-redactor-runtime/results/onnx_export_2026-05-08.md`):
//! the structured graph round-trips fine but ORT-CPU runs ~10Ã— slower
...
//! reference within fp32 noise (logit max|Î”| â‰ˆ 3e-5 across 8 layers),
//! beats PyTorch CPU on Mac (74 ms vs 70 ms p50), and matches PyTorch
//! MPS on Metal (41 ms vs 40 ms p50). See `MEMORA/opf-rs` for the
//! full benchmark.
//!
//! ## Status
//!
//! Feature-gated behind `opf-text`. Off by default â€” local dev opts in
//! with `cargo build --features opf-text`. The async PII reconciliation
//! worker is still gated separately at the engine layer (see
//! `apps/memora-app-tauri/src-tauri/src/server_core.rs`); this
...
//! translate to memora-redact's `RedactionOutput` (same shape, but
...
/// canonical MEMORA install layout.
#[derive(Debug, Clone)]
pub struct OpfConfig {
    /// Directory containing `config.json` + `model.safetensors`.
    /// Default: `~/.memora/models/opf-v6`. First-run is created
...
const HF_BASE_URL: &str = "https://huggingface.co/MEMORA/pii-text-redactor/resolve/v6";
...
/// latency â€” acceptable since the worker is async, off the capture
...
impl OpfAdapter {
    ...
    /// model â€” that's deferred to the first [`redact_batch`] call.
    ...
    /// itself doesn't get loaded into memory here â€” that happens on
    ...
    /// forever â€” useful for benchmarks where reload cost would skew
    /// numbers).
    ...
    /// Spawn the idle-unload watchdog. Returns its `JoinHandle` â€”
    ...
    pub fn spawn_idle_unloader(self: Arc<Self>) -> tokio::task::JoinHandle<()> {
        tokio::spawn(async move {
            ...
            // Skip the immediate first tick â€” gives the first batch
            ...
            loop {
                ...
                if state.model.is_some() && state.last_used.elapsed() >= self.idle_timeout {
                    tracing::info!(
                        ...
                        "OPF model idle past threshold â€” unloading to free ~2.8 GB"
                    );
...
impl Redactor for OpfAdapter {
    ...
    // No custom `redact()` â€” inherit the trait default, which forwards
...
mod tests {
    ...
    async fn label_map_is_total() {
        // Compile-time exhaustiveness via match in `map_label` â€” call
