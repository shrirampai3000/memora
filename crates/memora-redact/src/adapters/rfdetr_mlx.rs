...
//! RF-DETR-Nano image-PII detector via [`MEMORA_rfdetr_mlx`] â€”
//! pure-Rust MLX runtime, Apple-Silicon-only.
//!
//! Same model as [`super::rfdetr`], different runtime: ~6Ã— faster than
//! the ONNX + CoreML EP path on M-series hardware (â‰ˆ8 ms vs â‰ˆ54 ms p50
...
//! `huggingface.co/MEMORA/pii-image-redactor` on first run
//! (~108 MB), verifies SHA-256, atomic-renames into
//! `~/.memora/models/rfdetr_v9.safetensors`. Subsequent starts
//! are instant.
...
///
/// Mirrors `memora_config::PARAKEET_MIN_MACOS_MAJOR` â€” mlx-sys's
...
/// gates on [`MIN_MACOS_MAJOR`] â€” same shape as
/// `memora_config::macos_major_version`, inlined here so this
/// crate doesn't pick up memora-config just for one fn. On any
/// other platform this always returns `false` â€” the cfg gates
...
pub struct RfdetrMlxConfig {
    ...
    /// `~/.memora/models/rfdetr_v9.safetensors`.
    pub model_path: PathBuf,
    /// Score floor below which detections are dropped. The downstream
    /// [`crate::image::ImageRedactionPolicy`] applies a second floor â€”
...
impl RfdetrMlxConfig {
    ...
    pub const HF_DOWNLOAD_URL: &'static str =
        "https://huggingface.co/MEMORA/pii-image-redactor/resolve/main/rfdetr_v9.safetensors";
    ...
    /// Make sure the safetensors is present on disk. Idempotent â€”
    ...
    /// [`is_runtime_supported`]) â€” the engine falls through to the
    ...
    pub async fn ensure_model_present(&self) -> Result<(), RedactError> {
        ...
        tracing::info!(
            ...
            "downloading rfdetr_v9.safetensors (~108 MB) â€” first-run only"
        );
...
// â”€â”€â”€ feature-on path: real implementation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
mod imp {
    ...
    /// â†’ reload cost is acceptable, steady-state RAM matters more.
    /// On a 107 MB safetensors the resident footprint is ~150â€“200 MB
    /// including MLX activation buffers â€” small in absolute terms, but
    /// pure waste while recording is paused or the reconciliation queue
    /// has drained.
    const DEFAULT_IDLE_TIMEOUT: Duration = Duration::from_secs(60);
    const IDLE_CHECK_INTERVAL: Duration = Duration::from_secs(30);

    /// Class index â†’ [`SpanLabel`]. Order mirrors
    /// [`crate::adapters::rfdetr::CLASSES`] â€” fixed by the training
    /// pipeline (`MEMORA-pii-bench-image/src/score.py`).
    ...
    type ModelHandle = Arc<StdMutex<MEMORA_rfdetr_mlx::RfDetr>>;
    ...
    impl RfdetrMlxRedactor {
        /// Sync constructor. Validates inputs only â€” does **not** load
        ...
        async fn ensure_loaded(&self) -> Result<ModelHandle, RedactError> {
            ...
            if state.model.is_none() {
                ...
                let model = tokio::task::block_in_place(|| {
                    let mut m = MEMORA_rfdetr_mlx::RfDetr::load(&path)
                ...
                loop {
                    ...
                    if state.model.is_some() && state.last_used.elapsed() >= self.idle_timeout {
                        tracing::info!(
                            ...
                            "rfdetr-mlx idle â€” dropping model (will reload on next detect)"
                        );
        ...
        async fn infer(&self, image_path: &Path) -> Result<Vec<ImageRegion>, RedactError> {
            ...
            tokio::task::block_in_place(move || {
                ...
                // the 12 PII classes â€” the model never returns the
                // no_object slot because postprocess in
                // MEMORA-rfdetr-mlx already excludes it.
...
// â”€â”€â”€ feature-off path: stub â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
mod tests {
    ...
    #[test]
    fn default_path_under_MEMORA_dir() {
        let p = RfdetrMlxConfig::default_model_path();
        let s = p.to_string_lossy();
        assert!(s.contains(".memora/models/rfdetr_v9.safetensors"));
    }
