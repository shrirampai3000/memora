...
//! Local ONNX-runtime inference of the v50 PII redactor â€” a 6-layer
...
//! (see `MEMORA-pii-bench` + the run reports in
//! gs://sp-pii-train-artifacts-2026/).
//!
//! Off by default â€” feature-gated with `onnx-cpu`, `onnx-coreml`, and
...
//!     Windows â€” neither requires the user to install CUDA toolkit or
...
//!   - `model.onnx` (or `model_quantized.onnx` for INT8 â€” config.json
...
//!
//! See `MEMORA-pii-redactor-runtime/artifacts/mdeberta_v45_phase3_onnx_int8/`
...
//! original UTF-8 input, so spans land directly on the input â€” no
...
pub struct OnnxConfig {
    ...
    /// Default 256 â€” matches the v45 training config.
...
impl OnnxConfig {
    /// `~/.memora/models/v50_distilled6l/` by convention.
    pub fn default_model_dir() -> PathBuf {
        dirs::home_dir()
            .map(|h| h.join(".memora").join("models").join("v50_distilled6l"))
            .unwrap_or_else(|| PathBuf::from(".memora/models/v50_distilled6l"))
    }
    ...
    /// all bumped together â€” same discipline as `RfdetrConfig`).
    pub const HF_REPO_BASE: &'static str =
        "https://huggingface.co/MEMORA/pii-redactor/resolve/main/v50_distilled6l";
    ...
    /// pull) instead of pseudo-labeled by v45_phase5 â€” whose own false
    ...
    /// While the SHAs are placeholders, verification is skipped â€” the
...
/// Map `"private_person"` â†’ [`SpanLabel::Person`], etc. Unknown labels
...
// =========================================================================
// Stub fallback when ONNX features are off â€” pipeline gets Unavailable
...
mod runtime {
    ...
    pub struct OnnxRedactor {
        ...
        tokenizer: Tokenizer,
        /// `id â†’ "B-private_person"` from config.json.
        id2label: Vec<String>,
        /// For vocab-pruned models: maps full-tokenizer ids â†’ the model's
    ...
    impl OnnxRedactor {
        ...
        pub fn load(cfg: OnnxConfig) -> Result<Self, RedactError> {
            ...
            // so the tokenizer must hand us *every* token â€” otherwise PII
        ...
        /// windows instead of being truncated â€” otherwise PII past the
        ...
        fn infer(&self, text: &str) -> Result<RedactionOutput, RedactError> {
            ...
            let mut spans = if ids.len() <= max_len {
                // Fits in one window â€” fast path, identical to before.
            ...
            // Windows overlap (and may split an entity at an edge) â†’ sort
        ...
        fn run_window(&self, ids: &[u32], mask: &[u32]) -> Result<Vec<usize>, RedactError> {
            let len = ids.len();
            // Vocab-pruned models: remap full-tokenizer ids â†’ sliced rows
            // (ids outside the kept set â†’ the pruned UNK row). Byte offsets are
            // unaffected â€” they index `text`, not the model vocab â€” so decoding
            ...
            let logits = logits_view.index_axis(Axis(0), 0); // drop batch dim â†’ [seq_len, num_labels]
        ...
        /// encode's `<s>` / `</s>` at each window edge) so every token â€”
        /// including PII at the very end â€” is inspected. Returns the
        ...
        fn infer_windowed(
            ...
            let n = ids.len();
            // Special tokens framing the full encode (xlm-roberta: <s> â€¦ </s>).
            let bos = ids[0];
            let eos = ids[n - 1];
        ...
        for (i, &id) in label_ids.iter().enumerate() {
            ...
            match prefix {
                ...
                "I" => {
                    ...
                    // I- without matching B- â€” treat as new span.
                    flush(&mut cur, &mut out, text);
                    cur = Some((label, off.0, off.1));
                }
                _ => {
    ...
    /// earlier span's label wins â€” the union of both ranges is still
    /// fully covered.
    ...
    /// `{"remap": {"<old_id>": <new_row>, â€¦}, "unk_new": <row>}`.
        ...
        match std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            || -> Result<Session, ort::Error> {
                ...
                let mut builder = Session::builder()?
                    ...
                    // This session serves a background batch worker â€” never let the
