...
//!
//! - Emails, US/intl phone numbers (with separators â€” see issue #2340 for
//!   the bare-digit false-positive that the original MEMORA regex
//!   triggered)
//! - JWTs, common API-key prefixes (`sk-`, `sk_live_`, `ghp_`, `xoxb-`,
//!   `AKIAâ€¦`, `ya29.â€¦`, `hf_â€¦`), private-key block markers
...
struct Pattern {
    ...
    /// plausibly an order ID as a Canada SIN â€” issue #2340).
...
static PATTERNS: Lazy<Vec<Pattern>> = Lazy::new(|| {
    let raw: &[(&str, SpanLabel)] = &[
        ...
        // Generic URL with `user:pass@host` â€” keep AFTER the more
        ...
        // OpenAI sk-â€¦ / sk-proj-â€¦
        (
            r"\bsk-(?:proj-|ant-)?[A-Za-z0-9_-]{16,}\b",
            SpanLabel::Secret,
        ),
        // Stripe sk_live_â€¦ / sk_test_â€¦ / rk_live_â€¦
        (
        ...
        // Phone â€” REQUIRE separator/parens/+ to avoid grabbing 9-digit
    ...
    let detailed: &[DetectorSpec] = &[
        ...
        // MAC address â€” hex pairs with separators; nothing else looks like it.
        (
        ...
        // US SSN (formatted) â€” shape is specific enough on its own.
        (
            r"\b\d{3}-\d{2}-\d{4}\b",
            SpanLabel::Id,
            Some("us_ssn"),
        ...
        // IBAN â€” contiguous form only (the spaced/grouped form needs a
        ...
        // ISIN â€” country + 9-char NSIN + Luhn-over-expanded check digit.
        (
        ...
        // VIN â€” 17 chars (no I/O/Q), transliteration mod-11.
        (
        ...
        // CUSIP â€” 9 chars, custom mod-10. Context-gated (9-char tokens are
        ...
        // SEDOL â€” 7 chars, weighted mod-10. Context-gated.
        (
        ...
        // Credit / debit card â€” brand IIN + length + Luhn (the brand gate
        ...
        // Brazil CPF â€” 11 digits, two mod-11 check digits.
        (
        ...
        // India Aadhaar â€” 12 digits, Verhoeff check.
        (
            r"\b\d{4} ?\d{4} ?\d{4}\b",
            SpanLabel::Id,
            Some("india_aadhaar"),
        ...
        // Spain DNI / NIF â€” 8 digits + mod-23 control letter.
        (
            r"\b\d{8}[A-Za-z]\b",
            SpanLabel::Id,
            Some("spain_dni"),
        ...
        // Spain NIE â€” X/Y/Z + 7 digits + mod-23 letter. Distinctive prefix.
        (
        ...
        // US NPI â€” 10 digits (first 1/2), Luhn over 80840 prefix.
        (
            r"\b[12]\d{9}\b",
            SpanLabel::Id,
            Some("us_npi"),
        ...
        // US DEA â€” 2 letters + 7 digits, custom mod-10.
        (
            r"\b[A-Za-z]{2}\d{7}\b",
            SpanLabel::Id,
            Some("us_dea"),
        ...
        // Netherlands BSN â€” 8-9 digits, elfproef. Context-gated.
        (
            r"\b\d{8,9}\b",
            SpanLabel::Id,
        ...
        // US ABA routing â€” 9 digits, 3-7-1 mod-10. Context-gated.
        (
            r"\b\d{9}\b",
            SpanLabel::Id,
            Some("us_aba_routing"),
        ...
        // Australia TFN â€” 9 digits, weighted mod-11. Context-gated.
        (
            r"\b\d{3} ?\d{3} ?\d{3}\b",
            SpanLabel::Id,
            Some("australia_tfn"),
        ...
        // Canada SIN â€” 9 digits, Luhn. Weak checksum â†’ context-gated.
        (
            r"\b\d{3}[- ]?\d{3}[- ]?\d{3}\b",
            SpanLabel::Id,
            Some("canada_sin"),
        ...
        // IMEI â€” 15 digits, Luhn. Context-gated to avoid 15-digit Amex.
        (
            r"\b\d{15}\b",
            SpanLabel::Id,
            Some("imei"),
        ...
        // China resident ID â€” 17 digits + 0-9/X check, ISO 7064 MOD 11-2.
        (
            r"\b\d{17}[\dXx]\b",
            SpanLabel::Id,
        ...
        // Italy Codice Fiscale â€” 6 letters + 9 alnum + letter, mod-26.
        (
        ...
        // Mexico CURP â€” distinctive 18-char shape. Format/context-only:
        ...
        // Finland HETU â€” DDMMYY + century sign + NNN + mod-31 check char.
        (
        ...
        // France NIR â€” 15 digits starting 1/2, mod-97 key.
        (
            r"\b[12]\d{14}\b",
            SpanLabel::Id,
            Some("france_nir"),
        ...
        // Germany Steuer-ID â€” 11 digits, ISO 7064 MOD 11,10.
        (
            r"\b\d{11}\b",
            SpanLabel::Id,
            Some("germany_tax_id"),
        ...
        // Poland PESEL â€” 11 digits, weighted mod-10.
        (
            r"\b\d{11}\b",
            SpanLabel::Id,
            Some("poland_pesel"),
        ...
        // Turkey TC Kimlik â€” 11 digits, custom checks.
        (
            r"\b\d{11}\b",
            SpanLabel::Id,
        ...
        // Belgium national number â€” 11 digits, mod-97.
        (
        ...
        // Norway fÃ¸dselsnummer â€” 11 digits, two mod-11.
        (
            r"\b\d{11}\b",
            SpanLabel::Id,
            Some("norway_fodselsnummer"),
            &["fÃ¸dselsnummer", "fodselsnummer", "fnr"],
            Some(national_id::norway_fodselsnummer),
        ),
        // South Africa ID â€” 13 digits, Luhn.
        (
            r"\b\d{13}\b",
            SpanLabel::Id,
        ...
        // South Korea RRN â€” 13 digits, weighted mod-11.
        (
            r"\b\d{6}-?\d{7}\b",
            SpanLabel::Id,
            Some("south_korea_rrn"),
            &["resident registration", "rrn", "ì£¼ë¯¼"],
            Some(national_id::south_korea_rrn),
        ),
        // Sweden personnummer â€” 10/12 digits, Luhn.
        (
            r"\b(?:\d{2})?\d{6}[-+]?\d{4}\b",
            SpanLabel::Id,
        ...
        // UK UTR â€” 10 digits, weighted mod-11.
        (
            r"\b\d{10}\b",
            SpanLabel::Id,
            Some("uk_utr"),
        ...
        // Australia Medicare â€” 10-11 digits (first 2-6), weighted mod-10.
        (
            r"\b[2-6]\d{9,10}\b",
            SpanLabel::Id,
        ...
        (
            ...
            SpanLabel::Id,
            Some("hungary_vat"),
            &["kÃ¶zÃ¶ssÃ©gi adÃ³szÃ¡m", "anum", "afa", "vat"],
        ...
        (
            ...
            &["code melli", "national code", "Ú©Ø¯ Ù…Ù„ÛŒ"],
        ...
        (
            ...
            SpanLabel::Id,
            Some("ukraine_rnokpp"),
            &["rnokpp", "ipn", "Ñ–Ð´ÐµÐ½Ñ‚Ð¸Ñ„Ñ–ÐºÐ°Ñ†Ñ–Ð¹Ð½Ð¸Ð¹"],
        ...
        (
            ...
            SpanLabel::Id,
            Some("panama_cedula"),
            &["cedula", "cÃ©dula", "panama", "tribunal electoral"],
            None,
        ),
        ...
        // UK NINO â€” 2 prefix letters + 6 digits + A-D suffix.
        (
        ...
        // India PAN â€” 5 letters + 4 digits + letter.
        (
            r"\b[A-Z]{5}\d{4}[A-Z]\b",
            SpanLabel::Id,
            Some("india_pan"),
        ...
        // SWIFT/BIC â€” 8 or 11 chars. Heavily context-gated (8 caps is common).
        (
        ...
        // US EIN â€” XX-XXXXXXX. Very generic; must be context-gated.
        (
            r"\b\d{2}-\d{7}\b",
            SpanLabel::Id,
            Some("us_ein"),
        ...
        // IPv6 â€” full 8-group form only. The compressed `::` form is
        ...
        // ICCID (SIM serial) â€” 19-20 digits, prefix 89, Luhn.
        (
            r"\b89\d{17,18}\b",
            SpanLabel::Id,
            Some("iccid"),
        ...
        // Bitcoin legacy address â€” Base58Check (1.../3...).
        (
        ...
        // Litecoin legacy address â€” Base58Check (L.../M...).
        (
        ...
        // Bitcoin SegWit/Taproot â€” bech32/bech32m (bc1.../tb1...), BCH
        ...
        // Ethereum address â€” 0x + 40 hex, EIP-55 Keccak checksum.
        (
        ...
        // XRP (Ripple) classic address â€” Base58Check, Ripple dictionary.
        (
        ...
        // IMSI â€” 15 digits, context-gated (shares the shape with IMEI).
        (r"\b\d{15}\b", SpanLabel::Id, Some("imsi"), &["imsi"], None),
        // US passport â€” 1 alnum + 8 digits, context-gated.
        (
            r"\b[A-Za-z0-9]\d{8}\b",
            SpanLabel::Id,
            Some("us_passport"),
        ...
        // ICD-10 diagnosis code â€” letter + 2 alnum + optional .subcode.
        (
...
struct Engine {
    ...
    /// gate is O(line length) instead of O(patterns Ã— keywords).
...
/// Single-text path â€” synchronous. Public for use inside the
...
pub fn redact_one(text: &str) -> RedactionOutput {
    ...
    for idx in cand {
        ...
        for m in pat.re.find_iter(text) {
            ...
            // Structural validator (checksum) â€” drop shapes that don't
            ...
            // Context requirement â€” weak-checksum numeric IDs only count
...
/// precede ("SIN: â€¦") but sometimes follow ("â€¦ (SIN)"), so both windows
...
fn render_redacted(text: &str, spans: &[RedactedSpan]) -> String {
    ...
    for span in spans {
        if span.start < cursor {
            // overlapping â€” caller filtered, but defensive
            continue;
        }
...
mod tests {
    ...
    fn luhn_filters_long_db_ids() {
        // 16 digits, fails Luhn â†’ must NOT be flagged as Id/card.
    ...
    fn luhn_passes_real_card_shape() {
        // Visa test number â€” passes Luhn.
    ...
    #[test]
    fn negative_no_pii() {
        let out = run("Cursor â€” Welcome");
        assert_eq!(out.spans.len(), 0);
        assert_eq!(out.redacted, "Cursor â€” Welcome");
    }
    ...
    fn iban_bad_checksum_rejected() {
        // Last digit flipped â†’ fails mod-97 â†’ must not be flagged.
    ...
    fn weak_checksum_ids_need_context() {
        // A bare 9-digit Luhn number is ambiguous â€” must NOT fire alone.
        let bare = run("046 454 286");
        assert!(!has_subtype(&bare, "canada_sin"), "spans: {:?}", bare.spans);
        // Same number next to its label â†’ recognized.
    ...
    fn unicode_context_window_is_case_insensitive() {
        ...
        let after = "1712345675 CÃ‰DULA";
        let after_end = "1712345675".len();
        assert!(has_context(after, 0, after_end, &["cÃ©dula"]));
    }

    #[test]
    fn unicode_context_keywords_are_case_insensitive() {
        for label in ["cÃ©dula", "CÃ‰DULA"] {
        ...
        let distant_seed = format!("registro civil {} CÃ‰DULA 1712345675", "x".repeat(64));
        assert!(
    ...
    fn spain_dni_and_cpf_caught() {
        ...
        // Wrong control letter â†’ mod-23 rejects.
    ...
    fn throughput_on_realistic_corpus() {
        // PII-free chrome / code / logs â€” the dominant real workload that
        // the RegexSet gate should let us skip the per-pattern scans on.
        let pii_free: Vec<String> = [
            "Cursor â€” main.rs â€” MEMORA",
        ...
        let mixed: Vec<String> = [
            ...
            "Order #2581473960 shipped â€” tracking 1Z999AA10123456784",
        ]
        .iter()
        .map(|s| s.to_string())
        .collect();
        ...
        // Regression guard â€” release only (debug is unoptimized and ~10x
