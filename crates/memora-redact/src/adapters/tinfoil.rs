...
//! Confidential-compute redactor backed by the MEMORA-hosted
//! Tinfoil enclave running the privacy-filter model.
//!
//! Wire format kept compatible with the existing
//! `memora-engine::privacy_filter` HTTP shape so we can phase the
...
//! 2. The `MEMORA_PRIVACY_FILTER_API_KEY` env var.
//! 3. The `MEMORA_LOCAL_API_KEY` env var (the local-API bearer the
//!    rest of the codebase uses), with `MEMORA_API_AUTH_KEY` honored
//!    as a deprecated alias.
//!
//! If none is set, requests still go out un-authenticated â€” the
//! adapter logs a warning at construction time so misconfiguration is
//! visible. The previous `memora-engine::privacy_filter`
...
//!      report, VCEKâ†’ASKâ†’ARK back to AMD's root of trust);
//!   2. Sigstore code-provenance check â€” verifies the latest GitHub
...
//! document, so a compromised CA can't MITM. Failures fail-closed â€”
...
//! constructing the redactor is cheap â€” important because the worker
...
//! - Failures bubble up â€” the reconciliation worker decides whether
...
// reconciliation worker is async background work â€” a long ceiling
...
pub struct TinfoilConfig {
    ...
    /// `MEMORA_PRIVACY_FILTER_ENCLAVE`, then [`DEFAULT_ENCLAVE`].
    /// `MEMORA_PRIVACY_FILTER_URL` is also honored for backwards
    /// compatibility â€” its scheme/path are stripped.
    ...
    /// `MEMORA_PRIVACY_FILTER_REPO`, then [`DEFAULT_REPO`].
    pub repo: Option<String>,
    /// Bearer token for the enclave. Falls back through
    /// `MEMORA_PRIVACY_FILTER_API_KEY`, `MEMORA_LOCAL_API_KEY`,
    /// and `MEMORA_API_AUTH_KEY` (deprecated alias).
...
pub struct TinfoilRedactor {
    enclave: String,
    repo: String,
    /// Bearer header attached per-request â€” the SDK doesn't put the
...
impl TinfoilRedactor {
    ...
    pub fn new(cfg: TinfoilConfig) -> Self {
        let enclave_raw = cfg
            .enclave
            .or_else(|| std::env::var("MEMORA_PRIVACY_FILTER_ENCLAVE").ok())
            .or_else(|| std::env::var("MEMORA_PRIVACY_FILTER_URL").ok())
            .unwrap_or_else(|| DEFAULT_ENCLAVE.to_string());
        let enclave = strip_scheme_and_path(&enclave_raw).to_string();

        let repo = cfg
            .repo
            .or_else(|| std::env::var("MEMORA_PRIVACY_FILTER_REPO").ok())
            .unwrap_or_else(|| DEFAULT_REPO.to_string());

        let api_key = cfg.api_key.or_else(|| {
            std::env::var("MEMORA_PRIVACY_FILTER_API_KEY")
                .ok()
                .or_else(|| std::env::var("MEMORA_LOCAL_API_KEY").ok())
                .or_else(|| std::env::var("MEMORA_API_AUTH_KEY").ok())
        });
        ...
        let (bearer, has_auth) = match api_key.as_deref() {
            ...
            None => {
                tracing::info!(
                    "tinfoil redactor has no api key â€” requests will be un-authenticated. \
                     Set MEMORA_PRIVACY_FILTER_API_KEY or pass via TinfoilConfig.api_key."
                );
                (None, false)
            }
        };
    ...
    /// Default constructor â€” pulls everything from env.
    ...
    async fn http(&self) -> Result<reqwest::Client, RedactError> {
        ...
        // `self.bearer` below â€” pass empty here to keep this
        ...
        // SDK Client â€” we clone (cheap: reqwest::Client is Arc-shaped)
    ...
    /// cert-rotation drift â€” see [`looks_like_attestation_drift`].
    ...
    async fn redact_one(&self, text: &str) -> Result<RedactionOutput, RedactError> {
        ...
        let resp = match req.send().await {
            ...
            Err(e) => {
                ...
                // The reconciliation worker handles the retry â€” we just
                // need to make sure the next attempt isn't stuck on the
                // stale fingerprint.
                if looks_like_attestation_drift(&e) {
                    tracing::warn!(
                        error = %e,
                        "tinfoil: attestation drift detected â€” invalidating cached client; \
...
mod tests {
    ...
    async fn short_text_bypasses_round_trip() {
        ...
        // never gets attested â€” clean check that the short-circuit works.
    ...
    async fn unreachable_enclave_bubbles_up_as_error() {
        ...
        // error propagates â€” exact variant depends on the SDK's
    ...
    async fn no_api_key_means_no_auth() {
        ...
        std::env::remove_var("MEMORA_PRIVACY_FILTER_API_KEY");
        std::env::remove_var("MEMORA_LOCAL_API_KEY");
        std::env::remove_var("MEMORA_API_AUTH_KEY");
        let r = TinfoilRedactor::new(TinfoilConfig {
            enclave: Some("example.invalid".into()),
            repo: Some("test/never".into()),
            api_key: None,
