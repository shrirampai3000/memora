...
/// stable â€” search UIs render against these.
...
pub enum SpanLabel {
    ...
    Phone,
    /// Postal address â€” street, city, postcode, country.
    Address,
    ...
    Secret,
    /// GDPR Art. 9 / HIPAA non-Safe-Harbor PHI â€” sensitive *context*
...
impl SpanLabel {
    ...
    /// unknown names â€” callers decide whether to ignore or error.
...
/// order.
///
/// [`SpanLabel::Secret`] is **always** included regardless of input â€”
...
/// Per-label policy for the text pipeline â€” the text-side mirror of
...
/// safer trade-off right now is **secrets only** â€” that's the class
...
impl TextRedactionPolicy {
    ...
    /// [`allow`](Self::allow) (see [`parse_allow_list`] â€” Secret is always
...
mod tests {
    ...
    fn parse_allow_list_always_includes_secret() {
        // Empty input â†’ just the baseline.
        assert_eq!(parse_allow_list(&[]), vec![SpanLabel::Secret]);
        // All-unknown input â†’ still just the baseline.
        assert_eq!(
