...
//! users want to keep â€” *correlation*. With every name collapsed to the
...
//! derived from a keyed hash of the value: same input â†’ same token
//! (e.g. `[PERSON_1a2b3c4d5e6f]`), different inputs â†’ (almost always)
...
//!   encrypted vault is part B of the issue â€” a separate, higher-risk
//!   feature â€” and is deliberately NOT implemented here.)
...
//! keeps the server-applied static placeholders â€” there is nothing to
...
/// only ~256 distinct values â€” that would merge distinct entities and
...
/// Subdirectory of the MEMORA data dir holding redaction secrets.
...
impl Pseudonymizer {
    ...
    /// machine-local; it must be **stable** â€” a new key reshuffles every
    ...
    pub fn load_or_create(data_dir: &Path) -> std::io::Result<Self> {
        ...
        // *atomically* with owner-only perms â€” `create_new` (O_EXCL) plus
    ...
    ///
    /// Same `(label, subtype, value)` â†’ same token under a given key.
...
/// Human-readable, taxonomy-aligned prefix for a label â€” the static
...
/// to a single space. Case is **preserved** deliberately â€” lowercasing
...
/// if it exists but is the wrong size (corrupt) â€” we never silently
        ...
        return Err(std::io::Error::new(
            ...
            format!(
                "pseudonym key at {} is {} bytes, expected {}; refusing to \
                 regenerate (a new key would break every existing token) â€” \
...
/// (Windows has no mode here â€” it inherits the user-profile dir's ACL.)
...
mod tests {
    ...
    /// changing the keyed hash â€” which would reshuffle every token.
    #[test]
    ...
    fn token_has_label_prefix_and_hex_suffix() {
        ...
        // [EMAIL_ + 12 hex + ]  (TOKEN_BYTES=6 â†’ 12 hex chars).
    ...
    fn whitespace_is_normalized_but_case_is_not() {
        let p = fixed();
        // Trailing / collapsed whitespace â†’ same token.
        assert_eq!(
            p.token(SpanLabel::Person, None, "Marcus  Chen "),
            p.token(SpanLabel::Person, None, "Marcus Chen")
        );
        // Case is preserved â†’ distinct token (no false merge).
        assert_ne!(
            p.token(SpanLabel::Person, None, "marcus chen"),
            p.token(SpanLabel::Person, None, "Marcus Chen")
        );
    ...
    fn load_or_create_persists_a_stable_key() {
        ...
        // Second load reads the same key back â†’ identical tokens.
