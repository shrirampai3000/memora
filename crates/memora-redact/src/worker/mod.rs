...
//! / migration backfill workers in shape â€” pause / resume / status,
...
use memora_resource::ResourceGovernor;
...
/// gone after the UPDATE returns â€” that's the whole point of the
...
pub struct WorkerConfig {
    ...
    /// string in each record â€” independent of `tables`, `columns`, and the
    /// (possibly model-backed) row redactor â€” so a worker that does no DB work
...
impl Worker {
    ...
    /// signal â€” left for tests + the standalone CLI. Production callers
    ...
    async fn run(self, shutdown: Option<Arc<Notify>>) {
        ...
        // image is malformed") won't clear on retry â€” the DB must be
        ...
        // skew â€” e.g. an older engine sharing a `db.sqlite` that a newer one
        ...
        // regex `Pipeline` â€” independent of `self.redactor`, whose policy may be
        // full-PII and/or model-backed â€” so secret-stripping agent logs never
        ...
        loop {
            ...
            for table in &self.cfg.tables {
                ...
                // schema â€” see `is_missing_object` and the Err arm below.
                ...
                match result {
                    ...
                    Some(Err(e)) => {
                        ...
                        if is_missing_object(&e) {
                            ...
                            // for the run and move on â€” the others reconcile
                            // normally. Logged once per target via the disable.
                            warn!(
                                table = ?table,
                                error = %e,
                                "target table/column missing from this schema â€” disabling its \
                                 reconciliation for this run (binary predates a migration that \
                                 retired it?); upgrade MEMORA to clear this"
                            );
                            disabled.push(*table);
                            continue;
                        }
                        if is_db_hard_fault(&e) {
                            ...
                            if !corruption_logged {
                                error!(
                                    table = ?table,
                                    error = %e,
                                    "database hard fault detected â€” backing off reconciliation \
            ...
            // Nothing anywhere had work this sweep â€” idle at the poll
    ...
    /// (MEMORA/website#291); `UiEvents` gets the multi-column
    ...
    /// `help_text` / `role_description` / `url`) â€” the accessibility contents
    ...
    /// be covered by frame propagation â€” it needs direct detection here).
    ///
    ...
    /// back â€” same number of model calls as the old text-only Elements pass,
    ...
    /// redactable field). Malformed `properties` â†’ warn + skip that blob but
    ...
    async fn process_elements(&self) -> Result<u32, anyhow::Error> {
        let cols = self.cfg.columns;
        // Nothing in this table is enabled â†’ don't even fetch.
    ...
    async fn process_ui_events(&self) -> Result<u32, anyhow::Error> {
        ...
        // warned + left as-is rather than wedging the row forever â€” same
                ...
                .await
                {
                    ...
                    Err(e) => {
                        warn!(
                            row_id = row.id,
                            error = %e,
                            "element_ancestors JSON unredactable â€” leaving as captured"
                        );
                    }
                }
            }
    ...
    /// detection pass, propagate the result to that frame's DERIVED copies â€”
        ...
        for row in &rows {
            ...
            match self.redactor.redact_with_map(&augmented.text).await? {
                Some((out, mut map)) => {
                    ...
                    // still needs it (no extra model pass) â€” CRITICAL: before
                    ...
                    // column fetch gate relies on: full_text stamped âŸ¹ every
                    // enabled derived copy is done (malformed blobs excepted â€”
                ...
                None => {
                    ...
                    // derived copies by driving the redactor directly â€”
                    ...
                    // redacted below â€” same as the map arm, and it avoids a
    ...
    /// per-word OCR `text_json` (#4117). Pure string application â€”
    ...
    /// malformed copy is not retried â€” it's malformed, retrying can't fix it.
    ...
    async fn propagate_frame_derived(
        ...
        // accessibility_text âŠ† full_text â€” apply the map verbatim.
        ...
        // accessibility_tree_json â€” scrub the configured node fields field-wise.
            ...
            if let Some(tree) = row.accessibility_tree_json.as_deref() {
                if !tree.is_empty() && row.accessibility_tree_redacted_at.is_none() {
                    match crate::tree_json::redact_tree_json_with_fields(
                        ...
                        // Ok(None) means the map was empty â€” impossible here
        ...
        // window_name â€” short prose, apply the map verbatim.
        if cols.window_name {
        ...
        // browser_url â€” also frames_fts; apply the map verbatim (scrubs
        // on-screen PII in the path/query that's also in full_text). OFF by
        // default â€” URLs are structured and often non-PII (opt-in).
        if cols.browser_url {
        ...
        // text_json â€” per-word OCR boxes; scrub each block's `text` field-wise
        ...
        // OCR words survive here â€” and are served verbatim by the overlay
        // endpoints â€” is exactly the #4117 leak, so the two move together.
    ...
    /// derived copies by driving the redactor directly â€” there is no map to
    ...
    /// redacts + stamps `full_text` â€” same as the map arm. `Err` only on a
    ...
    async fn redact_frame_derived_with_redactor(
            ...
            if let Some(tree) = row.accessibility_tree_json.as_deref() {
                if !tree.is_empty() && row.accessibility_tree_redacted_at.is_none() {
                    ...
                    .await
                    {
                        ...
                        // No redactable text â†’ stamp the verbatim blob so the
        ...
        // text_json â€” per-word OCR boxes; drive the redactor over each block's
        ...
        //
        // NOT gated by a column toggle â€” see the map-path note above: text_json
