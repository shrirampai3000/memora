...
//!
//! 1. **`frames.full_text`** â€” OCR + accessibility screen text, unified on the
...
//! 2. **`audio_transcriptions`** â€” speech-to-text output. Source column
//!    `transcription`.
//! 3. **`frames.accessibility_text`** â€” accessibility-tree text. The
...
//! 4. **`ui_events`** â€” user input events. The same table holds typed
...
//!    target â€” see [`TargetTable::source_cols`] and
//!    [`fetch_unredacted_ui_events`] / [`write_redacted_ui_events`]
//!    (issue #4115).
//! 5. **`elements`** â€” per-element OCR + accessibility rows (issue
...
//!
//! `<redacted_at_col> IS NULL`. Single bit per row â€” no version-based
...
pub enum TargetTable {
    ...
    AudioTranscription,
    /// Accessibility-tree text â€” lives on `frames.accessibility_text`
    ...
    /// `element_value` (the focused form-field's contents â€” the key PII
    /// sink), and `window_title` (app-authored at runtime â€” routinely an
    ...
    /// skip. Multi-column â€” uses the dedicated
...
/// label, and `element_name` is indexed in `ui_events_fts` â€” so a raw copy
...
///
/// - `text_content` â€” typed / keystroke / clipboard text.
/// - `element_value` â€” focused control's contents (the key PII sink).
/// - `window_title` â€” runtime window title (FTS-indexed).
/// - `element_name` â€” AX name (FTS-indexed; can mirror field content).
/// - `element_description` â€” AX description / help text.
/// - `element_ancestors` â€” the ONE non-free-text member: compact JSON
...
impl TargetTable {
    ...
    /// Source column the redactor reads AND overwrites â€” for the
...
/// Fetch up to `limit` rows that need redaction. Newest-first â€” users
...
/// redacts from the SAME detection pass (MEMORA/website#291):
...
/// so every PII value in those derived copies is present in `full_text` â€”
...
pub struct FrameTextRow {
    ...
    /// Window title â€” short prose, indexed in `frames_fts`, so a raw copy
    ...
    /// Browser URL â€” also a `frames_fts` column (so a raw copy stays
    ...
    /// Per-word OCR bounding boxes (`OcrTextBlock[]`) â€” each block's `text`
...
pub async fn fetch_unredacted_frames_fulltext(
    ...
    let out = rows
        ...
        .map(|r| FrameTextRow {
            id: r.get::<i64, _>("id"),
            // Lossy UTF-8 decode â€” same invalid-byte guard as
...
/// â€” it's never redacted via plain string redaction (that would mangle the
...
/// [`write_redacted`] because `text_json` is JSON â€” never redacted via plain
...
/// `placeholder` / `help_text` / `role_description` of the control â€” the
...
/// surface â€” `text` OR a `properties` JSON that may hold redactable fields.
...
pub async fn fetch_unredacted_elements(
    ...
    let out = rows
        ...
        .map(|r| ElementRow {
            id: r.get::<i64, _>("id"),
            // Lossy UTF-8 decode (issue #4139) â€” never panic the worker.
            text: r
...
pub async fn write_redacted(
        ...
        .await
    {
        ...
        // redundant duplicate â€” the audio pipeline's recovery/retry path can
        ...
        // it. Resolve by deleting the duplicate â€” it drops the un-redacted copy
...
pub async fn fetch_unredacted_ui_events(
    ...
    // `col IS NOT NULL AND col != '' OR â€¦` across every requested column.
...
mod tests {
    ...
    /// and re-fetched forever â€” the ~25k-errors/day reconciliation loop). It is
    ...
    /// Every event type that carries free text must be fetched â€” including
    ...
    async fn ui_events_fetch_covers_all_event_types_and_element_cols() {
        ...
        // and is NOT a redaction target â€” but the row is still fetched
        ...
        .await
        .unwrap();
        // A pure mouse-move with no free text at all â€” must be skipped.
        ...
        // this row MUST be fetched â€” the coverage-audit change.
        sqlx::query(
    ...
    /// re-fetched â€” the watermark is the "is processed" bit.
    #[tokio::test]
    async fn ui_events_write_clean_row_still_stamps_watermark() {
        ...
        sqlx::query("INSERT INTO ui_events (event_type, text_content) VALUES ('text', 'hello')")
            ...
            .await
            .unwrap();
        // No column changed (clean text â†’ redactor returns it verbatim, but
    ...
    async fn elements_fetch_skips_container_nodes_and_redacted_rows() {
        let pool = setup().await;
        // Container node: NULL text â€” must never reach the redactor.
    ...
    async fn frames_variants_have_independent_watermarks() {
        ...
        // accessibility_text is still raw and still pending â€” its
    ...
    async fn fetch_elements_covers_text_and_value_skips_structural() {
        ...
        .await
        .unwrap();
        // 3: structural-only properties (no redactable field) â†’ must be skipped.
        sqlx::query(
        ...
        .await
        .unwrap();
        // 4: NULL/NULL container â†’ skipped.
        sqlx::query("INSERT INTO elements (text) VALUES (NULL)")
            .execute(&pool)
            .await
            .unwrap();
        // 5: already redacted â†’ skipped.
    ...
    async fn write_element_overwrites_present_cols_and_stamps() {
        ...
        .await
        .unwrap();
        // Only properties changed (text left as-is â†’ None).
