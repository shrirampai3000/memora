...
//! (`memora_a11y::tree::AccessibilityTreeNode`). Each node carries a
...
//! of the same screen content â€” every node's visible text is a substring
//! of that frame's `full_text` â€” so it was leaking the same PII the
...
//!
//! - `text` â€” the node's label / visible text (the primary surface)
//! - `value` â€” current value of a text field / slider / combo box
//! - `help_text` â€” tooltip / AXHelp / description
//! - `placeholder` â€” input placeholder
//! - `role_description` â€” localized control-type string
//! - `url` â€” associated URL (can embed a username / token / email)
//!
//! Structural fields (`role`, `depth`, `bounds`, `on_screen`, booleans,
//! `automation_id`, `class_name`, `subrole`, `accelerator_key`,
//! `access_key`, `lines`) are left untouched â€” they are not free text
...
//! ## How
//!
//! Parse â†’ for each string field in [`REDACTABLE_FIELDS`] on each node,
//! apply the [`RedactionMap`] built from a single detection on the
//! frame's `full_text` â†’ re-serialize. No per-node model call: the map
//! is the same one used to propagate to `accessibility_text`
//! (MEMORA/website#291). Boundary-safe value matching means
...
/// [`redact_value`] â€” it special-cases scalar strings, not string arrays.
...
/// re-serialized JSON.
///
/// Returns `Ok(None)` when there is nothing to do â€” the map is empty (no
/// PII was detected on the frame) â€” so callers can skip the write and
...
/// are scrubbed â€” lets the worker honor the per-column config (e.g. drop
...
pub fn redact_tree_json_with_fields(
    ...
    // Empty map â†’ identity transform; signal "no write needed".
    if map.is_empty() {
        return Ok(None);
    }
    ...
    if !changed {
        // Parsed fine but nothing matched â€” preserve verbatim so the
...
/// offsets â€” correctness over partial-precision on the redaction path.
fn redact_value_with(
    ...
    match value {
        Value::Object(obj) => {
            ...
            // The node's `text` changed length â†’ its `lines` char offsets
...
/// `full_text` but leave `accessibility_tree_json` raw forever â€” the
...
pub async fn redact_tree_json_with_redactor_fields(
    ...
    if originals.is_empty() {
        // Parsed fine, no free text to scrub â†’ preserve verbatim so the
        // caller still stamps the watermark (row is genuinely clean).
        return Ok(Some(blob.to_string()));
    }
...
/// / `help_text` / `role_description` fields as a tree node) â€” issue #4115
...
/// `value` + `allowed_fields` â€” same length, same order. Used by the
...
pub(crate) fn apply_redacted_strings(
    ...
    redact_value_with(value, allowed_fields, &mut changed, &mut |s| {
        // Defensive: if the caller under-supplied (shouldn't happen â€” it
...
mod tests {
    ...
    fn covers_all_redactable_fields_and_skips_structural() {
        ...
        // shape) so every field â€” including `url` â€” can carry it.
        ...
        // Structural string fields are NOT in the allowlist â†’ left raw.
    ...
    fn non_pii_untouched() {
        ...
        // No match anywhere â†’ blob preserved verbatim, watermark still stamped.
        let out = redact_tree_json(blob, &m).unwrap().unwrap();
        assert_eq!(out, blob);
    }

    /// Empty map â†’ no write needed (None), so the caller skips stamping a
    /// no-op redaction.
    #[test]
    ...
    /// stamps it done while raw text may survive) â€” and never panics.
    #[test]
    ...
    fn redacting_text_clears_stale_lines_only_for_changed_node() {
        ...
        // Node 0's text changed â†’ lines removed.
        assert!(
        ...
        // Node 1's text untouched â†’ lines preserved.
        assert!(
    ...
    /// label, exposes NO map (`redact_with_map` defaults to `None`) â€” this
    ...
    /// fields, and clears stale `lines` â€” proving the enclave backend no
    ...
    /// No redactable text â†’ blob preserved verbatim (caller still stamps).
    #[tokio::test]
    ...
    /// Text present but nothing matched â†’ blob round-trips unchanged.
    #[tokio::test]
    ...
    /// Malformed JSON â†’ Json error (worker leaves the row pending), never a
