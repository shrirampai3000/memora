...
//! Async PII redaction reconciliation for MEMORA.
//!
//! ## Why this crate exists
//!
//! MEMORA captures OCR text, accessibility-tree text, audio
...
//! redactor as a **background reconciliation worker** instead â€” same
...
//! polling interval.
//!
//! See [issue #3185](https://github.com/MEMORA/MEMORA/issues/3185)
//! for the full design.
//!
//! ## Layout
//!
//! Two parallel modalities â€” text and image â€” sharing infra (error
...
//! ### Text
//!
//! - [`Redactor`] â€” text trait. Batch of strings â†’ redacted strings.
//! - [`adapters::regex`] â€” deterministic, on-device, free.
//!   Emails / cards / phones / connection strings / common API-key
//!   shapes / private-key block markers. Always runs first.
//! - [`adapters::tinfoil`] â€” HTTP client for the Tinfoil-hosted
...
//! - [`adapters::onnx`] (feature-gated) â€” local ONNX-runtime
//!   inference of the text PII redactor. CoreML on Mac, DirectML on
//!   Windows. **No CUDA / Vulkan / GPU-vendor SDKs bundled.**
//! - [`pipeline`] â€” regex pre-pass plus AI fallback for residual
//!   text. Hash-cached per-string so repeated UI chrome is free on
//!   the second pass.
//! - [`worker`] â€” text reconciliation loop. Polls the DB for
//!   un-redacted rows on the four text-bearing tables (`ocr_text`,
//!   `audio_transcriptions`, `accessibility`, `ui_events`).
//!
//! ### Image
//!
//! - [`ImageRedactor`] â€” image trait. JPG path â†’ pixel-space bboxes.
//!   Same canonical [`SpanLabel`] enum the text path uses.
//! - [`adapters::rfdetr`] (feature-gated) â€” RF-DETR-Nano image PII
//!   detector. Same EP chain as `adapters::onnx`.
//! - [`image::frame_redactor`] â€” applies detected regions to a JPG.
//!   Solid black (NOT blur â€” blur is reversible). Pure pixel pushing,
//!   no model.
//! - [`image::worker`] â€” image reconciliation loop. Scans the
...
//! ## Defaults
//!
//! Off. Users opt in via Settings â†’ Privacy. See worker docs for the
//! full set of knobs.
...
pub trait Redactor: Send + Sync {
    /// A short identifier â€” historically written into the database
    /// `redaction_version` column. The column is gone (the 20260507
    /// drop-duplicates migration removed re-redaction tracking â€” the
    ...
    /// â€” notably the span-less enclave, whose detections aren't exposed
    /// as spans. Callers then fall back to redacting each copy directly.
    /// The default returns `None`; [`Pipeline`] overrides it.
    ///
    /// See MEMORA/website#291 for why this exists (one detection per
