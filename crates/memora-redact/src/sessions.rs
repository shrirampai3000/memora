...
//!
//! Coding agents (pi, etc.) persist full sessions â€” every bash output, file read,
//! and tool result â€” with no redaction, so any credential the agent touches lands
//! in plaintext on disk. This reuses the crate's secrets-only [`Pipeline`] over the
//! session records.
//!
//! Unlike [`crate::tree_json`] (restricted to MEMORA's record fields), this
...
//! Only [`crate::SpanLabel::Secret`] spans are rewritten (the default policy) â€”
