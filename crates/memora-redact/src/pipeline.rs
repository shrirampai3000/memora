...
//!    Catches the "obvious" stuff â€” emails, cards, JWTs, private-key
//!    blocks, connection strings.
//! 2. **Cache hit on the (text, regex_version) tuple** ends the call â€”
...
//! input â€” we re-anchor AI-detected spans by searching for them in
...
//! produced a replacement â€” we just need offsets for the audit trail.
...
pub struct PipelineConfig {
    ...
    /// least this many spans â€” input is "covered enough" already.
...
/// The replacement string for one span â€” a stable pseudonym token when
...
impl Pipeline {
    ...
    /// (emails, phones, cards, connection strings, â€¦) â€” without this
    ...
    /// with no spans, so its detections can't be tokenized â€” rather than
...
impl Redactor for Pipeline {
    fn name(&self) -> &str {
        ...
        // the `onnx` substring rather than an exact literal â€” otherwise a
                ...
                match ai.redact(&current.redacted).await {
                    Ok(ai_out) => {
                        ...
                        let redacted = if ai_out.spans.is_empty() {
                            ...
                            // be wrong â€” it rebuilds `redacted` from
                    ...
                    Err(RedactError::Unavailable(_)) => {
                        // AI not available â€” keep regex output and avoid
                    ...
                    Err(e) => {
                        ...
                        // Transient failure â€” log via tracing, keep
...
mod tests {
    ...
    async fn regex_only_runs_without_ai_secret_policy() {
        ...
        // rewritten â€” only credentials are. This is the live shipping
    ...
    async fn span_less_ai_output_is_trusted() {
        ...
        // verbatim â€” it applied the policy server-side. UppercaseAi
    ...
    async fn ai_skipped_when_input_short() {
        ...
        // Below ai_min_chars=100 â†’ AI must NOT have been called.
    ...
    async fn ai_skipped_when_regex_already_matched_lots() {
        ...
        let p = Pipeline::regex_then_ai(ai.clone(), cfg);
        // 3 secret-shape tokens â†’ regex finds 3 spans, â‰¥ 2 â†’ skip AI.
        let _ = p
    ...
    async fn pseudonyms_are_consistent_across_inputs() {
        ...
        // Same secret in two different surrounding sentences â†’ same token.
        let a = p
        ...
        let b = p
            ...
            .await
            .unwrap();
        // A different secret â†’ a different token.
        let c = p
    ...
    async fn pseudonyms_forced_off_for_span_less_enclave() {
        use crate::pseudonym::Pseudonymizer;
        // UppercaseAi reports name "tinfoil" â€” stands in for the span-less
