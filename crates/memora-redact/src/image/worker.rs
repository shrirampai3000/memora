...
//!
//! Mirrors [`crate::worker::Worker`] (text path) â€” same poll / batch /
//! pause / resume / status pattern â€” but operates on the `frames`
...
//! and no version-tracking column â€” the timestamp IS the gate. See
...
pub struct ImageWorkerConfig {
    ...
    /// Sleep between rows when there IS work â€” pacing knob so a
...
impl ImageWorker {
    ...
    async fn process_one(&self) -> Result<Option<FrameRedactionOutcome>, anyhow::Error> {
        ...
        if !path.exists() {
            debug!(frame = id, path = %path.display(), "frame jpg missing â€” marking redacted to skip");
        ...
        // logs every poll. Mark it processed to skip â€” the mp4 itself
        ...
        if !is_image_path {
            debug!(
                frame = id,
                path = %path.display(),
                "frames.name is not a still-image path (likely mp4 chunk) â€” marking redacted to skip"
            );
            self.mark_redacted(id).await?;
            return Ok(Some(FrameRedactionOutcome::default()));
        }
...
mod tests {
    ...
    /// Stub redactor â€” flags one region per frame.
    struct StubRedactor;
    #[async_trait]
    ...
    async fn skips_recent_frames() {
        ...
        // recent frame â€” within min_age_seconds = 60
        sqlx::query(
        ...
        // Process one â€” should be a no-op (frame too new).
    ...
    /// path (db.rs `insert_frame` binds `video_chunks.file_path` â†’
    /// `frames.name`). Worker must skip those instead of dying on
    /// `image::open`. Regression guard for product-stability path â€”
