...
//! - same async-worker shape (poll â†’ batch â†’ write-back â†’ status)
...
pub struct ImageRegion {
    ...
    pub bbox: [u32; 4],
    /// Canonical PII label â€” same enum the text path uses.
    pub label: SpanLabel,
...
pub trait ImageRedactor: Send + Sync {
    /// Short identifier â€” used in worker logs / metrics. Historically
...
///
/// **Why secrets-only.** For MEMORA's use case (local memory of
/// the user's screen), most of what the 12-class model flags as "PII"
/// is actually *value* â€” emails, names, dates, URLs, channels are
...
/// image augmented bench in MEMORA/MEMORA-pii-bench-image)
/// ships per-class metrics on the Secret class:
///   thr 0.30 â€” P=0.90  R=0.78  F1=0.83
///   thr 0.50 â€” P=0.97  R=0.66  F1=0.78   â† this default
///   thr 0.70 â€” P=1.00  R=0.60  F1=0.75
/// 0.50 keeps near-perfect precision (â‰ˆ 1 false redaction per 33
...
///
/// Earlier hotfix history (commits 0d55e6c4b â†’ 8125837ff) pushed the
...
///
/// To re-broaden the allow-list later (Person, Email, Phone, Url, â€¦),
/// validate per-class precision on the augmented bench and surface a
/// per-class toggle in Settings â†’ Privacy so users opt in per class.
...
pub struct ImageRedactionPolicy {
    ...
    /// Score floor â€” detections below this are ignored.
    pub min_score: f32,
}

impl Default for ImageRedactionPolicy {
    fn default() -> Self {
        // Secrets-only at 0.50 â€” see the type-level docs for the v9
...
impl ImageRedactionPolicy {
    /// Convenience: same as the `Default` for now â€” kept as a named
    ...
    /// See [`crate::span::parse_allow_list`] â€” Secret is always
    /// included.
