...
//! for an OCR-robust outcome â€” re-running OCR on the redacted output
//! must produce no recoverable PII text.
//!
//! No model in this module â€” it's pure pixel pushing. Pairs with
...
//! used to exist but was removed â€” it left every original JPG in place
...
pub struct FrameRedactionOutcome {
    ...
    /// Total redacted pixel count â€” useful for telemetry to detect
    ...
    /// `image_path`, since redaction is destructive â€” kept on the
...
/// - missing / unreadable file â†’ `RedactError::Runtime`
/// - bbox extends past image bounds â†’ clamped silently
/// - empty `regions` â†’ still re-encodes the image to the destination
...
pub fn redact_frame(
    ...
    //
    // We must keep the original extension on the tempfile too â€” `image`
    // infers format from the path. So `frame.jpg` â†’ `frame.jpg.redact-tmp`
    ...
    // Make sure bytes hit disk before rename â€” otherwise a crash between
    ...
    std::fs::rename(&tmp_path, &output_path).map_err(|e| {
        // On rename failure, try to clean up the tempfile â€” best effort.
...
mod tests {
    ...
    fn make_test_jpg(dir: &Path) -> PathBuf {
        // PNG, not JPG â€” JPEG's lossy compression smears the
    ...
    fn redacts_kept_regions_and_skips_filtered() {
        ...
        // "filtered by score" path â€” the live default is intentionally
