...
//! A reusable `value â†’ replacement` map for **propagating** one detection
//! to derived copies of the same text without re-running the model.
//!
//! MEMORA stores the same screen content at several granularities â€”
...
//! spans as a [`RedactionMap`] (each PII value â†’ its replacement, which
//! is a static `[LABEL]` or a pseudonym token), and apply it to the
//! derived copies by multi-pattern string search (aho-corasick) â€” no
//! second model call. See the design at MEMORA/website#291.
//!
...
//! built from, the values appear verbatim â€” this is exact matching, not
//! fuzzy guessing.

use aho_corasick::{AhoCorasick, MatchKind};

/// A set of `detected value â†’ replacement` rules, compiled into a single
...
impl RedactionMap {
    ...
    /// True when there are no rules â€” [`apply`](Self::apply) is then a
...
/// alphanumeric token â€” i.e. its first char is a word char AND the char
...
mod tests {
    ...
    fn boundary_guard_blocks_subtoken_matches() {
        ...
        let m = map(&[("Al", "[NAME]")]);
        assert_eq!(m.apply("Algorithm"), "Algorithm"); // 'Al' inside a word â€” left alone
        assert_eq!(m.apply("Al did it"), "[NAME] did it"); // standalone token â€” replaced
        assert_eq!(m.apply("hi Al!"), "hi [NAME]!"); // punctuation neighbour â€” replaced
    }

    #[test]
    fn punctuation_edged_values_match_against_neighbours() {
        // Email value sits flush against other text â€” still matches,
    ...
    fn matching_is_case_sensitive() {
        // Verbatim values from detection â€” derived copies share the case,
    ...
    fn multibyte_values_and_neighbours() {
        ...
        // neighbours â€” must never panic on a byte slice and must match.
        let m = map(&[("JosÃ©", "[N]"), ("åŒ—äº¬", "[CITY]")]);
        assert_eq!(
            m.apply("from JosÃ© in åŒ—äº¬ today"),
            "from [N] in [CITY] today"
        );
        // 'JosÃ©' inside 'JosÃ©se' is a sub-token (next char 's' is a word
        // char) â†’ boundary guard leaves it.
        assert_eq!(m.apply("JosÃ©se"), "JosÃ©se");
        // non-word (emoji) neighbours â†’ matches.
        assert_eq!(m.apply("ðŸŽ‰JosÃ©ðŸŽ‰"), "ðŸŽ‰[N]ðŸŽ‰");
    }

    #[test]
    fn boundary_guard_blocks_glued_tokens_allows_separated() {
        let m = map(&[("ab", "[X]")]);
        // "abab" is a single alphanumeric token â€” redacting "ab" inside it
        ...
        // Separated by non-word chars â†’ both occurrences replaced.
    ...
    fn never_leaves_a_standalone_value_and_preserves_the_rest() {
        ...
        assert_eq!(m.apply("untouched âœ“ text"), "untouched âœ“ text");
    }
    ...
    fn apply_is_cheap_over_many_values() {
        ...
        // lower) over an ~8 KB blob â€” pure string scanning, no model.
