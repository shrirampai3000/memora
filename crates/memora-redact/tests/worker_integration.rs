...
use memora_sqlite_coordinator::SqliteWritePool;
use sqlx::sqlite::{SqliteConnectOptions, SqlitePoolOptions};
use sqlx::Row;
use tempfile::TempDir;

/// Every column enabled â€” tests that want to verify full coverage opt in to
...
async fn seed(pool: &sqlx::SqlitePool) {
    sqlx::query(
        "INSERT INTO frames (id, full_text) VALUES (1, 'Hi alice@example.com â€” meeting at 3pm')",
    )
    .execute(pool)
    .await
    .unwrap();
    ...
    // One frame carries both reconciled columns â€” accessibility_text and
    // the consolidated full_text search surface (issue #4097) â€” each with
    ...
    sqlx::query(
        "INSERT INTO frames (accessibility_text, full_text) \
         VALUES ('AXButton[Send to bob@example.com]', 'Send to dave@example.com â€” agenda')",
    )
    .execute(pool)
    .await
    .unwrap();
    // ui_events: a keyboard event (text_content), a clipboard event, and
    // a CLICK event that carries element PII but no typed text â€” the
    ...
    sqlx::query(
        "INSERT INTO ui_events (event_type, element_value, element_name, element_description, window_title) \
         VALUES ('click', 'erin@example.com', 'frank@example.com', 'Field for henry@example.com', 'Inbox â€” grace@example.com')",
    )
    .execute(pool)
    .await
    .unwrap();
...
async fn worker_redacts_all_targets() {
    ...
    for target in [
        ...
        // Elements is multi-column now (text + properties) â€” asserted
        ...
        assert!(
            !rows.is_empty(),
            "expected â‰¥1 redacted row in {} after worker drained",
            target.label()
        );
        ...
        assert!(
            ...
            "{}: source column {:?} doesn't contain a placeholder â€” destructive overwrite did not happen",
            target.label(),
            red
        );
    }

    // ui_events: EVERY free-text column must have its raw PII removed â€”
    ...
    // elements: the properties-only row â€” PII lived ONLY in
    ...
    // role_description ("text field") is non-PII â†’ preserved.
    assert!(
    ...
    // full_text seeded on both frames â†’ 2 writes. audio (1), accessibility
...
async fn worker_skips_already_redacted_rows() {
    let (pool, _temp_dir) = setup_db().await;
    // Frame 1 is already processed â€” source already redacted, redacted_at set.
    sqlx::query(
...
async fn worker_overwrites_source_columns_destructively() {
    ...
    // Source column overwritten â€” raw email gone forever.
    assert!(
...
/// must overwrite `full_text` in place â€” the issue's repro
/// (`select count(*) from frames where full_text like '%canary%'` â†’ 0).
#[tokio::test]
...
/// raw value must be gone â€” no `token -> value` mapping is stored.
#[tokio::test]
...
impl Redactor for CountingPipeline {
    ...
    async fn redact_with_map(
        ...
        // the detection â€” NOT this wrapper's, so `batch_calls` stays 0
...
/// redaction to the same frame's `accessibility_text` â€” no second model
...
async fn frame_fulltext_redaction_propagates_to_accessibility_once() {
    let (pool, _temp_dir) = setup_db().await;
    // accessibility_text âŠ† full_text (full_text = accessibility || ocr),
    // both carrying the same secret â€” mirrors how capture assembles them.
    ...
    // The whole point: ONE detection, propagated â€” not two.
    assert_eq!(
...
/// `accessibility_tree_json` node text and `window_name` â€” all from one map,
...
async fn frame_fulltext_propagates_to_all_derived_copies_once() {
    ...
    let window = format!("Dashboard â€” {secret}");
    ...
    // ONE detection, propagated to the derived copies â€” never re-run.
    assert_eq!(
        redactor.map_calls.load(Ordering::SeqCst),
        1,
        "full_text should be detected exactly once"
...
/// prior run (watermark set) â€” and don't re-stamp it.
#[tokio::test]
...
/// SAME stable token as `full_text` for the same value â€” so the two
...
/// `None`, e.g. the span-less enclave â€” `RegexRedactor` stands in here),
...
/// redacted â€” no silent data loss.
#[tokio::test]
...
/// A target table missing from the schema (binary/DB version skew â€” the
...
///
/// Repro shape: reconcile a missing target (`Elements` â€” no `elements`
...
async fn worker_disables_missing_table_and_keeps_reconciling_others() {
    // Schema WITHOUT an `elements` table â€” mirrors an engine whose code
    ...
    // 400ms â‰ª the 2s transient backoff: only reachable if the missing
    ...
    assert!(
        ...
        "full_text must be redacted despite the missing `elements` table â€” \
         the missing target should be disabled, not block/spam the rotation (got {red:?})"
    );
    // NB: `last_error` is intentionally not asserted here â€” the successful
...
async fn frame_fulltext_no_map_path_scrubs_all_derived_copies() {
    ...
    let window = format!("Inbox â€” {email}");
...
async fn default_columns_leave_optin_columns_untouched() {
    ...
    let redactor = Arc::new(RegexRedactor::new()) as Arc<dyn Redactor>;
    // DEFAULT columns (no `columns:` override) â€” browser_url / element_name off.
    let cfg = WorkerConfig {
        batch_size: 16,
        idle_between_batches: Duration::from_millis(1),
        poll_interval: Duration::from_millis(20),
...
/// the row watermarked â€” a raw copy of a window title surviving inside the
...
async fn ui_events_ancestors_json_scrubbed_structure_preserved() {
    ...
    sqlx::query(
        "INSERT INTO ui_events (event_type, window_title, element_ancestors) VALUES ( \
            'click', \
            'Mail â€” dave@example.com', \
            '[{\"role\":\"AXWindow\",\"name\":\"Mail â€” dave@example.com\"},{\"role\":\"AXGroup\"},{\"role\":\"AXButton\",\"name\":\"Reply\"}]' \
        )",
    )
    .execute(&pool)
    .await
