...
//!   /usr/bin/time -l cargo run --release -p memora-redact \
//!     --example onnx_text_memory_probe --features onnx-cpu
//!
//! `MEMORA_ONNX_MEMORY_PROBE_ROUNDS` controls the number of rounds (default 8).
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let rounds = std::env::var("MEMORA_ONNX_MEMORY_PROBE_ROUNDS")
        .ok()
