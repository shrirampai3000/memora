...
//
// Run:
//   cargo run --example tinfoil_probe -p memora-redact

use memora_redact::{adapters::tinfoil::TinfoilRedactor, Redactor};
...
async fn main() {
    ...
    let realistic_long = "Marcus Chen <marcus@helios-ai.io> Â· 415-555-0142 Â· ".repeat(40);
    let payloads = [
        "hi".to_string(),
        "Email me at alice@example.com or call 415-555-0142.".to_string(),
        "Marcus Chen pushed the prod creds to the MEMORA/widget repo on slack.".to_string(),
        realistic_long,
    ];
    for (i, text) in payloads.iter().enumerate() {
        println!("\n[probe] sending #{i} (len={})â€¦", text.len());
