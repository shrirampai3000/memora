...
//!
//! The recall gain from 2Ã—2 tiling was first measured in a Python harness.
//! This runs the same comparison through [`memora_redact::adapters::rfdetr`]
//! â€” the code the app actually ships â€” so the win is confirmed on the real
//! path rather than in a reimplementation that might decode boxes differently.
//!
//! ```bash
//! cargo run -p memora-redact --example rfdetr_tiling_check \
...
//! plus wall-clock, so the ~4Ã— CPU cost is visible next to the benefit.
