...
//!
//! It copies the input first â€” `redact_frame` is destructive by design (it
//! overwrites the JPG in place) and these are the user's real captured frames.
//!
//! ```bash
//! cargo run -p memora-redact --example rfdetr_end_to_end \
...
async fn main() -> anyhow::Result<()> {
    ...
    use memora_redact::image::frame_redactor::redact_frame;
    use memora_redact::image::{ImageRedactionPolicy, ImageRedactor};
    ...
    // Never redact in place â€” this is a real captured frame.
