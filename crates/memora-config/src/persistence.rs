...
//!
//! Used by the CLI binary to persist settings in `~/.memora/config.toml`.
...
/// Default config file path: `~/.memora/config.toml`.
...
/// Resolve the MEMORA config directory: `~/.memora/`
fn dirs_next() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".memora"))
}
