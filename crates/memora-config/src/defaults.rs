...
pub enum DeviceTier {
    /// High-end: â‰¥24 GB RAM and â‰¥8 cores (safe for parakeet ~2GB)
    High,
    /// Mid-range: â‰¥12 GB or (>8 GB and â‰¥6 cores)
    Mid,
...
fn classify_detected_tier(ram_gb: Option<u64>, cores: Option<u64>) -> DeviceTier {
    ...
    match (ram_gb, cores) {
        ...
        // device for Mid even with a small CPU. Likewise, 9â€“11 GB needs a CPU
...
/// | Tier | Criteria                              |
/// |------|---------------------------------------|
/// | High | â‰¥24 GB RAM and â‰¥8 cores               |
/// | Mid  | â‰¥12 GB or (>8 GB and â‰¥6 cores)        |
...
impl DbConfig {
    ...
    pub fn for_tier(tier: DeviceTier) -> Self {
        match tier {
            ...
            DeviceTier::Mid => Self {
                mmap_size: 0,          // disabled â€” see DbConfig::default (prevents DB corruption)
...
impl Default for DbConfig {
    /// High-tier defaults â€” identical to the previous hardcoded values.
    fn default() -> Self {
        Self {
            // mmap disabled (0): memory-mapped I/O maps the DB file *writably*
            // into MEMORA's address space, where a stray write from any
...
/// the threshold runs the checkpoint on *that* connection â€” under heavy load
...
impl Default for ChannelConfig {
    /// High-tier defaults â€” identical to the previous hardcoded values (1000).
    fn default() -> Self {
        Self {
...
///
/// Cached for process lifetime â€” the value cannot change at runtime, and the
...
///
/// Decision matrix (CPU with AVX2 â€” i.e. every x86-64 since Haswell/2013):
///
/// | Tier | macOS â‰¥ 26         | macOS < 26         | Windows/Linux |
/// |------|--------------------|--------------------|---------------|
...
/// whisper/qwen3 cannot run (their kernels are AVX2-compiled â€” see
/// `memora_core::cpu_features`), so Windows/Linux fall back to
...
pub fn best_engine_for_platform(tier: DeviceTier) -> &'static str {
    best_engine_for_platform_for_cpu(tier, memora_cpu_features::has_avx2())
}
...
pub fn best_engine_for_platform_for_cpu(tier: DeviceTier, has_avx2: bool) -> &'static str {
    ...
    // MEMORA-audio disables it on EVERY x86-64 target â€” this picker must
    ...
    if cfg!(target_arch = "x86_64") && !has_avx2 {
        ...
        // to MLX and needs macOS 26 â€” a non-AVX2 Intel Mac (Mac Pro 2013 is
        ...
        // CPU dispatch â€” safe everywhere, but too heavy for Low tier.
    ...
    // High tier only (â‰¥24GB RAM) â€” safe for large models
    #[cfg(target_os = "macos")]
    {
...
///   are AVX2-compiled â€” STATUS_ILLEGAL_INSTRUCTION at first use)
...
pub fn is_engine_unsafe(engine: &str, tier: DeviceTier) -> bool {
    is_engine_unsafe_for_cpu(engine, tier, memora_cpu_features::has_avx2())
}
...
pub fn is_engine_unsafe_for_cpu(engine: &str, tier: DeviceTier, has_avx2: bool) -> bool {
    ...
    // Applies on every x86-64 target (macOS included) â€” mirrors the runtime
    // gate in MEMORA-audio's TranscriptionEngine::new exactly, so the
    ...
    if tier == DeviceTier::Mid {
        ...
        // (whisper/qwen3 are AVX2-compiled) and Mid (â‰¥12GB) holds it â€” it is
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        // macOS â‰¥ 26 (Metal buffer allocation segfaults on older versions).
        let macos_ok = macos_major_version()
            .map(|v| v >= PARAKEET_MIN_MACOS_MAJOR)
            .unwrap_or(false);
        !macos_ok
    ...
    #[cfg(not(target_os = "macos"))]
    {
        // Off-macOS, plain parakeet is ONNX CPU â€” safe on High tier. Only the
...
mod tests {
    ...
    fn whisper_and_qwen3_unsafe_without_avx2_on_x86() {
        ...
        // parakeet is ONNX (runtime-dispatched MS DLL) â€” safe without AVX2 off-macOS
        assert!(!is_engine_unsafe_for_cpu(
            "parakeet",
            DeviceTier::High,
            false
    ...
    // â”€â”€ classify_tier boundary tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn classify_high_tier() {
        ...
        // 16 GB, 8 cores â†’ Mid now (parakeet too heavy for 16GB)
        assert_eq!(classify_tier(16, 8), DeviceTier::Mid);
    }

    #[test]
    fn classify_mid_tier() {
        // 16 GB, 8 cores â†’ Mid (not enough RAM for parakeet)
        assert_eq!(classify_tier(16, 8), DeviceTier::Mid);
        // 16 GB but only 4 cores â†’ â‰¥12 GB â†’ Mid
        assert_eq!(classify_tier(16, 4), DeviceTier::Mid);
        // 12 GB, 2 cores â†’ â‰¥12 GB alone qualifies for Mid
        assert_eq!(classify_tier(12, 2), DeviceTier::Mid);
        // 8 GB, 6 cores â†’ Low (8 GB machines use whisper-tiny to avoid MLX OOM)
        assert_eq!(classify_tier(8, 6), DeviceTier::Low);
        // 9 GB, 6 cores â†’ (>8 GB and â‰¥6 cores) â†’ Mid
        assert_eq!(classify_tier(9, 6), DeviceTier::Mid);
        // 10 GB, 8 cores â†’ >8 GB and â‰¥6 cores â†’ Mid
    ...
    fn classify_low_tier() {
        ...
        // 7 GB, 8 cores â†’ <8 GB, not â‰¥12 â†’ Low
    ...
    fn classify_zero_ram_or_cores() {
        ...
        assert_eq!(classify_tier(32, 0), DeviceTier::Mid); // â‰¥12 GB â†’ Mid even with 0 cores
    }
