...
//! The core recording settings type shared across all MEMORA components.
...
/// - **CLI**: built from command-line args or loaded from `~/.memora/config.toml`
...
/// JSON schema (store.bin). This ensures backwards compatibility â€” existing
...
pub struct RecordingSettings {
    ...
    // dropped â€” cutting cloud-transcription cost, disk, and the PII/transcription
    ...
    #[serde(rename = "MEMORAAecEnabled", default)]
    ...
    // â”€â”€ Mitsukeru fork: event-driven capture overrides â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // ãƒŸãƒ„ã‚±ãƒ«æ‹¡å¼µï¼šPowerProfile ã«ä¾ã‚‰ãšå€‹åˆ¥ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿ã‚’ç›´æŽ¥æŒ‡å®šã™ã‚‹ãŸã‚ã®ä¸Šæ›¸ãå€¤ã€‚
    // None ã®å ´åˆã¯é€šå¸¸é€šã‚Š PowerProfile ãŒæ±ºå®šã€‚ãƒ‡ã‚¹ã‚¯ãƒˆãƒƒãƒ—å¸¸æ™‚è¨˜éŒ²ã®ã‚ˆã†ãªç”¨é€”ã§
    // ã€ŒAC é›»æºã ãŒ Balanced ç›¸å½“ã®é »åº¦ã«å›ºå®šã—ãŸã„ã€ã‚±ãƒ¼ã‚¹ã«å¯¾å¿œã™ã‚‹ã€‚
    ...
    /// `true`, the `memora_redact::image::worker` runs alongside
    ...
    ///   `MEMORA_PRIVACY_FILTER_API_KEY` (or the cloud auth key).
    ///
    ...
    /// `[LABEL]` tags when `asyncPiiRedaction` is on. Same value â†’ same
    ...
    // â”€â”€ Cloud / Auth â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    /// MEMORA cloud user ID. Empty string means not logged in.
    /// Kept as String (not Option) to match existing store.bin schema.
    #[serde(rename = "userId")]
    pub user_id: String,

    /// Display name for speaker identification.
    /// Fallback chain: this field â†’ cloud auth name â†’ cloud auth email.
    ...
    /// HTTP server port for the MEMORA API.
    pub port: u16,
...
impl RecordingSettings {
    ...
    /// Returns effective AEC booleans as `(MEMORA, windows, macos)`.
    pub fn effective_aec_flags(&self) -> (bool, bool, bool) {
        match self.aec_mode {
            AecMode::Off => (false, false, false),
            AecMode::MEMORA => (true, false, false),
...
/// Default redaction allow-list: secrets only. The safety baseline â€”
...
/// `element_properties` (form-field values â€” the surface where real PII like
...
/// memora-redact (this crate can't depend on it) and the
...
mod tests {
    ...
    fn deserializes_from_empty_json() {
        ...
        assert!(!settings.MEMORA_aec_enabled);
    ...
    #[test]
    fn explicit_MEMORA_aec_mode_enables_software_aec() {
        let settings: RecordingSettings = serde_json::from_str(
            r#"{
                "aecMode": "MEMORA",
                "MEMORAAecEnabled": false,
    ...
    fn deserializes_legacy_extra_fields() {
        ...
        assert_eq!(settings.vocabulary[0].word, "MEMORA");
    ...
    fn deserializes_real_store_bin_shape() {
        ...
        let json = r#"{
            ...
            "aiPresets": [{"name": "test"}],
            "showMEMORAShortcut": "Super+Alt+S",
        ...
        // This is critical for flatten compat â€” SettingsStore has aiPresets, shortcuts, etc.
    ...
    fn flatten_simulation() {
        ...
        struct FakeSettingsStore {
            ...
            overlay_mode: String,
            #[serde(rename = "showMEMORAShortcut", default)]
            show_MEMORA_shortcut: String,
        ...
        assert_eq!(store.show_MEMORA_shortcut, "Super+Alt+S");
    ...
    fn vocab_entry_accepts_bare_string_and_struct_mixed() {
        // Regression for MEMORA-APP-DN: legacy store.bin stored vocabularyWords
        ...
        let json = r#"{"vocabularyWords": ["ã›ã°ã‚“ã¬", {"word": "MEMORA"}, {"word": "foo", "replace_with": "bar"}]}"#;
        let s: RecordingSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.vocabulary.len(), 3);
        assert_eq!(s.vocabulary[0].word, "ã›ã°ã‚“ã¬");
        assert_eq!(s.vocabulary[0].replace_with, None);
        assert_eq!(s.vocabulary[1].word, "MEMORA");
