...
/// Aggregated resource usage for a MEMORA-related process group.
...
impl ProcessBreakdown {
    ...
    fn mcp_count(&self) -> usize {
        self.group("MEMORA_mcp_child")
            .map(|group| group.process_count)
            .unwrap_or(0)
            + self
                .group("MEMORA_mcp_external")
                .map(|group| group.process_count)
                .unwrap_or(0)
    }
    ...
    pub fn compact_summary(&self) -> String {
        ...
        if groups.is_empty() {
            "no MEMORA-related processes found".to_string()
        } else {
            groups
        }
    }
...
fn collect_snapshot(system: &System) -> ResourceSnapshot {
    ...
    let descendant_ids = MEMORA_descendant_ids(system, current_pid);
...
fn MEMORA_descendant_ids(system: &System, current_pid: sysinfo::Pid) -> HashSet<u32> {
...
fn related_process_group(
    ...
    if pid == current_pid {
        return Some("MEMORA_app");
    }
    if text.contains("MEMORA-mcp") && descendant_ids.contains(&pid.as_u32()) {
        return Some("MEMORA_mcp_child");
    }
    if text.contains("MEMORA-mcp") {
        return Some("MEMORA_mcp_external");
    }
    if descendant_ids.contains(&pid.as_u32()) {
        return Some("MEMORA_app_child");
    }
    text.contains("MEMORA").then_some("MEMORA_other")
}
