...
//!
//! Spawning a console-subsystem program from MEMORA's GUI process makes
...
/// function, excludes Windows â€” a spawn in a `#[cfg(not(windows))]` or
...
fn console_programs_are_never_spawned_without_create_no_window() {
    ...
    collect_rs(
        &root.join("apps/memora-app-tauri/src-tauri/src"),
        &mut files,
    );
    assert!(
        files.len() > 100,
        "source scan found only {} files â€” the walk is broken, not the code",
        files.len()
    );
    ...
    for file in &files {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            // unguarded one â€” an earlier draft scoped to the whole function
            ...
            let region_end = ((i + 1)..end)
                .find(|&j| {
                    ...
                    // region too â€” otherwise a correct sibling two lines down
    ...
    assert!(
        ...
        "console window would flash on Windows. Use \
         `memora_core::no_window_command`/`no_window_command_async` (or \
         `apply_no_window`) at:\n  {}",
        offenders.join("\n  ")
    );
}
...
/// entirely. That is how the MEMORA-mcp prewarm shipped a terminal on every
/// Windows launch â€” `bun_command` returned an unguarded `Command`, the install
...
fn command_factories_guard_every_command_they_hand_out() {
    ...
    for file in &files {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            for j in (i + 1)..end {
                ...
                offenders.push(format!(
                    "{}:{} hands out an unguarded Command â€” `{}`",
                    file.strip_prefix(&root).unwrap_or(file).display(),
                    j + 1,
                    line.trim()
                ));
    ...
    assert!(
        offenders.is_empty(),
        "a Command factory must guard what it returns â€” its callers cannot \
         tell that it did not. Build it with \
         `memora_core::no_window_command`/`no_window_command_async`:\n  {}",
        offenders.join("\n  ")
    );
}
