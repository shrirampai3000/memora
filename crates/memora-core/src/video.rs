...
/// Ten seconds is the midpoint of the 5â€“15 second range that gives a useful
...
/// even CRF 14 ("max") looked blurry â€” the encoder simply didn't spend
/// enough effort to use those bits well.
///
/// Trade-offs chosen here:
/// - low/balanced: `ultrafast` â€” minimal CPU, recording must never lag
/// - high: `fast` â€” noticeable quality bump, still real-time on most machines
/// - max: `medium` â€” best quality, may use significant CPU on older hardware
...
/// (49" 5120Ã—1440 â†’ 37% scale) â€” the cap now scales with the user's
...
/// JPEG encoder quality (1â€“100) for the on-disk snapshot at the given
...
///
/// Power profiles can still drop this further on battery/thermal â€” these
    ...
    match child.wait_with_output().await {
        Ok(output) => {
            ...
            if !output.status.success() && !stderr.is_empty() {
                ...
                // FFmpeg writes metadata/codec info to stderr by design â€” not an error
