...
//! of app" rules â€” adding "Slack" can't mean "block #general in Slack but
//! keep #engineering".
//!
...
//!
//! ```text
//! "Slack"             â†’ legacy: matches app contains "slack" OR title contains "slack"
//! "Slack::#general"   â†’ scoped: app contains "slack" AND title contains "#general"
//! "::confidential"    â†’ any app, title contains "confidential"
//! "Slack::"           â†’ equivalent to legacy "Slack"
//! ```
//!
...
pub fn passes_includes(patterns: &[WindowPattern], app_lc: &str, title_lc: &str) -> bool {
    ...
    if has_app_scoped {
        // App is in explicit per-app whitelist mode â€” at least one of its
        // scoped rules must match. Globals are ignored here.
        return app_scoped_matched;
    }
    if has_global {
        // Legacy global include â€” at least one must match app or title.
...
mod tests {
    ...
    fn parse_split_once_handles_title_with_colons() {
        // The title side keeps any further `::` â€” split_once consumes only the first.
    ...
    fn includes_legacy_global_only() {
        // Same as historical "include_set non-empty â†’ must match" behavior.
    ...
    fn includes_scoped_creates_per_app_whitelist() {
        ...
        // Other apps pass â€” they're not restricted by any rule.
    ...
    fn includes_mixed_legacy_and_scoped() {
        ...
        assert!(passes_includes(&patterns, "slack", "#engineering"));
        assert!(!passes_includes(&patterns, "slack", "#hr"));
        // For other apps the legacy global `Slack` applies â€” they're blocked
