...
lazy_static! {
    static ref PII_PATTERNS: Vec<(Regex, &'static str)> = vec![
        ...
        // shows the full secret in plaintext â€” common MEMORA failure
        ...
        // ghs_, ghr_) â€” 36-40 chars. Existing legacy 40-hex SHA path
        // doesn't catch these.
        ...
        (Regex::new(r"[â€¢Â·â—â—‹â—¦â¦âƒ]{4,}|\.{8,}|\*{8,}").unwrap(), "PASSWORD_DOTS"),
    ...
    // RegexSet for fast "any PII present?" check â€” single DFA pass over the text.
...
mod tests {
    ...
    fn test_password_dots_detection() {
        ...
        assert!(contains_pii("â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢"));
        assert!(contains_pii("â—â—â—â—â—â—â—â—"));
        assert!(contains_pii(".........."));
        assert!(contains_pii("****************"));

        let result = remove_pii("Password: â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢");
    ...
    fn test_combined_new_patterns() {
        ...
        assert!(!result.contains("â€¢â€¢â€¢â€¢"), "Password dots not redacted");
    }
    ...
    fn test_slack_token_detection() {
        // Synthetic tokens used only to verify PII regex patterns â€” not real credentials.
        assert!(contains_pii(
            "xoxb-FAKE00000000-FAKE00000000000-FAKEFAKEFAKEFAKE"
        ));
        assert!(contains_pii(
            "xoxp-FAKE00000000-FAKE00000000000-FAKEFAKEFAKEFAKE"
        ));
        assert!(contains_pii("xapp-1-AFAKE00000-FAKE00000000000-fakef"));

        let result = remove_pii("SLACK_TOKEN=xoxb-FAKE00000000-FAKE00000000000-abc");
    ...
    fn test_discord_token_detection() {
        // Synthetic tokens used only to verify PII regex patterns â€” not real credentials.
        let _discord_token = "RkFLRUZBS0VGQUtFRkFLRQ.FAKEFA.FAKEFAKEFAKEFAKEFAKEFAKEfake";
        // Note: This is a fake token format
        assert!(contains_pii(
            "RkFLRUZBS0VGQUtFRkFLRUZBS0VGQUs.FAKEFA.FAKEFAKEFAKEFAKEFAKEfake"
        ));
    }
    ...
    fn test_bitwarden_password_manager_scenarios() {
        ...
        assert!(contains_pii("â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢"));
