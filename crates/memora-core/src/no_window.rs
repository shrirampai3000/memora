...
//!
//! MEMORA's UI process is a GUI-subsystem binary with no console of its
//! own. When such a process spawns a console-subsystem program, Windows
//! allocates a brand new console for the child and shows it â€” a black terminal
...
//! forgetting it is invisible to everyone developing on macOS or Linux â€” which
...
/// `CREATE_NO_WINDOW` â€” do not allocate a console for the child process.
///
...
mod tests {
    ...
    fn a_no_window_command_still_runs_the_program() {
        ...
        assert!(String::from_utf8_lossy(&output.stdout).contains("MEMORA"));
    }
    ...
    /// `CREATE_NEW_CONSOLE` (`0x00000010`) is the exact bug being fixed â€” a
