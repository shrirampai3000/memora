...
//!
//! These are pure check functions with no UI dependencies â€” usable from both CLI and GUI.
...
pub fn preflight_check(need_screen: bool, need_audio: bool) -> bool {
    ...
    // accessibility (always optional â€” input capture only)
    ...
    if !ok {
        eprintln!();
        eprintln!("run \"MEMORA\" again after granting permissions.");
...
// â”€â”€ macOS implementations â”€â”€

/// macOS Screen Recording TCC probes.
///
/// - `preflight`: `CGPreflightScreenCaptureAccess` â€” fast, no side effects. A `false`
...
/// - `capture_probe`: `CGWindowListCreateImage` â€” real capture attempt. Reliable on
///   macOS â‰¤ 14 and in CLI. On macOS 15+ inside a Tauri app, Apple changed it to
///   return the calling app's own windows even without permission â†’ always non-NULL â†’
...
mod macos_screen_recording {
    ...
    /// `SCShareableContent` fails â€” observed live in a user log where the
    ...
    /// True on macOS 15 (Sequoia) or later. Cached â€” spawns `sw_vers` once per process.
    ...
    /// Real capture probe via `CGWindowListCreateImage` â€” NULL means denied, non-NULL means granted.
...
/// positive â€” Apple changed `CGWindowListCreateImage` to return the app's own windows
...
/// permission (the onboarding/settings grant flow) â€” never from a passive
/// launch-time gate.
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn probe_implies_granted() {
        if macos_screen_recording::preflight() || macos_screen_recording::capture_probe() {
            ...
            // tests run with debug_assertions â†’ check_screen_recording_tauri uses full probe chain
