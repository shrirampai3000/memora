...
//! memora-engine's `process_priority`: the desktop app embeds the engine
...
//!   MEMORA-audio's `transcribe_sync` for the pattern).
//! - Never wrap sections that hold locks other Normal-priority threads block
//!   on (e.g. the WGC D3D context lock) â€” that recreates the priority
//!   inversion this feature exists to avoid.
//!
//! `MEMORA_KEEP_NORMAL_PRIORITY=1` disables all dips process-wide â€” the
...
/// Mirrors memora-engine's `analytics::env_value_truthy`.
fn env_value_truthy(value: &str) -> bool {
    let v = value.trim().to_ascii_lowercase();
    !(v.is_empty() || v == "0" || v == "false" || v == "no" || v == "off")
}

/// True when `MEMORA_KEEP_NORMAL_PRIORITY` opts the process out of
...
pub fn keep_normal_priority() -> bool {
    ...
    *KEEP_NORMAL.get_or_init(|| {
        std::env::var("MEMORA_KEEP_NORMAL_PRIORITY")
...
/// Note this is the RELATIVE thread priority â€” under a lowered process class
...
impl BackgroundWorkDip {
    ...
    /// their own mechanisms (nice/QoS) â€” see #4849. No-op guard.
    #[cfg(not(windows))]
    pub fn new() -> Self {
        Self {}
    }
    ...
    fn dips_and_restores_current_thread() {
        ...
        unsafe {
            let original = GetThreadPriority(GetCurrentThread());
            {
                ...
                // A nested dip must be inert (never stacks below BELOW_NORMAL)â€¦
                let nested = BackgroundWorkDip::new();
                assert_eq!(
                    GetThreadPriority(GetCurrentThread()),
                    THREAD_PRIORITY_BELOW_NORMAL
                );
                // â€¦and its drop must not disturb the outer dip.
                drop(nested);
                assert_eq!(
                    GetThreadPriority(GetCurrentThread()),
                    THREAD_PRIORITY_BELOW_NORMAL
