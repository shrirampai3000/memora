...
//! External memory sync â€” write a MEMORA-owned digest of memories
//! into the user's other AI assistants' memory files (Claude Code's
//! `CLAUDE.md`, Codex CLI's `AGENTS.md`, â€¦).
//!
//! This module is the *pure* layer. It does no DB I/O and knows nothing
//! about scheduling â€” given a slice of `MemoryEntry` rows and a target
//! file, it produces a markdown digest, wraps it in a MEMORA-owned
//! marker block, and writes it atomically.
//!
//! The scheduler that decides *when* to call us lives in
//! `memora-engine`, next to the DB it queries. Splitting the two
...
//! out a region MEMORA fully owns; everything outside is preserved
//! byte-for-byte across every sync. Rewrites are idempotent â€” running
...
/// inside the block changes incompatibly so older MEMORA builds can
/// detect a newer block and refuse to rewrite it.
pub const RENDER_SCHEMA: u32 = 1;

/// Start sentinel for the MEMORA-owned region. Both halves include
/// the schema version so a future format change is visible to anyone
/// reading the file.
pub fn marker_start() -> String {
    format!("<!-- MEMORA-memories:start v{} -->", RENDER_SCHEMA)
}

pub fn marker_end() -> String {
    "<!-- MEMORA-memories:end -->".to_string()
}

/// One memory row as the renderer needs it. Intentionally minimal â€”
/// callers translate their richer DB rows into this shape, which keeps
/// `memora-core` from needing to depend on `memora-db`.
...
pub struct Destination {
    /// Stable identifier â€” e.g. `"claude_code"` or `"codex"`.
    pub id: &'static str,
    ...
    /// an `@<sidecar_filename>` reference â€” used for assistants that
    /// natively resolve file imports (Claude Code's `@path` syntax). This
    /// keeps the outer file small and hand-editable while MEMORA owns
    /// the sidecar end-to-end.
    pub sidecar_filename: Option<&'static str>,
    /// When true, MEMORA owns the *entire* target file â€” there is no
    ...
    /// into existing content. Used for dedicated, MEMORA-only notes
...
impl Destination {
    pub const CLAUDE_CODE: Destination = Destination {
        ...
        filename: "CLAUDE.md",
        sidecar_filename: Some("MEMORA-memories.md"),
        owns_target: false,
    };
    ...
    /// Obsidian vault note. Unlike Claude Code / Codex â€” whose `CLAUDE.md` /
    /// `AGENTS.md` the user co-authors â€” this file is created and owned
    /// entirely by MEMORA, so it carries the full digest with no marker
...
// destination must NOT also declare a sidecar â€” the engine's owned-file write
...
/// file (`CLAUDE.md` / `AGENTS.md`). Pure â€” no I/O.
///
...
/// full digest inline â€” same shape as before.
pub fn render_block_body(entries: &[MemoryEntry], dest: &Destination) -> String {
    if let Some(sidecar) = dest.sidecar_filename {
        ...
        return format!(
            "## MEMORA memories\n\n\
            MEMORA writes durable memory facts to the sidecar below. \
            Manage them in the MEMORA app â€” the file is rewritten \
            on every sync.\n\n\
            @{}\n",
            sidecar
        );
...
pub fn render_digest(entries: &[MemoryEntry], dest: &Destination) -> String {
    ...
    out.push_str(&format!(
        "## MEMORA memories\n\n\
        Auto-synced by MEMORA from this user's local memory store. \
    ...
    if sorted.is_empty() {
        out.push_str("_no memories yet â€” MEMORA will populate this on the next sync._\n");
        return out;
    }

    for e in &sorted {
        out.push_str("- ");
        // Newlines inside content would break list rendering â€” collapse
        ...
        if !meta_parts.is_empty() {
            out.push_str(&format!(" _({})_", meta_parts.join(" Â· ")));
        }
        out.push('\n');
    }
...
/// Render the full note MEMORA writes into a [`Destination::owns_target`]
/// file (today: an Obsidian vault). Because MEMORA owns the whole file
/// there is no marker block â€” instead we lead with YAML frontmatter so the
...
pub fn splice_block(existing: &str, block_body: &str) -> String {
    ...
    if let Some(start_idx) = existing.find(&start) {
        ...
        // the implicit end â€” better to over-replace than to duplicate.
...
pub enum SyncOutcome {
    /// The file already contained the same rendered block â€” no write
...
/// contents. Used for MEMORA-owned sidecar files where there's no
...
mod tests {
    ...
    fn block_body_for_claude_is_import_reference_not_full_digest() {
        ...
        assert!(
            body.contains("@MEMORA-memories.md"),
    ...
    fn obsidian_destination_uses_expected_id_and_filename() {
        ...
        assert_eq!(Destination::OBSIDIAN.filename, "MEMORA-memories.md");
    ...
    fn owned_note_leads_with_frontmatter_and_inlines_digest() {
        ...
        // Frontmatter must be the very first bytes â€” Obsidian only parses a
        ...
        assert!(!note.contains("@MEMORA-memories.md"));
        assert!(!note.contains(&marker_start()));
    }

    #[test]
    fn owned_note_is_deterministic() {
        // No timestamps / volatile fields â†’ identical input renders identical
    ...
    fn sidecar_path_set_for_claude_unset_for_codex() {
        ...
        assert_eq!(
            ...
            Some(home.join("MEMORA-memories.md"))
        );
    ...
    fn write_atomic_creates_missing_parent_dirs() {
        let dir = tempfile::tempdir().unwrap();
        // Target sits two levels deep â€” neither directory exists yet.
    ...
    fn write_atomic_full_creates_new_file() {
        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("MEMORA-memories.md");
    ...
    fn write_atomic_full_overwrites_entire_file() {
        // Sidecar files are MEMORA-owned end-to-end â€” no splice.
    ...
    fn write_atomic_leaves_no_temp_sibling_after_success() {
        ...
        assert!(
            entries.iter().all(|n| !n.contains(".memora-tmp")),
