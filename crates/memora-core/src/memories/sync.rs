...
//! Cross-device memory sync â€” last-write-wins merge of memory rows.
//!
...
//! file. Stable cross-device identity is the `sync_uuid` field â€” minted
...
//!   * `id` (autoincrement, device-local) â€” receivers map by uuid.
//!   * `frame_id` (FK to local frames table; that frame doesn't exist
//!     on the receiving machine). Stripped on push, ignored on pull.
//!   * FTS rows â€” rebuilt by the existing INSERT/UPDATE triggers when
...
/// `pipes::sync::TOMBSTONE_TTL_DAYS` â€” long enough that a laptop that
...
pub struct MemoriesSyncManifest {
    ...
    /// Tombstones for deleted memories: `sync_uuid` â†’ deletion timestamp.
    #[serde(default)]
...
/// memora-engine) translates each variant into the right SQL.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MemorySyncAction {
    /// New from remote â€” INSERT.
    Imported(String),
    /// Existing locally, remote is newer â€” UPDATE.
    Updated(String),
    /// Tombstone won â€” DELETE local row (or skip insert).
    Deleted(String),
    /// Local is newer or identical â€” leave alone.
    Skipped(String),
}
...
/// the per-uuid actions to apply locally. Pure function â€” no I/O.
///
...
pub fn merge_manifests(
    ...
    // Borrow the keys rather than cloning every UUID into an owned set â€” the
    ...
    for uuid in all_uuids {
        ...
        match (local_mem, remote_mem, remote_ts) {
            // Remote tombstone â€” delete local if tombstone is newer.
            (Some(lm), _, Some(ts)) => {
            ...
            // Remote-only â€” import unless we have a newer local tombstone.
            (None, Some(rm), _) => {
            ...
            // Local-only â€” keep, will be pushed next cycle.
            (Some(lm), None, None) => {
                merged.memories.insert(uuid.clone(), lm.clone());
                actions.push(MemorySyncAction::Skipped(uuid.clone()));
            }
...
fn remote_wins(local: &SyncedMemory, remote: &SyncedMemory) -> bool {
    ...
    // Tie on timestamp â€” break by author id, lexicographic.
...
mod tests {
    ...
    fn remote_tombstone_deletes_older_local() {
        ...
        // Pure functions â€” local should be unchanged.
    ...
    fn local_resurrects_after_older_remote_tombstone() {
        // Local edited *after* remote deleted â€” local wins, no delete.
