...
//! Path resolution for MEMORA data directory.

use std::path::Path;
use std::{env, path::PathBuf};

/// Default MEMORA data directory. When `MEMORA_DATA_DIR` is set (e.g. for E2E tests),
/// that path is used; otherwise `~/.memora`.
pub fn default_memora_data_dir() -> PathBuf {
    env::var("MEMORA_DATA_DIR")
        .ok()
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| {
...
#[inline]
pub fn default_MEMORA_data_dir() -> PathBuf {
    default_memora_data_dir()
}

/// Get or create a persistent local machine ID.
pub fn get_or_create_machine_id() -> String {
    let machine_id_file = default_memora_data_dir().join("machine_id");
    ...
    let _ = std::fs::create_dir_all(default_memora_data_dir());
    let _ = std::fs::write(&machine_id_file, &id);
    id
}
...
/// copy a skill folder into the MEMORA skills store and to mirror that
...
/// before any window exists (`Failed to setup app: â€¦ Spotlight returned a null
        ...
        Err(e) => {
            tracing::warn!(
                "could not exclude {} from Spotlight indexing: {e:#}. \
                 continuing without the exclusion â€” recordings in this directory \
...
mod best_effort_tests {
    ...
    /// and the best-effort call must swallow it â€” startup depends on never
    /// aborting over a failed exclusion.
    #[test]
    fn best_effort_exclusion_never_fails_startup() {
        let missing = PathBuf::from("/nonexistent-MEMORA-spotlight-probe/data");
