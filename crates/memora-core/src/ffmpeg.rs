...
// until the app restarts â€” the entire session's audio is lost, never persisted.
// See MEMORA-CLI-V8 (337Ã—/day on Windows) and MEMORA-CLI-VC.
...
// start another download (MEMORA-CLI-WB).
...
pub fn find_ffmpeg_path() -> Option<PathBuf> {
    ...
    // Re-check under the write lock â€” another thread may have just healed it.
    if let Some(p) = guard.as_ref() {
        if ffmpeg_path_is_usable(p) {
            return Some(p.clone());
        }
...
/// somewhere on PATH. Frame extraction requires both â€” if we return an
...
/// path with NO side effects â€” unlike `get_ffmpeg_install_dir`, which creates the
...
fn find_ffmpeg_path_internal() -> Option<PathBuf> {
    ...
    // fail with "Library not loaded: â€¦/Cellar/ffmpeg/8.x_y/lib/libavdevice.62.dylib"
    // â€” we'd otherwise pick that broken binary over our own working bundle.
    #[cfg(target_os = "macos")]
    {
    ...
    // executable before anything on PATH â€” same rationale as the macOS block
    ...
    // launch â€” and the install's PATH-based version probe then fails with
    // "No such file or directory (os error 2)" even though the binary unpacked
    // fine (MEMORA-CLI-W3, macOS).
    #[cfg(target_os = "macos")]
    {
    ...
    //
    // We MUST only accept a PATH ffmpeg if a matching ffprobe is available â€”
    ...
    if let Ok(path) = which(EXECUTABLE_NAME) {
        ...
        debug!(
            "ffmpeg in PATH at {:?} has no matching ffprobe â€” falling through",
            path
        );
    }
    ...
    #[cfg(target_os = "macos")]
    {
        if let Ok(home) = std::env::var("HOME") {
            ...
            if ffmpeg_in_local_bin.exists() {
                ...
                debug!(
                    "ffmpeg in ~/.local/bin at {:?} has no matching ffprobe â€” falling through",
                    ffmpeg_in_local_bin
                );
            }
...
fn handle_ffmpeg_installation() -> Result<(), anyhow::Error> {
    ...
    // ffmpeg_version() â€” that resolves ffmpeg through PATH/sidecar_dir, which on
    ...
    // every launch (MEMORA-CLI-W3). Trust the file if it's on disk.
...
mod tests {
    ...
    fn missing_absolute_path_is_not_usable() {
        ...
        // This is the MEMORA-CLI-V8 condition: a once-resolved ffmpeg whose
        // file was relocated by an update â€” it must NOT be reported usable, so
        // find_ffmpeg_path() re-resolves instead of spawning a missing binary.
        #[cfg(windows)]
        let missing = PathBuf::from(r"C:\MEMORA-does-not-exist\nope-7f3a\ffmpeg.exe");
        #[cfg(not(windows))]
        let missing = PathBuf::from("/MEMORA-does-not-exist/nope-7f3a/ffmpeg");
        assert!(!ffmpeg_path_is_usable(&missing));
    }

    // MEMORA-CLI-W3: the macOS auto-install lands ffmpeg in ~/.local/bin and
