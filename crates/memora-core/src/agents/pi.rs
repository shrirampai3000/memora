...
pub const MEMORA_API_URL: &str = "https://api.memora.com/v1";
...
const CUSTOM_PROVIDER_USER_AGENT: &str = "MEMORA";
...
/// (0x00004000) so the bunâ†’piâ†’tool-call subtree yields CPU to whatever the
/// user is doing (#4849) â€” children inherit the class. Interactive chat is
...
/// accumulated *before* this attempt â€” kept separate so a run that alternates
...
/// budget.
///
/// Extracted out of `run_streaming` so the retry *loop* â€” not just the pure
/// `next_rate_limit_retry` policy â€” can be driven directly in tests via a
...
pub async fn MEMORA_cloud_models(api_url: &str, token: Option<&str>) -> serde_json::Value {
...
/// Only auto â€” if the gateway is down, nothing works anyway.
...
pub struct PiExecutor {
    /// MEMORA cloud token (for LLM calls via MEMORA proxy).
    ///
    /// Wrapped in `ArcSwap` so the desktop app can refresh it at
    /// runtime via the `set_cloud_token` Tauri command â€” without this the
    ...
    /// webview doesn't restart the MEMORA sidecar.
    pub user_token: Arc<ArcSwap<Option<String>>>,
    /// MEMORA API base URL (default: `https://api.memora.com/v1`).
    pub api_url: String,
    /// Bearer token for the *local* MEMORA-server API (localhost:3030).
    /// Exposed to the Pi subprocess as `MEMORA_LOCAL_API_KEY` so bash/TS
    /// pipe code can authenticate against the local server. `MEMORA_API_AUTH_KEY`
...
impl PiExecutor {
    pub fn new(user_token: Option<String>) -> Self {
        Self {
            ...
            api_url: MEMORA_API_URL.to_string(),
            api_auth_key: None,
        }
    }

    /// Construct a PiExecutor that shares its cloud-token storage with an
    /// external `Arc<ArcSwap>` â€” typically the same Arc held by the server's
    ...
    /// (the cloud_proxy.rs reader, Tauri-managed state) â€” write through any
    ...
    fn render_MEMORA_api_skill() -> String {
        ...
        if Self::cloud_media_analysis_enabled() {
            ...
            s.push('\n');
            s.push_str(
                include_str!("../../assets/skills/MEMORA-api/cloud_media_analysis_block.md")
                    .trim_end(),
            );
            s.push('\n');
        }
    ...
    /// Install or wipe the `MEMORA-team` enterprise-admin skill under an
    ...
    /// search, records) via `https://github.com/shrirampai3000/memora/api/enterprise/v1/*`. It
    ...
    ///
    /// Source of truth: `~/.memora/enterprise.json`. The Tauri host
    ...
    pub fn ensure_MEMORA_team_skill(skill_root: &Path) -> Result<Option<PathBuf>> {
        if !cfg!(feature = "enterprise-build") {
            return Ok(None);
        }

        let skill_dir = skill_root.join("MEMORA-team");
        ...
        if should_install {
            ...
            std::fs::write(
                &skill_path,
                include_str!("../../assets/skills/MEMORA-team/SKILL.md"),
            )?;
            debug!("MEMORA-team skill installed at {:?}", skill_path);
            return Ok(Some(skill_path));
        } else if skill_dir.exists() {
            // Wipe the whole dir â€” defense against partial state if a user
            // hand-edited or we ever ship sub-files in the future.
            std::fs::remove_dir_all(&skill_dir)?;
            info!(
                "MEMORA-team skill removed (no longer an enterprise admin or license inactive)"
            );
        }
    ...
    /// True only in the Enterprise app when `~/.memora/enterprise.json`
    ...
    /// Enterprise apps share `~/.memora` on one machine.
    ///
    /// Conservative: any I/O or parse error means "no" so we fail closed â€”
    ...
    fn is_enterprise_admin() -> bool {
        ...
        let ent_path = home.join(".memora").join("enterprise.json");
        ...
        // token (sk_ent_â€¦) the admin mints once at
        // github.com/shrirampai3000/memora/enterprise?tab=tokens and pastes into Settings â†’
        // Enterprise â†’ Admin API token. Stored on disk under
    ...
    /// Ensure MEMORA skills exist in `project_dir/.pi/skills/`.
    pub fn ensure_MEMORA_skill(project_dir: &Path) -> Result<()> {
        // Always-on baseline skills (every pi-agent session needs these).
        let api_skill = Self::render_MEMORA_api_skill();
        let skills: &[(&str, &str)] = &[
            ("MEMORA-api", api_skill.as_str()),
            (
                "MEMORA-cli",
                include_str!("../../assets/skills/MEMORA-cli/SKILL.md"),
            ),
            (
                "render-html-report",
        ...
        // Clean up deprecated skills from the 8â†’2 consolidation.
        ...
        let legacy_team_skill = skills_root.join("MEMORA-team");
        if legacy_team_skill.exists() {
            std::fs::remove_dir_all(&legacy_team_skill)?;
            debug!("removed legacy auto-discovered team skill");
        }

        // Mirror user-imported skills (Settings â†’ Connections â†’ Skills) into
    ...
    /// baseline (`MEMORA-api`/`-cli`/`-team`) and hand-authored skills and
    /// safely remove ones the user has since deleted from the store.
    const USER_SKILL_MARKER: &'static str = ".memora-managed";

    /// Baseline skills MEMORA writes into every session itself
    /// ([`Self::ensure_MEMORA_skill`]).
    ...
    /// desktop app's Settings â†’ Connections â†’ Skills importer.
    ///
    ...
    pub fn sync_user_skills(project_dir: &Path) -> Result<()> {
        let store = crate::paths::default_MEMORA_data_dir().join("skills");
    ...
    fn sync_user_skills_from(store: &Path, project_dir: &Path) -> Result<()> {
        ...
        if let Ok(entries) = std::fs::read_dir(store) {
            for entry in entries.flatten() {
                ...
                // Never let a store entry shadow a baseline skill MEMORA
                // writes itself â€” that would clobber it and, once marked, risk
                ...
                let copy = (|| -> std::io::Result<()> {
                    ...
                    std::fs::write(
                        ...
                        b"mirrored from <data>/skills by MEMORA\n",
                    )?;
                    Ok(())
                })();
                match copy {
    ...
    /// Install or remove the MEMORA-permissions extension based on config.
    ...
    pub fn ensure_permissions_extension(
        ...
        let ext_path = ext_dir.join("MEMORA-permissions.ts");

        if perms.has_any_restrictions() {
            std::fs::create_dir_all(&ext_dir)?;
            let ext_content = include_str!("../../assets/extensions/MEMORA-permissions.ts");
            std::fs::write(&ext_path, ext_content)?;
            debug!(
                "MEMORA-permissions extension installed at {:?}",
                ext_path
            );
        } else if ext_path.exists() {
            std::fs::remove_file(&ext_path)?;
            info!("MEMORA-permissions extension removed (no restrictions configured)");
        }
    ...
    pub fn ensure_MEMORA_skill_filtered(
        project_dir: &Path,
        config: &crate::pipes::PipeConfig,
    ) -> Result<()> {
        use crate::pipes::permissions::PipePermissions;
        ...
        let all_skills: &[(&str, &str, Box<dyn Fn(&PipePermissions) -> bool>)] = &[
            (
                "MEMORA-api",
                api_skill.as_str(),
                Box::new(|_| true), // always installed â€” unified API skill
            ),
            (
                "MEMORA-cli",
                include_str!("../../assets/skills/MEMORA-cli/SKILL.md"),
                Box::new(|_| true), // always installed â€” pipe & connection management
            ),
            (
                "render-html-report",
                include_str!("../../assets/skills/render-html-report/SKILL.md"),
                // Output-formatting skill, not endpoint-gated â€” always staged,
    ...
    /// If PipeManager already wrote `.memora-permissions.json`, use filtered.
    /// Otherwise install all skills (CLI / direct executor usage).
    pub fn ensure_MEMORA_skill_auto(project_dir: &Path) -> Result<()> {
        let perms_path = project_dir.join(".memora-permissions.json");
        if perms_path.exists() {
            // PipeManager already handled filtered installation â€” skip
            // to avoid overwriting with unfiltered skills.
            debug!("permissions file found, skipping unfiltered skill install");
            Ok(())
        } else {
            Self::ensure_MEMORA_skill(project_dir)
        }
    }
    ...
    /// Install orphan guard extension â€” kills bun if the parent (MEMORA) dies.
    ...
    /// Always installed â€” does nothing harmful when zero servers are
    ...
    /// Web search uses the MEMORA cloud backend, so we only enable it
    /// for MEMORA-cloud to avoid sending data to our backend when the
    /// user chose a local/custom provider.
    pub fn ensure_web_search_extension(project_dir: &Path, provider: Option<&str>) -> Result<()> {
        Self::ensure_web_search_extension_with_api_url(project_dir, provider, MEMORA_API_URL)
    }
    ...
    fn ensure_web_search_extension_with_api_url(
        ...
        let is_MEMORA_cloud = matches!(
            provider,
            None | Some("MEMORA") | Some("MEMORA-cloud") | Some("pi")
        );

        if is_MEMORA_cloud {
            std::fs::create_dir_all(&ext_dir)?;
            let ext_content = include_str!("../../assets/extensions/web-search.ts")
                .replace(MEMORA_API_URL, api_url);
            std::fs::write(&ext_path, ext_content)?;
            debug!("web-search extension installed at {:?}", ext_path);
        } else if ext_path.exists() {
            std::fs::remove_file(&ext_path)?;
            info!(
                "web-search extension removed (provider {:?} is not MEMORA-cloud)",
                provider
            );
        }
    ...
    /// Merge MEMORA provider (and optionally the pipe's own provider) into
    ...
    ///
    /// When a pipe uses a non-MEMORA provider (e.g. ollama, openai), pass
    ...
    pub async fn ensure_pi_config(
        ...
        // Only add MEMORA cloud provider if it's the intended provider
        // (or no provider specified). If the user explicitly chose ollama/openai/custom,
        // do NOT write MEMORA into models.json to avoid silent credit drain via fallback.
        let should_add_MEMORA = match provider {
            None => true,
            Some("MEMORA") | Some("MEMORA-cloud") | Some("pi") => true,
            Some(_) => false,
        };

        if should_add_MEMORA {
            // Use actual token value in apiKey â€” Pi doesn't resolve bare env var
            // names, so writing the literal string "MEMORA_API_KEY" causes
            ...
            let api_key_value = user_token
                .map(|t| t.to_string())
                .or_else(|| std::env::var("MEMORA_API_KEY").ok())
                .unwrap_or_else(|| "$MEMORA_API_KEY".to_string());
            let api_key_value = api_key_value.as_str();
            let models = MEMORA_cloud_models(api_url, user_token).await;
            ...
            // and an old gateway simply ignores the unknown header (â†’ standard),
            // so there's no deploy-order coupling.
            let MEMORA_provider = json!({
                "baseUrl": api_url,
                "api": "openai-completions",
                "apiKey": api_key_value,
                "authHeader": true,
                "headers": {
                    "x-MEMORA-latency": "background",
                    "x-MEMORA-workload": "pipe"
                },
                "models": models
            });
            ...
            {
                providers.insert("MEMORA".to_string(), MEMORA_provider);
            }
        }
        ...
        if let (Some(prov), Some(mdl)) = (provider, model) {
            if prov != "MEMORA" {
                ...
                // providers (including MEMORA cloud). Skip the entry instead.
                if base_url.is_empty() {
                        ...
                        .and_then(|p| p.as_object_mut())
                    {
                        ...
                        if let Some(obj) = entry.as_object_mut() {
                            ...
                            // api (wire format): always set â€” it's a function of model
        ...
        // models.json embeds the raw cloud JWT as the MEMORA provider's
        // apiKey while signed in (#3943) â€” same hardening as auth.json below.
        #[cfg(unix)]
        {
        ...
        // -- auth.json: merge/remove MEMORA token, preserve other providers --
        // Only manage MEMORA auth when MEMORA provider is actually being used.
        ...
        if should_add_MEMORA {
            if let Some(token) = user_token.filter(|token| !token.is_empty()) {
                ...
                if let Some(obj) = auth.as_object_mut() {
                    obj.insert("MEMORA".to_string(), api_key_credential(token));
                }

                write_auth_json(&auth_path, &auth)?;
            } else {
                remove_MEMORA_auth_from_path(&auth_path)?;
            }
        } else {
            // BYOK-only users never reach the MEMORA branch, but their
            // auth.json can still hold legacy entries seeded from the user's
            // global `~/.pi/agent/auth.json` â€” upgrade those too.
    ...
    /// Passthrough for non-MEMORA providers.
    fn resolve_model(requested: &str, provider: &str) -> String {
        if provider != "MEMORA" {
            return requested.to_string();
        }
        // Strip @date suffix if present â€” the gateway validates the model ID
    ...
    /// Resolve a MEMORA-cloud model AND validate it against the tier's
    ...
    /// gateway and got rejected â€” the pipe failed with an opaque error even
    ...
    ///
    /// For non-MEMORA providers (ollama / openai-byok / anthropic-byok /
    /// custom) we don't have an allow-list and must not touch the model â€” the
    ...
    ///
    /// Returns `(resolved_model, fell_back_from)` â€” `fell_back_from` is
    ...
    async fn resolve_MEMORA_model(
        ...
        // down) we get the minimal fallback list â€” in that case we trust the
        ...
        let fetch_allowed = |token: Option<String>| {
            ...
            async move {
                let models = MEMORA_cloud_models(&api_url, token.as_deref()).await;
                models
                    .as_array()
                    .map(|arr| {
                        arr.iter()
        ...
        // Tier-flicker self-heal: model looks disallowed â†’ re-read the token
    ...
    /// Pure validation step for [`resolve_MEMORA_model`] (network-free so
    /// it's unit-testable).
    ///
    /// `Ok(model)`  â†’ the requested model is allowed (or we can't validate).
    /// `Err(model)` â†’ requested not allowed; the returned value is the fallback.
    fn pick_allowed_model(requested: &str, allowed: &[String]) -> Result<String, String> {
        // No catalog, or only the gateway fallback sentinel â†’ we
    ...
    async fn spawn_pi(
        ...
        if let Some(key) = provider_api_key {
            if !key.is_empty() {
                match resolved_provider {
                    ...
                    // Ensure MEMORA API key is set as env var fallback
                    "MEMORA" if cloud_token.is_none() => {
                        cmd.env("MEMORA_API_KEY", key);
                    }
                    _ => {}
                }
            }
        ...
        // Canonical name: MEMORA_LOCAL_API_KEY. The AUTH_KEY alias is
        ...
        // overwrite). TODO(remove next release): drop MEMORA_API_AUTH_KEY.
        if let Some(ref key) = self.api_auth_key {
            cmd.env("MEMORA_LOCAL_API_KEY", key);
            cmd.env("MEMORA_API_AUTH_KEY", key); // deprecated alias
        }
    ...
    async fn spawn_pi_streaming(
        ...
        // pi 0.80 gates project-dir resources (.pi/extensions â€” mcp-bridge,
        // connection-gate, â€¦) behind a project-trust prompt that can never be
        ...
        // populated exclusively by MEMORA, so it is trusted by definition.
        cmd.arg("--approve");
        if continue_session {
            cmd.arg("--continue");
        } else {
        ...
        // See spawn_pi above â€” TODO(remove next release): drop the deprecated alias.
        if let Some(ref key) = self.api_auth_key {
            cmd.env("MEMORA_LOCAL_API_KEY", key);
            cmd.env("MEMORA_API_AUTH_KEY", key); // deprecated alias
        }
        ...
        if let Some(ids) = mcp_server_allowlist {
            cmd.env("MEMORA_MCP_SERVER_ALLOWLIST", ids.join(","));
        }
        ...
        // bash shim reads MEMORA_SESSION_ID and adds x-MEMORA-session;
        ...
        if let Some(owner) = session_owner {
            cmd.env("MEMORA_SESSION_ID", owner);
        ...
        // Read remaining stderr (lossy â€” same reason as stdout above)
...
impl AgentExecutor for PiExecutor {
    async fn run(
        ...
        // Provider resolution:
        // 1. Explicit provider from pipe frontmatter â†’ use it
        // 2. No provider specified â†’ MEMORA cloud (default)
        let resolved_provider = provider.unwrap_or("MEMORA").to_string();

        let (resolved_model, fell_back_from) = self
            .resolve_MEMORA_model(model, &resolved_provider)
            .await;
        if let Some(ref original) = fell_back_from {
            warn!(
                "pipe model '{}' unavailable on current tier â€” ran on '{}' instead",
        ...
        Self::ensure_MEMORA_skill_auto(working_dir)?;
        ...
        let pi_path = find_pi_executable().ok_or_else(|| {
            anyhow!(
                "pi not found. try restarting the app or delete ~/.memora/pi-agent and restart"
            )
        })?;
        ...
        // any stale entry gets refreshed. Do NOT delete the file â€” that would
        ...
        if !output.success && output.stderr.to_lowercase().contains("not found") {
            ...
            // Re-read the cloud token â€” it may have been refreshed via
    ...
    async fn run_streaming(
        ...
        // Re-read after resolution: resolve_MEMORA_model may have refreshed
        ...
        // delete the file â€” would wipe user-managed entries).
        if !output.success && output.stderr.to_lowercase().contains("not found") {
            warn!(
                "pi model not found, re-merging managed providers (stderr: {})",
                output.stderr.trim()
        ...
        // that hint, wait, and re-run instead of failing the whole run â€”
        // which previously left automations silently doing nothing.
        //
        // Capacity contention isn't a real error â€” someone else is just using
        // the shared slot â€” so it gets a time budget
    ...
    async fn ensure_installed(&self) -> Result<()> {
        if find_pi_executable().is_some() {
            ...
            if !is_local_pi_version_current() {
                info!("pi version mismatch â€” upgrading to {}", PI_PACKAGE);
        ...
        let bun = find_bun_executable()
            .ok_or_else(|| anyhow!("bun not found â€” install from https://bun.sh"))?;
        ...
        // reproducible from the log alone (and a bun that can't even run â€”
        // e.g. SIGILL on an unsupported CPU â€” is exposed before the install).
        info!(
        ...
        #[cfg(windows)]
        {
            // CPU/IO-heavy dependency install â€” background bootstrap work.
            cmd.creation_flags(BACKGROUND_SPAWN_FLAGS);
        }
        ...
        if output.status.success() {
            ...
            // which used to log here as "pi installation failed: " â€” nothing.
    ...
    fn MEMORA_api_url(&self) -> &str {
        &self.api_url
    }
}

// ---------------------------------------------------------------------------
// Helpers (extracted from apps/memora-app-tauri/src-tauri/src/pi.rs)
// ---------------------------------------------------------------------------

/// MEMORA's private pi agent dir (models.json, auth.json, sessions, â€¦).
///
/// Isolated from the user's global `~/.pi/agent` so MEMORA never rewrites
/// config a standalone pi install owns
/// (https://github.com/MEMORA/MEMORA/issues/4002) and never loads the user's
/// global pi packages â€” whose tools can collide with ours and abort the run â€”
/// into pipe/chat runs (https://github.com/MEMORA/MEMORA/issues/3812).
/// Every pi spawn must pass this dir via the
/// `PI_CODING_AGENT_DIR` env var (see [`apply_pi_isolation_env`]).
///
/// Escape hatch: `MEMORA_PI_AGENT_DIR` overrides the location; setting it
/// to `~/.pi/agent` restores the old shared-config behavior.
pub fn pi_config_dir() -> Result<PathBuf> {
    let dir = match std::env::var("MEMORA_PI_AGENT_DIR") {
        ...
        _ => crate::paths::default_MEMORA_data_dir().join("pi-config"),
    };
...
/// Return whether a Pi npm package is enabled in MEMORA's isolated config.
///
...
/// Set the env vars that scope a pi subprocess to MEMORA's private agent
...
///
/// Earlier releases wrote MEMORA's provider/auth into the global config
...
/// - `settings.json`: copied with the `packages` key stripped â€” globally
///   installed pi packages are exactly the conflict vector from
///   https://github.com/MEMORA/MEMORA/issues/3812.
/// - `sessions/<encoded-cwd>/`: copied only for cwds under the MEMORA
...
/// losers proceed without waiting â€” `ensure_pi_config` rewrites models.json
...
fn seed_pi_config_from_global(dest: &Path) {
    ...
    let data_dir = crate::paths::default_MEMORA_data_dir();
...
fn seed_from_global(global: &Path, dest: &Path, data_dir: &Path) -> bool {
        ...
        .open(&lock)
    {
        ...
        Err(_) => {
            ...
            // unseeded forever. Otherwise someone is actively seeding â€”
    ...
    // compaction, â€¦) so behavior matches pre-isolation, but drop:
    // - `packages`: global pi packages are the conflict vector from
    //   https://github.com/MEMORA/MEMORA/issues/3812;
    // - `defaultProvider`/`defaultModel`: those are the *user's* personal pi
    //   defaults. MEMORA passes --provider/--model on every spawn, so
    //   they'd never be read â€” except by a future flagless spawn, which must
    //   not silently land on the user's BYOK provider. Pin MEMORA's own
    //   safe fallback instead ("MEMORA"/"auto": the gateway picks a
    ...
    if let Some(obj) = settings.as_object_mut() {
        obj.remove("packages");
        obj.insert("defaultProvider".to_string(), json!("MEMORA"));
    ...
    // Sessions for MEMORA-owned cwds. Pi encodes a session dir name as
...
/// `undefined` unless it matches `{"type": "oauth"|"api_key", â€¦}` â€” there is no
/// fallback to the provider's `apiKey` in models.json once a credential exists.
/// MEMORA wrote the cloud token as a bare string, which pi <=0.80 accepted,
/// so after the 0.83 bump every signed-in user's hosted AI (chat *and* every
/// background pipe) fails with "Provider is not configured: MEMORA".
/// Legacy `{"apiKey": â€¦}` entries â€” seeded from a user's global
/// `~/.pi/agent/auth.json` for BYOK providers â€” fail identically.
///
...
fn upgrade_legacy_pi_credential(value: &serde_json::Value) -> Option<serde_json::Value> {
    match value {
        // pi <=0.80 MEMORA format: the bare token string.
        serde_json::Value::String(key) => Some(api_key_credential(key)),
        // Legacy BYOK format: `{"apiKey": "â€¦"}` with no discriminant.
        serde_json::Value::Object(obj) if !obj.contains_key("type") => {
            ...
            // `env` drives pi's `resolveConfigValue` indirection â€” preserve it.
...
/// sidecar â€” both must emit the tagged shape or pi 0.83 rejects the whole
/// provider.
...
fn remove_MEMORA_auth_from_path(auth_path: &Path) -> Result<()> {
    if !auth_path.exists() {
        return Ok(());
    }
    ...
    let removed = auth
        .as_object_mut()
        .map(|obj| obj.remove("MEMORA").is_some())
        .unwrap_or(false);
    // Signed-out users keep their BYOK credentials â€” upgrade those too.
...
/// the OOM killer) â€” exactly the case that used to log as an empty error.
...
/// Codeâ†’string mapping, factored out of [`describe_exit_status`] so it can
/// be unit-tested (`ExitStatus` isn't constructible portably in tests) â€”
...
pub fn describe_exit_status_code(code: i32) -> String {
    #[cfg(windows)]
    {
        ...
        // code before writing a single byte to stderr â€” exactly the case that
        ...
        if code == -1073741795i32 {
            return format!(
                "exit code {code} (0xC000001D, illegal instruction; this CPU may lack AVX2 â€” the stock bun build requires it; use the baseline bun variant)"
            );
        }
    }
...
/// signal deaths leave both streams empty â€” the status is then the only clue).
...
/// Returns the MEMORA-managed pi install directory (`~/.memora/pi-agent/` or MEMORA_DATA_DIR/pi-agent).
fn pi_local_install_dir() -> Option<PathBuf> {
    Some(crate::paths::default_MEMORA_data_dir().join("pi-agent"))
}
...
/// nested `pi` invocations resolve to MEMORA's pinned Pi package, not an
...
fn is_local_pi_version_current() -> bool {
    ...
    // PI_PACKAGE is "<scope>/pi-coding-agent@<ver>" â€” extract version after last '@'
...
fn seed_pi_package_json(install_dir: &Path) {
    ...
    if pkg_path.exists() {
        ...
        // in package.json (MEMORA-APP-AR â€” bun then errors at SyntaxError).
        ...
        if corrupted {
            warn!(
                "pi-agent package.json at {} is unreadable or corrupted â€” re-seeding",
                pkg_path.display()
            );
...
pub fn find_pi_executable() -> Option<String> {
    // 1. Check MEMORA-managed local install first (preferred â€” we control the deps)
...
///
/// Pi's shebang is `#!/usr/bin/env node`, but MEMORA only bundles bun
...
/// from the `.cmd` shim and run it directly â€” no cmd.exe involved.
fn build_async_command(path: &str) -> tokio::process::Command {
    #[cfg(windows)]
    {
        ...
        let mut cmd = if let Some(ref js_path) = js_entry {
            ...
            // Local install returns a .js entrypoint â€” run with bun
            if let Some(bun) = find_bun_executable() {
                let mut c = tokio_bun_command(&bun);
                c.arg(path);
...
/// Kill a process group (SIGTERM â†’ 5s â†’ SIGKILL).
...
/// members of its process group â€” e.g. a stdio MCP server or bun helper that
...
/// This is a blocking function â€” call from a background thread.
...
fn download_portable_git() -> std::result::Result<String, String> {
    ...
    let MEMORA_dir = PathBuf::from(&local_app_data).join("MEMORA");
    let git_dir = MEMORA_dir.join("git-portable");
    ...
    std::fs::create_dir_all(&MEMORA_dir)
        .map_err(|e| format!("Failed to create MEMORA data dir: {}", e))?;
    ...
    // failure. We're about to execute this file â€” never run it unverified.
    ...
    let extract_temp = MEMORA_dir.join("git-portable-extracting");
...
/// `expected` (hex, case-insensitive). On ANY failure â€” unreadable file or
/// mismatch â€” the file is deleted before returning Err, so a bad archive can
...
/// fail-open. These downloads are executables we run â€” verification is
/// mandatory.
#[cfg(windows)]
...
/// no AVX2 required) to `%LOCALAPPDATA%\MEMORA\bun-baseline\bun.exe`.
/// Same structure as [`download_portable_git`], with one crucial difference:
/// download via curl.exe ONLY â€” the bun-based download path is useless here
...
fn download_baseline_bun() -> std::result::Result<String, String> {
    ...
    // apps/memora-app-tauri/scripts/pre_build.js (bunVersion) â€” bump both
    ...
    let local_app_data =
        std::env::var("LOCALAPPDATA").map_err(|_| "LOCALAPPDATA env var not set".to_string())?;
    let MEMORA_dir = PathBuf::from(&local_app_data).join("MEMORA");
    let bun_dir = MEMORA_dir.join("bun-baseline");
    ...
    let extract_temp = MEMORA_dir.join("bun-baseline-extracting");
    let _ = std::fs::remove_dir_all(&extract_temp);
...
/// Safe to call â€” never crashes, only logs warnings on failure.
...
mod tests {
    ...
    fn managed_pipe_guidance_only_ships_in_enterprise_team_skill() {
        let consumer_skill = include_str!("../../assets/skills/MEMORA-cli/SKILL.md");
        let enterprise_skill = include_str!("../../assets/skills/MEMORA-team/SKILL.md");

        assert!(!consumer_skill.contains("Enterprise Team Pipes"));
        assert!(!consumer_skill.contains("MEMORA team pipes"));
        assert!(enterprise_skill.contains("MEMORA team pipes list"));
        assert!(enterprise_skill.contains("MEMORA team pipes schedule"));
    ...
    fn consumer_build_never_enables_enterprise_team_skill() {
        ...
        let sentinel = root.path().join("MEMORA-team").join("KEEP");
        ...
        let installed = PiExecutor::ensure_MEMORA_team_skill(root.path()).unwrap();
    ...
    fn web_search_extension_uses_executor_gateway_url() {
        ...
        assert!(!content.contains(MEMORA_API_URL));
    }
    ...
    #[test]
    fn pi_child_path_prefers_MEMORA_local_pi() {
    ...
    ///
    /// Run with:
    /// MEMORA_E2E_CLOUD_TOKEN=... cargo test -p memora-core \
    ///   pi_subagents_parent_child_e2e -- --ignored --nocapture
    #[tokio::test]
    #[ignore]
    async fn pi_subagents_parent_child_e2e() {
        let token = std::env::var("MEMORA_E2E_CLOUD_TOKEN")
            .expect("MEMORA_E2E_CLOUD_TOKEN is required for this live test");
        assert!(
    ...
    #[test]
    fn clear_MEMORA_auth_preserves_other_provider_tokens() {
        let dir = tempfile::tempdir().expect("tempdir");
        ...
        remove_MEMORA_auth_from_path(&auth_path).expect("clear MEMORA auth");

        let auth: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&auth_path).unwrap()).unwrap();
        assert!(auth.get("MEMORA").is_none());
    ...
    /// credential that is not tagged, with no fallback to models.json â€” so a
    /// bare-string token makes hosted AI fail with
    /// "Provider is not configured: MEMORA".
    #[test]
    fn legacy_credentials_upgrade_to_tagged_api_key() {
        ...
        // `env` drives pi's resolveConfigValue indirection â€” must survive.
        assert_eq!(
            auth["openai"],
    ...
    fn upgrade_at_path_rewrites_legacy_file_and_skips_clean_one() {
        let dir = tempfile::tempdir().expect("tempdir");
        let auth_path = dir.path().join("auth.json");
        std::fs::write(&auth_path, r#"{"MEMORA":"bare-jwt"}"#).expect("write auth");
    ...
    #[test]
    fn clear_MEMORA_auth_missing_file_is_noop() {
        let dir = tempfile::tempdir().expect("tempdir");
        let auth_path = dir.path().join("missing-auth.json");

        remove_MEMORA_auth_from_path(&auth_path).expect("missing auth is ok");
    ...
    fn sync_user_skills_mirrors_and_self_cleans() {
        ...
        // A baseline skill already written by MEMORA (no marker) must survive.
        std::fs::create_dir_all(skills.join("MEMORA-api")).unwrap();
        std::fs::write(skills.join("MEMORA-api").join("SKILL.md"), "base").unwrap();
        // A store entry colliding with a baseline name must be ignored, never
        // mirrored â€” otherwise it would clobber the baseline above.
        std::fs::create_dir_all(store.join("MEMORA-api")).unwrap();
        std::fs::write(store.join("MEMORA-api").join("SKILL.md"), "evil").unwrap();
        ...
        assert_eq!(
            std::fs::read_to_string(skills.join("MEMORA-api").join("SKILL.md")).unwrap(),
            "base"
        );
        assert!(!skills
            .join("MEMORA-api")
        ...
        // Remove from store, sync again â†’ our mirror is gone, baseline stays.
        std::fs::remove_dir_all(store.join("foo")).unwrap();
        PiExecutor::sync_user_skills_from(&store, &project).unwrap();
        assert!(!skills.join("foo").exists());
        assert!(skills.join("MEMORA-api").join("SKILL.md").exists());
    ...
    fn test_lossy_utf8_handles_invalid_bytes() {
        ...
        // Lossy conversion should succeed â€” this is what our fix does
    ...
    /// First-run seed copies config + MEMORA-owned sessions from the
    ...
    #[test]
    fn seed_from_global_copies_config_and_MEMORA_sessions() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let global = tmp.path().join("global");
        let dest = tmp.path().join("isolated");
        let data_dir = tmp.path().join("home").join(".memora");
        ...
        .unwrap();
        std::fs::write(global.join("auth.json"), r#"{"MEMORA":"tok"}"#).unwrap();
        std::fs::write(
            global.join("settings.json"),
            // The user's personal pi defaults must NOT leak into MEMORA's
            // config; run-affecting settings (theme, thinking, â€¦) must.
            r#"{"theme":"dark","packages":["npm:pi-web-access"],"defaultProvider":"anthropic","defaultModel":"claude-opus-4-8"}"#,
        )
        .unwrap();

        // Session dirs: one for a MEMORA cwd (copied), one for an
        // unrelated project (left behind). Encoding mirrors pi's
        // session-manager: leading separator stripped, [/\:] â†’ '-'.
        ...
        // Personal defaults replaced by MEMORA's safe fallback: a future
        // flagless spawn must never silently run on the user's BYOK provider.
        assert_eq!(settings["defaultProvider"], "MEMORA");
        assert_eq!(settings["defaultModel"], "auto");

        // Only the MEMORA-owned session dir came over.
        assert!(dest
            .join("sessions")
            .join(format!("--{}-pi-chat--", encoded))
            .join("s1.jsonl")
    ...
    fn seed_from_global_handles_missing_global_and_self_target() {
        ...
        let dest = tmp.path().join("isolated");
        let data_dir = tmp.path().join(".memora");

        assert!(seed_from_global(&global, &dest, &data_dir));
        assert!(dest.join(PI_MIGRATION_MARKER).exists());

        // dest == global (MEMORA_PI_AGENT_DIR=~/.pi/agent escape hatch):
    ...
    fn test_parse_rate_limit_reset_secs() {
        ...
        // No structured field â€” fall back to the prose hint.
        assert_eq!(
    ...
    fn test_pick_allowed_model() {
        ...
        // Empty catalog (gateway returned an empty list) â†’ trust the requested
        ...
        // Gateway fallback sentinel ["auto"] (gateway unreachable â†’ fallback_cloud_models)
    ...
    async fn gateway_output_budget_flows_from_http_to_pi_catalog() {
        ...
        let models = MEMORA_cloud_models(&server.uri(), None).await;
    ...
    // scheduled pipes both execute through it â€” see the caller in
    // `PiExecutor::run_streaming`) â€” so one set of cases here covers the
    ...
    fn test_next_rate_limit_retry_capacity_reserved_outlasts_fixed_retry_cap() {
        ...
        // Simulate holding capacity for 35s (7 retries at 5s each) â€” longer
    ...
    // The tests below drive `retry_rate_limited_output` itself â€” the actual
    // loop `run_streaming` calls in production (both interactive chat and
    // scheduled pipes go through it) â€” rather than only the extracted
    ...
    async fn test_retry_rate_limited_output_capacity_reserved_succeeds_after_more_than_three_retries(
        ...
        // Simulate the shared hosted-AI slot staying busy for 4 attempts â€”
        // one more than MAX_RATE_LIMIT_RETRIES=3 â€” before it frees up.
        ...
        // The status channel actually received one event per retry â€” proves
    ...
    async fn test_retry_rate_limited_output_capacity_reserved_stops_at_reservation_ttl_live() {
        ...
        // (60s) â€” same safety clamp a generic rate limit gets, so an oversized
        // hint can't stall a run â€” leaving 60s waits until the budget is spent.
    ...
    fn custom_provider_compat_repairs_ai_genesis_and_overrides_sdk_user_agent() {
        for base_url in ["https://ai.ai-genesis.app", "https://api.ai-genesis.app/"] {
            ...
            assert_eq!(provider["headers"]["User-Agent"], "MEMORA");
        }
    }
    ...
    async fn test_ensure_pi_config_adds_ollama_provider() {
        ...
        // the developer's real `~/.memora/pi-chat/.pi/models.json`, so it
        // mutates live config on every `cargo test -p memora-core` and then
        ...
        std::env::set_var("MEMORA_PI_AGENT_DIR", temp.path());
        ...
        std::env::remove_var("MEMORA_PI_AGENT_DIR");
    }
    ...
    /// `with_shared_user_token` â€” verify both work end-to-end.
    #[tokio::test]
    ...
    async fn shared_arc_propagates_token_writes_across_executors() {
        ...
        // Write via executor A â€” both see it.
        ...
        // path which holds only the Arc, not the executor) â€” both see it.
    ...
    /// Regression guard for MEMORA-APP-AR: a corrupted package.json
    ...
    fn install_failure_message_is_never_empty() {
        ...
        // Non-zero exit, NOTHING on either stream â€” the exact shape that used
        ...
        // stderr empty but stdout has the diagnostics â€” both tails included.
