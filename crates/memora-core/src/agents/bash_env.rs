...
//! then reads the skill file, learns about `$MEMORA_LOCAL_API_KEY`, and
//! retries â€” burning a wasted tool call per fresh session.
//!
...
//! MEMORA API (localhost:3030) or local app server (localhost:11435).
//! Non-MEMORA calls pass through untouched â€” the token never leaks.
//!
...
//!
//! Reads `$MEMORA_LOCAL_API_KEY` only â€” every spawn path (Tauri chat,
//! core pipe-executor) is now contractually required to export it.
//! `MEMORA_API_AUTH_KEY` was a historical second name from when the two
...
///
/// Only matches literal localhost/loopback MEMORA API or app-server
/// substrings in command arguments. If none match, `curl` runs unchanged.
pub const WRAPPER_SCRIPT: &str = r#"# MEMORA â€” auto-injected by pi-agent bash subshells (do not edit by hand)
# Transparently adds Authorization: Bearer to curl calls that target the
# local MEMORA API/app server, tags them with x-MEMORA-session
# (the chat/pipe that owns this agent, from MEMORA_SESSION_ID) so
# owned-browser and notification source links route to the right chat, and â€” when
# MEMORA_FILTER_PII=1 â€” rewrites any /search URL to include filter_pii=1
# so responses are PII-redacted before Pi sees them. Other curl calls pass
# through unchanged â€” the token never leaks to third-party hosts.
#
# Regenerated on every pi-agent spawn from memora-core::agents::bash_env.

# Hide the cloud-LLM JWT (MEMORA_API_KEY) from the agent's bash. The
...
# 2026-05-05): the agent saw an env var named "MEMORA_API_KEY", used
...
# phantom auth bug before concluding â€” wrongly â€” that "the local API
# needs a full JWT token". Unset it here so the only auth-shaped name
# the agent can see is MEMORA_LOCAL_API_KEY, which is correct.
unset MEMORA_API_KEY

_sp_auth_key() {
  # spawn paths guarantee MEMORA_LOCAL_API_KEY is set (see pi.rs).
  printf '%s' "${MEMORA_LOCAL_API_KEY:-}"
}
...
curl() {
  ...
  # (x-MEMORA-session); the navigate handler rides it to the frontend so a
  # background pipe's page doesn't pop into whatever chat is on screen. Empty
  # for spawn paths that don't set it â€” then the call is simply untagged.
  sid="${MEMORA_SESSION_ID:-}"
  if [ "${MEMORA_FILTER_PII:-}" = "1" ]; then
    add_filter=1
  fi
  ...
  if [ "$has_local" = "1" ]; then
    ...
    [ -n "$sid" ] && hdrs+=(-H "x-MEMORA-session: $sid")
...
/// Path to the bash wrapper under `~/.memora/<WRAPPER_RELATIVE_PATH>`,
/// relative to the supplied MEMORA data dir.
pub fn wrapper_path_under(MEMORA_dir: &Path) -> PathBuf {
    MEMORA_dir.join(WRAPPER_RELATIVE_PATH)
}

/// Write the wrapper into the given MEMORA data dir if the on-disk
/// content doesn't match the current [`WRAPPER_SCRIPT`]. Idempotent â€” safe
/// to call on every spawn. Returns the absolute path to the wrapper.
///
/// Intended caller pattern:
/// ```no_run
/// # use std::path::Path;
/// let sp_dir = Path::new("/Users/me/.memora");
/// let p = memora_core::agents::bash_env::ensure_wrapper(sp_dir).unwrap();
/// // cmd.env("BASH_ENV", p);
/// ```
pub fn ensure_wrapper(MEMORA_dir: &Path) -> Result<PathBuf> {
    let path = wrapper_path_under(MEMORA_dir);

    // Skip the write when the file already has the right content â€”
...
/// Resolve `~/.memora` using the same logic the rest of the app uses,
...
pub fn ensure_wrapper_in_default_dir() -> Result<PathBuf> {
    ...
    ensure_wrapper(&home.join(".memora"))
}
...
mod tests {
    ...
    fn ensure_wrapper_is_idempotent() {
        ...
        // content unchanged â†’ we did not rewrite the file
        assert_eq!(
            mtime1, mtime2,
    ...
    fn wrapper_script_reads_canonical_env_var_name() {
        assert!(WRAPPER_SCRIPT.contains("MEMORA_LOCAL_API_KEY"));
        // The deprecated alias must NOT be referenced here â€” every spawn
        ...
        assert!(
            !WRAPPER_SCRIPT.contains("MEMORA_API_AUTH_KEY"),
    ...
    fn wrapper_script_unsets_cloud_token_env_var() {
        // The agent must not see MEMORA_API_KEY (cloud JWT) â€” it's
        // distinct from MEMORA_LOCAL_API_KEY (local sp-<uuid>) and
        // exposing both has caused real users' agents to send the JWT
        // to localhost:3030 and chase phantom auth bugs (justinspillers,
        // 2026-05-05 â€” feedback log shows the agent burning 30+ tool
        // calls before concluding "API needs full JWT" â€” wrong).
        assert!(
            WRAPPER_SCRIPT.contains("unset MEMORA_API_KEY"),
            "wrapper must unset MEMORA_API_KEY in pi-agent subshells"
        );
    }
    ...
    fn wrapper_script_references_filter_pii_env() {
        assert!(
            WRAPPER_SCRIPT.contains("MEMORA_FILTER_PII"),
    ...
    fn wrapper_script_tags_session_owner() {
        assert!(
            WRAPPER_SCRIPT.contains("MEMORA_SESSION_ID"),
            "wrapper must read the session id env var"
        );
        assert!(
            WRAPPER_SCRIPT.contains("x-MEMORA-session"),
            "wrapper must send the owner header so navigations route to the right chat"
        );
    }

    /// End-to-end: the shim adds `x-MEMORA-session: <id>` to local API
    /// calls when `MEMORA_SESSION_ID` is set, and never leaks it to
    ...
    fn shim_tags_session_header_for_local_only() {
        ...
        // Local API call â†’ owner header present.
        ...
        let status = Command::new("bash")
            ...
            .env("MEMORA_LOCAL_API_KEY", "sp-test")
            .env("MEMORA_SESSION_ID", "conv-abc-123")
            .arg("-c")
        ...
        assert!(
            local.contains("x-MEMORA-session: conv-abc-123"),
        ...
        let status = Command::new("bash")
            ...
            .env("MEMORA_LOCAL_API_KEY", "sp-test")
            .env("MEMORA_SESSION_ID", "pipe:daily:7")
            .arg("-c")
        ...
        assert!(
            notify.contains("x-MEMORA-session: pipe:daily:7"),
            "local notification call must carry the session owner header; got: {notify}"
        );

        // Third-party host â†’ owner header must NOT leak.
    ...
    /// running server â€” we capture the final argv written to a tmpfile.
    #[test]
    #[cfg(unix)]
