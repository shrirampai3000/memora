...
//!
//! An agent is a CLI coding tool (pi, claude-code, opencode, â€¦) that can
//! execute a natural-language prompt against a working directory.  The
//! [`AgentExecutor`] trait lets the pipe runtime stay agent-agnostic â€” swap
...
/// The scheduler reads this to kill the process on timeout â€” no async
/// channel race.
...
///
/// `agent` â‰  `model`.  The *agent* is the CLI tool that executes the prompt
/// (pi, claude-code, opencode â€¦).  The *model* is the LLM the agent talks to
/// (haiku, opus, llama4 â€¦).  Both are configurable per-pipe.
...
pub trait AgentExecutor: Send + Sync {
    ...
    /// If `None`, uses MEMORA cloud as default.
    ...
    /// `continue_session` â€” when `true`, the agent resumes its last session for
    ...
    async fn run_streaming(
        ...
        // agent subprocess as `MEMORA_SESSION_ID` so its local API calls are
    ...
    /// Optional cloud auth token for MEMORA provider proxy.
