...
//! `MEMORA-audio::core::sck_output_watchdog` (which now re-exports these).
//!
...
//!
//! - [`usable_display_ids`] â€” "which displays should the audio SCK output
//!   watchdog still consider present?" A sleeping EXTERNAL display stays in
//!   this set on purpose (see [`filter_usable`]).
//! - [`capture_ready_display_ids`] â€” "could screen capture actually produce a
...
//!   distinguishes "every display is asleep/clamshell" (empty set â€” benign)
...
/// idle timer is still enumerated and stays usable â€” otherwise a routine
...
/// capture right now?" â€” use [`filter_capture_ready`] for the latter.
...
/// a multi-display Mac look like "displays are awake yet SCK sees none" â€” a
...
/// filter in `memora-screen`) reads the same attributes the same way.
#[cfg(target_os = "macos")]
...
/// callers would attempt to capture a sleeping display every frame â€” wasted
...
///
/// `None` means "unknown" â€” a `CGGetActiveDisplayList` error â€” and callers MUST
...
mod tests {
    ...
    fn asleep_external_is_usable_but_not_capture_ready() {
        ...
        // All displays asleep â†’ nothing is capture-ready, so an empty SCK
