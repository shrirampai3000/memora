...
description: Produce a human-facing visual report â€” a chart, dashboard, scorecard, or styled summary â€” that renders as a live page in the MEMORA viewer instead of plain text. Use when the task asks for a visual/graphical output rather than a plain note; do NOT use for plain text or raw data (prefer a markdown note for those).
---

# render an html report

MEMORA's in-app viewer can render a standalone `.html` file as a live page (charts, dashboards, styled reports) â€” but only when you **opt in** and keep the file **self-contained**. Otherwise it shows as raw source.
...
1. **Opt in.** The first line of the file must be the marker:
   ...
   (or `<meta name="MEMORA:render" content="human">` inside `<head>`). Without it the file shows as source, not a rendered page.

2. **Self-contained â€” no network.** It renders in a locked-down sandbox with **zero network access**. So:
   ...
   Anything that touches the network is silently blocked â€” **a CDN-loaded chart library renders blank**. Draw charts with **inline SVG** or plain HTML/CSS bars instead (see the template).

## how the user sees it

Write the file (e.g. `./output/report.html`), then link it with an **absolute** path from your note or notification â€” e.g. `[open report](/Users/me/.../output/report.html)`. The user clicks it; the viewer offers **"preview rendered"**; it renders inside the sandbox. Only standalone `.html` / `.htm` files render â€” HTML pasted inside a markdown note does not.
...
<body>
  ...
  <!-- inline SVG chart â€” no chart library (CDNs are blocked in the sandbox) -->
...
- [ ] first line is `<!-- MEMORA:render=human -->`
