---
name: MEMORA-team
description: Use the native MEMORA CLI to query Enterprise team activity or safely preview, deploy, and schedule managed team Pipes. Injected only by the Enterprise app for active admins.
---

# MEMORA Enterprise admin

Use `MEMORA team`; it reads the admin token and API base from MEMORA's
...
MEMORA team devices --raw

# Search first; default to 24h and at most 20 results.
MEMORA team search "TOPIC" --since 24h -n 20 --raw > /tmp/sp-team.jsonl

# Fetch a short chronology only after the device and window are known.
MEMORA team records --device-id DEVICE --since 4h --kind all -n 50 --raw > /tmp/sp-team.jsonl
```
...
For the user's own machine, use `MEMORA-api`, not this skill.

## Managed team Pipes

```bash
MEMORA team pipes list
MEMORA team pipes preview ./my-pipe/pipe.md
MEMORA team pipes deploy ./my-pipe/pipe.md
MEMORA team pipes schedule my-pipe "every day at 9am"
```
...
`write:pipes`. Direct the user to <https://github.com/shrirampai3000/memora/enterprise?tab=tokens> to
mint or rotate a token, then Settings â†’ Privacy â†’ Admin Team API Token.
