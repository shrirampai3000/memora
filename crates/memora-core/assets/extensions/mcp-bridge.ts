...
const API_BASE = `http://localhost:${process.env.MEMORA_PORT || 3030}/mcp-servers`;
const AUTH_KEY =
  process.env.MEMORA_LOCAL_API_KEY ||
  process.env.MEMORA_API_AUTH_KEY || // deprecated alias, drop next release
  "";
const SESSION_ID = process.env.MEMORA_SESSION_ID || "";
const MCP_ALLOWLIST_RAW = process.env.MEMORA_MCP_SERVER_ALLOWLIST;
const MCP_ALLOWLIST =
...
function authHeaders(): Record<string, string> {
  return {
    ...
    ...(SESSION_ID ? { "x-MEMORA-session": SESSION_ID } : {}),
  };
}
        ...
        if (result?.isError) {
          return {
            content: [
              {
                type: "text" as const,
                text: `âš  MCP tool "${params.tool}" on server "${params.server_id}" reported an error (isError=true).`,
              },
              ...baseContent,
            ],
