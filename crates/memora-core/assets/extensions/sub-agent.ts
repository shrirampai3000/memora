...
// - No nesting (MEMORA_SUBAGENT env blocks recursive spawning)
// - Sub-agents inherit pipe token for MEMORA API auth
// - All children killed on parent exit

import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";

// Prevent nesting â€” sub-agents must not spawn further sub-agents
if (process.env.MEMORA_SUBAGENT === "1") {
...
// â”€â”€ Read pipe permissions for sub-agent inheritance â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
try {
  ...
  const permPath = path.join(process.cwd(), ".MEMORA-permissions.json");
...
// â”€â”€ Sub-agent spawner â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

async function spawnSubAgent(prompt: string): Promise<string> {
  const { spawn } = require("child_process");
  const parentPid = process.pid;

  // Inject auth token so sub-agent can call MEMORA API
  let authHint = "";
  if (pipeToken) {
    authHint = `\n\nWhen calling the MEMORA API (localhost:3030), include: -H "Authorization: Bearer ${pipeToken}"`;
  }
  ...
  return new Promise((resolve, reject) => {
    const child = spawn("bun", [
      ...
      "--mode", "print",
      "--no-session",
      "--provider", process.env.PI_PROVIDER || "MEMORA",
      ...
      env: {
        ...process.env,
        MEMORA_SUBAGENT: "1",
        MEMORA_PARENT_PID: String(parentPid),
      },
    });
...
**Rules:**
- Each sub-agent is isolated â€” it only sees the prompt you give it, not your conversation
- Sub-agents can use bash (curl MEMORA API at localhost:3030)
- You have ${MAX_CONCURRENT} concurrent slots and ${MAX_TOTAL} total per run
- Each sub-agent has a ${TIMEOUT_MS / 1000}s timeout â€” keep tasks focused
- Results are returned as text â€” parse them in your main context
- Sub-agents CANNOT spawn other sub-agents

**Use for:** parallel MEMORA queries, independent research, breaking analysis into pieces
