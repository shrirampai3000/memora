...
/**
 * Context management extension for MEMORA pipes and chat.
 *
 ...
 *
 * 1. `tool_result` â€” When a tool returns a result that's too large for the
 ...
 *
 * 2. `context` (old tool results) â€” Before each LLM call, prune raw
 ...
 *
 * 3. `context` (oversized single message) â€” Issue #3852. pi's built-in
 ...
 *    window â€” that request hard-fails with
...
// 30K chars â‰ˆ 7-8K tokens â€” leaves room for the model to work.
...
// content_type, q query) â€” guidance that only makes sense for the MEMORA
...
// an agent literally cannot read its own skill once it grows past 30K chars â€”
// e.g. the morning-brief pipe failing to read MEMORA-api/SKILL.md (~33K).
...
// â”€â”€ single-message clamp (issue #3852) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export function resolveContextWindowTokens(ctx: any): number {
  ...
  try {
    ...
    // getContextUsage can throw right after a compaction â€” fall through.
  }
 ...
 * from the closing tag onward â€” that's the user's actual current message, which
...
export function boundOversizedMessages(messages: any[], contextWindowTokens: number): boolean {
  ...
  for (const msg of messages) {
    ...
    // User messages can carry content as a bare string â€” this is where the
...
export default function (pi: ExtensionAPI) {
  // â”€â”€ 1. Feedback on oversized tool results â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  pi.on("tool_result", async (event) => {
    ...
    // Don't lecture the model to "narrow its filters" for a file read â€” it's
    ...
    // morning-brief pipe couldn't read MEMORA-api/SKILL.md once it hit 33K.)
      ...
      content: [
        {
          type: "text" as const,
          text: [
            `âš ï¸ TOOL RESULT TOO LARGE â€” ${totalChars} chars (~${Math.round(totalChars / 4)} tokens).`,
  ...
  // â”€â”€ 2 + 3. Prune old tool results AND clamp oversized messages â”€â”€â”€â”€â”€
      ...
      for (let i = 0; i < cutoff; i++) {
        ...
        for (let j = 0; j < msg.content.length; j++) {
          ...
          if (
            ...
            msg.content[j] = {
              ...item,
              text: `[previous tool result â€” ${item.text.length} chars, processed in earlier turn]`,
            };
            modified = true;
          }
        }
