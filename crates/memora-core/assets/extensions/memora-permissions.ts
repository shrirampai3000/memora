...
// Evaluation order: deny â†’ allow â†’ default allowlist â†’ reject
...
try {
  ...
  const filePath = path.join(process.cwd(), ".MEMORA-permissions.json");
  ...
  // No permissions or invalid JSON â€” extension is a no-op
}
...
function hitsMEMORAApi(cmd: string): boolean {
...
function checkCurlCommand(cmd: string): string | null {
  ...
  if (!url) {
    return "could not safely parse the MEMORA API URL";
  }
  let pathname: string;
  ...
  try {
    ...
    if (parsed.hostname !== "localhost" && parsed.hostname !== "127.0.0.1") {
      return "MEMORA API URL must use an exact loopback hostname";
    }
  ...
  if (!isEndpointAllowed(method, pathname)) {
    return `${method} ${pathname} is not permitted â€” configure \`permissions\` in pipe.md`;
  }
...
/// Check a direct file-tool call (`write`, `edit`, â€¦) against the sandbox.
...
function buildPermissionRules(): string {
  ...
  rules.push(
    "You MUST obey these rules. The server enforces them â€” violating requests return 403.\n"
  );
  ...
  if (PERMS.pipe_dir) {
    ...
    rules.push(
      "Write targets must be literal paths. A target built from a variable or " +
        "command substitution (e.g. `$OUT`, `$(â€¦)`) is blocked because it " +
        "cannot be checked before it runs."
    );
    rules.push(
      "Running a script through an interpreter (python, node, ruby, â€¦) is " +
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event: any) => {
    ...
    if (hitsMEMORAApi(cmd) && !isParsableCurl(cmd)) {
      return {
        block: true,
        reason: "Use curl to access the MEMORA API. Other HTTP clients are not supported with permissions enabled.",
      };
    }
    if (isParsableCurl(cmd) && hitsMEMORAApi(cmd)) {
