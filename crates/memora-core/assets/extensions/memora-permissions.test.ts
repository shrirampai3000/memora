...
import extension, { __testing } from "./MEMORA-permissions";
...
describe("MEMORA permissions curl guard", () => {
  ...
  it("fails closed when a loopback command cannot be parsed safely", () => {
    ...
    ).toBe("could not safely parse the MEMORA API URL");
  ...
  it("rejects URLs that disguise a remote host as loopback user info", () => {
    ...
    ).toBe("MEMORA API URL must use an exact loopback hostname");
  });
});

const PIPE_DIR = "/home/u/.MEMORA/pipes/daily-report";
...
describe("MEMORA permissions filesystem sandbox", () => {
...
describe("MEMORA permissions file-tool sandbox", () => {
  beforeEach(() => __testing.setPermissions(sandboxPermissions));
  afterEach(() => __testing.setPermissions(null));
...
describe("MEMORA permissions tool_call hook", () => {
