...
//! made. macOS has had this since the stage panel landed and win32 never did â€”
//! `OverlayState::dragging` and `drag_target` were already being maintained here
//! and simply nobody painted them.
//!
//! Kept free of any win32 type, like `layout` and `state`, so the decisions â€”
//! which circles, where, which one is lit â€” are unit-tested on every platform
