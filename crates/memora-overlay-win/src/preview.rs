...
//! Preview harness for the native overlay â€” the win32 counterpart of
...
/// exists while MEMORA is recording, so a silent meter is the exception.
...
fn meeting_lines() -> Vec<TranscriptItem> {
    vec![
        ...
        TranscriptItem {
            speaker: String::new(),
            text: "yes â€” layered windows hit-test per pixel".into(),
            device_type: "input".into(),
        },
    ]
}
        ...
        OverlayState {
            notification: notification::parse(
                r#"{
                    ...
                    "body": "google meet â€” recurring, 14 participants, started 3 minutes ago",
...
fn shots(dir: &str) -> windows::core::Result<()> {
    ...
    // The drag stage is not an `OverlayState` the pill can be rendered from â€”
    // it is a desktop-sized sheet â€” so it gets its own pass. One shot per edge,
...
/// running overlay, not renders â€” the honest proof that the window works.
fn live_shots(dir: &str) -> windows::core::Result<()> {
    ...
    for (name, mut state) in flows() {
        ...
        // instead â€” the pointer is not on the pill during an unattended run.
        if state.hovering {
...
/// deliberately does not carry a notification â€” which is why the deepest stack
...
const MEETING_TOAST: &str = r#"{
    ...
    "body": "google meet â€” product sync",
    "actions": [
        {"label": "open note + HD", "action": "record-hd"},
        {"label": "open note", "action": "deeplink",
         "url": "MEMORA://meeting/42", "primary": true}
    ]
}"#;

/// Drive the real window through the flows so the interaction â€” not just the
/// pixels â€” gets exercised, then leave it up and interactive for a human.
fn live(anchor: Anchor, cycle: bool, toast: bool) {
    ...
    let resting = if toast {
        ...
        // exercised: chip â†’ disclosure â†’ live card â†’ toast, with the card
    ...
    println!("resting pill is up at {anchor:?} â€” hover it, click it, drag it. ctrl-c to quit");
    ...
    if toast {
        ...
        match overlay.show_notification(MEETING_TOAST) {
            Ok(()) => println!("toast raised â€” aim at 'open note' and click"),
