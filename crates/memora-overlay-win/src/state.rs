...
/// `middle-left` / `top-right` / â€¦ . The store only ever accepted the four
...
/// a frame is always internally consistent â€” no half-applied update can paint.
...
pub struct OverlayState {
    ...
    /// Pointer is inside the pill's hit rect â€” the only thing that expands the
    ...
    /// `showMEMORAShortcut`, `searchShortcut`, `showChatShortcut`, and the
...
/// point to one, and the renderer highlights the hovered/pressed one â€” so what
