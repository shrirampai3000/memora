...
//! panel in another corner â€” the win32 half of PR #6179.
//!
...
//! feature â€” a payload this one row cannot represent honestly must be rejected,
...
pub enum Refusal {
    ...
    MissingId,
    MissingTitle,
    /// An action had no usable label â€” dropping it silently would lose a button.
    UnusableAction,
    TooManyActions,
...
mod tests {
    ...
    const OPEN_NOTE: &str = r#"{
        ...
        "actions": [
            {"label": "open note", "action": "deeplink", "url": "MEMORA://note/42", "primary": true},
    ...
    fn parses_the_meeting_started_alert() {
        ...
        assert_eq!(n.body, "zoom â€” product sync");
    ...
    fn action_payload_survives_verbatim_so_dispatch_is_shared() {
        ...
        // â€” url, meeting id, anything added later â€” has to reach
        ...
        assert_eq!(payload["url"], "MEMORA://note/42");
    ...
    fn auto_dismiss_accepts_both_shapes_and_ignores_zero() {
        ...
        // caller meant â€” treat it as "wait for the user".
        assert_eq!(
