...
//! effect â€” effects need an `ID2D1DeviceContext`, and the DC render target that
...
const APP_ICON_PNG: &[u8] =
    include_bytes!("../../../apps/memora-app-tauri/src-tauri/icons/32x32.png");
...
impl Renderer {
    ...
    fn stroke_round(
        ...
        let brush = self.brush(rt, c);
        // Inset by half the stroke so the border lands inside the surface â€” an
    ...
    /// *uniform* low alpha.
    ///
    /// The obvious version â€” few rings, alpha ramped up toward the surface â€”
    ...
    fn draw_fixing(&self, rt: &ID2D1RenderTarget, state: &OverlayState, r: Rect, s: f32) {
        ...
        let label = if state.health_detail.is_empty() {
            ...
            format!("fixing â€” {}...", state.health_detail)
        };
        self.mono_text(
            rt,
            &label,
    ...
    fn draw_transcript(&self, rt: &ID2D1RenderTarget, state: &OverlayState, r: Rect, s: f32) {
        ...
        // Newest line last, oldest clipped off the top â€” same reading order as
        // the webview card.
        let line_h = 26.0 * s;
...
/// First installed family from `candidates`, else the first candidate â€” a
