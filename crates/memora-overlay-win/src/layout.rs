...
/// shadow themselves â€” there is no `DWMWA_*` shadow on a `WS_EX_LAYERED`
/// borderless window â€” so the blur has to fit inside the window rect.
...
impl Layout {
    ...
    /// past the threshold. The whole pill stack drags â€” chip, dock, health
    /// banner, disclosure â€” even when the press lands on a button: travel
    ...
    /// True when the point is over any painted surface â€” what decides whether
    ...
    /// pointer back on the chip â€” the pill flickered and its hit zone felt a
    ...
    /// not â€” the corridor the pointer travels along to reach an attachment.
    ///
    ...
    /// everything between the pill and the toast â€” the disclosure row, the
    /// transcript card â€” which slides the toast out from under the pointer, so
...
/// Three icon buttons, a divider, the audio meter, a divider, settings â€” the
...
pub fn notification_action_rects(state: &OverlayState, r: Rect, s: f32) -> Vec<(Control, Rect)> {
    ...
    // right edge, where the pointer already is after reading the title â€” so the
...
mod tests {
    ...
    fn notification_buttons_do_not_overlap_the_dismiss_x() {
        let s = OverlayState {
            notification: parse(
                r#"{"id":"m","title":"meeting detected","body":"zoom â€” product sync","actions":[
    ...
    fn expanding_under_the_pointer_slides_the_toast_off_its_own_button() {
        ...
        // button jumps ~190pt and a dock cell â€” the timeline â€” lands where the
    ...
    fn the_toast_is_the_only_surface_that_freezes_the_dock() {
        // The pill and the transcript still drive expansion â€” only the toast
    ...
    fn the_notification_row_hugs_its_content() {
        ...
        // keep the gutter small â€” this row used to be 44pt tall.
    ...
    fn every_face_of_the_pill_stack_can_start_a_drag() {
        ...
        // Hovered, the dock tiles the primary with buttons â€” a press on a
