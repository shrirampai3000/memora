...
/// modifier â€” and a hint that clips teaches the wrong shortcut.
...
fn labelled(label: &str, shortcut: &str) -> String {
    ...
    if pretty.is_empty() {
        ...
        format!("{label} Â· {pretty}")
    }
}

/// Clip to `max` characters with a trailing ellipsis.
///
/// DirectWrite can trim, but only at the width it measures â€” and the strings
...
mod tests {
    ...
    fn every_dock_control_names_its_shortcut() {
        // A dock cell whose hint reads only "timeline" teaches nothing â€” the
        // point of the disclosure row is that the pill is a shortcut tutor.
        let mut s = state();
        for (control, expected) in [
            (Control::Timeline, "timeline Â· alt+S"),
            (Control::Search, "search Â· alt+K"),
            (Control::Chat, "chat Â· alt+L"),
        ] {
            s.hovered_control = Some(control);
            assert_eq!(disclosure_hint(&s), expected);
        }
        s.hovered_control = None;
        assert_eq!(disclosure_hint(&s), "hide Â· alt+O");
    }
    ...
    fn audio_cell_reports_whether_anything_is_being_heard() {
        ...
        s.audio_active = true;
        s.speech_ratio = 0.62;
        assert_eq!(disclosure_hint(&s), "listening Â· 62%");
        // A ratio outside 0..1 must not render "listening Â· 320%".
        s.speech_ratio = 3.2;
        assert_eq!(disclosure_hint(&s), "listening Â· 100%");
    }
    ...
    fn ellipsize_does_not_split_a_multibyte_character() {
        let s = "rÃ©union produit â€” synchronisation hebdomadaire";
