...
//! does per-pixel hit-testing for free â€” fully transparent pixels pass clicks
//! through to whatever is underneath â€” and it renders identically on remote
...
/// otherwise need â€” cheaper to name the constant than to pull it in.
...
/// the overlay â€” see `Layout::is_in_corridor`.
...
impl Overlay {
    ...
    /// standalone notification panel â€” which is the whole point: a payload this
...
struct Ctx {
    ...
    /// cursor every time â€” the obvious implementation â€” teleports the pill to
    ...
    /// becomes a drag even when it began on a dock button â€” the macOS
    ...
    /// because it must not expand the pill â€” it only suspends the toast's
...
fn run_message_loop(
    ...
    unsafe {
        ...
        let class = windows::core::w!("MEMORA_overlay_pill");
        let wc = WNDCLASSEXW {
...
/// subtracted back out â€” otherwise every anchor would sit ten pixels away from
...
fn repaint(hwnd: HWND) {
    unsafe {
        ...
        // Rebuild the backing DIB whenever the content box changes size â€” which
...
/// the pill's `WM_WINDOWPOSCHANGING` rule â€” always reassert topmost â€” does not
/// apply here: the stage has to stay *under* the thing being dragged over it.
fn stage_class() -> windows::core::PCWSTR {
    static ONCE: std::sync::Once = std::sync::Once::new();
    let class = windows::core::w!("MEMORA_overlay_drag_stage");
    ...
    unsafe {
        if ctx.stage.is_none() {
            ...
            let hwnd = CreateWindowExW(
                ...
                stage_class(),
                windows::core::w!("MEMORA overlay drag stage"),
                WS_POPUP,
                0,
                0,
                10,
...
/// must cost nothing â€” this overlay is on screen all day.
...
/// re-pushes its whole state constantly â€” the audio meter alone arrives many
/// times a second â€” and its copy still holds the pre-drag anchor until it has
...
/// a value the user just changed here â€” where the pill sits, whether the card
/// is pinned â€” arrives back stale a few milliseconds later and would undo
    ...
    while let Ok(cmd) = ctx.rx.try_recv() {
        match cmd {
            Cmd::Update(s) => {
                ...
                // The caller does not own the notification either â€” it arrives
...
/// Report an action, taking care not to hold a `&mut Ctx` across the callback â€”
    ...
    unsafe {
        match msg {
            ...
            WM_MOUSEMOVE => {
                ...
                // driving it â€” see `Layout::is_notification_at`. Toggling the
                ...
                // â€¦and it holds the toast open too: a notification that expires
            ...
            WM_LBUTTONUP => {
                ...
                if let Some(ctx) = ctx_of(hwnd) {
                    ...
                    if ctx.dragging {
                        ...
                        // the press did â€” dragging off a button cancels it.
...
mod tests {
    ...
    /// The pin lives only in the window â€” clicking it reports nothing to the
    /// app â€” so the app's copy is always the value from before the click, and
