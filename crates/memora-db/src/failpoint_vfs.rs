...
//! read path of a live sqlx connection â€” reproducing the production write-queue wedge
...
//!   header while ARMED â€” a genuine read failure surfaced through the real sqlite read
...
//!   on most paths, so 522 does not reliably wedge writes â€” see `fp_read`.)
//! * `xClose` tracks live handles so the fault can be configured to **heal only when
//!   every connection has closed** (`set_auto_heal(true)`) â€” faithfully modelling the
...
unsafe extern "C" fn fp_read(
    ...
    // reads past the file header (offset > 0) â€” i.e. the data/index pages and WAL
    ...
    //     WAL-index desync made reads genuinely unable to complete â€” i.e. a hard
    ...
    //     SQLITE_NOTADB (26) and refuse to open the file â€” not the wedge.
...
/// ("disk I/O error") â€” the fault that wedges the write queue. See `fp_read`.
pub fn arm() {
...
/// closes â€” models "only a full reconnect/restart cures the wedge".
...
mod tests {
    ...
    /// disk I/O failure â€” i.e. exactly what triggers the write-queue's escalation.
    ...
    /// hit `xRead` on (almost) every page â€” mirroring how production's warm pooled
    ...
    async fn failpoint_injects_disk_io_error_and_heals_only_on_full_close() {
        ...
        // Warm pool stays open the whole time â€” it is the heal anchor (its final
        // close is what drives handlesâ†’0). min_connections(1) keeps a live handle.
        ...
        // Arm â†’ reads of uncached pages return a real SQLITE_IOERR_SHORT_READ (522).
        set_auto_heal(false);
        arm();
        ...
        // off disk to modify them â€” a read SQLite cannot zero-fill, so the 522
        ...
        // Only a full close (every handle gone) heals it â€” the restart semantics.
        set_auto_heal(true);
        warm.close().await;
    ...
    async fn write_queue_quarantines_ioerr_across_manager_generations() {
        ...
        let write_pool = SqlitePoolOptions::new()
            ...
            .await
            .unwrap();
        let sem = memora_sqlite_coordinator::sqlite_write_lock(&db);
        ...
        assert!(
            memora_sqlite_coordinator::sqlite_write_lock(&db).is_closed(),
        ...
        let replacement_error = match crate::DatabaseManager::new(
            ...
            memora_config::DbConfig::for_tier(memora_config::DeviceTier::Low),
        )
        .await
        {
            Ok(database) => {
    ...
    async fn write_queue_quarantines_sqlite_full_without_tail_flush() {
        ...
        let queue = spawn_write_drain_with(
            pool.clone(),
            memora_sqlite_coordinator::sqlite_write_lock(&db),
        ...
        assert_eq!(
            memora_sqlite_coordinator::registered_sqlite_hard_fault(&db),
            Some(13)
        );
        assert!(memora_sqlite_coordinator::sqlite_quarantine_exists(&db));
    ...
    async fn direct_transaction_statement_ioerr_quarantines_before_rollback() {
        ...
        let writer_gate = memora_sqlite_coordinator::sqlite_write_lock(&db);
