...
pub struct SpeakerReassignment {
    ...
    /// that path, so the only way back is to write the old name again â€” which
...
/// `audio_transcriptions` row â€” which conflated silent/duplicate/transcribed
...
pub enum ChunkOutcome {
    ...
    /// `idx_audio_transcription_chunk_text` index â€” the writer uses
    ...
    Transcribed {
        ...
        /// Capture timestamp of the chunk â€” used for the row's `timestamp`
    ...
    /// chunk as processed â€” semantically "we don't need to retry" â€” to
...
/// the new health diagnostic â€” direct measurement of a pipeline stall
...
pub struct OutputRecord {
    ...
    /// SAF stable artifact id â€” survives across re-emits/versions.
...
/// Used only by [`DatabaseManager::list_memories_for_sync`] â€” the
...
pub struct FrameRow {
    ...
    pub ocr_text: String,
    pub text_json: String,
    /// Accessibility tree JSON â€” used as fallback for bounding boxes when OCR text_json is empty
...
pub struct UiEventRow {
    ...
    /// (`[{"role":"AXWindow","name":"Inbox"},...]`) â€” disambiguates identical
...
pub struct InsertUiEvent {
    ...
    pub element_automation_id: Option<String>,
    pub element_bounds: Option<String>,
    /// Root-first JSON path of the element in the window hierarchy â€” see
...
pub struct Element {
    ...
    /// (legacy rows) â€” search filters treat that as "unknown" rather than
    ...
    /// `is_focused`, `is_selected`, `is_expanded`, `value`, `placeholder`, â€¦)
