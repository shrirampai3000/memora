...
// â”€â”€ Sleep/wake pause mechanism â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Disk-I/O wedge recovery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
// The old loop retried the SAME pool 3Ã— then dropped the batch forever, silently
...
//     once â€” the seam the app uses to restart the engine, the only thing that
...
/// is ~6s+ of uninterrupted total write failure â€” long enough to rule out a
...
/// ~30s â€” 40 consecutive fatal batches is ~20 MINUTES of lost writes, not ~6s
...
// â”€â”€ Sustained write-lock contention â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
//
// But a *permanently* held permit â€” a leaked `OwnedSemaphorePermit` from a
// hung or aborted task â€” is indistinguishable from that, batch by batch, and
...
/// multi-GB capture database, and neither announces itself â€” they do not use
...
/// Pure escalation state for a fatal run â€” decides when the persistent-failure
...
// â”€â”€ What a failed write actually means â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
// flattened into `Error::Protocol(String)`. Both reached callers â€” and
// Sentry â€” as the single string "encountered unexpected or invalid data:
// pool timed out while waiting for an open connection", so:
//
//   * MEMORA-CLI-SQ conflated "another writer holds the lock" (expected
...
pub const WRITE_LOCK_HELD_MESSAGE: &str =
    "MEMORA write queue: another writer held the SQLite write lock past this batch's budget";

/// A batch owned the write coordinator â€” so every ordinary writer was already
/// excluded â€” and still could not check out a pooled connection. That is real
/// pool starvation, not contention.
pub const WRITE_POOL_STARVED_MESSAGE: &str =
    "MEMORA write queue: no write-pool connection became available inside this batch's budget";
...
pub(crate) enum BatchOutcome {
    /// The batch committed, or only hit per-row errors â€” the connection path is fine.
    Healthy,
...
impl WriteQueueHealth {
    ...
    pub fn is_hard_faulted(&self) -> bool {
        self.inner.hard_faulted.load(Ordering::SeqCst)
            || self.inner.hard_fault_path.as_ref().is_some_and(|path| {
                memora_sqlite_coordinator::registered_sqlite_hard_fault(path.as_ref()).is_some()
            })
    }
    ...
    pub(crate) fn latch_hard_fault_code(&self, code: i32) -> bool {
        ...
        if let Some(path) = self.inner.hard_fault_path.as_ref() {
            memora_sqlite_coordinator::latch_sqlite_hard_fault(path.as_ref(), code);
        }
        first_for_manager
    }
    fn note_reopen(&self) {
...
/// Hook invoked once when writes have failed persistently â€” the seam the app uses
...
/// existing tests â€” behaviour unchanged).
...
// â”€â”€ Write operation definitions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub(crate) enum WriteOp {
    ...
    InsertSnapshotFrameWithOcr {
        ...
        /// Distinct from `browser_url` â€” this is for editor file paths.
    ...
    /// Pipe execution writes â€” routed through write queue for proper serialization.
    PipeCreateExecution {
        pipe_name: String,
        trigger_type: String,
        model: String,
...
// â”€â”€ Drain loop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async fn drain_loop(
    ...
    loop {
        ...
        // in a single atomic call. No periodic wake-ups â€” the previous
        ...
        let n = tokio::select! {
            ...
            // DatabaseManager::close(): exit WITHOUT flushing â€” the pools are
        ...
        if n == 0 {
            // Channel closed â€” all senders dropped.
            break;
        }

        // â”€â”€ Sleep/wake pause gate â”€â”€
        ...
        match outcome {
            ...
            BatchOutcome::Contention => {
                ...
                // was no further escape â€” no reopen (correctly, it cannot help)
                ...
                if fire_hook {
                    ...
                    error!(
                        "write_queue: SQLite write lock held by another writer for {:?} across {} batch(es) â€” requesting engine restart to clear the stuck coordinator",
            ...
            BatchOutcome::FatalConnection => {
                ...
                // that rebuilds the shared WAL-index + read pool â€” the cure for a
                ...
                if fire_hook {
                    health.note_persistent_signal();
                    error!(
                        "write_queue: persistent write failure ({} fatal batches over {:?}) â€” requesting engine restart to rebuild all pools + WAL-index",
    ...
    while rx.recv_many(&mut tail_batch, MAX_BATCH_SIZE).await > 0 {
        debug!(
            "write_queue: shutdown â€” flushing {} remaining writes",
            tail_batch.len()
        );
        let _ = execute_batch(
            &write_pool,
...
async fn execute_batch(
    ...
    .await
    {
        Ok(Ok(permit)) => permit,
        Ok(Err(_)) => {
            // Pool closed â€” shutdown path, not a disk wedge.
    ...
    for attempt in 1..=MAX_BUSY_RETRIES {
        ...
        // it â€” the first-failure event still reaching Sentry as
        // MEMORA-CLI-RC / CLI-SR. It never discards committed data, only an
        // orphaned uncommitted transaction that was already doomed. The reactive
        // handler below stays as a belt-and-suspenders net.
        //
        // `Ok` means the ROLLBACK actually cleared an orphaned transaction â€” i.e.
        ...
        // masked â€” without re-flooding Sentry. The common `Err` is the harmless
    ...
    let mut conn = match conn_opt {
        ...
        None => {
            ...
            // MEMORA-CLI-RC) â€” escalate to FatalConnection so the drain
        ...
        match execute_single_write(&pending.op, &mut conn).await {
            ...
            Err(e) => {
                ...
                if is_connection_error(&e) {
                    ...
                    // Per-row error (constraint violation, etc.) â€” log but continue
    ...
    if any_fatal {
        ...
        // 17:25-17:39 â€” 11 audio chunks lost). A fresh connection
...
async fn execute_single_write(
    ...
    match op {
        ...
        WriteOp::InsertAudioTranscription {
            ...
            // "transcribed" â€” the row already exists, we've considered
            // this chunk.
            sqlx::query(
                "UPDATE audio_chunks \
        ...
        WriteOp::InsertAudioChunkAndTranscription {
            ...
            // because we *did* process this chunk â€” its content is captured
            ...
            // Empty STT result â€” same story as Duplicate but marked 'silent'
...
/// can still `matches!` on them â€” flattening *everything* into
...
fn send_error_to_all(batch: &mut Vec<PendingWrite>, error: sqlx::Error) {
    ...
    // checkpoint raise a Sentry issue (MEMORA-CLI-SQ). The drain loop still
...
/// impl â€” see `is_fatal_sqlite_message_*` tests below.
///
...
/// SQLITE_CANTOPEN â€” "unable to open database file". At runtime this means the
...
/// grandparent is also missing â€” the signature of an unmounted volume) it does
...
/// (mountpoint-safe, via [`ensure_db_parent_dir`]) AND an empty db file â€”
...
async fn ensure_db_openable(db_path: &str) -> bool {
    ...
    // Only recreate the file if the parent dir actually exists now â€”
...
mod tests {
    ...
    // â”€â”€ FatalRunEscalation (count / wall-clock / refire rules) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    /// `Contention` arm set `degraded` and stopped there â€” no pool reopen
    ...
    /// drain loop against a genuinely stuck coordinator â€” the production
    /// failure â€” and proves three things end to end:
    ...
    fn escalation_count_rule_fires_at_threshold() {
        ...
        assert!(!e.on_fatal(t0), "latched â€” no immediate refire");
    }
    ...
    fn escalation_healthy_streak_clears_and_rearms() {
        ...
        assert!(e.on_healthy(), "streak reaches clear_after â†’ run over");
    ...
    fn escalation_refires_while_run_persists() {
        ...
        // connection pinning the WAL-index) â€” the hook must refire so the app
        ...
        assert!(!e.on_fatal(just_before), "paced â€” not yet");
    ...
    /// Regression for MEMORA-CLI-HA: after the data dir/file vanishes
    /// mid-run, the runtime recovery must make a *fresh* connection openable
    /// again. Recreating the parent dir alone is NOT enough â€” the write pool
    ...
    /// The two batch timeouts have opposite meanings â€” "someone else is
    /// writing, retry" versus "this pool has no connections, recover" â€” and
    /// both used to be reported as a fabricated `PoolTimedOut`, flattened to
    /// the same string. MEMORA-CLI-SQ was therefore untriageable: one
    ...
    /// transport variants. MEMORA-CLI-SN's collateral-abort signal
    ...
    async fn test_batch_coalescing() {
        ...
        // Submit many writes concurrently â€” they should be batched
    ...
    fn persistent_stuck_transaction_escalates_to_fatal_on_exhausted_retries() {
        ...
        // plain recycle set â€” that's the gap this guards.
