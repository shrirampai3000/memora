...
//! SQLite storage for everything MEMORA captures: frames, OCR text,
//! accessibility trees, audio chunks, transcriptions, meetings, and their FTS
//! indexes.
//!
//! - `db` â€” the `DatabaseManager`, migrations, and query surface.
//! - `write_queue` â€” the single serialized writer. Every mutation goes here.
//! - `recovery` â€” page-level repair and fresh-generation swap after corruption.
//! - `cancellable_query` â€” search that can be aborted when a caller disconnects,
//!   so a slow query cannot pin a connection forever.
//! - `text_normalizer` / `text_similarity` â€” FTS5 query sanitizing and dedupe.
//!
...
//!   [`memora_sqlite_coordinator`]; never construct a second pool against a
...
pub use memora_sqlite_coordinator::{
