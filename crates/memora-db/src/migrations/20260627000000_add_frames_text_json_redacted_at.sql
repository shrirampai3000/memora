...
-- reconciled: `frames.text_json` â€” the per-word OCR bounding boxes, which
...
-- detection â€” applying the resulting map to ONLY each block's `text` field
-- while leaving the bounding-box geometry (left/top/width/height/â€¦) byte-for-
-- byte intact. No extra model pass; see MEMORA/website#291 and
-- `redaction_map.rs`. The image-PII (blur) worker is unaffected â€” it detects
...
-- redacted its originalâ†’redacted map can no longer be reconstructed â€” so
...
-- Same shape as idx_frames_full_text_redacted_at & friends â€” keeps the
