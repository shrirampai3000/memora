...
-- here so the schema matches the worker's actual behavior â€” and so we
...
--   *.text_redacted               â€” the source column IS the redacted text now
--   *.redaction_version           â€” re-redaction not supported (raw text gone)
...
--
-- Indexes on the dropped columns (none â€” all redaction indexes are on
