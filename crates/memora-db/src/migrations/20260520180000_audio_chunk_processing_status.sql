...
--   - Silent audio â†’ STT returns empty â†’ no row inserted â†’ still looks "untranscribed"
--   - Cross-device dedup â†’ live path drops the second device's row â†’ still "untranscribed"
...
-- Partial index â€” only indexes the rows the reconciliation worker actually
