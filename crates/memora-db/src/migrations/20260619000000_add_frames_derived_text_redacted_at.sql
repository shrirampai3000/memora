...
--
--   * `frames.accessibility_tree_json` â€” the full accessibility tree, the
--     raw text of every node; served verbatim by `/frames/:id/context`
--     (#4116).
--   * `frames.window_name` â€” the window title, indexed in `frames_fts`, so
...
-- extra model pass) â€” see MEMORA/website#291 and `redaction_map.rs`.
--
...
-- Same shape as idx_frames_full_text_redacted_at & friends â€” keeps each
