...
--
-- Duplicate open rows (meeting_end IS NULL) are an integrity bug â€” the
...
--      than "merge into one" to keep this migration cheap and lossless â€”
...
--      for an expression index like this one â€” code that maps the failure to
...
-- instead â€” every matching row indexes the same value `1`, so a second
