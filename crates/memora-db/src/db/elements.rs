...
"#,
    ...
    /// Returns a flat `Vec<Element>` â€” clients reconstruct the tree from `parent_id`/`depth`.
    #[allow(clippy::too_many_arguments)]
    pub async fn search_elements(
        ...
        // Some(false) keeps only off-screen elements (rare â€” useful for
        ...
        if on_screen.is_some() {
            // `e.on_screen = ?` is intentional â€” does NOT match NULL rows.
...
mod tests {
    ...
    async fn element_queries_run_against_real_schema_when_empty() {
        ...
        // check can't catch â€” assert both run on the real (migrated) schema.
        let db = mem_db().await;
        let (els, total) = db
