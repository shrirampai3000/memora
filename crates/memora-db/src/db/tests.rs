...
fn test_parse_all_text_positions_basic() {
    // Stored text_json uses screen coords (top-left origin, normalized 0â€“1); use as-is.
    let blocks = vec![
...
fn test_narrow_bbox_full_match_keeps_bbox() {
    // text == query â†’ narrowing produces the same bbox
...
fn test_narrow_bbox_single_line_query_at_start() {
    // "rotor mech" 100..200 (w=100), query "rotor" at offset 0 of 10 chars â†’ first half
...
fn test_narrow_bbox_multiline_paragraph_keeps_full() {
    // text is much longer than aspect ratio capacity â†’ multi-line, leave alone
...
// -----------------------------------------------------------------
// find_matching_a11y_positions â€” line-span aware search
// -----------------------------------------------------------------
...
fn a11y_match_uses_line_bbox_not_paragraph() {
    ...
    // width should narrow within the line â€” narrower than the full line width
    assert!(
...
fn a11y_match_falls_back_to_paragraph_when_no_lines_field() {
    ...
    // as a single bbox â€” original behavior, multi-line guard kicks in.
    let json = r#"[{
            "role": "AXStaticText",
    ...
    assert_eq!(positions.len(), 1);
    // No narrowing â€” paragraph bbox is preserved (multi-line guard in
...
fn a11y_match_falls_back_when_no_line_covers_match() {
    // Line capture aborted partway â€” only line 1 is present. A query that
    ...
    // "india" appears at char 53 â€” not covered by the only line span.
...
fn a11y_match_uses_line_for_line_3_when_multiline_capture_complete() {
    ...
    // The first occurrence (0..26 â†’ line 0) is what should match.
...
/// rarely go deeper). Mirrors the field set real captures serialize â€”
/// container nodes carry no text, leaves do â€” so the batch insert exercises
...
/// boundaries (chunk size is 70 â€” 200 nodes spans three flushes).
#[tokio::test]
async fn ax_bulk_insert_preserves_tree_shape_across_chunks() {
    for n in [1usize, 69, 70, 71, 200, 574] {
        ...
        // depth shallower that was already inserted (smaller id) â€” the
...
/// varies too much for a stable threshold) â€” run with
/// `cargo test -p memora-db --release -- --ignored --nocapture perf_ax_bulk_insert`
/// to see the numbers.
#[tokio::test]
