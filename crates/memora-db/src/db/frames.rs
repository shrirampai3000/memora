...
impl DatabaseManager {
    ...
    /// metadata is attached â€” the HD recorder writes these as sparse scrub
    ...
    pub async fn insert_snapshot_frame(
        ...
        self.insert_snapshot_frame_with_ocr(
            ...
            window_name,
            browser_url,
            None, // document_path â€” legacy callers don't carry it
            focused,
            capture_trigger,
            accessibility_text,
            text_source,
    ...
    // ========================================================================
    // Elements helpers â€” dual-write OCR + accessibility as structured rows
    // ========================================================================

    /// Insert OCR elements from `text_json` (serialized `Vec<OcrTextBlock>`) into the
    /// `elements` table. Builds a pageâ†’blockâ†’paragraphâ†’lineâ†’word hierarchy using
    /// `RETURNING id` to chain parent IDs within the same transaction.
    ///
    /// Level-0 blocks (Apple Native OCR â€” the default macOS path) have no hierarchy,
    ...
    pub(crate) async fn insert_ocr_elements(
        ...
        // 12 params per row Ã— 80 rows = 960 params, well below SQLite's
        ...
        // Track hierarchy: (page, block, par, line) â†’ element_id
    ...
    /// for `elements`) so a depthâ†’parent_id stack can resolve parent
    ...
    /// writes onto a single connection â€” nothing else can consume ids
    ...
    pub(crate) async fn insert_accessibility_elements(
        ...
        struct AxNode {
            ...
            /// out off-screen accessibility text â€” see issue #2436.
        ...
        // depth â†’ most-recent element_id at that depth
        ...
        for (i, node) in nodes.iter().enumerate() {
            ...
            // SQLite stores BOOLEAN as INTEGER. Map Noneâ†’NULL, Some(true)â†’1,
            // Some(false)â†’0 so the partial index from
        ...
        // 13 params per row Ã— 70 rows = 910 params, well below SQLite's
        // default SQLITE_LIMIT_VARIABLE_NUMBER (999 on older builds, 32766 on
        ...
        // executed) every parent id was assigned â€” and inserted â€” before
        // any of its children.
    ...
    /// AXDocument). Distinct from `browser_url` â€” the latter is for http(s),
    /// the former for file://.
    #[allow(clippy::too_many_arguments)]
    pub async fn insert_snapshot_frame_with_ocr(
        ...
        // data â€” the frame is usable for search immediately after the first commit.
    ...
    /// the lock once per capture cycle instead of 2Ã— per window result.
    ///
    ...
    pub async fn insert_frames_with_ocr_batch(
        ...
        // Nothing to insert â€” skip acquiring the write lock entirely.
        if windows.is_empty() {
            return Ok(vec![]);
        }
        ...
        let (video_chunk_id, file_path) = match video_chunk {
            ...
            None => {
                tracing::warn!(
                    "No video chunk found for device '{}' â€” frames will not be inserted",
                    device_name
                );
                return Ok(vec![]);
            }
    ...
    pub async fn insert_multi_frames_with_ocr_batch(
        ...
        // Single transaction for all frames â€” one semaphore acquisition.
        let mut tx = self.begin_immediate_with_retry().await?;
    ...
    pub async fn find_video_chunks_limited(
        ...
        // OCR text is truncated to 200 chars for the timeline stream â€” full text
        ...
        // background reconciler skips them â€” leaving no audio_transcriptions row for that
        ...
        // moment (Â±15s). Normally the two paths are complementary â€” live-covered
        // chunks get no audio_transcriptions row â€” so this only trims rare overlap
        ...
        let live_rows: Vec<_> = live_rows
            ...
            .filter(|r| match r.try_get::<DateTime<Utc>, _>("timestamp") {
                Ok(ts) => {
                    ...
                    // keep the live row only if NO background row falls within Â±window
                    background_ts_ms
                        .get(lo)
        ...
        for row in audio_rows.into_iter().chain(live_rows) {
            ...
            // static-screen-meeting case â€” on a video call the screen barely changes,
            ...
            if matching_keys.is_empty() {
                ...
                // timestamp merge into one entry. A later audio row whose Â±pad window
                // covers this timestamp will match the synthetic frame above and
                // cluster onto it â€” the same behaviour as real frames, so a long gap
