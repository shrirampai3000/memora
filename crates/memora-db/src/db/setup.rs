...
fn quarantine_startup_error(database_path: &Path, error: SqlxError) -> SqlxError {
    if let Some(code) = crate::sqlite_error::sqlite_hard_fault_code(&error) {
        memora_sqlite_coordinator::latch_sqlite_hard_fault(database_path, code);
    }
    error
}
...
impl DatabaseManager {
    ...
    pub fn coordinated_writer(&self) -> memora_sqlite_coordinator::SqliteWritePool {
        memora_sqlite_coordinator::SqliteWritePool::new(
    ...
    pub async fn new(database_path: &str, config: DbConfig) -> Result<Self, sqlx::Error> {
        memora_sqlite_coordinator::verify_sqlite_runtime().map_err(SqlxError::Protocol)?;
        debug!(
        ...
        // Ensure the data dir exists before opening the file â€” a missing parent
        ...
        memora_sqlite_coordinator::prepare_sqlite_quarantine_reserve(database_file).map_err(
            |error| {
                SqlxError::Protocol(format!(
        ...
        if let Some(code) =
            memora_sqlite_coordinator::registered_sqlite_hard_fault(database_path)
        {
            return Err(SqlxError::Protocol(format!(
                "SQLite database remains durably quarantined after a hard fault (code: {code}); run `MEMORA db recover` while the app is closed"
            )));
        }
        ...
        if database_file.is_file() {
            if let Err(error) = preflight_existing_database_header(database_file) {
                if let Some(code) = crate::sqlite_error::sqlite_hard_fault_code(&error) {
                    memora_sqlite_coordinator::latch_sqlite_hard_fault(database_file, code);
                }
                return Err(error);
            }
        }
            ...
            .map_err(|error| quarantine_startup_error(database_file, error))?
        {
            ...
            memora_sqlite_coordinator::prepare_sqlite_quarantine_reserve(database_file)
                .map_err(|error| {
                    SqlxError::Protocol(format!(
        ...
        // Every MEMORA-controlled capture writer and checkpoint resolves
        // this process-wide coordinator for the physical database path.
        let write_semaphore = memora_sqlite_coordinator::sqlite_write_lock(database_path);
        ...
        // for the write lock â€” the app-level retry in begin_immediate_with_retry
        ...
        // SQLx connections internally. MEMORA already requires exclusive
        ...
        for (pragma, value) in memora_config::WAL_SAFETY_PRAGMAS {
        ...
        // DB and skip conversion entirely â€” no race.
        {
        ...
        // here in `new()` â€” next to the integrity check â€” so EVERY caller gets
        // it: the desktop app runs the engine in-process and previously never
        // started it (only the standalone `memora-engine` CLI did), so app
    ...
    /// connections â€” its next query fails fast with PoolClosed instead of
    ...
    /// SQLITE_IOERR (code 522) until the whole process restarts â€” the exact
    ...
    async fn run_migrations(pool: &SqlitePool) -> Result<(), sqlx::Error> {
        ...
        // sqlite-vec, sqlx recorded it as applied but the CREATE TABLE failed â€”
    ...
    async fn ensure_event_driven_columns(pool: &SqlitePool) -> Result<(), sqlx::Error> {
        ...
        // If missing, warn â€” the migration should have created it.
        ...
        if fts_has_full_text.0 == 0 {
            tracing::warn!(
                "frames_fts is missing full_text column â€” consolidation migration may not have run"
            );
        }
    ...
    /// recorded the migration version but the DDL silently failed â€” the tables
    ...
    pub async fn begin_immediate_with_retry(&self) -> Result<ImmediateTx, sqlx::Error> {
        // Acquire the write semaphore â€” this is where serialization happens.
        ...
        for attempt in 1..=max_retries {
            ...
            match sqlx::query("BEGIN IMMEDIATE").execute(&mut *conn).await {
                ...
                Err(e) if Self::is_nested_transaction_error(&e) => {
                    // Connection has a stuck transaction â€” ROLLBACK it and retry.
                    ...
                    match sqlx::query("ROLLBACK").execute(&mut *conn).await {
                        Ok(_) => {
                            ...
                            // Connection is clean â€” drop returns it to pool
                            drop(conn);
                        }
                        Err(rb_err) => {
    ...
    /// degraded" and, on sustained failure, restart the engine â€” the cure for a
...
mod shutdown_tests {
    ...
    async fn startup_rebuilds_missing_speaker_tables_with_current_schema() {
        ...
        let database = DatabaseManager::new(
            ...
            DbConfig::for_tier(memora_config::DeviceTier::Low),
        )
        .await
    ...
    fn early_startup_hard_fault_closes_the_process_writer_gate() {
        ...
        let writer_gate = memora_sqlite_coordinator::sqlite_write_lock(&db_path);
        ...
        assert_eq!(
            memora_sqlite_coordinator::registered_sqlite_hard_fault(&db_path),
            Some(522)
        );
        assert!(
