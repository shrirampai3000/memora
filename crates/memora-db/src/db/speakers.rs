...
impl DatabaseManager {
    ...
    pub async fn add_embedding_to_speaker(
        ...
        if (count as usize) < max_stored {
            ...
            // At capacity â€” replace the most redundant embedding (closest to centroid)
    ...
    pub async fn update_speaker_centroid(
        ...
        let (new_centroid, new_count) = match row {
            ...
            _ => {
                // First embedding â€” centroid IS the embedding
    ...
    pub async fn get_audio_chunks_for_speaker(
        ...
        // â€” but ONLY when â‰¥1 row matches (an empty result never inspects
    ...
    pub async fn merge_speakers(
        ...
        // Move every reference â€” meeting segments and diarization rows too,
        // not just transcriptions and embeddings â€” then retire the row only
    ...
    ///
    /// `scope` decides how much moves â€” see [`ReassignScope`]. The default
    ...
    pub async fn reassign_speaker(
        ...
        // Phase 1: Short IMMEDIATE write transaction â€“ apply the rename.
        let (current_speaker_id, target_speaker_id) = {
            ...
            //    no speaker until backfill) â€” only a missing row is an error.
            ...
            let target_speaker_id = match (effective_scope, current_speaker_id, existing_target) {
                ...
                // row moves, nothing deleted â€” every segment already pointing
                ...
                // Already this person under a different spelling â€” normalise.
                ...
                (_, _, existing_target) => {
                    ...
                    // payload â€” skip them; undo leaves those on the new speaker.
            ...
            // from meeting_transcript_segments, not audio_transcriptions â€” sync
        ...
        // Phase 2: Majority-vote propagation â€” only absorb speakers where >50% of
        ...
        if propagate_similar {
            ...
            if !speakers_to_absorb.is_empty() {
                ...
                for old_speaker_id in &speakers_to_absorb {
                    ...
                    // Move every reference â€” transcriptions, meeting segments,
                    // embeddings, diarization rows, identity evidence â€” before
        ...
        // Phase 4: Clean up â€“ drop the original speaker only once every table
    ...
    /// transcriptions â€” otherwise undo left the Meeting view still showing
