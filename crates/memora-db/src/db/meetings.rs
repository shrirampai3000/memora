...
/// violation â€” i.e. another meeting already owns this calendar event.
...
impl DatabaseManager {
    // â”€â”€ Meeting persistence â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    //
    ...
    // `MEETING_END_REASON_*` constants below â€” these are the canonical values
    ...
    /// that collision the meeting is still created â€” it simply keeps no
    ...
    pub async fn bind_calendar_event(
        ...
        .execute(&mut **tx.conn())
        .await?;
        // Ownership, not "did this statement write a row" â€” the owner
    ...
    /// in [`Self::find_recent_meeting_for_app`] â€” explicit stops are excluded.
    ...
    /// sorted alphabetically â€” or None when nothing qualifies.
    ///
    /// Edge cases handled:
    /// * `document_path IS NULL` for browsers / OS chrome / terminals â†’
    ///   filtered out by the WHERE clause.
    /// * Same file appears in many frames (typical for the focused doc) â†’
    ///   `DISTINCT` dedupes.
    /// * Empty result â†’ `Ok(None)` so caller skips emitting the section.
    ...
    pub async fn end_meeting_with_typed_text(
        ...
        // signal is independently optional â€” a meeting where the user only
    ...
    /// no longer applies â€” the row is active again and shouldn't be
    ...
    /// already covered â€” without that, every live-transcribed meeting also
    ...
    /// - `speaker_id` is left NULL â€” live diarization stores a free-text
    ...
    pub async fn mirror_live_meeting_to_audio_transcriptions(
        ...
        // Read phase (read pool â€” no write lock held while we gather).
        ...
        // is ~80 chunks), then matched in memory â€” avoids a per-segment query. We
    ...
    /// `audio_transcriptions` â€” so the Meeting view shows the cross-meeting, nameable
    ...
    /// timestamp, so once the chunk backfill stamps it, it matches first. Idempotent â€”
    ...
    pub async fn backfill_meeting_segment_speakers(
        ...
        // One statement instead of fetch-candidates â†’ per-row nearest-lookup â†’
        // update-by-id (up to 501 round-trips per pass). The scalar lookup is
        // correlated to each of the capped candidates, but â€” critically â€” its
        ...
        // candidate Ã— the entire audio history under a BEGIN IMMEDIATE lock.
        //
    ...
    /// (same text, same device direction, Â±2s). Live segments carry no chunk
    ...
    async fn resolve_live_segment_chunk_links(
        ...
        // (is_input, text) â†’ [(ts_ms, chunk_id, file_path)]
        ...
        let mut rows = sqlx::query_as::<_, MeetingTranscriptSegment>(
            ...
            background_segments AS (
                ...
                WHERE julianday(at.timestamp) >= julianday(mw.meeting_start)
                  ...
                  -- same meeting (within Â±15s). Live + background both writing the
                  ...
                  -- happens to fall within 15s of the user talking â€” silently
