...
impl DatabaseManager {
    ...
    /// saturation (see the pending-chunk recovery path in MEMORA-audio).
    ...
    /// instead of one round-trip per id â€” turns a 2N-statement pass into a
    ...
    pub async fn insert_audio_transcription(
        ...
        // Empty STT result for an existing chunk â†’ mark Silent so the
        ...
        // chunk's status â€” otherwise this chunk loops in the reconciliation
    ...
    pub async fn insert_audio_chunk_and_transcription(
        ...
        // pipes, so a burst there used to fail this insert outright â€” before the
        ...
        // for the same file. Retry it instead â€” read-pool pressure is bursty, so
        ...
        // pool â€” search, timeline, the API, pipes all share it â€” threw away a
    ...
    pub async fn update_audio_transcription(
        ...
        match result {
            ...
            // benignly â€” caller in transcription_result.rs already treats this
    ...
    pub async fn replace_audio_transcription(
        ...
        let trimmed = transcription.trim();
        if trimmed.is_empty() {
            // Funnel through Silent â€” never let an empty input become a no-op
    ...
    pub async fn replace_audio_transcriptions(
        ...
        // Empty inputs are a legitimate "STT returned nothing" signal â€” translate
    ...
    /// - Empty / whitespace-only Transcribed segments â†’ falls through to Silent.
    /// - Duplicate text within Transcribed (diarization splits + same word) â†’
    ...
    /// - Chunk deleted between query and outcome â†’ the UPDATE is a no-op, the
    ...
    /// - Failed with attempts >= cap â†’ escalates to FailedPermanent.
    pub async fn record_chunk_outcome(
        ...
        match outcome {
            ...
            ChunkOutcome::Silent | ChunkOutcome::Duplicate => {
                ...
                // `transcribed` for Duplicate (we DID transcribe â€” on the
    ...
    /// engine. Existing rows are kept so the UI doesn't flash empty â€”
