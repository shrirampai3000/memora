...
impl DatabaseManager {
    ...
    pub async fn delete_time_range(
        ...
        // NOTE: filter out NULL video_chunk_id in the NOT IN subquery â€” SQL `x NOT IN
        ...
        // Same NULL-in-NOT-IN pitfall as above â€” filter NULL audio_chunk_id explicitly.
        ...
        // 5. Delete frames â€” triggers frames_fts delete; vision_tags CASCADE'd automatically
        ...
        // 7. Delete audio_transcriptions â€” triggers audio_transcriptions_fts delete
        ...
        // 8. Delete orphaned audio_chunks â€” audio_tags CASCADE'd automatically
        ...
        // 10. Delete ui_events â€” triggers ui_events_fts delete
        ...
        // (rows carry personal device names) â€” but KEEP the newest snapshot
        ...
        // 11. Commit â€” if this fails, no files are touched (auto-rollback)
    ...
    /// this variant returns every video/audio/snapshot file â€” used by local
    ...
    pub async fn delete_time_range_local(
        ...
        // explicitly â€” otherwise the user clicks "delete last 15 minutes"
        ...
        // 12. Commit â€” if this fails, no files are touched
        tx.commit().await.map_err(|e| {
            error!(
    ...
    /// frame/transcription it owns falls inside [start, end] â€” straddling
    ...
    pub async fn evict_media_in_range(
        ...
        // window â€” straddling chunks are skipped so old playback still works.
        // NOT IN (subquery) silently filters out everything if the subquery
        // contains NULL â€” frames.video_chunk_id is nullable. Same trap applies
        ...
        // silently zero out every match and the UPDATE would never fire â€”
    ...
    ///   - `frames.text_json` (the per-word OCR bounding-box blob) â€” dropped
    ...
    /// `audio_transcriptions`, and `memories`. So OCR *text* survives â€” only the
    ...
    pub async fn strip_heavy_text_in_range(
        ...
        // FTS-indexed nor watched by frames_au, so no trigger fires â€” full_text
    ...
    /// `tokio::fs::metadata`, so cost is O(N) syscalls â€” keep ranges
    ...
    pub async fn estimate_evictable_bytes(
        ...
        // out-of-range snapshot frame silently zeroes out this whole estimate â€”
    ...
    /// audio_chunks_deleted will always be 0 â€” orphans are cleaned separately.
    pub async fn delete_time_range_batch(
        ...
        // NULL-guard the anti-join subqueries â€” same trap as evict_media_in_range
        ...
        // NO orphan video_chunks cleanup here â€” done separately
        ...
        // NO orphan audio_chunks cleanup here â€” done separately
    ...
    pub async fn repair_database(&self) -> Result<(), anyhow::Error> {
        anyhow::bail!(
            "online SQLite repair is disabled; stop every MEMORA process and run `MEMORA db recover` so recovery operates on a preserved offline generation"
        )
    }
    ...
    pub fn start_wal_maintenance(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                tokio::select! {
                    _ = interval.tick() => {}
                    // Exit on DatabaseManager::close() â€” this task's pool clone
                ...
                // 24/7 recorder â€” ANALYZE otherwise only runs inside the
    ...
    /// command so the user can self-heal via the existing `MEMORA db
    ...
    pub(crate) fn spawn_startup_integrity_check(&self, database_path: Arc<str>) {
        ...
        tokio::spawn(async move {
            ...
            // quick_check(1) stops after the first error â€” we only need a
                ...
                .await
            {
                ...
                Ok(detail) => {
                    ...
                    error!(
                        ...
                        "DATABASE CORRUPTION DETECTED at startup. Recording is stopping to \
                         protect the database. Quit MEMORA and run \
                         `MEMORA db recover` to rebuild the database (it backs up the \
                         original first)."
                    );
                ...
                Err(e) => {
                    ...
                    // damaged to even scan â€” still actionable.
                    ...
                    error!(
                        ...
                        "startup integrity check could not run (database may be corrupt). \
                         If problems persist, quit MEMORA and run `MEMORA db recover`."
                    );
                }
            }
        });
    ...
    /// `auto_vacuum=NONE` (how it ships) â€” it only recycles pages internally.
    ...
    /// of contending (the "recording briefly pauses" the UI warns about â€”
...
mod wal_maintenance_tests {
    ...
    use memora_config::{DbConfig, DeviceTier};
