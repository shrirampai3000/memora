...
pub(crate) fn narrow_bbox_to_needle(
    ...
    if (text_len as f32) > chars_per_line_est {
        // Likely multi-line â€” leave bbox alone, otherwise we'd draw a thin sliver
...
fn find_matching_a11y_positions_in_nodes(
    ...
    let mut matches: Vec<TextPosition> = nodes
        ...
        .filter_map(|n| {
            ...
            // Locate the needle's char offset inside the node's text â€” used both
...
/// match â€” caller falls back to the paragraph bbox in that case.
fn match_against_line_spans(
    ...
    for line in lines {
        ...
        // The needle must still appear in the lowered line text (it does â€” we
...
/// coordinates are already screen-relative normalized (0â€“1) with top-left origin (Y down).
...
pub fn parse_all_text_positions(blocks: &[OcrTextBlock]) -> Vec<TextPosition> {
    blocks
        ...
        .filter_map(|block| {
            ...
            // Parse bounding box (already screen space, top-left origin, normalized 0â€“1)
            let left = block
                .left
                .parse::<f32>()
                .ok()
