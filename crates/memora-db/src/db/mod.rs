...
/// submits its write. Read-pool pressure is bursty â€” search, timeline, the local
/// API and pipes all share that pool â€” and failing the whole insert on the first
...
/// Auto-merge MUST NOT reopen these â€” a new detected meeting in the same
...
/// stream (`ui_events`) â€” the three biggest text contributors to db.sqlite.
...
impl ImmediateTx {
    ...
    /// Commit the transaction. Must be called explicitly â€” drop without commit = rollback.
    ...
    fn drop(&mut self) {
        if !self.committed {
            if let Some(mut conn) = self.conn.take() {
                ...
                // a transaction" â†’ stuck transaction cascade â†’ pool exhaustion.
                //
                ...
                warn!("ImmediateTx dropped without commit â€” rolling back");
...
pub struct DatabaseManager {
    ...
    /// serialized by write_semaphore anyway â€” the extra connections handle
    ...
    /// (and the shared `-shm` WAL-index mapping) alive after teardown â€” a leaked
...
/// multi-row INSERT instead of N `RETURNING id` round-trips â€” the win is
