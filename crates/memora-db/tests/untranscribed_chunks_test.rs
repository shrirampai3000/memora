...
/// Tests for `get_untranscribed_chunks` â€” the reconciliation sweep query.
///
/// Run with: cargo test --package memora-db --test untranscribed_chunks_test -- --nocapture
#[cfg(test)]
mod tests {
    use chrono::{Duration, Utc};
    use memora_db::{AudioDevice, ContentType, DatabaseManager, DeviceType, SearchResult};
    ...
    async fn test_respects_since_filter() {
        ...
        // Query only the last hour â€” should skip the old chunk
        let since = Utc::now() - Duration::hours(1);
        let untranscribed = db.get_untranscribed_chunks(since, 100).await.unwrap();
    ...
    async fn test_ordered_by_timestamp_descending() {
        ...
        // Newest first â€” ensures recent audio gets transcribed before old backlog
        assert_eq!(untranscribed.len(), 3);
    ...
    async fn test_meeting_transcript_query_merges_live_and_background_paths() {
        ...
        // Well inside the Â±15s live-coverage window â€” placing this on the exact
        ...
        let transcripts = rows
            ...
            .collect::<Vec<_>>();
        // Background rows within Â±15s of a live segment are dropped only when they
        // share the live segment's DIRECTION (input vs output) â€” see #3776. The
        ...
        // "me" â€” otherwise mic lines masked user assignments (unrenameable
