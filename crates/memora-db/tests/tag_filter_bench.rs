...
//!   cargo test -p memora-db --test tag_filter_bench -- --ignored --nocapture
//!
...
//!         idx_vision_tags_tag_id) then PK-looks-up frames â€” it does NOT scan
...
use memora_db::{ContentType, DatabaseManager};
...
///   cargo test -p memora-db --test tag_filter_bench related_tags -- --ignored --nocapture
///
...
///   1. a HOT input tag â€” on many items, each carrying many other tags â€” makes
///      the `co` CTE materialize (items Ã— tags-per-item) rows before GROUP BY.
...
/// extra co-tags (so 50kÃ—5 = 250k rows feed the aggregation) plus a 50k-memory
...
async fn bench_related_tags_scaling() {
    ...
    const CO_POOL: i64 = 50; // each hot frame carries 4 of these â†’ wide fan-out
    ...
    .await;
    // Each hot frame also carries 4 co-tags â†’ wide co-occurrence fan-out.
    for off in 0..4i64 {
        exec(
            &db,
            &format!(
