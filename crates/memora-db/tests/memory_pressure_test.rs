...
fn pressure_text(i: usize) -> String {
    format!(
        "MEMORA memory pressure row {i}. github customer meeting pricing todo error slack \
    ...
    for i in 0..frame_count {
        let windows = vec![
            ...
            FrameWindowData {
                ...
                browser_url: Some("https://github.com/shrirampai3000/memora".to_string()),
                focused: i % 5 == 0,
                text: format!(
...
async fn repeated_search_timeline_meeting_reads_do_not_grow_unbounded() {
    let frame_count = env_usize("MEMORA_PRESSURE_FRAMES", 6_000);
    let audio_count = env_usize("MEMORA_PRESSURE_AUDIO", 1_000);
    let ui_count = env_usize("MEMORA_PRESSURE_UI", 4_000);
    let meeting_count = env_usize("MEMORA_PRESSURE_MEETINGS", 60);
    let rounds = env_usize("MEMORA_PRESSURE_ROUNDS", 40);
    let allowed_growth_mb = env_usize("MEMORA_PRESSURE_MAX_RSS_GROWTH_MB", 1024) as f64;
...
async fn concurrent_write_read_churn_stays_bounded() {
    ...
    let duration_sec = env_usize("MEMORA_PRESSURE_CHURN_SECONDS", 60) as u64;
    let reader_count = env_usize("MEMORA_PRESSURE_READERS", 4);
    let writer_sleep_ms = env_usize("MEMORA_PRESSURE_WRITER_SLEEP_MS", 0) as u64;
    let allowed_growth_mb = env_usize("MEMORA_PRESSURE_MAX_RSS_GROWTH_MB", 1024) as f64;
