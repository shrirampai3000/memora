...
async fn close_severs_leaked_pool_clones_and_allows_reinit() {
    ...
    // The leaked clone must be severed â€” fail fast with a closed-pool error
    ...
    // And the same process can re-open the db cleanly â€” the step that failed
...
async fn repeated_restart_cycles_reopen_cleanly() {
    ...
    // The engine-respawn path is new â†’ use â†’ close, repeated. Each cycle must
