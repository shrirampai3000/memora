...
//! Tests for `mark_chunks_covered_by_live` â€” the marker that stops the
...
mod tests {
    ...
    async fn marks_chunk_inside_window_with_adjacent_live_segment() {
        ...
        // Live segment 2s later â€” well inside the Â±15s coverage window.
    ...
    async fn leaves_chunk_with_no_nearby_live_segment_pending() {
        ...
        // Live segment 2 minutes away from the chunk â€” outside the Â±15s window
    ...
    async fn handles_open_meeting_with_null_end() {
        let db = setup_test_db().await;
        let start = Utc::now() - Duration::minutes(10);
        // No meeting_end â€” still in progress.
    ...
    async fn is_idempotent() {
        ...
        // Already transcribed â€” second call must not bump anything.
        let second = db
            .mark_chunks_covered_by_live(meeting_id, 15.0)
            .await
            .unwrap();
    ...
    /// difference left the meeting's chunk pending â€” re-transcribed by the batch
