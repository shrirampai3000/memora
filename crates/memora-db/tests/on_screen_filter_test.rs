...
//! (issue #2436). The bug: AX trees capture off-screen text â€” terminal
//! scroll buffers, scrolled-off IDE editor lines â€” and search hits
...
//!
//! 1. No filter â†’ all matches (preserves pre-fix behavior).
//! 2. on_screen=true â†’ only visible matches.
//! 3. on_screen=false â†’ only off-screen matches.
...
mod tests {
    use chrono::Utc;
    use memora_db::{DatabaseManager, ElementSource};
    ...
    /// `memora_a11y::tree::AccessibilityTreeNode` so the db's private
    ...
    /// â€” we sleep briefly so SELECTs in tests see the row.
    async fn settle() {
    ...
    async fn search_elements_no_filter_returns_all_three_states() {
        // Sanity: with the filter omitted, every element matches â€”
    ...
    async fn search_elements_on_screen_false_keeps_only_off_screen() {
        // Inverse of the above â€” useful for "what was scrolled off?"
    ...
    async fn search_elements_filter_excludes_null_legacy_rows() {
        ...
        // have NULL â€” we cannot determine after the fact whether they
    ...
    async fn search_accessibility_visible_collapses_to_one_row_per_frame() {
        ...
        // elements live on the same frame â€” otherwise pagination math
