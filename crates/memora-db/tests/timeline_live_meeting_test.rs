...
mod timeline_live_meeting_tests {
    ...
    async fn test_live_meeting_segment_appears_on_timeline() {
        ...
        // A live meeting transcript ~2s later (within the frame's Â±15s window),
        // with NO audio chunk â€” exactly what the live path persists.
        let meeting_id = db
    ...
    /// (within Â±15s) is suppressed so the timeline doesn't show the speech twice.
    #[tokio::test]
    async fn test_live_segment_deduped_against_background() {
        ...
        // Live segment 3s later (within Â±15s of the background row) â†’ deduped out.
        let meeting_id = db
            .insert_meeting("zoom.us", "ui_scan", None, None)
            .await
            .unwrap();
    ...
    async fn test_mirror_live_meeting_into_audio_transcriptions() {
        ...
        // Idempotent â€” a second mirror is a no-op (INSERT OR IGNORE).
        let again = db
            .mirror_live_meeting_to_audio_transcriptions(meeting_id, 15.0)
            .await
            .unwrap();
    ...
    /// silently dropped â€” losing live transcript text is worse than a small
    ...
    async fn test_mirror_uses_far_same_device_chunk_instead_of_dropping() {
        ...
        // models a live final landing far from the nearest chunk timestamp â€” a
        ...
        // fetch is bounded by min/max segment time Â±window) and matches normally.
        db.insert_meeting_transcript_segment(
            meeting_id,
            "MEMORA-cloud",
            None,
    ...
    async fn test_meeting_segment_falls_back_to_freetext_speaker() {
        ...
        // No identified audio â†’ nothing to map.
        let mapped = db
            .backfill_meeting_segment_speakers(base - Duration::hours(1), 15.0)
            .await
            .unwrap();
    ...
    /// different-device chunk is nearer in time â€” otherwise a mic (input) segment
    ...
    async fn test_mirror_associates_segment_to_same_device_chunk() {
        ...
        // An INPUT (mic) segment at `base` â€” naively it would grab the nearer OUTPUT chunk.
        let meeting_id = db
            .insert_meeting("zoom.us", "ui_scan", None, None)
            .await
            .unwrap();
    ...
    async fn test_speaker_backfill_uses_bounded_index_scan_and_releases_write_lock() {
        ...
        // A large, irrelevant history makes the former candidateÃ—history CTE
