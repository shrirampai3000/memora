...
//! snapshot-only frames â€” see migration `20260220000000_event_driven_capture`).
...
//! UPDATE's WHERE clause never matches ANY row â€” `evicted_at` is never set,
...
async fn evict_marks_chunk_evicted_despite_out_of_range_null_chunk_frame() {
    ...
    // A recent, unrelated snapshot frame with NO video chunk at all â€” the
    ...
    assert_eq!(
        ...
        "the UPDATE must actually flip evicted_at â€” this was 0 under the NULL-poisoned anti-join"
    );
    assert_eq!(
    ...
    // this call would return the same file path again â€” forever.
...
/// Straddling chunks (some frames in range, some not â€” the case the anti-join
...
async fn straddling_chunk_is_still_skipped() {
    ...
    sqlx::query("INSERT INTO video_chunks (id, file_path) VALUES (1, '/data/straddle.mp4')")
        ...
        .await
        .unwrap();
    // One frame in range, one frame from the SAME chunk outside the range â€”
