...
//! connection resets the WAL while another connection begins a write. MEMORA
...
async fn capture_and_secret_databases_keep_distinct_safety_pragmas() {
    ...
    let secret_store = memora_secrets::SecretStore::open_for_data_dir(data_dir.path(), None)
        .await
        .expect("secret store");
    let secret_path = data_dir
        .path()
        .join(memora_secrets::SECRETS_DATABASE_FILENAME);
    let secret_pool =
        memora_secrets::shared_secret_pool(secret_path.to_str().expect("utf8 secrets path"))
            .await
...
async fn explicit_checkpoint_waits_for_capture_writer_coordinator() {
    ...
    let coordinator = memora_secrets::sqlite_write_lock(&path);
...
async fn isolated_engine_secret_writes_and_checkpoints_keep_integrity_ok() {
    ...
    let secret_store = Arc::new(
        memora_secrets::SecretStore::open_for_data_dir(data_dir.path(), None)
            .await
            .expect("secret store"),
    );
    let secret_path = data_dir
