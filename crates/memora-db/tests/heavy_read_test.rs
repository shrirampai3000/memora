...
//!   cargo test -p memora-db --test heavy_read_test -- --nocapture
...
async fn open_real_db() -> Arc<DatabaseManager> {
    let db_path = memora_core::paths::default_MEMORA_data_dir().join("db.sqlite");

    if !db_path.exists() {
        panic!("No DB found at {:?} â€” skip this test", db_path);
    }
...
/// still go through.
///
/// Requires the real production DB at `~/.memora/db.sqlite` â€” ignored
/// in CI where that doesn't exist. Run locally with:
///   cargo test -p memora-db --test heavy_read_test -- --ignored --nocapture
#[tokio::test]
#[ignore = "requires real ~/.memora/db.sqlite"]
async fn test_concurrent_ocr_searches_dont_starve_pool() {
        ...
        search_tasks.spawn(async move {
            ...
            let result = db
                .search(
                    ...
                    None,
                    None,
                    None, // no max_length â€” worst case
                    None,
                    None,
                    None,
                    None,
    ...
    assert!(
        ...
        "write should complete in <5s (got {:.2}s) â€” pool not starved",
        write_elapsed.as_secs_f64()
    );
...
///
/// Requires the real production DB â€” ignored in CI (see sibling test).
#[tokio::test]
#[ignore = "requires real ~/.memora/db.sqlite"]
