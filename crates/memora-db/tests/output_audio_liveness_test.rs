...
//! websocket blip) but remote audio is still flowing â€” i.e. the call is still
...
//!
//! Run with:
//!   cargo test --package memora-db --test output_audio_liveness_test -- --nocapture
...
mod tests {
    ...
    /// â€” otherwise the cross-device dedup check silently drops the row. We assert
    ...
    async fn old_output_excluded_by_short_window_included_by_long() {
        ...
        .await;
        // 60s old vs a 30s window â†’ out.
        assert!(!db.has_recent_output_audio(30).await.unwrap());
        // Same row, 120s window â†’ in. Proves the bound is computed against the
    ...
    async fn recent_input_with_old_output_is_false() {
        ...
        // row is stale (excluded by window) â†’ false. Proves the guard needs a
    ...
    async fn output_just_inside_window_is_true() {
        let db = setup_test_db().await;
        // 25s old against a 30s window â†’ comfortably inside. The 5s margin keeps
    ...
    async fn output_just_outside_window_is_false() {
        let db = setup_test_db().await;
        // 35s old against a 30s window â†’ comfortably outside (5s margin).
        insert_aged(
            &db,
            &output_device(),
            "outside",
