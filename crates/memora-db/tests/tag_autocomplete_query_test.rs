...
async fn tag_autocomplete_query_tolerates_malformed_memory_tags() {
    ...
    // Must not error â€” this is the regression. `.unwrap()` inside `tag_rows`
...
/// otherwise). A malformed row therefore 500s `GET /memories?tags=...` too â€”
...
async fn memory_tag_filter_tolerates_malformed_memory_tags() {
    ...
    // Must not error â€” both helpers `json_each` over every candidate row, so a
