...
///
/// Run with: cargo test --package memora-db --test chunk_outcome_test -- --nocapture
#[cfg(test)]
mod tests {
    ...
    async fn duplicate_outcome_marks_transcribed_without_rows() {
        ...
        // â€” the chunk has been considered, don't retry.
        assert_eq!(status, "transcribed");
        assert_eq!(attempts, 1);
        assert_eq!(transcription_count(&db, chunk).await, 0);
    }
    ...
    /// orphan chunk â†’ STT returns empty â†’ old helper no-op'd the silent
    /// mark â†’ next sweep picks the SAME chunk â†’ forever. This test locks
    ...
    /// it. The old code returned Ok(0) without marking the chunk â†’ the
    /// chunk's audio_chunks row never got a transcription row â†’ the chunk
