...
//! per-row attributes as the original per-row `INSERT â€¦ RETURNING id` path.

#[cfg(test)]
mod tests {
    use chrono::Utc;
    use memora_db::{DatabaseManager, FrameWindowData, OcrEngine, OcrTextBlock};
    use std::sync::Arc;
    ...
    /// Synthetic Apple Nativeâ€“shape text_json: N flat level-0 blocks.
