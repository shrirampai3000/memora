...
//! onto the nearest DISTANT frame â€” leaving the audio's own moment blank on the
//! timeline â€” or dropped it entirely when the range had no frames at all. The fix
...
mod timeline_frameless_audio_tests {
    ...
    /// the timeline at its OWN timestamp as an audio-only frame â€” not piled onto
    ...
    async fn test_frameless_audio_appears_at_own_timestamp() {
        ...
        // Speech 5 minutes later â€” far outside the Â±15s frame-attach window.
        ...
        // audio-only frame â€” not be dumped onto the distant `base` screen frame.
        assert!(
    ...
    /// Guard the normal path: speech within Â±15s of a real frame still attaches to
    ...
    async fn test_audio_near_frame_attaches_without_synthetic_frame() {
        ...
        // Speech 3s after the frame â€” inside the Â±15s attach window.
