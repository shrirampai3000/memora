...
mod timeline_performance_tests {
    ...
    use memora_db::{AudioDevice, DatabaseManager, DeviceType, OcrEngine};
    use std::sync::{
    ...
    async fn insert_frames_with_ocr(
        ...
        for i in 0..count {
            ...
            let ocr_text = format!(
                "Frame {} - Application: VSCode - Window: main.rs - MEMORA - \
                 ...
                 Finder - Documents/projects/MEMORA - 42 items, 1.2 GB available \
                 ...
                 Terminal output: Compiling MEMORA v0.2.76 warning: unused variable \
                 Browser tabs: GitHub - MEMORA/MEMORA - Stack Overflow - rust async \
    ...
    async fn test_file_based_db_performance() {
        ...
        let db_path = temp_dir.path().join("MEMORA_test_perf.db");
    ...
    /// Run with: cargo test -p memora-db --test timeline_performance_test test_stress -- --nocapture --ignored
    #[tokio::test]
