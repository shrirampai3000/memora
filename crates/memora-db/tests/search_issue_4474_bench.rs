...
//! SQLite backup rather than the actively-written MEMORA database:
//!
//! ```text
//! sqlite3 ~/.memora/db.sqlite ".backup '/tmp/MEMORA-4474.sqlite'"
//! MEMORA_BENCH_DB=/tmp/MEMORA-4474.sqlite \
//! MEMORA_BENCH_SAMPLES=5 \
//! cargo test -p memora-db --release --test search_issue_4474_bench \
...
fn benchmark_db_path() -> Result<PathBuf> {
    env::var_os("MEMORA_BENCH_DB")
        .map(PathBuf::from)
        .context("MEMORA_BENCH_DB must point to a consistent SQLite backup")
}

async fn read_only_pool(path: &Path, max_connections: u32) -> Result<SqlitePool> {
    let cache_size_kb = env_usize("MEMORA_BENCH_CACHE_KB", 64_000);
...
async fn print_database_manifest(pool: &SqlitePool, path: &Path) -> Result<()> {
    ...
    let selector_matches: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM frames_fts WHERE frames_fts MATCH ?1")
            .bind(env::var("MEMORA_BENCH_QUERY").unwrap_or_else(|_| "the".to_string()))
            .fetch_one(pool)
            .await?;
...
async fn benchmark_issue_4474_search_queries() -> Result<()> {
    ...
    let samples = env_usize("MEMORA_BENCH_SAMPLES", 5);
    let pool_size = env_usize("MEMORA_BENCH_POOL_SIZE", 27) as u32;
    let limit = env_usize("MEMORA_BENCH_LIMIT", 5) as u32;
    let projection_limit = env_usize("MEMORA_BENCH_PROJECTION_LIMIT", 100) as u32;
    let audio_limit = env_usize("MEMORA_BENCH_AUDIO_LIMIT", 100);
    let selector = env::var("MEMORA_BENCH_QUERY").unwrap_or_else(|_| "the".to_string());
