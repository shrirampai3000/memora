...
mod fts_contention_tests {
    ...
    use memora_db::{DatabaseManager, FrameWindowData, OcrEngine};
    use std::sync::Arc;
    ...
    async fn test_microbatch_fts_lock_time_bounded() {
        ...
        for chunk in rows.chunks(micro_batch_size) {
            ...
            // Lock released â€” other writers can go
        }
    ...
    // This is THE key test â€” proves the fix works.
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async fn test_frame_inserts_not_starved_during_microbatch_fts() {
        ...
        assert!(
            result.is_ok(),
            "Timed out â€” FTS and writes deadlocked or starved"
        );
        ...
        assert!(
            *max_latency < std::time::Duration::from_secs(2),
            "Frame insert was blocked for {:?} during FTS â€” write starvation!",
            max_latency
        );
    }
