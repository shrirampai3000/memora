...
mod tests {
    ...
    async fn test_search_filter_by_tags() {
        ...
        // Single tag â†’ only the frame carrying it.
        ...
        // Shared tag â†’ both frames carrying it.
        let atlas = run(&db, &["project:atlas".to_string()]).await;
        assert_eq!(atlas.len(), 2);

        // Multiple tags â†’ AND semantics: frame must carry all of them.
        let both = run(
            &db,
        ...
        // Unknown tag â†’ nothing.
        assert_eq!(run(&db, &["person:nobody".to_string()]).await.len(), 0);

        // No tag filter â†’ all three frames (the filter is strictly opt-in).
    ...
    async fn test_tag_filter_audio_and_cross_modal() {
        ...
        // Audio-only, by tag â†’ just the tagged chunk.
        ...
        // Input has no tag table â†’ empty under a tag filter.
        assert_eq!(
    ...
    async fn test_memory_filter_by_tags() {
        ...
        // Shared tag â†’ both memories carrying it.
        assert_eq!(
    ...
    async fn test_related_tags_co_occurrence() {
        ...
        // Unrelated frame â€” its tag must never surface for person:ada.
        let f_x = db
            .insert_frame("dev", None, None, Some("app"), Some(""), false, None)
            .await
            .unwrap();
        ...
        // Single input tag: project:atlas co-occurs 3Ã— (f_a, f_b, ac); the
        ...
        // Multiple inputs â†’ AND: only items carrying BOTH person:ada AND
        // project:atlas (f_a, f_b, ac â€” not the memory, which lacks the
        ...
        // Empty input â†’ empty result (no tags to relate against).
    ...
    async fn test_related_tags_edge_cases() {
        ...
        // (e.g. a URL tag) â€” the split into a namespace happens in the route,
        ...
        db.insert_memory("legacy", "user", None, Some("not valid json"), 0.5, None)
            .await
            .unwrap();
        // ...and one carrying the anchor twice (deliberately) â€” the value must
        ...
        assert!(names.contains("emoji:ðŸ¦€"));
        ...
        // limit = 0 â†’ no rows (not an error).
        assert!(db
        ...
        // A tag that exists on nothing â†’ empty, no error.
        assert!(db
    ...
    async fn test_dropped_tx_rolls_back_and_returns_connection() {
        ...
        // The Drop impl should rollback and return the connection to the pool.
        {
            ...
            sqlx::query("INSERT INTO audio_chunks (file_path, timestamp) VALUES ('should_not_exist.mp4', datetime('now'))")
                ...
                .await
                .unwrap();
            // tx is dropped here without commit â€” should rollback
        }
        ...
        // Verify the pool is still healthy â€” we can acquire connections and do work.
    ...
    async fn index_accessibility_fts(_db: &DatabaseManager) {
        // Tables dropped â€” nothing to index.
    }
