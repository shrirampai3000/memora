...
//! (MEMORA/MEMORA#4843) found in three more places while auditing
...
//! (`archive.rs`) â€” before the fix, neither actually reclaimed video_chunks
...
/// `cleanup_orphaned_chunks` has no time-range filter at all â€” it scans the
...
async fn cleanup_orphaned_chunks_deletes_despite_unrelated_null_chunk_frame() {
    ...
    // chunk at all â€” routine under event-driven capture.
    sqlx::query(
...
/// retention branch tested above â€” it needs its own coverage of the same
/// NULL-guard fix.
#[tokio::test]
...
/// orphan-cleanup DELETE after removing frames in range â€” a chunk whose only
...
async fn delete_time_range_local_cleans_up_orphaned_chunk_despite_unrelated_null_chunk_frame() {
    ...
    // it's deleted, chunk 1 has zero references anywhere â€” a true orphan.
    sqlx::query(
        "INSERT INTO frames (id, video_chunk_id, offset_index, timestamp) VALUES (1, 1, 0, ?1)",
    )
    .bind(&old_ts)
