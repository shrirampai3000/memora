...
//! is reaching the capture pipeline â€” i.e. the meeting's far side is being
...
//! states â€” the piggyback never acts on silence (a silent meeting mic is the
...
/// subscribers can filter without inspecting the payload â€” same convention
...
pub struct AudioCaptureHealthEvent {
    ...
    /// Output devices MEMORA is currently capturing
    /// (e.g. `["Speakers (Realtek(R) Audio) (output)"]`).
    pub captured_outputs: Vec<String>,
    /// Input devices MEMORA is currently capturing
