...
//! code-522 / code-11 corruption class), the breaker tries to stopâ†’spawn the
...
pub enum DbRecoveryState {
    ...
    /// port never rebound). Nothing else retries â€” a fully-down server never
    /// fires the persistent-failure hook again â€” so recording stays stopped.
    RestartFailed,
    /// The breaker gave up after repeated restarts that didn't clear the wedge.
    /// This is on-disk corruption a restart can't repair â€” manual recovery
    /// (`MEMORA db recover`) is required.
    NeedsRecovery,
}
...
/// payload â€” same convention as `audio_capture_health_*`.
