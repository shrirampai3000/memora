...
//! Detection happens at the source â€” SCStream errors, CoreAudio errors, or
//! an `AXIsProcessTrusted()` transition â€” so events fire within ~100ms of
//! the actual OS state change.
//!
//! The Tauri app subscribes via `/ws/events` and opens the recovery modal.
//! CLI users can tail these events with `MEMORA events --follow`.

use serde::{Deserialize, Serialize};

/// Which permission changed.
///
/// `Keychain` is not a TCC permission â€” it models the "Encrypt Data at Rest"
/// toggle (see `memora-secrets`). Emitted when the stored encryption key
...
/// recording, so the app should surface it as a softer notification â€” not
...
/// payload â€” matches the pattern used by `meeting_started` / `meeting_ended`.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PermissionEvent {
    pub kind: PermissionKind,
    pub state: PermissionState,
    /// OS error message or context for diagnostics (stack-trace style, human readable).
    /// Not required â€” set only when emission site has a useful error string.
