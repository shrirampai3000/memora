...
//! `store: Option<&memora_secrets::SecretStore>` as its first parameter.
//!
//! **Writes**: when `store` is `Some`, the token is written *only* to the
//! SecretStore â€” no plaintext shadow on disk. If the write fails, the error
//! is propagated (we do not silently downgrade to plaintext â€” that would
...
//! existing MEMORA API server at a fixed path:
//!
...
//!   3. The provider redirects back; the MEMORA server handles
...
// ---------------------------------------------------------------------------
// Fixed redirect URI â€” registered in the provider's developer console
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Pending callback map â€” shared between oauth_connect (Tauri) and the
// /connections/oauth/callback HTTP handler (memora-engine)
// ---------------------------------------------------------------------------
...
/// and optional description (RFC 6749 Â§4.1.2.1) â€” never codes, tokens, or
/// state values.
#[derive(Debug)]
...
/// exchange is proxied through `https://github.com/shrirampai3000/memora/api/oauth/exchange` which
...
/// send-only vs send+read). The scope strings stay here, server-side â€” the
...
// ---------------------------------------------------------------------------
// Legacy plaintext file location  (~/.memora/{id}-oauth.json)
//
...
pub fn oauth_token_path_instance(integration_id: &str, instance: Option<&str>) -> PathBuf {
    ...
    memora_core::paths::default_MEMORA_data_dir().join(name)
}
...
/// email address. We do NOT fall back when multiple instances exist â€”
...
/// write back under the same key they read from â€” otherwise the refreshed
...
pub(crate) async fn load_oauth_json_with_instance(
    ...
    // safe to serve it â€” we are not guessing between multiple accounts.
    if instance.is_some() {
    ...
    match recoverable_named.len() {
        ...
        _ => {
            ...
            // gets None (returning a random instance would be worse â€” we
            // could leak the wrong account's data).
            tracing::warn!(
                "oauth: {} default lookup empty and {} recoverable instances exist ({}) â€” caller passed instance=None; pick one explicitly",
                integration_id,
...
/// `access_token` â€” so a freshly connected Slack account otherwise reads as
    ...
    if let Some(s) = store {
        ...
        match s.get_json::<Value>(&key).await {
            ...
            Err(e) => {
                ...
                // key is unavailable (devâ†”prod bundle ACL split, recent encryption
...
/// Check if an OAuth instance is recoverable â€” has a valid token or a
...
/// unavailable" â€” the former requires a reconnect, the latter just needs the
...
/// same "not connected" string â€” which is wrong (and infuriating) when the
...
/// - no instances at all                       â†’ "not connected, please connect"
/// - one instance, default lookup unhealthy    â†’ "instance X token can't refresh, reconnect"
/// - multiple instances, none specified        â†’ "ambiguous: pick one of [X, Y]"
/// - explicit instance specified but unhealthy â†’ "instance X token can't refresh, reconnect"
pub async fn describe_oauth_error(
    ...
    match (instance, instances.as_slice(), all_instances.as_slice()) {
        (Some(inst), _, _) => format!(
            "{display_name} account '{inst}' is not connected or its token can't be refreshed â€” reconnect it from the Connections page in the desktop app"
        ),
        (None, [], []) => format!(
            "{display_name} not connected â€” use 'Connect {display_name}' from the Connections page in the desktop app"
        ),
        (None, [], stale) => format!(
            "{display_name} has saved account row(s) ({}) but none can be refreshed â€” reconnect it from the Connections page in the desktop app",
            stale.join(", "),
        ),
        (None, [only], _) => format!(
            "{display_name} account '{only}' token can't be refreshed â€” reconnect it from the Connections page in the desktop app"
        ),
        (None, many, _) => format!(
            "multiple {display_name} accounts connected ({}) â€” specify which one with `instance`. On JSON-body endpoints add `\"instance\": \"<email>\"` to the request body; on proxy/query endpoints add `?instance=<email>` (e.g. instance=\"{}\")",
            many.join(", "),
            many[0],
        ),
    }
...
///   error is propagated â€” we do not silently fall back to plaintext.
...
///
/// The generation only advances when the refresh token *value* changes â€” a
...
/// or observed, even after a disconnectâ†’reconnect or a transient keychain read
/// miss reset `prior` to absent. See [`write_oauth_token_instance`] and
/// `memora_core::connections::sync::remote_is_newer`.
fn next_refresh_gen(
    ...
    let rotated = match (incoming_refresh, prior_refresh) {
        ...
        (None, _) => false,      // no refresh token in this write â€” don't bump
    };
    if rotated {
...
pub async fn write_oauth_token_instance(
    ...
    // Stamp every write. Every path into this function â€” initial
    // `exchange_code`, lazy `refresh_token_instance`, and the background
    // scheduler â€” has just round-tripped the provider's token endpoint,
    ...
    // disconnectâ†’reconnect or a transient keychain read miss (the documented
    // devâ†”prod bundle ACL split) that would otherwise read prior as absent,
    ...
    // `memora_core::connections::sync::remote_is_newer` immune to the
    // wall-clock skew between two devices refreshing the same connection.
    {
        let gen_field = memora_core::connections::sync::OAUTH_REFRESH_GEN_FIELD;
        ...
        let prior_gen = prior
            ...
            .unwrap_or(0);
        let highwater =
            memora_core::connections::sync::read_oauth_gen_highwater(&data_dir, &manifest_key);
        ...
        memora_core::connections::sync::bump_oauth_gen_highwater(
            &data_dir,
            &manifest_key,
            new_gen,
        );
    ...
    // SecretStore path â€” no plaintext shadow on disk.
    if let Some(s) = store {
        let key = store_key(integration_id, instance);
        ...
        memora_core::connections::sync::clear_connection_tombstone(
    ...
    // Fallback: no SecretStore available â€” `0o600` plaintext file.
    let path = oauth_token_path_instance(integration_id, instance);
...
pub async fn delete_oauth_token_instance(
    ...
    // Always try to sweep the plaintext file â€” it may exist even when we
    ...
    memora_core::connections::sync::record_connection_tombstone(
        &memora_core::paths::default_MEMORA_data_dir(),
        &manifest_key,
    );
    Ok(())
...
/// second account â€” promote it to its own named slot instead of deleting it,
...
pub async fn sweep_shadowed_default_slots(store: &SecretStore) -> Result<usize> {
    ...
    // after the prefix â†’ default slot.
    // `oauth:google-calendar:alice@x.com` â†’ named instance.
    ...
    for id in &has_default {
        let Some(named_for_id) = named.get(id) else {
            // Lonely default slot â€” the normal single-account happy path.
            continue;
        };
        ...
        if is_distinct_recoverable {
            // A real, separate account sitting in the default slot â€” promote it
            ...
            if let Err(e) =
                write_oauth_token_instance(Some(store), id, Some(&email), &default_val.unwrap())
                    .await
            {
                // Promotion failed â€” leave the default slot intact rather than
...
/// post-save cleanup used to blindly delete that default slot â€” silently
...
///   just saved â†’ delete it (stale-duplicate / zombie cleanup, the original
///   intent â€” a stale default slot otherwise shadows every `instance=None`
///   read).
/// - default slot holds a *distinct, recoverable* account â†’ promote it to its
...
/// Per RFC 6749 Â§5.2 a rotated-out or revoked refresh token comes back as
...
pub async fn refresh_token_instance(
    ...
    // written a *different* refresh token to our store â€” i.e. another device
    ...
    //
    // 1. Refresh responses for most providers (Google, Microsoft, Slack, â€¦)
    ...
    //    project_url, service_key, team_id, â€¦). The connection then
    ...
    //    subsequent lookups find first (exact-match wins over fallback) â€”
...
///    lifetime fields no longer apply â€” they're stripped before overlay
...
///    refresh loop: read sees stale expiry â†’ refresh â†’ write keeps stale
///    expiry â†’ read sees stale expiry â†’ â€¦
///
/// 2. JSON `null` in `resp` is treated as "field absent". Per RFC 6749
///    Â§5.1 the refresh response MAY omit `refresh_token`; a few providers
...
fn merge_refresh_response(stored: &Value, resp: &Value) -> Value {
    ...
    let Some(merged_obj) = merged.as_object_mut() else {
        // Stored wasn't a JSON object â€” caller corrupted the slot. Best we
...
pub async fn get_valid_token_instance(
    ...
    // a failure â€” keep it at debug. Real refresh failures (a token exists but
    ...
    {
        tracing::debug!(
            "oauth: no stored token for {}(instance={:?}) â€” skipping refresh",
            integration_id,
            instance,
        );
        return None;
...
// ---------------------------------------------------------------------------
// Token exchange (via backend proxy â€” secrets stay server-side)
// ---------------------------------------------------------------------------

const EXCHANGE_PROXY_URL: &str = "https://github.com/shrirampai3000/memora/api/oauth/exchange";

#[cfg(test)]
#[allow(clippy::items_after_test_module)]
mod tests {
    //! Covers the instance fallback added to `load_oauth_json`. The fallback
    //! is the safety net for callers that predate multi-account support â€”
    ...
    fn refresh_gen_bumps_only_on_rotation() {
        // First token for a slot (no prior) â†’ generation 1.
        assert_eq!(next_refresh_gen(Some("r1"), None, 0, 0), 1);
        // Same refresh token (plain access-token refresh) â†’ unchanged.
        assert_eq!(next_refresh_gen(Some("r1"), Some("r1"), 5, 5), 5);
        // Refresh token rotated â†’ one past the prior generation.
    ...
    fn refresh_gen_seeds_from_highwater_on_reconnect() {
        // prior reads as absent (disconnectâ†’reconnect, or a keychain read
    ...
    fn invalid_grant_detection_gates_retry() {
        ...
        // A server-side hiccup is not the token's fault â€” not retryable.
        assert!(!looks_like_invalid_grant(
            StatusCode::INTERNAL_SERVER_ERROR,
            "invalid_grant"
        ));
        // 400 without the marker is some other client error â€” not retryable.
        assert!(!looks_like_invalid_grant(
            StatusCode::BAD_REQUEST,
    ...
    // in `load_oauth_json_exact` (which scans `~/.memora/{id}*-oauth.json`)
    ...
    async fn get_valid_token_with_no_stored_token_returns_none_without_refresh() {
        ...
        // path attempted a refresh anyway, hit "no stored token", and WARNed â€”
    ...
    async fn load_with_none_falls_back_to_sole_named_instance() {
        ...
        store
            .set_json(
                &format!("oauth:{}:louis@github.com/shrirampai3000/memora", id),
    ...
    async fn load_with_none_prefers_exact_match_over_fallback() {
        ...
        // recoverable match wins â€” don't accidentally prefer a random named
        // instance.
        let store = mem_store().await;
        let id = "_t_prefer";
        store
    ...
    async fn load_with_none_is_ambiguous_when_multiple_instances() {
        // Two named instances, no default â€” we must NOT pick one arbitrarily.
    ...
    async fn load_with_wrong_explicit_instance_does_not_fall_back() {
        ...
        // don't silently substitute another one â€” that's worse than "not
    ...
    async fn explicit_instance_falls_back_to_sole_default_slot() {
        ...
        // only one account â€” we should serve it rather than 401.
        let store = mem_store().await;
    ...
    async fn explicit_instance_does_not_fall_back_when_multiple_exist() {
        // Two named accounts â€” passing a non-matching explicit instance must
    ...
    async fn sweep_leaves_lonely_default_slot() {
        ...
        // normal single-account happy path â€” don't touch it.
        let store = mem_store().await;
        store
    ...
    async fn sweep_leaves_named_only_state_alone() {
        // No default slot, only named entries â€” also a healthy state,
    ...
    async fn reconcile_promotes_distinct_default_account() {
        ...
        // just connected under his own instance. alice must survive â€” promoted
        ...
        store
            ...
            .await
            .unwrap();
        // bob's slot â€” written by oauth_connect before reconcile runs
        store
            .set_json(
                &format!("oauth:{}:bob@x.com", id),
                &json!({"access_token": "b", "refresh_token": "rb", "email": "bob@x.com"}),
    ...
    async fn reconcile_drops_unrecoverable_default() {
        ...
        // shadow instance=None reads â€” the original cleanup intent â€” and must
    ...
    fn merge_preserves_refresh_token_when_response_omits_it() {
        let stored = json!({
            ...
            "expires_at": 100,
            "email": "louis@github.com/shrirampai3000/memora",
        });
        let resp = json!({
            "access_token": "new",
            "expires_in": 3599,
        ...
        assert_eq!(merged["email"], "louis@github.com/shrirampai3000/memora");
    ...
    fn merge_strips_stale_expires_at_when_response_omits_expires_in() {
        ...
        // triggering another refresh â€” forever.
        let stored = json!({
            "access_token": "old",
            "refresh_token": "rt",
    ...
    fn merge_treats_response_nulls_as_field_absent() {
        // RFC 6749 Â§5.1 lets refresh responses OMIT refresh_token. Some
    ...
    fn merge_preserves_provider_identity_metadata() {
        ...
        // project_url/service_key, Slack stamps team_id/workspace_name â€”
    ...
    async fn load_with_instance_reports_fallback_target() {
        ...
        assert_eq!(inst.as_deref(), Some("louis@github.com/shrirampai3000/memora"));
    }
...
/// Exchange an authorization `code` for tokens via the MEMORA backend
/// proxy at `github.com/shrirampai3000/memora`.  The backend holds `client_secret` â€” the desktop
/// app never sees it.
///
/// On failure, includes the raw response body in the error so callers can
/// surface the upstream provider message (AADSTS, invalid_grant, â€¦) instead
...
pub async fn exchange_code(
    ...
    // the token URL â€” e.g. Zendesk's `subdomain`, whose token endpoint lives on
