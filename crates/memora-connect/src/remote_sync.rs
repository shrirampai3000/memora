...
//! Pure-Rust SSH/SFTP sync for MEMORA data.
//!
//! Copies the entire ~/.memora directory to a remote server over SFTP.
...
/// Per-file upload timeout (10 minutes â€” enough for ~2GB at slow speeds).
...
fn should_skip(name: &str) -> bool {
    ...
    name.ends_with(".sqlite")
        ...
        // db.sqlite), so every other `db.sqlite*` file here â€” `-wal`/`-shm`
        // plus manual `.bak-*` / `.corrupt-*` / `.recovered` recovery copies â€”
...
pub struct SyncOptions {
    ...
    /// (mp4/wav/jpg/...). Ships the searchable db + config only â€” turns a
    /// multi-hundred-GB `~/.memora` into roughly the db size.
    pub no_media: bool,
    /// Extra exclude globs, matched against the path relative to the data dir
    /// AND the bare file name. Sourced from `--exclude` and
    /// `<data-dir>/.memoraignore`. Supports `*` wildcards.
...
/// everything else is literal. Enough for `.memoraignore` entries like
...
/// MEMORA skill file dropped on the remote so AI agents know how to query the data.
const MEMORA_SKILL: &str = r#"# MEMORA Data Reference

This directory contains synced data from [MEMORA](https://github.com/shrirampai3000/memora) â€” an AI that knows everything you've seen, said, or heard.
...
**audio_transcriptions** â€” Speech-to-text (mic + system audio)
```sql
...
**meetings** â€” Detected meetings/calls
```sql
...
// â”€â”€ SSH plumbing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/// Used to wire an agent (e.g. `MEMORA agent setup <target>`) on a VPS
/// without the user touching a terminal â€” reuses the same SSH creds as sync.
...
// â”€â”€ File transfer helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async fn list_remote_recursive(sftp: &SftpSession, base: &str, prefix: &str) -> HashSet<String> {
    ...
    for entry in entries {
        ...
        // Try reading as directory â€” if it works, recurse
...
async fn sync_dir(
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        // User selectivity (--no-media / --exclude / .memoraignore), on top
...
/// Sync the entire ~/.memora directory to the remote server.
...
/// (`--no-media`, `--exclude`, `.memoraignore`). The plain
...
/// dropped before we return â€” important because we then SFTP the snapshot.
...
async fn sync_to_remote_inner(
    ...
    //     yielding torn pages on the destination â†’ "database disk image
    ...
    // RAII-style cleanup â€” unlink the snapshot whether we succeed or fail.
    struct SnapshotGuard(Option<PathBuf>);
    impl Drop for SnapshotGuard {
        fn drop(&mut self) {
            if let Some(p) = self.0.take() {
                // Sync remove â€” Drop can't await. Acceptable: tempfile is
    ...
    // Resolve ~ to absolute path â€” SFTP doesn't understand tilde
    ...
    // â€” uploaded separately below from the snapshot).
    sync_dir(
        &sftp,
        data_dir,
        &remote_base,
    ...
    // Always full-upload (not incremental) â€” the snapshot reflects the
    ...
    let skill_path = format!("{}/MEMORA.md", remote_base);
    if let Ok(mut f) = sftp.create(&skill_path).await {
        let _ = f.write_all(MEMORA_SKILL.as_bytes()).await;
...
pub async fn test_connection(config: &SyncConfig) -> Result<()> {
    let (session, sftp) = connect_sftp(config).await?;
    info!("SSH OK â†’ {}@{}:{}", config.user, config.host, config.port);
    drop(sftp);
    let _ = session
        .disconnect(russh::Disconnect::ByApplication, "", "")
        .await;
...
// â”€â”€ Host discovery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub async fn discover_ssh_hosts() -> Vec<DiscoveredHost> {
    ...
    // mDNS discovery â€” find MEMORA instances advertising on LAN
...
mod tests {
    ...
    fn test_should_skip() {
        ...
        // remote host on 2026-06-20 â€” multi-GB db copies + secret backups).
