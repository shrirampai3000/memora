...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Continuously sync MEMORA memories into the OpenAI Codex CLI's memory store (CODEX_HOME/AGENTS.md by default). MEMORA writes a marker block that it owns and rewrites idempotently â€” hand-edited content outside the block is left alone. Leave home_path empty to use the default ($CODEX_HOME or ~/.codex).",
    fields: &[FieldDef {
        key: "home_path",
...
impl Integration for Codex {
    ...
    async fn test(
        ...
        let probe = path.join(".memora-write-probe");
...
/// `home_path` field â†’ `$CODEX_HOME` â†’ `~/.codex`. Mirrors what the
/// Codex CLI itself does so MEMORA writes to the same place the
...
/// Inner pure-function variant of [`resolve_home_path`] â€” the env-var
...
mod tests {
    ...
    fn empty_env_falls_back_to_default() {
        // CODEX_HOME="" should be treated as unset, not as "" â†’ that
