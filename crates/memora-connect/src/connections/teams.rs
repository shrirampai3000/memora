...
// token storage â€” users can connect Teams without granting full M365 access.
...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Microsoft Teams integration with two modes: \
        (1) OAuth â€” connect your account for full read/write access via Microsoft Graph API. \
        ...
        (2) Webhook â€” paste an Incoming Webhook URL to send messages to a channel without OAuth. \
        \
        ...
        OAuth endpoints (all prefix with /connections/teams/proxy/): \
          GET  me/chats â€” list all chats (DMs + group chats). \
          GET  me/chats/{chatId}/messages â€” read messages in a chat. \
          POST me/chats/{chatId}/messages {\"body\":{\"content\":\"...\"}} â€” send a DM. \
          GET  me/joinedTeams â€” list joined Teams. \
          GET  teams/{teamId}/channels â€” list channels in a team. \
          GET  teams/{teamId}/channels/{channelId}/messages â€” read channel messages. \
        \
        Webhook endpoint (no OAuth): \
        POST {\"text\": \"your message\"} to /connections/teams/proxy. The webhook target stays server-side.",
    fields: &[FieldDef {
        key: "webhook_url",
        label: "Incoming Webhook URL (optional â€” for send-only without OAuth)",
        secret: true,
...
impl Integration for Teams {
    ...
    async fn test(
            ...
            oauth::get_valid_token_instance(secret_store, client, "teams", None).await
        {
            ...
            return Ok(format!("connected via OAuth â€” {} team(s) found", count));
        }
        ...
        client
            .post(url)
            .json(&json!({"text": "MEMORA connected"}))
            .send()
            .await?
            .error_for_status()?;
