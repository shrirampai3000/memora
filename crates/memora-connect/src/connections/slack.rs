...
static SCOPE_VARIANTS: &[ScopeVariant] = &[
    ScopeVariant {
        id: "send",
        label: "Send only",
        description: "Post messages as you. MEMORA can't read your Slack.",
...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Act in Slack as your own account â€” no bot is installed. \
...
impl Integration for Slack {
    ...
    async fn test(
        ...
        let token_json = oauth::load_oauth_json(secret_store, "slack", None)
            .await
            .ok_or_else(|| anyhow!("not connected â€” use 'Connect with Slack' button"))?;
        ...
        if let Some(user_token) = token_json["authed_user"]["access_token"].as_str() {
            ...
            let resp: Value = client
                ...
                .json(&serde_json::json!({"channel": channel, "text": "MEMORA connected"}))
                .send()
                .await?
                .json()
                .await?;
            ...
            return Ok(match team {
                ...
                None => "connected â€” sent you a test message".into(),
            });
        }
        ...
        let url = token_json["incoming_webhook"]["url"]
            .as_str()
            .ok_or_else(|| anyhow!("Slack connection is missing a user token â€” reconnect Slack"))?;
        client
            .post(url)
            .json(&serde_json::json!({"text": "MEMORA connected"}))
            .send()
            .await?
            .error_for_status()?;
