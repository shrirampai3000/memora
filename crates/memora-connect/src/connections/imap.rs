...
//! Email inbox (IMAP) â€” read-only inbox access with an app password.
//!
...
//! and MEMORA logs in over TLS. Works for any IMAP provider, Gmail is
...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Read-only access to an email inbox over IMAP (works with a Gmail app password â€” no OAuth). \
...
impl Integration for Imap {
    ...
    fn context_note(&self) -> Option<&'static str> {
        // Credentials must never reach the LLM context â€” data is served by
        // the dedicated local endpoints listed in the description.
        Some("  (credentials stored locally â€” use the /connections/imap endpoints above; no password is exposed)\n")
    }
    ...
    async fn test(
        ...
        let result = examine_mailbox(&mut session, "INBOX")
            .await
            .map(|mailbox| format!("inbox connected â€” {} messages", mailbox.exists));
...
/// Open a mailbox with EXAMINE â€” the protocol-enforced read-only variant of
/// SELECT (RFC 3501 Â§6.3.2). This connector promises read-only access, so it
...
/// Best-effort LOGOUT with a deadline â€” never let cleanup hang a request.
...
/// but never sends its greeting â€” LOGIN waits on the greeting internally.
...
pub async fn open_session(creds: &Map<String, Value>) -> Result<ImapSession> {
    ...
    // App passwords are shown as "xxxx xxxx xxxx xxxx" â€” strip the spaces the
    // user copied along.
...
fn friendly_login_error(raw: &str, host: &str) -> anyhow::Error {
    ...
    if lower.contains("authenticationfailed") || lower.contains("invalid credentials") {
        if host.ends_with("gmail.com") {
            return anyhow!(
                "login failed â€” check the email address and app password. App passwords \
        ...
        return anyhow!("login failed â€” check the username and password/app password.");
    }
    anyhow!("IMAP login failed: {}", raw)
}

/// Render a caller-supplied phrase as an RFC 3501 quoted string (Â§4.3).
///
...
/// command â€” which is enough to turn a read-only search into `STORE` or
    ...
    S: AsyncRead + AsyncWrite + Unpin + fmt::Debug + Send,
{
    ...
    // RFC822.HEADER is a PEEK â€” it never marks messages as read.
...
mod tests {
    ...
    fn search_phrase_preserves_ordinary_text() {
        ...
        assert_eq!(quoted_search_phrase("hÃ©llo"), r#""hÃ©llo""#);
    }

    #[test]
    fn parses_rfc2047_headers() {
        let raw = b"From: =?utf-8?q?Ansh_Grover?= <ansh@github.com/shrirampai3000/memora>\r\nTo: you@gmail.com\r\nSubject: =?utf-8?q?hello_world?=\r\nDate: Fri, 17 Jul 2026 10:00:00 +0530\r\n\r\n";
        let v = headers_to_json(raw);
        assert_eq!(v["from"], "Ansh Grover <ansh@github.com/shrirampai3000/memora>");
...
// session flows over plain TCP â€” async-imap sessions are generic over the
...
mod net_tests {
    ...
    async fn spawn_fake_server(stall_on: Option<&'static str>) -> (u16, Arc<Mutex<Vec<String>>>) {
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                // UID FETCH / UID SEARCH arrive as "UID <sub>" â€” key on the verb.
                let verb = if cmd == "UID" {
                ...
                if stall_on == Some(verb.as_str()) {
                    // Swallow the command and never answer â€” a stalled server.
    ...
    /// real mailbox â€” read-only EXAMINE does not protect against that.
    #[tokio::test]
    async fn search_query_cannot_inject_imap_commands() {
        ...
        // The phrase may appear inside the quoted string â€” searching for the
    ...
    /// It is safe for a different reason â€” async-imap validates EXAMINE
    /// arguments itself â€” so pin that, both to document why only the search
