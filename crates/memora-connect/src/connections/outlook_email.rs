...
// storage â€” they can connect just their Outlook mailbox without granting the
...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Read and send Outlook email via OAuth and the Microsoft Graph API. \
        Connected via OAuth â€” click 'Connect Outlook'. Works with both personal \
        ...
        GET /connections/outlook-email/proxy/me/messages â€” NOT \
        ...
        Endpoints (all prefix with /connections/outlook-email/proxy/): \
          ...
          GET  me/messages?$top=<n>&$search=\"<query>\"&$select=subject,from,receivedDateTime,bodyPreview â€” list/search emails. \
          GET  me/messages/{id} â€” read a full email. \
          POST me/sendMail {\"message\":{\"subject\":\"...\",\"body\":{\"contentType\":\"Text\",\"content\":\"...\"},\"toRecipients\":[{\"emailAddress\":{\"address\":\"...\"}}]}} â€” send an email. \
...
impl Integration for OutlookEmail {
    ...
    async fn test(
        ...
        let token = oauth::get_valid_token_instance(secret_store, client, "outlook-email", None)
            .await
            .ok_or_else(|| anyhow!("not connected â€” use 'Connect Outlook' button"))?;
        ...
        Ok(format!("connected â€” {} unread in inbox", unread))
    }
}
