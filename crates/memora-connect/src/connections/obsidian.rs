...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Write notes to an Obsidian vault. vault_path is the absolute path to the vault root â€” write .md files directly to that directory using bash (e.g. echo '...' > {vault_path}/note.md). There is no HTTP write API; use the filesystem. Use [[wikilinks]] for internal links, YAML frontmatter, and #tags.",
    fields: &[
        FieldDef {
            key: "vault_path",
            label: "Vault Path",
...
impl Integration for Obsidian {
    ...
    async fn test(
        ...
        let test_file = path.join(".memora-test");
