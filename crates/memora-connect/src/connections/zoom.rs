...
//
// SETUP â€” website-side (must land alongside this PR before the connection
// works end-to-end):
// 1. Create a Zoom Marketplace app at https://marketplace.zoom.us/develop/create
//    (User-managed â†’ OAuth, mark as "Private" for design-partner scope; promote
//    to public listing after review later).
// 2. Set redirect URL to https://github.com/shrirampai3000/memora/api/oauth/callback (the website
//    relays the code back to the local server, same pattern Notion/Linear use).
// 3. Subscribe the chosen granular scopes (below â€” `user:read:user` is the
//    minimum for `test()`; the others unlock recordings + meetings).
// 4. Replace `client_id` below with the Marketplace app's client ID.
// 5. Wire the token-exchange handler in website-MEMORA so
//    POST https://github.com/shrirampai3000/memora/api/oauth/exchange with `provider=zoom`
...
//
// Granular scopes (current standard â€” Zoom is migrating away from the
// classic `meeting:read` style):
//   - user:read:user                              â†’ /users/me  (used by test())
//   - meeting:read:list_meetings                  â†’ list past + scheduled meetings
//   - cloud_recording:read:list_user_recordings   â†’ list cloud recordings
...
static OAUTH: OAuthConfig = OAuthConfig {
    ...
    // private distribution). Safe to ship in the binary â€” the secret half
    ...
    redirect_uri_override: Some("https://github.com/shrirampai3000/memora/api/oauth/callback"),
};
...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Read-only access to Zoom meetings + cloud recordings. \
        Agent control surface (proxied â€” secret stays server-side): \
        GET /connections/zoom/proxy/users/me â€” current user. \
        GET /connections/zoom/proxy/users/me/meetings?type=past&page_size=30 â€” list past meetings (also accepts type=scheduled, upcoming, live). \
        GET /connections/zoom/proxy/meetings/{meetingId}/recordings â€” cloud recording files (mp4, m4a, transcript VTT, chat log). \
        GET /connections/zoom/proxy/users/me/recordings?from=YYYY-MM-DD&to=YYYY-MM-DD â€” recordings in a date range. \
...
impl Integration for Zoom {
    ...
    fn proxy_config(&self) -> Option<&'static ProxyConfig> {
        // Zoom REST API â€” every endpoint under /v2 takes a Bearer token
    ...
    async fn test(
        ...
        let token = oauth::get_valid_token_instance(secret_store, client, "zoom", None)
            .await
            .ok_or_else(|| anyhow!("not connected â€” use 'Connect Zoom' button"))?;
