...
// OAuth 2.0 (3LO) app registered at developer.atlassian.com â†’ "MEMORA".
...
// github.com/shrirampai3000/memora/api/oauth/exchange.
...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Create and manage Jira issues. Connected via OAuth â€” no API token required. \
        Proxy base: /connections/jira/proxy/. \
        Useful endpoints: \
        GET rest/api/3/myself â€” current user info. \
        GET rest/api/3/project â€” list all projects. \
        GET rest/api/3/issue/{issueKey} â€” get issue details. \
        POST rest/api/3/issue â€” create a new issue (body: {fields:{project:{key},summary,issuetype:{name}}}). \
        POST rest/api/3/issue/{issueKey}/transitions â€” transition issue status. \
        GET rest/api/3/search?jql=... â€” search issues with JQL.",
    fields: &[],
};
...
impl Integration for Jira {
    ...
    async fn test(
        ...
        let token = oauth::get_valid_token_instance(secret_store, client, "jira", None)
            .await
            .ok_or_else(|| {
                anyhow!("not connected â€” use 'Connect with Jira' from the Connections page in the desktop app")
            })?;
        ...
        let cloud_id = oauth_json["cloud_id"]
            .as_str()
            .ok_or_else(|| anyhow!("Jira cloud_id missing â€” disconnect and reconnect Jira"))?;
