...
// MEMORA's Zendesk OAuth app is a **global** OAuth client so a single
...
// config â€” the `{subdomain}` placeholder is templated in at connect time from
// the `instance` the user enters (see apps/.../src-tauri/src/oauth.rs and the
// github.com/shrirampai3000/memora /api/oauth/exchange proxy, which both substitute it).
//
// External setup (one-time, done outside this repo):
//   * Register a global OAuth client in a `d3v-` Zendesk account with the
//     unique identifier `zdg-MEMORA` (the identifier becomes the client_id).
//   * Redirect URL: https://github.com/shrirampai3000/memora/api/oauth/callback
//   * Add OAUTH_ZENDESK_CLIENT_SECRET (+ OAUTH_ZENDESK_CLIENT_ID = zdg-MEMORA)
//     to the github.com/shrirampai3000/memora proxy env.
static OAUTH: OAuthConfig = OAuthConfig {
    auth_url: "https://{subdomain}.zendesk.com/oauth/authorizations/new",
    client_id: "zdg-MEMORA",
    extra_auth_params: &[("scope", "read write")],
    // Global OAuth clients require an HTTPS redirect URL; the github.com/shrirampai3000/memora relay
