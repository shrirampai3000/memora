...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "QuickBooks Online accounting for invoicing, time tracking, and client billing. \
        Proxy base: /connections/quickbooks/proxy/ â€” the realm_id (company ID) captured \
        during OAuth is injected automatically into every request. \
        Useful endpoints: \
        GET query?query=SELECT+*+FROM+Customer â€” list customers. \
        POST timeactivity â€” log a time entry. \
        GET invoice â€” list invoices. \
        GET companyinfo/{realmId} â€” company details.",
    fields: &[],
};
...
static OAUTH: OAuthConfig = OAuthConfig {
    ...
    // redirect â€” relayed through the website at github.com/shrirampai3000/memora/api/oauth/callback,
...
impl Integration for QuickBooks {
    ...
    fn proxy_config(&self) -> Option<&'static ProxyConfig> {
        ...
        // by the proxy handler in memora-engine/src/connections_api.rs.
    ...
    async fn test(
        ...
        let token = oauth::get_valid_token_instance(secret_store, client, "quickbooks", None)
            .await
            .ok_or_else(|| {
                anyhow!("not connected â€” connect QuickBooks Online from the Connections page in the desktop app")
            })?;
        ...
        let realm_id = stored["realmId"]
            .as_str()
            .ok_or_else(|| anyhow!("realmId missing â€” please reconnect QuickBooks Online"))?;
        ...
        Ok(format!("connected to QuickBooks Online â€” {}", name))
    }
}
