...
//! Pipe-facing integrations â€” credential storage + context for Pi.
//!
...
use memora_core::connections::sync as core_connections_sync;
...
/// Configuration for the credential proxy â€” tells the proxy how to forward
...
/// 90d) â€” for those, leaning on natural access-token expiry is enough and
...
/// against the provider's published limit (e.g. Zoom's 15h â‡’ 12h floor).
...
pub trait Integration: Send + Sync {
    ...
    /// send-only vs send+read). Default is empty â€” `extra_auth_params` is used
    ...
    /// and the server injects auth automatically â€” no secrets in the LLM context.
    ...
    /// Default `None` â€” system roots only.
    ///
    /// The proxy handler in memora-engine and the integration's own
...
fn build_client_for_with_timeouts(
    ...
    if let Some(pem) = integ.extra_root_pem() {
        match reqwest::Certificate::from_pem(pem.as_bytes()) {
            ...
            Err(e) => tracing::warn!(
                "extra_root_pem for {} failed to parse â€” falling back to system roots: {}",
                integ.def().id,
                e
            ),
        }
...
/// small standard set â€” `Authorization`, `Cookie`, `Proxy-Authorization`,
/// `WWW-Authenticate` â€” on a cross-origin hop, so a custom-header credential
/// survives one 3xx to an arbitrary host. Third-party APIs do legitimately
/// redirect (httpâ†’https, moved paths), so unlike the enterprise control-plane
...
// ---------------------------------------------------------------------------
// Credential store (~/.memora/connections.json)
// ---------------------------------------------------------------------------
...
fn store_path(MEMORA_dir: &Path) -> PathBuf {
    MEMORA_dir.join("connections.json")
}

pub fn load_store(MEMORA_dir: &Path) -> HashMap<String, SavedConnection> {
...
pub fn save_store(MEMORA_dir: &Path, data: &HashMap<String, SavedConnection>) -> Result<()> {
    let path = store_path(MEMORA_dir);
...
pub async fn load_connection(
    ...
    let file_store = load_store(MEMORA_dir);
...
pub async fn is_connection_configured(
    ...
    if integration.oauth_config().is_some() {
        ...
        load_connection(secret_store, MEMORA_dir, conn_id)
            .await
...
async fn save_connection(
    ...
    // No SecretStore (CLI without DB) â€” fall back to file
    let mut file_store = load_store(MEMORA_dir);
    file_store.insert(key.to_string(), conn.clone());
    save_store(MEMORA_dir, &file_store)
}
...
async fn remove_connection(
    ...
    // Always also clear from the legacy file â€” handles the migration case where
...
impl ConnectionManager {
    ...
    pub async fn list(&self) -> Vec<ConnectionInfo> {
        ...
        for i in &self.integrations {
            ...
            let connected = if is_oauth {
                ...
                // that way â€” including everyone who connected before the
                // integration gained OAuth â€” must not see the tile flip to off;
    ...
    pub async fn connect(&self, id: &str, creds: Map<String, Value>) -> Result<()> {
        ...
        if result.is_ok() {
            core_connections_sync::clear_connection_tombstone(&self.MEMORA_dir, id);
        }
    ...
    pub async fn get_credentials_instance(
        ...
        Ok(
            load_connection(self.secret_store.as_deref(), &self.MEMORA_dir, &key)
                .await
                .map(|c| c.credentials),
        )
    }
    ...
    pub async fn disconnect(&self, id: &str) -> Result<()> {
        let result =
            remove_connection(self.secret_store.as_deref(), &self.MEMORA_dir, id).await;
        if result.is_ok() {
            core_connections_sync::record_connection_tombstone(&self.MEMORA_dir, id);
        }
        result
    }
    ...
    pub async fn connect_instance(
        ...
        if result.is_ok() {
            core_connections_sync::clear_connection_tombstone(&self.MEMORA_dir, &key);
        }
    ...
    pub async fn get_all_instances(
        ...
        self.find(id)?;
        Ok(load_all_instances(self.secret_store.as_deref(), &self.MEMORA_dir, id).await)
    }
    ...
    pub async fn disconnect_instance(&self, id: &str, instance: Option<&str>) -> Result<()> {
        let key = make_key(id, instance);
        let result =
            remove_connection(self.secret_store.as_deref(), &self.MEMORA_dir, &key).await;
        if result.is_ok() {
            core_connections_sync::record_connection_tombstone(&self.MEMORA_dir, &key);
        }
        result
    }
...
// ---------------------------------------------------------------------------
// Pi context rendering â€” uses proxy URLs instead of raw credentials
// ---------------------------------------------------------------------------
...
pub async fn render_context(
    ...
    for integration in &integrations {
        ...
        for (instance, conn) in load_all_instances(secret_store, MEMORA_dir, def.id).await {
    ...
    let ics_settings =
        crate::ics_calendar::load_ics_calendar_settings_from_store(MEMORA_dir).ok();
    ...
    let mut out = String::from(
        "\nConnected integrations â€” use the proxy URLs below to make API calls.\n\
    ...
    for (integration, def, instance) in &cred_connected {
        ...
        if integration.webhook_proxy_config().is_some() {
            ...
            // Integration opted out of credential exposure â€” its data is
    ...
    for (integration, def, instance) in &oauth_connected {
        ...
        if integration.proxy_config().is_some() {
            ...
            // OAuth without proxy â€” still don't expose the token
            out.push_str("  (connected via OAuth â€” use the endpoints listed above; no raw token is exposed)\n");
        }
    }
...
///
/// `"notion:work"` â†’ `("notion", Some("work"))`
/// `"notion"`      â†’ `("notion", None)`
...
async fn load_all_instances(
    ...
    let store = load_store(MEMORA_dir);
