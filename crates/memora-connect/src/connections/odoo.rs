...
Step 1 (uid): POST {url}/jsonrpc with body {\"jsonrpc\":\"2.0\",\"method\":\"call\",\"params\":{\"service\":\"common\",\"method\":\"authenticate\",\"args\":[db, login, api_key, {}]}} â€” the result is the integer uid. \
...
Read example â€” search_read opportunities: args=[db, uid, api_key, \"crm.lead\", \"search_read\", [[[\"type\",\"=\",\"opportunity\"]]], {\"fields\":[\"name\",\"partner_id\",\"expected_revenue\",\"stage_id\"],\"limit\":20}]. \
Create example â€” new lead: METHOD=\"create\", positional=[{\"name\":\"...\",\"contact_name\":\"...\",\"email_from\":\"...\"}]. \
Update example â€” METHOD=\"write\", positional=[[record_id], {\"field\":\"value\"}]. \
If db is blank it defaults to the subdomain for *.odoo.com hosts.",
    fields: &[
        ...
        FieldDef {
            ...
            label: "API Key",
            secret: true,
            placeholder: "Odoo API key (Preferences â†’ Account Security â†’ New API Key)",
            help_url: "https://www.odoo.com/documentation/19.0/developer/reference/external_api.html",
        },
        FieldDef {
            key: "db",
...
/// (`acme.odoo.com` â†’ `acme`). Self-hosted instances must set the field
...
impl Integration for Odoo {
    ...
    async fn test(
        ...
        let db = creds
            ...
            .ok_or_else(|| {
                anyhow!("could not detect the Odoo database â€” fill the Database field (for Odoo Online it's the part before .odoo.com)")
            })?;
        ...
        let resp = client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("could not reach {endpoint} â€” check the Odoo URL ({e})"))?
            .error_for_status()
            .map_err(|e| anyhow!("Odoo returned an error for {endpoint} â€” check the URL ({e})"))?;

        let payload: Value = resp.json().await.map_err(|e| {
            anyhow!("unexpected (non-JSON) response from Odoo â€” check the URL ({e})")
        })?;
        ...
        match payload.get("result") {
            ...
            _ => Err(anyhow!(
                "authentication failed â€” check the login email, API key, and database"
            )),
        }
    }
}
