...
// calls â€” PostgREST, Storage, Edge Functions, Auth admin â€” work identically
// to the previous key-based flow. The Management API token is kept for refresh.
//
// Proxy auth model:
//   - PostgREST (`/rest/v1/...`)  â€” `apikey: <service_role_key>`, bypasses RLS
//   - Storage (`/storage/v1/...`) â€” same apikey header
//   - Edge Functions              â€” same apikey header
//   - GoTrue admin                â€” apikey + `Authorization: Bearer <key>` per-request
//
...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Query a Supabase project (PostgREST data API, Storage, Edge \
        ...
        to the proxy URL â€” e.g. GET /rest/v1/<table>?select=* for table \
...
impl Integration for Supabase {
    ...
    fn proxy_config(&self) -> Option<&'static ProxyConfig> {
        ...
        // `apikey` header â€” sufficient for service_role on PostgREST/Storage/
    ...
    async fn test(
        ...
        let token = oauth::get_valid_token_instance(secret_store, client, "supabase", None)
            .await
            .ok_or_else(|| anyhow!("not connected â€” use 'Connect with Supabase' button"))?;
        ...
        // PostgREST schema â€” verifies both that the host resolves AND that
