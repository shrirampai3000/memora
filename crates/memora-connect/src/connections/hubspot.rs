...
// MEMORA's HubSpot OAuth app (app id 42350513, HubSpot account 245485039),
// managed as a developer project named "MEMORA" via the HubSpot CLI.
// Redirect URI registered: http://localhost:3030/connections/oauth/callback
// client_secret lives in the github.com/shrirampai3000/memora proxy env (OAUTH_HUBSPOT_CLIENT_SECRET).
...
impl Integration for HubSpot {
    ...
    async fn test(
            ...
            oauth::get_valid_token_instance(secret_store, client, "hubspot", None).await
        {
            ...
            if t.is_empty() {
                return Err(anyhow!(
                    "not connected â€” use 'Connect with HubSpot' button or paste a Private App token"
                ));
            }
            t.to_string()
        } else {
