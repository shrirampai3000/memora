...
//! [`UserBrowser`] â€” a [`Browser`] backed by the user's real browser via
//! the MEMORA Chrome extension. Wraps a shared [`BrowserBridge`] so the
...
impl UserBrowser {
    ...
    /// Default instance â€” what the engine registers when an extension
    ...
    /// distinct instances per real browser (`user-arc`, `user-chrome`, â€¦).
    pub fn default_instance(bridge: Arc<BrowserBridge>) -> Arc<Self> {
        Self::new(
            "user-browser",
            "Your Browser",
            "The user's real browser via the MEMORA extension. Has \
             their actual logged-in sessions, cookies, passkeys â€” use this \
