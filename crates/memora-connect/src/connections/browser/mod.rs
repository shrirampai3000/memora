...
//!
//! Every "browser" â€” the user's real Chrome/Arc/Edge driven via the
//! extension, an app-managed Tauri webview with its own cookie jar, a
//! future Playwright cluster, etc. â€” implements one trait: [`Browser`].
...
//!
//! Submodules:
//! - [`bridge`] â€” protocol state for extension-driven browsers
//! - [`user`] â€” `UserBrowser`, the [`Browser`] impl that wraps a `BrowserBridge`
//! - [`owned`] â€” `OwnedBrowser`, the [`Browser`] impl that drives a Tauri-owned
//!   webview
...
// ---------------------------------------------------------------------------
// The Browser trait â€” one shape, every kind of browser
// ---------------------------------------------------------------------------
...
/// CDP to a headless Chromium, â€¦); the trait surface is the same so the
/// engine and pipes don't care which one they're talking to.
#[async_trait]
pub trait Browser: Send + Sync {
    /// Stable identifier â€” referenced in URLs (`/connections/browsers/:id/...`)
    ...
    /// which browser to call â€” e.g. "user's real Arc browser, has their
    ...
    /// substring filter â€” for browsers that have multiple tabs, the impl
    ...
    /// short timeout â€” the round-trip just confirms the JS executed,
    ...
    /// own `document.title` setters. See `crates/memora-connect`
    ...
    /// event so the embedded owned-browser sidebar â€” a singleton shared by
    /// every chat and background pipe â€” can ignore navigations that belong to a
...
impl BrowserRegistry {
    ...
    /// Add (or replace) a browser. Replacing is intentional â€” when an
...
/// What the agent sees. Keep this stable â€” it's the LLM-facing surface.
