...
//! [`OwnedBrowser`] â€” a [`Browser`] backed by an app-managed Tauri child
...
//! user browsers. Keeping the seam here means `memora-connect` never
//! pulls in a UI framework â€” handy for tests, headless deployments, and
...
// ---------------------------------------------------------------------------
// Handle trait â€” engine's contract with the desktop shell
// ---------------------------------------------------------------------------

/// One direction: engine â†’ owned webview. The desktop shell implements
...
/// types don't leak into `memora-connect`.
#[async_trait]
pub trait OwnedWebviewHandle: Send + Sync {
    ...
    /// handle" is *not* enough â€” the native child webview (or a window to host
...
impl OwnedBrowser {
    ...
    /// Default instance â€” what the engine registers at startup so the
    ...
    pub fn default_instance() -> Arc<Self> {
        Self::new(
            ...
            "An isolated app-managed webview with its own persistent cookie \
             ...
             MEMORA asks the user before copying that browser session into \
             this webview. Use this for: scraping, signups under MEMORA's \
             own accounts, scheduled background tasks, navigating to a public \
             URL the user asked you to open. \
             Navigating auto-opens the embedded sidebar in the user's chat â€” \
...
mod tests {
    ...
    /// Records the owner forwarded to `eval_with_owner` â€” the eval-with-url
