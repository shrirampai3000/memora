...
//!
//! This module owns the *protocol* â€” the pending-eval map, the request/response
//! correlation, connection lifecycle â€” but knows nothing about WebSockets or
...
//! `memora-connect`.
//!
//! See `memora-engine/src/routes/browser.rs` for the axum-flavored
...
/// Result of an `eval` round-trip â€” extension's reply, normalized.
...
pub enum EvalError {
    ...
    // owned webview), so the wording must not name a specific transport â€” an
...
// ---------------------------------------------------------------------------
// Transport abstraction â€” the engine plugs in a WebSocket impl, tests can
// plug in an in-process channel.
// ---------------------------------------------------------------------------

/// One direction: server â†’ extension. Implementors send the serialized JSON
...
impl BrowserBridge {
    ...
    /// Detach the given transport â€” but only if it's still the registered one.
    ...
    pub async fn eval(
        ...
        // Snapshot the transport â€” if we lose it after this point, the send
        ...
        if let Err(e) = transport.send_text(frame).await {
            ...
            // The transport is dead â€” clear it so /status reflects reality.
...
mod tests {
    ...
    async fn detach_only_clears_matching_transport() {
        ...
        // First's detach must NOT clear â€” second is the live one.
