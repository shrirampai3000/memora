...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Continuously sync MEMORA memories into Claude Code so it has long-term context across every session. CLAUDE.md gets a tiny MEMORA-owned marker block that `@`-imports a sibling `MEMORA-memories.md` sidecar â€” the sidecar holds the full digest and is rewritten end-to-end on each sync, while CLAUDE.md stays small and hand-editable outside the marker block. Leave home_path empty to use the default (~/.claude). For per-project memory, point home_path at a specific project's directory containing CLAUDE.md.",
    fields: &[FieldDef {
        key: "home_path",
...
mod tests {
    ...
    fn defaults_to_dot_claude_when_field_blank() {
        ...
        // current working directory â€” that's a footgun. Fall back to
    ...
    async fn test_creates_missing_directory_and_reports_ready() {
        // Target a path that doesn't exist yet â€” `test()` should create
        ...
        assert!(!target.join(".memora-write-probe").exists());
    }
}
