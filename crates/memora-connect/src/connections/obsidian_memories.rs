...
//! `memora-engine::external_memory_sync`). It mirrors how `claude_code`
...
//! can never clobber the other's credentials â€” connecting/disconnecting the
...
/// avoids surprising the user with a MEMORA file next to their notes and
/// makes it trivial to exclude from search if they want.
const DEFAULT_FOLDER: &str = "MEMORA";
...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "MEMORA continuously writes a digest of your durable memories into this Obsidian vault as a single MEMORA-owned note (<vault>/<memories_folder>/MEMORA-memories.md). The note is rewritten end-to-end on each sync and carries YAML frontmatter + #tags so it lives in Obsidian's graph. Managed by MEMORA â€” no action needed.",
    fields: &[
        FieldDef {
            key: "vault_path",
            label: "Vault Path",
...
impl Integration for ObsidianMemories {
    ...
    async fn test(
        ...
        // Guard against pointing memory sync at an arbitrary folder â€” the
        // `.obsidian` dir is the cheapest proof this is really a vault.
        if !vault.join(".obsidian").exists() {
            anyhow::bail!("no .obsidian folder found â€” are you sure this is an Obsidian vault?");
        }
        ...
        let probe = target_dir.join(".memora-write-probe");
        std::fs::write(&probe, "ok")
            .map_err(|e| anyhow::anyhow!("{} is not writable: {}", target_dir.display(), e))?;
        let _ = std::fs::remove_file(&probe);

        let note = target_dir.join("MEMORA-memories.md");
...
/// `<vault>/<memories_folder>` (folder defaults to `MEMORA`).
///
...
mod tests {
    ...
    #[test]
    fn defaults_folder_to_MEMORA() {
        let dir = resolve_target_dir(&creds(Some("/tmp/vault"), None)).unwrap();
        assert_eq!(dir, PathBuf::from("/tmp/vault/MEMORA"));
    }
    ...
    fn honors_custom_nested_folder() {
        let dir = resolve_target_dir(&creds(Some("/tmp/vault"), Some("inbox/MEMORA"))).unwrap();
        assert_eq!(dir, PathBuf::from("/tmp/vault/inbox/MEMORA"));
    }

    #[test]
    fn cannot_escape_vault_with_dotdot() {
        // `..` and leading slashes must never let the note land outside the
        // vault root â€” every traversal component is dropped.
        ...
        assert_eq!(only_dots, PathBuf::from("/tmp/vault/MEMORA"));
    }
    ...
    async fn test_creates_target_folder_and_reports_note_path() {
        ...
        assert!(vault.path().join("MEMORA").is_dir());
        assert!(msg.contains("MEMORA-memories.md"));
        // Probe must be cleaned up so the vault isn't littered with dotfiles.
        assert!(!vault
            .path()
            .join("MEMORA/.memora-write-probe")
            .exists());
    }

    #[tokio::test]
    async fn test_rejects_non_vault_directory() {
        // A directory without `.obsidian` is not a vault â€” bail rather than
