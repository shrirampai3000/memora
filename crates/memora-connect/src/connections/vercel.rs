...
// authorize URL the user's browser hits) â€” the secret lives only as
...
impl Integration for Vercel {
    ...
    async fn test(
        ...
        let token = oauth::get_valid_token_instance(secret_store, client, "vercel", None)
            .await
            .ok_or_else(|| anyhow!("not connected â€” use the 'Connect Vercel' button"))?;
        ...
        Ok(format!("connected â€” {} project(s)", count))
    }
}
