...
/// terminated by `CN=BeeCertificateAuthority` â€” a private root, not WebPKI.
...
/// Validity: 2025-08-21 â†’ 2035-08-21.
...
// ---------------------------------------------------------------------------
//
// Bee has no redirect-style OAuth and â€” post-Amazon-acquisition â€” no web
...
// here so MEMORA can offer one-click "connect with Bee" instead of asking
// the user to paste a token:
//
//   1. generate an ephemeral X25519 keypair
//   2. POST {app_id, publicKey} to the auth service â†’ {requestId, expiresAt}
...
//      `encryptedToken` â€” a NaCl crypto_box sealed to our ephemeral public key
//   5. crypto_box-open it with our secret key â†’ the Bearer JWT, which is the
...
/// Not a secret â€” it's embedded in the public npm package and only names the
...
/// `(secret_key_bytes, public_key_base64)` â€” the base64 public key is sent to
...
/// â€” the auth host is on a public CA (unlike the Bee data API's private CA).
...
/// Pure parser for the `/apps/pairing/request` JSON body â€” split out so the
...
mod tests {
    ...
    /// service emits, proving our key handling + the NaCl MAC-prepended â†’
    ...
    /// at startup with an opaque TLS error â€” pin it here.
    #[test]
    ...
    ///
    ///   cargo test -p memora-connect connections::bee::tests::live_api_smoke \
    ///     -- --ignored --nocapture
    ///
    ...
    async fn live_api_smoke() {
        ...
        if !out.status.success() {
            eprintln!("skipping: no bee-cli token in keychain â€” run `bee login` first");
            return;
        }
