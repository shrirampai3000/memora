...
const SESSION_DIR = process.env.WHATSAPP_SESSION_DIR || join(homedir(), ".MEMORA", "whatsapp-session");
const HTTP_PORT = parseInt(process.env.WHATSAPP_HTTP_PORT || "3035", 10);
mkdirSync(SESSION_DIR, { recursive: true });

// Self-terminate when parent process dies.
// Two mechanisms: (1) stdin EOF â€” fires when parent closes pipes (normal exit),
// (2) PPID polling â€” catches SIGKILL/crash where pipes aren't cleaned up.
...
let sock = null;
// In-memory contact store â€” populated from contacts.update events
const contactStore = {};
// In-memory message store â€” keeps last N messages per chat
...
// HTTP server â€” Pi curls this directly
