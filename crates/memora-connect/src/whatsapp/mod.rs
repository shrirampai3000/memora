...
async fn apply_gateway_event(
    ...
    match event {
        ...
        GatewayEvent::Disconnected { reason } => {
            warn!("whatsapp: disconnected â€” {}", reason);
            if reason == "logged_out" {
                ...
                *status.lock().await = if let Some(error) = cleanup_error {
                    WhatsAppStatus::Error {
                        message: format!(
                            "WhatsApp signed this device out, but MEMORA could not reset the old session: {}",
                            error
                        ),
                    }
                } else {
                ...
                *status.lock().await = WhatsAppStatus::Reconnecting {
                    message: "Connection interrupted. Reconnecting to WhatsAppâ€¦".to_string(),
                };
            }
        }
...
impl WhatsAppGateway {
    pub fn new(MEMORA_dir: PathBuf) -> Self {
        Self {
            MEMORA_dir,
    ...
    ///
    /// `bun_hint` is advisory â€” if it's an absolute path to an existing binary,
    ...
    async fn ensure_deps(&self, bun_path: &str) -> Result<()> {
        let session_dir = self.MEMORA_dir.join("whatsapp-session");
        std::fs::create_dir_all(&session_dir)?;

        let script_path = self.MEMORA_dir.join("whatsapp-gateway.mjs");
    ...
    async fn spawn_gateway(&self, bun_path: &str) -> Result<()> {
        ...
        // Take stdin handle and hold it â€” dropping it would close the pipe and kill the gateway.
        ...
        *self.http_port.lock().await = None;
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                // multi-line pretty-printed objects, etc.) â€” only try to parse
        ...
        tokio::spawn(async move {
            ...
            // Block forever â€” stdin handle stays open, keeping the gateway alive.
    ...
    fn spawn_watchdog(&self, my_generation: u64) {
        ...
        let MEMORA_dir = self.MEMORA_dir.clone();
        ...
        tokio::spawn(async move {
            loop {
                ...
                // Process is dead â€” should we restart?
                ...
                *status.lock().await = WhatsAppStatus::Error {
                ...
                // Re-check generation after backoff â€” may have been superseded
                if generation.load(Ordering::SeqCst) != my_generation {
                ...
                // Re-spawn
                let session_dir = MEMORA_dir.join("whatsapp-session");
                let script_path = MEMORA_dir.join("whatsapp-gateway.mjs");
...
///      `memora_core::agents::pi::find_bun_executable` â€” this is the
...
///   3. `which bun` / `where bun` PATH lookup â€” last resort, works when the
...
pub fn resolve_bun_path(hint: Option<&str>) -> Result<String> {
    ...
    if let Some(p) = memora_core::agents::pi::find_bun_executable() {
        return Ok(p);
    }
    ...
    anyhow::bail!(
        "bun binary not found. MEMORA normally ships bun bundled inside the \
...
/// resolution â€” this is the PATH-only fallback.
...
mod tests {
    ...
    /// An absolute hint pointing at an existing file is returned verbatim â€”
    /// this is the escape hatch for users who want to force a specific bun.
    #[test]
    fn hint_with_real_absolute_path_is_honored() {
        // current_exe always exists and is absolute â€” it's not bun, but the
    ...
    fn placeholder_hints_dont_short_circuit() {
        for placeholder in [Some(""), Some("bun"), Some("BUN.EXE"), Some("  "), None] {
            match resolve_bun_path(placeholder) {
                ...
                Err(_) => {
                    // Acceptable â€” bun genuinely isn't installed anywhere on
