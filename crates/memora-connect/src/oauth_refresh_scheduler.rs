...
//! provider expires the refresh token on a sliding inactivity window â€”
...
//!    not centrally â€” adding a new quirky provider is a one-line change
...
/// Access-token refresh threshold â€” fire when `expires_at` is within this
...
// ---------------------------------------------------------------------------
// Decision logic (pure â€” exposed for tests)
// ---------------------------------------------------------------------------

/// Decide whether `value` (the stored OAuth JSON) needs a refresh *now*.
///
/// Pure function â€” no I/O. The scheduler calls this with `now = unix_now()`
...
// ---------------------------------------------------------------------------
// Refresh adapter â€” injectable for tests
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Metrics â€” observable via OAuthRefreshScheduler::snapshot()
// ---------------------------------------------------------------------------
...
impl OAuthRefreshScheduler {
    ...
    pub fn start_with_runner(&self, store: Arc<SecretStore>, runner: Arc<dyn RefreshRunner>) {
        if self
            ...
            .is_err()
        {
            debug!("oauth refresh scheduler: start called while already running â€” no-op");
            return;
        }
    ...
    pub async fn stop(&self) {
        ...
        if let Some(h) = handle {
            ...
            // wedged. We've already flipped the flag â€” the OS will reap on
            // process exit.
            h.abort();
        }
    }
...
// ---------------------------------------------------------------------------
// Tick â€” the work done on each iteration
// ---------------------------------------------------------------------------
...
async fn tick(
            ...
            Err(e) => {
                ...
                if entry.consecutive >= MAX_CONSECUTIVE_FAILURES {
                    ...
                    warn!(
                        ...
                        "oauth refresh failed for {}(instance={:?}): {e:#} â€” entering cooldown",
    ...
    if attempted > 0 {
        info!(
            "oauth refresh scheduler: tick done â€” attempted={} succeeded={} failed={}",
            attempted, succeeded, failed
        );
    } else {
        debug!("oauth refresh scheduler: tick done â€” nothing due");
    }
}

// ---------------------------------------------------------------------------
// Refreshable table â€” built once from the integration registry
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    fn parse_key_instance() {
        assert_eq!(
            parse_oauth_key("oauth:google-calendar:louis@github.com/shrirampai3000/memora"),
            Some(("google-calendar", Some("louis@github.com/shrirampai3000/memora")))
        );
    }
    ...
    struct RecorderRunner {
        ...
        // Each call to refresh() pulls from `outcomes` in order â€” Ok(()) or Err.
    ...
    async fn tick_refreshes_token_whose_access_is_expiring() {
        ...
        let now = unix_now();
        // Stored token expires in 30s â€” well inside ACCESS_TOKEN_SOON_WINDOW.
        store
            .set_json(
