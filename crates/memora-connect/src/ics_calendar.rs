...
fn store_encryption_key() -> Result<[u8; 32], String> {
    match memora_secrets::keychain::get_key() {
        ...
        memora_secrets::keychain::KeyResult::AccessDenied => Err(
            "store.bin is encrypted, but keychain access was denied. Grant MEMORA keychain access or disable store encryption in the app, then try again.".to_string(),
        ),
        memora_secrets::keychain::KeyResult::NotFound => Err(
            "store.bin is encrypted, but the MEMORA keychain key was not found. Open the app once or disable store encryption, then try again.".to_string(),
        ),
        memora_secrets::keychain::KeyResult::Unavailable => Err(
...
pub fn load_ics_calendar_settings_from_store(
    ...
    let path = MEMORA_dir.join("store.bin");
