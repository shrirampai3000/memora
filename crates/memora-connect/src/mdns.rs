...
//! mDNS service advertisement and discovery for MEMORA instances on the LAN.
//!
//! Uses `_MEMORA._tcp.local.` as the service type. Each instance advertises
...
//! MEMORA continues to work. Discovery falls back to SSH/Tailscale.
...
fn get_hostname() -> String {
    ...
    cmd.output()
        ...
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "MEMORA".to_string())
}

const SERVICE_TYPE: &str = "_MEMORA._tcp.local.";
const BROWSE_TIMEOUT: Duration = Duration::from_secs(3);

/// Global daemon handle â€” kept alive for the lifetime of the process so the
...
/// `MEMORA_ENABLE_MDNS`), so MEMORA stays silent on the network unless
...
/// Advertise this MEMORA instance on the local network.
///
...
pub fn advertise(port: u16) -> Result<(), String> {
    ...
    // Instance name must be unique on the network â€” append PID to avoid
    // collisions when the same user runs MEMORA on multiple machines
    ...
    info!("mdns: advertising MEMORA on port {}", port);
    Ok(())
}

/// Discover MEMORA instances on the local network.
///
...
fn browse_blocking() -> Vec<(String, u16)> {
        ...
        if let ServiceEvent::ServiceResolved(info) = event {
            ...
            for addr in info.get_addresses() {
                let host = if addr.is_ipv6() {
                    // Skip link-local IPv6 â€” they require a scope ID and
                    // rarely work for HTTP connections across machines.
                    continue;
                } else {
                    addr.to_string()
                };
                debug!("mdns: found MEMORA at {}:{}", host, port);
    ...
    // Stop browsing â€” we only need a snapshot
    ...
    if !results.is_empty() {
        info!("mdns: discovered {} MEMORA instance(s)", results.len());
    }
...
mod tests {
    ...
    fn test_mdns_long_hostname_utf8() {
        ...
        let mut hostname = "ã“ã‚“ã«ã¡ã¯ä¸–ç•Œ".repeat(10); // 210 bytes
