...
//!   `~/.memora/mcp_servers.json` so it survives without the secret
//!   store.
//! * Header values are secret â€” stored in [`SecretStore`] under
...
pub use memora_core::pipes::connections::{parse_mcp_connection_id, MCP_CONNECTION_PREFIX};
...
fn store_path(MEMORA_dir: &Path) -> PathBuf {
    MEMORA_dir.join("mcp_servers.json")
}

pub async fn is_mcp_connection_configured(MEMORA_dir: &Path, conn_id: &str) -> bool {
    let Some(server_id) = parse_mcp_connection_id(conn_id) else {
        return false;
    };
    let Ok(file) = load_file(MEMORA_dir).await else {
        return false;
    };
    file.servers
        .iter()
...
pub async fn configured_mcp_connection_ids(MEMORA_dir: &Path) -> Result<HashSet<String>> {
    let file = load_file(MEMORA_dir).await?;
    Ok(file
        .servers
        .iter()
...
async fn load_file(MEMORA_dir: &Path) -> Result<McpServersFile> {
    let path = store_path(MEMORA_dir);
...
/// Write the servers file atomically: write â†’ sync â†’ rename.
/// A crash mid-write leaves a `.tmp` stale file rather than a corrupt live file.
async fn save_file(MEMORA_dir: &Path, file: &McpServersFile) -> Result<()> {
    let path = store_path(MEMORA_dir);
...
pub struct McpServerStore {
    ...
    /// Per-server mutexes â€” many MCP servers can't safely handle
...
impl McpServerStore {
    pub fn new(MEMORA_dir: PathBuf, secret_store: Option<Arc<SecretStore>>) -> Self {
        // Client-level timeout is the long ceiling for tool calls â€” many
    ...
    pub async fn list(&self) -> Result<Vec<McpServerConfig>> {
        Ok(load_file(&self.MEMORA_dir).await?.servers)
    }

    pub async fn get(&self, id: &str) -> Result<Option<McpServerConfig>> {
        Ok(load_file(&self.MEMORA_dir)
            .await?
            .servers
            .into_iter()
            .find(|s| s.id == id))
    ...
    /// secret store. Names without a value are skipped â€” that means
    ...
    async fn write_headers(&self, id: &str, headers: &[McpHeader]) -> Result<()> {
        let Some(ss) = &self.secret_store else {
            return Err(anyhow!(
                "secret store unavailable â€” cannot persist MCP header values"
            ));
        };
        if headers.is_empty() {
            let _ = ss.delete(&secret_key(id)).await;
    ...
    async fn start_oauth_with_config(
        ...
        let Some(ss) = &self.secret_store else {
            return Err(anyhow!(
                "secret store unavailable â€” cannot persist OAuth state"
            ));
        };
    ...
    pub async fn complete_oauth(&self, state: &str, code: &str) -> Result<String> {
        let Some(ss) = &self.secret_store else {
            return Err(anyhow!(
                "secret store unavailable â€” cannot persist OAuth token"
            ));
        };
        ...
        if let Err(e) = probe_mcp_server(&self.client, &probe_url, &probe_headers).await {
            ...
            // A failed probe does not mean the token is unusable â€” save it and let
    ...
    async fn discover_oauth(&self, cfg: &McpServerConfig) -> Result<DiscoveredOAuth> {
        if let Some(oauth) = cfg.oauth.as_ref() {
            if !oauth.auth_url.trim().is_empty() && !oauth.token_url.trim().is_empty() {
                // Manual config: don't assume RFC 8707 resource binding â€” the server
        ...
        // full path URL ("https://mcp.notion.com/mcp") â†’ audience mismatch â†’ 401
...
/// RFC 7636 Â§4.1 PKCE code verifier: 32 random bytes (256 bits of entropy)
/// rendered as base64url-no-pad â†’ exactly 43 chars, safely inside the spec's
/// 43..=128 length bound and the unreserved character set.
///
/// Note: do NOT use [`random_url_token`] here â€” it base64-encodes a 128-char
...
fn protected_resource_metadata_url(resource: &str) -> Result<Vec<String>> {
    ...
    // Origin-based URL first â€” this is what Notion's guide and RFC 9470 recommend:
    ...
    // Path-appended variant (append /.well-known/â€¦ to the server's own path).
...
// simple â€” no persistent process lifecycle, no orphan reaping, no
...
fn registry_path() -> &'static str {
    ...
    CACHED.get_or_init(|| {
        ...
        // console program â€” without the flag the app pops a black terminal over
        // the user's desktop the first time any stdio MCP server is reached.
        let out = memora_core::no_window_command("powershell")
...
struct StdioSession {
    ...
    /// Kept alive via kill_on_drop â€” dropped (and process killed) when session is dropped.
...
impl StdioSession {
    async fn spawn(command: &str, args: &[String], env: &HashMap<String, String>) -> Result<Self> {
        ...
        // that can't be spawned directly â€” they require cmd.exe as the
    ...
    async fn send(&mut self, method: &str, params: Value, timeout: Duration) -> Result<Value> {
        ...
        match tokio::time::timeout(timeout, async move {
            loop {
                ...
                // Notification or response for a different id â€” ignore.
            }
        })
        .await
        {
...
impl Drop for StdioSession {
    fn drop(&mut self) {
        ...
        let _ = memora_core::no_window_command("taskkill")
    ...
    if let Err(e) = session
        .send(
            ...
            json!({
                ...
                "capabilities": {},
                "clientInfo": { "name": "MEMORA", "version": env!("CARGO_PKG_VERSION") },
            }),
            probe_timeout,
        )
        .await
...
// back (MCP Streamable HTTP Â§Session Management). We capture it here and
...
async fn probe_mcp_server(
    ...
    // Probes are interactive â€” the user is waiting in the settings
    // dialog. Cap each step short to fail loud rather than spin.
    let probe_timeout = Duration::from_secs(20);

    // Step 1 â€” initialize. Capture Mcp-Session-Id for subsequent requests.
    ...
    // Step 2 â€” tools/list, paginated. MCP's `tools/list` may return
...
async fn call_mcp_tool(
    ...
    // tools/call uses the client-level ceiling (5 min) â€” real MCP
...
/// Streamable HTTP Â§Session Management).
...
fn parse_sse_data(text: &str, expected_id: &str) -> Result<Value> {
    for line in text.lines() {
        ...
        // frames â€” accept the first non-notification frame regardless.
...
pub async fn render_context(MEMORA_dir: &Path, api_port: u16) -> String {
    render_context_for_ids(MEMORA_dir, api_port, None).await
}
...
pub async fn render_context_for_connections(
    ...
    let ids: Vec<String> = connections
        ...
        .collect();
    render_context_for_ids(MEMORA_dir, api_port, Some(&ids)).await
}
...
pub async fn render_context_for_ids(
    ...
    let file = match load_file(MEMORA_dir).await {
        Ok(f) => f,
        Err(e) => {
    ...
    let mut out = String::from(
        "\nUser-registered MCP servers â€” invoke their tools via the `sp_mcp_call` and `sp_mcp_list_tools` bridge tools.\n\
...
mod tests {
    ...
    // These guard against regressions in the wire protocol â€” they're
    ...
    async fn probe_against_mock_returns_tools() {
        ...
        // Generic responder â€” wiremock can't introspect the JSON-RPC
    ...
    /// Custom wiremock matcher â€” wiremock has body_string_contains
    ...
    fn pkce_verifier_is_rfc7636_compliant() {
        // RFC 7636 Â§4.1: verifier is 43..=128 chars from the unreserved set.
        // The previous code reused random_url_token(), which base64-encodes a
        // 128-char hex string â†’ a 171-char verifier that strict token
    ...
    async fn concurrent_calls_to_same_server_serialise() {
        ...
        // wiremock with a deliberate delay â€” if both calls overlap we
        ...
        // fully serialised â€” that's the regression we actually care
    ...
    async fn concurrent_calls_to_different_servers_do_not_block_each_other() {
        ...
        // each delayed 300ms by wiremock â€” so a single call_tool
        // takes ~600ms minimum. With per-server locks the two
        // call_tools overlap and we see ~600ms total. With a global
        // lock they'd serialise â†’ ~1200ms. Cap at 1000ms gives clean
    ...
    // fail to match â†’ 404 â†’ complete_oauth errors.
    #[tokio::test]
    async fn confidential_client_sends_basic_auth_on_token_exchange() {
        ...
        // Protected-resource metadata â†’ this server is its own AS.
        ...
        // `none` â€” exactly what Krisp does.
        Mock::given(method("POST"))
            .and(path("/register"))
