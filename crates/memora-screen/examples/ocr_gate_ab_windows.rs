...
//!
//! The canonical A/B harness (`memora-capture/examples/ocr_gate_ab.rs`) is
//! macOS-only â€” it drives Apple Vision. This is its Windows sibling: it runs the
...
//!
//! - **baseline** â€” full-frame OCR on every tick. This is the pre-#5054
//!   behavior: every gated capture paid a full-frame OCR pass.
//! - **optimized** â€” the production gate mechanics (#5060): detect text regions
//!   â†’ crop to the padded union of the detected text â†’ pixel-signature that crop
//!   â†’ OCR it **only when the signature changed** since the last indexed crop.
//!   Identical crop â†’ skip (0 OCR). This mirrors `memora_capture::OcrGate` +
...
//!   conservative floor on the savings â€” detection runs on the whole frame here).
//!
//! Two input modes:
//!
//! ```sh
//! # LIVE: capture the default monitor N times and A/B each frame.
//! cargo run --release -p memora-screen --example ocr_gate_ab_windows -- \
...
//! # simulate an unchanged tick â€” that is where the gate's skip shows up).
//! cargo run --release -p memora-screen --example ocr_gate_ab_windows -- \
...
mod win {
    ...
    use memora_screen::monitor::{get_default_monitor, get_monitor_by_id};
    use memora_screen::perform_ocr_windows;
    ...
    /// The optimized arm's gate â€” a faithful inline mirror of
    /// `memora_capture::OcrGate` for a single capture stream: remember the
    ...
    async fn ab_tick(
        ...
        // --- OPTIMIZED arm: detect â†’ union crop â†’ signature â†’ gate. ---
