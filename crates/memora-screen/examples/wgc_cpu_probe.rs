...
//! core:
//!
//!   cargo run --release -p memora-screen --example wgc_cpu_probe -- \
//!       --duration 20 --interval-ms 500
//!
//! To compare implementations of `wgc_capture.rs` (the probe only touches its public
//! API), build the probe against each variant â€” e.g. stash/checkout the version under
//! test while keeping this file (it does not exist on pre-#4840 branches) â€” and run
//! both builds.
//!
...
#[cfg(target_os = "windows")]
mod windows_main {
    use memora_screen::wgc_capture::PersistentCapture;
    use std::str::FromStr;
    ...
    pub fn run() {
        ...
        let monitor_id = monitor_id.unwrap_or_else(|| {
            ...
            rt.block_on(async {
                memora_screen::monitor::get_default_monitor()
                    .await
