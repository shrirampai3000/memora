...
mod tests {
    use chrono::Utc;
    use memora_screen::capture_screenshot_by_window::{CapturedWindow, WindowFilters};
    ...
    // crate actually exposes them (see memora-screen/src/lib.rs which
    // re-exports `RealtimeVisionEvent` from `core` but not the helpers).
    use memora_screen::core::{continuous_capture, process_ocr_task, RawCaptureResult};
    use memora_screen::monitor::get_default_monitor;
    use memora_screen::ocr_cache::WindowOcrCache;
    use memora_screen::{OcrEngine, PipelineMetrics};
    use std::sync::Arc;
