...
static SKIP_APPS: Lazy<HashSet<&'static str>> = Lazy::new(|| {
    HashSet::from([
        ...
        "TheBoringNotch",
        "Grammarly Desktop",
        // MEMORA's own UI should never be captured
        "MEMORA",
        "MEMORA - Development",
        "MEMORA beta",
    ])
...
impl WindowFilters {
    ...
    /// Apps that are always excluded â€” system lock screen processes.
    const BUILTIN_IGNORED: &'static [&'static str] = &["loginwindow", "logonui"];

    /// True when no user-configured filters exist â€” resolving exclusion ids
    ...
    /// `AppName::WindowTitle` scope (parsed in `memora-core::window_pattern`);
    ...
    pub fn is_valid(&self, app_name: &str, title: &str) -> bool {
        ...
        // Check ignore list first â€” always reject ignored windows
    ...
    ///
    /// Delegates to [`memora_a11y::url_filter::is_url_blocked`] so the
    /// vision path and the a11y tree walker agree on what "blocked" means.
    pub fn is_url_blocked(&self, url: &str) -> bool {
        memora_a11y::url_filter::is_url_blocked(url, &self.ignored_urls)
    }
...
fn get_frontmost_pid() -> Option<i32> {
    // Wrap in autorelease pool â€” running_apps() returns autoreleased
...
/// Skips overlay windows (layer > 0), system apps, and MEMORA's own UI.
#[cfg(target_os = "macos")]
pub fn find_topmost_pid_on_monitor(
    ...
    for w in cg_windows {
        ...
        // Skip tiny windows (< 100x100 â€” status icons, tracking areas)
        ...
        // Skip MEMORA's own UI
        if w.owner_name.to_lowercase().contains("MEMORA") {
            continue;
        }
...
/// Non-macOS stub â€” SCK exclusion is not available.
#[cfg(not(target_os = "macos"))]
...
pub async fn capture_all_visible_windows(
    ...
    // topmost visible window on THIS monitor â€” not the system-wide "focused" app.
    ...
    for window_data in windows_data {
        ...
        // it's a floating overlay like Wispr Flow â€” don't treat as focused.
        #[cfg(target_os = "macos")]
        ...
        // Safety-net: always exclude MEMORA's own UI regardless of exact app name variant
        let is_MEMORA_ui = app_name.to_lowercase().contains("MEMORA");
        let is_valid = !is_MEMORA_ui
...
mod tests {
    ...
    fn test_is_valid_with_ignore_list() {
        ...
        // Chrome matches include, "Google" not ignored â†’ valid
        assert!(filters_with_include.is_valid("Chrome", "Google"));
        // "Private" is in ignore list â†’ rejected even though Chrome is in include list
        ...
        // Firefox doesn't match include, and doesn't match ignore â†’ still rejected (not in include list)
    ...
    fn test_is_valid_ignore_takes_precedence_over_include() {
        ...
        // Wispr Flow is in both ignore and include â€” ignore wins
        assert!(!filters.is_valid("Wispr Flow", "Status"));
        // Arc is only in include â€” passes
        assert!(filters.is_valid("Arc", "GitHub"));
    }
    ...
    fn test_is_valid_scoped_ignore_per_window() {
        ...
        // core scoped-pattern contract â€” guards against the helper getting
    ...
    fn test_is_valid_scoped_ignore_app_only_form() {
        // `Slack::` is equivalent to legacy `Slack` â€” blocks the whole app.
    ...
    fn test_is_valid_scoped_include_per_app_whitelist() {
        ...
        // Regression target: naÃ¯ve include semantics would block Slack/Chrome.
    ...
    fn test_is_valid_mixed_legacy_and_scoped_includes() {
        ...
        // Non-Slack apps fall back to the legacy `Slack` global â€” they don't match.
        assert!(!filters.is_valid("Chrome", "google docs"));
    }
    ...
    fn test_rect_no_overlap_right() {
        ...
        // Touching edges â€” no overlap (right edge of a == left edge of b)
        assert!(!a.overlaps(&b));
    }
    ...
    mod topmost_tests {
        ...
        fn test_topmost_skips_overlay_layers() {
            ...
            let windows = vec![
                ...
                make_window(100, 0, 0, 0, 1920, 1080, "WezTerm", "Ï€ - brain"),
            ];
            assert_eq!(
                find_topmost_pid_on_monitor(&windows, &monitor_1080p()),
                Some(100)
        ...
        fn test_topmost_skips_system_apps() {
            let windows = vec![
                ...
                make_window(100, 0, 0, 25, 1920, 1055, "WezTerm", "Ï€ - brain"),
            ];
            assert_eq!(
                find_topmost_pid_on_monitor(&windows, &monitor_1080p()),
                Some(100)
        ...
        #[test]
        fn test_topmost_skips_MEMORA_ui() {
            let windows = vec![
                make_window(999, 0, 0, 0, 1920, 1080, "MEMORA", "Main"),
                make_window(100, 0, 0, 0, 1920, 1080, "WezTerm", "Ï€ - brain"),
            ];
            assert_eq!(
                find_topmost_pid_on_monitor(&windows, &monitor_1080p()),
        ...
        #[test]
        fn test_topmost_skips_MEMORA_dev() {
            let windows = vec![
                make_window(
                    999,
                    0,
        ...
        fn test_topmost_small_dialog_on_top() {
            // Save dialog on top of large app â€” dialog IS what user sees
            let windows = vec![
        ...
        fn test_topmost_monitor_with_negative_coords() {
            ...
            let windows = vec![
                ...
                make_window(200, 0, 0, 0, 1920, 1080, "WezTerm", "Ï€ - brain"),
            ];
            assert_eq!(
        ...
        fn test_topmost_window_partially_on_monitor() {
            // Window straddles two monitors â€” should match on both
            let monitor1 = Rect {
                x: 0,
                y: 0,
                width: 1920,
