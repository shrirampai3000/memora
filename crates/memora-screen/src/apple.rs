        ...
        if let Some(results) = request.results() {
            if !results.is_empty() {
                ...
                results.iter().for_each(|result| {
                    ...
                    // Rust byte offsets â€” walk chars summing len_utf16() to build the offsets.
...
mod tests {
    ...
    fn cjk_no_whitespace_collapses_to_one_range() {
        // No whitespace in CJK strings â†’ single range covering the whole string.
        // Each Han char is 1 UTF-16 code unit (BMP), so utf16_len == char count.
        let r = utf16_word_ranges("ä½ å¥½ä¸–ç•Œ");
        assert_eq!(r.len(), 1);
        assert_eq!(r[0].0, 0);
        assert_eq!(r[0].1, 4);
        assert_eq!(r[0].2, "ä½ å¥½ä¸–ç•Œ");
    }

    #[test]
    fn supplementary_chars_count_as_two_utf16_units() {
        // Emoji ðŸŽ‰ (U+1F389) lives outside the BMP and takes 2 UTF-16 code units.
        let r = utf16_word_ranges("a ðŸŽ‰ b");
        assert_eq!(r.len(), 3);
        assert_eq!(r[0], (0, 1, "a".to_string()));
        // ðŸŽ‰ starts at utf16 offset 2, length 2
        assert_eq!(r[1].0, 2);
        assert_eq!(r[1].1, 2);
    ...
    /// (user `ingvar2424`, MEMORA v2.5.79 at 19.6 GB RSS after weeks of
    /// idle background running on DEFAULT settings â†’ ~1 GB/day steady growth).
    ///
    ...
    /// permission** â€” Vision runs on an in-memory image â€” so it reproduces in
    /// CI / headless. Kept `#[ignore]` (perf/memory repro, not correctness):
    ///   cargo test -p memora-screen --lib apple::tests::repro_apple_ocr_leak -- --ignored --nocapture
    #[cfg(target_os = "macos")]
    #[test]
    ...
    fn repro_apple_ocr_leak() {
        ...
        // Diagnostic only â€” we want the printed curve, not a hard assertion that
