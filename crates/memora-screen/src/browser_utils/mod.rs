...
pub trait BrowserUrlDetector {
    ...
    /// by screen capture to attribute the frame being captured â€” must not
    ...
    ///
    /// Used by the meeting probe to subtract full windows â€” whose active-tab
    /// URL `get_window_urls` already resolved â€” from AX-title fallbacks that
...
mod tests {
    ...
    fn test_strip_emoji_badge() {
        assert_eq!(
            strip_title_badge("ðŸ’¬1 - MEMORA | Discord"),
            "MEMORA | Discord"
        );
    }
    ...
    fn test_no_badge_passthrough() {
        ...
        assert_eq!(
            strip_title_badge("GitHub - MEMORA/MEMORA"),
            "GitHub - MEMORA/MEMORA"
        );
    }
    ...
    fn test_substring_match() {
        ...
        assert!(titles_match(
            "GitHub - MEMORA/MEMORA: AI powered",
            "GitHub - MEMORA/MEMORA: AI powered by what you see"
        ));
    }
    ...
    fn test_real_desync_cases_from_data() {
        // These are real desyncs found in MEMORA data.
