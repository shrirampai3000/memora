...
/// so 17 Ã— 1s worst case still exceeds the caller's 3s tokio timeout â€” but
...
impl MacOSUrlDetector {
    ...
    ///
    /// `window` is borrowed â€” the caller keeps ownership of the AX element.
    ...
    unsafe fn find_url_in_tree(&self, element: AXUIElementRef) -> Option<String> {
        ...
        if status == accessibility_sys::kAXErrorSuccess && !role.is_null() {
            // AXUIElementCopyAttributeValue returns +1 retained â€” use create_rule
            ...
            if role_str == kAXTextFieldRole {
                ...
                if status == accessibility_sys::kAXErrorSuccess && !value.is_null() {
                    // +1 retained â€” use create_rule so it's released on drop
                ...
                // If CopyAttributeValue failed, value is still null â€” nothing to release
            }
        }
        // If CopyAttributeValue failed for role, role is still null â€” nothing to release
        ...
        if status == accessibility_sys::kAXErrorSuccess && !children.is_null() {
            // +1 retained â€” use create_rule so the array is released on drop
            let children_array =
    ...
    fn get_url_via_accessibility(&self, process_id: i32) -> Result<Option<String>> {
        unsafe {
            // First try AXDocument â€” returns the loaded page URL, not address bar text.
            ...
            // find_url_in_tree returns the URL string directly â€”
    ...
    /// Little Arc mini windows are NOT in the `windows` collection â€” they have
    ...
    /// the `windows` collection â€” so this is exactly the set of full windows
    ...
    /// AXDocument-only â€” a tree walk per window would multiply the dominant
    ...
    fn get_window_urls_via_accessibility(&self, process_id: i32) -> Result<Vec<String>> {
        ...
        unsafe {
            ...
            if status == accessibility_sys::kAXErrorSuccess && !windows_ref.is_null() {
                // +1 retained â€” create_rule releases the array on drop
                let windows =
...
impl BrowserUrlDetector for MacOSUrlDetector {
    ...
    /// Other browsers keep the default empty answer â€” the meeting probe only
