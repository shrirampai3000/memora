...
//! (`memora-a11y`); OCR here is the fallback for pixels with no tree.
//!
//! - `monitor`, `capture_screenshot_by_window`, `wgc_capture` â€” grabbing pixels
//!   per platform. macOS goes through ScreenCaptureKit, Windows through WGC.
//! - `apple`, `microsoft`, `tesseract`, `custom_ocr` â€” the OCR backends. Pick
//!   via `utils::OcrEngine`; the native one is the default on each platform.
//! - `frame_comparison`, `ocr_cache`, `text_regions` â€” skipping work when the
//!   screen did not meaningfully change. This is what keeps idle cost near zero.
//! - `snapshot_writer` â€” writing frames out for the recorder.
//!
...
/// stream teardown happens in a safe context â€” not inside the C callback.
#[cfg(target_os = "macos")]
