...
/// `memora_capture::ocr_gate::OcrDecision` (that crate depends on this
...
/// (â‰¤10s), yet production still shows multi-minute gone-silent freezes that
...
/// stack. `/health` therefore could not tell "MEMORA turned pixels off on
...
/// All counters use relaxed ordering â€” we care about approximate accuracy, not exact sequencing.
#[derive(Debug)]
pub struct PipelineMetrics {
    ...
    /// OCR runs that yielded (near-)empty text â€” an OCR-quality failure proxy.
    ...
    /// fast path â€” `skips / (skips + crop_ocr)` is the ratio that
    ...
    /// Frames dropped (OCR done but not written to DB â€” e.g. tracker miss)
    ...
    // --- Timestamps ---
    /// Unix timestamp (secs) of most recent DB write â€” used by health check to avoid DB queries
    ...
    /// attempt and write â€” `attempts - captured - dedup_skips` over a window that should
    /// not be growing is the leak signal.
    pub capture_attempts: AtomicU64,
    /// Total number of dedup skips â€” capture cycle ran but content matched a previous
...
impl PipelineMetrics {
    ...
    /// frozen loop never reaches. Two relaxed stores per stage â€” the loop
    ...
    /// Record that an OCR run produced (near-)empty text â€” an OCR-quality
    ...
    /// A Skip means no OCR ran â€” the saving the gate exists to make.
    ...
    /// previous one â€” typical for a static screen, video call, slide
    /// deck). Advances `last_db_write_ts` so the health-check stall
    /// detector treats "nothing new to write" as healthy. Does NOT bump
    /// the `frames_db_written` counter or record latency â€” those still
    ...
    /// running fine; on Louis's machine that produced 8â€“14 false
    ...
    /// Record a frame skipped because it was corrupt â€” near-all-black
    ...
    /// health-check stall alarm â€” the pipeline is running fine, there's just
    ...
    pub fn record_db_write(&self, latency: std::time::Duration) {
        ...
        // Record first frame time (only once â€” compare-and-swap from 0)
        if count == 0 {
    ...
    /// Record a dropped frame (generic â€” prefer the categorized variants
    ...
    /// (exceeded the 15s budget â€” typically a stuck DB write / saturated
    ...
    /// Seconds since the pipeline started â€” used by the coverage sampler's
    ...
    pub fn snapshot(&self) -> MetricsSnapshot {
        ...
        // `silent_loss` is therefore the RESIDUAL â€” attempts we can't attribute
        ...
        // a dedup), so it must be subtracted here too â€” otherwise every skipped
        ...
        MetricsSnapshot {
            ...
            avg_db_latency_ms: {
                ...
                // accumulator â€” prevents early spikes from permanently inflating the metric.
                let rolling = self
                    .db_latency_window
                    .lock()
                    .map(|w| w.avg_ms())
...
pub struct MetricsSnapshot {
    ...
    /// OCR-gate captures resolved without OCR â€” the fast path.
...
mod tests {
    ...
    fn silent_loss_is_unaccounted_residual_and_drop_rate_is_real() {
        ...
        // 10 accounted. The 11th attempt recorded NO outcome â€” simulating an
    ...
    fn fully_accounted_pipeline_has_zero_residual() {
        let m = PipelineMetrics::new();
        // Every attempt resolves to a known outcome â†’ residual is 0.
        for _ in 0..10 {
            m.record_capture_attempt();
        }
        for _ in 0..6 {
    ...
    /// watchdog silently reports "unknown" for the new stage â€” the exact
