...
//! full-buffer clone) *every* compositor frame delivered â€” up to display refresh rate â€”
//! even though MEMORA only samples a frame every couple of seconds. That is a
//! constant ~0.5 core of waste on every Windows machine, regardless of motion.
//! See https://github.com/MEMORA/MEMORA/issues/4840.
//!
//! Design:
...
//!   reinits â€” a closed session never serves stale frames.
...
/// Best-effort â€” `SetMinUpdateInterval` needs Win11 24H2+ and is a no-op if unsupported,
...
/// first creation panicked â€” creation errors here propagate as `Result` instead, so
...
impl D3dContext {
    fn create() -> Result<Self> {
        ...
        // critical section so those uses can't race our context calls â€” the
...
/// usage/CPU access (bind/misc flags cleared) â€” used for both the GPU-side "latest"
...
pub struct PersistentCapture {
    ...
    /// `get_latest_image` â€” a closed session errors out (triggering reinit in
...
impl PersistentCapture {
    ...
    pub fn new(monitor_id: u32) -> Result<Self> {
        ...
        tracing::info!("START â†’ persistent WGC capture started for monitor {}", monitor_id);
    ...
    /// Keep only the latest GPU texture â€” no CPU readback, no allocation beyond the
    ...
    fn store_frame(
        ...
        if frame_size != slot.pool_size {
            // Frame from a buffer generation before the last Recreate â€” drop it.
            return Ok(false);
        }
    ...
    /// BGRA->RGBA swizzle happen here, lazily, only when a frame is actually needed â€”
    ...
    fn readback(&self, texture: &ID3D11Texture2D, width: u32, height: u32) -> Result<DynamicImage> {
        ...
        } // staging + context locks released â€” FrameArrived (all monitors) unblocked
    ...
    /// Stop the persistent capture session. Idempotent â€” `Drop` calls it again after
    ...
    pub fn stop(&mut self) {
        ...
        tracing::info!("STOP â†’ persistent WGC capture stopped");
    }
...
mod tests {
    ...
    /// find_hmonitor re-derives (xcap Monitor::id() == HMONITOR value as u32) â€”
    ...
    /// Needs a live, unlocked desktop session â€” same requirement as the rest of this
    ...
    /// After stop() (or the item's Closed event â€” same flag and cleanup), the session
    ...
    fn stop_fails_fast_and_is_idempotent() {
        ...
        // Second explicit stop is a no-op (Drop adds a third) â€” must not panic or
