...
impl SafeMonitor {
    ...
    // *mut c_void (HANDLEs) which are not Send â€” can't move into spawn_blocking.
    ...
    async fn capture_image_inner(&self, streaming: bool) -> Result<DynamicImage> {
        ...
        let result = tokio::task::spawn_blocking(move || -> Result<DynamicImage> {
                    ...
                    match image {
                        ...
                        Err(e) => {
                            if !capture.is_closed() {
                                // If the session is alive (not closed by OS/device lost), a timeout
                                // on a static screen is expected because WGC only emits frames on change.
                                // Fallback to reading the cached GPU texture instead of tearing down the session.
                                if let Ok(cached) = capture.get_latest_image_streaming(std::time::Duration::from_millis(50)) {
                                    persistent_failures.store(0, Ordering::Relaxed);
                                    return Ok(cached);
                                }
                            }
                            tracing::info!(
                                "RESTART â†’ persistent WGC capture restarting for monitor {} (reason: persistent session failed: {}, closed={})",
                                monitor_id,
                                e,
                                capture.is_closed()
                            );
            ...
            match crate::wgc_capture::PersistentCapture::new(monitor_id) {
                Ok(mut capture) => {
                    // First frame â€” allow longer timeout for WGC to deliver
                    let image = if streaming {
    ...
    pub async fn refresh(&mut self) -> Result<()> {
        ...
        // Invalidate cached index â€” monitor list may have changed
