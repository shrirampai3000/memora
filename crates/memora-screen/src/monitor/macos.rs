...
/// Coupling these means MEMORA captures exactly the pixels it stores â€”
...
impl SafeMonitor {
    ...
    /// Call this after a capture failure â€” the monitor may have been reconnected
...
/// we'd attempt to capture a sleeping display every frame â€” wasted cycles
...
fn is_clamshell_inactive_builtin(display_id: u32) -> bool {
    memora_core::display_topology::is_clamshell_inactive_builtin(display_id)
}
    ...
    F: FnOnce() -> std::result::Result<T, MonitorListError> + Send + 'static,
{
    ...
    // N * timeout. That is the #3939 shape â€” a 250ms-bounded await in the
    ...
    let permit = workers.try_acquire_owned().map_err(|e| match e {
        tokio::sync::TryAcquireError::NoPermits => MonitorListError::Other(
            "ScreenCaptureKit monitor enumeration retry budget exhausted; two Apple callbacks remain blocked, restart MEMORA to recover"
                .to_string(),
        ),
...
fn enumerate_sck_monitors() -> std::result::Result<Vec<SafeMonitor>, MonitorListError> {
    #[cfg(debug_assertions)]
    if std::env::var("MEMORA_E2E_SEED")
        .ok()
        .is_some_and(|seeds| {
            seeds
                .split(',')
...
/// detection is unchanged â€” it runs on that fresh path, never this one â€” and
...
/// authoritative signals â€” the CoreGraphics display-reconfiguration callback,
/// and wake/unlock â€” and both are exactly when a cached list stops describing
...
impl SafeMonitor {
    ...
    /// path. `excluded_window_ids` are excluded at the OS level â€” ignored /
...
mod tests {
    ...
    fn topology_cache_outlives_the_lookup_ttl_then_expires() {
        ...
        // path is still happy â€” that difference is the whole point of the
    ...
    async fn enumeration_wait_stays_bounded_when_callers_queue() {
        ...
        assert!(
            ...
            "queued caller waited {worst:?} for a {TIMEOUT:?} bounded call \
             ({CALLERS} callers, ceiling {ceiling:?}) â€” the serializer wait is unbounded, \
    ...
    /// (user's MEMORA at 13.2 GB RSS after ~48 h).
    ///
    ...
    /// test â€” run with:
    ///   cargo test -p memora-screen --lib monitor::macos::tests::repro_list_monitors_autorelease_leak -- --ignored --nocapture
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    ...
    async fn repro_list_monitors_autorelease_leak() {
        ...
        // 5 k calls x ~2.4 kB/call ~= 12 MB growth â€” enough to rise cleanly
        ...
        for _ in 0..N {
            tokio::task::spawn_blocking(|| {
                // No ar_pool â€” matches current production path in
        ...
        // so phase 2 cannot shrink below phase 1 â€” we check that phase 2
        ...
        assert!(
            delta1 > 2 * 1024 * 1024,
            "expected >2 MB growth without ar_pool; got only {} â€” leak hypothesis not confirmed",
            fmt_mb(delta1)
        );
        assert!(
            delta1 > 3 * delta2,
