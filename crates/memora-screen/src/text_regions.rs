...
//! grayscale (BT.601, cv2 fixed-point) â†’ 3x3 morphological gradient â†’ Otsu
//! threshold â†’ 9x1 morphological close (merges glyph strokes into word/line
//! blobs) â†’ connected components â†’ bounding boxes filtered to text-like
...
//! machine â€” a three-OS bundling project for four textbook raster ops). The
...
//! boxes are pixel-exact, with one known topological difference â€” cv2's
...
/// text. Purely geometric â€” no character recognition â€” so false positives on
...
/// density gate treats "many boxes" as dense â†’ full-frame OCR (today's
...
pub fn detect_text_regions(image: &DynamicImage) -> Vec<TextRegion> {
    ...
    // Otsu threshold â†’ binary (1 = candidate text stroke edge).
    ...
    // Connected components (8-connectivity) â†’ bounding boxes â†’ text filter.
...
/// visually identical â€” any text edit, moved window, or new caption flips
...
/// OCR result we already stored?" â€” deliberately heuristic-free (#5060
...
///
/// Cost: one BT.601 luma pass + hash, `O(pixels)` â€” ~1-4ms on a window
...
/// identical content â€” moved boxes, in-place digit flips, and new text all
...
/// signal â€” region scoping made the skip only as good as the region
...
/// ((R*4899 + G*9617 + B*1868 + 8192) >> 14) so the Otsu histogram â€” and
/// therefore the binary image â€” matches the benchmarked cv2 reference.
...
fn to_gray_bt601(image: &DynamicImage) -> Vec<u8> {
    ...
    // Fast paths avoid `to_rgb8()`'s full-frame conversion copy â€” screen
...
/// white within Â±4 columns) then horizontal erode (all white within Â±4,
/// where out-of-bounds counts as white â€” cv2 +inf erode border). Bridges
...
mod tests {
    ...
    fn draw_text_like_line(img: &mut RgbImage, x0: u32, y0: u32, strokes: u32) {
        for s in 0..strokes {
            let sx = x0 + s * 7; // 3px stroke + 4px gap â†’ bridged by close
            for dy in 0..12 {
                for dx in 0..3 {
    ...
    fn tall_thin_bar_is_rejected_by_aspect_filter() {
        let mut canvas = light_canvas(400, 300);
        // 4px wide, 100px tall: aspect 0.04 â€” not text-shaped.
        for dy in 0..100 {
            for dx in 0..4 {
    ...
    fn image_signature_exact_on_identity_sensitive_to_any_change() {
        ...
        // Identical pixels â†’ identical signature.
        assert_eq!(
            s_a,
            image_pixel_signature(&DynamicImage::ImageRgb8(a.clone()))
        );

        // ANY visible change flips it â€” including one outside anything a
    ...
    fn pixel_signature_exact_on_identity_sensitive_to_content_and_position() {
        ...
        // Identical pixels + regions â†’ identical signature.
        ...
        // strokes across the same span) â†’ different signature. This is the
        ...
        // Same content at a different position â†’ different signature (the
        ...
        // Sub-quantum luma noise (Â±2 on a 32-level quantization) does not
        // flip the signature â€” tolerance for capture-pipeline jitter.
        let mut noisy = a.clone();
        for px in noisy.pixels_mut() {
            px.0 = [px.0[0].saturating_add(2), px.0[1], px.0[2]];
        }
    ...
    fn otsu_separates_bimodal_histogram() {
        // Half 40s, half 200s â†’ threshold lands between the modes.
