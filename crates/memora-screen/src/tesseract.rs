...
use memora_core::{Language, TESSERACT_LANGUAGES};
...
/// `MEMORA` binary, because â€” unlike the `.deb` (which `depends` on
/// `tesseract-ocr`) â€” it has no package manager to pull one in, and hosts
...
/// AppImage paths, or a dev build) â€” those fall back to `ensure_tessdata_prefix`.
...
/// (command.rs:108) â€” when no binary is found it PANICS ("called `Option::unwrap()`
/// on a `None` value") instead of returning Err, unwinding the tokio capture
/// worker (MEMORA-CLI-V3 / CLI-T0: Linux/npx hosts with no system tesseract
/// and no bundled one). #4564's `catch_unwind` stops the crash, but the panic
/// hook still fires on every frame â†’ continued Sentry noise plus stack-unwind +
/// backtrace cost ~30Ã—/sec for the whole session. Pre-flighting the *same*
        ...
        if !found {
            warn!(
                "tesseract binary not found (no system install and none bundled next to \
                 the engine); OCR disabled for this session â€” install tesseract-ocr to \
...
pub fn perform_ocr_tesseract(
    ...
    // No tesseract binary â†’ skip OCR instead of letting rusty-tesseract panic on
    // every frame (MEMORA-CLI-V3 / CLI-T0). Returns the same empty sentinel
    ...
    // an empty OCR result instead of an unwind â€” the established pattern for
    // panicky deps (fbank #4159, ort #3290). MEMORA-CLI-V3 / CLI-T0.
    ...
    let data_output = match ocr_result {
        ...
        Err(_) => {
            warn!(
                "tesseract: OCR panicked (tesseract binary missing or misbehaving) â€” skipping frame"
            );
            return (String::new(), "[]".to_string(), None);
        }
    };
...
mod tests {
    ...
    // MEMORA-CLI-V3 / CLI-T0: rusty_tesseract panics (unwrap on None) when
    ...
    // not tesseract is installed: missing â†’ exercises the tesseract_available()
    // pre-flight skip; present â†’ normal OCR of a blank image. The catch_unwind
    ...
    // the test host â€” we assert consistency, not a specific value.)
    #[test]
