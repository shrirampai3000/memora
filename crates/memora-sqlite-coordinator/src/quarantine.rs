...
/// MEMORA has hit under heavy concurrent load: the WAL index desyncs in
/// memory, SQLite reports a short read, and every pool is torn down. Forensics
/// on three separate incidents found the file itself intact each time â€”
...
/// contract, so the identity check is inverted too â€” the installed generation
...
mod tests {
    ...
    const CHILD_PHASE: &str = "MEMORA_QUARANTINE_CHILD_PHASE";
    const CHILD_DB: &str = "MEMORA_QUARANTINE_CHILD_DB";
