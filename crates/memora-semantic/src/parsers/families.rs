...
/// matched with exact `has_class`, never substring signatures â€” 2-3 char
...
///
/// The sender is the per-turn person chip â€” a `Button` inside the container.
...
/// digits). Range validation resolves title-digit collisions: "â€¦731w" cannot
...
///
/// The capture adapter has no window node on Windows â€” the walk starts inside
/// the content â€” so the "first root" is whatever the walker reached first, and
...
/// present a `file:///â€¦/index.html` string as the largest "document" on screen.
...
/// "Add a task"), never its content â€” only `Value` holds what the user typed.
