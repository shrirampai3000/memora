...
fn relative_day_delta(value: &str) -> Option<i64> {
    ...
    for token in tokens {
        match token.trim_matches('\'') {
            "today" | "aujourdâ€™hui" | "aujourd'hui" | "hoy" | "heute" => return Some(0),
...
fn month_number(value: &str) -> Option<u32> {
    match value.trim_end_matches('.') {
        ...
        "february" | "feb" | "fÃ©vrier" | "fevrier" | "febrero" | "februar" => Some(2),
        "march" | "mar" | "mars" | "marzo" | "mÃ¤rz" | "maerz" => Some(3),
        "april" | "apr" | "avril" | "abril" => Some(4),
        "may" | "mai" | "mayo" => Some(5),
        "june" | "jun" | "juin" | "junio" | "juni" => Some(6),
        "july" | "jul" | "juillet" | "julio" | "juli" => Some(7),
        "august" | "aug" | "aoÃ»t" | "aout" | "agosto" => Some(8),
        ...
        "december" | "dec" | "dÃ©cembre" | "decembre" | "diciembre" | "dezember" | "dez" => {
            Some(12)
        }
        _ => None,
    }
...
fn fingerprint(fields: &[&str]) -> [u8; 32] {
    ...
    digest.update(b"MEMORA-message-identity-v1");
    for field in fields {
...
fn relative_regex() -> &'static Regex {
    ...
    REGEX.get_or_init(|| {
        Regex::new(r"(?i)^(?:il\s+y\s+a|hace|vor)?\s*(\d{1,4})\s*(s|sec|secs|second|seconds|seconde|secondes|sek\.?|sekunde|sekunden|seg\.?|segundo|segundos|m|min\.?|mins|minute|minutes|minuto|minutos|h|hr|hrs|hour|hours|heure|heures|stunde|stunden|hora|horas)(?:\s*(?:ago|atrÃ¡s|atras))?$")
...
fn clock_only_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
        Regex::new(r"(?i)^(?:at|Ã |a\s+las|um)?\s*\d{1,2}:\d{2}(?::\d{2})?\s*(?:[ap]\.?m\.?)?$")
...
fn relative_day_label_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
        Regex::new(r"(?i)^(?:today|yesterday|aujourd'hui|hier|hoy|ayer|heute|gestern)(?:\s*(?:at|Ã |a\s+las|um)?\s*\d{1,2}:\d{2}(?::\d{2})?\s*(?:[ap]\.?m\.?)?)?$")
...
fn weekday_label_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
        Regex::new(r"(?i)^(?:monday|mon|lundi|lun|lunes|montag|tuesday|tue|mardi|mar|martes|dienstag|wednesday|wed|mercredi|mer|miÃ©rcoles|miercoles|mittwoch|thursday|thu|jeudi|jeu|jueves|donnerstag|friday|fri|vendredi|ven|viernes|freitag|saturday|sat|samedi|sam|sÃ¡bado|sabado|samstag|sunday|sun|dimanche|dim|domingo|sonntag)\.?(?:\s*(?:at|Ã |a\s+las|um)?\s*\d{1,2}:\d{2}(?::\d{2})?\s*(?:[ap]\.?m\.?)?)?$")
...
fn numeric_date_label_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
        Regex::new(r"(?i)^\d{1,4}[./-]\d{1,2}[./-]\d{1,4}(?:\s*(?:,|at|Ã |a\s+las|um)?\s*\d{1,2}:\d{2}(?::\d{2})?\s*(?:[ap]\.?m\.?)?)?$")
...
fn named_date_label_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
        Regex::new(r"(?i)^(?:\d{1,2}\s+[[:alpha:]]+|[[:alpha:]]+\s+\d{1,2}),?\s+\d{4}(?:\s*(?:,|at|Ã |a\s+las|um)?\s*\d{1,2}:\d{2}(?::\d{2})?\s*(?:[ap]\.?m\.?)?)?$")
