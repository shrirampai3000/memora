    ...
    match args.first().map(String::as_str) {
        ...
        Some("--run-live-installed-pipes") => {
            ...
            let repeats = args
                ...
                .unwrap_or(1);
            let case_filter = args.get(3).map(String::as_str);
            let api_url = std::env::var("MEMORA_API_URL")
                .unwrap_or_else(|_| "http://localhost:3030".to_owned());
            let api_key = std::env::var("MEMORA_LOCAL_API_KEY")
                .map_err(|_| "MEMORA_LOCAL_API_KEY is required for live pipe evals")?;
            let preset_id =
                std::env::var("MEMORA_EVAL_PRESET").unwrap_or_else(|_| "auto".to_owned());
            let fallback_model = std::env::var("MEMORA_EVAL_MODEL").unwrap_or_else(|_| {
                if preset_id == "auto" {
                    "auto".to_owned()
                } else {
                    String::new()
            ...
            if fallback_model.is_empty() {
                return Err(
                    "MEMORA_EVAL_MODEL is required when MEMORA_EVAL_PRESET is not auto"
                        .into(),
                );
            }
            let fallback_provider = std::env::var("MEMORA_EVAL_PROVIDER")
                .unwrap_or_else(|_| "MEMORA".to_owned());
            println!(
                "{}",
