...
- MEMORA's current text-bearing element outline;
...
```bash
cargo run -p MEMORA-semantic --example context_eval --locked -- --report
```
...
scripts/eval-semantic-replay.sh \
  ...
  --samples-per-app 25 \
  --max-apps 25 \
  --output /tmp/MEMORA-semantic-replay-report.json
```
...
```bash
cargo run -p MEMORA-semantic --example context_eval --locked -- --prompts \
  > /tmp/MEMORA-semantic-context-prompts.jsonl
```
...
```bash
cargo run --release -p MEMORA-semantic --example context_eval --locked -- \
  --run-pi ollama/MEMORA-gemma4:latest
```
...
```bash
cargo test -p MEMORA-db --test semantic_storage_test -- --nocapture
```
