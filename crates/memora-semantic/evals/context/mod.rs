...
fn evaluate_case(
    registry: &memora_semantic::ParserRegistry,
    case: EvalCase,
...
fn render_current_outline(nodes: &[CapturedAccessibilityNode]) -> String {
    ...
    let mut output = format!("frame 7 Â· accessibility Â· {} text elements\n", kept.len());
    ...
    if kept.len() > emitted || nodes.len() > emitted {
        output.push_str(&format!(
            "â€¦ showing {emitted} of {} elements â€” narrow with ?q=, ?role=, ?on_screen=true, or ?limit=\n",
            nodes.len()
        ));
    }
    output
