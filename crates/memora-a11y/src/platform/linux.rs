...
impl UiRecorder {
    ...
    pub fn request_permissions(&self) -> PermissionStatus {
        // Linux doesn't have permission dialogs â€” print guidance instead
    ...
    fn start_internal(
        ...
        // Thread 1: evdev input monitoring (optional â€” gracefully degrade without it)
        if perms.input_monitoring {
            let tx1 = tx.clone();
        ...
        // Thread 2: App/window observer (always runs â€” uses compositor/X11 metadata, no evdev needed)
        let tx2 = tx.clone();
...
fn monitor_keyboards(
    ...
    while !stop.load(Ordering::Relaxed) {
        ...
        for device in &mut devices {
            if let Ok(events) = device.fetch_events() {
                for event in events {
                    ...
                    // Modifier + key â†’ record as Key event
                    if ctrl_held || super_held {
                        ...
                        // Normal key â†’ aggregate into text buffer
        ...
        if !had_events {
            // No events â€” sleep briefly to avoid busy loop
...
mod tests {
    ...
    fn test_parse_sway_tree_focused_wayland_window_json() {
        ...
        assert_eq!(window_title, "MEMORA-wayland-ci-active");
        assert_eq!(pid, 4242);
    }
    ...
    fn linux_live_sway_active_window_info_reads_focused_window() {
        if std::env::var("MEMORA_RUN_LIVE_SWAY_TEST")
            .ok()
            .as_deref()
            != Some("1")
        {
            eprintln!("skipping live Sway test; set MEMORA_RUN_LIVE_SWAY_TEST=1");
            return;
        }

        let expected_title = std::env::var("MEMORA_WAYLAND_TEST_TITLE")
            .unwrap_or_else(|_| "MEMORA-wayland-ci-active".to_string());
