...
// ============================================================================
// Test App Fixture â€” launch and auto-cleanup
// ============================================================================
...
/// Run with: cargo test -p memora-a11y test_notepad_tree_structure -- --nocapture --ignored
#[test]
#[ignore]
...
/// Run with: cargo test -p memora-a11y test_notepad_text_capture -- --nocapture --ignored
#[test]
#[ignore]
fn test_notepad_text_capture() {
    ...
    let test_text = "Hello MEMORA accessibility test 12345";
...
/// Run with: cargo test -p memora-a11y test_calculator_tree_structure -- --nocapture --ignored
#[test]
#[ignore]
...
/// Run with: cargo test -p memora-a11y test_multi_app_tree_isolation -- --nocapture --ignored
#[test]
#[ignore]
...
/// Run with: cargo test -p memora-a11y test_full_pipeline_with_app -- --nocapture --ignored
#[test]
#[ignore]
fn test_full_pipeline_with_app() {
    ...
    // Now launch Notepad â€” should trigger a new snapshot when we focus it
    let app = TestApp::launch(&NOTEPAD, Duration::from_secs(10)).expect("Failed to launch Notepad");
    app.focus();
...
/// Run with: cargo test -p memora-a11y test_focusable_state_capture -- --nocapture --ignored
#[test]
#[ignore]
...
/// Run with: cargo test -p memora-a11y test_real_app_capture_perf -- --nocapture --ignored
#[test]
#[ignore]
...
// ============================================================================
// Quantitative Benchmarks â€” Memory, Completeness, Consistency
// ============================================================================

/// Quantify memory footprint of a real app tree capture.
/// Asserts byte size is within reasonable bounds per element.
/// Run with: cargo test -p memora-a11y test_memory_footprint -- --nocapture --ignored
#[test]
#[ignore]
fn test_memory_footprint() {
    ...
    assert!(
        ...
        "Bytes per node {:.1} exceeds 1KB â€” possible bloat",
        bytes_per_node
    );
...
/// Run with: cargo test -p memora-a11y test_memory_stability -- --nocapture --ignored
#[test]
#[ignore]
fn test_memory_stability() {
    ...
    assert!(
        drift_pct < 20.0,
        "Memory drift {:.1}% exceeds 20% threshold â€” possible leak or unbounded growth",
        drift_pct
    );
...
/// This is a ground-truth test â€” if we miss a button, the capture is incomplete.
/// Run with: cargo test -p memora-a11y test_calculator_completeness -- --nocapture --ignored
#[test]
#[ignore]
fn test_calculator_completeness() {
    ...
    assert!(
        missing.is_empty(),
        "Missing {} buttons: {:?} â€” capture is incomplete",
        missing.len(),
        missing
    );
    ...
    assert!(
        named_pct > 50.0,
        "Only {:.1}% of nodes have names â€” data quality too low",
        named_pct
    );
    // All nodes should have bounds (Calculator is fully visible)
    assert!(
        bounds_pct > 80.0,
        "Only {:.1}% of nodes have bounds â€” missing spatial data",
        bounds_pct
    );
...
/// Run with: cargo test -p memora-a11y test_capture_consistency -- --nocapture --ignored
#[test]
#[ignore]
fn test_capture_consistency() {
    ...
    assert!(
        ...
        "Capture consistency {:.1}% below 90% â€” too many missing/extra nodes",
        consistency_pct
    );
    // Hash stability: at least 90% of captures should produce the same hash
    assert!(
        hash_stability_pct >= 90.0,
        "Hash stability {:.1}% below 90% â€” tree content is flickering",
        hash_stability_pct
    );
    ...
    assert!(
        variance_pct < 10.0,
        "Node count variance {:.1}% exceeds 10% â€” capture is unstable",
        variance_pct
    );
...
/// Run with: cargo test -p memora-a11y test_serialization_fidelity -- --nocapture --ignored
#[test]
#[ignore]
fn test_serialization_fidelity() {
    ...
    // Serialize â†’ deserialize â†’ compare
