...
use memora_core::pii_removal::remove_pii;
use std::ffi::c_void;
...
/// Guard to serialize accessibility queries â€“ concurrent calls to
...
impl UiRecorder {
    ...
    fn start_internal(
        ...
        let perms = self.check_permissions();
        // Accessibility is the hard requirement â€” without it we can't
        ...
        // below â€” the user loses keystroke/click capture but keeps
        ...
        if perms.input_monitoring {
            ...
            tracing::warn!(
                "input monitoring not granted â€” running in reduced mode: \
        ...
        // Thread 2: App/window observer. Needs accessibility only â€” no
...
/// Polling-safe â€” does not trigger the system prompt and, crucially, NEVER
...
///      ground truth â€” it cannot be fooled by stale ("ghost") TCC records and
...
/// native prompt (and registers the process in System Settings â†’
/// Privacy & Security â†’ Input Monitoring); subsequent calls return the
/// current status without re-prompting.
///
/// Like `check_input_monitoring`, this no longer creates a probe tap â€” it
...
// reads â€” we replace that with periodic change_count polling when the tap
// is unavailable.
...
/// at 100ms bounds shutdown latency â€” without this, the recorder takes up
...
/// Operation marker for poll-detected clipboard mutations â€” distinguishes
...
/// Request to capture element context for a click â€” processed by a
...
/// Clipboard capture request â€” processed by a dedicated worker thread.
...
struct TapState {
    ...
    /// the callback drops events instead of holding them â€” so no keystroke can
    ...
    /// queues all subsequent events and replays them as a burst â€” a separate
    ...
    /// Scroll-burst coalescer â€” raw ~60-120 Hz wheel ticks aggregate into one
    ...
    /// Lock-free reads for app/window context â€” no mutex contention in the
    /// event tap callback (the hot path for every input event).
    current_app: Arc<ArcSwap<Option<String>>>,
    current_window: Arc<ArcSwap<Option<String>>>,
    current_pid: Arc<AtomicI32>,
    activity_feed: Option<ActivityFeed>,
    /// Bounded channel for context capture requests â€” a single worker thread
    /// processes these instead of spawning a thread per click.
    context_tx: Sender<ContextCaptureRequest>,
    /// Bounded channel for clipboard capture â€” avoids spawning a thread per
...
/// persists â€” timestamped at burst START so it orders correctly against the
...
///      port â€” never rely on release alone to stop delivery.
...
impl InstalledTap {
    ...
    /// (e.g. Input Monitoring denied) â€” the caller records ground truth.
    fn install<U>(
        mask: cg::EventMask,
    ...
    /// Idempotent teardown â€” KeyCastr's `removeTap`. Safe to call multiple
    ...
    /// source is gone â€” nothing new can arrive after this returns.
...
impl Drop for InstalledTap {
    fn drop(&mut self) {
        // KeyCastr's `dealloc { if (_tapInstalled) [self removeTap]; }` â€”
...
fn run_event_tap(
    ...
    // Single worker thread for context capture â€” avoids spawning a thread per click
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            while let Ok(req) = context_rx.recv() {
                ...
                // are never AX-inspected â€” parity with the tree walker's
    ...
    // Single worker thread for clipboard capture â€” avoids spawning a thread per
    ...
    let Some(mut installed) = InstalledTap::install(mask, tap_callback, state as *mut TapState)
        ...
        // Real tap creation is the ground truth for Input Monitoring â€” record
    ...
    // The real capture tap exists â€” Input Monitoring is genuinely granted.
    ...
    // while loop â€” well after any callback that could read this pointer.
    state
        .tap_ptr
    ...
    // â€” the callback already goes inert the instant `stop` is set â€” this just
    // bounds how long the final buffer flush is delayed.)
    while !stop.load(Ordering::Acquire) {
        cf::RunLoop::run_in_mode(cf::RunLoopMode::default(), 0.01, true);

        // Release the lock BEFORE calling remove_pii â€” holding the mutex
    ...
    // characters into the buffer concurrently with â€” or right after â€” the
    ...
    // the state box below (belt-and-suspenders â€” teardown already made it inert).
    drop(installed);

    // Final flush â€” same lock discipline: release before PII removal.
    ...
    // Same for an in-flight scroll burst â€” the callback is inert now, so this
    ...
    // and this pointer has no other live aliases â€” it is safe to free. Without
...
extern "C" fn tap_callback(
    ...
    // in a burst when the tap is re-enabled â€” exactly the phantom-keystroke
    ...
    if event_type == cg::EventType::TAP_DISABLED_BY_TIMEOUT {
        ...
        if !tap_ptr.is_null() {
            ...
            warn!("CGEventTap disabled by timeout â€” re-enabled");
        }
        return Some(event);
    }
    ...
    // Lock-free reads â€” no mutex contention in the input event path
    ...
    match event_type {
        ...
        cg::EventType::SCROLL_WHEEL => {
            ...
            if state.config.capture_scroll {
                ...
                if dx != 0 || dy != 0 {
                    ...
                    // the focused window â€” attributing bursts to the focused
                    ...
                    // Coalesce: one row per gesture, not per 60-120 Hz tick â€”
        ...
        cg::EventType::KEY_DOWN => {
            ...
            // All routed to a single worker thread via bounded channel â€”
...
fn run_app_observer(
    ...
    // buffered keystrokes into the focused field â€” the exact "residue words in
    ...
    // the thread exits â€” prevents the replay of buffered keystrokes.
    for _ in 0..3 {
...
/// (`AXGroup`, `AXWebArea`, `AXScrollArea`) â€” especially for web content â€” so the
...
fn find_labeled_descendant_at(
    ...
    for child in children.iter() {
        ...
        *budget -= 1;
        // Each child is its own AX ref â€” cap its messaging timeout so a busy
...
fn get_element_at_position(
    ...
    // the event tap callback path â€“ if another query is in-flight, skip this one.
    ...
    // messaging timeout is ~6s PER CALL â€” a busy app (Electron under load, IDE
    ...
    if config.is_password_field(Some(&role), name.as_deref()) {
        // Don't capture value for password fields â€” and no ancestor chain
    ...
    // The element's path in the window hierarchy â€” what disambiguates "the
...
/// when present) per hop â€” the element's "path in the window hierarchy" for
...
/// app can't stall the worker â€” on timeout the partial chain is returned. The
...
/// tree/macos.rs) â€” worst case is one timed-out call, not the ~6s macOS
/// default. Read-only AXParent/AXTitle reads; never pokes
/// AXEnhancedUserInterface (the Chromium phantom-IME-commit bug â€” see
...
fn collect_ancestor_chain(
    ...
    // checked before this first fetch too â€” an already-blown budget (slow
...
// NSPasteboardItem have undocumented main-thread-only semantics â€” calling
...
// in 1Password/arboard#218 â€” even a private serial queue with autorelease
// pool isn't enough; the only safe place is the main thread, where AppKit's
// pasteboard observers are already serialized.
//
// We hop onto the main queue via `dispatch_sync` (bounded â€” see
...
// Tauri app (tao's event loop), but NOT for the headless `MEMORA` CLI
// binary â€” its `#[tokio::main]` main thread parks in tokio's executor and
// never services GCD's main queue â€” nor for `cargo test`, whose harness
...
fn check_clipboard_crash_marker() {
    CLIPBOARD_CRASH_CHECK.call_once(|| {
        ...
        if inflight.exists() {
            ...
            let _ = std::fs::remove_file(&inflight);
            tracing::warn!(
                "clipboard capture disabled for this session â€” previous run \
...
fn get_clipboard() -> Option<String> {
    ...
    // always time out here â€” turning every round-trip test into a
    ...
    // touching the pasteboard mid-test-run) â€” and the tests serialize
        ...
        match bounded_dispatch(
            ...
            BoundedDispatch::TimedOut => {
                ...
                warn!(
                    ...
                    "clipboard capture: main-queue dispatch timed out â€” \
...
fn run_activity_only_tap(activity_feed: ActivityFeed, stop: Arc<AtomicBool>) {
    ...
    let Some(mut installed) = InstalledTap::install(mask, activity_only_callback, feed_ptr) else {
        ...
        record_input_monitoring_truth(false);
        // Clean up the leaked box â€” the tap was never installed, so the
    ...
    // Clean up â€” no callback can run past this point.
    unsafe {
        let _ = Box::from_raw(feed_ptr);
    }
...
mod tests {
    ...
    fn test_decode_process_name_rejects_empty_results() {
        assert_eq!(decode_process_name(b"MEMORA", 0), None);
    ...
    fn test_decode_process_name_bounds_and_sanitizes_results() {
        assert_eq!(
            decode_process_name(b"MEMORA\0ignored", 18),
            Some("MEMORA".to_string())
        );
        assert_eq!(
            decode_process_name(b" MEMORA ", 12),
            Some("MEMORA".to_string())
        );
        assert_eq!(
            decode_process_name(b"app-\xFF", 5),
            Some("app-ï¿½".to_string())
        );
    }
    ...
    // `#[cfg(test)]` path in `get_clipboard` reads directly â€” so without
    ...
    fn test_get_clipboard_no_subprocess() {
        ...
        assert!(
            ...
            "10 clipboard reads took {:?} â€” too slow, may be spawning subprocesses",
            elapsed
        );
    }
    ...
    fn test_get_clipboard_set_and_read() {
        ...
        let test_text = "MEMORA_clipboard_test_12345";
        {
    ...
    fn test_get_clipboard_unicode() {
        let _guard = clipboard_test_guard();
        let unicode_text = "æ—¥æœ¬èªžãƒ†ã‚¹ãƒˆ ðŸŽ‰ Ã©mojis Ã±";
        {
            let mut clipboard = arboard::Clipboard::new().expect("clipboard init");
            clipboard
    ...
    fn test_get_clipboard_large_content() {
        let _guard = clipboard_test_guard();
        // 100KB of text â€” should not OOM or hang
    ...
    /// Regression test for the "hello hello â†’ ello ell ell / hehw" ghost-
    ...
    /// verbatim in the focused app's text field after MEMORA quit.
    #[test]
    fn test_no_ghost_keystrokes_after_stop() {
        ...
        // Phase 1: normal operation â€” "hello " typed while running.
        ...
        // Phase 3: teardown complete â†’ final flush.
        let flushed = buf.lock().flush();

        // The flush must contain exactly what was typed before stop â€” the two
    ...
    fn test_stop_flag_concurrent() {
        ...
        // Spawn 8 threads all trying to push characters after stop â€” none
        // should succeed.
