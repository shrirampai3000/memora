...
use windows::Win32::System::Threading::{
    GetCurrentProcessId, GetCurrentThread, GetCurrentThreadId, OpenProcess, SetThreadPriority,
    PROCESS_QUERY_LIMITED_INFORMATION, THREAD_PRIORITY, THREAD_PRIORITY_BELOW_NORMAL,
...
pub(crate) fn apply_extraction_thread_priority(priority: ExtractionThreadPriority) {
    ...
    unsafe {
        ...
        match SetThreadPriority(handle, level) {
            ...
            Err(e) => warn!(
                "SetThreadPriority({:?}) failed: {:?} â€” falling back to default priority",
                priority, e
            ),
        }
    }
...
// Watchdog bounds: if the OS receives user input within 2s but our hook callbacks
// stay silent for 5s, the hooks were likely removed silently by Windows
// LowLevelHooksTimeout. Reinstall them after a 30s backoff.
...
fn watchdog_should_reinstall(
    ...
    if system_input_age_ms > WATCHDOG_SYSTEM_INPUT_RECENT_MS
    ...
    // Don't reinstall if silence is expected due to an elevated or restricted foreground window (UIPI).
    unsafe {
        if is_foreground_window_elevated_or_restricted() {
            return false;
        }
    }
...
/// Returns true if the foreground window belongs to an elevated or restricted process (UIPI),
/// which intentionally blocks low-level input hooks for non-elevated applications.
unsafe fn is_foreground_window_elevated_or_restricted() -> bool {
    ...
    OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, false, pid).is_err()
}

/// Restore scheduling parity for the LL-hook thread when the process runs at
/// a lowered priority class: HIGHEST within BELOW_NORMAL is base priority 8 â€”
...
impl RecordingHandle {
    ...
    /// `capture_tree` is enabled â€” off by default because full-window UIA walks
...
impl UiRecorder {
    ...
    fn start_internal(
        ...
        // on a full channel â€” a bounded hand-off would recreate the input
        ...
        // Thread 1: Native Windows hooks â€” minimal callbacks that snapshot and
        ...
        // Thread 1b: Input worker â€” consumes raw input and owns app/window
...
// ============================================================================
// Raw input hand-off: LL hook callbacks â†’ input worker
// ============================================================================
...
/// the loop wakes to check the stop flag and run the hook watchdog â€” it does
...
enum RawInput {
    ...
    KeyDown {
        ...
        /// state â€” read from the worker thread it would be permanently stale.
        caps_on: bool,
...
/// (measured: 1121 â†’ 13 events in a 2-min session).
...
/// callbacks are the system-wide synchronous input path â€” every keystroke and
/// mouse event in every application waits on them â€” so they only snapshot and
...
fn run_native_hooks(
    ...
    unsafe {
        ...
        if loop_timer == 0 {
            warn!("failed to install hook loop timer â€” watchdog disabled, stop latency degraded");
        }
...
unsafe fn install_ll_hooks(_h_instance: HINSTANCE) {
    let h_mod = HINSTANCE::default();
    match SetWindowsHookExW(WH_KEYBOARD_LL, Some(keyboard_hook_proc), h_mod, 0) {
        Ok(hook) => {
    ...
    match SetWindowsHookExW(WH_MOUSE_LL, Some(mouse_hook_proc), h_mod, 0) {
        Ok(hook) => {
...
// registry LowLevelHooksTimeout â€” which could historically happen when this
...
unsafe fn watchdog_tick(
    ...
    *last_reinstall = Some(Instant::now());
    ...
    warn!(
        ...
        system_input_age_ms,
        hook_silent_ms,
        "a11y: LL input hooks reinstalled by watchdog â€” system input active but hook callbacks \
...
/// path â€” every keystroke in every application waits for it to return, and
    ...
    if code == HC_ACTION as i32 {
        ...
        HOOK_STATE.with(|state| {
            ...
            if let Some(ref mut s) = *guard {
                ...
                // Activity feed is lock-free atomics â€” safe here, and keeping
...
/// and enqueue only â€” never lock a mutex, allocate strings, or run filters
...
/// allocate freely â€” this thread is off the system input path.
struct InputWorker {
    tx: Sender<UiEvent>,
    start: Instant,
    config: UiCaptureConfig,
...
impl InputWorker {
    /// Blocking locks are fine here â€” the app observer holds these only briefly
    ...
    fn process_raw(&mut self, raw: RawInput) {
        match raw {
            RawInput::KeyDown {
                ...
                // content is fetched once the copy lands â€” see clipboard_read_due.
            ...
            RawInput::ButtonDown {
                ...
                // A click interrupts any in-flight scroll run â€” flush it first
    ...
    /// fell behind the soft cap â€” input capture lost events, but system input
    ...
    fn maybe_log_queue_drops(&mut self) {
        ...
        if due {
            warn!(
                ...
                "a11y: raw input queue over soft cap â€” input events dropped (worker falling behind)"
            );
...
// ============================================================================
// App Observer (Windows) â€” event-driven via SetWinEventHook
// ============================================================================
...
fn process_foreground_change(state: &mut AppObserverState) {
    unsafe {
        ...
        if is_transient_shell_window(hwnd) {
            debug!(hwnd = ?hwnd.0, "a11y: foreground change â€” skipped transient shell window");
            return;
        }
...
fn run_app_observer(
    ...
    // does some UIAutomation work on focus changes â€” let it yield to input.
...
/// Cached PIDâ†’process name mapping with TTL to avoid CreateToolhelp32Snapshot on every lookup.
...
pub(crate) fn get_process_name(pid: u32) -> Option<String> {
    ...
    // Cache miss â€” do the expensive lookup
...
pub(crate) const TRANSIENT_SHELL_WINDOW_CLASSES: &[&str] = &[
    ...
    "Shell_TrayWnd",    // Taskbar â€” transiently owns foreground during button clicks
    "tooltips_class32", // Explorer tooltip windows that can briefly grab focus
    "TopLevelWindowForOverflowXamlIsland", // System tray overflow popup (^ arrow) â€” noise, 12 tray icon nodes
];
...
/// one of them was therefore recorded as "ApplicationFrameHost.exe" â€” a name
...
pub(crate) fn normalize_app_name(raw_process: &str, window_class: &str) -> String {
    ...
    // Edge's WebView2 runtime sub-process â€” fold into msedge.exe.
    ...
    // Chromium content (Widgets, Search, re-parented WebView2 v117+) â€” attribute to Edge.
...
///
/// 1. **msedgewebview2.exe** â€” Edge's WebView2 runtime sub-process. Normalised to
///    `msedge.exe` so that a user exclusion for Edge covers all Edge-spawned windows.
///
/// 2. **Shell-hosted Chromium** â€” Windows 11 features like the Widgets panel and
...
/// steal focus on clicks â€” attributing those to explorer.exe would poison the
...
mod tests {
    ...
    fn test_normalize_app_name_shell_hosted_chromium() {
        // Shell-host + Chromium class â†’ msedge.exe (covers Widgets / Search / re-parented WebView2).
        assert_eq!(
    ...
    fn test_normalize_app_name_passthrough() {
        ...
        // Non-shell-host process with Chromium class must NOT be remapped â€” that would
    ...
    fn test_flush_text_buffer() {
        ...
        // Buffer has content â€” flush should send a Text event
    ...
    fn test_clipboard_combo_queues_pending_read() {
        ...
        // No immediate event â€” the content read is deferred until the copy
    ...
    fn test_clipboard_read_due_decision() {
        // Paste: content pre-exists the keystroke â€” read immediately.
        assert!(clipboard_read_due('v', Some(10), 10, 0));
        // Copy: wait for the sequence number to moveâ€¦
        assert!(!clipboard_read_due('c', Some(10), 10, 0));
        assert!(clipboard_read_due('c', Some(10), 11, 0));
        // â€¦or the settle deadline to pass (legacy read-anyway behavior).
        ...
        // No sequence snapshot â†’ read immediately (defensive).
    ...
    fn test_move_respects_capture_flag() {
        ...
        // Default: capture_mouse_move off â†’ dropped.
    ...
    fn test_watchdog_should_reinstall_decision() {
        ...
        // Idle system: hooks silent but no input either â€” not dead.
