...
/// active input â€” the resulting tree would be stale within milliseconds anyway, and the
...
fn pause_uia_while_screen_locked(
    ...
    if !memora_config::screen_is_locked() {
        return false;
    }
...
/// ad-hoc captures). Generous on purpose â€” the capture pipelines pass their
...
struct TreeBudget {
    ...
    /// deadline there can only discard data that was already paid for â€”
...
/// TreeWalker fallback (Chromium/Electron). Shared across capture pipelines â€”
...
impl UiaContext {
    ...
    /// wall-clock budget. Kept as the convenience entry point for tests â€”
    ...
    /// COM round-trips per element, serviced on the target app's UI thread â€” the
    ...
    /// in-process â€” so building it is bounded by the node cap only. Truncating
    ...
    pub(crate) fn capture_window_tree_bounded(
        ...
        if needs_walker_fallback(hwnd) {
            ...
            // Walker setup failed (window gone, provider error) â€” fall through
        ...
        unsafe {
            ...
            if count <= 10 {
                ...
                // Cached path works for this window â€” clear any stale memo
    ...
    fn element_to_context(&self, element: &IUIAutomationElement) -> ElementContext {
        ...
        ElementContext {
            ...
            automation_id,
            bounds,
            // Windows ancestor-chain capture not implemented yet â€” macOS only
...
pub fn run_uia_thread(
    ...
    // the heaviest a11y consumer â€” letting it yield to input is the highest-value
    ...
    // Initialize UIA context with retries â€” COM services may not be ready
    ...
    while !stop.load(Ordering::Relaxed) {
        ...
        // the full window tree; without it we only refresh the focused element â€”
        // a single cross-process call, same cost class as per-click ElementFromPoint â€”
        ...
        // Periodic safety-net re-capture â€” full tree walks only.
        if config.capture_tree
            && !skip_capture
...
/// - Time until debounce fires (if pending focus exists) â€” drives tree walks
...
/// crashing whenever they opened the **Sent Items** folder while MEMORA was
/// recording, and that ending the MEMORA process made the crash disappear. Its
/// virtualized message-list provider re-enters and faults when we cache the whole grid
/// in one cross-process call (Sent Items is worst â€” its recipient column resolves
...
fn capture_and_send(
    ...
    *last_capture_time = capture_start;
    ...
    // Items â€” see is_fragile_uia_tree_provider). Other event capture and OCR are
...
mod tests {
    ...
    fn test_fragile_uia_provider_detection() {
        ...
        // Match must be case-insensitive â€” szExeFile yields "OUTLOOK.EXE".
        ...
        assert!(!is_fragile_uia_tree_provider("OUTLOOK")); // no extension â†’ not our exact match
    ...
    /// per-element TreeWalker fallback) to see `Timeout` truncation â€” the
    ...
    /// cargo test -p memora-a11y test_live_bounded_capture -- --ignored --nocapture
    #[test]
    #[ignore]
    fn test_live_bounded_capture_truncates() {
        ...
        // assertion is loose â€” the point is it doesn't run for seconds.
        let start = std::time::Instant::now();
        let bounded = uia
        ...
        assert!(
            bounded_ms < 5_000,
            "bounded walk took {}ms â€” deadline not honored",
            bounded_ms
        );
        unsafe { CoUninitialize() };
    }
    ...
    fn test_screen_lock_pause_clears_deferred_work() {
        ...
        impl Drop for ResetScreenLock {
            fn drop(&mut self) {
                memora_config::set_screen_locked(false);
            }
        }

        let _reset = ResetScreenLock;
        memora_config::set_screen_locked(true);
    ...
    fn test_compute_next_timeout_honors_focus_debounce_for_element_refresh() {
        ...
        // refresh runs â€” enrichment for clicks/app-switches depends on it.
        let pending_focus = Arc::new(Mutex::new(Some(PendingFocus {
            hwnd: HWND::default(),
            time: Instant::now() - Duration::from_secs(1),
        })));
        ...
        assert_eq!(wait_ms, 0); // debounce elapsed â€” ready now
    }
    ...
    fn test_compute_next_timeout_ignores_periodic_interval_without_capture_tree() {
        ...
        // tree walks â€” it must not wake the worker when capture_tree is off.
        let pending_focus = Arc::new(Mutex::new(None::<PendingFocus>));
        let mut config = UiCaptureConfig::new();
        config.capture_tree = false;
        config.tree_capture_interval_ms = 1;
    ...
    /// Run with: cargo test --package MEMORA-accessibility test_live_all_windows -- --nocapture --ignored
    #[test]
    #[ignore]
    ...
    /// Run with: cargo test --package MEMORA-accessibility test_live_tree_capture -- --nocapture --ignored
    #[test]
    #[ignore]
    ...
    /// Run with: cargo test --package MEMORA-accessibility test_live_performance -- --nocapture --ignored
    #[test]
    #[ignore]
    ...
    /// Run with: cargo test --package MEMORA-accessibility test_live_dedup -- --nocapture --ignored
    #[test]
    #[ignore]
    fn test_live_dedup() {
        unsafe {
        ...
        // Capture 10 times rapidly â€” only first should produce a snapshot
        for _ in 0..10 {
            capture_and_send(
                &uia,
                hwnd,
    ...
    /// Run with: cargo test --package MEMORA-accessibility test_live_uia_thread_lifecycle -- --nocapture --ignored
    #[test]
    #[ignore]
    fn test_live_uia_thread_lifecycle() {
        // Tree capture is off by default (freezes foreground apps) â€” this test
    ...
    /// Run with: cargo test --package MEMORA-accessibility test_live_cpu_usage -- --nocapture --ignored
    #[test]
    #[ignore]
    fn test_live_cpu_usage() {
        ...
        // Opt into tree capture â€” off by default, and this test asserts snapshots.
        let mut config = UiCaptureConfig::new();
        config.capture_tree = true;
        config.tree_capture_interval_ms = 2000;
    ...
    /// Run with: cargo test --package MEMORA-accessibility test_live_capture_quality -- --nocapture --ignored
    #[test]
    #[ignore]
