...
//! Based on bigbrother's event format with extensions for MEMORA integration.
...
pub struct ElementContext {
    ...
    /// (see [`ancestors_to_json`]) â€” the element's "path in the window
...
impl UiEvent {
    ...
    pub fn to_db_insert(&self, session_id: Option<String>) -> memora_db::InsertUiEvent {
        use memora_db::{InsertUiEvent, UiEventType};
...
mod tests {
    ...
    fn test_text_event_accepts_app_context() {
        ...
        event.window_title = Some("main.rs â€” my-project".to_string());

        assert_eq!(event.app_name.as_deref(), Some("Cursor"));
        assert_eq!(event.window_title.as_deref(), Some("main.rs â€” my-project"));
