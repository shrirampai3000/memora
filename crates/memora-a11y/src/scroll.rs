...
//! ticks â€” persisting each tick was why `capture_scroll` shipped default-off
...
//! recorded data â€” macOS delivers scroll to the window under the cursor, so
...
/// Accumulates scroll ticks into bursts. Not thread-safe by itself â€” callers
...
struct Burst {
    ...
    /// scroll) â€” part of the burst identity so gestures over different apps'
    ...
    /// up) â€” the emit gate uses these, never the signed sums.
    abs_x: u64,
    abs_y: u64,
...
impl ScrollBuffer {
    ...
    /// burst aged out â€” the caller emits that flush, and the tick opens the
    /// next burst.
    ...
    /// carried any movement). The gate is on MAGNITUDE, not the signed sums â€”
...
mod tests {
    ...
    fn zero_net_gesture_still_emits() {
        ...
        // real â€” magnitude gates the emit, so the row survives.
        let mut buf = ScrollBuffer::new();
