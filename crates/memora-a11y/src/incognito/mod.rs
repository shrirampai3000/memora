...
//!
//! ```rust,no_run
//! use memora_a11y::incognito::{create_detector, is_title_private};
//!
//! // Quick title-only check (all platforms, no I/O)
//! assert!(is_title_private("New Tab - Google Chrome (Incognito)"));
//! assert!(is_title_private("Neuer Tab â€” Firefox (Privater Modus)"));
...
/// One macOS browser MEMORA knows how to address through Automation.
...
mod tests {
    ...
    fn test_title_detection_non_english() {
        ...
        // French Firefox
        assert!(is_title_private("Mozilla Firefox (Navigation privÃ©e)"));
        // Spanish Firefox
        assert!(is_title_private("Mozilla Firefox (NavegaciÃ³n privada)"));
    }
    ...
    fn test_normal_windows_not_flagged() {
        ...
        assert!(!is_title_private("MEMORA docs"));
