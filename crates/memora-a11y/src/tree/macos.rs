...
///      Pages, Numbers, Keynote, Xcode, Notes, BBEdit, Sublime, â€¦)
///      populate `AXDocument` on the focused window with a `file://`
///      URL. Browsers populate it with `http(s)` â€” we skip those so
...
///
/// Edge cases handled:
///   - Untitled / unsaved buffers â†’ `AXDocument` returns `None`,
///     fallback returns `None`. Field stays NULL.
///   - `AXDocument` is a `file://` URL with percent-encoding
///     (spaces â†’ `%20`) â†’ decoded into the raw absolute path.
...
/// walk (~tens of microseconds typical), plus â€” only for known
/// Electron apps â€” a small JSON file read that's cached behind a
...
fn parse_axdocument_value(raw: &str) -> Option<String> {
    ...
    // UTF-8 sequence after decoding fall back to the raw URL â€” better than
    // panicking and better than dropping the whole field.
    let without_scheme = raw.trim_start_matches("file://");

    // Drop a leading host segment if present (`file:///Users/...` â†’
    // `/Users/...`; `file://localhost/Users/...` â†’ `/Users/...`). On macOS
...
/// True for an `http`/`https` URL â€” the only schemes that belong in the
...
///   1. `AXDocument` on the window â€” set by Safari, Chrome, Edge.
///   2. **`AXWebArea` â†’ `AXURL` descent** ([`find_web_area_url`]). Chromium
...
///      and cached â€” never spawns `osascript`.
...
/// alone does not bound a miss on a slow, wide native AX provider â€” a depth-12
...
///
/// Returns `Some(url_opt)` once a web area is reached â€” `url_opt` is its `AXURL`
/// (possibly `None`) and the whole search stops â€” or `None` if no web area was
...
/// expires faster than a negative one â€” a window sitting mid-load / on a popup
...
/// (`memora_capture::paired_capture::walk_accessibility_tree` calls
...
/// composition/autocomplete buffer into the focused field â€” the "phantom text"
...
impl TreeWalkerPlatform for MacosTreeWalker {
    fn walk_focused_window(&self) -> Result<TreeWalkResult> {
        // Wrap in autorelease pool â€” cidre AX/NS APIs create autoreleased
...
impl MacosTreeWalker {
    fn walk_focused_window_inner(&self) -> Result<TreeWalkResult> {
        ...
        // since the window title isn't known yet â€” see `window_pattern`.
        ...
        if ENHANCED_MODE_CACHE.should_enable_once(pid) {
            ...
            // Also set AXManualAccessibility â€” the Chromium-specific flag.
            ...
            // Some apps (Obsidian in particular â€” issue #3002) never materialize
        ...
        // Full app + title check â€” scoped patterns (`App::Title`) and any
        ...
        // global semantics â€” see `window_pattern::passes_includes`.
        ...
        // VS Code and forks (Cursor, Windsurf, Antigravity, VSCodium, â€¦) â€” two adjustments:
        ...
        //      and looking for an AXList ancestor at â‰¥20 hops from AXWebArea (terminal
        //      rows are ~30 hops deep; sidebar file-tree AXLists are â‰¤12 hops).
        ...
        // skip the entire capture â€” including the screenshot â€” to prevent the
        ...
        // Don't bail on empty text â€” we still need the app_name and window_name
        ...
        // Extract document path. Skipped for browsers â€” their AXDocument
...
/// the capture image at any resolution. Requires both frames â€” without
...
/// stays clean and the hot path only pays for branching on VS Code frames â€”
/// the `AppState::None` arm is a single discriminant check that the compiler
/// folds away for every non-matching app.
#[derive(Clone, Copy)]
enum AppState {
    /// Not a recognised Electron IDE â€” no per-app filtering.
    None,
    ...
    VsCode {
        ...
        /// because [`walk_element`] passes `next_depth = 0` to its children â€”
...
/// terminal subtree. Pure helper â€” testable without a live AX session.
///
...
/// at â‰¤ 15 hops below AXWebArea; terminal output rows at â‰¥ 27 hops â€” so 20 is
...
struct WalkState {
    ...
    /// Per-app walk state. `AppState::None` for every non-VS Code app â€” no overhead.
    app: AppState,
    ...
    /// Supports `App::Title` scoping â€” `focused_app_lower` is the app side.
    ...
    /// `None` when line capture is disabled â€” see `TreeWalkerConfig::enable_line_bounds`.
    ...
    /// Multi-line safety factor â€” same field as `TreeWalkerConfig::line_bounds_min_height_ratio`.
...
///     at â‰¥28 steps from `AXWebArea` â†’ detected via `axlist_idx`.
///   - Typing in input: focused element is `AXTextField` (not inside `AXList`)
///     â†’ detected via the xterm.js container's `AXDescription` ("Terminal N, â€¦")
///     which is a common ancestor of both the `AXList` and `AXTextField`.
///
/// Sidebar `AXList` elements are â‰¤12 steps from `AXWebArea`; terminal ones â‰¥28.
/// The 20-step threshold keeps them apart. `AXDescription` scan is also gated
/// on depth â‰¥ 20 so shallow tab-bar labels cannot produce false positives.
fn vscode_terminal_window_name(ax_app: &ax::UiElement) -> Option<String> {
    ...
    for _ in 0..60 {
        ...
        // Scan desc/title on every ancestor for a terminal name â€” but only until
        ...
        // by some forks (e.g. Antigravity: "zsh Use âŒ¥F1 â€¦").
        ...
        match role.as_deref() {
            ...
            Some("AXWebArea") => {
                ...
                if steps < 20 {
                    // Shallow â€” sidebar or toolbar element, not terminal.
                    return None;
                }
                ...
                // Terminal AXLists sit at wa+28; editor/sidebar lists are â‰¤15.
                // Checking the focused-element depth (steps >= 20) alone is not enough â€”
                ...
                if let Some(list_idx) = axlist_idx {
                    if steps.saturating_sub(list_idx) < 20 {
                        // AXList is too close to AXWebArea â€” editor or sidebar, not terminal.
                        return None;
                    }
...
/// e.g. "zsh Use âŒ¥F1 for terminal accessibility help" â†’ "zsh"
...
///       e.g. "Terminal 4, 2.1.150" or "Terminal 2, zsh Use âŒ¥F1 â€¦"
///       â†’ returns "Terminal - {session_name}" (hint stripped)
///
///   (b) Antigravity / some forks: bare "{session_name}" or "{shell} Use âŒ¥â€¦"
///       e.g. "zsh Use âŒ¥F1 for terminal accessibility help"
///       â†’ handled by `parse_xterm_bare_desc`, not this function.
///
...
fn parse_vscode_terminal_name(val: &str) -> Option<String> {
    ...
    // "Terminal N, session_name â€¦" â€” extract session name after the comma.
...
/// e.g. "zsh Use âŒ¥F1 for terminal accessibility help" â†’ "Terminal - zsh"
///
/// REQUIRES the accessibility hint to be present in the raw string â€” this is
...
fn parse_xterm_bare_desc(val: &str) -> Option<String> {
    ...
    let has_hint = raw.contains(" Use \u{2325}")  // âŒ¥
        || raw.contains(" Use \u{2318}")           // âŒ˜
        || raw.contains(" Use ^");
    if !has_hint {
        return None;
    }
...
unsafe extern "C-unwind" {
    fn AXUIElementCopyMultipleAttributeValues(
        ...
        options: u32, // 0 â€” parallel array, NOT stop-on-error (1)
...
thread_local! {
    ...
    /// runs only for text-emitting nodes â€” folding these into the per-node batch
...
/// Coerce a batched entry to a `String` iff it is a `CFString` â€” the exact
...
/// Unwrap a batched `AXValue`-wrapped CGPoint â€” same as `get_element_frame`.
...
/// Unwrap a batched `AXValue`-wrapped CGSize â€” same as `get_element_frame`.
...
/// Coerce a batched entry to a `bool` iff it is a `CFBoolean` â€” the exact
/// coercion `get_bool_attr` applies (non-booleans / AXError placeholders â†’ None).
...
/// messaging timeout) â€” the same conditions under which the old `elem.role()`
...
fn read_node_attrs(
    ...
    // AXError placeholders for missing attrs. Never stop-on-error (1) â€” one
...
/// `None` on a failed batch â€” matching the old path, where every individual
...
fn walk_element(elem: &ax::UiElement, depth: usize, state: &mut WalkState) {
    ...
    // node without walking children â€” identical to the old `elem.role()` failing.
    ...
    // VS Code content isolation â€” all VS Code logic is gated behind AppState::VsCode.
    ...
    // `depth` is already post-reset (children of AXWebArea start at depth 0 â€”
    // see the `next_depth` block below). AXList nodes â‰¥ 20 hops past that point
    // are terminal output rows; sidebar/editor AXLists sit at â‰¤ 15 hops.
    //
    //  â€¢ Editor mode: prune the deep AXList entirely â€” early return, no children walked.
    //  â€¢ Terminal mode: mark entry into the AXList so only its descendants emit text.
    ...
    {
        return; // prune entire terminal subtree â€” no children walked, no text emitted
    }
    ...
    // apps (VS Code, Slack, Discord, Obsidian, Notion, â€¦) get the full
    ...
    // actual app content â€” silently dropping terminal output, editor text, etc.
...
fn extract_text(
    ...
    // whether the captured screenshot actually shows this element â€” see
...
/// walk, or for tools that don't set them) â€” the AX walker would still
...
/// `AXScrollArea` ancestor and intersects with its visible rect â€”
/// follow-up.
fn is_on_screen(
    elem_x: f64,
    elem_y: f64,
...
fn normalize_bounds(
    ...
    // Sanity check â€” skip if out of bounds
...
/// â€” silently stale in processes without a pumping AppKit run loop: CLI
...
fn resolve_focused_ax_app(
    ...
    // Chromium/Electron apps that haven't materialized their AX tree â€” it
                ...
                Some(fp) if fp != pid => {
                    debug!(
                        "AX focusedApplication (pid={}, app={}) disagrees with frontmost \
                         (pid={}) â€” trusting the window server (stale AX focus)",
                        pid,
...
fn capture_lines_for_node(
    ...
    // Only spend IPC on visually-present text â€” off-screen scroll-buffer
...
fn fill_ax_props(
    ...
    // fields are best-effort point-in-time Optionals (focus/selection/etc.) â€”
    // NOT a dedup surface like text_content/content_hash â€” so reading them at
...
mod tests {
    ...
    fn test_percent_decode_path_passes_through_malformed() {
        // Non-hex after % â†’ leave verbatim instead of dropping the whole path.
        assert_eq!(
            percent_decode_path("/Users/me/%g0.md").as_deref(),
            Some("/Users/me/%g0.md")
        );
        // Truncated trailing % â€” last 1-2 bytes pass through (no panic).
        assert_eq!(
    ...
    fn test_percent_decode_path_empty_and_unicode() {
        ...
        // %C3%A9 = Ã© in UTF-8 â€” confirm decode is bytewise so multi-byte
    ...
    fn test_walk_focused_window_returns_result() {
        ...
        // Should not panic â€” returns Ok(Some(...)) or Ok(None) depending on focus
    ...
    // â”€â”€ VS Code terminal helper unit tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn test_is_vscode_like_no_false_positive_on_xcode() {
        // "xcode" must NOT match the "code" entry â€” the whole point of the word-boundary check.
    ...
    fn test_is_vscode_like_allows_space_suffix() {
        // e.g. "code helper (renderer)" â€” macOS sometimes appends a suffix with a space.
    ...
    fn test_parse_vscode_terminal_name_strips_hint() {
        // Session name has accessibility hint appended â€” must strip it.
        assert_eq!(
    ...
    fn test_parse_vscode_terminal_name_fallback_no_session() {
        // "Terminal N" with no comma â†’ fall back to "Terminal N".
        assert_eq!(
    ...
    fn test_parse_xterm_bare_desc_rejects_without_hint() {
        // No accessibility hint â†’ must return None (avoids matching arbitrary UI labels).
    ...
    fn test_parse_xterm_bare_desc_rejects_multiword_session() {
        // Session name with a space (e.g. "my session") â†’ None (only bare single-token names).
        assert_eq!(
    ...
    fn test_parse_xterm_bare_desc_rejects_literal_terminal() {
        // Bare token equals "Terminal" â†’ reject. The numbered "Terminal N, â€¦" form
    ...
    // â”€â”€ VS Code terminal-list gating tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    //
    ...
    // checks `depth >= 20` directly â€” not `depth >= wd + 20`.
    ...
    fn test_terminal_list_gating_before_webarea() {
        // Inside VsCode but before AXWebArea was visited â†’ no terminal list yet.
    ...
    fn test_terminal_list_gating_inside_webarea_deep_axlist() {
        // Inside AXWebArea, depth >= 20, role == AXList â†’ terminal output row.
    ...
    fn test_terminal_list_gating_inside_webarea_shallow_axlist() {
        // Inside AXWebArea but depth < 20 â†’ sidebar/editor list, not terminal.
        // Sidebar AXLists sit at depth â‰¤ 11 in this frame.
        let app = AppState::VsCode {
            mode: VsCodeMode::Editor,
            inside_webarea: true,
            in_terminal_subtree: false,
    ...
    fn test_terminal_list_gating_wrong_role() {
        // Only AXList qualifies â€” AXGroup/AXStaticText at the same depth don't.
        let app = AppState::VsCode {
            mode: VsCodeMode::Terminal,
            inside_webarea: true,
            in_terminal_subtree: false,
    ...
    // ---- Fix 1: AXWebArea â†’ AXURL descent caps ----
    ...
    fn test_web_area_dfs_finds_first_url() {
        // AXGroup > AXGroup > AXWebArea(url) â€” mirrors Arc's depth-2 web area.
    ...
    fn test_web_area_dfs_visit_cap_bounds_wide_tree() {
        ...
        // visit_cap â€” the depth cap alone would not bound this.
    ...
    fn test_arc_cache_different_key_misses() {
        ...
        let other = (1234, "Arc â€” Other window".to_string());
        assert_eq!(cache.get(&other, t0), None, "different window_name misses");

        let other_pid = (9999, "Arc â€” Inbox".to_string());
