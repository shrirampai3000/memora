...
//! plenty â€” and if a renderer ever dropped the mode, we re-enable on the next
//! TTL cycle.
//!
...
///
/// Chosen to be long enough that the per-walk cost disappears (~20Ã— reduction
...
/// * Stale entries older than `EVICT_AFTER_TTLS Ã— ttl` are dropped on every
///   [`Self::should_enable`] call â€” no background thread needed.
#[derive(Debug)]
...
impl EnhancedModeCache {
    ...
    ///
    /// Returning `false` means "the renderer has been poked recently â€” skip".
    ...
    pub(crate) fn should_enable_at(&self, pid: i32, now: Instant) -> bool {
        ...
        // map state is still valid in that case â€” recover the inner guard.
    ...
    /// it disappears for `EVICT_AFTER_TTLS Ã— ttl`.
    ///
    ...
    /// often duplicates) it into the focused field â€” the "phantom text" bug.
...
mod tests {
    ...
    fn stale_entries_are_evicted_on_access() {
        ...
        // Jump past EVICT_AFTER_TTLS Ã— ttl and touch pid 30. Prior entries
    ...
    fn concurrent_access_serializes_cleanly() {
        ...
        // 8 threads Ã— 50 pids = 400 unique pids inserted.
    ...
    fn poisoned_mutex_still_serves_reads() {
        ...
        // the cache â€” we intentionally recover from poison.
        use std::sync::Arc;
        use std::thread;
    ...
    fn should_enable_once_never_reasserts_while_present() {
        ...
        // Seen regularly thereafter (each gap < eviction window) â†’ never
        ...
        let mut t = t0;
        for _ in 0..20 {
            t += Duration::from_secs(60); // 60s..1200s, all < EVICT_AFTER_TTLSÃ—ttl (300s) per gap
    ...
    fn should_enable_once_reenables_after_pid_disappears() {
        ...
        // Gone long enough to be evicted (not seen for > EVICT_AFTER_TTLS Ã— ttl),
        // then re-focused â†’ poke once more.
