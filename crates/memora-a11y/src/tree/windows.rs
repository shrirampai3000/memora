...
//! the focused window's tree and extract all visible text â€” matching macOS behavior.
...
/// Excluded apps â€” password managers and security tools (matches macOS list).
...
/// After the initial send, it is never moved again â€” all access is single-threaded.
/// The `Send` bound is required by `TreeWalkerPlatform` (for `Box<dyn â€¦>` transfer).
...
impl TreeWalkerPlatform for WindowsTreeWalker {
    fn walk_focused_window(&self) -> Result<TreeWalkResult> {
        ...
        // Resolve logical app name â€” handles WebView2 and shell-hosted Edge.
        let app_name = crate::platform::windows::get_effective_app_name(hwnd, pid);

        // Skip excluded apps
        let app_lower = app_name.to_lowercase();
        if EXCLUDED_APPS.iter().any(|ex| app_lower.contains(ex)) {
            debug!(app = %app_name, pid, "a11y: skipped â€” hardcoded excluded app");
            return Ok(TreeWalkResult::Skipped(SkipReason::ExcludedApp));
        }
        ...
        // Items) â€” same exemption as the periodic worker path. OCR still captures
        // the on-screen content for these apps.
        if crate::platform::windows_uia::is_fragile_uia_tree_provider(&app_name) {
            debug!(app = %app_name, pid, "a11y: skipped â€” fragile UIA tree provider (crash-prone)");
            return Ok(TreeWalkResult::Skipped(SkipReason::ExcludedApp));
        }
        ...
        debug!(
            ...
            pid,
            title = %window_name,
            "a11y: walk_focused_window â€” evaluating window"
        );
        ...
        {
            debug!(app = %app_name, title = %window_name, "a11y: skipped â€” incognito/private window");
        ...
        if window_pattern::matches_any(ignored_patterns.as_ref(), &app_lower, &window_lower) {
            debug!(
                ...
                "a11y: skipped â€” matched user ignored pattern"
            );
        ...
        if !window_pattern::passes_includes(included_patterns.as_ref(), &app_lower, &window_lower) {
            debug!(
                ...
                "a11y: skipped â€” not in include list"
            );
        ...
        // Stored once per walk â€” GetWindowRect is cheap and the focused
        ...
        // for any unknown app or any failure â€” never panics.
        let document_path =
        ...
        // pixels from its own frame. (macOS gets this for free â€” it
        ...
        // clamps away.) Mismatch or unverifiable â†’ None â†’ full-frame
...
/// â€” distinct from `MonitorRect` because we need the *window* clip, not
...
/// fallback) â€” safe. Unconfigured monitor dims â†’ false (can't verify).
...
/// window's rect â€” the on-screen visibility test for issue #2436.
...
fn extract_text_from_tree(
    ...
    // bounds aren't available â€” caller's filter logic must treat that
    ...
    if TEXT_TYPES.iter().any(|&t| ct.eq_ignore_ascii_case(t)) {
        ...
        // Note: Document is handled separately below â€” its children are the
        ...
        if matches!(ct, "Edit" | "ComboBox") {
            ...
            if let Some(ref val) = node.value {
                if !val.trim().is_empty() {
                    ...
                    // Don't recurse into text controls â€” their children are sub-elements of the same text
                    return;
                }
            }
        }
        ...
        // node is the root of the entire web content tree â€” its children are the
        ...
        if ct.eq_ignore_ascii_case("Document") {
            ...
            if !ignored_patterns.is_empty() {
                ...
                // matches an ignored pattern â€” extension UIs always render their
            ...
            if let Some(ref val) = node.value {
                ...
                if !trimmed.is_empty() {
                    ...
                    if trimmed.starts_with("http://") || trimmed.starts_with("https://") {
                        ...
                        // Non-URL value â€” treat as text content
            ...
            // Always recurse into Document children â€” they contain the actual content
        } else {
...
mod tests {
    ...
    fn test_incognito_detection() {
        ...
        // indicators (false-positive reduction â€” see incognito/titles.rs).
