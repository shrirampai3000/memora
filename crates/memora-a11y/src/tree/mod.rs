...
//! Accessibility tree walker â€” periodically walks the macOS AX tree of the focused window,
...
// returns `None` for unknown apps, missing files, or any error â€” call
...
use memora_core::window_pattern::WindowPattern;
...
/// using **char counts** (Unicode scalar values), not byte offsets â€” the same
...
pub struct AccessibilityTreeNode {
    ...
    /// capture time, computed from `element_rect âˆ© focused_window_rect`.
    ///
    /// Why: the AX tree captures off-screen text â€” terminal scroll buffers,
    /// scrolled-off lines in IDE editors, hidden/clipped overflow regions
    /// â€” that the user can't actually see in the captured image. Without
    ...
    /// is the proper fix for those cases â€” see issue #2436.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// AND the node looks multi-line (height > ~1.5Ã— a single text line).
    ...
    /// instead of painting the whole paragraph yellow â€” `narrow_bbox_to_needle`
...
/// window's rect â€” the visibility test for issue #2436.
///
...
/// `Option::None` rather than calling this with a zero-size window â€”
...
/// alternative â€” strict containment â€” would drop edge-clipped text the
/// user clearly saw.
...
pub fn rects_intersect(
    ...
    // Zero-area rects render no pixel â€” they cannot be visible. AX trees
...
/// capacity implied by the bounds aspect ratio. `min_height_ratio â‰¥ 1.0`
/// adds a safety factor â€” `1.5` requires text to be ~1.5Ã— wider than one
...
pub fn node_looks_multiline(text: &str, bounds: &NodeBounds, min_height_ratio: f32) -> bool {
    ...
    // width â‰ˆ 0.55 Ã— line height, with 1.6Ã— slack for variable fonts.
...
pub enum TruncationReason {
    /// Walk completed naturally â€” visited all reachable nodes.
    None,
...
/// these are deliberately NOT clamped to 0-1 â€” a window dragged past the
...
pub struct TreeSnapshot {
    ...
    /// `browser_url` â€” extractors here MUST skip non-file scheme so we don't
    ...
    /// Locality-sensitive hash for fuzzy dedup (similar text â†’ small hamming distance).
    pub simhash: u64,
    ...
    /// monitor's extent â€” see [`WindowBounds`]. `None` when the platform
...
pub struct TreeWalkerConfig {
    ...
    /// User-configured windows to include (whitelist â€” if non-empty, only these are captured).
    ...
    /// browser tab's URL â€” see [`crate::url_filter::is_url_blocked`]).
    ...
    /// Cap on parameterized AX calls per multi-line node â€” prevents pathological
    ...
    /// A node's height must exceed this multiple of `line_height_est = width Ã— 0.55 / 1.6`
    /// before line capture runs â€” single-line text already highlights tightly via
...
pub enum SkipReason {
    ...
    Incognito,
    /// App is in the hardcoded exclusion list (e.g. MEMORA, keychain access).
    ExcludedApp,
...
fn apply_focused_window_filters(
    ...
    if memora_core::window_pattern::matches_any(
    ...
    if !memora_core::window_pattern::passes_includes(
...
mod tests {
    ...
    fn test_simhash_similar() {
        // Realistic page content â€” scrolling changes a few lines out of many
    ...
    // ---------------------------------------------------------------
    // rects_intersect â€” issue #2436 visibility geometry
    // ---------------------------------------------------------------

    /// Window at (100, 200) of size 800Ã—600 â€” used as the reference frame
    ...
    fn rects_intersect_element_fully_inside_window() {
        let (wx, wy, ww, wh) = win();
        // Centered 100Ã—100 element clearly inside the window.
    ...
    fn rects_intersect_element_fully_outside_window_below() {
        let (wx, wy, ww, wh) = win();
        // Element below the window â€” common case for terminal scroll
    ...
    fn rects_intersect_element_fully_outside_window_right() {
        let (wx, wy, ww, wh) = win();
        // Element to the right of window â€” horizontal scroll-buffer case.
        assert!(!rects_intersect(
    ...
    fn rects_intersect_element_partially_overlapping() {
        let (wx, wy, ww, wh) = win();
        // Element clipped at the bottom edge â€” partial visibility counts
    ...
    fn rects_intersect_element_just_above_window() {
        let (wx, wy, ww, wh) = win();
        // Element that ends exactly at window top â€” no overlap (`<` is
    ...
    fn rects_intersect_zero_size_element() {
        let (wx, wy, ww, wh) = win();
        // Degenerate element with zero width â€” no rendered pixel, so
    ...
    fn rects_intersect_element_around_window() {
        let (wx, wy, ww, wh) = win();
        // Element bigger than (and containing) the window â€” every window
    ...
    // ---------------------------------------------------------------
    // node_looks_multiline â€” capture-time gate for per-line geometry
    // ---------------------------------------------------------------

    /// Build a NodeBounds at (0, 0) with the given width and height â€” the
    ...
    fn multiline_short_label_in_wide_box_is_single_line() {
        // "ok" in a 100Ã—20 box: width / 0.55 * 1.6 â‰ˆ 14.5 chars per line. 2 chars fits.
        assert!(!node_looks_multiline("ok", &bb(0.1, 0.02), 1.5));
    }

    #[test]
    fn multiline_paragraph_in_narrow_tall_box_is_multiline() {
        // Long paragraph: ~400 chars in a near-square bounds â†’ clearly multi-line.
    ...
    fn multiline_borderline_at_safety_factor_1_5() {
        // Aspect 5.0 â†’ chars_per_line_est â‰ˆ 14.5. With ratio 1.5 â†’ threshold â‰ˆ 21.7.
        // 22 chars: just over â†’ multi-line. 18 chars: under â†’ single.
    ...
    fn multiline_min_ratio_clamped_to_one() {
        ...
        let bounds = bb(0.5, 0.1); // chars_per_line_est â‰ˆ 14.5
                                   // 16 chars > 14.5 â†’ should be multi-line even with ratio < 1.
    ...
    fn multiline_cjk_no_whitespace_still_evaluated() {
        ...
        let cjk: String = "æ¼¢å­—".repeat(50); // 100 chars total
        assert!(node_looks_multiline(&cjk, &bb(0.4, 0.4), 1.5));
    }

    // ---------------------------------------------------------------
    // LineBudget â€” per-frame call/time budget for line capture
    // ---------------------------------------------------------------
    ...
    fn budget_consumes_calls_until_cap() {
        ...
        // 8 used, requesting 5 more â†’ over cap, denied
        assert!(!b.try_consume(5));
    ...
    fn budget_zero_calls_always_fits_until_time_out() {
        ...
        // Zero-call requests don't consume â€” but they still pass the deadline check.
    ...
    // ---------------------------------------------------------------
    // LineSpan / AccessibilityTreeNode â€” serde round-trip and back-compat
    // ---------------------------------------------------------------
