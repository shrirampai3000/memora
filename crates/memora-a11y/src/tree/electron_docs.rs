...
//! Xcode, Notes, BBEdit, â€¦) populate `AXDocument` on the focused window
//! with a `file://` URL we can read in one IPC call. Electron apps don't
//! â€” they aren't `NSDocument` subclasses, and `AXDocument` returns
...
//! supported OS â€” `~/Library/Application Support` (macOS), `%APPDATA%`
//! (Windows, i.e. `C:\Users\<user>\AppData\Roaming`), `~/.config`
//! (Linux). Snap / Flatpak Linux installs use different roots (e.g.
//! `~/snap/code/common/.config/Code/`) and are not covered yet â€” those
...
/// Multiple display names map to the same fork resolver â€” VS Code,
...
/// negligible â€” `catch_unwind` is essentially free on the happy path.
...
mod obsidian {
    ...
    ///      Obsidian knows about. Exactly one has `"open": true` â€”
    ...
    ///
    /// Returns `None` if the dir can't be resolved (rare â€” happens on
    ...
    fn find_leaf_file_inner(node: &Value, active_id: &str, depth: usize) -> Option<String> {
        ...
        if let Some(obj) = node.as_object() {
            ...
            if is_active_leaf {
                ...
                // "search", â€¦ don't have a backing file.
...
pub(super) mod vscode_fork {
    ...
    /// memento keys) â€” they differ only in the support-dir name and
    /// the menubar.
    ...
    ///   2. Pick the most recently modified entry â€” that's the focused
    ...
    pub(super) fn active_path(support_dir: &str) -> Option<String> {
        ...
        // Cache by db path â€” when the user is actively editing one
    ...
    /// layout on every supported OS â€” only the support-dir name
    ...
    ///
    /// We stat the `state.vscdb` file itself, not the parent dir â€”
    ...
    /// the connection â€” no WAL replay, no SHM/WAL temp files, no
    ...
    /// Walk the editor memento â†’ activeGroup â†’ first editor's
    /// serialized input â†’ `resourceJSON.fsPath`.
    ///
    ...
    /// `editors[i].value` is a JSON-encoded *string* â€” must parse twice.
...
/// Returns `None` if the file can't be read or parsed â€” caller decides
...
mod tests {
    ...
    /// cross-platform â€” only the fixture layout is OS-specific.
    #[test]
    #[cfg(target_os = "macos")]
    fn vscode_fork_active_path_against_real_sqlite() {
        ...
        // Drive the public entry point â€” workspace_storage_root() reads
    ...
    fn read_cached_returns_none_for_missing_file() {
        ...
        let nonexistent = PathBuf::from("/tmp/__MEMORA_does_not_exist_xyz.json");
    ...
    fn read_cached_caches_within_ttl() {
        ...
        // Truncate the file â€” read_cached should still return the
