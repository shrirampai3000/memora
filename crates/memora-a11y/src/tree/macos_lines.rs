...
//! against such a bbox paints the entire paragraph yellow â€” a poor UX. This
...
//!
//! Per multi-line node:
//!   - 1 Ã— `AXLineForIndex` (last char â†’ last line index)
//!   - For each line: 1 Ã— `AXRangeForLine` + 1 Ã— `AXBoundsForRange` = 2 calls
...
// ---------------------------------------------------------------------------
// FFI shim â€” `AXUIElementCopyParameterizedAttributeValue` is in
...
fn copy_param_attr(
    ...
    if status.is_ok() {
        ...
        // page can hit this thousands of times â€” the first failure already
        ...
        if !WARNED.swap(true, Ordering::Relaxed) {
            warn!(
                "lines: AXUIElementCopyParameterizedAttributeValue({}) failed status={:?} \
                 â€” first failure (further failures suppressed); search highlights will fall \
...
/// `AXLineForIndex(charIndex: Int) -> Int` â€” returns the visual line index
...
/// `AXRangeForLine(lineIndex: Int) -> CFRange` â€” returns the UTF-16 range
...
/// implement `AXLineForIndex` but not `AXRangeForLine` â€” calls fail with
...
/// inside fails â€” caller bails to paragraph bbox.
///
...
/// `AXBoundsForRange(range: CFRange) -> CGRect` â€” returns the screen-absolute
...
fn bounds_for_range(elem: &ax::UiElement, range: cf::Range) -> Option<cg::Rect> {
    ...
    // Cast `&ax::Value` to `&cf::Type` â€” Value derefs to cf::Type via define_cf_type!
...
// ---------------------------------------------------------------------------
// UTF-16 â†” Unicode scalar conversion
// ---------------------------------------------------------------------------

/// Convert a UTF-16 (loc, len) range â€” what `AXRangeForLine` returns â€” into
...
/// on char boundaries (defensive â€” AX should always return aligned ranges,
...
/// relative bounds â€” mirrors the existing `normalize_bounds` in `macos.rs`
...
/// - `AXLineForIndex(charIdx) -> Int`     â† supported by Chromium + AppKit
/// - `AXRangeForLine(lineIdx) -> CFRange` â† **not** supported by Chromium
/// - `AXBoundsForRange(CFRange) -> CGRect` â† supported by both
///
...
pub(crate) fn capture_line_spans(
    ...
    //    bails immediately â€” caller's heuristic was over-eager.
    if !budget.try_consume(1) {
        return None;
    }
    ...
    //    (line N's start â‰¥ line N-1's start).
    ...
    if spans.len() < 2 {
        // Need â‰¥2 spans for line-aware highlighting to be useful â€” if we
...
// ---------------------------------------------------------------------------
// Tests â€” UTF-16 conversion is the only non-FFI logic, so test it hard.
...
mod tests {
    ...
    fn utf16_ascii_round_trip() {
        // "hello world" â€” all BMP, char count == utf16 count
    ...
    fn utf16_emoji_supplementary_plane() {
        // ðŸ¦€ is U+1F980, encoded as a UTF-16 surrogate pair (2 code units, 1 char).
        let text = "abðŸ¦€cd";
        // Char count: a, b, ðŸ¦€, c, d â†’ 5 chars
        // UTF-16: a=1, b=1, ðŸ¦€=2, c=1, d=1 â†’ 6 code units total
        // Range [0, 4) covers "abðŸ¦€" (1+1+2=4 utf16, 3 chars)
    ...
    #[test]
    fn utf16_cjk_in_bmp() {
        // CJK ideographs are BMP â€” 1 char = 1 utf16 code unit, like ASCII.
        let text = "æ¼¢å­—ãƒ†ã‚¹ãƒˆ";
        assert_eq!(utf16_range_to_char_range(text, 0, 5), Some((0, 5)));
    ...
    fn utf16_unaligned_range_returns_none() {
        // ðŸ¦€ occupies utf16 [2, 4). A range [3, 1) lands inside the surrogate
        // pair â€” no valid char boundary, so we reject it.
        let text = "abðŸ¦€cd";
    ...
    fn utf16_range_past_end_returns_none() {
        let text = "abc";
        // Past-the-end range â€” no matching char_start
        assert_eq!(utf16_range_to_char_range(text, 5, 1), None);
        // Range starts inside but extends past â€” should fail the precise
        // boundary check
    ...
    fn normalize_refs_monitor_relative_clamps_to_unit_square() {
        ...
        // Full rect â†’ top-left quarter of the monitor.
    ...
    fn normalize_refs_rejects_offscreen_rect() {
        ...
        // Rect way past the monitor's right edge â€” left ratio > 1.1
