...
//!
//! D-Bus is local socket IPC, typically 0.1â€“0.5ms per call â€” comparable to
//! macOS AX IPC cost.
//!
//! ## Compatibility
//!
...
/// AT-SPI2 `TEXT_BOUNDARY_LINE_START` â€” boundary type passed to
...
/// AT-SPI2 `ATSPI_COORD_TYPE_SCREEN` â€” coordinate frame for `GetRangeExtents`.
...
/// Reference into an AT-SPI2 accessible â€” bus name + object path. Mirrors
...
/// implemented, or the loop fails to advance (defensive â€” protects against
...
pub(crate) fn capture_line_spans(
    ...
    if spans.len() <= 1 {
        ...
        // disagreed with AT-SPI's wrap calculation) â€” drop the lines list so
...
// ---------------------------------------------------------------------------
// D-Bus helpers â€” thin wrappers around `Text.GetStringAtOffset` and
...
/// `Text::GetStringAtOffset(offset, type) â†’ (s, startOffset, endOffset)`.
...
/// `Text::GetRangeExtents(startOffset, endOffset, coordType) â†’ (x, y, w, h)`.
