...
const EXCLUDED_APPS: &[&str] = &[
    ...
    "polkit",
    // Window managers / desktop shells â€” no user content
    "xfwm4",
    "mutter",
    "muffin",
    "kwin",
...
fn get_accessible_children(conn: &Connection, aref: &AccessibleRef) -> Vec<AccessibleRef> {
    ...
    for i in 0..child_count {
        if let Ok(reply) = dbus_call(
            ...
            // Return type is (so) â€” bus name + object path
...
fn find_url_in_children(
    ...
    for child in children {
        ...
        // Entry (79) or ComboBox (11) â€” might contain URL
        if role == 79 || role == 11 {
...
fn extract_text(
    ...
    // the search filter â€” see issue #2436.
...
/// macOS helper of the same name â€” see `tree/macos.rs::capture_lines_for_node`
/// for the rationale.
...
/// window's screen rect â€” see issue #2436. Delegates to the shared
...
/// 1. IsEnabled=true on session bus â€” checked at startup (future launches)
/// 2. RegisterEvent on a11y bus â€” emits EventListenerRegistered signal (running apps)
...
impl TreeWalkerPlatform for LinuxTreeWalker {
    fn walk_focused_window(&self) -> Result<TreeWalkResult> {
        ...
        // Scoped includes act as per-app whitelists â€” see
        ...
        // file, or read error â€” never panics. AT-SPI's Document
...
mod tests {
    ...
    fn test_incognito_detection() {
        ...
        // not password prompts â€” "Enter Password - Chrome" is a sign-in dialog
