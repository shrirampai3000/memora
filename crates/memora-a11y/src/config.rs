...
pub struct UiCaptureConfig {
    ...
    /// UIA calls serviced on the *target app's* UI thread â€” measured at p95
    /// 213ms message-pump stalls in Edge, and responsible for ~100% of severe
    /// foreground freezes (see TESTING.md Â§15). The resulting snapshots also
    /// have no consumers â€” `paired_capture.rs` owns accessibility text capture.
    ...
    /// Supports `App::Title` scoping â€” see `memora-core::window_pattern`.
    #[serde(default)]
    ...
    ///   1. `mouse_hook_proc` / `keyboard_hook_proc` blocking locks â†’ `try_lock` (fall back to None on contention)
...
impl Default for UiCaptureConfig {
    fn default() -> Self {
        Self {
            ...
            // applies there â€” a heavy browsing day is a few thousand rows,
            ...
            // Bare "password"/"secret" patterns removed â€” too many false
...
impl UiCaptureConfig {
    ...
    /// so only legacy (unscoped) ignore patterns can block here â€” scoped
    ...
    pub fn should_capture_target(&self, app_name: &str, window_title: Option<&str>) -> bool {
        ...
        // Scoped ignore patterns (e.g. `Slack::#general`) are evaluated here â€”
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert!(!config.capture_tree); // Off by default â€” freezes foreground apps on Windows
    ...
    fn test_window_exclusion() {
        // Defaults have no window patterns â€” password/secret patterns were
    ...
    fn test_user_window_filters() {
        ...
        config.included_windows = vec!["Chrome".to_string(), "MEMORA".to_string()];

        assert!(config.should_capture_target("Chrome", Some("Docs")));
        assert!(config.should_capture_target("Terminal", Some("MEMORA logs")));
    ...
    fn test_scoped_include_per_app_whitelist() {
        ...
        // Greenhouse; other apps stay unaffected (regression target â€” naive
    ...
    fn test_cached_pattern_path_is_consistent_with_lazy_path() {
        // Exercise both code paths in `resolved_ignored()` â€” verify they
        ...
        // Note: NOT calling compile_patterns â€” hits the lazy fallback.
