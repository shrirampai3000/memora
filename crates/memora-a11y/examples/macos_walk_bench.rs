...
//! End-to-end walk bench for Fix 2 in ~/MEMORA-notes/"AX Walk Speed Fixes.md".
//!
...
//! identical across both builds (the hard invariant â€” batching may not change a
//! single emitted byte).
//!
//! Usage: `cargo run --release -p memora-a11y --example macos_walk_bench [iters] [--semantic|--automation] [--expect-identifier ID] [--timeout-ms N]`
...
fn main() {
    ...
    if durations.is_empty() {
        eprintln!("no successful walks â€” focus a window and grant Accessibility permission");
        std::process::exit(1);
    }
    ...
    println!(
        "content_hash: {last_hash:#018x}  ({} distinct across run â€” expect 1 on a static window)",
        hashes.len()
    );
    println!(
