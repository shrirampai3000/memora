...
//! Micro-bench for the Fix-2 plan in ~/MEMORA-notes/"AX Walk Speed Fixes.md":
...
//!
//! Usage: `cargo run -p memora-a11y --example macos_axbatch_bench [pid] [max_nodes]`
    ...
    for round in 1..=4 {
        ...
        for pass in 0..2 {
            if (pass == 0) == batched_first {
                ...
                println!(
                    "round {round} BATCHED   : {ms:7.1}ms total, {:6.1}Âµs/node ({got}/{} ok)",
                ...
                let ms = t.elapsed().as_secs_f64() * 1000.0;
                println!(
                    "round {round} INDIVIDUAL: {ms:7.1}ms total, {:6.1}Âµs/node ({got} attr hits)",
                    ms * 1000.0 / nodes.len() as f64
                );
            }
        }
