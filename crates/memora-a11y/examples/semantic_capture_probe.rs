...
//! `cargo run -p memora-semantic --example replay -- --jsonl` consumes, so
...
//! local evaluation input only â€” the same status as a user's own frames.
//! Never commit probe output; commit only the replay metrics derived from it.
//!
//! Usage:
//!   cargo run -p memora-a11y --example semantic_capture_probe -- \
