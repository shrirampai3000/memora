...
//! Standing diagnostic for the AXWebArea â†’ AXURL browser-url path (Fix 1 in
//! ~/MEMORA-notes/"AX Walk Speed Fixes.md"). Against the FRONTMOST app it
//! measures, per iteration:
//!   1. focused app + window resolution cost,
//!   2. a depth-capped children()-only DFS to the first AXWebArea â€” nodes
...
//!
//! Usage: `cargo run -p memora-a11y --example macos_weburl_probe [depth_cap] [iters] [pid]`
...
fn main() {
    ...
    for i in 1..=iters {
        ...
        match hit {
            ...
            None => {
                println!(
                    "iter {i}: window={title:?} resolve={resolve_ms:.1}ms | NO web area within depth {depth_cap} â€” visited={visited} wasted={descent_ms:.1}ms | tier1 AXDocument={ax_document:?}"
                );
            }
        }
        std::thread::sleep(std::time::Duration::from_millis(400));
