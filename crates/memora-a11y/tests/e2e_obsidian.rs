...
//!
//! See https://github.com/MEMORA/MEMORA/issues/3002 â€” captured frames
...
//!
//! ```sh
//! cargo test -p memora-a11y --test e2e_obsidian -- --test-threads=1 --nocapture --ignored
//! ```
//!
...
#[cfg(target_os = "macos")]
mod obsidian {
    use memora_a11y::tree::{create_tree_walker, TreeWalkResult, TreeWalkerConfig};
    use std::path::Path;
    ...
    fn walker() -> Box<dyn memora_a11y::tree::TreeWalkerPlatform> {
    ...
    /// the bug â€” if text comes back non-empty here, the AX path is fine and
    ...
    fn obsidian_walk_returns_non_empty_text() {
        ...
        // Short settle â€” matches the production pipeline which walks almost
        ...
        match result {
            TreeWalkResult::Found(snap) => {
                ...
                assert_eq!(
                    ...
                    "walker returned Found but app_name is not 'Obsidian' â€” pid/RunningApp mismatch"
                );
                ...
                assert!(
                    ...
                    "Obsidian walk produced empty text (nodes={}). This is issue #3002. \
                     Hypothesis test: retry with longer settle â€” see `obsidian_walk_retries_after_settle`.",
                    snap.node_count
                );
            }
            TreeWalkResult::NotFound => panic!(
                "walker returned NotFound â€” focused_window AX query failed. \
                 Obsidian may have no window focused, or AX access is denied."
            ),
            TreeWalkResult::Skipped(reason) => panic!(
                "walker skipped Obsidian with reason={:?} â€” unexpected, \
    ...
    ///
    /// This test doesn't assert a pass/fail for the hypothesis â€” it prints the
    ...
    fn obsidian_walk_retries_after_settle() {
        ...
        // Walk 2: after a short settle â€” enough for Chromium to push its
        ...
        // Walk 3: after a full second â€” enough for anything async.
        ...
        assert!(
            ...
            "all three walks returned empty text (lens={lens:?}, nodes={nodes:?}). \
             Async-settle hypothesis disproven â€” likely need AXManualAccessibility \
        ...
        // This is informational, not enforced â€” user input (typing, cursor
        // movement) can shrink visible text between walks.
        if lens[0] < lens[2] {
            println!(
                "  async-settle hypothesis SUPPORTED: text grew from {} â†’ {} over 1s",
                lens[0], lens[2]
            );
        }
    }

    fn measure(walker: &dyn memora_a11y::tree::TreeWalkerPlatform) -> (usize, usize) {
    ...
    /// `ns::RunningApp::with_pid` returns None and `app_name` ends up empty â€”
    ...
    fn obsidian_focused_pid_is_main_process() {
        ...
        assert!(
            running.is_some(),
            "NSRunningApplication::with_pid({pid}) returned None â€” AX handed us a helper-process pid \
        ...
        assert_eq!(
            name, "Obsidian",
            "pid {pid} resolves to {name:?}, not 'Obsidian' â€” wrong process"
        );
    }
}
