...
//!
//! ```sh
//! cargo test -p memora-a11y --test e2e_tree_walker -- --test-threads=1 --nocapture
//! ```
//!
//! **Requirements**:
...
mod e2e {
    ...
    fn create_walker(max_nodes: usize) -> Box<dyn memora_a11y::tree::TreeWalkerPlatform> {
    ...
    fn test_textedit_type_and_capture() {
        ...
        let test_text = "MEMORA accessibility test 12345";
        ...
        if let TreeWalkResult::Found(snap) = walk_result {
            ...
            if snap.app_name == "TextEdit" {
                assert!(
                    snap.text_content.contains("MEMORA accessibility test"),
    ...
    fn test_cache_dedup_real_window() {
        ...
        // Use whatever app is currently focused â€” walk twice rapidly
        ...
        // Walk again immediately â€” no user interaction, content should be stable
    ...
    // =========================================================================
    // Test: Performance â€” walk 50 times, measure CPU and timing
    // =========================================================================
    #[test]
    ...
    fn test_rapid_app_switching_stress() {
        ...
        if deduped > 0 {
            ...
            println!(
                "  No dedup triggered â€” content changed between all visits (normal for dynamic apps)"
            );
        }
    ...
    // =========================================================================
    // Test: Sustained walking over 10 seconds â€” measure CPU load
    // =========================================================================
    #[test]
