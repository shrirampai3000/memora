...
/// Migrate legacy secret files from `~/.memora` into the unified SecretStore.
///
...
pub async fn migrate_legacy_secrets(
    ...
    let fixed = fix_secret_file_permissions(memora_dir);
    if fixed > 0 {
        ...
        for entry in entries.flatten() {
            ...
            if filename.ends_with("-oauth.json") && path.is_file() {
                ...
                match std::fs::read(&path) {
                    Ok(contents) => {
                        // Check if already migrated â€” but re-import if file is newer
                        ...
                        if let Err(e) = store.set(&store_key, &contents).await {
                            ...
                            // Keep the legacy file â€” Phase 1 per module doc. Readers
    ...
    // Migrate connections.json â€” store each full connection entry under `cred:{name}`
    // so ConnectionManager can read credentials from SecretStore (encrypted).
    let connections_path = memora_dir.join("connections.json");
...
/// Walk `memora_dir` and set 0o600 on any file matching secret-like patterns.
/// Returns the count of files whose permissions were fixed.
pub fn fix_secret_file_permissions(memora_dir: &Path) -> usize {
    let mut count = 0;
    ...
    // Walk whatsapp-session/ directory recursively â€” but only once.
    // This can be 7000+ files; skip if already done (marker file).
    let whatsapp_dir = memora_dir.join("whatsapp-session");
    let whatsapp_marker = memora_dir.join(".whatsapp-perms-fixed");
...
mod tests {
    ...
    async fn test_migrate_skips_already_migrated() {
        ...
        // First migration â€” file migrated (kept for legacy readers)
        ...
        // Second migration â€” already in store, skipped
