...
const SERVICE: &str = "com.memora.app";
...
pub enum KeyResult {
    ...
    /// Do NOT create a new key â€” disable encryption instead.
    AccessDenied,
    /// Keychain is not available on this platform/environment.
    Unavailable,
}

/// Cached availability check â€” probed once per process.
...
pub fn is_keychain_available() -> bool {
    *AVAILABLE.get_or_init(|| {
        ...
        {
            let probe = format!("__MEMORA_probe_{}", std::process::id());
...
pub fn get_or_create_key() -> Option<[u8; 32]> {
    match get_key() {
        ...
        KeyResult::AccessDenied => {
            warn!("keychain: access denied â€” cannot create or retrieve encryption key");
            return None;
        }
...
// â”€â”€ Platform-specific keychain access â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
