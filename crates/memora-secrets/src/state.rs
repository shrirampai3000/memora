...
//! monitor all agree on whether the keychain should be touched. Read-only â€”
...
/// - `MEMORA_ENCRYPT_STORE=1` env var (testing/override)
...
pub fn is_encryption_requested(data_dir: &Path) -> bool {
    if std::env::var("MEMORA_ENCRYPT_STORE").is_ok_and(|v| v == "1") {
        return true;
    }
...
/// Mark encryption as opted out. Idempotent â€” succeeds if the flag was already
