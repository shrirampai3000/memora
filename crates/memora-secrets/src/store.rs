...
impl SecretStore {
    ...
    pub async fn set(&self, key: &str, value: &[u8]) -> Result<()> {
        ...
        let (stored_value, nonce) = match &self.key {
            ...
            None => {
                // No encryption key â€” store as base64 with zero nonce
        ...
        match row {
            ...
            Some((stored_value, nonce)) => {
                ...
                let plaintext = if is_plaintext {
                    ...
                    // Value is encrypted but we don't have a key â€” can't decrypt
                    anyhow::bail!(
...
mod tests {
    ...
    async fn test_plaintext_written_read_by_encrypted_store() {
        ...
        // The zero nonce signals plaintext â€” should decode as base64
        let pool = SqlitePool::connect(":memory:").await.unwrap();
    ...
    async fn test_encrypted_written_read_by_plaintext_store() {
        // Tauri writes encrypted, CLI reads without key â€” should error
        let pool = SqlitePool::connect(":memory:").await.unwrap();
    ...
    // â”€â”€ #4263: shared secret-store pool (no ad-hoc per-call connections) â”€â”€â”€â”€â”€â”€

    /// `open()` returns a working store backed by the shared pool and creates
    /// the db file on open â€” preserving the old `?mode=rwc` behavior exactly.
    #[tokio::test]
    #[serial]
    ...
    /// `close_all_secret_pools` closes every cached pool â€” releasing its handle
    /// on the db file and the shared `-shm` WAL-index so a wedge recovery can
    /// rebuild it â€” and the cache re-populates lazily on the next open.
    #[tokio::test]
    #[serial]
    async fn close_all_secret_pools_closes_and_allows_reopen() {
        ...
        // the previously-cached pool is now closed â€” its handle on the db file
    ...
    /// `SqlitePool::connect`-per-op pattern was unbounded churn â€” the WAL-index
    /// thrash this fixes.
    #[tokio::test]
    #[serial]
    async fn shared_pool_is_reused_not_recreated() {
        ...
        assert!(
            pool.size() <= 1,
            "shared pool must stay bounded (got {} connections) â€” proves reuse, not per-call pools",
            pool.size()
        );
    }
    ...
    async fn first_migration_refuses_a_quarantined_capture_generation() {
        ...
        let legacy = SqlitePoolOptions::new()
            ...
            .unwrap();
        legacy.close().await;
        memora_sqlite_coordinator::prepare_sqlite_quarantine_reserve(&legacy_path).unwrap();
        assert!(memora_sqlite_coordinator::latch_sqlite_hard_fault(
            &legacy_path,
            13
        ));
    ...
    async fn repro_adhoc_pool_churn_contends() {
        ...
        for i in 0..128 {
            let db = db_str.clone();
            set.spawn(async move {
                // OLD pattern: fresh pool per op, no wait â€” the ad-hoc churn.
                let opts = SqliteConnectOptions::new()
                    .filename(&db)
                    .create_if_missing(true)
