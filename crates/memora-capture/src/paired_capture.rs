...
/// Apple Vision extracts as long digit globs: "93154155156157158159â€¦". These
...
/// almost certainly a gutter â€” real prose rarely has that density. Phone
...
/// is therefore bounded either way â€” this semaphore is the equivalent gate for
...
/// extraction plus OCR), and â€” for the blocking-pool paths â€” move the permit
...
/// Reject window crops smaller than this on either side â€” resize/move races
...
/// Clamp focused-window bounds to the frame. Returns `None` (â†’ full-frame
...
/// Context for a paired capture operation â€” replaces positional arguments.
pub struct CaptureContext<'a> {
    ...
    pub languages: Vec<memora_core::Language>,
    ...
    /// The accessibility tree walk still runs â€” metadata row is still written.
    ...
    /// the meeting window â€” side content was/will be a11y-indexed when
...
pub struct PairedCaptureResult {
    ...
    pub duration_ms: u64,
    /// Wall-clock of the OCR step in ms â€” `Some` only when OCR actually ran
    ...
    /// True when OCR ran but produced (near-)empty text â€” an OCR-quality
    /// failure proxy. False when OCR didn't run or returned usable text.
    pub ocr_was_empty: bool,
    /// How the OCR gate resolved this capture â€” `Some` only when an OCR
    ...
    /// Wall-clock of the gate's text-region detect pass â€” `Some` only when
...
pub async fn paired_capture(
    ...
    // Write JPEG snapshot to disk â€” skipped when screenshot_disabled (AudioPaused / FullPause).
    ...
    let app_prefers_ocr = !ctx.screenshot_disabled
        && app_name.is_some_and(|name| {
            ...
            // Note: Ghostty, iTerm2, and Terminal.app were removed â€” they have
            // full AX support and the thin-detection heuristic handles them
            // correctly. See https://github.com/MEMORA/MEMORA/issues/2685
            n.contains("wezterm")
    ...
    // Check if accessibility text is "thin" â€” the tree returned SOME text
    ...
    // Meeting apps/URLs are deliberately no longer part of this check â€”
    ...
    // to the current crop position â€” frames whose only text source is OCR
    ...
    if wants_ocr {
        match ocr_gate.as_deref_mut() {
            Some(gate) => {
                ...
                // The gated OCR pipeline (#5060) â€” applies to EVERY OCR
                // trigger, not just meetings: screenshot â†’ crop to the app
                // window â†’ detect text â†’ crop to the padded union of the
                // detected text â†’ pixel-compare that crop to the last
                // indexed one â†’ different? OCR that same crop. One blocking
                ...
                match union_and_sig {
                    ...
                    // indexed state is deliberately untouched â€” text that
                    ...
                    Some((union, signature, region_count)) => {
                        ...
                        match decision {
                            OcrDecision::Skip => {
                                ocr_gate_decision = Some(OcrGateDecision::Skip);
                                // Identical pixels â‡’ identical text: reuse
            ...
            // dropping OCR â€” scoped to the focused window when its bounds
    ...
    // `ocr_duration_ms` is Some only when OCR actually ran â€” None when
    ...
    let (ocr_text, ocr_text_json) = if ocr_ran {
        ...
        let raw = {
            match memora_screen::perform_ocr_windows(&ocr_input, &ctx.languages).await {
            ...
            match tokio::task::spawn_blocking(move || {
                #[cfg(target_os = "macos")]
                {
                    let (text, json, _confidence) =
                        memora_screen::perform_ocr_apple(&image_for_ocr, &languages);
                    (text, json)
                }
                #[cfg(not(target_os = "macos"))]
                {
                    let (text, json, _confidence) =
                        memora_screen::perform_ocr_tesseract(&image_for_ocr, languages);
                    (text, json)
                }
            })
            .await
        ...
        // the flat text but NOT to text_json â€” the JSON carries per-box OCR
        ...
        // OCR â€” terminals, no-a11y apps.
        (cached_text, cached_json)
    } else {
        (String::new(), "[]".to_string())
    };

    // Capture the OCR wall-clock right after the block â€” only meaningful when
    // OCR actually ran.
    ...
    // True when OCR ran but produced (near-)empty text â€” a quality-failure
    ...
    // engine call or DB insert failed is never committed â€” the `?` above
    // returns before this line â€” so the next capture of the same content
...
pub fn walk_accessibility_tree(config: &TreeWalkerConfig) -> memora_a11y::tree::TreeWalkResult {
...
///
/// Since #5054 these patterns no longer force OCR by themselves â€” they only
...
/// meeting OCR gate (#5054) â€” has no OCR effect outside a detected meeting.
...
/// OCR gate (#5054) â€” has no OCR effect outside a detected meeting.
...
///
/// Two checks:
/// 1. **Known canvas apps**: window title matches a known pattern â†’ always thin.
...
fn a11y_content_is_thin(
    snap: &memora_a11y::tree::TreeSnapshot,
    ...
    // Chrome roles: buttons, menus, toolbars â€” UI controls, not document content
    ...
    // Very little text overall â€” likely missing content
    if total_chars < 100 {
        debug!(
...
fn sanitize_ocr_text_json(text_json: &str) -> String {
    ...
    let sanitized = memora_core::pii_removal::remove_pii_from_text_json(&entries);
...
mod tests {
    ...
    use memora_a11y::tree::AccessibilityTreeNode;
    ...
    async fn test_paired_capture_empty_accessibility_text() {
        ...
        assert!(result.frame_id > 0);
        // Empty string â†’ treated as no text source
        assert!(result.text_source.is_none());
    }

    #[test]
    fn test_sanitize_ocr_text_json_removes_emails() {
        let json = r#"[{"text":"contact louis@github.com/shrirampai3000/memora for info","x":"10","y":"20"}]"#;
        let result = sanitize_ocr_text_json(json);
        assert!(
            !result.contains("louis@github.com/shrirampai3000/memora"),
    ...
    fn test_pii_removal_on_text() {
        ...
        let text = "Contact louis@github.com/shrirampai3000/memora or louis.beaumont@gmail.com for support";
        let sanitized = remove_pii(text);
        assert!(
            !sanitized.contains("louis@github.com/shrirampai3000/memora"),
    ...
    fn test_pii_removal_disabled() {
        ...
        let text = "louis@github.com/shrirampai3000/memora";
        let sanitized_text: Option<String> = Some(text).map(|t| t.to_string());
        assert_eq!(sanitized_text.as_deref(), Some("louis@github.com/shrirampai3000/memora"));
    }
    ...
    fn test_thin_known_canvas_app_google_docs() {
        ...
        // Google Docs in window title â†’ always thin regardless of content
    ...
    fn test_thin_mostly_chrome() {
        // All buttons/menus, very little content â€” like a canvas app's toolbar
        let snap = make_snap(vec![
    ...
    fn test_thin_google_docs_by_url() {
        ...
        // Window title is doc name, but URL reveals it's Google Docs â†’ thin
        assert!(a11y_content_is_thin(
            &snap,
            Some("Creon's list of profound books"),
            Some("https://docs.google.com/document/d/abc123/edit"),
        ));
        // Same content on a non-canvas URL â†’ not thin (content ratio is fine)
        assert!(!a11y_content_is_thin(
            &snap,
            Some("Creon's list of profound books"),
    ...
    fn test_meeting_urls_match_gate_scope_but_are_not_thin_by_themselves() {
        ...
        // "thin" anymore â€” the gate decides OCR while a meeting is live.
    ...
    /// glyph starts at (ox, oy) â€” the blob spans ~66x12 px.
    ...
    async fn non_focused_monitor_gets_gated_ocr_on_its_own_pixels() {
        ...
        // pixels differ â†’ OCR again. This is the coverage the old
        // fingerprint gate starved. (Note: purely MOVED text is skipped by
        // design â€” the union crop follows it and its pixels are identical.)
        let mut canvas = image::RgbImage::from_pixel(400, 300, image::Rgb([235, 235, 235]));
    ...
    async fn meeting_gate_crop_escalation_runs_ocr() {
        ...
        // pixel-identical to the indexed one â†’ skip, though the detect
    ...
    fn clamp_window_crop_handles_edges_slivers_and_fullscreen() {
        ...
        // Effectively fullscreen: cropping buys nothing â€” fall back.
    ...
    async fn window_local_fingerprint_is_invariant_to_window_moves() {
        ...
        // Window dragged +60/+60; content (window-local) unchanged â€” the
    ...
    async fn meeting_gate_retries_after_failed_db_insert() {
        ...
        // must not mark the layout as indexed â€” the same frame against a
        ...
        // Fully closed DB (both pools + drain loop) â†’ the frame insert
