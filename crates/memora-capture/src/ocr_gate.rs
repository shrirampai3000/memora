...
//! was last durably stored. Identical signature â†’ skip (the text on screen
//! is byte-for-byte what search already has). Anything else â†’ OCR. There
//! are deliberately NO other heuristics â€” earlier designs (geometry
...
//! actually changed, and always on the crop â€” sparse frames pay a fraction
...
//! persistence via [`OcrGate::ocr_indexed`] â€” OCRs whose engine call
//! or DB insert fails are never committed and retry on the next capture of
//! the same content (#5060 review).
//!
//! Pure logic â€” no I/O, no platform dependencies.
//!
//! [`image_pixel_signature`]: memora_screen::text_regions::image_pixel_signature
...
struct AppGate {
    ...
    /// â€” terminals, no-a11y apps â€” still carry text at zero OCR cost.
...
impl OcrGate {
    ...
    /// image. Callers with NO detected text skip without calling this â€”
    ...
    pub fn observe(&mut self, app_key: &str, crop_signature: u64) -> OcrDecision {
        ...
        // Deliberately NOT committed to `last_ocr_signature` here â€” the
    ...
    /// crop_relative_text_json)`. Callers reuse it on [`Skip`] ticks â€”
    /// re-mapping the json to the current crop position â€” so OCR-only
    ...
    /// anywhere â€” engine error, DB insert error â€” are simply never
...
mod tests {
    ...
    fn any_pixel_change_ocrs_immediately() {
        ...
        gate.observe("zoom", 1);
        gate.ocr_indexed("zoom", "hello world", "[]");
        // In-place edit, moved text, new caption â€” all just "different
        ...
        // indexed crop is remembered (known Aâ†’Bâ†’A limitation, bounded to
    ...
    fn unpersisted_ocr_retries_until_committed() {
        ...
        let mut gate = OcrGate::new();
        assert_eq!(gate.observe("zoom", 1), OcrDecision::Ocr);
        // No ocr_indexed() â€” simulates engine or DB failure. Retry fires.
        assert_eq!(gate.observe("zoom", 1), OcrDecision::Ocr);
        gate.ocr_indexed("zoom", "hello world", "[]");
        assert_eq!(gate.observe("zoom", 1), OcrDecision::Skip);
    ...
    fn indexed_text_available_only_after_commit() {
        ...
        // Pending (uncommitted) OCR exposes no cached text â€” a failed
