...
//!
//! - **baseline** â€” full-frame OCR on every tick, i.e. the pre-#5054
//!   behavior for meeting apps during a call.
//! - **optimized** â€” the production gate mechanics: `OcrGate`
...
//!   plumbing `paired_capture` uses (walker `window_bounds` fractions â†’
//!   frame pixels â†’ clamp â†’ crop â†’ offset back to frame space).
//!
//! The optimized arm's *index* is the text of its most recent escalation â€”
...
//!   in the index. This is the gate-fidelity number â€” 1.0 means skipping
...
//!
//! ```sh
//! cargo run -p memora-capture --example ocr_gate_ab --release -- \
...
//! just meeting apps during a detected call) â€” that stresses the mechanism
...
mod macos {
    ...
    use memora_capture::paired_capture::walk_accessibility_tree;
    use memora_capture::{OcrDecision, OcrGate};
    use memora_screen::monitor::{get_default_monitor, get_monitor_by_id, SafeMonitor};
    ...
    /// Deltas around a stage attribute its true CPU cost â€” Apple Vision
    ...
    struct Args {
        ...
        /// word-merging comes from edge cutoffs (if the substrâˆ’token gap
    ...
    pub async fn run() -> Result<()> {
        ...
        // Keep the default 250ms walk timeout â€” 150ms proved too tight for
        // browser/webview pages and silently degraded every tick to
        // app=unknown / no window crop.
        let mut walker_config = memora_a11y::tree::TreeWalkerConfig {
        ...
        if mean_luma(&probe) < 3.0 {
            eprintln!(
                "WARNING: first frame is near-black â€” does this terminal have \
        ...
        if !memora_core::permissions::check_accessibility().is_granted() {
            eprintln!(
                "WARNING: this process is NOT trusted for Accessibility â€” the focused-app \
                 walker will fail for every app except the launching terminal's own windows. \
                 Grant your terminal Accessibility in System Settings â†’ Privacy & Security â†’ \
    ...
    struct Summary {
        ...
        fulls: u64,
        offtarget: u64,
        /// Focused app seen on the most recent tick â€” surfaces WHY ticks are
    ...
    async fn one_tick(
        tick: u64,
        monitor: &SafeMonitor,
        walker_config: &memora_a11y::tree::TreeWalkerConfig,
        gate: &mut OcrGate,
        ...
        // Focused app + window bounds via the production walker â€” before the
        ...
        let (app_key, window_name, window_bounds, walk_outcome) = match walk {
            memora_a11y::tree::TreeWalkResult::Found(snap) => {
            ...
            memora_a11y::tree::TreeWalkResult::Skipped(reason) => (
                "unknown".to_string(),
                String::new(),
                None,
                format!("skipped:{reason}"),
            ),
            memora_a11y::tree::TreeWalkResult::NotFound => (
                "unknown".to_string(),
                String::new(),
                None,
                "notfound".to_string(),
        ...
        // Fractions â†’ frame pixels, optionally expanded by --pad on every
        ...
        let (baseline_text, baseline_json) = tokio::task::spawn_blocking(move || {
            let (text, json, _conf) =
                memora_screen::perform_ocr_apple(&frame_for_baseline, &[]);
            (text, json)
        })
        .await?;
        ...
        // screenshot â†’ crop to app window â†’ detect text â†’ crop to the
        // padded union of detected text â†’ pixel-compare that crop to the
        // last indexed one â†’ different? OCR that same crop.
        ...
        // Signature of the union crop â€” the gate's only skip signal.
                ...
                match gate.observe(&app_key, signature) {
                    ...
                    OcrDecision::Ocr => {
                        ...
                        let text = tokio::task::spawn_blocking(move || {
                            memora_screen::perform_ocr_apple(&ocr_input, &[]).0
                        })
                        .await?;
        ...
        // Full texts only for real-loss incidents (substring recall â€” spacing
    ...
    /// the window rect â€” "what the old system saw *in the window*".
    ...
    fn analyze(path: &str) -> Result<()> {
        ...
        println!(
            "window-recall SUBSTR (pixels actually indexed â€” the real-loss metric):\n  mean {substr_mean:.4} | p50 {:.4} | p10 {:.4} | p1 {:.4} | min {:.4}",
            pct(&substr_v, 0.5),
            pct(&substr_v, 0.1),
            pct(&substr_v, 0.01),
            substr_v[0]
        ...
        println!(
            "window-recall TOKEN (word-boundary strict â€” what FTS-style search sees):\n  mean {token_mean:.4} | p50 {:.4} | p10 {:.4} | min {:.4}",
            pct(&token_v, 0.5),
            pct(&token_v, 0.1),
            token_v[0]
        );
        println!(
            "spacing-artifact gap (substr âˆ’ token): {:.4} â€” crop OCR merging words; hurts token search, not substring search",
        ...
        println!(
            "\ncost (wall): baseline OCR {:.1}s vs optimized (detect+OCR) {:.1}s â†’ {:.1}% saved",
            base_ms / 1000.0,
            opt_ms / 1000.0,
        ...
        if cpu_rows > 0 {
            println!(
                "cost (CPU, getrusage over {} ticks): baseline {:.1}s vs optimized {:.1}s â†’ {:.1}% saved",
                cpu_rows,
                base_cpu_ms / 1000.0,
                opt_cpu_ms / 1000.0,
        ...
        if detects > 0 {
            println!(
                "\nfingerprint churn: {churned}/{detects} consecutive detects differed ({:.1}%) â€” \
