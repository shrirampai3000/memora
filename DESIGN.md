...
MEMORA turns the trace of human work into memory, models, and agents. The
...
Do not frame the product as erasing people. MEMORA handles intimate context,
...
**Flat by default â€” use 1px borders for separation.** Subtle shadows are allowed to lift floating / elevated surfaces (chat input, overlays, popovers, dialogs) off the background. Keep them soft and low-opacity (e.g. `shadow-lg shadow-black/5`); never round corners to sell the lift â€” corners stay sharp.
...
Take screenshots of modern apps with great design you find on internet and use it as inspiration for the UX but apply MEMORA brand style to it.
...
- [ ] 0px border radius (sharp corners) â€” always, even on shadowed surfaces
...
| Purpose | Location |
|---------|----------|
| Design tokens | `apps/MEMORA-app-tauri/app/globals.css` |
| Tailwind config | `apps/MEMORA-app-tauri/tailwind.config.ts` |
| Color constants | `apps/MEMORA-app-tauri/lib/constants/colors.ts` |
| UI components | `apps/MEMORA-app-tauri/components/ui/*.tsx` |
