...
export function AppEntitlementGate({
  ...
  // `-shm` desync, or the user runs `memora db recover`), the token returns
  ...
  useEffect(() => {
    ...
    commands.stopMemora().catch((err) => {
      console.warn("failed to stop memora after entitlement gate:", err);
    });
  }, [
    enterpriseAuthenticationPending,
    isManagedDeployment,
  ...
  // macOS, so paste the login token (or the whole memora://...api_key=...
  ...
  const resumeRecordingAfterGate = useCallback((requireGateStop: boolean) => {
    ...
    void (async () => {
      try {
        ...
        console.warn("failed to restart memora after access restored:", err);
      } finally {
  ...
  const devLoginBlock = isDevLoginEnabled() ? (
    <div className="mt-5 border-t border-border pt-4">
      ...
      <input
        ...
        placeholder="paste token or memora://…api_key=…"
        spellCheck={false}
  ...
  if (!isManagedDeploymentResolved) {
    if (managedDeploymentResolutionError) {
      return (
        <EntitlementShell
          ...
          description="memora could not confirm which build is installed. it will retry automatically."
        >
          <Button
    ...
    return (
      <EntitlementShell
        ...
        description="checking which memora build is installed on this device."
      >
        <div className="h-10 w-full animate-pulse bg-muted" />
      </EntitlementShell>
    );
  ...
  if (shouldGateForEnterpriseApp) {
    return (
      <EntitlementShell
        ...
        description={`${email} belongs to ${enterpriseOrgName}. download the memora enterprise app so this device follows workspace policy and uploads to your org storage.`}
      >
        <div className="flex flex-col gap-3">
  ...
  if (!user?.token) {
    return (
      <EntitlementShell
        ...
        description="create or sign in to a memora account to start the free plan."
      >
        <div className="flex flex-col gap-3">
  ...
  return (
    <EntitlementShell
      ...
      description={
        needsRefresh
          ? `${email} has saved app access, but memora needs to verify it again before recording starts.`
          : shouldVerifyPlan
            ? `memora could not verify the plan for ${email}. refresh the account before recording starts.`
