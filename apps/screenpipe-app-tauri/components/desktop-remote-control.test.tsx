...
describe("DesktopRemoteControl", () => {
  beforeEach(() => {
    ...
    mocks.stopMemora.mockResolvedValue({ status: "ok", data: null });
    mocks.spawnMemora.mockResolvedValue({ status: "ok", data: null });
  });
  ...
  it("applies the input-latency default only on Windows", async () => {
    ...
    await waitFor(() => expect(mocks.spawnMemora).toHaveBeenCalled());
  });
