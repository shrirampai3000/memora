  ...
  return [
    {
      ...
      owner: "agent",
      detail: `You sign in to ${agentName} itself. Your Memora plan does not pay for it.`,
    },
    {
      what: "Model choice",
      owner: "agent",
      detail: `Set the model inside ${agentName}. The model picker in Memora presets does not apply.`,
    },
    {
      what: "API keys in AI presets",
      owner: "memora",
      detail: `Used for Memora's own AI. They are not passed to ${agentName}.`,
    },
    {
      what: "Screen and audio context",
      owner: "memora",
      detail: `Memora supplies what you have seen, said, and heard as tools ${agentName} can call.`,
    },
    {
...
export function AcpBoundaries({
  ...
  return (
    ...
    >
      ...
      >
        What {agentName} controls, and what Memora controls
      </p>
        ...
        {rows.map((row) => (
          <div key={row.what} className="flex flex-col gap-0.5">
            <div className="flex items-baseline gap-1.5">
              ...
              >
                {row.owner === "agent" ? agentName : "Memora"}
              </span>
            </div>
            <dd
              className={cn(
