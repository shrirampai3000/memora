...
// Auth + tool calls run through memora's server (screenpipe.com/api/composio),
  ...
  if (!token) {
    return (
      <div className="space-y-3">
        {valueLine}
        <p className="text-xs text-muted-foreground">log in to your memora account to connect {label}.</p>
        {privacyNote}
      </div>
    );
  }
