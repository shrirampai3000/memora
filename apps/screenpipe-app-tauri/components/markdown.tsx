...
export function createMemoraUrlTransform(allowedHosts: readonly string[]) {
  const allowed = new Set(allowedHosts);

  return (url: string): string => {
    try {
      const parsed = new URL(url);
      if (parsed.protocol === "memora:" && allowed.has(parsed.host)) {
        return url;
      }
    } catch {
...
export const notificationUrlTransform = createMemoraUrlTransform(["view"]);
export const viewerUrlTransform = createMemoraUrlTransform(["view"]);
export const chatUrlTransform = createMemoraUrlTransform(["timeline", "frame", "view"]);

export function memoraViewerPathFromHref(href: string): string | null {
  try {
    const url = new URL(href);
    if (url.protocol !== "memora:" || url.host !== "view") {
      return null;
    }
...
export async function openMemoraViewerLink(href: string): Promise<boolean> {
  const path = memoraViewerPathFromHref(href);
...
export function rewriteLocalMarkdownLinksForChat(text: string): string {
  return rewriteLocalMediaLinksForChat(text).replace(
    ...
    (match, sigil: string, label: string, rawUrl: string) => {
      ...
      return `[${label}](memora://view?path=${encodeURIComponent(localPath)})`;
    },
  );
}
