...
describe("isStaleClaudeMemoraEntry", () => {
  it("flags a raw npx entry with no env", () => {
    expect(isStaleClaudeMemoraEntry({ command: "npx", args: ["-y", "screenpipe-mcp"] })).toBe(true);
  });
  ...
  it("does NOT flag an entry with an unfamiliar command", () => {
    expect(
      isStaleClaudeMemoraEntry({ command: "/opt/custom/runner", args: ["x", "screenpipe-mcp@latest"] }),
    ).toBe(false);
  });
