...
async function getCacheDir(): Promise<string> {
  ...
  if (res.status === "ok") {
    ...
    const baseRes = await commands.getMemoraBaseDir();
    if (baseRes.status === "ok") {
      baseDir = baseRes.data;
    } else {
      // last resort
      const { homeDir } = await import("@tauri-apps/api/path");
      baseDir = await join(await homeDir(), ".memora");
    }
  }
 ...
 * Cache lives inside the memora data dir (~/.screenpipe/cache/).
 */
