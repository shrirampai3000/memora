...
const SCREENPIPE_PRESET_ID = "memora";
		...
		_store = (async () => {
			...
			try {
				...
				if (res.status === "ok") {
					...
					console.warn("get_screenpipe_base_dir failed, using ~/.memora:", res.error);
				}
			} catch (e) {
				console.warn("get_screenpipe_base_dir unavailable, using ~/.memora:", e);
			}
			if (!baseDir) {
	...
	const loadUser = async (token: string, verify = false) => {
		...
		try {
			const response = await fetch(
				memoraWebUrl("/api/user", "https://screenpipe.com"),
				{
					method: "POST",
					headers: {
