...
const API = memoraWebUrl("/api/team", "https://screenpipe.com");
...
export function useTeam() {
  ...
  const createInviteLink = useCallback(
    async (teamName: string, key: CryptoKey, forceNew = false) => {
      ...
      return memoraWebUrl(`/join/${tokenData.invite_token}#key=${encodeURIComponent(base64Key)}`, "https://screenpipe.com");
    },
    [headers]
  );
