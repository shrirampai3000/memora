...
Read the memora skill first so you know the API and how pipes work. During discovery, use progressive disclosure and the memora API only. Authenticate every localhost:3030 request with \`Authorization: Bearer $SCREENPIPE_LOCAL_API_KEY\`; \`SCREENPIPE_API_KEY\` is for the hosted API and is not proof that the local API is misconfigured. Never estimate time from frame counts and never use /raw_sql for this task.
