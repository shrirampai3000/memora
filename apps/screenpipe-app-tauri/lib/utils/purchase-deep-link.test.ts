...
describe("isBusinessSubscriptionPurchaseDeepLink", () => {
  it("accepts the Business subscription return from /purchase-success", () => {
    expect(match("memora://purchase-successful?subscription=true")).toBe(
      true,
    );
    expect(
      match("memora://purchase-successful?subscription=true&session_id=cs"),
    ).toBe(true);
  });
  ...
  it("ignores the pipe purchase return", () => {
    expect(match("memora://purchase-successful?pipe_id=my-pipe")).toBe(
      false,
    );
  });

  it("ignores the team purchase return", () => {
    expect(match("memora://purchase-successful?plan=team")).toBe(false);
  });

  it("ignores a purchase return with no subscription marker", () => {
    expect(match("memora://purchase-successful")).toBe(false);
    expect(match("memora://purchase-successful?subscription=false")).toBe(
      false,
    );
    expect(match("memora://purchase-successful?subscription=1")).toBe(
      false,
    );
  });

  it("ignores unrelated deep links", () => {
    expect(match("memora://subscription-success?purchase_token=t")).toBe(
      false,
    );
    expect(match("memora://auth?success=true&subscription=true")).toBe(
      false,
    );
  });
});
