...
export function buildAccountStandingMessage(errorStr: string): string | null {
  ...
  return "memora cloud AI is blocked for this account — it's flagged as not in good standing. Signing out and back in can refresh your account status. If you believe this is a mistake, contact memora support. Local models and your own provider keys keep working in Settings → AI.";
}
...
export function buildAgentRefusalMessage(
  ...
  if (mentionsPermission && mentionsPolicy) {
    return {
      ...
      retryable: false,
      message: `${agent} signed in fine, but its own service refused this account: it needs a plan or organization policy you don't have. Signing in again won't change that — it isn't a memora limit and memora can't grant it.\n\nWhat does work: switch this preset's model calls to Memora Cloud in Settings → AI presets, if this agent supports it, or pick a different agent. Ask whoever administers the account to enable it if you need this one specifically.`,
    };
  }
