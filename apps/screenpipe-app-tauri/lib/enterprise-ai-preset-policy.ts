...
export function normalizeEnterpriseAiPresetPolicy(value: unknown): EnterpriseAiPresetPolicy {
  ...
  const allowMemoraCloud = value.allow_screenpipe_cloud !== false;
  ...
  const validDefaults = new Set([
    ...(allowMemoraCloud ? ["screenpipe-cloud"] : []),
    ...managedPresets.map((preset) => preset.id),
  ]);

  return {
    version: 2,
    allow_screenpipe_cloud: allowMemoraCloud,
    ...
    default_preset_id: validDefaults.has(defaultPresetId)
      ? defaultPresetId
      : managedPresets[0]?.id ?? (allowMemoraCloud ? "screenpipe-cloud" : null),
