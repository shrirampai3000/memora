...
vi.mock("@/lib/web-url", () => ({
  ...
  memoraWebUrl: () => "https://screenpipe.com/login",
...
describe("isMemoraApi", () => {
  it("matches the cloud API host and its subdomains", () => {
    expect(isMemoraApi("https://screenpipe.com/api/user")).toBe(true);
    expect(isMemoraApi("https://screenpi.pe/api/oauth/exchange")).toBe(true);
    expect(isMemoraApi("https://api.screenpipe.com/v1/chat/completions")).toBe(true);
    expect(isMemoraApi("https://clerk.screenpipe.com/")).toBe(true);
  });
  ...
  it("does NOT match the local engine when a screenpipe-domain email rides in the query", () => {
    ...
    expect(
      isMemoraApi("http://127.0.0.1:3030/connections/google-calendar?instance=x@screenpipe.com")
    ).toBe(false);
  });

  it("does NOT match a third-party host that merely mentions the domain in path/query", () => {
    expect(isMemoraApi("https://evil.example.com/?ref=screenpi.pe")).toBe(false);
    // not a subdomain — must not match on a bare suffix
    expect(isMemoraApi("https://notscreenpipe.com/api")).toBe(false);
    expect(isMemoraApi("https://screenpipe.com.evil.com/api")).toBe(false);
  });
...
describe("isMemoraAuthApi", () => {
  it("treats the website auth surface as session-bearing", () => {
    expect(isMemoraAuthApi("https://screenpipe.com/api/user")).toBe(true);
    expect(isMemoraAuthApi("https://screenpi.pe/api/oauth/exchange")).toBe(true);
    expect(isMemoraAuthApi("https://clerk.screenpipe.com/")).toBe(true);
  });

  it("EXCLUDES the AI inference gateway — its 401/403 is not session expiry (SCR-132)", () => {
    expect(isMemoraAuthApi("https://api.screenpipe.com/v1/chat/completions")).toBe(false);
    expect(isMemoraAuthApi("https://api.screenpipe.com/v1/messages")).toBe(false);
    expect(isMemoraAuthApi("https://api.screenpi.pe/v1/models")).toBe(false);
  });
  ...
  it("with a web-base override, ONLY the override host is the session authority", () => {
    ...
    expect(isMemoraAuthApi("https://screenpipe.com/api/team", base)).toBe(false);
    expect(isMemoraAuthApi("https://screenpi.pe/api/user", base)).toBe(false);
  ...
  it("an override equal to prod behaves exactly like no override", () => {
    ...
    expect(isMemoraAuthApi("https://screenpipe.com/api/user", base)).toBe(true);
    expect(isMemoraAuthApi("https://api.screenpipe.com/v1/messages", base)).toBe(false);
  });
  ...
  it("returns false for the local engine and non-screenpipe hosts", () => {
    ...
    expect(isMemoraAuthApi("https://evil.example.com/?ref=screenpi.pe")).toBe(false);
  });
});
