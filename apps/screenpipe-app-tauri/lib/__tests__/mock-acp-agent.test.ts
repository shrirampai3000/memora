...
const nodeExecutable = process.env.SCREENPIPE_NODE_PATH || process.execPath;
...
class MockAcpClient {
  ...
  constructor(
    ...
    const command = process.env.SCREENPIPE_NODE_PATH || (process.platform === "win32" ? "npx.cmd" : "npx");
    const args = process.env.SCREENPIPE_NODE_PATH
