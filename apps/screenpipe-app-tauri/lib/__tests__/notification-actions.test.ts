...
describe("notification deeplink routing", () => {
  ...
  it("parses meeting deeplinks from both path and query forms", () => {
    expect(parseMeetingDeeplink("memora://meeting/123")).toEqual({
      meetingId: 123,
      transcript: true,
    });
    expect(parseMeetingDeeplink("memora://meeting?id=456&live=0")).toEqual({
      meetingId: 456,
      transcript: false,
    });
    expect(parseMeetingDeeplink("memora://timeline")).toBeNull();
  });
  ...
  it("keeps non-meeting deeplinks on the generic deep-link event path", async () => {
    const routePromise = routeNotificationDeeplink("memora://timeline", {
      emitEvent: emitMock,
      showWindowActivated: showWindowActivatedMock,
      sleepMs: fakeSleep,
    });
    ...
    expect(windowForDeeplink("memora://timeline")).toBe("Main");
