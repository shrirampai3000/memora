...
fn build_livetext_bridge() {
    ...
    // Compile Swift â†’ static library targeting macOS 13.0
...
fn generate_and_validate_tauri_commands() {
    ...
    for required in [
        "MEMORA_app::config::get_MEMORA_base_dir",
        "MEMORA_app::commands::get_cloud_token",
        "MEMORA_app::commands::is_enterprise_build_cmd",
    ] {
        assert!(
...
fn main() {
    ...
    let build_time = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    println!("cargo:rustc-env=MEMORA_BUILD_UNIX_TIME={}", build_time);
        ...
        && std::env::var("CARGO_CFG_TARGET_ARCH").as_deref() == Ok("aarch64")
    {
        ...
        if let Ok(openblas) = std::env::var("OPENBLAS_PATH") {
            ...
            if dll_src.exists() {
                ...
                // OUT_DIR = target/{profile}/build/{crate}-{hash}/out â€” three pops â†’ target/{profile}/
                ...
                if !dll_dst.exists() {
                    match std::fs::copy(&dll_src, &dll_dst) {
                        Ok(_) => println!(
                            "cargo:warning=openblas: copied libopenblas.dll â†’ {}",
                            dll_dst.display()
                        ),
                        Err(e) => {
    ...
    // tauri.windows.conf.json always resolves â€” tauri-build hard-fails on a
    ...
    // same guard here. Reserve is address space, committed on demand â€” no
...
/// Build the `MEMORA-wer-dump-helper` cdylib with a nested cargo
...
fn build_wer_dump_helper() {
    ...
    match status {
        ...
        other => panic!("failed to build MEMORA-wer-dump-helper: {other:?}"),
    }

    let dll_name = "MEMORA_wer_dump_helper.dll";
...
fn stage_mlx_metallib() {
    ...
    if needs_download {
        ...
        let url =
            "https://github.com/MEMORA/MEMORA/releases/download/mlx-metallib-v0.2.0/mlx.metallib";
        ...
        match status {
            ...
            _ => println!(
                "cargo:warning=mlx-metallib: download failed â€” parakeet-mlx will crash at runtime"
            ),
        }
    } else {
        let size = std::fs::metadata(&metallib).map(|m| m.len()).unwrap_or(0);
...
fn copy_permission_flow_bundle() {
    ...
    // cargo's DEBUG flag (`[profile.dev] debug = false` here â‡’ `release/`), so
    ...
    // ship the real bundle â€” hard-fail. Debug builds (e2e CI) only need the
