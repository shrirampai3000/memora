...
// core MEMORA capabilities: query_recordings, list_connections,
// save_artifact, sp_web_search, MEMORA_connect_app, live_view, and the
// sp_mcp_* bridge. It speaks newline-delimited JSON-RPC on stdin/stdout (MCP
// stdio) or, when MEMORA_TOOLS_HTTP_PORT is set, a stateless Streamable-HTTP
...
// MEMORA HTTP server does not expose, so those agents reach parity. Uses
// only runtime built-ins, so it needs no npm install and runs from the bundled
// bun. It talks to the local MEMORA engine over REST; it never sees the
// cloud credential (that stays out of ACP process trees â€” web search goes
...
const SERVER_INFO = { name: "MEMORA-tools", version: "0.1.0" };

function apiBase() {
  const url =
    process.env.MEMORA_API_URL ||
    process.env.MEMORA_LOCAL_API_URL ||
    `http://localhost:${
      process.env.MEMORA_LOCAL_API_PORT || process.env.MEMORA_PORT || "3030"
    }`;
  return url.replace(/\/+$/, "");
}
...
// The local engine (apiBase) serves /mcp-servers, /search, /raw_sql â€” it is
// never itself an MCP endpoint. A bridge server pointing back at it (a stale
// "MEMORA -> http://localhost:3030/mcp" is the classic case) only 404s and
...
function chatSessionId() {
  return process.env.MEMORA_CHAT_SESSION_ID || "chat";
}

// Like authHeaders, but also carries the session tag the engine's per-session
// MCP-server access gate reads (x-MEMORA-session). Matches the Pi
...
// path sets MEMORA_CHAT_SESSION_ID; the pi path sets MEMORA_SESSION_ID)
// â€” prefer whichever is present.
function mcpBridgeHeaders() {
  const headers = authHeaders();
  const session =
    process.env.MEMORA_SESSION_ID || process.env.MEMORA_CHAT_SESSION_ID || "";
  if (session) headers["x-MEMORA-session"] = session;
  return headers;
}

// GET /mcp-servers filtered to the enabled set â€” the same list the Pi
...
// ---------------------------------------------------------------------------
// Connection enrichment â€” a faithful port of the Pi connection-gate extension,
...
const COMPOSIO_URL_RE = /^https:\/\/(www\.)?(MEMORA\.com|screenpi\.pe)\/api\/composio\/mcp\/?$/;
...
// Live Views (dashboards) â€” mirrors the Pi-only MEMORA_live_view extension.
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "List the user's MEMORA app connections (Gmail, Notion, Slack, Linear, calendars, and other integrations), whether each is connected, and how to use it (native proxy vs MCP OAuth). Use this before an action that depends on a connected app.",
  ...
  },
  {
    ...
    async run(args) {
      ...
      const tmpDir = join(tmpdir(), "MEMORA-artifacts", sessionId);
  ...
  },
  {
    name: "sp_web_search",
    description:
      "Search the public internet via Google Search. Use ONLY for public, external information the user explicitly asks about (current events, news, public people or companies, public product docs). Do NOT use it for the user's own MEMORA data. When unsure, do not search. Returns results with sources.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    async run(args) {
      ...
      if (!res.ok) {
        ...
        if (res.status === 503) {
          throw new Error("web search unavailable: sign in to MEMORA first");
        }
    ...
    async run(args) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // "timeout" or anything else â†’ fall through to the async card.
        }
      } catch {
        // network/route error â†’ fall through to the async card.
      }
  ...
  },
  {
    ...
    // this, ACP agents are told by list_connections / MEMORA_connect_app to
    ...
    // such tool â€” so those connections are advertised yet unreachable.
    ...
    async run(args) {
      ...
      // the whole listing â€” mirror the extension's per-server error capture.
...
// runs the core `MEMORA` server over HTTP â€” but that transport exposes
...
const HTTP_PARITY_TOOLS = [
  {
    ...
    description:
      "Rich activity overview for a time range: app usage, window/tab titles with URLs, time spent, key text per context, and audio transcriptions. The right first call for any broad 'what was I doing / how long on X / recap my morning' question â€” usually sufficient without follow-up searches. Proxies the local engine /activity-summary.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "health_check",
    description:
      "Check whether the MEMORA engine is running and healthy (recording status, frame and audio stats). Proxies the local engine /health.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
    async run() {
...
// `send` is the transport sink â€” stdout for stdio, a per-request collector for
// HTTP â€” so the same handler serves both transports.
...
const httpPort = Number.parseInt(process.env.MEMORA_TOOLS_HTTP_PORT || "", 10);
...
// because the core MEMORA server exposes only search_content over HTTP.
...
function startStdioServer() {
  ...
  process.stdin.on("data", (chunk) => {
    ...
    if (buffer.length > 4_000_000) {
      process.stderr.write("MEMORA-tools: stdin line exceeded 4MB, exiting\n");
      process.exit(1);
    }
    let index;
...
function startHttpServer(port) {
  ...
  server.listen(port, "127.0.0.1", () => {
    process.stderr.write(`[MEMORA-tools] http mcp listening on http://127.0.0.1:${port}/mcp\n`);
  });
}
