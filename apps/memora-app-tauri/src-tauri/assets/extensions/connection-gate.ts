...
const API_BASE =
  ...
  `http://localhost:${process.env.MEMORA_LOCAL_API_PORT || process.env.MEMORA_PORT || "3030"}`;
const AUTH_KEY =
  process.env.MEMORA_LOCAL_API_KEY ||
  process.env.MEMORA_API_AUTH_KEY ||
  "";
...
// registers one shared MCP server whose URL points at github.com/shrirampai3000/memora's
...
const connectParams = {
  ...
  properties: {
    connectionId: {
      type: "string",
      description:
        "The MEMORA connection id required for the task, e.g. linear, notion, github, google-docs.",
    },
    reason: {
      type: "string",
      description:
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    name: "MEMORA_list_connections",
    label: "List MEMORA connections",
    description:
      "List MEMORA app connections and whether each one is connected. Use this before sending, pushing, creating, or reading from an external app.",
    promptSnippet:
      "List MEMORA app connections and connected status before using external apps",
  ...
  pi.registerTool({
    ...
    label: "Connect MEMORA app",
    description:
      "Ask the user to connect a required MEMORA app inline and wait for their answer. Call this when the task depends on an app that is not connected. Do not continue the dependent task until this returns connected.",
    promptSnippet:
      "Ask the user to connect a required MEMORA app inline and wait before continuing",
    promptGuidelines: [
      "Before using an external app, call MEMORA_list_connections or otherwise verify it is connected.",
      "If the required app is not connected, call MEMORA_connect_app and wait for the result.",
      "Do not continue a task that depends on an unconnected app while MEMORA_connect_app is waiting.",
    ...
    async execute(
      ...
      try {
        ...
        if (!ctx.hasUI || !ctx.ui) {
          return {
            content: [
              {
                type: "text" as const,
                text: `${name} is not connected. Connect ${name} in MEMORA to continue.`,
              },
            ],
        ...
        const title = `MEMORA:connect:${connectionId}:${name}`;
