...
//! default â€” it can scrape and navigate to public URLs, but the moment a
//! task touches an authenticated surface (Twitter, Gmail, GitHub, the
//! user's bank, â€¦) it hits a login wall. Without this module the only
...
//!   login page â€” same UX as before this module existed.
//!
//! ## What we don't cover
//!
//! - **localStorage / IndexedDB** â€” some auth flows (notably some OAuth
...
//! - **Passkeys / WebAuthn** â€” hardware-bound to the user's real browser.
...
//!   page JS, only sent on requests â€” same semantics as a real browser.
...
/// and has an exclusive hold on the file). Different from v20 â€” the cookies
...
/// implemented yet â€” call sites can invoke this unconditionally.
...
async fn cookies_for_host_impl(host: &str) -> Vec<Cookie> {
    ...
    let cookies = tokio::task::spawn_blocking(move || {
        ...
        for source in WIN_SOURCES {
            match read_cookies_windows(source, &host_owned) {
                ...
                Err(e) => {
                    info!(source = source.name, "owned-browser cookies: skip â€” {e}");
                }
            }
        }
        out
    ...
    debug!(
        host,
        count = cookies.len(),
        "owned-browser cookies: cache miss â†’ read"
    );
    cookies
}
...
async fn has_cookies_for_host_impl(host: &str) -> bool {
    ...
    tokio::task::spawn_blocking(move || {
        // Collect locked sources locally first â€” don't touch the shared map
        ...
        for source in WIN_SOURCES {
            match has_cookie_rows_windows(source, &host_owned) {
                ...
                Err(ref e) if e.starts_with("sqlite open:") => {
                    // Browser IS installed (file exists) but the DB is locked â€”
                    // browser is running with an exclusive hold on the file.
                    info!(
                        source = source.name,
                        host = host_owned.as_str(),
                        "owned-browser cookies: row preflight skip â€” browser running, db locked ({e})"
                    );
                ...
                Err(e) => info!(
                    source = source.name,
                    host = host_owned.as_str(),
                    "owned-browser cookies: row preflight skip â€” {e}"
                ),
            }
        }
        ...
        // locked, store exactly those sources â€” no incremental accumulation
...
#[cfg(target_os = "macos")]
async fn cookies_for_host_impl(host: &str) -> Vec<Cookie> {
    // Lookup cache first â€” same host hit twice in 30s is the common
        ...
        for source in SOURCES {
            match read_cookies(source, &host_owned) {
                ...
                Err(e) => {
                    ...
                    // skipped without a rebuild â€” most "0 cookies"
...
// ---------------------------------------------------------------------------
// Windows â€” Chromium-derived browsers (Chrome / Brave / Edge / Chromium /
// Vivaldi / Opera)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Locked-browser block registry â€” parallel to the v20 registry
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// macOS â€” Chromium-derived browsers (Chrome / Brave / Edge / Arc)
// ---------------------------------------------------------------------------
...
/// different file paths) â€” see the trait sketch in the doc comment at
...
struct KeychainEntry {
    ...
    /// macOS Keychain *account* attribute â€” usually the brand name.
...
/// Domain â†’ (cookies, fetched_at). Per-process, in-memory, no eviction
/// (cookie sets per host are tiny â€” kilobytes â€” and the cache lives
...
/// Source name â†’ derived AES-128 key. We cache this per process to
...
fn open_cookie_db(source: &KeychainEntry) -> Result<rusqlite::Connection, String> {
    ...
    // Open read-only â€” the SQLite file is also held open for write by
...
fn safe_storage_likely_prompts_for_host_impl(host: &str) -> bool {
    ...
    for source in SOURCES {
        match has_cookie_rows(source, host) {
            Ok(true) => {
                ...
                match safe_storage_entry_trusts_current_app(source) {
                    ...
                    Err(e) => {
                        info!(
                            source = source.name,
                            host, "owned-browser cookies: Safe Storage ACL preflight unknown â€” {e}"
                        );
                        return true;
                    }
                }
            ...
            Err(e) => debug!(
                source = source.name,
                host, "owned-browser cookies: Safe Storage ACL preflight row skip â€” {e}"
            ),
        }
    }
    !checked_any
...
/// Synchronous worker â€” runs inside spawn_blocking. Returns Vec on
...
fn read_cookies(source: &KeychainEntry, host: &str) -> Result<Vec<Cookie>, String> {
    ...
    // signed (-1..=2). expires_utc is microseconds since 1601 â€” convert
    ...
    // Each Chromium-derived browser has its own Keychain entry â€” Chrome
        ...
        let value = if enc_val.is_empty() {
            ...
            match decrypt_v10(&enc_val, &key) {
                ...
                None => {
                    ...
                    // the whole batch â€” one corrupt row shouldn't deny the
                    // agent every cookie.
                    decrypt_failed += 1;
                    continue;
                }
...
/// `mail.google.com` should pick up both `.google.com` and `.com` (well â€”
...
/// (the Windows FILETIME epoch â€” yes, even in macOS Chrome). `0` means
...
/// (These are not secret â€” Chromium's open-source cookie code
...
/// libsecret key â€” we don't see those on macOS.
///
...
/// for injection â€” if a malicious actor swapped rows in the SQLite, the
...
/// "no prefix, return as-is" â€” best-effort.
#[cfg(target_os = "macos")]
