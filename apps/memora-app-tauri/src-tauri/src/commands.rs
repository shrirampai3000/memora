...
/// "failed to create webview: WebView2 error: â€¦". Without knowing which
...
/// a bare `error!(...)`. Return the error unchanged â€” this function is
...
mod tests {
    ...
    /// The overlay ships unhideable. A stored `showShortcutOverlay: false` â€”
    /// whether left over from before this shipped or set while the remote
    /// capability was on â€” must stay inert until the flag grants it back.
    #[test]
    ...
    fn team_config_writes_and_updates_gateway_url() {
        ...
        std::env::set_var("MEMORA_DATA_DIR", dir.path());
        ...
        std::env::remove_var("MEMORA_DATA_DIR");
    }
    ...
    fn enterprise_json_parses_license_and_ingest_url() {
        ...
        // Legacy file: license only â€” ingest stays None (default base applies).
    ...
    fn chat_entries_missing_dir_is_empty() {
        ...
        let res =
            scan_chat_entries_by_mtime("/definitely/not/a/real/MEMORA/chats/path").unwrap();
        assert!(res.is_empty());
    }
    ...
    fn login_callback_accepts_website_fallback_scheme() {
        assert!(is_login_callback_scheme("MEMORA"));
    }

    // Regression for b7dc02415: `get_local_api_config` returned {key: null}
    // during the cold-spawn window between webview load and `spawn_MEMORA`
    ...
    // fallback shape â€” the part that actually broke. Seeding the static and
    // reading it back is covered by store.rs tests / the manual repro:
    // open Settings â†’ Privacy with recording paused; key field must populate.
...
/// (`MEMORA_DISABLE_TELEMETRY` / `GITHUB_ACTIONS` / `CI`).
///
/// The Rust senders already consult
/// [`memora_engine::analytics::telemetry_disabled_by_env`] directly, but the
...
/// (e.g. `github.com/shrirampai3000/memora`, `github.com/shrirampai3000/memora.beta`, `github.com/shrirampai3000/memora.dev`,
/// `github.com/shrirampai3000/memora.enterprise`). The onboarding stuck-screen surfaces this so
/// users who switched build channels (prod â†” beta â†” dev) can see they're
...
pub fn get_low_disk_guard_config() -> LowDiskGuardConfig {
    LowDiskGuardConfig {
        threshold_bytes: memora_events::LOW_DISK_THRESHOLD_BYTES,
        check_interval_seconds:
            memora_engine::disk_pressure::LOW_DISK_CHECK_INTERVAL_SECS,
    }
}
...
pub async fn get_local_api_config(app_handle: tauri::AppHandle) -> serde_json::Value {
    ...
    if let Some(state) = app_handle.try_state::<RecordingState>() {
        ...
        // opened WebSockets without ?token= â†’ endless 403 / abnormal close (1006).
    ...
    // *guard is None â€” server hasn't been constructed yet (early-mount race
    // against spawn_MEMORA, or pause window). The webview's
...
pub fn get_app_server_config() -> serde_json::Value {
    let port = std::env::var("MEMORA_FOCUS_PORT")
        .ok()
...
/// the `MEMORA_PORT` E2E/dev override.
...
pub async fn regenerate_api_auth_key(app_handle: tauri::AppHandle) -> Result<String, String> {
    ...
    let key = memora_engine::auth_key::regenerate_api_auth_key(&data_dir)
        .await
...
pub async fn set_api_auth_key(app_handle: tauri::AppHandle, key: String) -> Result<(), String> {
    let data_dir = memora_core::paths::default_MEMORA_data_dir();
    memora_engine::auth_key::set_api_auth_key(&data_dir, &key)
        .await
        .map_err(|e| e.to_string())?;
    persist_api_auth_key_to_settings(&app_handle, &key)
}
...
/// Resolve the chat-conversations directory under the *active* MEMORA data
/// dir (honors `MEMORA_DATA_DIR` / a relocated data dir), creating it if
/// needed. The frontend previously hardcoded `~/.memora/chats` via
...
pub fn get_chats_dir() -> Result<String, String> {
    ...
    let is_isolated = std::env::var("MEMORA_E2E_SEED").is_ok()
        || crate::dev_isolation::is_active();
    if !is_isolated {
        if let Some(home) = dirs::home_dir() {
            let legacy = home.join(".memora").join("chats");
            if legacy != chats
                && legacy.is_dir()
                && !dir_has_conversations(&chats)
                && dir_has_conversations(&legacy)
            {
                match memora_core::paths::copy_dir_all(&legacy, &chats) {
...
/// in the MEMORA-api skill that Pi installs on every run.
///
/// Mechanism: the memora-core `Pi::ensure_MEMORA_skill` reads
...
/// to `<project>/.pi/skills/MEMORA-api/SKILL.md`. Default (no
...
pub fn set_cloud_media_analysis_skill(enabled: bool) -> Result<(), String> {
    ...
    if enabled {
        ...
        std::fs::write(
            &marker,
            "Opt-out marker â€” Pi will install the MEMORA-api skill \
             without the Gemma 4 E4B confidential-enclave block. \
             Delete this file (or toggle in Settings â†’ Privacy) to \
...
/// control planes work without env vars. Both optional â€” a file with only
...
impl EnterpriseFileConfig {
    /// Assertion helper for the `enterprise.json` parser tests â€” production
...
pub fn get_enterprise_file_config() -> EnterpriseFileConfig {
    ...
    let user_path = memora_core::paths::default_MEMORA_data_dir().join("enterprise.json");
...
/// Merge-write device config fields into `~/.memora/enterprise.json`,
...
/// Save the enterprise license key to `~/.memora/enterprise.json`.
...
/// Persist the resolved "hide app UI" decision into `~/.memora/enterprise.json`
...
/// it on the very first window-gate check at startup â€” before the frontend has
...
fn persist_enterprise_hide_app(hidden: bool) {
    let path = memora_core::paths::default_MEMORA_data_dir().join("enterprise.json");
...
/// `~/.memora/enterprise.json`. Returns None when the file is
/// missing, malformed, or the field is empty.
///
/// Used by the Settings â†’ Enterprise â†’ Admin API token card to render
...
/// Read the user's MEMORA cloud session JWT.
///
...
/// `~/.memora/auth.json` (the CLI credential file) remains as a fallback
...
pub fn get_cloud_token() -> Option<String> {
    ...
    let path = memora_core::paths::default_MEMORA_data_dir().join("auth.json");
    let raw = std::fs::read_to_string(&path).ok()?;
    let parsed: serde_json::Value = serde_json::from_str(&raw).ok()?;
    parsed
        .get("token")
        ...
        // engine auth_key.rs) â€” never serve a non-JWT value as a cloud login.
...
pub async fn set_cloud_token(
    ...
    // Unblock cloud calls for THIS session first â€” the ArcSwap + cache are the
    ...
    // Sign-out: scrub the MEMORA token from pi's auth files before the
    ...
    if should_clear_pi_auth {
        if let Err(e) = crate::pi::clear_MEMORA_auth_token_files() {
            warn!("failed to clear pi MEMORA auth token: {}", e);
        }
    }
...
/// decide whether to inject `MEMORA-team`; the native CLI resolves the API
...
pub fn save_enterprise_team_config(
    ...
    crate::enterprise_config_file::update(&path, |json| {
        ...
        // written â€” a junk value would silently redirect all three readers.
...
pub fn set_tray_unhealth_icon(app_handle: tauri::AppHandle) {
    ...
    let _ = app_handle.run_on_main_thread(move || {
        if let Some(main_tray) = app.tray_by_id("MEMORA_main") {
            match tauri::image::Image::from_path("icons/MEMORA-logo-tray-failed.png") {
                Ok(icon) => {
...
pub fn set_tray_health_icon(app_handle: tauri::AppHandle) {
    ...
    let _ = app_handle.run_on_main_thread(move || {
        if let Some(main_tray) = app.tray_by_id("MEMORA_main") {
            match tauri::image::Image::from_path("icons/MEMORA-logo-tray-black.png") {
                Ok(icon) => {
                    if let Err(e) = crate::safe_icon::safe_set_icon(&main_tray, icon) {
...
pub fn show_main_window(app_handle: tauri::AppHandle) {
    ...
    match window_to_show.show(&app_handle) {
        Ok(window) => {
            ...
            // Don't call set_focus() on macOS â€” both overlay and window modes use
            ...
            // On Windows/Linux, bypass the is_visible() guard â€” window.show() posts
            ...
            // IMPORTANT: spawn a new thread â€” show_main_window is invoked from within
...
pub fn hide_main_window(app_handle: tauri::AppHandle) {
    ...
    // handler in window/show.rs. Do NOT also unregister them here â€” doing
...
#[specta::specta]
pub fn update_show_MEMORA_shortcut(
    app_handle: tauri::AppHandle,
    new_shortcut: String,
    enabled: bool,
) -> Result<(), String> {
    ...
    if let Err(e) = app_handle.global_shortcut().on_shortcut(
        ...
        move |app_handle, _event, _shortcut| {
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                // window.is_visible() â€” the latter returns true even when
...
pub async fn get_disk_usage(
    ...
    let MEMORA_dir_path = match data_dir {
        Some(d) if !d.is_empty() && d != "default" => std::path::PathBuf::from(d),
        _ => memora_core::paths::default_MEMORA_data_dir(),
    };
    ...
    match crate::disk_usage::disk_usage(&MEMORA_dir_path, freshness).await {
...
/// produced with NEXT_PUBLIC_MEMORA_WEB_URL signs in against that
/// deployment, so its session token verifies on the same control plane
/// the rest of the app talks to.
fn login_url() -> String {
    crate::web_base::MEMORA_web_url("/login")
}
...
mod login_url_intent_tests {
    ...
    fn carries_explicit_signup_intent_and_return_scheme() {
        let login_url = login_url_with_intent(Some(LoginMode::SignUp), Some("MEMORA"))
    ...
    fn keeps_neutral_login_urls_free_of_mode() {
        let login_url =
            login_url_with_intent(None, Some("MEMORA-enterprise")).expect("valid login URL");
        let parsed: tauri::Url = login_url.parse().expect("parse generated login URL");
        let pairs = parsed
            .query_pairs()
            .collect::<std::collections::HashMap<_, _>>();
...
/// `MEMORA://` on machines that have both installed (see #3890). Login
...
/// redirects stay on `MEMORA://`, matching the consumer path.
...
fn is_login_callback_scheme(scheme: &str) -> bool {
    scheme == deep_link_scheme() || scheme == "MEMORA"
}
...
/// Open the github.com/shrirampai3000/memora login page.
/// macOS: ASWebAuthenticationSession (system-managed sheet, forwards callback).
/// Windows/Linux: in-app WebView that intercepts the MEMORA:// redirect.
///
...
pub async fn open_login_window(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        // scheme routing), so the consumer `MEMORA` scheme cannot collide
    ...
    #[cfg(not(target_os = "macos"))]
    {
        ...
        // browser â€” no cookies, no password manager, no live SSO session â€” so
        ...
        if !fresh_session {
            ...
            // website deep-link `MEMORA://auth?api_key=â€¦` straight back.
            //
            // #5936 correctly wanted the real browser here â€” the embedded
            // WebView is a cold browser with no cookies, SSO or password
            // manager â€” but reached for the CLI's device-code flow to get it,
                ...
                .open_url(login_url.as_str(), None::<&str>)
            {
                ...
                Err(e) => {
                    // No usable default browser â€” fall through to the WebView
...
mod login_window_tests {
    ...
    fn reused_login_window_returns_to_login_page() {
        ...
        let login_url: tauri::Url = "https://github.com/shrirampai3000/memora/login?return_scheme=MEMORA"
            .parse()
            .unwrap();
        let window =
...
pub async fn show_window(
    ...
    if !matches!(
        ...
        // Hide Main without restoring the previous frontmost app â€” we're
        // transitioning to another MEMORA window so focus should stay
...
pub async fn search_navigate_to_timeline(
    ...
    // Emit the navigation event multiple times â€” the Main webview may take
...
pub async fn get_keychain_status() -> Result<KeychainStatus, String> {
    ...
    let state = if !is_enabled {
        // Encryption not enabled in settings â€” definitely disabled
        "disabled"
    } else {
...
pub async fn enable_keychain_encryption() -> Result<KeychainStatus, String> {
    ...
    let data_dir = memora_core::paths::default_MEMORA_data_dir();
    if let Err(e) = memora_secrets::mark_encryption_enabled(&data_dir) {
        tracing::warn!("failed to write .encrypt-store flag: {}", e);
    }

    if let Ok(store) =
        memora_secrets::SecretStore::open_for_data_dir(&data_dir, Some(key)).await
    {
...
pub async fn disable_keychain_encryption() -> Result<KeychainStatus, String> {
    let data_dir = memora_core::paths::default_MEMORA_data_dir();
    let secrets_path = memora_secrets::secrets_database_path(&data_dir);

    if secrets_path.exists() || data_dir.join("db.sqlite").exists() {
        let plain_store = memora_secrets::SecretStore::open_for_data_dir(&data_dir, None)
            .await
        ...
        if encrypted_count > 0 {
            ...
            let encrypted_store =
                memora_secrets::SecretStore::open_for_data_dir(&data_dir, Some(key))
                    .await
    ...
    memora_secrets::mark_encryption_disabled(&data_dir)
...
/// The overlay is the app's only always-on surface â€” it carries recording
/// health, live meeting state and meeting notifications â€” so it ships
...
/// wait-for-server handshake â€” the server being down is often the incident.
pub(crate) async fn show_shortcut_reminder_impl(
    ...
    // The MEMORA shortcut only opens the timeline/rewind overlay, so the
        ...
        if native_shortcut_reminder::is_available() {
            ...
            if wait_for_server {
                ...
                loop {
                    ...
                    if waited >= MAX_WAIT {
                        warn!(
                            "native shortcut reminder: server core not ready after {:?} â€” pass authenticated metrics URLs to Swift after overlay is reopened",
                            MAX_WAIT
                        );
                        break;
                    }
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            let _ = app_handle.run_on_main_thread(move || {
                if let Ok(panel) = app_clone.get_webview_panel("shortcut-reminder") {
                    ...
                    // Re-set level, style mask, and behaviors on every show â€”
        ...
        if let Ok(_panel) = window.to_panel() {
            ...
            let _ = app_handle.run_on_main_thread(move || {
                ...
                if let Ok(panel) = window_clone.to_panel() {
                    ...
                    // style_mask(0) was wrong â€” it cleared NonActivatingPanel.
...
/// same stop â†’ settle â†’ spawn sequence as the native panel's restart action;
...
pub async fn show_notification_inbox(app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if native_notification::is_available() {
            ...
            // Anchor under the pill's live frame â€” it's draggable, so a fixed
...
pub(crate) async fn deliver_notification_panel(
    ...
    // Delivery gate â€” the single choke point that catches both `/notify` and
    ...
    // Repeat gate â€” see `gate::repeat_suppressed_now`. Critical alerts are
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        // second window into the corner. Anything else â€” and the pill being
        // hidden â€” keeps the standalone panel.
        if notification_belongs_to_overlay(notification_type.as_deref())
            && native_shortcut_reminder::show_notification(&payload)
        {
            info!("meeting notification rendered from the shortcut overlay");
    ...
    if let Some(window) = app_handle.get_webview_window(label) {
        ...
        // On macOS, skip window.show() â€” it calls makeKeyAndOrderFront which
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            let _ = app_handle.run_on_main_thread(move || {
                if let Ok(panel) = app_clone.get_webview_panel("notification-panel") {
                    ...
                    // NSNonactivatingPanelMask â€” shows the panel without
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if let Ok(_panel) = window.to_panel() {
            ...
            // Don't use window.show() â€” it calls makeKeyAndOrderFront which
            ...
            let _ = app_handle.run_on_main_thread(move || {
                ...
                if let Ok(panel) = window_clone.to_panel() {
                    ...
                    // orderFront: (not orderFrontRegardless) respects
                    // NSNonactivatingPanelMask â€” shows without stealing focus.
                    let _: () = unsafe { msg_send![&*panel, orderFront: std::ptr::null::<objc::runtime::Object>()] };
...
fn register_window_shortcuts_inner(app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    // Register Escape shortcut â€” emits event so frontend can decide
...
#[cfg(not(target_os = "macos"))]
pub(crate) fn any_MEMORA_webview_has_focus(app: &tauri::AppHandle) -> bool {
    app.webview_windows()
        .values()
...
/// overlay is still shown â€” otherwise Main's blur path unregisters Escape and
...
pub async fn rollback_to_version(
    ...
    if let Err(e) =
        crate::stop_MEMORA(app_handle.state::<RecordingState>(), app_handle.clone()).await
    {
...
pub async fn perform_ocr_on_image(
    ...
    let languages: Vec<memora_core::Language> = store
        .recording
        .languages
        .iter()
        .filter(|s| s != &"default")
    ...
    let (text, _text_json, _confidence) = match ocr_engine {
        #[cfg(target_os = "macos")]
        OcrEngine::AppleNative => memora_screen::perform_ocr_apple(&img, &languages),
        OcrEngine::Tesseract => memora_screen::perform_ocr_tesseract(&img, languages),
        #[cfg(target_os = "windows")]
        OcrEngine::WindowsNative => memora_screen::perform_ocr_windows(&img, &languages)
            .await
...
/// Copy a frame deeplink (MEMORA://frame/N) to clipboard. Native API only.
#[tauri::command]
#[specta::specta]
pub async fn copy_deeplink_to_clipboard(frame_id: i64) -> Result<(), String> {
    let link = format!("MEMORA://frame/{}", frame_id);
    let mut clipboard = arboard::Clipboard::new().map_err(|e| format!("clipboard error: {}", e))?;
    clipboard
        .set_text(link)
        .map_err(|e| format!("failed to set clipboard: {}", e))?;
...
pub async fn open_note_path(path: String) -> Result<(), String> {
    // Citations from the pi agent can be relative (e.g. `.pi/skills/â€¦`); resolve
...
pub async fn list_cache_files() -> Result<Vec<CacheFile>, String> {
    ...
    // Pi agent node_modules (~/.memora/pi-agent/)
    ...
    // Pi config (~/.memora/pi-config/). Never list the user's global
    // ~/.pi/agent here â€” that belongs to their standalone pi install and
    // offering to delete it risked destroying the user's own setup
    // (https://github.com/MEMORA/MEMORA/issues/4002).
    ...
    // Stale root-level node_modules (~/.memora/node_modules/)
    ...
    for entry in std::fs::read_dir(&data_dir).map_err(|e| e.to_string())? {
        ...
        // Old log files (MEMORA.*.log â€” legacy CLI format)
        if name.starts_with("MEMORA.") && name.ends_with(".log") {
        ...
        // Empty/stale DB files (data.db, memora.db, store.sqlite)
        if matches!(name.as_str(), "data.db" | "memora.db" | "store.sqlite") {
            let size = path.metadata().map(|m| m.len()).unwrap_or(0);
            if size == 0 {
                files.push(CacheFile {
                    path: path.to_string_lossy().to_string(),
