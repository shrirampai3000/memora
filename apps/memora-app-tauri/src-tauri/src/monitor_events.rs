...
pub fn start(app: AppHandle) {
    tauri::async_runtime::spawn(async move {
        let mut sub =
            memora_events::subscribe_to_event::<serde_json::Value>("monitor_topology_changed");

        while let Some(event) = sub.next().await {
            if !display_changes_enabled(&app) {
                debug!("monitor topology event â†’ notify: skipped (display-change toasts disabled)");
                continue;
            }
            if let Some((title, body)) = format_event(&event.data) {
                debug!("monitor topology event â†’ notify: {} | {}", title, body);
...
fn join_names<I: IntoIterator<Item = String>>(names: I) -> String {
    ...
    match v.len() {
        ...
        _ => {
            // Oxford-comma-ish â€” readable in a toast even when many names land.
...
fn format_event(data: &serde_json::Value) -> Option<(String, String)> {
    ...
    match (added.len(), removed.len()) {
        ...
        (1, 0) => Some((
            ...
            format!("{} â€” {}", pick_name(added[0]), recording_summary(active)),
        )),
        (n, 0) => Some((
        ...
        (0, 1) => Some((
            ...
            format!("{} â€” {}", pick_name(removed[0]), recording_summary(active)),
        )),
        (0, n) => Some((
        ...
        // Symmetric swap (1â†”1) â€” this is the common clamshell case: plug
        // in the external + lid stays closed â†’ the built-in goes inactive
        // at the same time. Reading "+1 / âˆ’1" for that is confusing; the
        ...
        // left â€” no asymmetric "+a / âˆ’r" arithmetic the user has to parse.
        (_, _) => Some((
...
mod tests {
    ...
    fn clamshell_swap_uses_arrow() {
        ...
        assert!(b.contains("MacBook Pro Display â†’ Studio Display"));
        assert!(b.contains("now recording 1 display"));
    }
