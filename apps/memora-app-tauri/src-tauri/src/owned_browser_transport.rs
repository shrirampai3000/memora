...
//! silently truncated any large result â€” a page snapshot, a long scrape â€” into
...
//!   then pulled chunk-by-chunk â€” each chunk is base64 (title-safe, no
...
/// Bridge installed as the child webview's `initialization_script` â€” defines
...
pub enum Marker {
    /// A complete inline result â€” the small-result fast path.
    Result(EvalPayload),
...
mod tests {
    ...
    fn reassembles_unicode() {
        // base64 of UTF-8 bytes â€” snapshots contain â†’ arrows and emoji.
        let original = "â†’ a11y tree â˜ƒ cafÃ© æ—¥æœ¬èªž";
    ...
    fn reassembles_large_payload_roundtrip() {
        let original: String = "x [a] node â†’ ref ".repeat(5000); // ~85KB
