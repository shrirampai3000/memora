...
//! install's state.
//!
//! Before this existed, running the dev app next to an installed MEMORA
...
//!   the port â€” i.e. the production engine, mid-write to a multi-GB SQLite DB.
...
//!   writes `~/.claude`, `~/.codex/config.toml`, `~/.cursor/mcp.json`, â€¦ and
...
//! top of `main()` â€” before the focus-port handoff, the DB-recovery-lock check,
...
//! and because child processes â€” the engine, Pi, ACP runtimes, bundled JS
//! extensions â€” inherit them for free.
//!
//! ## Contract
//!
...
//! * `MEMORA_DEV_USE_PROD_DATA=1` opts out entirely, for the rare case of
...
/// Data directory used by isolated debug builds. A sibling of `~/.memora`
...
pub const DEV_DATA_DIR_NAME: &str = ".memora-dev";
...
pub const AI_TOOLS_HOME_ENV: &str = "MEMORA_AI_TOOLS_HOME";

/// Opt-out: run a debug build against the production profile.
// Read only by the debug-only `apply_inner` (and the tests below).
#[cfg_attr(not(any(debug_assertions, test)), allow(dead_code))]
pub const OPT_OUT_ENV: &str = "MEMORA_DEV_USE_PROD_DATA";
...
#[cfg(debug_assertions)]
fn apply_inner() -> bool {
    // The E2E harness sets MEMORA_DATA_DIR / MEMORA_PORT /
    // MEMORA_FOCUS_PORT itself and compiles out the single-instance plugin.
    ...
    if is_truthy(OPT_OUT_ENV) {
        eprintln!(
            "MEMORA[dev]: {OPT_OUT_ENV} set â€” using the PRODUCTION profile \
             (~/.memora, port 3030). Quit the installed app first; this build \
    ...
    let Some(home) = dirs::home_dir() else {
        ...
        eprintln!("MEMORA[dev]: no home directory â€” dev isolation NOT applied");
        return false;
    };

    let data_dir: PathBuf = home.join(DEV_DATA_DIR_NAME);

    set_if_unset("MEMORA_DATA_DIR", &data_dir.to_string_lossy());
    set_if_unset("MEMORA_PORT", DEV_API_PORT);
    set_if_unset("MEMORA_FOCUS_PORT", DEV_FOCUS_PORT);
    // Dev sessions are not product usage; keep them out of PostHog/Sentry.
    set_if_unset("MEMORA_DISABLE_TELEMETRY", "1");
    ...
    eprintln!(
        "MEMORA[dev]: isolated from production â€” data {}, api :{}, focus :{}. \
         Set {OPT_OUT_ENV}=1 to use the production profile instead.",
        env_or("MEMORA_DATA_DIR", &data_dir.to_string_lossy()),
        env_or("MEMORA_PORT", DEV_API_PORT),
        env_or("MEMORA_FOCUS_PORT", DEV_FOCUS_PORT),
    );
...
fn set_if_unset(key: &str, value: &str) {
    ...
    // `default_MEMORA_data_dir()`'s handling of MEMORA_DATA_DIR.
...
fn is_truthy(key: &str) -> bool {
    // Lenient, matching the engine's MEMORA_DISABLE_TELEMETRY handling:
...
mod tests {
    ...
    fn dev_data_dir_is_not_inside_the_production_data_dir() {
        // A child of ~/.memora would be walked by production's disk-usage
        // accounting and retention sweeps.
        assert!(!DEV_DATA_DIR_NAME.starts_with(".memora/"));
        assert_ne!(DEV_DATA_DIR_NAME, ".memora");
    }
    ...
    fn is_truthy_accepts_common_spellings_and_rejects_others() {
        ...
        let key = "MEMORA_DEV_ISOLATION_TRUTHY_TEST";
    ...
    fn set_if_unset_never_overrides_an_explicit_value() {
        ...
        let key = "MEMORA_DEV_ISOLATION_SET_IF_UNSET_TEST";
