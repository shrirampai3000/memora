...
use super::{
    bounded_teardown, spawn_MEMORA_inner, RecordingState, TeardownOutcome,
};
...
enum WedgeAction {
    /// Attempt a stopâ†’spawn restart (the timestamp was recorded).
    Restart,
    /// Too many restarts in the window â€” don't restart. `notify` is true only
...
fn db_wedge_recovery_decision(
    signaled_health: &memora_db::WriteQueueHealth,
    signaled_recovery_epoch: u64,
    current_health: Option<&memora_db::WriteQueueHealth>,
...
pub fn make_db_wedge_recovery_hook(
    ...
    health: memora_db::WriteQueueHealth,
) -> memora_db::PersistentFailureHook {
...
async fn recover_from_db_wedge(
    ...
    if !hard_faulted {
        ...
        if let WedgeAction::GiveUp { notify } = action {
            ...
            drop(capture_guard);
            error!(
                "db wedge auto-recovery: {} restarts within {:?} did not clear the write wedge â€” \
    ...
    warn!(
        "db wedge auto-recovery: persistent write failure detected â€” restarting recording to \
    ...
    *recording_state.interrupted_meeting.lock().await = None;
    ...
    // reaping) that would otherwise keep a connection â€” and the poisoned `-shm`
    // WAL-index â€” open across the restart. SQLite only rebuilds `-shm` once the
    ...
    if let Err(e) = spawn_MEMORA_inner(&recording_state, app.clone()).await {
        ...
        // again â€” and if the server is fully down it never will â€” so recording
        ...
        error!("db wedge auto-recovery: spawn_MEMORA failed: {}", e);
        let evt = memora_events::DbRecoveryEvent::restart_failed();
        let _ = memora_events::send_event(evt.event_name(), evt);
        // A DB-init failure right after a full pool close means the WAL-index
        // is pinned by something outside our teardown â€” count it toward the
...
mod tests {
    ...
    use memora_db::WriteQueueHealth;
    ...
    fn gives_up_after_cap_and_notifies_once() {
        ...
        // Hook keeps firing while tripped â€” no more notifications.
        assert_eq!(
            s.decide(t, WINDOW, MAX),
    ...
    // trips the breaker â€” every attempt restarts and nothing is suppressed.
    #[test]
