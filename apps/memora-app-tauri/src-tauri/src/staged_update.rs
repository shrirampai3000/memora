...
//! Deferred update install â€” macOS only.
//!
...
//! declined" â€” vision and system-audio capture die until the app is fully
...
//! gunzip+untar ~420 MB â€” a 10â€“40 s window (M4 Max â†’ old Air) in which the
...
//! `rename(2)` calls (bundle â†’ scratch, extracted â†’ bundle). If anything
//! about the pre-extracted tree is off â€” wrong shape, cross-volume rename,
//! permission error â€” the exit path falls back to the original
...
struct Staged {
    ...
    /// when extraction failed or the archive had an unexpected shape â€” the
...
/// bundle parked by the rename fast path, so deletion runs off-thread â€” boot
...
pub fn clear_stage_dir<R: tauri::Runtime>(app: &tauri::AppHandle<R>) {
    ...
    if dir.exists() {
        ...
        match std::fs::rename(&dir, &renamed) {
            ...
            // Rename failed (unusual) â€” delete in place, still off-thread.
...
/// `â€¦/MEMORA.app/Contents/MacOS/MEMORA-app` â†’ `â€¦/MEMORA.app`.
...
/// `.app`, extraction error) is an `Err` â€” callers treat that as "no fast
...
/// install-at-exit. Replaces any previously staged version â€” with the
...
fn stage_impl<R: tauri::Runtime>(
    ...
    // Pre-extract now, while the app is alive and can afford the CPU/IO â€”
...
/// Install the staged update if one exists. Idempotent â€” the first caller
/// takes the handle, later callers no-op â€” because restart paths can reach
...
pub fn install_staged_if_any<R: tauri::Runtime>(app: &tauri::AppHandle<R>) -> bool {
    ...
    if let (Some(extracted), Some(bundle)) = (extracted_app.as_deref(), bundle_root.as_deref()) {
        let scratch = path
            ...
            .unwrap_or_else(|| std::env::temp_dir().join("MEMORA-replaced"));
...
/// `tauri_plugin_updater` check â†’ download â†’ `stage()` â†’ `install_staged_if_any()`
/// against a throwaway minisign key, a localhost HTTP server, and fake .app
/// bundles in a temp dir. No MEMORA process, no real bundle, no network.
#[cfg(test)]
mod tests {
    use super::*;
    use base64::Engine;
    ...
    async fn staged_update_full_cycle_installs_via_pre_extracted_fast_path() {
        ...
        // Install at "exit": swap happens via renames â€” prove it by deleting
    ...
    async fn staged_update_without_pre_extraction_falls_back_to_full_install() {
        ...
        // No bundle root (dev-style executable) â†’ fast path disabled even
    ...
    /// bundle rename and fall through to the verified full install â€” never
    ...
    fn bundle_root_requires_macos_bundle_shape() {
        assert_eq!(
            bundle_root_from_exe(Path::new(
                "/Applications/MEMORA.app/Contents/MacOS/MEMORA-app"
            )),
            Some(PathBuf::from("/Applications/MEMORA.app"))
        );
        // dev binary â†’ no bundle â†’ fast path disabled
        assert_eq!(
            bundle_root_from_exe(Path::new("/tmp/target/debug/MEMORA-app")),
            None
        );
    ...
    fn pre_extract_rejects_multi_root_archives() {
        let tmp = tempfile::tempdir().unwrap();
        // two top-level entries â†’ not the expected updater artifact shape
