...
//! call site â€” startup, health incidents, meeting state, notifications, the E2E
//! seams â€” work on windows without a per-platform branch, and it is why linux
...
use memora_overlay_win::state::{Anchor, Health, OverlaySize, OverlayState};
use memora_overlay_win::transcript::{FeedItem, TranscriptFeed};
use memora_overlay_win::Overlay;
...
/// The stop button's outcome. The native pill has no separate stopping state â€”
...
/// â€” the sockets were then never opened for the rest of the session. That is a
...
async fn metrics_feeder(url: String) {
    feed(url, "metrics", |m| {
        // Raw RMS is 0.001-0.05 for speech; the same Ã—15 the webview overlay
...
mod tests {
    ...
    /// meant the socket was never opened for the rest of the session â€” a pill
    ...
    fn a_feed_with_no_url_yet_does_not_burn_its_one_chance() {
        ...
        assert_eq!(
            ...
            None,
            "and only that one does â€” the feed reconnects on its own"
        );
    }
}
