...
/// Shared panel visibility sequence â€” the single source of truth for making an
...
///
/// Steps: save_frontmost_app â†’ alpha:1 â†’ [optional activate] â†’
///        order_front â†’ make_key â†’ first_responder â†’ emit("window-focused")
///
...
///
/// Default is `Regular` â€” dock icon and tray both visible. Three cases switch to
...
///   3. the user opted into `hideDockIcon` in Settings â†’ Display.
///
...
/// toggles the setting) â€” NOT the per-tick watchdog toggling that was removed
...
pub fn reset_to_regular_and_refresh_tray(app: &AppHandle) {
    if !super::window_activation_allowed() {
        info!("Setting activation policy to Accessory (e2e â€” non-activating test run)");
    ...
    if crate::headless::is_dormant() {
        info!("Setting activation policy to Accessory (headless â€” tray only)");
        let _ = app.set_activation_policy(tauri::ActivationPolicy::Accessory);
        return;
    }
    ...
    if hide_dock_icon {
        info!("Setting activation policy to Accessory (hideDockIcon â€” menu bar only)");
        let _ = app.set_activation_policy(tauri::ActivationPolicy::Accessory);
        return;
    }
