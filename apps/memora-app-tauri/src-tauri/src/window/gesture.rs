...
pub fn init_magnify_handler(app: tauri::AppHandle) {
    ...
    if Class::get("MEMORAMagnifyHandler").is_none() {
        let superclass = Class::get("NSObject").unwrap();
        let mut decl = ClassDecl::new("MEMORAMagnifyHandler", superclass).unwrap();
        extern "C" fn handle_magnify(_this: &Object, _sel: Sel, recognizer: *mut Object) {
            // ObjCâ†’Rust trampoline: a panic here aborts the app via
    ...
    // wheel events at the native level â€” they never reach JavaScript.
    ...
    if Class::get("MEMORAScrollInterceptor").is_none() {
        ...
        extern "C" fn swizzled_scroll_wheel(this: &Object, sel: Sel, event: *mut Object) {
            // This fires on EVERY WKWebView scroll event â€” a panic in
            ...
            // Always forward to the original scrollWheel: â€” outside the
        ...
        let superclass = Class::get("NSObject").unwrap();
        let decl = ClassDecl::new("MEMORAScrollInterceptor", superclass).unwrap();
        decl.register();
    }
}
...
/// Safe to call multiple times â€” skips if already attached.
/// Safe to call before init_magnify_handler â€” returns early if the class isn't registered yet.
#[cfg(target_os = "macos")]
pub(crate) unsafe fn attach_magnify_gesture_to_view(view: tauri_nspanel::cocoa::base::id) {
    with_autorelease_pool(|| {
        ...
        // Look up the handler class safely â€” if init_magnify_handler hasn't run
        // yet the class won't exist. Return early instead of panicking.
        let handler_class = match Class::get("MEMORAMagnifyHandler") {
            Some(cls) => cls,
            None => {
                tracing::warn!(
                    "MEMORAMagnifyHandler class not registered yet â€” skipping gesture attach"
                );
                return;
            }
        };

        // Check if we already added our recognizer (look for MEMORAMagnifyHandler target)
