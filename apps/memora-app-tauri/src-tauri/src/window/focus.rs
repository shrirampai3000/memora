...
/// clicks/shows from replacing the real origin with MEMORA itself.
#[cfg(target_os = "macos")]
...
/// panel must not overwrite it after macOS has activated MEMORA.
#[cfg(target_os = "macos")]
...
/// activation to MEMORA while the non-activating panel was being used,
/// reactivate the original app so dismissing Search does not reveal MEMORA.
...
/// Used when the user intentionally switches Spaces â€” we don't want to
...
/// Check if our own app (MEMORA) is still the active macOS application.
/// When the overlay panel loses focus to another MEMORA window (Settings,
...
/// If our own app is still active (focus moved to another MEMORA window
/// like Settings), we just clear without restoring â€” otherwise Settings/Chat
/// would lose keyboard focus. Also skip restoring if any MEMORA window
/// (like Settings) is still open â€” during workspace swipes the app
...
pub(crate) fn restore_frontmost_app_if_external_with_app(app: Option<&AppHandle>) {
    ...
    if is_own_app_still_active() {
        info!("Focus moved to another MEMORA window, clearing saved app without restoring");
        clear_frontmost_app();
        return;
    }

    // Check if Settings, Chat, or Search windows exist in Tauri's registry.
    // These are regular windows (not panels) that may be on another Space â€”
    ...
    if let Some(app) = app {
        ...
        for label in &non_panel_labels {
            if app.get_webview_window(label).is_some() {
                info!(
                    "MEMORA '{}' window still open, clearing saved app without restoring",
                    label
                );
                clear_frontmost_app();
                return;
...
mod tests {
    ...
    #[test]
    fn restores_only_when_search_activated_MEMORA() {
