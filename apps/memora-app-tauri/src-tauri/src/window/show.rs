...
pub fn apply_chat_panel_on_top(panel: &tauri_nspanel::raw_nspanel::RawNSPanel, on_top: bool) {
    ...
    if on_top {
        ...
        // Normal window level â€” allow it to go behind other windows
        panel.set_level_gated(0);
...
impl RewindWindowId {
    ...
    pub fn title(&self) -> &str {
        match self {
            RewindWindowId::Main => "MEMORA",
            RewindWindowId::Home => "MEMORA",
            RewindWindowId::Search => "search",
            RewindWindowId::Onboarding => "onboarding",
...
/// was narrower than the header's own contents â€” the query input and the four
/// scope segments cannot both fit â€” so one of them got squeezed to nothing.
...
impl ShowRewindWindow {
    fn window_builder_with_label<'a>(
        ...
        // under the traffic lights â€” sibling helper window_builder() opts into
    ...
    fn show_existing_main(
        ...
        if overlay_mode == "window" {
            ...
            #[cfg(target_os = "windows")]
            {
                ...
                // Without this, re-showing an already-created overlay when another MEMORA
                // window (e.g. Home) holds focus leaves the overlay visible but not interactive â€”
    ...
    pub fn show(&self, app: &AppHandle) -> tauri::Result<WebviewWindow> {
        ...
        if id.label() == RewindWindowId::Main.label() {
            ...
            // No existing window for this mode â€” fall through to creation below
        ...
        // === Other windows: standard show path ===
            ...
            if id.label() == RewindWindowId::Search.label() {
                ...
                // rather than destroying it â€” see `close()` below), so a full
            ...
            if id.label() == RewindWindowId::Chat.label() {
                #[cfg(target_os = "macos")]
                {
                    // NOTE: Accessory mode removed â€” it hides dock icon and tray on notched MacBooks
        ...
        let window = match self {
            ShowRewindWindow::Main => {
                ...
                *MAIN_CREATED_MODE.lock().unwrap_or_else(|e| e.into_inner()) = overlay_mode.clone();
                ...
                if use_window_mode {
                    ...
                    // Created hidden â€” shown after webview + panel setup.
                    // ============================================================
                    ...
                    #[cfg(target_os = "macos")]
                    let window = {
                        // NOTE: Do NOT switch to Accessory mode here â€” it hides dock icon
                        ...
                        if let Ok(_panel) = window.to_panel() {
                            ...
                            run_on_main_thread_safe(app, move || {
                                ...
                                if let Ok(panel) = window_clone.to_panel() {
                                    // Same level as overlay â€” above fullscreen
                                    panel.set_level_gated(1001);
                    ...
                    window.on_window_event(move |event| {
                        match event {
                            tauri::WindowEvent::Focused(is_focused) => {
                                if !is_focused {
                                    // Synchronous alpha=0 â€” no order_out (which
                                    ...
                                    std::thread::spawn(move || {
                                        ...
                                        // Dispatch all AppKit work to main thread â€” this
                                        ...
                                        #[cfg(target_os = "macos")]
                                        {
                                            ...
                                            let _ = app.run_on_main_thread(move || {
                                                ...
                                                // MEMORA window (Settings, Chat), just clear.
                                        ...
                                        // and scheduled unregister â€” do not schedule a second unregister here.
                                    ...
                                    #[cfg(target_os = "macos")]
                                    {
                                        ...
                                        if let Ok(panel) = app_clone.get_webview_panel("main-window") {
                                            unsafe {
                                                ...
                                                // show_existing_main â€” NonActivatingPanel
                ...
                // Start hidden â€” show after webview finishes loading to avoid
                    ...
                    if let Ok(_panel) = window.to_panel() {
                        ...
                        run_on_main_thread_safe(app, move || {
                            ...
                            if let Ok(panel) = window_clone.to_panel() {
                                ...
                                // key window without activating the app â€” this prevents
                ...
                // On Linux the main window is a normal decorated window â€” don't auto-hide on focus loss,
                    ...
                    match event {
                        ...
                        tauri::WindowEvent::Focused(is_focused) => {
                            if !is_focused {
                                ...
                                // Synchronous alpha=0 â€” panel stays in window list
                                ...
                                std::thread::spawn(move || {
                                    ...
                                    #[cfg(target_os = "macos")]
                                    {
                                        ...
                                        let _ = app.run_on_main_thread(move || {
                                            // Conditional restore: if focus moved to another
                                            // MEMORA window, just clear. Only activate
                        ...
                        tauri::WindowEvent::ScaleFactorChanged { scale_factor: _, new_inner_size: _,.. } => {
                            // Handle display resolution/scale changes â€” use the window's current monitor
            ...
            ShowRewindWindow::Home { page } => {
                ...
                // when Home blurs and no MEMORA window has focus (user left the app).
                #[cfg(not(target_os = "macos"))]
                {
                        ...
                        if let tauri::WindowEvent::Focused(focused) = event {
                            if !focused {
                                ...
                                std::thread::spawn(move || {
                                    ...
                                    if crate::commands::any_MEMORA_webview_has_focus(&app_c) {
                                        return;
                                    }
                                    if crate::commands::is_main_close_in_progress() {
                                        return;
                                    ...
                                    std::thread::spawn(move || {
                                        info!(
                                            "shortcut-sync: unregister execute (reason=home_blur_no_MEMORA_focus)"
                                        );
                                        let _ = crate::commands::unregister_window_shortcuts_if_generation_unchanged(
                                            app2,
                                            expected_gen,
                                            "home_blur_no_MEMORA_focus",
                                        );
                                    });
                                });
                            } else {
            ...
            ShowRewindWindow::Chat => {
                ...
                let window = {
                    ...
                    // Only configure level + behaviors here â€” do NOT activate
    ...
    /// is created hidden and NOT activated â€” used by `prewarm_search` so startup
    ...
    fn create_search_window(
        ...
        // Skip tauri-nspanel to_panel() â€” it causes SIGSEGV in
            ...
            run_on_main_thread_safe(app, move || {
                ...
                if let Ok(ns_win) = window_clone.ns_window() {
                    ...
                    unsafe {
                        ...
                        // Level 1002 â€” above fullscreen (CGShieldingWindowLevel+2)
                        crate::window::ns::set_level_gated(ns_win, 1002_i64);

                        // NSNonactivatingPanelMask (128) â€” appear over fullscreen
    ...
    /// Used when transitioning from Main to another MEMORA window (e.g. Home/Settings)
        ...
        if id.label() == RewindWindowId::Main.label() {
            #[cfg(target_os = "macos")]
            {
                ...
                run_on_main_thread_safe(app, move || {
                    ...
                    // Intentionally do NOT call restore_frontmost_app() here â€”
                    // we're transitioning to another MEMORA window.
                });
            }
    ...
    pub fn close(&self, app: &AppHandle) -> tauri::Result<()> {
        ...
        if id.label() == RewindWindowId::Main.label() {
            #[cfg(target_os = "macos")]
            {
                ...
                // status (alphaâ†’0), then NSNonactivatingPanelMask let it reassert
                // key (alphaâ†’1), and only THEN did order_out run â€” creating a
            ...
            #[cfg(target_os = "windows")]
            {
                // Hide overlay windows â€” the Home window is the persistent taskbar
