...
///
/// Always true in shipping builds â€” this never changes what users see.
#[cfg(feature = "e2e")]
pub fn window_activation_allowed() -> bool {
    ...
    INIT.call_once(|| {
        let opted_in = matches!(
            std::env::var("MEMORA_E2E_ALLOW_ACTIVATION").as_deref(),
            Ok("1") | Ok("true")
        );
...
/// They are the loudest thing in a suite run â€” floating level plus
...
/// `CanJoinAllSpaces`/`MoveToActiveSpace` â€” that combination is exactly what
/// lets MEMORA draw on top of whatever you are doing, including another
...
///
/// `makeKeyWindow` is what actually pulls MEMORA to the foreground (and, for
...
///
/// The gates above stop MEMORA *taking focus*, but focus is only half of
...
pub(crate) fn sink_below_apps_if_non_intrusive(window: &tauri::WebviewWindow) {
    ...
    // on whichever thread created the window â€” often a tokio worker. Reaching
...
///
/// The per-callsite gates above stop MEMORA *asking* for the foreground, but
...
/// `NSApplicationDidBecomeActiveNotification` is the backstop â€” whatever wins the
...
/// creation, before any of the runtime gates above can run â€” so a non-intrusive
...
///
/// The startup window gate (`main.rs`) already honors `is_app_ui_hidden()` â€”
...
/// to Rust via `set_enterprise_policy` â€” long after the Home/Onboarding window
...
///   * hidden  â†’ order main app windows off-screen and drop the dock icon
...
///     finished.
///   * visible â†’ restore the normal activation policy + full tray menu and
///     reopen Home immediately, unless the user separately enabled Headless.
///
/// Permission-recovery is intentionally never hidden â€” a managed background
...
pub fn enforce_enterprise_ui_visibility(app: &tauri::AppHandle) {
    ...
    // on an actual transition â€” otherwise we'd tear down an already-dormant UI
    ...
    if hidden {
        ...
        tracing::info!("enterprise: hidden-UI policy enforced â€” headless teardown");
    } else {
        ...
        if crate::headless::set_enterprise_hidden(app, false) {
            ...
            if let Err(error) = app.run_on_main_thread(move || {
                match ( ShowRewindWindow::Home { page: None } ).show(&app_for_show) {
                    Ok(_) => tracing::info!(
                        "enterprise: hidden-UI policy disabled â€” Home window restored"
                    ),
