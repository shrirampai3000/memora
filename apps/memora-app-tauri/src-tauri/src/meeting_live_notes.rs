...
struct CalendarMatch {
    title: Option<String>,
    /// RFC3339 start time of the matched event â€” paired with title in the
...
pub fn start(app: AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            memora_events::subscribe_to_event::<Vec<CalendarEventSignal>>("calendar_events");
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            memora_events::subscribe_to_event::<MeetingPrewarmEvent>("meeting_about_to_start");
        while let Some(event) = sub.next().await {
            ...
            // note deeplink yet â€” keeps the plain "+ HD" label.
            ...
            client::send_typed_with_actions_and_priority(
                &header,
                format!("MEMORA is ready to transcribe: {title}"),
                "meeting",
                Some(30_000),
                actions,
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            memora_events::subscribe_to_event::<MeetingStartedEvent>("meeting_started");
        ...
        while let Some(event) = sub.next().await {
            ...
            // note â€” this notification fires at meeting start and the user
            // clicked "join and take notes", so they want the live view, not
            // a static note pane.
            let url = format!("MEMORA://meeting/{meeting_id}");
            ...
            // (â‰¤60s stale) and re-fetching just wastes API calls â€” most
            ...
            // "join and take notes" is exclusively the prewarm CTA â€” the v2
            ...
            client::send_typed_with_actions_and_priority(
                "meeting detected",
                format!("MEMORA is saving this meeting for transcription: {title}"),
                "meeting",
                Some(30_000),
                actions,
                NotificationPriority::High,
...
fn forward_MEMORA_event(app: AppHandle, source: &'static str, target: &'static str) {
    tauri::async_runtime::spawn(async move {
        let mut sub = memora_events::subscribe_to_event::<serde_json::Value>(source);
        while let Some(event) = sub.next().await {
...
async fn was_prewarmed(
    ...
    // `title|start` match â€” recurring same-title events must not bleed into
    ...
    // Fallback only when calendar enrichment failed â€” title is the best
...
/// - `"ask"` (default) â€” yes, add the action.
/// - `"always"` â€” no, the engine auto-starts the session itself; an action
///   here would be a confusing no-op.
/// - `"never"` â€” no, the user has opted out.
...
/// the whole call â€” not the previous 1-hour timer that clipped long calls.
///
/// `note_url` is the live-note deeplink. When present (started
/// notification), the button both opens the note and starts HD in one
/// click â€” labeled "open note + HD" â€” so the user doesn't have to choose
...
mod tests {
    ...
    async fn dedup_distinguishes_recurring_same_title_events() {
        ...
        // 9:00 started toast â†’ should be suppressed (prewarm already fired).
        ...
        assert!(
            ...
            "9:00 started toast must be suppressed â€” its prewarm fired"
        );

        // 9:30 started toast (same title, different start) â†’ should NOT be
        ...
        assert!(
            ...
            "9:30 started toast must fire â€” its prewarm never did"
        );
    }
    ...
    async fn dedup_untitled_prewarm_does_not_suppress_titled_meeting_at_same_start() {
        let suppressed: Arc<RwLock<HashMap<String, Instant>>> =
            Arc::new(RwLock::new(HashMap::new()));
        let start = "2026-05-25T09:00:00+00:00";
        // An untitled event prewarmed at this start â†’ guard holds `|{start}`.
        suppressed
            .write()
            .await
            .insert(prewarm_key("", start), Instant::now());
