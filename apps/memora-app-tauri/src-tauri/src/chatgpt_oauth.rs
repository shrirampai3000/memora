...
//!  1. PKCE authorize â†’ auth code
//!  2. Exchange auth code â†’ access_token + refresh_token
...
//! key in OS keychain) via `memora_secrets::SecretStore`.
...
/// The legacy migration in memora-secrets also writes to this key
/// when it finds chatgpt-oauth.json.
const SECRET_KEY: &str = "oauth:chatgpt";

// â”€â”€ Token storage â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async fn open_secret_store() -> Result<memora_secrets::SecretStore, String> {
...
async fn write_tokens_to_store(tokens: &OAuthTokens) -> Result<(), String> {
    ...
    // Retry up to 3 times â€” the MEMORA server may hold a brief write lock.
    let mut last_err = String::new();
    for attempt in 0..3u32 {
        if attempt > 0 {
            tokio::time::sleep(std::time::Duration::from_millis(200 * (attempt as u64))).await;
...
// â”€â”€ Account-id claim validation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/// the claim â€” the same check pi's Codex provider performs on every request.
...
// â”€â”€ PKCE helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Token refresh â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async fn do_refresh_token(refresh_token: &str) -> Result<OAuthTokens, String> {
    ...
    // turn fail in the Codex provider â€” keep the old (still stored) token
    // instead of overwriting a working credential.
    if extract_chatgpt_account_id(&new_access_token).is_none() {
        warn!("ChatGPT token refresh returned a token without chatgpt_account_id â€” not storing it");
...
// â”€â”€ Background proactive refresh â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
///
/// The existing `OAuthRefreshScheduler` only handles memora-connect
/// integrations (Google Calendar, Zoom, etc.) â€” ChatGPT uses its own
/// direct OAuth flow, so it needs a dedicated refresh loop.
pub fn start_background_refresh() {
    tauri::async_runtime::spawn(async move {
        // Startup delay â€” avoid fighting cold-start resources.
...
// â”€â”€ Local callback server â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        match code {
            Some(code) => {
                let html = concat!(
                    ...
                    "<p>You can close this tab and return to MEMORA.</p>",
...
// â”€â”€ Tauri commands â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub async fn chatgpt_oauth_login(
    ...
    if extract_chatgpt_account_id(&access_token).is_none() {
        warn!("ChatGPT OAuth login returned a token without chatgpt_account_id â€” rejecting login");
        return Err(MISSING_ACCOUNT_ID_MSG.to_string());
    }
    ...
    write_tokens_to_store(&tokens).await?;
    info!("ChatGPT OAuth login successful â€” token saved to secret store");

    // Activate MEMORA without forcing the legacy `main` window or changing
...
pub async fn chatgpt_oauth_status() -> Result<ChatGptOAuthStatus, String> {
    // Only check token existence â€” no network refresh here.
