...
/// Monitor-name prefix in a media filename: `monitor_1_2026-â€¦` or
/// `Display 3 (output)_2026-â€¦`. Compiled once â€” this used to be built inside
...
/// Recording date in a media filename (`â€¦_YYYY-MM-DD_HH-MM-SS.ext`).
...
///
/// Walking the MEMORA data dir is O(files) and that tree reaches hundreds
...
pub enum Freshness {
    ...
    ///
    /// For background callers â€” telemetry, health pings â€” where the number is
...
pub struct CachedDiskUsage {
    pub timestamp: i64,
    pub usage: DiskUsage,
    /// The MEMORA data directory this entry was computed for. Used to
    /// invalidate the cache when the user switches data dirs in Settings â€”
    /// otherwise we'd return stale sizes from the previous location for up
    /// to an hour (see #2987).
    #[serde(default)]
    pub MEMORA_dir: String,
}
...
/// Everything `disk_usage` needs from the MEMORA directory tree.
///
...
/// walked `pipes/` a third time â€” three passes over the same inodes for
/// numbers that one pass can produce.
#[derive(Debug, Default, PartialEq, Eq)]
pub struct TreeScan {
    /// Every file under the MEMORA dir.
    pub total_bytes: u64,
    ...
    pub pipes_bytes: u64,
    /// `*.log` files sitting directly in the MEMORA dir.
    pub logs_bytes: u64,
...
enum Subtree {
    ...
    Other,
    /// Directly inside `data/` â€” media *and* `recording_since` apply here.
    DataRoot,
    /// Nested under `data/` â€” media applies, `recording_since` does not.
    DataNested,
...
fn attribute_file(scan: &mut TreeScan, subtree: Subtree, file_name: &str, size: u64) {
    ...
    if extension == "mp4" {
        ...
        // "MacBook Pro Microphone (input)_2026-â€¦mp4".
...
/// Single recursive pass over the MEMORA directory.
///
...
/// a symlink is counted at its own size â€” matching what `directory_size` did.
...
fn scan_tree_inner(dir: &Path, subtree: Subtree, scan: &mut TreeScan) -> io::Result<()> {
    for entry in fs::read_dir(dir)? {
        ...
        if file_type.is_dir() {
            ...
            // must not abort the whole scan â€” the old code let one bad dir
...
/// Walk `MEMORA_dir` once and collect every size `disk_usage` reports.
pub fn scan_tree(MEMORA_dir: &Path) -> io::Result<TreeScan> {
    SCAN_COUNT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    let mut scan = TreeScan::default();
    if !MEMORA_dir.exists() {
        return Ok(scan);
    }
    scan_tree_inner(MEMORA_dir, Subtree::Other, &mut scan)?;

    // Logs are only the `*.log` files directly in the MEMORA dir.
    if let Ok(entries) = fs::read_dir(MEMORA_dir) {
...
fn read_cache(
    ...
    // `MEMORA_dir` defaults to "" on older cache entries â€” those predate
    ...
    let cached_key_normalized = canonical_dir_key(Path::new(&cached.MEMORA_dir));
    if cached.MEMORA_dir.is_empty() || cached_key_normalized != current_dir_key {
        info!(
            "Cache dir mismatch (cached={}, current={}), recalculating",
            cached.MEMORA_dir, current_dir_key
        );
        return None;
    }
...
pub async fn disk_usage(
    ...
    disk_usage_in(MEMORA_dir, &cache_dir, freshness).await
}
...
pub(crate) async fn disk_usage_in(
    ...
    let data_dir = MEMORA_dir.join("data");
    let cache_file = cache_dir.join("disk_usage.json");

    let current_dir_key = canonical_dir_key(MEMORA_dir);
    ...
    // for our own â€” this is what collapses a burst into a single traversal.
    //
    ...
    info!("Calculating disk usage for {}", MEMORA_dir.display());
    ...
    for file_name in [
        ...
        let db_path = MEMORA_dir.join(file_name);
        if db_path.exists() {
    ...
    // Calculate "other" â€” everything not accounted for above
    ...
    let available_space = {
        ...
        let path_obj = Path::new(&MEMORA_dir);
        let available = sys
            .disks()
            .iter()
    ...
    // Cache the result â€” keyed by data dir so switching dirs invalidates it
    let cached = CachedDiskUsage {
        timestamp: chrono::Local::now().timestamp(),
        usage: disk_usage.clone(),
        MEMORA_dir: current_dir_key,
    };
...
mod tests {
    ...
    mod reference {
        ...
        pub fn logs(MEMORA_dir: &Path) -> u64 {
            let mut logs = 0;
            if let Ok(entries) = fs::read_dir(MEMORA_dir) {
                for entry in entries.flatten() {
                    let path = entry.path();
    ...
    fn build_tree(root: &Path) {
        ...
        write(&root.join("MEMORA.2026-08-14.log"), 70);
        ...
        // audio-by-name mp4 â€” must count as audio, not video
        write(
    ...
    fn recording_since_uses_oldest_direct_child_only() {
        ...
        let root = tmp.path();
        build_tree(root);
        // nested file is dated 2001 but must not win â€” only direct children count
        assert_eq!(
    ...
    async fn cache_is_invalidated_when_the_data_dir_changes() {
        ...
        let other = other_tmp.path().join("MEMORA");
