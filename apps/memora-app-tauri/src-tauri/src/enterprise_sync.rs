...
//! `apps/memora-app-tauri/src-tauri/src/enterprise/sync.rs`.
//! This shim only:
//!   - implements the `LocalApiClient` trait against the running MEMORA
...
//!
//! Everything is gated by `#[cfg(feature = "enterprise-build")]` â€” consumer
...
mod imp {
    ...
    // â”€â”€â”€ Local API client over the running MEMORA HTTP server â”€â”€â”€â”€â”€â”€

    pub(super) struct MEMORALocalClient {
        api_url_base: String,
        // App handle, so we can re-read the current api_auth_key from
        // `RecordingState` on every request. The previous implementation
        // captured the key once at startup â€” but `enterprise_sync::spawn`
    ...
    // Minimal response shape â€” only the fields we forward upstream. Decoupled
    ...
    enum LocalSearchItem {
        ...
        Audio(LocalAudio),
        Input(LocalInput),
        // Memory/UI variants ignored â€” only the ones we sync are listed.
        #[serde(other)]
        Other,
    }
    ...
    // /memories has a different envelope from /search â€” it's a paginated list,
    ...
    impl LocalApiClient for MEMORALocalClient {
        async fn fetch_frames_since(
            ...
            // /search returns DESC by default â€” sort ASC so the cursor advances
            // monotonically.
        ...
        async fn fetch_ui_events_since(
            ...
            // `ui_events` table â€” clicks, keystrokes, focus changes,
            // clipboard. (The DB enum is `Input`, not `UI` â€” `UiContent`
            // is a separate deprecated content type for AX text snapshots.)
            // Only sync events that have an element_name (real AX-resolved
            // targets) â€” keystroke noise without element context isn't
        ...
        async fn fetch_latest_snapshot(&self) -> Result<Option<SnapshotRow>, EnterpriseSyncError> {
            ...
            // frame therefore failed on every tick â€” snapshots never synced and
            ...
            for (frame_id, ts) in candidates {
                ...
                // expected here â€” a 404 means the frame is still in the
                ...
                // Step 3: decode â†’ resize 320Ã—180 â†’ JPEG @ Q60 â†’ base64.
                // Done in spawn_blocking â€” image decoding is CPU-bound and we
            ...
            // Skip this tick quietly â€” a later tick will catch a flushed frame.
            Ok(None)
        }
        ...
        async fn fetch_frame_jpeg(
            ...
            // Same image `/frames/{id}` serves in the UI â€” decoded from local
            ...
            if resp.status() == reqwest::StatusCode::NOT_FOUND {
                // Expired from retention or never existed â€” report back so the
    ...
    // â”€â”€â”€ Spawn â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn license_key_from_env_or_config() -> Option<String> {
        std::env::var("MEMORA_ENTERPRISE_LICENSE_KEY")
            .ok()
            .filter(|s| !s.trim().is_empty())
    ...
    /// (crate::web_base â€” `NEXT_PUBLIC_MEMORA_WEB_URL` at build time).
    fn default_policy_url() -> String {
        crate::web_base::MEMORA_web_url("/api/enterprise/policy")
    }
    ...
    fn spawn_hidden_ui_policy_watcher(app: &tauri::AppHandle) {
        ...
        tauri::async_runtime::spawn(async move {
            let policy_url = std::env::var("MEMORA_ENTERPRISE_POLICY_URL")
                .ok()
    ...
    pub fn configure_telemetry_context(app: &tauri::AppHandle) {
        ...
        set_env_default("MEMORA_ENTERPRISE_LICENSE_HASH", &org_key);
        set_env_default("MEMORA_CUSTOMER_ID", &org_key);
        set_env_default("MEMORA_ORG_ID", &org_key);

        if let Some(device_id) = device_id.as_deref() {
            set_env_default("MEMORA_ENTERPRISE_DEVICE_ID", device_id);
            set_env_default("MEMORA_DEPLOYMENT_ID", device_id);
            set_env_default("MEMORA_SUPPORT_ID", &format!("{org_key}:{device_id}"));
        }
    ...
    /// required env (`MEMORA_ENTERPRISE_LICENSE_KEY`) is missing â€” this is
    ...
    pub fn spawn(app: &tauri::AppHandle) -> Option<tokio::sync::watch::Sender<bool>> {
        ...
        // live in the MEMORA data dir (RollingFileAppender), NOT Tauri's
        // app_data_dir â€” so the from_env default would miss them.
        let mut log_dirs = Vec::new();
        if let Ok(d) = crate::log_files::get_MEMORA_data_dir(app) {
            log_dirs.push(d);
        }
        ...
        // NB: don't capture `api.api_key` here â€” at spawn-time the
        // recording server hasn't finished booting and the key is
        // usually `None`. MEMORALocalClient re-reads it from
        // RecordingState on every request via the app handle.
        let local: Arc<dyn LocalApiClient> =
            Arc::new(MEMORALocalClient::new(api_url_base, app.clone()));
        ...
        tauri::async_runtime::spawn(async move {
            ...
            // check that returned None forever â€” pasting a license in the
            // in-app prompt or signing in required a full app restart):
            //   1. env / enterprise.json (MDM, in-app prompt) â€” as before
                ...
                if let Some(token) = crate::commands::get_cloud_token() {
                        ...
                        .await
                    {
                        ...
                        Err(e) => {
                            ...
                            // unreachable â€” all normal, keep waiting quietly.
                            tracing::debug!(
            ...
            // Small startup delay so the local MEMORA server is up before
    ...
    /// priority order. The settings `deviceId` wins â€” that's the id the
    ...
    /// deliberately don't read the OS hardware UUID â€” that would let an admin
