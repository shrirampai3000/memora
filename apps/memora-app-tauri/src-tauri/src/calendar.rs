...
//! Apple Calendar integration â€” Tauri commands + background event publisher.
//!
...
pub async fn calendar_status() -> Result<CalendarStatus, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        let auth_status = MEMORACalendar::authorization_status();
...
/// "Fix Calendar Permission" â†’ macOS opened the Calendars privacy pane
/// with an EMPTY app list, so they had no way to grant access. Root cause
/// is a stale TCC record (dev-build â†’ prod-build reinstall, OS update,
...
/// consent popup again and registers the app in Privacy â†’ Calendars.
///
...
pub async fn calendar_authorize() -> Result<String, String> {
    ...
    #[cfg(target_os = "windows")]
    {
        set_calendar_publisher_enabled(true);
...
static CALENDAR_PUBLISHER_ENABLED: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(false);

pub fn is_calendar_publisher_enabled() -> bool {
    CALENDAR_PUBLISHER_ENABLED.load(std::sync::atomic::Ordering::Relaxed)
}

pub fn set_calendar_publisher_enabled(enabled: bool) {
    CALENDAR_PUBLISHER_ENABLED.store(enabled, std::sync::atomic::Ordering::Relaxed);
}

// â”€â”€â”€ Event publisher â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub async fn start_calendar_events_publisher() {
    if !is_calendar_publisher_enabled() {
        info!("calendar events publisher: disabled by default (optional integration)");
        return;
    }
    info!("calendar events publisher: started");
    loop {
        if !is_calendar_publisher_enabled() {
            info!("calendar events publisher: stopped (disabled)");
            break;
        }
        let items: Vec<CalendarEventItem> = collect_calendar_events().await;

        if let Err(e) = memora_events::send_event("calendar_events", items) {
...
async fn collect_calendar_events() -> Vec<CalendarEventItem> {
    #[cfg(target_os = "macos")]
    {
        ...
        let status = MEMORACalendar::authorization_status();
        if format!("{}", status) != "Full Access"
            && !MEMORACalendar::access_granted_this_session()
        {
            return Vec::new();
        }
...
#[cfg(target_os = "macos")]
fn calendar_event_to_item(event: memora_connect::calendar::CalendarEvent) -> CalendarEventItem {
...
fn calendar_event_to_item_win(
    event: memora_connect::calendar_windows::CalendarEvent,
) -> CalendarEventItem {
    let start_display = event.start_local.format("%-I:%M %p").to_string();
    let end_display = event.end_local.format("%-I:%M %p").to_string();
