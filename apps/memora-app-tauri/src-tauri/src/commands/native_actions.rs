...
///
/// A Rust panic crossing this Cocoaâ†’Rust trampoline aborts the whole app via
/// `panic_cannot_unwind` (extern "C" can't unwind through ObjC frames). Catch
/// any panic and log it instead â€” losing one notification click is much better
...
pub(crate) fn dispatch_notification_action(json: String) {
    ...
    // analytics. Internal to the inbox panel â€” handled before the JS emit so
    ...
    if let Some(inbox_op) = action_type.and_then(|t| t.strip_prefix("inbox_")) {
        let id = parsed
            ...
            .and_then(|v| v.get("id"))
            .and_then(|v| v.as_str());
        // Look up before mutating â€” a removed entry can't be described after.
    ...
    // "manage" â€” open the Home window to notifications settings. Handled in
    ...
    if action_type == Some("source") {
        ...
        let is_in_app = url.starts_with("MEMORA://");
        let app_clone = app.clone();
        std::thread::spawn(move || {
            if is_in_app {
    ...
    // can't conflate them:
    //   "link"      â†’ external URL, opened in the user's default browser
    //   "deeplink"  â†’ MEMORA:// in-app route, dispatched to DeeplinkHandler
    //
    ...
    // overlay wasn't running â€” which is the common case for a native
    ...
    if action_type == Some("link") || action_type == Some("deeplink") {
        ...
        // MEMORA:// URL into "link". We route on actual scheme, not on
        ...
        std::thread::spawn(move || {
            if is_in_app {
                ...
                // External URL â€” hand off to the opener plugin.
                use tauri_plugin_opener::OpenerExt;
                if let Err(e) = app_clone.opener().open_url(&url, None::<&str>) {
            ...
            == Some("record-hd")
    {
        ...
        std::thread::spawn(move || {
            ...
            if ok {
                // Confirmation toast â€” mirrors the JS handler in
...
fn is_meeting_deeplink(url: &str) -> bool {
    url.starts_with("MEMORA://meeting/") || url.starts_with("MEMORA://meeting?")
}
...
/// page reads on initial mount â€” surviving full-page navigations.
...
///
/// Same panic_cannot_unwind hazard as native_notif_action_callback â€” wrap the
    ...
    if let Some(app) = GLOBAL_APP_HANDLE.get() {
        ...
        std::thread::spawn(move || {
            ...
            if let Some(meeting_id) = native_overlay_meeting_note_id(&action) {
                ...
                let deeplink_url = format!("MEMORA://meeting/{meeting_id}");
                let meeting_page = meeting_page_with_id(&deeplink_url);
...
mod tests {
    ...
    fn rejects_anchors_that_would_strand_the_pill() {
        // An unknown or malformed anchor must not be persisted â€” the pill would
    ...
    fn parses_meeting_deeplink_path_id() {
        assert_eq!(
            parse_meeting_deeplink("MEMORA://meeting/123"),
            Some((123, true))
        );
    }
    ...
    fn parses_meeting_deeplink_query_id_and_live_flag() {
        assert_eq!(
            parse_meeting_deeplink("MEMORA://meeting?id=456&live=0"),
            Some((456, false))
        );
    }
    ...
    fn rejects_invalid_meeting_deeplink() {
        assert_eq!(
            parse_meeting_deeplink("MEMORA://meeting/not-a-number"),
            None
        );
        assert_eq!(parse_meeting_deeplink("MEMORA://settings"), None);
    }
    ...
    fn notification_source_action_uses_url_field() {
        let action = json!({
            "type": "source",
            "label": "source",
            "url": "https://github.com/shrirampai3000/memora"
        });

        assert_eq!(
            notification_source_url(&action),
            Some("https://github.com/shrirampai3000/memora".to_string())
        );
    }
    ...
    fn notification_source_action_accepts_source_and_deeplink_aliases() {
        for (field, expected) in [
            ("source_url", "MEMORA://chat/source-url"),
            ("sourceUrl", "MEMORA://chat/sourceUrl"),
            ("deeplink_url", "MEMORA://meeting/123"),
            ("deeplinkUrl", "MEMORA://meeting/456"),
        ] {
            let action = json!({
                "type": "source",
                "label": "source",
