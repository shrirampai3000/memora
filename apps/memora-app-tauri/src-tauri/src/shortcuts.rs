...
impl ShortcutConfig {
    async fn from_store(app: &AppHandle) -> Result<Self, String> {
        ...
        Ok(Self {
            show: store.show_MEMORA_shortcut,
...
async fn apply_shortcuts(app: &AppHandle, config: &ShortcutConfig) -> Result<(), String> {
    ...
    register_shortcut(app, &config.show, config.is_disabled("show"), |app| {
        track_shortcut_used(app, "show_MEMORA");
    ...
    register_shortcut(
        ...
        |app| {
            ...
            // invisible when the main window is hidden â€” i.e. exactly when a
    ...
    register_shortcut(
        ...
        |app| {
            ...
            crate::notifications::client::send(
                "recording paused",
                "capture paused â€” pipes and search still available",
            );
        },
    )
    .await?;
