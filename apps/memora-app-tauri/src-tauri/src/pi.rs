...
use memora_core::agents::pi::{
    apply_custom_provider_compat, MEMORA_cloud_models, PI_AI_PACKAGE, PI_NAMESPACE_DIR,
    PI_PACKAGE, MEMORA_API_URL,
};
...
/// Unlike `BufReader::lines()`, this never fails on invalid UTF-8 â€”
...
/// These sessions produce very short output (â‰¤50 chars) and must bypass
/// text-delta batching so titles stream visibly token-by-token.
/// Keep in sync with TypeScript: lib/utils/internal-session.ts â†’ INTERNAL_TITLE_PREFIX
...
fn build_command_for_path(path: &str) -> Command {
    if path.ends_with(".cmd") || path.ends_with(".bat") {
        ...
        // twice is harmless.
        let mut cmd = memora_core::no_window_command("cmd.exe");
        cmd.args(["/C", path]);
        cmd
        ...
        if let Some(bun) = find_bun_executable() {
            if let Some(js) = resolve_bun_global_entrypoint(path) {
                info!("Resolved bun shim {} â†’ JS entrypoint {}", path, js);
                let mut cmd = bun_command(&bun);
                cmd.arg(js);
                return cmd;
            }
            // Not a bun shim or resolution failed â€” run .exe directly. Still a
            // console program, so it gets the same treatment as the bun path
            // above rather than being the one branch that flashes.
            memora_core::no_window_command(path)
        } else {
            memora_core::no_window_command(path)
        }
...
/// On Unix, pi's shebang is `#!/usr/bin/env node` but MEMORA only bundles
...
fn check_package_bin(pkg_dir: std::path::PathBuf, bin_name: &str) -> Option<String> {
    ...
    let relative = if let Some(obj) = bin_val.as_object() {
        ...
        // Single-bin package â€” name must match package name
...
/// Pool of Pi sessions â€” each session_id gets its own PiManager/process.
pub struct PiPool {
...
pub struct PiManager {
    ...
    pending_responses: PendingResponses,
    /// Command queue handle â€” all commands go through here for serialization.
...
fn terminate_acp_process_tree(pid: u32) {
    #[cfg(unix)]
    {
        ...
        // negative pid targets only that process group, never MEMORA's.
        unsafe {
...
impl PiManager {
    ...
    /// NOTE: does NOT emit `pi_terminated` â€” the stdout reader thread is the
...
/// Get the Pi config directory â€” MEMORA's isolated agent dir
/// (`~/.memora/pi-config`), never the user's global `~/.pi/agent`.
/// Delegates to memora-core, which also runs the one-time seed
/// migration from the global dir.
/// See https://github.com/MEMORA/MEMORA/issues/4002
/// and https://github.com/MEMORA/MEMORA/issues/3812.
fn get_pi_config_dir() -> Result<PathBuf, String> {
    memora_core::agents::pi::pi_config_dir().map_err(|e| e.to_string())
}

fn remove_MEMORA_auth_from_path(auth_path: &Path) -> Result<(), String> {
    if !auth_path.exists() {
        return Ok(());
    }
    ...
    let removed = auth
        ...
        .unwrap_or(false);
    // Signed-out users keep their BYOK credentials â€” those need the tagged
    // shape too, or pi 0.83 rejects them the same way.
    let upgraded = memora_core::agents::pi::upgrade_legacy_pi_credentials(&mut auth);
...
pub(crate) fn clear_MEMORA_auth_token_files() -> Result<(), String> {
...
/// cloud JWT while signed in (#3943) â€” keep them out of reach of other local
...
/// Find pi executable
/// Returns the MEMORA-managed pi install directory (`~/.memora/pi-agent/`).
fn pi_local_install_dir() -> Option<PathBuf> {
    Some(memora_core::paths::default_MEMORA_data_dir().join("pi-agent"))
}
...
/// upstream â€” see issue #3527) so installs migrating from 2.4.258 and
...
fn seed_pi_package_json(install_dir: &std::path::Path) {
    ...
    // version ranges in place after a pi-coding-agent bump. The 0.60.0 â†’ 0.73.1
    // jump silently leaves users on the old `^0.33.1` anthropic-sdk range that
    // bun cannot reconcile with the new pi-ai's `^0.91.1` requirement â†’
            ...
            Some(mut pkg) => {
                ...
                if changed {
                    if let Ok(new_contents) = serde_json::to_string_pretty(&pkg) {
                        ...
                        // bun.lock pins the old transitive tree â€” must be
...
/// one implementation of the flag, in `memora_core::no_window`.
fn apply_no_window(cmd: &mut Command) {
    memora_core::apply_no_window(cmd);
}
...
fn bun_command(bun: &str) -> Command {
    ...
    // MEMORA-mcp prewarm spawns this directly at every launch and waits up
    // to two minutes for it, so the missing flag was a terminal sitting on the
    // user's desktop for the whole startup.
    let mut cmd = memora_core::no_window_command(bun);
    // Single source of truth in memora-core: on Linux, bun subprocesses
    ...
    memora_core::agents::pi::scrub_bun_runtime_env(&mut cmd);
    cmd
}
...
fn format_install_failure(tool: &str, output: &Output) -> String {
    ...
    // and never let both-empty reduce the message to a bare "stderr: " â€”
    ...
    format!(
        ...
        tool,
        memora_core::agents::pi::describe_exit_status(&output.status),
        details
    )
}
...
///
/// An interrupted cacheâ†’node_modules copy (app quit, AV lock, EPERM) leaves a
/// package dir without `dist/`, and a later `bun install` trusts bun.lock
/// ("no changes") without re-checking file contents â€” so the corruption is
...
fn run_pi_package_install_once(install_dir: &Path, bun: &str) -> Result<(), String> {
    ...
    info!(
        ...
        install_dir.display(),
        memora_core::agents::pi::bun_version_string(bun),
    );
    ...
    bun_cmd
        ...
        // can launch fresh console processes, so disable them for MEMORA's
...
/// Ensure the MEMORA skills exist in the project's .pi/skills directory.
/// Delegates to memora-core's canonical implementation.
fn ensure_MEMORA_skill(project_dir: &str) -> Result<(), String> {
    use memora_core::agents::pi::PiExecutor;
    PiExecutor::ensure_MEMORA_skill(std::path::Path::new(project_dir))
        .map_err(|e| format!("Failed to install MEMORA skills: {}", e))
}
...
fn ensure_enterprise_team_skill(project_dir: &str) -> Result<Option<std::path::PathBuf>, String> {
    ...
    PiExecutor::ensure_MEMORA_team_skill(&skill_root)
...
/// Web search uses the MEMORA cloud backend (Gemini + Google Search),
/// so we only enable it for MEMORA-cloud presets to avoid sending
...
fn ensure_web_search_extension(
    ...
    let is_MEMORA_cloud = match provider_config {
        Some(config) => matches!(config.provider.as_str(), "MEMORA-cloud" | "pi"),
        None => true, // default preset = MEMORA cloud
    };
    ...
    if is_MEMORA_cloud {
        ...
        let api_url = crate::config::MEMORA_ai_gateway_url()?;
...
/// `/mcp-servers/*` API. Always installed â€” does nothing when zero
...
/// populated exclusively by MEMORA, so it is trusted by definition. Keyed
...
pub struct AcpAgentConfig {
    ...
    /// Send the agent's model calls through MEMORA Cloud instead of the
    ...
    #[serde(default)]
    pub use_MEMORA_cloud: Option<bool>,
}
...
pub struct PiProviderConfig {
    ...
    /// Provider type: "openai", "native-ollama", "custom", "MEMORA-cloud"
    pub provider: String,
    ...
    /// tokens, so MEMORA converts this value using four characters/token.
    #[serde(default)]
...
async fn build_models_json(
    ...
    build_models_json_with_api_url(user_token, provider_config, MEMORA_API_URL).await
}
...
async fn build_models_json_with_api_url(
    ...
    // Always add MEMORA cloud provider. A real token is inlined as a
    // literal; the logged-out fallback must use `$` env-var syntax (pi >= 0.80
    // treats bare names as literal keys).
    let api_key_value = user_token.unwrap_or("$MEMORA_API_KEY");
    ...
    providers_map.insert("MEMORA".to_string(), MEMORA_provider);

    // Add the user's selected provider (if not MEMORA-cloud)
    if let Some(config) = provider_config {
        let provider_name = match config.provider.as_str() {
            ...
            "custom" => "custom",
            _ => "", // MEMORA-cloud already added above
        };
...
async fn ensure_pi_config(
    ...
    // Subagents are a baseline MEMORA capability, not an optional Pi
    ...
    // -- auth.json: merge MEMORA token, preserve other providers --
    ...
    if let Some(token) = user_token.filter(|token| !token.is_empty()) {
        ...
        // fallback to models.json â€” a bare token string makes the whole
        // provider resolve to "Provider is not configured: MEMORA".
        memora_core::agents::pi::upgrade_legacy_pi_credentials(&mut auth);

        if let Some(obj) = auth.as_object_mut() {
            obj.insert(
                "MEMORA".to_string(),
                memora_core::agents::pi::api_key_credential(token),
            );
        }
...
/// always times out â€” it's just a grace period to let bun finish loading before
...
pub(crate) const ACP_PRESET_WITHOUT_BACKEND: &str =
    "This coding-agent preset is missing its agent configuration. Re-select the agent in Settings â†’ AI presets.";
...
/// catch-all used to answer that with "MEMORA" â€” which sent the *agent id*
...
/// MEMORA Business": a billing dead end for what is really a broken preset,
...
fn pi_registry_provider(provider: &str, url: &str) -> Result<&'static str, String> {
    Ok(match provider {
        ...
        "native-ollama" => "ollama",
        "anthropic" => "anthropic-byok",
        // "custom" requires a valid URL; fall back to MEMORA cloud if missing
        "custom" if !url.is_empty() => "custom",
        "acp" => return Err(ACP_PRESET_WITHOUT_BACKEND.to_string()),
        "MEMORA-cloud" | "pi" | _ => "MEMORA",
    })
}

/// Resolve a model name for the MEMORA provider.
///
/// The gateway (api.memora.com) is the source of truth for model validation
...
fn resolve_MEMORA_model(requested: &str, provider: &str) -> String {
    // Only touch MEMORA provider â€” other providers use their own model names
    if provider != "MEMORA" {
...
fn resolve_pi_model(requested: &str, provider: &str) -> String {
    match provider {
        "MEMORA" => resolve_MEMORA_model(requested, provider),
...
/// subprocess holding ~150â€“300 MB RSS plus a live LLM connection, so we
...
/// 4 was too small for normal multi-tab chat use â€” opening a 5th tab would
...
/// Core Pi start logic â€” callable from both Tauri commands and Rust boot code.
pub async fn pi_start_inner(
    ...
    // Ensure MEMORA skills exist in project
    ensure_MEMORA_skill(&project_dir)?;
    ...
    // natively, in the same project dir and PI_CODING_AGENT_DIR â€” but it drops
    ...
    // ACP agents receive MEMORA via MCP during session/new. Pi-only
    // extensions and packages must not be installed or required there â€” except
    // pi-acp, which is pi and can't consume the MCP servers.
    if !use_acp {
        // Install web-search extension only for MEMORA-cloud presets
        ...
        // Same pi as native â€” seed the project-local extensions so its tools
        // reach the model. web-search uses the LOCAL-proxy variant: the cloud
        // extension needs the MEMORA-cloud JWT (which ACP sessions never
        ...
        // and its env (MEMORA_LOCAL_API_*) is already set on this path.
        ensure_live_views_extension(&project_dir)?;
        ensure_connection_gate_extension(&project_dir)?;
        ...
        // Grant trust on disk â€” the on-disk equivalent of --approve.
        ...
        // must get MEMORA's provider setup and the required pi-subagents
        // install exactly like native â€” a fresh profile would otherwise start
        // without them.
        ...
        // guarantee â€” a fresh pi-acp user gets the baseline or fails loudly,
    ...
    let (pi_provider, pi_model) = match &provider_config {
        ...
        // provider to resolve â€” these values are only used for logging below.
        ...
        None => ("MEMORA".to_string(), "auto".to_string()),
    };
    ...
    //   1. Skip sessions with in-flight RPC responses â€” those are mid-turn
    ...
    // remain exempt â€” same as before.
    if pool.sessions.len() >= MAX_PI_SESSIONS && !pool.sessions.contains_key(&sid) {
        ...
        if let Some(key) = evict_key {
            ...
            // streaming session â€” caller surfaces a "too many active chats"
            // toast, user can close one manually.
            return Err(format!(
                "pi pool full ({} active sessions, all busy) â€” close one before opening a new chat",
                MAX_PI_SESSIONS
            ));
        }
    }
                    ...
                    .ok_or_else(|| {
                        ...
                        if bun_found {
                            let install_err = take_pi_install_error()
                                ...
                                .unwrap_or_default();
                            format!("Pi not found after install attempt.{} Try restarting the app or delete ~/.memora/pi-agent and restart.", install_err)
                        } else {
                            format!("Pi not found: bun is not installed. MEMORA needs bun to run the AI assistant. Expected bundled bun next to the app executable.")
                        }
                    })?
            }
        }
    ...
    let mut cmd = if use_acp {
        ...
        let executable = std::env::current_exe()
            .map_err(|error| format!("Failed to locate MEMORA ACP runtime: {error}"))?;
        ...
        command.args([
            ...
            // rpc mode can never answer â€” without --approve, mcp-bridge and
    ...
    if use_acp {
        ...
        // silently strip MEMORA's tools for a user who has a `MEMORA`
        ...
        // Route the agent's model calls through MEMORA Cloud when the
        ...
        // else here forwards it (the ACP runtime scrubs `MEMORA_API_KEY`
        ...
        if let Some(routing) = routing {
            let gateway = crate::config::MEMORA_ai_gateway_url().unwrap_or_default();
        ...
        // itself. We never read or parse Claude Code's credential store â€” the
        ...
        cmd.env("MEMORA_ACP_ID", agent_id)
            .env("MEMORA_ACP_CWD", &project_dir)
            .env("MEMORA_BUN_PATH", &bun_path)
            .env(
                "MEMORA_ACP_ARGS_JSON",
            ...
            .env(
                "MEMORA_ACP_SESSION_CONFIG_JSON",
            ...
            .env_remove("MEMORA_ACP_COMMAND")
            .env_remove("MEMORA_ACP_AUTH_METHOD")
            .env_remove("MEMORA_ACP_SYSTEM_PROMPT");
        if let Some(command) = acp
            .command
            .as_deref()
            .filter(|value| !value.trim().is_empty())
        {
            cmd.env("MEMORA_ACP_COMMAND", command);
        }
        if let Some(auth_method) = acp
            ...
            .filter(|value| !value.trim().is_empty())
        {
            cmd.env("MEMORA_ACP_AUTH_METHOD", auth_method);
        }
        ...
        {
            cmd.env("MEMORA_ACP_RESUME_SESSION_ID", resume_id);
        }
    }
    ...
    if bun_path != "NOT FOUND" {
        if let Some(bun_dir) = std::path::Path::new(&bun_path).parent() {
            ...
            let new_path = {
                ...
                let bash_result =
                    tokio::task::spawn_blocking(memora_core::agents::pi::ensure_bash_available)
                        .await
                        .unwrap_or_else(|e| {
                ...
                match bash_result {
                    ...
                    None => {
                        warn!("bash not available â€” Pi's bash tool may fail on Windows");
                    }
                }
                path
            };
            ...
            let path_for_pi =
                memora_core::agents::pi::pi_child_path(std::ffi::OsStr::new(&new_path))
    ...
    // MEMORA-api skill file â€” they often skip reading skills on their own.
    let is_local_model = !use_acp && matches!(pi_provider.as_str(), "ollama" | "custom");
    if is_local_model {
        let api_hint = "IMPORTANT: You MUST read the MEMORA-api skill file BEFORE making any API calls. It contains authentication instructions, endpoint docs, and examples. Without reading it first, your API calls will fail with 403 unauthorized.";
        cmd.args(["--append-system-prompt", api_hint]);
    }

    // Append the user's AI preset system prompt (enables Anthropic prompt caching â€”
    ...
    // Scope pi to MEMORA's isolated agent dir (never ~/.pi/agent).
    memora_core::agents::pi::apply_pi_isolation_env(&mut |k, v| {
        cmd.env(k, v);
    });
    ...
    // Pass local API config so the Pi agent can authenticate to the runtime local API.
    {
        ...
        cmd.env("MEMORA_LOCAL_API_PORT", api.port.to_string());
        cmd.env("MEMORA_LOCAL_API_URL", api.url(""));
    ...
    // x-MEMORA-session and the navigate handler rides it to the frontend.
    ...
    cmd.env("MEMORA_SESSION_ID", &sid);

    // Chat session ID for per-session artifact isolation
    cmd.env("MEMORA_CHAT_SESSION_ID", &sid);
    ...
    // memora-core.
    if let Ok(p) = memora_core::agents::bash_env::ensure_wrapper_in_default_dir() {
        cmd.env("BASH_ENV", p);
    }

    // Pass the user's API key as env var for non-MEMORA providers
    if let Some(ref config) = provider_config {
    ...
    // TODO(remove next release): drop MEMORA_API_AUTH_KEY alias.
    ...
    // login state. Never expose the signed-in user's MEMORA cloud JWT to a
    ...
    if let Some(m) = pool.sessions.get_mut(&sid) {
        ...
        // Fresh flag for this session â€” old reader threads keep their own Arc
    ...
    // streaming message and badges in the sidebar â€” without this, the UI has
    ...
    // Drop the lock before spawning reader threads â€” this is critical to prevent
    // queued pi_start calls from stacking behind a 500ms sleep while holding the lock
    drop(pool);

    // Readiness signal â€” stdout reader notifies when first JSON line arrives,
    ...
    // Spawn stdout reader thread â€” this is the SOLE emitter of `pi_terminated`.
    ...
    std::thread::spawn(move || {
        ...
        while let Some(line) = read_lines_lossy(&mut reader) {
            ...
            //
            // The "done" type was the original intent but pi-mono never emits it â€”
            ...
            match parsed {
                Some(event) => {
                    ...
                    if matches!(
                        ...
                        // Title sessions bypass batching â€” they produce â‰¤50 chars
                        ...
                        // (`apps/memora-app-tauri/lib/events/bus.ts`).
                        // Stage 5 cleanup: legacy `pi_event` topic removed
                        // â€” every consumer now reads from `agent_event`.
        ...
        // Only emit once per session â€” overlapping sessions could race
        if terminated_guard
    ...
    // Spawn stderr reader thread â€” Pi may emit JSON events to stderr in some
    ...
    if let Some(stderr) = stderr {
        ...
        std::thread::spawn(move || {
            ...
            while let Some(line) = read_lines_lossy(&mut reader) {
                ...
                {
                    warn!(
                        "Pi extension startup failed for '{}'; the next automatic restart will load only MEMORA-managed extensions",
                        project_dir_stderr
                    );
                }
                ...
                if let Ok(event) = serde_json::from_str::<Value>(&line) {
                    ...
                    // Not JSON â€” log as warn so Pi startup errors are visible
            ...
            if let Some(ref mut child) = m.child {
                match child.try_wait() {
                    Ok(Some(status)) => {
                        ...
                        error!("Pi process exited immediately with code {} â€” check 'Pi stderr:' warnings above for details (bun path: {})", code, bun_path);
                        m.child = None;
                        m.stdin = None;
                        let install_hint = take_pi_install_error()
                            .map(|e| format!(" The Pi install previously failed: {} Restart MEMORA to retry the install automatically.", e))
                            .unwrap_or_default();
    ...
    // Handled in the frontend â€” standalone-chat.tsx suppresses startsWith errors.
    // Do NOT send a warmup prompt here â€” it burns rate limits on free models
...
/// here (desktop-side) via the SecretStore; the "MEMORA" built-in server
...
async fn resolve_user_mcp_servers_json() -> Option<String> {
    use memora_connect::mcp_servers::{McpServerStore, McpTransport};
    let data_dir = memora_core::paths::default_MEMORA_data_dir();
    ...
    let mut out = Vec::new();
    for cfg in servers {
        if !cfg.enabled || cfg.name.eq_ignore_ascii_case("MEMORA") {
            continue;
        }
...
async fn open_secret_store_for_connection_context() -> Option<memora_secrets::SecretStore> {
    let data_dir = memora_core::paths::default_MEMORA_data_dir();
    memora_secrets::SecretStore::open_for_data_dir_with_vault_key(&data_dir)
        .await
        .ok()
}
...
async fn foreground_connections_context(app: &AppHandle) -> String {
    ...
    memora_connect::connections::render_context(&data_dir, api.port, store.as_ref()).await
}
...
async fn attach_foreground_connections_context(
    ...
    // chat titles and spawned a duplicate "Current MEMORA connected
    // integrations context" chat via the background router â€” the tag makes the
    ...
    // TypeScript side.
    format!(
        "<connections_context>\nCurrent MEMORA connected integrations context, refreshed for this turn:\n{}\n</connections_context>\n\n{}",
        ctx, message
    )
}
...
pub async fn pi_set_model(
    ...
    // Map frontend provider name â†’ Pi's internal registry name. Shared with
    // `pi_start_inner` so the two can't drift â€” a mismatch means Pi can't find
...
pub async fn pi_acp_probe_agent(agent: AcpAgentConfig) -> Result<String, String> {
    ...
    cmd.arg(crate::acp_runtime::RUNTIME_ARG)
        ...
        .env_remove("MEMORA_ACP_AUTH_METHOD")
        .env_remove("MEMORA_ACP_SYSTEM_PROMPT")
        .env_remove("MEMORA_ACP_SESSION_CONFIG_JSON")
            ...
            match event.get("type").and_then(|t| t.as_str()) {
                ...
                Some("acp_external_auth_required") => {
                    ...
                    return Err(match event.get("command").and_then(|v| v.as_str()) {
                        Some(command) => format!(
                            "{name} needs a one-time sign in â€” run `{command}` in a terminal, then retry."
                        ),
...
fn validate_pi_extension_package_source(source: &str) -> Result<String, String> {
    ...
    Err("Only npm: packages and GitHub package URLs can be installed from MEMORA".to_string())
}
...
fn ensure_pi_cli_for_package_command() -> Result<(String, String), String> {
    let bun = find_bun_executable().ok_or(
        "Could not find bundled bun. Restart MEMORA or reinstall the app before installing Pi extensions.",
    )?;
    ...
    let pi_path = find_local_pi_entrypoint()
        ...
        .ok_or_else(|| {
            "Pi could not be installed. Restart MEMORA and try again.".to_string()
        })?;
...
fn run_pi_package_command_blocking(args: Vec<String>) -> Result<(), String> {
    ...
    let project_dir = memora_core::paths::default_MEMORA_data_dir().join("pi-chat");
    std::fs::create_dir_all(&project_dir)
...
pub async fn pi_remove_extension_package(
    ...
    if is_required_pi_extension_package_source(&source) {
        return Err("Subagents are required by MEMORA and cannot be disabled".to_string());
    }
    stop_idle_pi_sessions_for_package_change(&state).await?;
...
pub async fn pi_set_thinking_level(
    ...
    // Always persist â€” Pi reads this on startup, so changing before a conversation works.
...
pub async fn pi_get_thinking_level() -> Result<String, String> {
    ...
    let raw = settings
        ...
        .and_then(|l| l.as_str())
        .unwrap_or("medium");
    // Clamp to supported values â€” settings.json may have stale levels from old versions
    let level = match raw {
        "low" | "medium" | "high" => raw,
        _ => "medium",
    };
...
pub async fn pi_update_config(
    ...
    info!(
        "Pi preset changed (provider: {:?}, model: {:?}) â€” restarting chat session",
        provider_config.as_ref().map(|c| &c.provider),
        provider_config.as_ref().map(|c| &c.model),
    );

    // Resolve the chat project directory
    let project_dir = memora_core::paths::default_MEMORA_data_dir()
        .join("pi-chat")
        .to_string_lossy()
        .to_string();
...
pub(crate) fn find_bun_executable() -> Option<String> {
    ...
    // 0xC000001D at spawn. Delegate to memora-core, which waits for the
    // verified baseline build and fails closed instead of returning stock bun.
    #[cfg(windows)]
    if !memora_core::cpu_features::has_avx2() {
        return memora_core::agents::pi::find_bun_executable();
    }

    // First check next to our own executable (bundled bun in AppData/Local/MEMORA/)
...
/// Background Pi installation â€” call once from app setup.
/// Installs pi into `~/.memora/pi-agent/` (local install, not global)
...
/// Warm bun's default package cache with the pinned MEMORA-mcp so the first
...
/// `bun x MEMORA-mcp` fetch is why those tools sometimes never registered
...
fn prewarm_MEMORA_mcp(bun: &str) {
    use std::process::Stdio;
    let mut cmd = bun_command(bun);
    cmd.arg("x")
        .arg(crate::acp_runtime::MEMORA_MCP_PKG)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    match cmd.spawn() {
        Ok(mut child) => {
            ...
            info!("MEMORA-mcp prewarm complete ({})", crate::acp_runtime::MEMORA_MCP_PKG);
        }
        Err(e) => warn!("MEMORA-mcp prewarm could not spawn bun: {}", e),
    }
}

pub fn ensure_pi_installed_background() {
    // Warm the MEMORA-mcp cache in the background so the first ACP session
    ...
    if let Some(bun) = find_bun_executable() {
        let _ = std::thread::Builder::new()
            .name("MEMORA-mcp-prewarm".to_string())
            .spawn(move || prewarm_MEMORA_mcp(&bun));
    }
    ...
    #[cfg(windows)]
    {
        let _ = std::thread::Builder::new()
            ...
            .spawn(|| {
                memora_core::agents::pi::ensure_bash_available();
            });
    }
    ...
    if find_local_pi_entrypoint().is_some() {
        if let Some(install_dir) = pi_local_install_dir() {
            ...
            if needs_lru_fix || needs_anthropic_sdk || needs_upgrade {
                if needs_lru_fix {
                    info!("Pi installed but missing lru-cache overrides â€” patching");
                }
                if needs_anthropic_sdk {
                    info!("Pi installed but missing @anthropic-ai/sdk dependency â€” patching");
                }
                if needs_upgrade {
                    info!(
                        "Pi version mismatch â€” upgrading to {} in background",
                        PI_PACKAGE
                    );
                }
                seed_pi_package_json(&install_dir);
                // Drop bun.lock whenever ANY patch fires â€” a stale lockfile
                ...
                // until it completes â€” otherwise pi_start will launch the stale version
    ...
    match std::thread::Builder::new()
        ...
        .spawn(move || {
            let result = std::panic::catch_unwind(|| {
                ...
                info!(
                    "Pi not found â€” installing into {} via bundled package manager",
                    install_dir.display()
                );
...
mod tests {
    ...
    fn identical_pi_launches_share_a_fingerprint() {
        let config = super::PiProviderConfig {
            backend: None,
            acp_agent: None,
            provider: "MEMORA-cloud".to_string(),
            url: String::new(),
...
"#,
    ...
    fn clear_MEMORA_auth_preserves_other_provider_tokens() {
        ...
        super::remove_MEMORA_auth_from_path(&auth_path).expect("clear MEMORA auth");
    ...
    /// bare token string, which pi 0.83 rejects outright â€” chat then fails with
    /// "Provider is not configured: MEMORA" even though core was fixed.
    #[test]
    fn app_writer_emits_tagged_MEMORA_credential() {
        let mut auth = json!({"MEMORA": "bare-jwt", "anthropic": {"apiKey": "sk-ant"}});

        memora_core::agents::pi::upgrade_legacy_pi_credentials(&mut auth);
        auth.as_object_mut().unwrap().insert(
            "MEMORA".to_string(),
            memora_core::agents::pi::api_key_credential("fresh-jwt"),
        );
    ...
    fn clear_MEMORA_auth_missing_file_is_noop() {
        ...
        super::remove_MEMORA_auth_from_path(&auth_path).expect("missing auth is ok");
...
error: InstallFailed extracting tarball"#;
    ...
    fn self_heals_a_failed_install_that_left_a_broken_dependency_tree() {
        ...
        // unusable â€” same corruption, so it must take the same self-heal path.
        assert!(super::install_failure_is_self_healable(
            install_dir,
    ...
    fn test_pi_rpc_prompt_response() {
        let mut child = match spawn_pi_rpc("MEMORA", "claude-haiku-4-5") {
            Some(c) => c,
            None => {
    ...
    fn test_pi_rpc_restart_race() {
        let mut child1 = match spawn_pi_rpc("MEMORA", "claude-haiku-4-5") {
            Some(c) => c,
            None => {
                eprintln!("SKIP: pi executable not found");
                return;
        ...
        let mut child2 = match spawn_pi_rpc("MEMORA", "claude-haiku-4-5") {
            Some(c) => c,
            None => {
    ...
    /// Mixed valid and invalid UTF-8 lines â€” valid lines unaffected
    #[test]
    fn test_read_lines_lossy_mixed_valid_invalid() {
        ...
        // Third line: valid JSON â€” reader recovered
    ...
    fn test_resolve_pi_model_maps_unsupported_chatgpt_codex_model() {
        ...
        assert_eq!(
            resolve_pi_model("gpt-5.5-codex", "MEMORA"),
            "gpt-5.5-codex"
        );
    }

    #[tokio::test]
    async fn test_build_models_json_default_has_MEMORA_provider() {
        let config = build_models_json(None, None).await;
        let providers = config["providers"].as_object().unwrap();
        assert!(providers.contains_key("MEMORA"));
        assert_eq!(providers.len(), 1);

        let sp = &providers["MEMORA"];
        assert_eq!(sp["baseUrl"], "https://api.memora.com/v1");
        assert_eq!(sp["api"], "openai-completions");
        // `$` prefix is required: pi >= 0.80 treats bare names as literal keys
        assert_eq!(sp["apiKey"], "$MEMORA_API_KEY");
    ...
    async fn test_build_models_json_uses_resolved_gateway_url() {
        ...
        assert_eq!(
            config["providers"]["MEMORA"]["baseUrl"],
    ...
    /// as a model name, and the 403 surfaced as "upgrade to MEMORA
    ...
    fn known_providers_still_map_as_before() {
        ...
        assert_eq!(pi_registry_provider("custom", "").unwrap(), "MEMORA");
        assert_eq!(
            pi_registry_provider("MEMORA-cloud", "").unwrap(),
            "MEMORA"
        );
    }
    ...
    async fn test_build_models_json_with_user_token() {
        ...
        let sp = &config["providers"]["MEMORA"];
        assert_eq!(sp["apiKey"], "tok_abc123");
    }

    #[tokio::test]
    async fn test_build_models_json_MEMORA_cloud_no_extra_provider() {
        let pc = make_provider_config("MEMORA-cloud", "auto");
        let config = build_models_json(None, Some(&pc)).await;
        let providers = config["providers"].as_object().unwrap();
        // MEMORA-cloud maps to "" (empty), so only the MEMORA provider is added
        assert_eq!(providers.len(), 1);
        assert!(providers.contains_key("MEMORA"));
    }
    ...
    fn test_extension_safe_mode_loads_only_managed_project_extensions() {
        ...
        let header = "// MEMORA â€” AI that knows everything you've seen, said, or heard\n";
    ...
    fn test_tool_allowlist_restricts_bounded_pi_sessions() {
        let mut config = make_provider_config("MEMORA-cloud", "auto");
        config.allowed_tools = Some(vec!["MEMORA_live_view".to_string()]);
        let mut command = Command::new("pi");
        ...
        assert_eq!(args, vec!["--tools", "MEMORA_live_view"]);
    }
    ...
    async fn test_build_models_json_custom_with_empty_url_skipped() {
        ...
        let config = build_models_json(None, Some(&pc)).await;
        let providers = config["providers"].as_object().unwrap();
        assert_eq!(providers.len(), 1); // only MEMORA
    ...
    async fn test_build_models_json_custom_with_url() {
        ...
        assert_eq!(providers["custom"]["headers"]["User-Agent"], "MEMORA");
    }
    ...
    async fn test_build_models_json_repairs_ai_genesis_custom_url() {
        for base_url in ["https://ai.ai-genesis.app", "https://api.ai-genesis.app/"] {
            ...
            assert_eq!(custom["headers"]["User-Agent"], "MEMORA");
        }
    }
    ...
    async fn test_build_models_json_custom_generic_no_compat_override() {
        ...
        // should NOT have compat.maxTokensField set â€” Pi's auto-detection
    ...
    async fn test_build_models_json_regular_gpt4_no_compat_override() {
        // gpt-4 and gpt-4o should NOT be forced â€” they work with both field names
    ...
    async fn test_build_models_json_no_stale_providers() {
        ...
        // no file to corrupt â€” the test verifies the output shape is always valid.
        let config = build_models_json(Some("tok"), None).await;
        let providers = config["providers"].as_object().unwrap();

        // Only "MEMORA" â€” no leftover providers
        assert_eq!(providers.len(), 1);

        // Every model has required fields for pi-coding-agent schema
        let models = providers["MEMORA"]["models"].as_array().unwrap();
        for m in models {
    ...
    /// baseline seeds MEMORA skills, the Live Views extension, and registers
    /// the required pi-subagents package â€” the exact pieces the pi-acp path now
    /// calls ensure_MEMORA_skill / ensure_live_views_extension /
    ...
    #[test]
    fn fresh_profile_seeds_MEMORA_baseline() {
        let project = tempfile::tempdir().expect("project dir");
        let project_dir = project.path().to_str().expect("utf8 path");

        // MEMORA skills materialize under the fresh project's .pi/skills.
        super::ensure_MEMORA_skill(project_dir).expect("seed skills");
        ...
        assert!(
            std::fs::read_dir(&skills_dir)
                ...
                .any(|e| e.file_name().to_string_lossy().starts_with("MEMORA")),
            "at least one MEMORA skill must be seeded on a fresh profile"
        );
        ...
        // The required pi-subagents package is registered from empty settings â€”
        // this is what ensure_pi_config â†’ ensure_required_pi_extension_setting
