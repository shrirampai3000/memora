...
//! it finishes processing it. This means `new_session` â†’ `prompt` races even with
...
pub struct PiQueueState {
    ...
    /// the brief `agent_end` â†’ `agent_start` transition.
...
async fn wait_for_prompt_acceptance(
    ...
    loop {
        ...
        // `prompt_pending` cleared but the turn never started â†’ an abort/cancel
