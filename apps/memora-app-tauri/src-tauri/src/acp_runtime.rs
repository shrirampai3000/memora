...
//! hidden mode of the signed MEMORA executable so no second sidecar or
...
pub const RUNTIME_ARG: &str = "--MEMORA-acp-runtime";
const PROCESS_GUARD_ARG: &str = "--MEMORA-acp-process-guard";
pub(crate) const CLOUD_API_KEY_ENV: &str = "MEMORA_API_KEY";

/// The core MEMORA MCP server (activity-summary, search-content,
...
/// SQL. `pi::prewarm_MEMORA_mcp` seeds the cache at install. Bump alongside
/// packages/MEMORA-mcp.
pub(crate) const MEMORA_MCP_PKG: &str = "MEMORA-mcp@0.19.1";

/// Environment carried into the runtime process by `pi.rs` that belongs to the
/// runtime alone and must never be inherited by any child it spawns â€” neither
/// the agent adapter nor a client-requested terminal. These blobs hold resolved
/// provider API keys (`MEMORA_ACP_ENV_JSON`), third-party MCP secret headers
/// (`MEMORA_ACP_USER_MCP_JSON`), the system prompt, and session config.
/// Children receive what they legitimately need through CLI args, the resolved
/// per-process `env`, and the structured ACP protocol â€” never as raw inherited
/// env. A single list keeps the two spawn sites from drifting (a terminal that
/// forgot one of these was a secret-exfiltration path â€” an agent could `env` it
/// out). `CLOUD_API_KEY_ENV` is the signed-in user's cloud JWT.
const RUNTIME_ONLY_ENV: &[&str] = &[
    CLOUD_API_KEY_ENV,
    "MEMORA_ACP_ENV_JSON",
    "MEMORA_ACP_USER_MCP_JSON",
    "MEMORA_ACP_SESSION_CONFIG_JSON",
    "MEMORA_ACP_SYSTEM_PROMPT",
    "MEMORA_ACP_AUTH_METHOD",
    "MEMORA_ACP_COMMAND",
    "MEMORA_ACP_ARGS_JSON",
    "MEMORA_ACP_CWD",
    "MEMORA_ACP_ID",
    "MEMORA_ACP_RESUME_SESSION_ID",
];
...
#[cfg(windows)]
const WRAPPED_COMMAND_ENV: &str = "MEMORA_INTERNAL_ACP_WRAPPED_COMMAND";
#[cfg(windows)]
const WRAPPED_ARGS_ENV: &str = "MEMORA_INTERNAL_ACP_WRAPPED_ARGS_JSON";
...
struct RuntimeConfig {
    ...
    /// session/new alongside the MEMORA server.
...
impl RuntimeConfig {
    fn from_env() -> Result<Self, String> {
        let agent_id = env_nonempty("MEMORA_ACP_ID").unwrap_or_else(|| "custom".into());
        let bun_path =
            env_nonempty("MEMORA_BUN_PATH").ok_or("MEMORA_BUN_PATH is required for ACP")?;
        let configured_args =
            parse_json_env::<Vec<String>>("MEMORA_ACP_ARGS_JSON")?.unwrap_or_default();
        let mut env = parse_json_env::<HashMap<String, String>>("MEMORA_ACP_ENV_JSON")?
            .unwrap_or_default();
        env.retain(|name, _| !is_process_guard_env(name) && !is_forbidden_acp_env(name));
        let configured_command = env_nonempty("MEMORA_ACP_COMMAND");
        ...
        let project_dir = env_nonempty("MEMORA_ACP_CWD")
            .map(PathBuf::from)
        ...
        Ok(Self {
            ...
            project_dir,
            bun_path,
            preferred_auth_method: env_nonempty("MEMORA_ACP_AUTH_METHOD"),
            ...
            .unwrap_or_default(),
            resume_session_id: env_nonempty("MEMORA_ACP_RESUME_SESSION_ID"),
        })
    }
}

/// A short, harness-agnostic note about the MEMORA tools every ACP session
...
/// the bundled server registers (see assets/mcp/MEMORA-tools.mjs) plus the
/// core MEMORA search server, and points at the seeded `.pi/skills` guides
/// that third-party agents otherwise only find by chance.
const MEMORA_TOOLS_HINT: &str = "\
You are running inside MEMORA. Prefer its MCP tools over shell/curl (this is your usage guide). Tool names below are written with hyphens; some agents expose the same tools with underscores (activity_summary, search_content) or a query_recordings tool for read-only SQL â€” use whatever your own tool list shows, and never fall back to curl or /raw_sql just because a name here doesn't match exactly:
- the `MEMORA` server searches and summarizes the user's screen, audio, and UI history.
  - `activity-summary` for broad questions (\"what was I doing?\", \"which apps?\", \"how long on X?\"): it pre-summarizes apps, windows, and transcripts and owns the time math â€” pass natural-language times (\"today\", \"2h ago\") and never sum minutes yourself.
...
- `list_connections` shows the user's connected apps; `memora_connect_app` connects one and waits for the user when a task needs it.
- for a connection returned with mcp=true (Linear, Notion, Stripe, Sentry, Jira, Gmail, Zoom, Drive), use `sp_mcp_list_tools` then `sp_mcp_call` (with its `mcp_server_id`) to actually use it â€” not the connection proxy.
...
- `live_view` reads or edits the user's saved Live Views (dashboards): action=list to find one, action=get for its definition, action=save to persist edits â€” only when the user asks about a dashboard.
- MEMORA seeds task guides in `.pi/skills/*/SKILL.md` under your working directory. Before starting a task, list that folder and read the SKILL.md that matches the request; it names the exact tools and steps to use.
...
fn build_first_turn_context(user_prompt: Option<String>) -> String {
    match user_prompt {
        Some(prompt) => format!("{MEMORA_TOOLS_HINT}\n\n{prompt}"),
        None => MEMORA_TOOLS_HINT.to_string(),
    }
}
...
pub(crate) fn is_forbidden_acp_env(name: &str) -> bool {
    // This is the signed-in user's MEMORA cloud JWT, not the local API
...
/// How an agent can be pointed at MEMORA Cloud instead of the user's own
...
struct CatalogAgent {
    ...
    /// Serve MEMORA's MCP tools over loopback http instead of client stdio,
    /// for agents that reject stdio MCP servers (Cursor, GitHub Copilot). JSON
    /// key is `httpMcp`.
    #[serde(default)]
    http_mcp: bool,
    /// Present only for agents that can be pointed at MEMORA Cloud.
...
/// unintended â€” the caller then leaves the agent on its own account.
...
/// The env that routes an agent through MEMORA Cloud, plus the names to
...
async fn run_terminal_login(
    ...
    // the user's attention in the app â€” a terminal blinking beside it for up to
    // five minutes reads as a crash.
    let mut child = memora_core::no_window_command_async(program)
        .args(args)
...
/// Used to show an "Installing <agent>â€¦" hint instead of a bare spinner. ACP
...
/// installs *after* the app started may not be found â€” until they restart. To
...
/// Whether `id` names an agent MEMORA can launch (any agent in the static
...
fn builtin_agent(id: &str, bun: &str) -> Option<(String, Vec<String>)> {
    // Every agent â€” pins and launch â€” comes from our static catalog
...
fn windows_target_command(program: &str, args: &[String]) -> Result<Command, String> {
    ...
    // it holds no console â€” which is precisely why Windows would allocate a
    // fresh one for a console-subsystem agent here. The flag does not disturb
    // the inherited stdio the guard relays.
    let mut command = memora_core::no_window_command(resolved);
    command.args(args);
    Ok(command)
}
    ...
    fn handle_update(&self, update: Value) {
        ...
        match kind {
            ...
            "plan" => {
                ...
                // stacked one collapsed "Plan" blob per revision â€” a five-step
            ...
            "tool_call_update" => {
                ...
                // tool_call_update, not a tool_call â€” Claude Code stamps
                ...
                // Copilot, Codex, ...) keeps the original pass-through â€” a
                // first-seen update with no start flows to the normal
                // finish/progress paths â€” so we never fabricate a nameless row
...
fn tool_name(update: &Value) -> String {
    // The title is the human-readable name ("Grep", "mcp__MEMORA__search");
    ...
    update
        ...
        // starting tool_call we never saw) still carries the real name here â€”
...
/// becomes `pending` â€” the ACP default, and the safest thing to show for a step
...
fn spawn_terminal(state: &RuntimeState, request: CreateTerminalRequest) -> Result<String, String> {
    ...
    // spawn â€” a prompt-injected agent could otherwise `env` them out.
    ...
    std::thread::spawn(move || {
        ...
        // so the readers would not EOF for the grandchild's whole lifetime â€”
...
fn ensure_tools_mcp_server(config: &RuntimeConfig) -> Option<PathBuf> {
    const SOURCE: &str = include_str!("../assets/mcp/MEMORA-tools.mjs");
    let dir = config.project_dir.join(".memora");
    if let Err(error) = std::fs::create_dir_all(&dir) {
        eprintln!("[acp-runtime] could not create MCP tools dir: {error}");
        return None;
    }
    let path = dir.join("MEMORA-tools.mjs");
...
/// MEMORA's tools over loopback Streamable-HTTP instead (see
...
fn agent_needs_http_mcp(agent_id: &str) -> bool {
    ...
    // Copilot set it today; without it they get no MEMORA tools at all.
    agent_catalog()
        .into_iter()
        .find(|agent| agent.id == agent_id)
...
/// The local MEMORA engine base url, from the runtime's env.
fn engine_api_url() -> Option<String> {
    env_nonempty("MEMORA_LOCAL_API_URL").or_else(|| {
        env_nonempty("MEMORA_LOCAL_API_PORT").map(|port| format!("http://localhost:{port}"))
    })
}
...
fn mcp_servers(config: &RuntimeConfig) -> Vec<McpServer> {
    ...
    if let Some(http) = HTTP_MCP_URLS.get().filter(|urls| !urls.is_empty()) {
        ...
        // up loopback http servers for MEMORA's tools (see run_from_env),
        // so advertise those instead. No secrets on the wire â€” the servers bind
        ...
        let mut args = vec!["x".into(), MEMORA_MCP_PKG.into()];
        let mut env = Vec::new();
        if let Some(url) = engine_api_url() {
            args.extend(["--MEMORA-url".into(), url.clone()]);
            env.push(EnvVariable::new("MEMORA_API_URL", url));
        }
        if let Some(key) = env_nonempty("MEMORA_LOCAL_API_KEY") {
            env.push(EnvVariable::new("MEMORA_LOCAL_API_KEY", key));
        }
        servers.push(McpServer::Stdio(
            McpServerStdio::new("MEMORA", &config.bun_path)
                .args(args)
                .env(env),
        ));
        ...
        if let Some(tools_server) = ensure_tools_mcp_server(config) {
            ...
            if let Some(url) = engine_api_url() {
                tools_env.push(EnvVariable::new("MEMORA_API_URL", url));
            }
            if let Some(key) = env_nonempty("MEMORA_LOCAL_API_KEY") {
                tools_env.push(EnvVariable::new("MEMORA_LOCAL_API_KEY", key));
            }
            if let Some(chat_id) = env_nonempty("MEMORA_CHAT_SESSION_ID") {
                tools_env.push(EnvVariable::new("MEMORA_CHAT_SESSION_ID", chat_id));
            }
            servers.push(McpServer::Stdio(
                McpServerStdio::new("MEMORA-tools", &config.bun_path)
...
fn spawn_http_mcp_servers(config: &RuntimeConfig) -> Vec<std::process::Child> {
    ...
    let api_key = env_nonempty("MEMORA_LOCAL_API_KEY");
    ...
    if let (Some(tools_path), Some(port)) = (ensure_tools_mcp_server(config), free_loopback_port())
    {
        ...
        cmd.arg(&tools_path)
            .env("MEMORA_TOOLS_HTTP_PORT", port.to_string())
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::inherit());
        if let Some(url) = &engine_url {
            cmd.env("MEMORA_API_URL", url);
        }
        if let Some(key) = &api_key {
            cmd.env("MEMORA_LOCAL_API_KEY", key);
        }
        if let Some(chat_id) = env_nonempty("MEMORA_CHAT_SESSION_ID") {
            cmd.env("MEMORA_CHAT_SESSION_ID", chat_id);
        }
        match cmd.spawn() {
            Ok(child) => {
                children.push(child);
                urls.push(("MEMORA-tools".into(), format!("http://127.0.0.1:{port}/mcp")));
    ...
    // Core search over http. `MEMORA-mcp --http` dispatches to the package's
    // Streamable-HTTP server (there is no separate MEMORA-mcp-http package,
    ...
    if let Some(port) = free_loopback_port() {
        ...
        if let Some(engine_port) =
            env_nonempty("MEMORA_LOCAL_API_PORT").or_else(|| env_nonempty("MEMORA_PORT"))
        {
            cmd.arg("--MEMORA-port").arg(engine_port);
        }
        if let Some(url) = &engine_url {
            cmd.env("MEMORA_API_URL", url);
        }
        ...
        match cmd.spawn() {
            Ok(child) => {
                children.push(child);
                urls.push(("MEMORA".into(), format!("http://127.0.0.1:{port}/mcp")));
            }
...
/// MEMORA's own local, read-only screen-data tools (the `MEMORA` MCP
...
/// `MEMORA-tools` server (mcp__MEMORA-tools__*), which includes writes,
/// app-connect, and the user-MCP bridge and must keep prompting.
fn is_MEMORA_read_tool(tool_title: &str) -> bool {
    // The MEMORA-mcp read server (search-content, activity-summary, ...) is
    // entirely read-only.
    if tool_title.starts_with("mcp__MEMORA__") {
        return true;
    }
    // Specific read-only tools from the bundled MEMORA-tools server.
    ...
    // http-only agents (Cursor, Copilot) â€” all plain GETs of the user's own
    // recordings, so auto-approved exactly like their mcp__MEMORA__*
    // equivalents on stdio. The write/bridge tools (save_artifact, sp_mcp_call,
    // memora_connect_app, live_view, sp_web_search) stay NOT auto-approved.
    matches!(
        tool_title,
        "mcp__MEMORA-tools__query_recordings"
            | "mcp__MEMORA-tools__list_connections"
            | "mcp__MEMORA-tools__activity_summary"
            | "mcp__MEMORA-tools__keyword_search"
            | "mcp__MEMORA-tools__search_elements"
            | "mcp__MEMORA-tools__frame_context"
            | "mcp__MEMORA-tools__get_frame_elements"
            | "mcp__MEMORA-tools__list_meetings"
            | "mcp__MEMORA-tools__get_meeting"
            | "mcp__MEMORA-tools__health_check"
    )
}
...
fn external_auth_command(agent_id: &str) -> Option<&'static str> {
    ...
    // re-errors "Authentication required" â€” login can't complete over ACP. It
...
/// Consumer Claude subscription sign-in, which MEMORA must never offer.
///
...
async fn authenticate(
    ...
    if let Some(command) = external_auth_command(&config.agent_id) {
        ...
        // default provider) â€” see the acp_external_auth_required handler.
    ...
    // Codex, Cursor Login for Cursor, and â€” because we declare the terminal-auth
    // capability â€” Claude Subscription / Anthropic Console for Claude.
...
async fn run_protocol(
    ...
    Client
        ...
        .connect_with(transport, |connection: ConnectionTo<Agent>| async move {
            ...
            // The agent is up (any first-run download finished) â€” clear the
            ...
            loop {
                tokio::select! {
                    command = commands.recv() => {
                        ...
                        match command_type {
                            ...
                            "new_session" => {
                                ...
                                // run_protocol â€” that tears down the whole runtime
                    ...
                    completed = completed_rx.recv(), if active => {
                        ...
                        // re-send it â€” a clean session reads the new credential.
                        let _ = &last_prompt;
                        match result {
...
pub async fn run_from_env() -> Result<(), String> {
    ...
    // Agents that ignore client stdio MCP servers (Cursor) get MEMORA's
    // tools over loopback http instead â€” stand those servers up before the
    ...
    // so the UI can show "Installing <agent>â€¦" instead of a bare spinner.
    ...
    let _ = child.wait().await;
    // Reap the loopback http MCP servers (if any) â€” they die with the runtime's
...
mod tests {
    ...
    fn install_status_only_gates_binary_agents() {
        // npx agents run via the bundled bun â€” never gated on a user install.
    ...
    fn cloud_token_is_forbidden_from_acp_processes() {
        assert!(is_forbidden_acp_env("MEMORA_API_KEY"));
        assert!(is_forbidden_acp_env("MEMORA_api_key"));
        assert!(!is_forbidden_acp_env("MEMORA_LOCAL_API_KEY"));
    ...
    fn runtime_only_env_scrubs_every_secret_blob() {
        ...
        // secret-bearing vars â€” a missing entry was an exfiltration path (a
    ...
    /// Routing an agent at MEMORA Cloud is what lets someone use a coding
    ...
    #[test]
    fn bundled_tools_mcp_server_is_registered_alongside_MEMORA() {
        ...
        let names: Vec<String> = servers
            ...
            .collect();
        assert!(names.iter().any(|n| n == "MEMORA"));
        assert!(
            names.iter().any(|n| n == "MEMORA-tools"),
    ...
    fn first_turn_context_always_includes_the_tools_hint() {
        ...
        assert!(none.contains("memora_connect_app"));
    ...
    #[test]
    fn user_mcp_servers_are_forwarded_alongside_MEMORA() {
        let mut config = runtime_config("claude-acp");
        ...
        let servers = mcp_servers(&config);
        // The built-in MEMORA server is always present; match user
        ...
        assert!(servers
            .iter()
            .any(|s| matches!(s, McpServer::Stdio(stdio) if stdio.name == "MEMORA")));
    ...
    fn turn_end_leaves_background_tools_running_not_failed() {
        ...
        // subagent the agent left running â€” not a failure. Only cancel/error
        // render red.
    ...
    fn non_subagent_first_seen_update_does_not_synthesize_a_row() {
        ...
        // get) must NOT fabricate a nameless start â€” that path is subagent-only,
    ...
    fn subagent_text_nests_under_its_task_not_the_main_message() {
        let output = ParentOutput::buffer();
        let state = test_state(&output);
        // Subagent text carries parentToolUseId â†’ attach to the Task row.
        state.handle_update(json!({
    ...
    fn plan_entries_are_forwarded_structurally_not_flattened_to_text() {
        // Regression: the plan used to be joined into a "âœ“/â†’/â—‹" string and
    ...
    fn tool_args_falls_back_past_missing_raw_input() {
        ...
        // Nothing input-bearing â†’ empty object, never a non-object value.
    ...
    #[test]
    fn MEMORA_read_tools_auto_approve_prefer_always() {
        // Only the read-only screen-data server auto-approves; writes/bridge
        // (MEMORA-tools) and everything else still prompt.
        assert!(is_MEMORA_read_tool("mcp__MEMORA__search-content"));
        assert!(is_MEMORA_read_tool("mcp__MEMORA-tools__query_recordings"));
        assert!(is_MEMORA_read_tool("mcp__MEMORA-tools__list_connections"));
        // Core read tools mirrored on MEMORA-tools for http-only agents.
        assert!(is_MEMORA_read_tool("mcp__MEMORA-tools__activity_summary"));
        assert!(is_MEMORA_read_tool("mcp__MEMORA-tools__keyword_search"));
        assert!(is_MEMORA_read_tool("mcp__MEMORA-tools__get_meeting"));
        assert!(is_MEMORA_read_tool("mcp__MEMORA-tools__health_check"));
        assert!(!is_MEMORA_read_tool("mcp__MEMORA-tools__sp_mcp_call"));
        assert!(!is_MEMORA_read_tool("mcp__MEMORA-tools__save_artifact"));
        assert!(!is_MEMORA_read_tool("bash"));
