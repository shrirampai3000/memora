...
pub fn install_user_data_dir() {
    ...
    eprintln!(
        "MEMORA: WebView2 user data dir source={} writable={} path={}{}",
        diagnostics.source,
...
fn ensure_writable_dir(path: &Path) -> Result<(), String> {
    ...
    let test_path = path.join(format!(".memora-write-test-{}", std::process::id()));
