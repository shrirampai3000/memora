...
pub async fn install_specific_version(app: &tauri::AppHandle, version: &str) -> Result<(), String> {
    ...
    let rollback_url = format!(
        "https://github.com/shrirampai3000/memora/api/app-update/rollback/{}/{}",
        target_arch, version
    );
...
fn enterprise_updates_managed_locally(app: &tauri::AppHandle) -> bool {
    ...
    match enterprise_update_mode(app).as_deref() {
        ...
        // Missing/unknown policy â†’ behave like a new org with the consumer
...
// Restart gate (#3622)
//
// Every code path that culminates in `process::exit` â€” the auto-update
// restart, banner-triggered relaunch, rollback restart â€” must wait for
...
pub enum RestartGate {
    /// Boot reached the "ready" phase â€” safe to call `process::exit` /
    /// `app.restart()` / `download_and_install` on Windows.
    Proceed,
    /// Boot reached the "error" phase. The failed boot is *finished* â€” no
...
impl RestartGate {
    ...
    /// making progress (`DeferPending`) â€” exiting mid-`AudioManager::new`
...
/// is actively waiting after a click â€” better to surface "still starting,
...
/// issue) re-downloads the same version every cycle forever â€” one stuck
...
/// (which passes `force=true`). In-memory only â€” a restart re-attempts once.
...
pub async fn await_restart_gate(timeout: Duration, label: &str) -> RestartGate {
    ...
    match outcome {
        ...
        // so the #3622 race this gate exists for cannot happen â€” and blocking
        ...
        crate::health::BootReadiness::Idle => {
            info!(
                "{}: engine idle (never started) â€” nothing is initializing, proceeding \
                 with restart",
                label
            );
            RestartGate::Proceed
        ...
        crate::health::BootReadiness::Errored => {
            warn!(
                "{}: boot phase is 'error' â€” restarting anyway; a relaunch is the \
        ...
        crate::health::BootReadiness::Pending => {
            warn!(
                "{}: boot phase still pending after {}s â€” deferring restart to avoid \
...
pub async fn restart_for_update(
    ...
    // stall the relaunch (2026-06-26 MacBook Air: VisionManager hung 10s â†’
    ...
    match bounded_teardown(
        ...
        stop_MEMORA(app.state::<RecordingState>(), app.clone()),
    )
    .await
    {
        ...
        TeardownOutcome::Failed(err) => {
            warn!(
                "banner restart: stop_MEMORA failed (continuing): {}",
                err
            )
        }
        TeardownOutcome::TimedOut => warn!(
            "banner restart: teardown exceeded {}s (capture shutdown wedged) â€” relaunching anyway",
...
/// (e2e driver / tray state) â€” the authoritative guard is the swap above.
...
// last log reader is gone â€” if it fails, nothing today would ever say so: the
...
enum UpdateAttemptOutcome {
    /// Running version moved off `from_version` â€” the install applied.
    Applied,
    /// Still running `from_version` â€” the install did not apply.
    Failed,
...
fn consume_update_attempt_marker(app: &tauri::AppHandle) -> Option<UpdateAttempt> {
    ...
    match classify_update_attempt(&attempt, &current) {
        UpdateAttemptOutcome::Applied => {
            info!(
                "previous update install applied: {} â†’ {}",
                attempt.from_version, attempt.to_version
            );
            None
        }
        ...
        UpdateAttemptOutcome::Unrelated => {
            info!(
                "update-attempt marker ({} â†’ {}) doesn't match running version {} â€” ignoring",
...
/// `Ok("pending")`, so a click while the gate deferred did nothing visible â€”
/// no dialog, no notification, label unchanged â€” and users clicked again
    ...
    if manager.has_update_installed().await {
        ...
        match restart_for_update(app.clone(), None).await {
            Ok(outcome) if outcome == "proceed" => {
                // Process exits shortly; "Installing updateâ€¦" stays until the
            ...
            Ok(outcome) => {
                ...
                notify_update_state(
                    &app,
                    "MEMORA is still starting up",
                    "the update will install once startup finishes â€” try again in a moment.",
                );
            }
            Err(e) => {
                ...
                notify_update_state(
                    ...
                    "MEMORA couldn't save settings before restarting. try again, or quit and reopen the app.",
                );
            }
        }
...
/// suppressed re-polls and pinned the app to the first version it saw â€” #5784).
/// These outcomes keep each re-poll cheap and correct:
enum PromoteDecision {
    /// No pending update, or `found` is strictly newer than the pending one â€”
    /// adopt `found`.
    Adopt,
    /// The pending version is already as new or newer than `found` â€” nothing to
    /// do this tick.
    Noop,
    /// `found` equals the version that previously failed to install â€” route it
...
/// installed â€” silently, forever, with no recourse. Force auto-update ON when the
/// app UI is hidden: the employee-override only makes sense when there's a UI to
/// honor it.
///
/// EXCEPTION â€” never force when updates are managed externally (`mode = mdm |
...
pub struct UpdatesManager {
    ...
    /// the same broken version every 5 min â€” see `UPDATE_FAILURE_COOLDOWN`.
...
fn sweep_sp_old_files(dir: &std::path::Path) -> usize {
    ...
    for entry in entries.flatten() {
        ...
        // `bun.exe.sp-old-<tick>` â€” hooks.nsh seeds the suffix with the tick
...
impl UpdatesManager {
    pub fn new(app: &tauri::AppHandle, interval_minutes: u64) -> Result<Self, Error> {
        ...
        // in-memory Update handle died with that process) â€” drop it.
        #[cfg(target_os = "macos")]
        ...
        let update_menu_item = if is_enterprise_build(app) {
            ...
            let (menu_text, enabled) = if is_source_build(app) {
                ...
                ("Update didn't apply â€” click to retry", true)
            } else {
                ("MEMORA is up to date", false)
            };
            Some(
    ...
    pub async fn check_for_updates(
        ...
        // explicitly allows the MEMORA updater for this install context.
        ...
        match &check_result {
            ...
            Ok(None) => {
                info!(
                    "updater returned None â€” no update available (current v{}, endpoints: {:?})",
                    current_version,
                    self.app
                        .config()
                        .plugins
            ...
            Err(ref e) => {
                // warn, not error â€” updater failures are mostly transient network
        ...
        if let Ok(Some(update)) = check_result {
            ...
            // ships â€” but we skip the expensive binary fetch (and the
                ...
                if in_cooldown {
                    info!(
                        "update v{} recently failed to install; skipping auto-download \
                         (cooldown {}h) â€” click 'check for updates' to retry",
                        update.version,
                        UPDATE_FAILURE_COOLDOWN.as_secs() / 3600
                    );
                    if let Some(ref item) = self.update_menu_item {
                        item.set_enabled(true)?;
                        item.set_text("Update failed â€” click to retry")?;
                    }
                    return Result::Ok(false);
                }
            }
            ...
            *self.pending_update.lock().await = Some(PendingUpdateSnapshot {
...

            {
                ...
                let icon_path = if theme == Mode::Light {
                    "assets/MEMORA-logo-tray-updates-black.png"
                } else {
                    "assets/MEMORA-logo-tray-updates-white.png"
                };
                ...
                let _ = self.app.run_on_main_thread(move || {
                    crate::window::with_autorelease_pool(|| {
                        if let Some(tray) = app_clone.tray_by_id("MEMORA_main") {
                            if let Err(e) =
            ...
            // install â€” there's no way to "silently pre-download" without
                ...
                *self.update_installed.lock().await = true;
                ...
                std::thread::spawn(move || {
                    let _ = app_notif
                        .notification()
                        .builder()
                        .title("MEMORA update ready")
                        .body(format!("v{} ready â€” restart to update", version_str))
                        .show();
                });
            ...
            // whether we restart automatically after â€” the banner is the user's
            ...
            if let Some(ref item) = self.update_menu_item {
                ...
                item.set_text("Downloading latest version of MEMORA")?;
            }

            #[cfg(target_os = "windows")]
            {
                // Windows: stop MEMORA before replacing the binary
                if let Err(err) =
                    stop_MEMORA(self.app.state::<RecordingState>(), self.app.clone()).await
                {
            ...
            // Auth errors (401/403) short-circuit out of the loop â€” see error arm.
            let retry_delays = [
                ...
                loop {
                    ...
                    match &result {
                        ...
                        Err(e) => {
                            ...
                            // Auth errors won't recover from a retry â€” bail out and let
                            ...
                            warn!(
                                "update download attempt {} failed: {} â€” retrying in {}s",
                                attempt + 1,
                                err_str,
                                delay.as_secs()
                            );
                            if let Some(ref item) = self.update_menu_item {
                                let _ = item.set_text(&format!(
                                    "Update download failed â€” retrying in {}s",
                                    delay.as_secs()
                                ));
                            }
            ...
            match download_result {
                Ok(_) => {
                    // Clear any prior failure marker â€” this version is good now.
                        ...
                        || err_str.contains("Forbidden")
                    {
                        ...
                        std::thread::spawn(move || {
                            let _ = app_notif
                                .notification()
                                .builder()
                                .title("MEMORA update available")
                                .body(format!("v{} is ready â€” sign in to download", version_str))
                                .show();
                        });
                        if let Some(ref item) = self.update_menu_item {
                            item.set_enabled(true)?;
                    ...
                    *self.pending_update.lock().await = None;
                    ...
                    std::thread::spawn(move || {
                        let _ = app_notif
                            .notification()
                            .builder()
                            .title("MEMORA update failed")
                            .body(format!(
                                "v{} couldn't download â€” open MEMORA to retry",
                                version_str
                            ))
                            .show();
                    });
            ...
            // after restart â€” needs to happen here because banner-driven restarts
            ...
            std::thread::spawn(move || {
                ...
                let result = if auto_update {
                    notification
                        .title("MEMORA updating")
                        .body(format!("v{} downloaded â€” restarting now", version_str))
                        .show()
                } else {
                    notification
                        .title("MEMORA update ready")
                        .body(format!("v{} downloaded â€” restart to update", version_str))
                        .show()
                };
                if let Err(e) = result {
            ...
            if auto_update && *self.update_installed.lock().await {
                ...
                match bounded_teardown(
                    PRE_EXIT_TEARDOWN_TIMEOUT,
                    stop_MEMORA(self.app.state::<RecordingState>(), self.app.clone()),
                )
                .await
                {
                    TeardownOutcome::Completed => {}
                    ...
                    TeardownOutcome::TimedOut => warn!(
                        "auto-update: teardown exceeded {}s (capture shutdown wedged) â€” relaunching anyway",
                        PRE_EXIT_TEARDOWN_TIMEOUT.as_secs()
                    ),
                }
                crate::process_exit::request_app_relaunch(
    ...
    pub fn set_menu_installing(&self) {
        if let Some(item) = &self.update_menu_item {
            let _ = item.set_text("Installing updateâ€¦");
    ...
    async fn show_source_build_dialog(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if clicked_download {
            ...
            let _ = self
                .app
                .opener()
                .open_url("https://github.com/shrirampai3000/memora/download", None::<&str>);
        } else {
            // Open GitHub releases
            let _ = self.app.opener().open_url(
                "https://github.com/MEMORA/MEMORA/releases",
                None::<&str>,
            );
        }
    ...
    pub async fn start_periodic_event(&self) {
        ...
        // Skip the first tick (fires immediately) â€” boot check already runs separately
        interval.tick().await;
...
fn check_whats_new(app: &tauri::AppHandle) {
    ...
    tokio::spawn(async move {
        ...
        let body = if release_notes.is_empty() {
            format!(
                "MEMORA updated to **v{}**! check the [changelog](https://github.com/shrirampai3000/memora/changelog) for details.",
                current_version
            )
        } else {
            ...
            format!(
                "MEMORA updated to **v{}**!\n\n{}\n\n[full changelog](https://github.com/shrirampai3000/memora/changelog)",
        ...
        match client
            ...
            .json(&serde_json::json!({
                ...
                "type": "app-update",
                "title": format!("ðŸŽ‰ MEMORA v{} â€” what's new", current_version),
                "body": body,
            }))
            .send()
            .await
...
/// User-initiated update check from Settings â†’ General. Returns:
...
pub async fn trigger_update_check(
    ...
    state
        // User clicked "check for updates" in Settings â€” force past the
...
pub fn start_update_check(
    ...
    {
        let rollback_dir = memora_core::paths::default_MEMORA_data_dir().join("rollback");
    ...
    tokio::spawn({
        ...
        async move {
            if let Err(e) = updates_manager.check_for_updates(false, false).await {
                // warn, not error â€” see updater check() note above.
                warn!("Failed to check for updates: {}", e);
            }
...
mod tests {
    ...
    fn sweep_removes_installer_leftovers_only() {
        ...
        std::fs::write(dir.path().join("memora.exe.sp-old2"), b"old").unwrap();
        // Live app files must survive.
        std::fs::write(dir.path().join("bun.exe"), b"new").unwrap();
        std::fs::write(dir.path().join("memora.exe"), b"new").unwrap();

        assert_eq!(sweep_sp_old_files(dir.path()), 2);
        assert!(!dir.path().join("bun.exe.sp-old").exists());
        assert!(!dir.path().join("memora.exe.sp-old2").exists());
        assert!(dir.path().join("bun.exe").exists());
        assert!(dir.path().join("memora.exe").exists());
    }
    ...
    fn cooldown_blocks_same_version_within_window() {
        // v2.5.57 failed 1h ago, 6h cooldown â†’ still blocked (the loop fix).
    ...
    fn cooldown_expires_after_window() {
        // Same version, but the failure was longer ago than the cooldown â†’
    ...
    fn classify_adopts_a_strictly_newer_version() {
        // 166 shipped while 165 was pending â†’ must supersede it (#5784).
        assert!(matches!(
    ...
    fn classify_noop_on_same_healthy_version() {
        // Re-poll sees the same pending version with no prior failure â†’ skip, so we
    ...
    fn classify_noop_when_pending_is_newer() {
        // Server regressed (found older than pending) â€” keep pending.
        assert!(matches!(
    ...
    fn hidden_ui_forces_auto_update_on_even_when_setting_off() {
        // a "run hidden" device has no banner/settings to update otherwise â€” it
    ...
    fn mdm_managed_updates_are_never_forced_even_when_hidden() {
        ...
        // intentionally off so it doesn't fight the external manager â€” hidden
    ...
    async fn await_safe_restart_proceeds_on_boot_error() {
        let _guard = BOOT_PHASE_TEST_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        // A failed boot is finished â€” nothing is mid-init, so restarting is
        ...
        assert_eq!(
            result, "proceed",
            "banner gate must let an errored boot restart â€” relaunch is the recovery path"
        );
    }
    ...
    async fn await_safe_restart_proceeds_when_engine_idle() {
        ...
        // cannot happen â€” and treating idle as pending permanently locked
    ...
    fn update_attempt_classification_matches_versions() {
        ...
        // Still on the version we tried to leave â†’ the install didn't apply.
        assert_eq!(
            classify_update_attempt(&attempt, "2.6.6"),
            UpdateAttemptOutcome::Failed
        );
        // Running the target version â†’ applied.
        assert_eq!(
            classify_update_attempt(&attempt, "2.6.7"),
            UpdateAttemptOutcome::Applied
        );
        // Running something else entirely (manual reinstall) â†’ no conclusion.
        assert_eq!(
    ...
    fn restart_gate_defers_only_while_boot_in_progress() {
        ...
        // both finished, so restarting is safe â€” and for Errored it's the
        // recovery path (#4726).
