...
//! suggestions (via MEMORA cloud AI) during idle/charging periods.
...
//! transcripts) â€” historically the source of repeated UTF-8 char-boundary
//! panics (MEMORA-APP-97 and predecessors). `deny(clippy::string_slice)`
//! forces every byte-index slice through [`memora_core::strings`] so we
...
use memora_core::strings::{safe_byte_prefix, truncate_string};
...
// â”€â”€â”€ Enhanced AI config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

/// When enabled, uses MEMORA cloud (api.memora.com) instead of Apple
...
pub struct EnhancedAIConfig {
    pub enabled: bool,
    /// User's Clerk JWT token for authenticating with MEMORA cloud.
    pub token: String,
}

// â”€â”€â”€ Managed state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub async fn get_cached_suggestions(
    ...
    // Cache is empty (app just started) â€” do a quick template generation
...
/// Enable or disable enhanced AI suggestions (uses MEMORA cloud).
#[tauri::command]
#[specta::specta]
...
// â”€â”€â”€ Auto-start â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub async fn auto_start_scheduler(app: AppHandle, state: &SuggestionsState) {
    ...
    let handle = tokio::spawn(async move {
        ...
        // Initial delay â€” let the server stabilize
        ...
        loop {
            ...
            // Debounce event triggers â€” wait a few seconds for context to settle
            if trigger != "timer" {
                info!(
            ...
            // Check CPU usage â€” skip if system is busy (but not for event triggers)
            ...
            // Check AC power on macOS â€” skip if on battery (but not for event triggers)
...
// â”€â”€â”€ CPU idle check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ AC power check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn detect_mode(apps: &[AppActivity], windows: &[WindowActivity]) -> &'static str {
    ...
    // Communication sites (slack, discord) in browser titles are too noisy â€”
...
// â”€â”€â”€ Template suggestions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn coding_suggestions(
    ...
    // Compute duration estimate from frame counts (1 frame/sec â†’ mins)
    ...
    let editor_windows: Vec<String> = editor
        .map(|e| {
            windows
                ...
                .take(3)
                .map(|w| {
                    // Extract filename from window title (often "filename â€” App")
                    let title = w
                        .window_name
                        .split(" â€” ")
                        .next()
...
fn meeting_suggestions(apps: &[AppActivity], windows: &[WindowActivity]) -> Vec<Suggestion> {
    ...
    let preview = if meeting_mins > 0 {
        if let Some(title) = &meeting_title {
            Some(format!("{}min â€” {}", meeting_mins, title))
        } else {
...
// â”€â”€â”€ Suggestion generation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async fn fetch_app_activity(api: &LocalApiContext) -> Result<Vec<AppActivity>, String> {
    ...
    let resp = api
        ...
        .json(&serde_json::json!({
            "query": "SELECT app_name, COUNT(*) as cnt FROM frames WHERE datetime(timestamp) > datetime('now', '-30 minutes') AND app_name != '' AND app_name != 'MEMORA' AND app_name != 'MEMORA-app' GROUP BY app_name ORDER BY cnt DESC LIMIT 15"
        }))
...
async fn fetch_window_activity(api: &LocalApiContext) -> Result<Vec<WindowActivity>, String> {
    ...
    let resp = api
        .apply_auth(client.post(api.url("/raw_sql")))
        .json(&serde_json::json!({
            "query": "SELECT app_name, window_name, COUNT(*) as cnt FROM frames WHERE datetime(timestamp) > datetime('now', '-30 minutes') AND app_name != '' AND app_name != 'MEMORA' AND app_name != 'MEMORA-app' AND window_name != '' GROUP BY app_name, window_name ORDER BY cnt DESC LIMIT 20"
        }))
        .timeout(std::time::Duration::from_secs(10))
        .send()
        .await
...
// â”€â”€â”€ Multi-source data fetchers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async fn fetch_accessibility_snippets(api: &LocalApiContext) -> Vec<AccessibilitySnippet> {
    ...
    let resp = api
        .apply_auth(client.post(api.url("/raw_sql")))
        .json(&serde_json::json!({
            "query": "SELECT app_name, window_name, SUBSTR(full_text, 1, 200) as snippet FROM frames WHERE datetime(timestamp) > datetime('now', '-15 minutes') AND LENGTH(full_text) > 30 AND app_name != 'MEMORA' AND full_text IS NOT NULL ORDER BY timestamp DESC LIMIT 8"
        }))
...
async fn build_activity_context(
    ...
    for w in windows.iter().take(6) {
        ...
        parts.push(format!("  {} â€” {}", w.app_name, title));
    }
    parts.push(String::new());
    ...
    // 3. Audio transcriptions â€” always include if available (~1500 chars budget)
...
// â”€â”€â”€ AI suggestion + tag generation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

// System prompt â€” returns both suggestions and tags in one AI call (zero extra cost)
const AI_SYSTEM_PROMPT: &str = r#"Analyze this MEMORA user's activity (records screen/audio 24/7). Return a JSON object with "suggestions" and "tags".
...
SUGGESTIONS â€” 6 natural English sentences with spaces between words:
...
TAGS â€” 3-8 short hyphenated labels (tags ARE hyphenated, suggestions are NOT):
...
async fn generate_ai_suggestions(
    ...
    debug!(
        "suggestions: AI prompt ~{} tokens, backend=MEMORA-cloud",
        context.len() / 4
    );

    let client = reqwest::Client::new();
    let gateway_url = match crate::config::MEMORA_ai_gateway_url() {
        Ok(url) => url,
        Err(error) => {
    ...
    let resp = client
        ...
        .header("x-MEMORA-latency", "background")
        .json(&serde_json::json!({
            "model": "auto",
            "messages": [
...
async fn generate_suggestions(
    ...
    let (suggestions, tags, ai_generated) =
        match generate_ai_suggestions(api, mode, &apps, &windows, enhanced_ai).await {
            ...
            None => {
                // Template fallback â€” generate basic tags from mode + top apps
...
mod tests {
    ...
    fn test_detect_mode_browsing_communication_sites() {
        ...
        // as communication mode (too noisy â€” searching "slack alternatives" etc).
    ...
    // â”€â”€â”€ Benchmark tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Run with: cargo test -p MEMORA-app -- --ignored benchmark --nocapture
    // Requires: MEMORA local API running

    #[tokio::test]
    #[ignore] // requires MEMORA running locally
    async fn benchmark_data_sources() {
        ...
        assert!(!apps.is_empty(), "no app data â€” is MEMORA running?");
    ...
    async fn benchmark_context_builder_coverage() {
        ...
        if acc_count > 5 {
            ...
            println!("  âœ“ correctly prioritized accessibility over OCR");
        } else if has_ocr_section {
            println!("  âœ“ correctly fell back to OCR (no accessibility data)");
        }
        ...
        println!("  âœ“ within token budget");
    }
}
