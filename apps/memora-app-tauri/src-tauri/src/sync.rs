...
use memora_core::sync::{get_or_create_machine_id, SyncManager};
...
// Machine ID is now provided by memora_core::sync::get_or_create_machine_id
...
/// Trigger an immediate sync via the MEMORA server.
#[tauri::command]
#[specta::specta]
...
/// This calls the local MEMORA server's /data/delete-device endpoint.
...
/// `lean`, so picking it silently ran `media` instead â€” that frees disk files
