...
/// failure also defaults to `true` â€” we'd rather show one extra toast
...
/// Pure helper split out for unit testing â€” same fail-open semantics
...
/// parse failure â†’ not muted (fail-open). An empty / whitespace name never
...
fn pipe_config_denies_notify(pipe_name: &str) -> bool {
    ...
    let pipe_md = memora_core::paths::default_MEMORA_data_dir()
        .join("pipes")
        .join(pipe_name)
        .join("pipe.md");
...
fn pipe_config_denies_notify_from_content(raw: &str) -> anyhow::Result<bool> {
    let (config, _) = memora_core::pipes::parse_frontmatter(raw)?;
    let perms = memora_core::pipes::permissions::PipePermissions::from_config(&config);
    Ok(!perms.is_endpoint_allowed("POST", "/notify"))
}

/// `POST /notify` â€” show a notification panel and persist to disk.
pub async fn send_notification(
    ...
    // trigger a one-off announcement on demand â€” the same surfaces the
    ...
    if super::gate::suppressed_now(
        ...
        debug!("notify: skipped (notifications paused â€” master/snooze/quiet)");
    ...
    // only thing that tells two distinct alerts apart â€” see the gate's notes.
    ...
    // Other types (`system`, `captureStalls`, â€¦) self-gate upstream
    ...
    // Rewrite file-path markdown links to MEMORA://view?path=â€¦ so they
    ...
    if transient {
        debug!(
            ...
            priority = ?priority,
            "notify: transient â€” delivering without an inbox row"
        );
    } else {
        // Persist to disk before attempting to show â€” survives crashes/restarts
    ...
    // notification type. (Low and lifecycle types are transient â€” delivery is
    ...
    if priority == NotificationPriority::High {
        ...
        tokio::spawn(async move {
            match tokio::time::timeout(
                ...
                // well as checks â€” running it again here would collide with the
    ...
    let source_url = payload.source_url.clone().or_else(|| {
        source_session_id.as_ref().map(|session_id| {
            ...
            if let Some(message_id) = &source_message_id {
                ...
                format!("MEMORA://chat/{}", encoded_session)
            }
        })
    });
...
/// `GET /notifications` â€” list notification history from disk.
pub async fn list() -> Json<Vec<NotificationHistoryEntry>> {
    Json(store::read_all())
}

/// `POST /notifications` â€” mark all notifications as read.
...
/// `DELETE /notifications` â€” clear notification history.
...
/// `DELETE /notifications/:id` â€” dismiss a single notification.
...
// â”€â”€ Payload types â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub struct NotifyPayload {
    ...
    // â”€â”€ announcement mode â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // When `surface` names an announcement surface, `/notify` pushes an
    // in-app announcement instead of a notification panel â€” the on-demand
...
/// with `SURFACES` in `lib/announcements.ts` â€” a surface the frontend renders
...
mod tests {
    ...
    fn gate_defaults_true_when_value_not_bool() {
        // Parse failure / wrong type â†’ fail open. Better one extra toast
    ...
    // â”€â”€ per-pipe mute â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    #[test]
    ...
    fn cta_is_forwarded_verbatim_for_the_frontend_to_narrow() {
        ...
        // contract â€” notably it drops `pipe` for a locally pushed announcement,
    ...
    fn derives_source_metadata_from_pipe_session_header() {
        ...
        assert_eq!(
            ...
            NotificationSourceMetadata {
                ...
                source_url: Some(
                    "MEMORA://chat/pipe%3Along-notification-source-test%3A389?message=notification-abc123"
                        .to_string()
                ),
