...
//! Disk-backed notification history at `~/.memora/notifications.json`.
...
///
/// Interruption and persistence are separate questions â€” that is why this is
...
pub struct NotificationHistoryEntry {
    ...
    /// dismiss â€¦). Persisted so the notification bell can re-offer the same
    /// actions as the transient toast â€” a user who misses the ~20s toast can
...
fn path() -> PathBuf {
    memora_core::paths::default_MEMORA_data_dir().join("notifications.json")
}
...
fn write_all_to(path: &Path, entries: &[NotificationHistoryEntry]) -> Result<String, String> {
    ...
    memora_core::memories::external_sync::write_atomic_full(path, &json)
...
mod tests {
    ...
    fn meeting_lifecycle_and_ambient_status_never_reach_the_inbox() {
        // "meeting detected" / "meeting starting in 2 min" still interrupt â€”
        ...
        // "switched display", "low power mode â€” saver".
    ...
    fn producers_can_override_the_transient_classification() {
        ...
        // Unknown types default to keeping the row â€” chatter is opt-in.
    ...
    fn legacy_action_and_recording_failures_stay_in_focused_inbox() {
        ...
        recording.title = "recording stopped â€” database needs recovery".to_string();
