...
//! viewer deeplinks (`MEMORA://view?path=â€¦`).
//!
...
//!
//! External URLs (`http://`, `https://`) are left alone â€” they should
...
/// and any non-paren character in the URL â€” markdown links can't contain
...
/// the transformed string. Idempotent â€” already-rewritten `MEMORA://`
        ...
        .replace_all(body, |caps: &Captures| {
            ...
            match resolve_local_path(url) {
                Some(abs) => {
                    ...
                    format!("[{}](MEMORA://view?path={})", label, encoded)
                }
...
/// - URL fragments (`/x.md#section` â€” fragment dropped, path opened)
/// - Tilde expansion
/// - Absolute Unix paths
/// - Windows drive paths (`C:\foo` and `C:/foo`)
fn resolve_local_path(url: &str) -> Option<String> {
    // Already a viewer deeplink â€” don't touch.
    if url.starts_with("MEMORA://") {
        return None;
    }
    ...
    let (encoded, was_file_uri) = if let Some(rest) = url_no_frag.strip_prefix("file://") {
        // file:///abs/path â†’ /abs/path; file://host/path â†’ /path (rare)
...
mod tests {
    ...
    fn rewrites_absolute_unix_path() {
        ...
        assert_eq!(
            out,
            "see [log](MEMORA://view?path=%2FUsers%2Fx%2Flog.md) here"
        );
    }
    ...
    fn rewrites_file_uri() {
        ...
        assert_eq!(out, "[x](MEMORA://view?path=%2Ftmp%2Fa.md)");
    }
    ...
    fn idempotent_on_viewer_links() {
        let body = "[x](MEMORA://view?path=%2Ftmp%2Fa.md)";
        assert_eq!(rewrite_file_links(body), body);
    }
    ...
    fn handles_multiple_links() {
        ...
        assert!(out.contains("MEMORA://view?path=%2Fx.md"));
        assert!(out.contains("https://y"));
        assert!(out.contains("MEMORA://view?path=%2Fz.json"));
    }

    #[test]
    fn rewrites_windows_drive_path() {
        let out = rewrite_file_links("[a](C:/foo/bar.md)");
        assert!(out.contains("MEMORA://view?path="));
    ...
    fn decodes_file_uri_with_spaces() {
        // file:// URIs are required to be percent-encoded â€” decode so the
        ...
        // The encoded segment is "User Name" â†’ "%20" â€” not double-encoded "%2520".
    ...
    fn drops_fragment_on_file_uri() {
        ...
        assert!(out.contains("MEMORA://view?path=%2Ftmp%2Fa.md"));
        assert!(!out.contains("section"));
    }

    #[test]
    fn preserves_label_with_unicode() {
        let out = rewrite_file_links("[ðŸ“ log](/x.md)");
        assert!(out.starts_with("[ðŸ“ log](MEMORA://view?path="));
    }
}
