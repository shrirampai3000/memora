...
//! Notification delivery gate â€” the single source of truth for whether a
...
//!
//!   1. master off  â€” "All notifications" turned off (`notificationsEnabled`)
//!   2. snooze       â€” paused until a timestamp (`snoozeUntil`, epoch ms)
//!   3. quiet hours  â€” a recurring daily window (`quietHours`, local wall-clock)
//!
//! Two things punch through a *temporary* pause (snooze / quiet hours):
//!   - `capture_stall` / `disk_pressure` / `db_recovery` â€” critical
//!     recording-stopped alerts.
//!     MEMORA's whole job is to keep recording, so no reduced state hides
//!     it (not even master-off).
//!   - VIP pipes â€” pipes the user marked "always notify" (`allowDuringPause`).
...
/// Keep this list tiny â€” it exists to prevent silent recording failure.
...
/// Wrap-aware: a 22:00â†’08:00 window is active before 08:00 OR after 22:00.
...
pub fn suppressed(
    ...
    // Critical alerts (recording stopped) always pass â€” no exceptions.
...
// â”€â”€ settings reads â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/// Decide for a live notification using the wall clock â€” what the choke point
...
// is coarse â€” `pipe`, `system`, `meeting` â€” so a type-only cooldown would let
...
// meeting inside the five-minute window an "identical repeat" and dropped it â€”
...
/// alerts, not a quota â€” it should erase tick-storms and be invisible
/// otherwise.
...
/// muting the alert until wall-clock catches up â€” failing open is the only
...
/// Check-and-record for a live notification â€” what the choke point calls
...
/// the payload to `show_notification_panel` â€” the choke point every other
...
/// Extract the `body` field. Part of the repeat identity â€” see the module
...
mod tests {
    ...
    // â”€â”€ master â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    #[test]
    ...
    // â”€â”€ critical exemption â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    #[test]
    ...
    // â”€â”€ snooze â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    #[test]
    ...
    // â”€â”€ quiet hours â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    #[test]
    ...
    // â”€â”€ VIP pipes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    fn vip_guard(
        snooze: Option<i64>,
    ...
    // â”€â”€ parsing + guard build â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    #[test]
    fn parses_hhmm() {
    ...
    /// dropped it â€” along with the "open note" button that is the whole point
    ...
    fn a_different_meeting_is_not_a_repeat() {
        ...
        let first = repeat_key(
            ...
            None,
            "meeting detected",
            "MEMORA is saving this meeting for transcription: Google Meet",
        );
        let second = repeat_key(
            Some("meeting"),
            None,
            "meeting detected",
            "MEMORA is saving this meeting for transcription: standup",
        );
    ...
    /// producer shares. Only the choke point may record â€” when both did, the
    ...
    fn looking_twice_at_one_alert_still_delivers_it() {
        ...
        assert!(
            ledger.is_empty(),
            "the peek must not write â€” that write is what ate the alert"
        );
    ...
    /// `show_notification_panel`, which gated again â€” so every high-priority
    ...
    fn asking_the_same_gate_twice_suppresses_a_first_time_alert() {
        ...
        assert!(
            ...
            "asking twice suppresses the alert the caller just cleared â€” \
