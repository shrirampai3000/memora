...
fn owned_log_files(files: Vec<LogFile>) -> Vec<LogFile> {
    files
        .into_iter()
        .filter(|file| is_MEMORA_owned_log_name(&file.name))
        .collect()
}

fn is_MEMORA_owned_log_name(name: &str) -> bool {
    matches!(
        name,
        "MEMORA.log"
            | "MEMORA-app.log"
            ...
            | "recover.stderr.log"
            | ".db_cleanup.log"
    ) || is_dated_rolling_log(name, "MEMORA.")
        || is_dated_rolling_log(name, "MEMORA-app.")
}
...
mod tests {
    ...
    async fn symlinks_are_never_followed() {
        ...
        let link = dir.path().join("MEMORA.log");
    ...
    fn redacted_bundle_remains_transport_bounded_and_utf8_safe() {
        let oversized = format!("{}̩", "x".repeat(MAX_REDACTED_BUNDLE_BYTES + 8));
    ...
    #[test]
    fn unattended_collection_only_accepts_known_MEMORA_log_names() {
        for name in [
            "MEMORA.log",
            "MEMORA.2026-07-10.log",
            "MEMORA.2026-07-10.2.log",
            "MEMORA-app.2026-07-10.log",
            "last-panic.log",
            "recover.stderr.log",
            ".db_cleanup.log",
        ] {
            assert!(
                is_MEMORA_owned_log_name(name),
        ...
        for name in [
            ...
            "customer.log",
            "memora-secrets.log",
            "MEMORA.2026-7-10.log",
            "MEMORA.2026-07-10.backup.log",
        ] {
            assert!(
                !is_MEMORA_owned_log_name(name),
