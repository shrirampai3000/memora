...
//! - audio stall â†’ OS-level notification via tauri_plugin_notification. Likely
//!   the user is in their meeting (Zoom in front), not the live note pane â€”
//!   they need a signal that breaks out of the app.
//! - transcript stall â†’ in-app `/notify` panel. The user is presumably already
...
//! true. Toggling them off in Settings â†’ Notifications silences this module
//! completely.
...
pub fn start(app: AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            memora_events::subscribe_to_event::<StallEvent>("meeting_streaming_audio_stall");
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub = memora_events::subscribe_to_event::<StallEvent>(
...
fn handle_audio_stall(app: &AppHandle, event: StallEvent) {
    ...
    // OS notification â€” fires through the system notification center so it
    // surfaces even when the meeting window is on top of MEMORA. Use
    ...
    std::thread::spawn(move || {
        if let Err(e) = app_for_notify
            .notification()
            .builder()
            .title("MEMORA is not capturing audio")
            .body(format!(
                "no audio reached the meeting after {elapsed}s â€” check mic permissions or restart the recorder"
            ))
            .show()
        {
...
fn handle_transcript_stall(app: &AppHandle, event: StallEvent) {
    ...
    client::send_typed_with_actions_and_priority(
        ...
        format!("audio is still being captured, but the transcript is delayed by {elapsed}s â€” keep the meeting open while MEMORA retries"),
        "meeting",
        Some(30_000),
        Vec::new(),
