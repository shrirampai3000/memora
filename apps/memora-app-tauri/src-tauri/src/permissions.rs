...
/// must never be treated as consent to relaunch MEMORA.
#[tauri::command]
#[specta::specta]
pub async fn restart_after_screen_recording_permission(app: tauri::AppHandle) {
    ...
    info!("Screen Recording restart confirmed by user â€” relaunching to apply permission");
...
pub async fn request_permission(app: tauri::AppHandle, permission: OSPermission) {
    #[cfg(target_os = "macos")]
    {
        ...
        match permission {
            ...
            OSPermission::Microphone => {
                ...
                match status {
                    ...
                    AVAuthorizationStatus::NotDetermined => {
                        // First time â€” show the system prompt
            ...
            OSPermission::Automation => {
                // Open Automation settings â€” user must toggle manually
                open_permission_settings(OSPermission::Automation);
            }
            OSPermission::InputMonitoring => {
                // Defer to the dedicated request flow (opens Settings + calls
                // CGRequestListenEventAccess). No probe tap is created â€” the
        ...
        let callback = move |granted: BOOL| {
            if is_audio && granted != NO {
                ...
                let capture_intended = app_for_callback
                    ...
                    .unwrap_or(false);
                if !capture_intended {
                    debug!("Microphone permission granted via AV callback â€” capture not intended, skipping restart");
                    return;
                }
                info!(
                    "Microphone permission granted via AV callback â€” restarting capture for audio reinit"
                );
...
/// Guards concurrent/duplicate `restart_capture_on_mic_grant` invocations â€”
...
/// Intel-Mac CI smoke-test investigation, MEMORA#4978).
#[cfg(target_os = "macos")]
...
/// this mic-grant attempt. Matches `updates.rs::AUTO_UPDATE_GATE_TIMEOUT` â€”
...
/// fixed retry budget against the "Server not running" error string â€” which
/// raced a slow-but-succeeding boot (DB migration, first-launch model
/// download) and gave up before `ServerCore` was even constructed â€” wait on
/// the boot-phase state machine that `spawn_MEMORA`'s race-prevention
/// path and the auto-update restart gate already use for this exact class of
/// race (found during the Intel-Mac CI smoke-test investigation, MEMORA#4978).
#[cfg(target_os = "macos")]
pub(crate) async fn restart_capture_on_mic_grant(app: tauri::AppHandle) {
    ...
    match crate::health::wait_for_boot_ready(BOOT_WAIT_TIMEOUT).await {
        crate::health::BootReadiness::Errored => {
            error!(
                "start_capture after mic grant: boot phase errored ({}) â€” capture cannot start \
        ...
        crate::health::BootReadiness::Pending => {
            error!(
                "start_capture after mic grant: boot still not ready after {}s (phase={}) â€” \
        ...
        crate::health::BootReadiness::Idle => {
            warn!(
                "start_capture after mic grant: engine is idle (never started) â€” \
    ...
    // Boot is confirmed ready â€” RecordingState.server is assigned a handful
...
fn check_accessibility_permission() -> OSPermissionStatus {
    core_to_os_status(memora_core::permissions::check_accessibility())
}
...
fn core_to_os_status(status: memora_core::permissions::PermissionStatus) -> OSPermissionStatus {
    match status {
        memora_core::permissions::PermissionStatus::NotNeeded => OSPermissionStatus::NotNeeded,
        memora_core::permissions::PermissionStatus::NotDetermined => OSPermissionStatus::Empty,
        memora_core::permissions::PermissionStatus::Granted => OSPermissionStatus::Granted,
        memora_core::permissions::PermissionStatus::Denied => OSPermissionStatus::Denied,
    }
}
...
pub fn check_microphone_permission() -> OSPermissionStatus {
    core_to_os_status(memora_core::permissions::check_microphone())
}
...
pub fn check_screen_recording_permission() -> OSPermissionStatus {
    #[cfg(target_os = "macos")]
    {
        screen_recording_status(
            ...
            memora_engine::permission_monitor::screen_enumeration_denied(),
            false,
        )
    }
...
pub fn check_accessibility_permission_live_cmd() -> OSPermissionStatus {
    core_to_os_status(memora_core::permissions::check_accessibility_live())
}
...
pub fn check_input_monitoring_permission_cmd() -> OSPermissionStatus {
    #[cfg(target_os = "macos")]
    {
        if memora_a11y::check_input_monitoring() {
            OSPermissionStatus::Granted
        } else {
            // The TCC preflight API doesn't distinguish NotDetermined from
            // Denied â€” both return false. We surface as Empty so the UI
...
pub async fn request_input_monitoring_permission() -> OSPermissionStatus {
    #[cfg(target_os = "macos")]
    {
        ...
        // calls it. Subsequent calls are no-ops if denied â€” the user has
        // to enable from System Settings, which we just opened.
        if memora_a11y::request_input_monitoring() {
            OSPermissionStatus::Granted
        } else {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if matches!(permission, OSPermission::Calendar) {
            tokio::task::spawn_blocking(|| {
                let cal = memora_connect::calendar::MEMORACalendar::new();
                cal.reset();
            })
            .await
...
pub async fn reset_and_request_permission(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let service = match &permission {
            ...
            OSPermission::Calendar => "Calendar",
            OSPermission::Automation => {
                // Automation doesn't use tccutil reset flow â€” just open settings
                open_permission_settings(OSPermission::Automation);
                return Ok(());
            }
        };
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if is_app_bundle() {
            ...
            for browser in &installed {
                // Only prompt browsers that are already running â€” never force-launch (#2510)
                ...
                if running {
                    ...
                    // Not running â€” silently check without prompting
...
pub fn request_single_browser_automation(browser_name: String) -> String {
    #[cfg(target_os = "macos")]
    {
        ...
        if !running {
            // Can't prompt â€” open System Settings as fallback
            open_permission_settings(OSPermission::Automation);
            return "not_asked".to_string();
        }
...
/// In production: triggers "MEMORA wants to control Arc" prompt via direct FFI.
...
pub fn request_arc_automation_permission(_app: tauri::AppHandle) -> bool {
    #[cfg(target_os = "macos")]
    {
        ...
        // Only prompt if Arc is already running â€” never force-launch (#2510)
        ...
        if is_app_bundle() {
            if arc_running {
                ...
                // Arc not running â€” open System Settings instead of force-launching
                open_permission_settings(OSPermission::Automation);
                false
            }
        } else {
...
// `memora-engine::permission_monitor` which emits `permission_lost` /
...
mod screen_recording_state_tests {
    ...
    fn stale_positive_preflight_does_not_read_as_granted_while_enumeration_denied() {
        ...
        // Verdict cleared (enumeration succeeded) â†’ preflight trusted again.
        assert_eq!(
            screen_recording_status(ScreenRecordingPermissionState::Granted, false, false),
            OSPermissionStatus::Granted
        );
...
mod mic_grant_restart_tests {
    ...
    // the Intel-Mac CI smoke-test investigation, MEMORA#4978).
    #[test]
    fn guard_resets_after_scope_so_a_later_call_can_retry() {
        ...
        } // guard drops here â€” must release the flag
        assert!(
