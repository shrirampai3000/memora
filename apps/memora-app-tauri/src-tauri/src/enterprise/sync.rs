...
//!
//! Periodically pulls new screen + audio + UI activity from the local MEMORA
//! API and POSTs it as JSONL to the MEMORA enterprise ingest endpoint,
...
//! the desktop crate stays a thin orchestration layer â€” only the
//! `LocalApiClient` trait is injected from outside (mockable for tests).
//!
//! # Edge cases handled
//!
//! - **Empty batch** â€” skip POST, advance no cursor, retry next tick
//! - **Network failure** â€” exponential backoff (60s â†’ 1h cap), task survives
//! - **4xx auth failure** â€” use the signed-in employee account to fetch the
//!   rotated device key, then retry without advancing the cursor
//! - **5xx server error** â€” exponential backoff (transient, can recover)
//! - **Cursor file corruption** â€” fall back to "now - SAFE_BACKFILL", never
//!   re-emit the entire DB
//! - **Cursor file missing on first run** â€” start from "now - SAFE_BACKFILL"
//!   (15min) so first batch isn't empty
//! - **Body too large** â€” paginate via `limit` + advance cursor incrementally
//! - **Clock skew** â€” cursor is the *server's* timestamp from frames table, not
//!   wall-clock; idempotency is by `(device_id, frame_id)` server-side
//! - **Graceful shutdown** â€” task respects cancellation token, drains in flight
...
/// Default endpoint. Overridable via `MEMORA_ENTERPRISE_INGEST_URL` for
/// staging / on-prem.
pub fn default_ingest_url() -> String {
    // Baked control-plane base (crate::web_base): a build produced with
    // NEXT_PUBLIC_MEMORA_WEB_URL targets that deployment app-wide,
    // exactly like a prod build targets github.com/shrirampai3000/memora.
    crate::web_base::MEMORA_web_url("/api/enterprise/ingest")
}
...
impl EnterpriseSyncConfig {
    ...
    /// required env (`MEMORA_ENTERPRISE_LICENSE_KEY`) is missing â€” caller
    ...
    /// old "set `MEMORA_ENTERPRISE_UPLOAD_MODE` on every device" UX â€”
    ...
    /// URL resolved from somewhere else (`~/.memora/enterprise.json`,
    /// populated by the desktop's in-app prompt or the sign-in-driven
    /// device-config fetch). Env vars still win when set â€” that keeps MDM
    /// rollouts working â€” but a missing env no longer disables enterprise
    ...
    pub fn from_env_with_fallback(
        ...
        let license_key = std::env::var("MEMORA_ENTERPRISE_LICENSE_KEY")
            .ok()
            .filter(|s| !s.trim().is_empty())
            .or_else(|| license_key_fallback.filter(|s| !s.trim().is_empty()))?;
        let ingest_url = std::env::var("MEMORA_ENTERPRISE_INGEST_URL")
            .ok()
            .filter(|s| !s.trim().is_empty())
        ...
        let explicit_mode = std::env::var("MEMORA_ENTERPRISE_UPLOAD_MODE")
            .ok()
            .map(|s| s.trim().to_ascii_lowercase())
            .filter(|s| {
                !s.is_empty() && s != "auto" && s != "MEMORA_write" && s != "hosted_ingest"
            });
    ...
    /// sync run â€” if the lookup fails, the existing mode is preserved.
    ///
    /// This is what makes the "install enterprise build â†’ enter license key
    /// â†’ uploads start" flow possible without any env-var setup on the
    /// customer's machine.
...
// â”€â”€â”€ Cursor (last-synced timestamp) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Local API trait (mockable) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

/// Abstraction over the local MEMORA HTTP API â€” kept generic so tests can
...
pub trait LocalApiClient: Send + Sync {
    ...
    /// extracted workflows their *verbs* â€” without them an SOP can only
    ...
    /// to the previously snapshotted one). Default returns None â€” shims
    ...
    /// Memories are the *distilled* layer above the raw frame/audio firehose â€”
    ...
    /// Fetch one frame's full-resolution JPEG by id â€” the same image the
...
// â”€â”€â”€ Wire types â€” what we POST upstream â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
// The record schema (`TelemetryRecord` + the `*Row` flattenings) is the
// shared wire contract â€” the customer query gateway parses batches with the
// same types â€” so it lives in `MEMORA-telemetry-wire`. Re-exported here
// so the desktop shim keeps importing everything from `ee_sync::`.
pub use MEMORA_telemetry_wire::{
...
pub enum EnterpriseSyncError {
    ...
    /// MDM-pinned destination list (`MEMORA_ENTERPRISE_UPLOAD_PINNED_HOSTS`).
    /// This is the destination-pin tamper signal â€” permanent, loud, never
...
// â”€â”€â”€ Pure logic: UTF-8-safe truncation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/// inside a multi-byte character â€” Polish `Å‚`, German `ÃŸ`, any CJK glyph or
...
pub fn truncate_on_char_boundary(s: &str, max_bytes: usize) -> &str {
    ...
    // loop terminates â€” at worst at 0 (a leading multi-byte char wider than
    // max_bytes), yielding "".
...
// â”€â”€â”€ Pure logic: build the JSONL payload â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub use MEMORA_telemetry_wire::{build_jsonl, build_jsonl_with_parsed};
...
// â”€â”€â”€ Ingest HTTP â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub async fn post_jsonl(
    ...
    if status == reqwest::StatusCode::FORBIDDEN {
        ...
        // problem â€” an admin must enable centralized data in the dashboard
    ...
    // Other 4xx â€” treat as fatal-ish but recoverable on next tick.
...
// â”€â”€â”€ Sync state machine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async fn run_one_sync_inner(
    ...
    // First-run safeguard: if cursor is empty, backfill SAFE_BACKFILL only â€”
    ...
    // Per-stream sync policy is fetched fresh on every tick â€” the admin can
    ...
    // UI events are best-effort â€” a backend that doesn't expose them yet
    ...
    // One snapshot per tick. Best-effort â€” failure to encode/fetch
    ...
    // Memories are best-effort too â€” a client that predates the trait
    ...
    body.extend_from_slice(&MEMORA_telemetry_wire::build_feedback_jsonl(
        &cfg.device_id,
        &cfg.device_label,
        &feedback,
    ));
    ...
    // Advance cursor only on success â€” partial failure must not skip records.
    *cursor = next_cursor;
...
// â”€â”€â”€ On-demand frame fulfillment (P3) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
// orgs â€” their telemetry bypasses our cloud, so frames must too. Errored ids
...
/// Per-tick batch in "all" mode â€” the org chose to centralize every frame,
...
/// Width bound for uploaded frames â€” readable for SOP steps, not a raw dump.
pub const FRAME_MAX_WIDTH: u32 = 1280;
const FRAME_JPEG_QUALITY: u8 = 70;
const FRAME_JPEG_QUALITY_FALLBACK: u8 = 50;

/// Derive the control-plane base (e.g. `https://github.com/shrirampai3000/memora`) from the
/// configured ingest URL, so staging / on-prem `MEMORA_ENTERPRISE_INGEST_URL`
...
pub struct FrameRequestsResponse {
    ...
    /// Server signals the stream is off by policy â€” don't bother uploading.
    #[serde(default)]
    pub disabled: bool,
}
...
/// Decode â†’ bound width at `FRAME_MAX_WIDTH` (aspect preserved) â†’ JPEG.
...
/// One fulfillment pass. Infallible by contract â€” every failure is logged
...
// â”€â”€â”€ Long-running task â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/// Stable, regex-safe identifier (`^[A-Za-z0-9._:-]+$`, â‰¤128) for the logs API.
...
///
/// Runs every tick regardless of telemetry sync outcome â€” including on devices
...
async fn fulfill_log_requests(
    ...
    // Existing readable customer-storage orgs deliberately grant MEMORA
    ...
    info!("enterprise sync: admin requested device logs â€” collecting + uploading");
    let feedback = format!(
    ...
    loop {
        ...
        match &result {
            ...
            Err(EnterpriseSyncError::CentralizedDataDisabled) => {
                error!(
                    "enterprise sync: centralized data is NOT enabled for this org â€” an admin must enable it in the dashboard before devices can upload; pausing {}s",
        ...
        match result {
            Ok(report) => {
                ...
                // On-demand frame fulfillment â€” best-effort, gated on the
...
/// graceful-shutdown protocol â€” tauri SIGKILLs on quit), which makes
...
/// the loop notices. That's fine â€” production `dur` is 5min, tests can use
/// short intervals.
...
mod tests {
    ...
    async fn enterprise_log_confirmation_uses_shared_device_metadata() {
        ...
        tokio::fs::write(
            dir.path().join("MEMORA.2026-07-10.log"),
    ...
    fn log_requests_response_wire_contract() {
        ...
        // nothing pending â€” both fields default cleanly
    ...
    // â”€â”€â”€ build_jsonl â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€â”€ truncate_on_char_boundary (UTF-8 safety) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn truncate_ascii_cuts_exactly() {
        ...
        // max_bytes == len â†’ whole string (len is always a char boundary).
    ...
    fn truncate_rounds_down_through_multibyte_char() {
        ...
        // bytes then Polish 'Å‚' (U+0142, 2 bytes) straddling byte 200. `&t[..200]`
        // panicked; the helper must round down to 199 and NOT panic.
        let prefix = "a".repeat(199);
        let t = format!("{prefix}Å‚ and more text");
        assert!(
            !t.is_char_boundary(200),
            "test premise: byte 200 splits 'Å‚'"
        );
        assert_eq!(truncate_on_char_boundary(&t, 200), prefix);
        // The real call-site shape ("{prefix}â€¦") stays panic-free.
        assert_eq!(
            format!("{}â€¦", truncate_on_char_boundary(&t, 200)),
            format!("{prefix}â€¦")
        );
    }
    ...
    fn truncate_keeps_char_when_boundary_lands_exactly() {
        // "æ—¥æœ¬èªž" â€” each char is 3 bytes.
        assert_eq!(truncate_on_char_boundary("æ—¥æœ¬èªž", 3), "æ—¥"); // byte 3 = boundary
        assert_eq!(truncate_on_char_boundary("æ—¥æœ¬èªž", 4), "æ—¥"); // inside æœ¬ â†’ round down
        assert_eq!(truncate_on_char_boundary("æ—¥æœ¬èªž", 9), "æ—¥æœ¬èªž"); // whole string
    }

    #[test]
    fn truncate_handles_emoji_and_leading_wide_char() {
        // ðŸ‘ is 4 bytes (U+1F44D); a limit inside it rounds all the way to 0.
        assert_eq!(truncate_on_char_boundary("ðŸ‘ab", 2), "");
        assert_eq!(truncate_on_char_boundary("ðŸ‘ab", 4), "ðŸ‘");
        assert_eq!(truncate_on_char_boundary("ðŸ‘ab", 5), "ðŸ‘a");
    }
    ...
    fn jsonl_serializes_memories_with_all_fields() {
        ...
        // Frame provenance is preserved as null when absent â€” server can still
    ...
    // â”€â”€â”€ EnterpriseSyncConfig::from_env â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn from_env_handles_all_cases() {
        ...
        let prior_license = std::env::var("MEMORA_ENTERPRISE_LICENSE_KEY").ok();
        let prior_url = std::env::var("MEMORA_ENTERPRISE_INGEST_URL").ok();
        let prior_mode = std::env::var("MEMORA_ENTERPRISE_UPLOAD_MODE").ok();

        // Case 1: no license env â†’ None.
        std::env::remove_var("MEMORA_ENTERPRISE_LICENSE_KEY");
        std::env::remove_var("MEMORA_ENTERPRISE_UPLOAD_MODE");
        let dir = TempDir::new().unwrap();
        assert!(
        ...
        // Case 2: blank license env â†’ None.
        std::env::set_var("MEMORA_ENTERPRISE_LICENSE_KEY", "   ");
        let dir = TempDir::new().unwrap();
        assert!(
            EnterpriseSyncConfig::from_env(dir.path().to_path_buf(), "dev".into(), "host".into())
                .is_none(),
        ...
        // Case 3: license set, ingest url unset â†’ default url.
        std::env::set_var("MEMORA_ENTERPRISE_LICENSE_KEY", "sek_test");
        std::env::remove_var("MEMORA_ENTERPRISE_INGEST_URL");
        let dir = TempDir::new().unwrap();
        let cfg =
            EnterpriseSyncConfig::from_env(dir.path().to_path_buf(), "dev".into(), "host".into())
        ...
        std::env::set_var("MEMORA_ENTERPRISE_INGEST_URL", "https://staging/ingest");
        let dir = TempDir::new().unwrap();
        let cfg =
            EnterpriseSyncConfig::from_env(dir.path().to_path_buf(), "dev".into(), "host".into())
                .unwrap();
        ...
        std::env::set_var("MEMORA_ENTERPRISE_UPLOAD_MODE", "carrier_pigeon");
        let dir = TempDir::new().unwrap();
        ...
        match prior_license {
            Some(v) => std::env::set_var("MEMORA_ENTERPRISE_LICENSE_KEY", v),
            None => std::env::remove_var("MEMORA_ENTERPRISE_LICENSE_KEY"),
        }
        match prior_url {
            Some(v) => std::env::set_var("MEMORA_ENTERPRISE_INGEST_URL", v),
            None => std::env::remove_var("MEMORA_ENTERPRISE_INGEST_URL"),
        }
        match prior_mode {
            Some(v) => std::env::set_var("MEMORA_ENTERPRISE_UPLOAD_MODE", v),
            None => std::env::remove_var("MEMORA_ENTERPRISE_UPLOAD_MODE"),
        }
    }

    // â”€â”€â”€ run_one_sync (with mocks) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    impl UploadModeEnvGuard {
        fn clear() -> Self {
            let prior = std::env::var("MEMORA_ENTERPRISE_UPLOAD_MODE").ok();
            std::env::remove_var("MEMORA_ENTERPRISE_UPLOAD_MODE");
            Self(prior)
        }
    }
    ...
    impl Drop for UploadModeEnvGuard {
        fn drop(&mut self) {
            match self.0.take() {
                Some(value) => std::env::set_var("MEMORA_ENTERPRISE_UPLOAD_MODE", value),
                None => std::env::remove_var("MEMORA_ENTERPRISE_UPLOAD_MODE"),
            }
        }
    }
    ...
    async fn explicit_mdm_mode_resolves_without_calling_the_control_plane() {
        let _env_lock = ENV_LOCK.lock().unwrap();
        let _mode_env = UploadModeEnvGuard(std::env::var("MEMORA_ENTERPRISE_UPLOAD_MODE").ok());
        std::env::set_var(
            "MEMORA_ENTERPRISE_UPLOAD_MODE",
            "direct_upload_write_only",
        );
    ...
    async fn first_run_seeds_cursor_to_recent_window() {
        ...
        run_one_sync(&cfg, &mut cursor, &local, &http)
            .await
            .unwrap();
        // Cursor is now seeded â€” second tick uses it as the `since`.
        ...
        let age = now - parsed;
        // Should be within SAFE_BACKFILL Â± a generous buffer.
    ...
    async fn old_cursor_drains_every_page_without_waiting_for_normal_ticks() {
        ...
        // MockLocal pops from the end: yield first â†’ second â†’ final.
    ...
    async fn memories_advance_their_own_cursor() {
        // Memory-only batch â€” no frame/audio activity. The tick should still
    ...
    async fn parsed_rows_upload_as_a_separate_gated_stream_end_to_end() {
        ...
        let batch = MEMORA_telemetry_wire::parse_jsonl(body);
    ...
    async fn direct_upload_success_advances_cursor_after_complete() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("POST"))
            ...
            .mount(&server)
            .await;
        // Write-only mode is plaintext JSONL on the wire â€” the trust model is
        // IAM (no MEMORA read principal), not ciphertext. The PUT body
    ...
    // â”€â”€â”€ per-stream sync gate (PR #3581) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    //
    ...
    async fn sync_gate_skips_disabled_streams_and_lets_enabled_through() {
        ...
        // wire â€” the most direct evidence that the gate worked, not just
        ...
        assert_eq!(
            *local.frames_calls.lock().unwrap(),
            0,
            "frames disabled â€” fetch_frames_since must not be called"
        );
        assert_eq!(
            *local.ui_calls.lock().unwrap(),
            0,
            "ui disabled â€” fetch_ui_events_since must not be called"
        );
        assert_eq!(
            *local.snapshot_calls.lock().unwrap(),
            0,
            "snapshots disabled â€” fetch_latest_snapshot must not be called"
        );
        ...
        // Cursors for disabled streams stay put â†’ re-enabling the stream
    ...
    // â”€â”€â”€ On-demand frame fulfillment (P3) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn control_plane_base_derives_from_ingest_url() {
        assert_eq!(
            control_plane_base("https://github.com/shrirampai3000/memora/api/enterprise/ingest").as_deref(),
            Some("https://github.com/shrirampai3000/memora")
        );
        assert_eq!(
            control_plane_base("https://staging.github.com/shrirampai3000/memora:8443/api/enterprise/ingest").as_deref(),
            Some("https://staging.github.com/shrirampai3000/memora:8443")
        );
        // No /api/ segment â†’ can't derive, must not guess.
    ...
    fn downscale_bounds_width_and_size() {
        ...
        // Aspect preserved: 1600x900 â†’ 1280x720.
    ...
    async fn fulfill_skips_when_stream_disabled() {
        ...
        let server = wiremock::MockServer::start().await;
        // Zero expected requests â€” the policy gate short-circuits before HTTP.
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .respond_with(wiremock::ResponseTemplate::new(200))
            .expect(0)
            .mount(&server)
