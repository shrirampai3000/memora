...
//!
//! The frontend fetches the policy from the MEMORA API and pushes it here
...
/// How many frame images (screenshots) may leave this device â€” the org's
...
///   Off   â†’ no frame images ever leave the device (default)
///   Cited â†’ on-demand only: upload exactly the frame_ids cloud pipes cite
///   All   â†’ continuous: the server auto-cites every ingested frame and the
///           device drains the manifest in larger batches
/// Screen pixels are a NEW data class vs the text streams, so unknown or
/// legacy values parse conservatively â€” only an explicit opt-in enables it.
...
impl FrameImagesMode {
    ...
    /// existed ("true" â†’ Cited). Anything else is Off â€” fail closed.
...
fn env_hides_app_ui() -> bool {
    std::env::var("MEMORA_ENTERPRISE_HIDE_APP")
        .ok()
        .as_deref()
        .map(truthy)
        .unwrap_or(false)
        || std::env::var("MEMORA_ENTERPRISE_UI_MODE")
            .ok()
            .as_deref()
            .map(|mode| {
                matches!(
...
fn user_enterprise_config_path() -> std::path::PathBuf {
    memora_core::paths::default_MEMORA_data_dir().join("enterprise.json")
}
...
/// poisoned â€” fail-open here mirrors the centralized-data master-switch
...
/// `enterprise_sync::run_one_sync` test mod â€” two tests racing on a global
