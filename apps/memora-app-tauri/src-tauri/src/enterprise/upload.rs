...
//!
//! Hosted ingest sends plaintext JSONL to MEMORA over TLS. Direct upload
...
//! S3-compatible endpoint via SigV4 presigned URLs â€” the ticket's
...
//!   MEMORA `PutObject` only â€” no MEMORA principal ever holds
...
//!   grants MEMORA read access so cloud pipes / Workflow Studio keep
//!   working against their bucket.
//!
//! In both cases MEMORA Cloud sees checksums and cursors, not the
//! telemetry body. (The former client-side encryption layer was dropped in
//! favor of IAM-enforced write-only access â€” rationale in the website
//! repo's `docs/query-gateway-architecture.md`, "Trust model".)

use reqwest::header::HeaderMap;
use memora_core::sync::crypto::compute_checksum;
use MEMORA_sync::pipeline::{TicketedConfig, TicketedPipeline};
use MEMORA_sync::SyncError;
use serde::{Deserialize, Serialize};
use std::time::Duration;
use tracing::warn;

// The manifest shapes, mode strings, and batch-id derivation are the shared
// wire contract (also parsed by the customer query gateway) â€” they live in
// `MEMORA-telemetry-wire`, not here.
...
pub enum EnterpriseUploadMode {
    HostedIngest,
    /// Plaintext JSONL PUT into a bucket no MEMORA principal can read.
...
pub struct DirectUploadConfig {
    ...
    /// (`MEMORA_ENTERPRISE_UPLOAD_PINNED_HOSTS`, comma-separated
    /// `host` / `host:port` entries). When non-empty, a ticket whose
    /// presigned `upload_url` points anywhere else is refused before any
    /// byte is uploaded â€” the device-side check that a compromised control
...
impl EnterpriseUploadMode {
    ...
    /// customers don't have to set `MEMORA_ENTERPRISE_UPLOAD_MODE`
    /// on every device â€” the storage binding in the dashboard is the
    ...
    pub async fn resolve(
        ...
        // Explicit env override â€” for MDM rollouts and local testing.
        // Only takes effect when set to a non-default value; the empty /
        // default case falls through to server resolution.
        if let Ok(raw) = std::env::var("MEMORA_ENTERPRISE_UPLOAD_MODE") {
            let normalized = raw.trim().to_ascii_lowercase();
            if !normalized.is_empty()
                && normalized != "MEMORA_write"
                ...
                if let Some(mode) = Self::from_env(ingest_url) {
                    tracing::info!(
                        "enterprise sync: upload mode taken from \
                         MEMORA_ENTERPRISE_UPLOAD_MODE env override ({})",
                        normalized
                    );
                    return Ok(Some(mode));
                }
        ...
        match fetch_desired_mode_from_server(license_key, ingest_url).await {
            ...
            Ok(ServerModeHint::DirectUpload) => {
                ...
                tracing::info!(
                    "enterprise sync: server requested direct upload â†’ direct_upload_readable"
                );
                Ok(Some(Self::DirectReadable(
                    DirectUploadConfig::from_ingest_url(ingest_url),
                )))
    ...
    pub fn from_env(ingest_url: &str) -> Option<Self> {
        let mode = std::env::var("MEMORA_ENTERPRISE_UPLOAD_MODE")
            .unwrap_or_else(|_| "MEMORA_write".to_string())
            .trim()
            .to_ascii_lowercase();

        match mode.as_str() {
            "" | "MEMORA_write" | "hosted_ingest" => Some(Self::HostedIngest),
...
// â”€â”€â”€ Control-plane mode hint â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn parse_server_mode_hint(raw: &str) -> Result<ServerModeHint, EnterpriseSyncError> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "hosted_ingest" | "MEMORA_write" | "" => Ok(ServerModeHint::HostedIngest),
...
impl DirectUploadConfig {
    fn from_ingest_url(ingest_url: &str) -> Self {
        let ticket_url = std::env::var("MEMORA_ENTERPRISE_UPLOAD_TICKET_URL")
            .ok()
            .filter(|s| !s.trim().is_empty())
            .unwrap_or_else(|| sibling_enterprise_endpoint(ingest_url, "upload-ticket"));
        let complete_url = std::env::var("MEMORA_ENTERPRISE_UPLOAD_COMPLETE_URL")
            .ok()
            .filter(|s| !s.trim().is_empty())
        ...
        if pinned_hosts.is_empty() {
            tracing::info!(
                "enterprise sync: no upload destination pin configured \
                 (MEMORA_ENTERPRISE_UPLOAD_PINNED_HOSTS unset) â€” tickets \
...
/// repurposes the env-var channel the (dropped) encryption root keys used â€”
/// customers already deploy MDM env vars to enterprise fleets.
fn pinned_hosts_from_env() -> Vec<String> {
    std::env::var("MEMORA_ENTERPRISE_UPLOAD_PINNED_HOSTS")
        .ok()
        .map(|raw| {
            raw.split(',')
...
/// because `DirectUploadCursors` now lives in the wire crate â€” the orphan
...
/// modes â€” the only difference on the wire is the `mode` string, which the
...
/// Glue between the MEMORA ingest wire format and
/// `MEMORA_sync::TicketedPipeline`. The pipeline does ticket â†’ PUT â†’
/// complete with backoff; this fn just builds the JSON shapes the
/// MEMORA control plane expects and maps errors back into the
...
mod tests {
    ...
    fn direct_cfg() -> DirectUploadConfig {
        DirectUploadConfig {
            ticket_url: "https://github.com/shrirampai3000/memora/api/enterprise/upload-ticket".to_string(),
            complete_url: "https://github.com/shrirampai3000/memora/api/enterprise/upload-complete".to_string(),
    ...
    fn sync_cfg() -> EnterpriseSyncConfig {
        EnterpriseSyncConfig {
            ...
            ingest_url: "https://github.com/shrirampai3000/memora/api/enterprise/ingest".to_string(),
    ...
    fn sibling_urls_are_derived_from_ingest_url() {
        assert_eq!(
            sibling_enterprise_endpoint(
                "https://github.com/shrirampai3000/memora/api/enterprise/ingest",
                "upload-ticket"
            ),
            "https://github.com/shrirampai3000/memora/api/enterprise/upload-ticket"
        );
        assert_eq!(
    ...
    fn write_only_batch_is_plaintext_jsonl_with_no_encryption_fields() {
        ...
        // all â€” absence, not null. A gateway or ingest validator keying off
    ...
    fn pinned_hosts_env_parsing_handles_lists_and_absence() {
        let prior = std::env::var("MEMORA_ENTERPRISE_UPLOAD_PINNED_HOSTS").ok();

        std::env::remove_var("MEMORA_ENTERPRISE_UPLOAD_PINNED_HOSTS");
        assert!(pinned_hosts_from_env().is_empty());

        std::env::set_var(
            "MEMORA_ENTERPRISE_UPLOAD_PINNED_HOSTS",
        ...
        match prior {
            Some(v) => std::env::set_var("MEMORA_ENTERPRISE_UPLOAD_PINNED_HOSTS", v),
            None => std::env::remove_var("MEMORA_ENTERPRISE_UPLOAD_PINNED_HOSTS"),
        }
    }
