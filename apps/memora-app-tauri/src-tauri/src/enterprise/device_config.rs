...
//! `~/.memora/enterprise.json` and starts sync â€” no app restart.
//!
//! What is deliberately NOT here: the MDM destination pin
//! (`MEMORA_ENTERPRISE_UPLOAD_PINNED_HOSTS`). It exists to catch a
...
/// first:
///   1. `MEMORA_ENTERPRISE_DEVICE_CONFIG_URL` (dev/staging override)
///   2. derived from the configured ingest URL (env or enterprise.json) â€”
...
pub fn device_config_url(file_ingest_url: Option<&str>) -> String {
    if let Ok(url) = std::env::var("MEMORA_ENTERPRISE_DEVICE_CONFIG_URL") {
        let url = url.trim();
        if !url.is_empty() {
    ...
    match control_plane_base(&ingest_url) {
        ...
        None => {
            // Malformed override â€” fall back to the baked control plane
...
/// as a short string for the caller to log at debug level â€” a signed-out
...
mod tests {
    ...
    fn device_config_url_layers() {
        ...
        let prior = std::env::var("MEMORA_ENTERPRISE_DEVICE_CONFIG_URL").ok();
        let prior_ingest = std::env::var("MEMORA_ENTERPRISE_INGEST_URL").ok();
        std::env::remove_var("MEMORA_ENTERPRISE_DEVICE_CONFIG_URL");
        std::env::remove_var("MEMORA_ENTERPRISE_INGEST_URL");

        // default = the baked control-plane base (prod unless the build
        // set NEXT_PUBLIC_MEMORA_WEB_URL â€” assert against the helper
        // so the test holds for dev builds too)
        let baked_default = crate::web_base::MEMORA_web_url("/api/enterprise/device-config");
        ...
        std::env::set_var(
            "MEMORA_ENTERPRISE_DEVICE_CONFIG_URL",
        ...
        match prior {
            Some(v) => std::env::set_var("MEMORA_ENTERPRISE_DEVICE_CONFIG_URL", v),
            None => std::env::remove_var("MEMORA_ENTERPRISE_DEVICE_CONFIG_URL"),
        }
        match prior_ingest {
