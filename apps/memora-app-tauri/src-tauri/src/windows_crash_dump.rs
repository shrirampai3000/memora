...
//! not honored for this path either (verified empirically â€” see PR #5568).
...
//! On the next launch this module moves those dumps into the MEMORA data
//! directory and prunes old ones. MEMORA never uploads them automatically.
...
const HELPER_DLL_NAME: &str = "MEMORA_wer_dump_helper.dll";
...
pub fn install() {
    match register_helper() {
        Ok(dll) => eprintln!(
            "MEMORA: fail-fast crash minidumps enabled via {}",
            dll.display()
        ),
        Err(reason) => {
            eprintln!("MEMORA: fail-fast crash minidumps unavailable: {reason}")
        }
    }
    ...
    let _ = std::thread::Builder::new()
        .name("MEMORA-wer-dump-harvest".to_string())
        .spawn(harvest_dumps);
}
...
/// Move dumps the helper DLL wrote next to the executable into the MEMORA
...
fn harvest_dumps() {
    ...
    let output_dir = memora_core::paths::default_MEMORA_data_dir().join("crash-dumps");

    let moved = move_dumps(&source_dir, &output_dir);
    if moved > 0 {
        eprintln!(
            "MEMORA: recovered {moved} crash minidump(s) into {}",
            output_dir.display()
        );
    }
