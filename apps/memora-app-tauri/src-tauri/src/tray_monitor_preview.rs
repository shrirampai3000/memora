...
/// ~2.5 stream creations/sec forever â€” observed in production as continuous
...
/// Call once at tray setup â€” polls SCK frame sequence for cached tray previews.
...
fn preview_capture_expected() -> bool {
    preview_capture_expected_from(
        memora_engine::sleep_monitor::screen_is_locked(),
        memora_engine::sleep_monitor::recently_woke_from_sleep(),
        memora_engine::drm_detector::drm_content_paused(),
        memora_engine::schedule_monitor::schedule_paused(),
    )
}
...
async fn poll_sck_frames(app: &AppHandle) {
    ...
    for monitor_id in active_monitor_ids() {
        ...
        if memora_screen::stream_invalidation::monitor_frame_seq(monitor_id).unwrap_or(0) == 0 {
...
fn refresh_monitor_from_sck(monitor_id: u32) -> PreviewUpdate {
    let Some(frame) = memora_screen::stream_invalidation::peek_monitor_frame(monitor_id) else {
...
async fn bootstrap_sck_stream(monitor_id: u32) {
    ...
    if memora_screen::stream_invalidation::peek_monitor_frame(monitor_id).is_some() {
    ...
    let Some(monitor) = memora_screen::monitor::get_monitor_by_id(monitor_id).await else {
    ...
    if memora_screen::stream_invalidation::ensure_monitor_stream(
        monitor_id,
        monitor.width(),
        monitor.height(),
        &[],
...
/// tray poller holding live streams against those displays â€” capture the user
