...
//!
//! Renders a recording â€” a meeting, or an explicit wall-clock range â€” to a
//! single MP4 (screen frames + synced audio) by calling the engine export core
//! in-process. This is the headless `MEMORA export` CLI's in-app twin:
...
//! than opening a second `DatabaseManager` â€” the export is read-only and the
...
async fn live_db(app: &tauri::AppHandle) -> Result<Arc<DatabaseManager>, String> {
    ...
    let core = guard
        .as_ref()
        .ok_or("recording isn't running yet â€” start MEMORA, then export")?;
    Ok(core.db.clone())
}
