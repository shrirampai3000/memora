...
//! result onto the shared `calendar_events` bus topic â€” the same topic
//! the native EventKit and ICS publishers feed. Meeting detection and the
//! prewarm logic in memora-events::meetings consume that single topic,
//! so adding this publisher is what makes the 2â€“3 min prewarm toast work
...
pub async fn start_google_calendar_publisher(app: AppHandle) {
    ...
    loop {
        if let Some((port, api_key)) = local_api_config(&app).await {
            match fetch_events(&client, port, api_key.as_deref()).await {
                ...
                Err(PublisherError::NotConnected) => {
                    // Not connected is a stable state â€” back off hard instead
                    // of re-asking every minute. poke() (fired on OAuth
                    // connect) wakes us immediately.
                    if interval != NOT_CONNECTED_POLL_INTERVAL {
                        debug!(
                            "google calendar publisher: not connected â€” backing off to {}s",
        ...
        tokio::select! {
            ...
            _ = recheck().notified() => {
                debug!("google calendar publisher: poked â€” rechecking now");
                interval = POLL_INTERVAL;
            }
        }
    }
...
async fn fetch_events(
    ...
    // 401 = OAuth not connected â€” silent skip.
