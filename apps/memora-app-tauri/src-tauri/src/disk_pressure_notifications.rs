...
use memora_events::{DiskSpaceLowEvent, DiskSpaceRecoveredEvent};
...
const STORAGE_SETTINGS_DEEPLINK: &str = "MEMORA://settings?section=storage";
...
impl Default for DiskPressureNotificationState {
    fn default() -> Self {
        Self::from_path(
            memora_core::paths::default_MEMORA_data_dir().join(NOTIFICATION_STATE_FILE),
        )
    }
}
...
impl DiskPressureNotificationState {
    ...
    async fn complete_notification(&self) -> Result<(), String> {
        ...
        let write_result = memora_core::memories::external_sync::write_atomic_full(
            &self.path, &raw,
        )
    ...
    /// below rewind the latch â€” production code clears it through `recover`.
    ...
    pub(crate) async fn recover(&self, event: &DiskSpaceRecoveredEvent) -> Result<bool, String> {
        if event.available_bytes < memora_events::LOW_DISK_RECOVERY_THRESHOLD_BYTES {
            return Ok(false);
        }
...
pub fn start(app: AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        let mut events = memora_events::subscribe_to_event::<DiskSpaceLowEvent>(
    ...
    tauri::async_runtime::spawn(async move {
        let mut events = memora_events::subscribe_to_event::<DiskSpaceRecoveredEvent>(
...
pub(crate) async fn handle(app: &AppHandle, event: DiskSpaceLowEvent) -> DiskPressureOutcome {
    ...
    let delivery = client::send_typed_with_actions_and_priority_confirmed(
        "recording stopped â€” disk almost full",
        low_disk_body(
...
fn low_disk_body(
    ...
    format!(
        "MEMORA stopped capture because only {available} is free (safety threshold: \
...
mod tests {
    ...
    async fn recovery_requires_hysteresis_and_the_same_data_volume() {
        ...
        let too_soon = DiskSpaceRecoveredEvent::new(
            memora_events::LOW_DISK_RECOVERY_THRESHOLD_BYTES - 1,
            "test-data".to_string(),
        );
        ...
        let other_volume = DiskSpaceRecoveredEvent::new(
            memora_events::LOW_DISK_RECOVERY_THRESHOLD_BYTES,
