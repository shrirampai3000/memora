...
//! recording stayed off â€” hours, in the 2026-08-11 case, because nothing resumes
...
pub enum SelfHealSkip {
    /// No durable marker â€” the normal path.
    NotQuarantined,
...
pub async fn try_resolve_quarantine(database_path: &Path) -> Result<(), SelfHealSkip> {
    if !memora_db::sqlite_quarantine_exists(database_path) {
        return Err(SelfHealSkip::NotQuarantined);
    }
    if memora_db::sqlite_hard_fault_latched(database_path) {
        return Err(SelfHealSkip::AlreadyLatchedThisProcess);
    }

    let marker = match memora_db::read_sqlite_quarantine(database_path) {
        ...
        Err(error) => {
            warn!(
                ...
                "db self-heal: quarantine marker is unreadable â€” staying fail-closed"
            );
    ...
    if !memora_db::sqlite_quarantine_is_self_healable(code) {
        info!(
            sqlite_extended_code = code,
            sqlite_primary_code = code & 0xff,
            database = %database_path.display(),
            "db self-heal: hard fault needs a verified replacement generation â€” not probing"
        );
    ...
    let probe = match memora_db::probe_quarantined_generation_health(database_path).await {
        Ok(probe) => probe,
        Err(error) => {
            warn!(
                error = %error,
                ...
                database = %database_path.display(),
                "db self-heal: quarantined generation failed its health check â€” recovery is required"
            );
    ...
    match memora_db::resolve_verified_sqlite_quarantine(database_path, &archive) {
        Ok(Some(archived)) => {
            info!(
                sqlite_extended_code = code,
                ...
                database = %database_path.display(),
                "db self-heal: generation verified healthy â€” quarantine resolved without a rebuild"
            );
            Ok(())
        }
...
pub async fn try_self_heal_at_launch(app: AppHandle, database_path: PathBuf) -> bool {
    ...
    if let Err(error) = crate::recording::spawn_MEMORA(app.state(), app.clone(), None).await {
        ...
        warn!(
            error = %error,
            "db self-heal: quarantine cleared but the first start failed â€” leaving it to the watchdog"
        );
        return false;
    }
    ...
    client::send_typed_with_priority(
        "recording resumed",
        "MEMORA hit a temporary database error. your existing database passed a health check, so recording resumed without a rebuild.",
        "db_recovery",
        Some(10_000),
...
mod tests {
    ...
    fn archive_path_is_unique_per_fault() {
        let db = Path::new("/tmp/MEMORA/db.sqlite");
    ...
    async fn launch_path_refuses_every_unproven_io_error_code() {
        ...
        for (recorded, expected) in [
            ...
            memora_db::persist_sqlite_quarantine(&db, recorded, "disk I/O error")
            ...
            assert!(memora_db::sqlite_quarantine_exists(&db));
        }
    }
}
