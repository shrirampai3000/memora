...
//!   teardown (2026-07-01 report on MEMORA 2.5.82)
//!
...
/// Latched once quit teardown starts â€” ignores duplicate Quit clicks.
pub static QUIT_TEARDOWN_STARTED: AtomicBool = AtomicBool::new(false);

/// Set when [`tauri::RESTART_EXIT_CODE`] is seen â€” `RunEvent::Exit` must spawn
        ...
        tauri::async_runtime::block_on(async move {
            ...
            .await
            {
                ...
                TeardownOutcome::TimedOut => warn!(
                    "Pre-exit teardown exceeded {}s â€” force-exiting anyway",
...
pub fn force_app_relaunch(app: AppHandle, status: i32) -> ! {
    ...
    // away â€” installing any earlier orphans the running bundle and kills TCC
...
/// â€” a confirmation dialog there would invisibly block the session from
...
/// Intercept native app termination â€” dock right-click â†’ Quit, AppleScript
/// `quit`, `osascript` â€” the same way ghostty does: an
...
/// flight, or the OS session is ending (shutdown/restart/logout â€” blocking
...
///
/// Swizzled via `class_addMethod` on the existing tao delegate â€” same
...
pub fn setup_terminate_interceptor(app_handle: AppHandle) {
    ...
    extern "C" fn should_terminate(_this: &Object, _sel: Sel, _sender: id) -> usize {
        // Runs inside the ObjCâ†’Rust trampoline (nounwind) â€” a panic here
        ...
        std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            if os_session_is_ending() {
                info!("terminate: OS session ending â€” allowing exit without confirmation");
                QUIT_REQUESTED.store(true, Ordering::SeqCst);
                return 1;
            }
            ...
            info!("terminate: intercepted â€” routing through quit confirmation");
    ...
    unsafe {
        ...
        let Some(mut decl) =
            objc::declare::ClassDecl::new("MEMORATerminateDelegate", superclass)
        else {
...
/// True when any MEMORA window is showing. Decides whether the quit
/// dialog offers "Minimize to Tray" â€” pointless when everything is already
...
/// Hide every window â€” the "Minimize to Tray" choice in the quit dialog.
...
///
/// Modeled on Ghostty's quit flow (`applicationShouldTerminate` â†’
...
///
/// Offers Quit / Minimize to Tray / Cancel â€” this is a tray app, so backing
/// out into the tray is a first-class choice.
///
/// Only user-initiated quit paths (app menu Cmd+Q, tray Quit, dock Quit via
/// `ExitRequested`) go through here â€” programmatic paths (updater restart,
...
pub fn confirm_and_request_app_quit(app: AppHandle) {
    ...
    let recording_active = app
        ...
        .unwrap_or(false);
    if !recording_active {
        info!("Quit requested with no active recording â€” skipping confirmation");
...
fn show_quit_alert(app: &AppHandle, show_minimize: bool, message: &str) {
    ...
    const QUIT_BUTTON: &str = "Quit MEMORA";
    ...
    unsafe {
        ...
        // NSAlertStyleInformational â€” app icon, never a caution-triangle badge.
        let _: () = msg_send![alert, setAlertStyle: 1i64];

        let title = NSString::alloc(nil).init_str("Quit MEMORA?");
...
pub fn request_app_quit(app: AppHandle) {
    ...
    if QUIT_TEARDOWN_STARTED.swap(true, Ordering::SeqCst) {
        tracing::debug!("Quit ignored â€” teardown already in progress");
        return;
    }

    tauri::async_runtime::spawn(async move {
        info!("Quit requested â€” running pre-exit teardown");
        match bounded_teardown(PRE_EXIT_TEARDOWN_TIMEOUT, async {
            run_pre_exit_teardown(&app).await;
            Ok(())
        })
        .await
        {
            ...
            TeardownOutcome::TimedOut => warn!(
                "Quit teardown exceeded {}s â€” force-exiting",
                PRE_EXIT_TEARDOWN_TIMEOUT.as_secs()
            ),
        }
        ...
        let plist = r#"
            <plist version="1.0">
              <dict>
                ...
                <string>MEMORA</string>
                <key>CFBundleExecutable</key>
                <string>MEMORA-app</string>
              </dict>
            </plist>
        "#;
