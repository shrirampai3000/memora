...
/// the overall budget â€” the first attempt may use most of it.
...
pub async fn upload_file_to_s3(file_path: &str, signed_url: &str) -> Result<bool, String> {
    ...
    // command forever â€” the stuck "sending..." state in #5360. Requests get
    ...
    while attempt < max_retries {
            ...
            .await
        {
            Ok(response) => {
                ...
                // Surface the response body â€” S3/Supabase wraps the reason for
...
mod tests {
    ...
    fn attempt_timeout_scales_with_file_size() {
        // 25MB at the 125KB/s floor rate needs 200s â€” a flat 90s timeout
    ...
    async fn uploads_file_bytes_with_put() {
        ...
        let contents = b"MEMORA-media-upload";
