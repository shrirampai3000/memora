...
struct TrayMenuData {
    ...
    /// Both audio and vision are disabled in settings â€” nothing can record.
...
fn prefetch_tray_menu_data(app: &AppHandle) -> TrayMenuData {
    ...
    let mut show_shortcut = if settings.show_MEMORA_shortcut.trim().is_empty() {
        default_show.to_string()
    } else {
        settings.show_MEMORA_shortcut.clone()
    };
    ...
    if settings
        .disabled_shortcuts
        .iter()
        .any(|shortcut| shortcut == "showMEMORAShortcut")
    {
        show_shortcut.clear();
    }
    if settings
...
/// The pricing page renames the tiers: standardâ†’"Basic", proâ†’"Business",
/// enterpriseâ†’"Enterprise". Keep in sync with `planDisplayName` in
...
/// org (e.g. an admin or member of an active license) â€” which still gets
/// Business-equivalent features here â€” is shown as "Business" rather than
...
/// Optimistic recording status override â€” set on start/stop click for instant UI feedback.
...
/// Pending "pause for X minutes" timer. Held so a manual resume â€” or a fresh
/// pause click â€” can abort the previous one and prevent a stale auto-resume
...
pub(crate) fn force_tray_rebuild(app: &AppHandle) -> Result<()> {
    ...
    if let Some(tray) = app.tray_by_id("MEMORA_main") {
...
fn get_effective_recording_status() -> RecordingStatus {
    ...
    if let Some((status, expiry)) = opt.as_ref() {
        if std::time::Instant::now() < *expiry {
            // Don't mask a failed start â€” optimistic "Starting" is only useful
    ...
    drop(opt);
    // Engine-side full-pause overrides Recording â€” the capture loop sleeps
    ...
    // tray honest. Don't override Stopped/Starting/Error â€” those reflect
...
///    items while AppKit still references them â€” a click then dereferences freed
...
///    queue in `kCFRunLoopCommonModes` â€” which *includes* the menu's
///    `NSEventTrackingRunLoopMode` â€” so a `set_menu` triggered from the
...
/// installs the queued menu when the run loop is idle â€” i.e. when no menu is
...
/// Installs the queued tray menu while no menu is open â€” the only flash-free,
...
/// menu is closed â€” never overlapping a presented menu.
#[cfg(target_os = "macos")]
...
struct MenuState {
    ...
    cloud_subscribed: bool,
    /// Plan id (Free/Basic/Business/â€¦) so plan-label changes also rebuild the menu
    subscription_plan: Option<String>,
...
pub fn setup_tray(app: &AppHandle, update_item: Option<&tauri::menu::MenuItem<Wry>>) -> Result<()> {
    ...
    if let Some(main_tray) = app.tray_by_id("MEMORA_main") {
        ...
        // poller or the startup "Startingâ€¦" menu is never rebuilt.
...
pub fn log_tray_position(app: &AppHandle) {
    if let Some(tray) = app.tray_by_id("MEMORA_main") {
        ...
        error!("tray icon 'MEMORA_main' not found");
    }
}
...
pub fn recreate_tray(app: &AppHandle) {
    ...
    // across the FFI boundary (nounwind â†’ abort). catch_unwind prevents this.
    let _ = app.run_on_main_thread(move || {
        if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            crate::window::with_autorelease_pool(|| {
                ...
                let _old = app.remove_tray_by_id("MEMORA_main");
                // Drop the old tray icon explicitly on main thread
                drop(_old);
                debug!("recreate_tray: old tray removed, building new one");

                // Create a new tray icon â€” macOS assigns it the rightmost position
                let icon = match app.path().resolve(
                    ...
                    Err(_) => {
                        tauri::image::Image::from_path("assets/MEMORA-logo-tray-white.png").ok()
                    }
                };

                let mut builder = TrayIconBuilder::<Wry>::with_id("MEMORA_main")
...
/// Uses Tauri's `with_inner_tray_icon` â†’ `ns_status_item()` for direct access.
...
fn recording_status_text(
    ...
    match (status, all_capture_disabled, audio_capture_status) {
        (RecordingStatus::Recording, true, _) => "â—‹ Stopped",
        (RecordingStatus::Recording, false, Some(AudioCaptureStatus::WaitingForMeeting)) => {
            "â— Screen recording Â· audio waiting for meeting"
        }
        (
            RecordingStatus::Recording,
            false,
            Some(AudioCaptureStatus::MeetingDetectorUnavailable),
        ) => "â— Screen recording Â· meeting detection unavailable",
        (RecordingStatus::Starting, _, _) => "â—‹ Startingâ€¦",
        (RecordingStatus::Recording, _, _) => "â— Recording",
        (RecordingStatus::Paused, _, _) => "â— Paused",
        (RecordingStatus::ScheduledPause, _, _) => "â—‹ Outside work hours",
        (RecordingStatus::Stopped, _, _) => "â—‹ Stopped",
        (RecordingStatus::Error, _, _) => "â—‹ Error",
    }
}
...
fn create_dynamic_menu(
    ...
    if !data.onboarding_completed && !data.app_ui_hidden {
        menu_builder = menu_builder
            ...
            .item(&PredefinedMenuItem::separator(app)?)
            .item(&MenuItemBuilder::with_id("quit", "Quit MEMORA").build(app)?);
    ...
    if !data.app_ui_hidden {
        menu_builder = menu_builder
            .item(&MenuItemBuilder::with_id("open_app", "Open MEMORA").build(app)?)
            .item(&PredefinedMenuItem::separator(app)?);
    }
    ...
    if !all_capture_disabled {
        ...
        for device in monitors {
            let label = format!("  â–£ {}", device.name);
            if let Some(monitor_id) = device.monitor_id {
                ...
                let dot = if device.active { "â—" } else { "â—‹" };
                let fallback_label = format!("  {} â–£ {}", dot, device.name);
                menu_builder = menu_builder.item(
    ...
    if effective_status == RecordingStatus::Error && data.has_permission_issue {
        menu_builder = menu_builder
            .item(&MenuItemBuilder::with_id("fix_permissions", "âš  Fix permissions").build(app)?);
    }
    ...
    if !data.app_ui_hidden && !is_tray_item_hidden("tray_plan") {
        ...
        if !has_cloud && !plan_includes_business(data.subscription_plan.as_deref()) {
            menu_builder = menu_builder
                .item(&MenuItemBuilder::with_id("upgrade", "âš¡ Upgrade to Business").build(app)?);
        }
    }
    ...
    let version_text = match (is_beta, is_enterprise) {
        (_, true) => format!("MEMORA v{} (Enterprise)", app.package_info().version),
        (true, false) => format!("MEMORA v{} (Beta)", app.package_info().version),
        (false, false) => format!("MEMORA v{}", app.package_info().version),
    };
    menu_builder = menu_builder.item(
    ...
    if !is_tray_item_hidden("tray_recording_controls") {
        ...
        let label = if all_capture_disabled {
            "Stopped â€” no devices enabled"
        } else {
            match effective_status {
                RecordingStatus::Recording => "Recording",
                RecordingStatus::Paused => "Paused â€” click to resume",
                RecordingStatus::ScheduledPause => "Outside work hours â€” paused by schedule",
                RecordingStatus::Starting => "Startingâ€¦",
                RecordingStatus::Error => "Error â€” click to retry",
                _ => "Stopped â€” click to record",
            }
        };
        ...
        // "Pause forâ€¦" submenu â€” only meaningful while currently recording.
        ...
        if is_recording {
            let pause_submenu = SubmenuBuilder::new(app, "Pause forâ€¦")
        ...
        // No indefinite mode â€” every session has a natural end (meeting end
        // or timer expiry). Hits /capture/hd/{start,stop} so changes take
        // effect on the next capture tick.
        // Hidden when no devices are enabled â€” nothing to record in HD.
        let hd = get_high_fps_status();
        if hd.active {
    ...
    // TODO: vault lock tray item disabled â€” CLI-only for now
    // menu_builder = menu_builder
    //     .item(&PredefinedMenuItem::separator(app)?)
    //     .item(
    ...
    menu_builder = menu_builder.item(
        &MenuItemBuilder::with_id("quit", "Quit MEMORA")
...
fn setup_tray_click_handlers(main_tray: &TrayIcon) -> Result<()> {
    main_tray.on_menu_event(move |app_handle, event| {
        ...
        // only clones and schedules work via run_on_main_thread â€” no heavy work here.
        if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
    ...
    // and AppKit would dismiss the just-opened menu â€” the "first click flashes
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        main_tray.on_tray_icon_event(|tray, event| {
            ...
            // here causes re-entrancy â€” tao panics at runner.rs:245 with:
            ...
            if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                ...
                } = event
                {
                    ...
                    // âš ï¸  Do NOT call run_on_main_thread() directly here â€” that would
...
/// do any heavy or panicking work here â€” defer all window/show/open work to
...
fn handle_menu_event(app_handle: &AppHandle, event: tauri::menu::MenuEvent) {
    ...
    match event.id().as_ref() {
        ...
        "start_recording" | "stop_recording" | "toggle_recording" => {
            // Manual toggle cancels any pending auto-resume â€” otherwise a user
        ...
        id if id.starts_with("pause_") => {
            ...
            // Schedule auto-resume â€” also fires a notification so the user knows
            ...
            let handle = tauri::async_runtime::spawn(async move {
                ...
                send_notify("Recording resumed", "MEMORA is recording again.");
            });
            *PAUSE_TIMER.lock().unwrap_or_else(|e| e.into_inner()) = Some(PauseTimer {
            ...
            send_notify(
                "Recording paused",
                format!("MEMORA will auto-resume in {}.", pretty),
            );
        ...
        "stop_hd_recording" => {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                if let Err(e) = api
                    ...
                    .await
                {
                    error!("stop_hd POST failed: {} â€” rolling back optimistic cache", e);
        ...
        "extend_hd_30" => {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                {
                    error!(
                        "extend_hd POST failed: {} â€” rolling back optimistic cache",
                        e
                    );
                    set_high_fps_status(cached);
                }
        ...
        id if id.starts_with("hd_timer_") => {
            ...
            tauri::async_runtime::spawn(async move {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        error!(
                            "hd_timer POST failed: {} â€” rolling back optimistic cache",
                            e
                        );
                        set_high_fps_status(cached);
                    }
        ...
        id if id.starts_with("toggle_audio_device_") => {
            ...
            // Just fire the API call â€” the health poll (every 1s) will sync state.
            let api = local_api_context_from_app(&app_handle);
        ...
        "releases" => {
            ...
            let _ = app_handle.run_on_main_thread(move || {
                let _ = app
                    .opener()
                    .open_url("https://github.com/shrirampai3000/memora/changelog", None::<&str>);
            });
        }
        "update_now" => {
            let app = app_handle.clone();
            let _ = app_handle.run_on_main_thread(move || {
                ...
                if is_source_build(&app) {
                    ...
                    // The whole flow â€” including surfacing deferred/failed
                    // outcomes, which the old inline handler silently
                    // discarded â€” lives in updates::trigger_update_now so the
        ...
        "book_call" => {
            ...
            let _ = app_handle.run_on_main_thread(move || {
                let _ = app
                    .opener()
                    .open_url("https://cal.com/team/MEMORA/chat", None::<&str>);
            });
        }
        "skip_onboarding" => {
            let app = app_handle.clone();
...
async fn update_menu_if_needed(
    ...
    // Tooltip refreshes every tick regardless of menu rebuild â€” countdown
    ...
    let tooltip: String = if has_perm_issue {
        "MEMORA â€” âš ï¸ permissions needed".to_string()
    ...
    {
        "MEMORA â€” screen recording; meeting detection unavailable".to_string()
    } else if effective_status == RecordingStatus::Recording
        && audio_capture_status == Some(AudioCaptureStatus::WaitingForMeeting)
    {
        "MEMORA â€” screen recording; audio waiting for meeting".to_string()
    } else if effective_status == RecordingStatus::Paused {
        match pause_remaining() {
            Some(d) => format!("MEMORA â€” paused, resumes in {}", format_remaining(d)),
            None => "MEMORA â€” paused".to_string(),
        }
    } else if effective_status == RecordingStatus::ScheduledPause {
        "MEMORA â€” outside work hours (paused by schedule)".to_string()
    } else {
        "MEMORA".to_string()
    };
    let app_for_tooltip = app.clone();
    let _ = app.run_on_main_thread(move || {
        if let Some(tray) = app_for_tooltip.tray_by_id("MEMORA_main") {
    ...
    if should_update {
        ...
        #[cfg(not(target_os = "macos"))]
        {
            ...
            if let Err(e) = app.run_on_main_thread(move || {
                if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    let Some(tray) = app_for_thread.tray_by_id("MEMORA_main") else {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            if let Err(e) = update_menu_if_needed(&app, update_item.as_ref()).await {
                ...
                // Windows â€” invalidate the cached store so the next tick rebuilds it.
