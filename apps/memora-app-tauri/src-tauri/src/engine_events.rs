...
//! `engine_events::audio_device`, â€¦) so the file named for one concern
...
//! `audio_device_fallback_*` arm â€” the rename + split makes the seam
//! explicit.
...
async fn run(app: AppHandle, server_port: u16, api_key: Option<String>) {
    ...
    loop {
        match connect(&app, server_port, api_key.as_deref()).await {
            Ok(()) => {
                // Connection closed cleanly â€” reset backoff and reconnect
...
async fn connect(app: &AppHandle, port: u16, api_key: Option<&str>) -> Result<(), String> {
    ...
    // permissions, audio devices, â€¦). We dispatch by event name client-side.
...
///
/// Frame shape: `{ "name": "<event-name>", "data": { â€¦ } }`. Unknown event
/// names are silently ignored â€” many event families are emitted by the engine
/// (workflows, meetings, â€¦) that this app doesn't currently consume.
