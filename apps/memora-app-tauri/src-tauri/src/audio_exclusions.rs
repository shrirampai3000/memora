...
//! (`MEMORA_audio::core::process_tap`) and read from a JSON file on
//! every tap rebuild. These commands are a thin file-IO bridge for the
//! Settings UI panel â€” the engine never goes through them, so the file
...
//! ```
//!
//! The path defaults to `$HOME/.memora/audio-exclusions.json` and can
//! be overridden with `MEMORA_AUDIO_EXCLUSIONS_PATH` â€” both the engine
...
const ENV_OVERRIDE: &str = "MEMORA_AUDIO_EXCLUSIONS_PATH";
const DEFAULT_RELATIVE_PATH: &str = ".memora/audio-exclusions.json";
...
mod tests {
    ...
    fn read_returns_empty_when_missing() {
        with_env(
            "/nonexistent/MEMORA-audio-exclusion-cmd-test.json",
            || {
