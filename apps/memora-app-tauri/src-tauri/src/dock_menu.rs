...
//! macOS dock right-click menu â€” fallback for when the tray icon is hidden
...
pub fn setup_dock_menu(app_handle: AppHandle) {
    ...
    unsafe {
        ...
        // All run inside the ObjCâ†’Rust trampoline (nounwind) â€” wrap in
        // catch_unwind so a panic logs instead of aborting the app.
        extern "C" fn show_MEMORA(_this: &Object, _sel: Sel, _sender: id) {
        ...
        extern "C" fn dock_menu(_this: &Object, _sel: Sel, _sender: id) -> id {
            unsafe {
                ...
                // "Show MEMORA"
                let title = NSString::alloc(nil).init_str("Show MEMORA");
                let action = sel!(showMEMORA:);
        ...
        let mut decl =
            objc::declare::ClassDecl::new("MEMORADockMenuDelegate", superclass).unwrap();
        decl.add_method(
            sel!(showMEMORA:),
            show_MEMORA as extern "C" fn(&Object, Sel, id),
        );
        decl.add_method(
            sel!(openSettings:),
        ...
        if !method.is_null() {
            ...
            for sel_name in &[sel!(showMEMORA:), sel!(openSettings:)] {
