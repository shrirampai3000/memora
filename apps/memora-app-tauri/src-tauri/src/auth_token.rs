...
//!
//! The Clerk session JWT used as the Bearer for MEMORA cloud used to live
//! in **plaintext** in two on-disk files â€” `store.bin` (`settings.user.token`)
//! and `auth.json` â€” plus the recovery snapshot `store.bin.last-good`. The
...
//! is preserved by hydrating from here, so cloud-auth readers are unchanged â€”
...
//!
//! ## Cross-platform
//! Paths come from `default_MEMORA_data_dir()` and the SecretStore is
...
/// encryption or this platform has no keychain â€” the token is then stored
...
/// callers treat it as a persist failure â€” the existing copy stays where it
...
// â”€â”€ Public API (production: default data dir + resolved keychain key) â”€â”€â”€â”€â”€â”€â”€â”€
...
pub async fn store_cloud_token(token: Option<&str>) -> anyhow::Result<()> {
    ...
    match token {
        ...
        None => {
            ...
            // fallbacks `get_cloud_token` reads must go too â€” a stale copy
...
/// Idempotent and best-effort â€” safe to call on every startup.
pub async fn migrate_plaintext_token(data_dir: &Path) -> Option<String> {
    let read_key = memora_secrets::keychain::get_key_for_read();
    match write_encryption_key() {
        ...
        Err(e) => {
            ...
            // the plaintext files â€” resolve a token for this session only and
...
async fn migrate_at(
    ...
    // Resolve the token, in priority order: SecretStore (already migrated) â†’
    // store.bin â†’ auth.json. Only open SQLite when a legacy capture database or
    ...
    // is GATED on this succeeding â€” never drop the last plaintext copy.
    ...
    // to protect). auth.json is intentionally left â€” it's a pi-agent subprocess
...
// â”€â”€ Pure helpers (no IO / no async â€” exhaustively unit-tested) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/// API key (`sp-<uuid8>` â€” see the engine's `auth_key.rs`
...
/// priority order: `store.bin` `settings.user.token` â†’ `store.bin`
/// `settings.userId` (the CLI's RecordingSettings mirror, JWT-shaped only)
/// â†’ `auth.json`. Every source the scrub touches must be represented here so
...
/// Extract `settings.userId` from raw `store.bin` JSON when it holds a JWT â€”
...
pub fn redact_token_in_store_json(data: &[u8]) -> Option<Vec<u8>> {
    ...
    // token â†’ user_id). Null it only when JWT-shaped: it can also hold a
...
fn scrub_store_bin_token(path: &Path) {
    ...
    // Encrypted store file (magic header) â€” leave it; the decrypt path owns it.
...
/// from (`get_cloud_token`'s auth.json fallback, pi's provider config) â€”
/// otherwise the session resurrects at the next settings hydration:
/// - `<data_dir>/auth.json` â€” the CLI credential file; the engine's own
///   `auth logout` removes it outright (auth_key.rs), so match that.
/// - `~/.pi/agent/auth.json` â€” drop only the `MEMORA` provider entry,
///   other providers' credentials stay.
/// - `~/.pi/agent/models.json` â€” reset the MEMORA provider's `apiKey`
///   back to the `MEMORA_API_KEY` placeholder (`build_models_json` embeds
...
/// Remove the `MEMORA` entry from pi's `auth.json`, preserving every other
...
fn scrub_pi_auth_json(path: &Path) {
    rewrite_json_file(path, |json| {
        json.as_object_mut()?.remove("MEMORA").map(|_| ())
    });
}

/// Reset pi's `models.json` MEMORA provider `apiKey` to the signed-out
/// `MEMORA_API_KEY` placeholder, preserving the rest of the provider map.
fn scrub_pi_models_json(path: &Path) {
    rewrite_json_file(path, |json| {
        let api_key = json.pointer_mut("/providers/MEMORA/apiKey")?;
        if api_key.as_str() == Some("MEMORA_API_KEY") {
            return None;
        }
        *api_key = serde_json::Value::String("MEMORA_API_KEY".to_string());
        Some(())
    });
}
...
mod tests {
    ...
    fn missing_or_empty_token_yields_none() {
        ...
        // auth.json historically held the LOCAL api key â€” never treat a
    ...
    fn redacts_jwt_shaped_user_id_but_keeps_plain_ids() {
        ...
        // A plain (non-JWT) user id is not a secret â€” left alone.
    ...
    fn scrub_skips_encrypted_and_missing_files() {
        ...
        scrub_store_bin_token(&dir.join("nope.bin")); // missing â†’ no panic
    ...
    #[test]
    fn signout_scrub_removes_MEMORA_from_pi_auth() {
        let dir = unique_dir("pi_auth");
        let p = dir.join("auth.json");
        std::fs::write(&p, br#"{"MEMORA":"jwt-x","anthropic":"sk-other"}"#).unwrap();
        scrub_pi_auth_json(&p);
        let json: serde_json::Value = serde_json::from_slice(&std::fs::read(&p).unwrap()).unwrap();
        assert!(json.get("MEMORA").is_none(), "MEMORA entry removed");
    ...
    fn signout_scrub_pi_auth_noop_cases() {
        ...
        scrub_pi_auth_json(&dir.join("missing.json")); // missing â†’ no panic
        let p = dir.join("auth.json");
    ...
    fn signout_scrub_resets_pi_models_api_key() {
        ...
        std::fs::write(
            &p,
            br#"{"providers":{"MEMORA":{"apiKey":"jwt-x","baseUrl":"u"},"ollama":{"apiKey":"none"}}}"#,
        )
        .unwrap();
        scrub_pi_models_json(&p);
        let json: serde_json::Value = serde_json::from_slice(&std::fs::read(&p).unwrap()).unwrap();
        assert_eq!(
            json.pointer("/providers/MEMORA/apiKey")
                .unwrap()
                .as_str(),
            Some("MEMORA_API_KEY")
        );
        assert_eq!(
            json.pointer("/providers/MEMORA/baseUrl")
                .unwrap()
                .as_str(),
            Some("u"),
    ...
    // NOTE: `scrub_signout_fallbacks` itself is deliberately untested â€” it
    ...
    // global `static`. It is deliberately NOT asserted here â€” `cargo test` runs
    ...
    // â”€â”€ end-to-end against a real SecretStore (temp db.sqlite) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async fn store_load_roundtrip_encrypted() {
        ...
        let raw = std::fs::read(memora_secrets::secrets_database_path(&dir)).unwrap();
        assert!(
    ...
    async fn migrate_already_in_secret_store_still_scrubs_plaintext() {
        ...
        let got = migrate_at(&dir, None, None).await;
        // SecretStore wins â€” the stale plaintext does NOT overwrite it.
        assert_eq!(got, Some(JWT.to_string()));
        assert_eq!(read_back(&dir, None).await, Some(JWT.to_string()));
    ...
    async fn migrate_no_token_anywhere_is_noop() {
        ...
        // No store.bin, no auth.json, empty db â†’ resolves to nothing, no panic.
