...
/// (frontend polls `local_api_context_from_app`). Resolving the key â€”
/// which requires async I/O against `db.sqlite` â€” happens once per
/// recording start via `memora_engine::auth_key::resolve_api_auth_key`,
...
/// within the same process â€” OnceLock would silently ignore the second
...
// it can't parse â€” still-encrypted ciphertext after a keychain failure, a
// torn write â€” comes back as a *successfully built EMPTY store*. init_store
...
//       the snapshot has aiPresets â€” never freeze a degraded state). The
...
//       An encrypted-at-rest file counts as healthy â€” ciphertext is user
//       data, not an empty store.
//   L4: never write over the encrypted file on key failures â€” back it up and
//       leave it in place.
//   L5: after the plugin builds, if the disk file has a `settings` key but
//       the loaded store doesn't, the load silently failed â€” refuse to hand
...
/// "healthy" and replaces `.last-good` on its first save â€” the user's real
/// state survives here.
...
/// Used as the "is this a real user state" signal â€” empty presets means the
...
/// zeroes the target first and never flushes, so a power loss mid-save â€” or
/// even seconds after a "successful" save, before the OS flushes dirty pages â€”
...
/// therefore always either the previous complete file or the new complete one â€”
...
pub(crate) fn durable_write(path: &Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    // set_overlay_anchor and set_overlay_display saved 103Âµs apart: the
    ...
    // target â€” the torn file this helper exists to prevent.
    //
...

    {
        ...
        #[cfg(unix)]
        {
            ...
            // store.bin and its snapshots hold API keys â€” owner-only from
...
/// L1 â€” copy `store.bin` â†’ `store.bin.last-good` if the current file parses
...
fn restore_snapshot_over(store_path: &Path, why: &str) -> bool {
    ...
    if store_path.exists() {
        if let Err(e) = std::fs::copy(store_path, &pre_restore) {
            tracing::warn!(
                "settings recovery: failed to back up {} to {}: {} â€” aborting restore",
                store_path.display(),
                pre_restore.display(),
                e
            );
    ...
    tracing::warn!(
        "settings recovery: {} â€” restored {} from {}; {}",
        why,
        store_path.display(),
        src.display(),
        pre_restore_note
...
/// L2 â€” if `store.bin` is degraded (parses but missing aiPresets) or missing
...
pub fn auto_restore_if_wiped(store_path: &Path) -> bool {
    ...
    let cur = match std::fs::read(store_path) {
        ...
        // I/O) are not evidence of a wipe â€” leave the file alone.
...
#[derive(PartialEq)]
enum DecryptOutcome {
    /// File is plain JSON, empty, or missing â€” nothing to decrypt.
    PlainOrMissing,
    ...
    /// the user's data â€” the caller must run snapshot recovery (L2b).
    Locked,
}
...
fn decrypt_store_file(path: &Path) -> DecryptOutcome {
    ...
    // answer to a prompt â€” don't re-prompt.
    ...
    let key = match key_result {
        ...
        secrets::KeyResult::AccessDenied => {
            // L4 â€” DO NOT wipe. Keep a ciphertext backup; the caller restores
            ...
            tracing::error!(
                "store.bin is encrypted but keychain access was denied â€” \
        ...
        secrets::KeyResult::NotFound | secrets::KeyResult::Unavailable => {
            // L4 â€” DO NOT wipe. Previously this branch wrote `b"{}"` over
            ...
            let _ = std::fs::copy(path, &backup);
            tracing::error!(
                "store.bin is encrypted but keychain key not found â€” \
    ...
    match memora_vault::crypto::decrypt_small(&data[8..], &key) {
        Ok(plaintext) => {
...
///
/// DISABLED BY DEFAULT â€” the macOS keychain doesn't reliably persist keys across
...
///
/// To opt in: create ~/.memora/.encrypt-store or set MEMORA_ENCRYPT_STORE=1.
fn encrypt_store_file(path: &Path) {
    // Check opt-in flag
    let opted_in = std::env::var("MEMORA_ENCRYPT_STORE")
        .map(|v| v == "1")
        .unwrap_or(false)
        || path
            .parent()
    ...
    let key = match keychain::get_key() {
        ...
        keychain::KeyResult::AccessDenied => {
            // Keychain access denied â€” disable encryption
            ...
            if let Some(parent) = path.parent() {
                ...
                if flag.exists() {
                    ...
                    tracing::warn!(
                        "store encryption disabled â€” keychain access denied. \
        ...
        keychain::KeyResult::NotFound | keychain::KeyResult::Unavailable => {
            // Key doesn't exist or keychain unavailable â€” can't encrypt
            return;
        }
    };
    match memora_vault::crypto::encrypt_small(&data, &key) {
        Ok(ciphertext) => {
...
pub fn reencrypt_store_file(app: &AppHandle) {
    if let Ok(base_dir) = get_base_dir(app, None) {
        ...
        // L1 â€” snapshot the current state to .last-good IFF it's healthy
...
/// Cached store instance â€” reusable across the process lifetime.
...
/// â€” the registry the L5 guard must clean up lives in tauri-managed state,
...
fn build_store_at<R: tauri::Runtime>(
    ...
    if store_path.exists() && decrypt_store_file(&store_path) == DecryptOutcome::Locked {
        // L2b â€” the encrypted blob is unreadable (key denied/missing or
    ...
    // L2 â€” if the file is degraded (parses but has no aiPresets) or missing
    ...
    // L5 precondition â€” note whether the disk file holds a parseable
    ...
    for attempt in 0..3u32 {
        match StoreBuilder::new(app, store_path.clone()).build() {
            Ok(s) => {
                // L5 â€” tauri-plugin-store swallows deserialize errors on load
                ...
                if disk_has_settings && s.get("settings").is_none() {
                    ...
                    // registry ejection â€” close_resource() takes the
                    ...
                    match s.reload() {
                        ...
                        Err(e) => {
                            tracing::error!("store reload from disk failed: {} â€” retrying", e)
                        }
                    }
                    last_err = None;
            ...
            Err(e) => {
                ...
                if msg.contains("resource id") && msg.contains("invalid") {
                    // L3 â€” refuse `create_new()` over a healthy on-disk
                    ...
                    // 2026-05-09 wipe). If the file has aiPresets â€” or is
                    // encrypted at rest, which is user data we just can't
                    // parse here â€” surface the error so the retry loop runs
                    // again instead.
                    ...
                    if disk_healthy {
                        tracing::error!(
                            "store resource stale (attempt {}): {}, but disk \
                             has user data â€” refusing create_new() to avoid \
    ...
    Err(match last_err {
        ...
        None => anyhow::anyhow!(
            "store loaded empty while {} has a settings key â€” refused the wipe-primed store",
            store_path.display()
        ),
    })
}
...
/// Called when a "resource id â€¦ is invalid" error is detected.
pub fn invalidate_store_cache() {
    if let Ok(mut guard) = STORE_CACHE.lock() {
        if guard.is_some() {
            tracing::warn!("store cache invalidated â€” will rebuild on next access");
            *guard = None;
        }
    }
}
...
pub struct SettingsStore {
    // â”€â”€ Recording settings (shared source of truth) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    #[serde(flatten)]
    pub recording: memora_config::RecordingSettings,

    // â”€â”€ App-only fields (UI, shortcuts, metadata) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    pub user: User,
    #[serde(rename = "showMEMORAShortcut")]
    pub show_MEMORA_shortcut: String,
    ...
    /// Use MEMORA cloud for AI-powered features like suggestions.
    ...
    /// Disabled by default so the overlay doesn't appear in MEMORA's own recordings.
    #[serde(rename = "showOverlayInScreenRecording", default)]
    pub show_overlay_in_screen_recording: bool,

    /// Hide MEMORA windows from screenshots and screen-sharing viewers
    ...
    // flatten â€” do not add a second field here or serde will conflict.
...
pub enum AIProviderType {
    ...
    #[serde(rename = "custom")]
    Custom,
    #[serde(rename = "MEMORA-cloud", alias = "claude-code")]
    MEMORACloud,
    #[serde(rename = "acp")]
    Acp,
...
impl Default for AIPreset {
    fn default() -> Self {
        Self {
            id: String::new(),
            prompt: String::new(),
            provider: AIProviderType::MEMORACloud,
            acp_agent: None,
            url: "https://api.memora.com/v1".to_string(),
...
impl AudioEngineFallbackReason {
    pub fn notification_title(&self) -> &'static str {
        match self {
            Self::NotLoggedIn => "MEMORA Cloud unavailable",
    ...
    pub fn notification_body(&self) -> &'static str {
        match self {
            Self::NotLoggedIn => {
                "You are not logged in, so audio is being transcribed locally with Whisper Turbo (fast). Log in to use MEMORA Cloud."
            }
...
impl Default for SettingsStore {
    fn default() -> Self {
        ...
        let default_free_preset = AIPreset {
            id: "MEMORA-cloud".to_string(),
            prompt: r#"IMPORTANT: At the start of every conversation, read the files in .pi/skills/ directory (e.g. .pi/skills/MEMORA-api/SKILL.md and .pi/skills/MEMORA-cli/SKILL.md) before responding.
Rules:
...
- Always wrap local file paths in angle brackets because MEMORA paths often contain spaces or parentheses
...
"#.to_string(),
        ...
        Self {
            ...
            recording: memora_config::RecordingSettings {
                ...
                ignored_windows,
                ..memora_config::RecordingSettings::default()
            },
            ...
            user: User::default(),
            #[cfg(target_os = "windows")]
            show_MEMORA_shortcut: "Alt+S".to_string(),
            #[cfg(not(target_os = "windows"))]
            show_MEMORA_shortcut: "Super+Ctrl+S".to_string(),
            #[cfg(target_os = "windows")]
...
impl SettingsStore {
    ...
    /// e.g. `enableUiEvents` was renamed to `enableAccessibility` â€” if both exist
    ...
    fn sanitize_legacy_fields(mut val: Value) -> Value {
        if let Some(obj) = val.as_object_mut() {
            ...
            // Migrate unscoped "MEMORA" ignore-pattern to app-scoped "MEMORA::"
            // so browser tabs whose title contains "MEMORA" are no longer falsely
            ...
            if let Some(Value::Array(windows)) = obj.get_mut("ignoredWindows") {
                for entry in windows.iter_mut() {
                    if entry.as_str() == Some("MEMORA") {
                        *entry = Value::String("MEMORA::".to_string());
                    }
                }
            }
    ...
    /// preset into `provider: "custom"` with no URL, permanently â€” the agent id
    ...
    /// MEMORA Business". One downgrade, or one older build opening the
    /// store, was enough.
    ///
    ...
    /// a coding agent in the editor always rewrites `model` (cloud â†’ "auto",
    /// chatgpt â†’ "gpt-5.6-terra") or sets a URL (ollama, custom), so a
    ...
    pub fn to_recording_settings(&self) -> memora_config::RecordingSettings {
        let mut settings = self.recording.clone();
        // Override user_id with the Clerk JWT token from the auth user object.
        // This token is used as the Bearer credential for MEMORA cloud
        ...
        settings.user_id = self
            ...
            .unwrap_or_default();
        // Fallback chain: userName setting â†’ cloud name â†’ cloud email
            ...
            .filter(|policy| policy.get("schemaVersion").and_then(Value::as_u64) == Some(1))
        {
            ...
            if policy
                ...
                .unwrap_or(false)
            {
                settings.aec_mode = memora_config::AecMode::Off;
                settings.MEMORA_aec_enabled = false;
    ...
    pub fn to_recording_config(
    ...
    ) -> memora_engine::RecordingConfig {
        let resolved_engine = self.audio_engine_resolution().active;
        let settings = self.to_recording_settings();
        let mut config = memora_engine::RecordingConfig::from_settings(
            &settings,
            data_dir,
            Some(&resolved_engine),
        );
        // Resolve the API auth key from the seeded cache. The cache is populated
        // asynchronously by `recording::spawn_MEMORA` via the shared helper
        // (`memora_engine::auth_key::resolve_api_auth_key`) â€” which is the
        ...
        // throwaway instance can run alongside another MEMORA already on
        // :3030 instead of piggy-backing on it.
        if let Some(p) = std::env::var("MEMORA_PORT")
            .ok()
    ...
    fn audio_engine_resolution_with_cloud_auth(
        ...
        match engine.as_str() {
            // Any signed-in account may use cloud transcription â€” the free tier
            // includes a cloud transcription allowance enforced server-side.
            // Never gate on subscription/entitlement here.
            "MEMORA-cloud" if !has_cloud_auth => {
                tracing::warn!("MEMORA-cloud selected but user not logged in, falling back to whisper-large-v3-turbo-quantized");
...
pub fn init_store(app: &AppHandle) -> Result<SettingsStore, String> {
    ...
    let (mut store, mut should_save, can_run_settings_migrations) = match SettingsStore::get(app) {
        ...
        Err(e) => {
            ...
            // Non-fatal â€” logged as warn (not error) so Sentry doesn't pick it up.
            warn!(
    ...
    {
        let detected = memora_config::detect_tier();
        let stored_tier = store
            .recording
            .device_tier
            .as_deref()
            .and_then(memora_config::DeviceTier::from_str_loose);
        if stored_tier != Some(detected) {
            ...
            if is_new_store || store.recording.device_tier.is_none() {
                memora_config::apply_tier_defaults(&mut store.recording, detected);
            }
        ...
        if memora_config::is_engine_unsafe(
            &store.recording.audio_transcription_engine,
            detected,
        ) {
            let safe = memora_config::best_engine_for_platform(detected);
            tracing::warn!(
                "engine {} is unsafe on this platform (tier={:?}, macOS={:?}) â€” switching to {}",
                store.recording.audio_transcription_engine,
                detected,
                memora_config::macos_major_version(),
                safe,
            );
    ...
    // installed before this default would silently delete their old recordings â€”
    ...
    // recordings, and machine migrations often copy ~/.memora without the
    // app-config dir. So additionally require that the data dir holds no existing
    // recordings â€” retention may only default on when there is nothing to delete.
    if is_new_store && !store.extra.contains_key("localRetentionEnabled") {
        let (data_dir, _) = crate::config::resolve_data_dir(&store.data_dir)
            .map_err(|error| format!("failed to prepare MEMORA data directory: {error}"))?;
        let has_existing_recordings = data_dir.join("db.sqlite").exists();
        if has_existing_recordings {
            tracing::info!(
                "fresh settings store but existing recordings found at {:?} â€” \
...
// â”€â”€â”€ Cloud Sync Settings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Cloud Archive Settings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ ICS Calendar Settings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub fn locked_store_alert_message(detail: &str) -> String {
    format!(
        "MEMORA could not open your settings, so it stopped before starting.\n\n\
         Your settings file is encrypted and MEMORA needs the system keychain \
         ...
         Unlock your login keychain, then open MEMORA again. If it keeps \
...
pub fn show_fatal_startup_alert(title: &str, message: &str) {
    ...
    let spawned = if cfg!(target_os = "macos") {
        // `display alert` with an escaped literal â€” never interpolate the
        ...
        // console Windows allocates for a console child of a GUI process â€” the
        // alert would otherwise arrive with a black terminal beside it.
        memora_core::no_window_command("powershell")
            .args([
                "-NoProfile",
                "-WindowStyle",
                "Hidden",
...
mod tests {
    ...
    fn fresh_explicit_free_plan_applies_pipe_limit() {
        ...
        let config = store.to_recording_config(std::path::PathBuf::from("/tmp/MEMORA"));
    ...
    fn verified_free_policy_persists_offline_but_paid_plan_stays_unlimited() {
        ...
        let config = lifetime.to_recording_config(std::path::PathBuf::from("/tmp/MEMORA"));
    ...
    #[test]
    fn MEMORA_cloud_falls_back_when_not_logged_in() {
        let mut store = SettingsStore::default();
        store.recording.audio_transcription_engine = "MEMORA-cloud".to_string();
        store.user.id = None;
        ...
        assert_eq!(resolution.requested, "MEMORA-cloud");
    ...
    #[test]
    fn MEMORA_cloud_stays_active_for_signed_in_free_users() {
        ...
        assert_eq!(resolution.active, "MEMORA-cloud");
        assert_eq!(resolution.fallback_reason, None);
    }

    #[test]
    fn MEMORA_cloud_stays_active_for_app_entitled_users() {
        let mut store = SettingsStore::default();
        store.recording.audio_transcription_engine = "MEMORA-cloud".to_string();
        store.user.id = Some("user_paid".to_string());
        store.user.token = Some("token".to_string());
    ...
    #[test]
    fn MEMORA_cloud_stays_active_for_stale_legacy_cloud_subscribed_without_entitlement() {
    ...
    fn presets_n(n: usize) -> Vec<Value> {
        (0..n)
            .map(|i| json!({"id": format!("p{}", i), "model": "x", "provider": "MEMORA-cloud"}))
            .collect()
    }
    ...
    /// loser's rename failed with ENOENT â€” six times in production on
    ...
    fn durable_write_survives_concurrent_writers_to_same_path() {
        ...
        // writer's bytes â€” never a mix, never truncated.
    ...
    fn durable_write_writes_full_content_atomically() {
        ...
        // The atomic temp must be gone â€” never left as a torn sibling.
        assert!(
    ...
    fn snapshot_last_good_leaves_no_torn_tempfile() {
        ...
        // atomic rename â€” never an in-place write a crash could tear.
        let tmp = tempfile::tempdir().unwrap();
    ...
    fn auto_restore_recovers_wiped_store_from_last_good() {
        let tmp = tempfile::tempdir().unwrap();
        // Simulate the wipe â€” current file has no presets, last-good has them
        let store_path = write_store(tmp.path(), &json!({"settings": {"aiPresets": []}}));
        write_last_good(
            tmp.path(),
    ...
    fn auto_restore_recovers_missing_store_from_last_good() {
        ...
        // to it â€” must restore, not boot as a fresh install.
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join("store.bin");
        write_last_good(
            tmp.path(),
    ...
    fn auto_restore_noop_on_fresh_install() {
        // No store.bin and no snapshot â€” a genuinely fresh install. Must not
    ...
    fn auto_restore_noop_when_current_is_healthy() {
        ...
        // Even if last-good exists, current is fine â€” don't touch.
        write_last_good(
            tmp.path(),
    ...
    fn auto_restore_skips_encrypted_files() {
        // L2 must not try to "restore" over a still-encrypted blob â€” the
    ...
    fn snapshot_last_good_rotates_previous_to_prev() {
        ...
        // content must survive one generation as .last-good.prev â€” this is
    ...
    fn l5_refusal_heals_wipe_primed_store_in_place() {
        ...
        // dropped its local Arc, every retry â€” and the webview's Store.load,
        // and the plugin's exit-time save-all â€” would keep getting the same
        ...
        // The disk heals underneath it â€” the user's data was there all along.
        write_store(
            tmp.path(),
            &json!({"settings": {"aiPresets": presets_n(2)}}),
        );
        ...
        // The registry â€” what the webview's Store.load and the plugin's
        // exit-time save-all read â€” must also serve the healthy instance.
        let registered = app
    ...
    /// and shows the 403 as "upgrade to MEMORA Business".
    #[test]
    ...
    fn deliberate_non_acp_presets_are_left_alone() {
        ...
        assert_eq!(presets[0]["provider"].as_str(), Some("MEMORA-cloud"));
    ...
    fn remote_force_offs_win_after_recording_settings() {
        ...
        store.recording.aec_mode = memora_config::AecMode::Macos;
        ...
        assert_eq!(effective.aec_mode, memora_config::AecMode::Off);
        assert!(!effective.MEMORA_aec_enabled);
