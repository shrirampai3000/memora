...
/// apart (health-watchdog restart grace), so 2 â‰ˆ ten minutes of proven-futile
/// in-process restarts.
...
/// Call when an engine respawn succeeds â€” a healthy boot ends the episode.
...
/// problem â€” port, permissions â€” that DB recovery won't fix).
...
pub async fn surface_manual_recovery(reason: &str) {
    ...
    error!(
        "db recovery: {} â€” not auto-relaunching; surfacing manual recovery",
        reason
    );
    let evt = memora_events::DbRecoveryEvent::needs_recovery();
...
pub async fn surface_quarantined_recovery_at_launch(database_path: &Path) {
    ...
    let marker = memora_db::read_sqlite_quarantine(database_path)
        .ok()
        .flatten();
