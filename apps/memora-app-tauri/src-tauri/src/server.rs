...
async fn kill_process_on_port(port: u16) {
    #[cfg(unix)]
    {
        ...
        // lsof can hang indefinitely on macOS â€” always enforce a timeout
...
/// Requests with neither header â€” a local process using curl, a pipe, an agent
/// â€” still pass: loopback bind is not an authorization boundary, so a same-user
...
async fn e2e_updates_state(State(state): State<ServerState>) -> impl IntoResponse {
    ...
    // never a staged snapshot to report â€” but the response shape stays the
...
# Triggered by adding a `surface` field â€” the on-demand counterpart to the
...
mod tests {
    ...
    fn rejects_rebinding_hosts() {
        ...
        // loopback â€” reject anything that is not a loopback name.
