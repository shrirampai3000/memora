...
//!
//!   normal â†’ failure â†’ fixing â†’ recovered â†’ normal
//!
//! and pushes every transition to both overlay surfaces (the macOS SwiftUI
//! panel via FFI, the Tauri webview via event) â€” the surfaces render pushed
...
struct Inner {
    ...
    /// User dismissed the current incident â€” stay quiet until it resolves,
    ...
    /// healthy ticks are the OLD engine still answering â€” they must not count
    ...
    /// healthy engine (deliberate stop, scheduled/DRM pause) â€” used to stand
    ...
    match inner.state {
        OverlayHealthState::Normal => {
            if broken && !inner.dismissed {
                ...
                inner.healthy_ticks = 0;
                inner.not_broken_ticks = 0;
                info!("overlay health: recording incident confirmed â€” showing failure state");
                TickEffect::PushAndReveal(OverlayHealthState::Failure)
            } else {
                if !broken && inner.dismissed {
                    // Incident resolved while dismissed â€” re-arm for the next one.
                    inner.dismissed = false;
                }
                TickEffect::None
            }
        ...
        OverlayHealthState::Failure => {
            if healthy {
                ...
                // The incident evaporated without a healthy engine â€” the
                ...
                if inner.not_broken_ticks >= 3 {
                    ...
                    inner.not_broken_ticks = 0;
                    inner.dismissed = false;
                    info!("overlay health: incident no longer applies â€” standing down");
        ...
        OverlayHealthState::Fixing => {
            ...
            // (or 15s passed â€” covers odd cases), healthy responses come
            ...
            if healthy && counting {
                ...
                inner.healthy_ticks = 0;
                // Long restarts look hung behind a bare spinner â€” surface
                // the engine's boot phase ("fixing â€” updating database...")
                ...
                if timed_out {
                    ...
                    inner.fixing_since = None;
                    inner.last_detail.clear();
                    warn!("overlay health: restart did not recover in time â€” back to failure");
...
/// ("fixing â€” updating database..."). Empty when there is nothing useful to
...
/// Fire-and-forget product analytics â€” mirrors the webview's posthog events
...
///
/// `broken`  â€” confirmed incident (engine down with capture intended, a
///             debounced capture stall, or the debug-only simulated break).
/// `healthy` â€” engine responding and recording with no active stall; the only
///             signal that confirms a recovery.
/// `stand_down` â€” recording was deliberately stopped or intentionally paused.
/// `failure_detail` â€” privacy-safe explanation already classified by health.rs.
...
pub async fn on_tick(
    ...
    match effect {
        ...
        TickEffect::PushAndUnreveal(s) => {
            ...
            // The overlay was only on screen for this incident â€” put it back.
...
/// screen (issue #5127: "if hidden â€” show whatever visible"). Recovery puts it
...
async fn reveal_overlay_if_hidden(app: &tauri::AppHandle) {
    ...
    // Skip the disable_timeline gate and the wait-for-server handshake â€” the
...
/// platforms run the identical sequence, and the health loop â€” not this
/// function â€” confirms the recovery.
pub async fn restart_recording(app: tauri::AppHandle) {
    {
        ...
        if !begin_fixing(&mut inner, Instant::now()) {
            // Double-click / both surfaces racing â€” one restart is enough.
            info!("overlay health: restart already in progress â€” ignoring duplicate");
            return;
        }
    }
    ...
    let teardown = crate::recording::bounded_teardown(
        ...
        crate::recording::stop_MEMORA(app.state(), app.clone()),
    )
    .await;
    ...
    if let Err(e) = crate::recording::spawn_MEMORA(app.state(), app.clone(), None).await {
...
/// breaking a real engine: `touch ~/.memora/.simulate-recording-failure`
/// forces the broken signal; the overlay's restart clears it (and genuinely
/// restarts the engine), so the full failure â†’ fixing â†’ recovered loop runs.
#[cfg(debug_assertions)]
pub fn simulated_break_active() -> bool {
    dirs::home_dir()
        .map(|h| h.join(".memora/.simulate-recording-failure").exists())
        .unwrap_or(false)
}
...
fn clear_simulated_break() {
    if let Some(h) = dirs::home_dir() {
        let _ = std::fs::remove_file(h.join(".memora/.simulate-recording-failure"));
    }
}
...
mod tests {
    ...
    /// No detail at all stays a bare state â€” the shape every existing consumer
    /// already handles.
    #[test]
    ...
    /// contain one â€” otherwise the subsystem would absorb part of the reason.
    #[test]
