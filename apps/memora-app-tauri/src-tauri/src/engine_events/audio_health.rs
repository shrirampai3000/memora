...
//! pipeline â€” i.e. the transcript is losing the other side of the call.
...
//! be opened at all (no action button â€” capture already fell back to the
...
//! feedback loop â€” they fix it in the app and the piggyback follows).
//!
//! See `crates/MEMORA-audio/src/audio_manager/windows_output_follow.rs`
//! for the watchdog, and
//! `crates/memora-events/src/custom_events/audio_health.rs` for the
//! payload shape.
...
pub(super) fn handle(app: &AppHandle, name: &str, data: &Value) {
    ...
    match name {
        ...
        // recovered: tauri event emit only â€” no notification.
        _ => {}
    }
}
...
fn show_speaker_silent_notification(app: AppHandle, data: Value) {
    ...
    let body = if rendering.is_empty() {
        "you're in a meeting but MEMORA isn't capturing speaker audio, \
         so the transcript will be missing the other side. check your \
         playback device, or restart recording."
            .to_string()
    } else {
        format!(
            "you're in a meeting and audio is playing on \"{}\", but MEMORA \
...
/// Show the in-app notification panel telling the user MEMORA couldn't
/// open the meeting app's microphone (per-process capture failed to start),
/// but recording continues on the default mic. No action button â€” there's
...
fn show_mic_capture_failed_notification(app: AppHandle, data: Value) {
    ...
    let body = format!(
        "{} â€” recording continues on your default microphone.",
        reason
    );
