...
//!    surface "Paused" instead of lying with "Recording" â€” the engine's
...
pub(super) fn handle(app: &AppHandle, _name: &str, data: &Value) {
    ...
    // bypass the gate â€” those are capture-stalled-equivalent events
    ...
    if informational && !power_mode_changes_enabled(app) {
        debug!(
            "power_profile_changed â†’ notify: skipped (power-mode-change toasts disabled, to={})",
            evt.to
        );
        return;
    }
    ...
    let (title, body) = match (evt.to.as_str(), reason) {
        // â”€â”€ FullPause: only legitimate cause is critical battery â”€â”€â”€â”€â”€â”€â”€â”€
        ("FullPause", Some("thermal_critical") | Some("thermal_serious")) => (
            "device overheating â€” capture paused",
            "system is too hot â€” capture stopped to let it cool down.".to_string(),
        ),
        ("FullPause", Some("os_low_power")) => (
            "low power mode on â€” capture paused",
            "macos low power mode is on. plug in or turn off low power mode to resume.".to_string(),
        ),
        ("FullPause", _) => (
            "battery critical â€” capture paused",
            format!(
                "battery at {battery}% â€” all capture stopped. search and timeline still work \
                on existing data. plug in to resume."
            ),
        ),

        // â”€â”€ AudioPaused: vision still runs, audio is off â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ("AudioPaused", _) => (
            "battery low â€” audio paused",
            format!(
                "battery at {battery}% â€” audio transcription stopped, screenshots paused. \
                accessibility metadata still captured."
            ),
        ),

        // â”€â”€ Saver: throttled but still capturing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ("Saver", Some("os_low_power")) => (
            "low power mode â€” saver",
            "macos low power mode is on â€” switched to saver to match system throttling."
                .to_string(),
        ),
        ("Saver", Some("thermal_serious") | Some("thermal_critical")) => (
            "device warm â€” saver",
            "device is warm â€” throttling capture to reduce heat.".to_string(),
        ),
        ("Saver", _) => (
            "battery saver",
            format!(
                "switched to saver â€” battery at {battery}%, capture throttled to extend battery."
            ),
        ),

        // â”€â”€ Balanced: mild step down â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ("Balanced", _) => (
            "battery saver: balanced",
            "switched to balanced â€” slower capture, lower jpeg quality.".to_string(),
        ),
...
/// failure also defaults to true â€” better one extra toast than a
...
mod tests {
    ...
    fn other_toggles_do_not_affect_power_gate() {
        // captureStalls=false should NOT silence power-mode-change toasts â€”
    ...
    // `Balanced` or `Saver` â€” `AudioPaused` and `FullPause` bypass it.
