...
//! degradation is silent â€” the user keeps talking into a mic that is no longer
...
//!
//!   - `audio_device_fallback_engaged`  â†’ "mic offline â€” recording from backup"
//!     (degraded but still capturing on a substitute; informational, no action)
//!   - `audio_device_fallback_unavailable` â†’ "microphone offline â€” recording
//!     paused" (no substitute exists; capture has stopped â€” the severe case)
//!   - `audio_device_fallback_cleared`  â†’ "microphone reconnected" (recovery),
...
//!
//! See `crates/MEMORA-audio/src/audio_manager/device_monitor.rs` for the
//! state machine, and `crates/memora-events/src/custom_events/audio_devices.rs`
...
/// for a warning shown by a previous process â€” never a false "all good").
...
/// Pinned mic offline, capture moved to a substitute â€” degraded but still
...
/// Pinned mic gone and nothing to fall back to â€” capture has stopped. The
...
fn show_input_unavailable(app: AppHandle, data: Value) {
    ...
    let body = format!(
        "\"{pinned}\" went offline and there's no other microphone to record from, \
         ...
         MEMORA will resume automatically."
    );
    let payload = serde_json::json!({
        "id": format!("audio_device_fallback:{NO_INPUT_KEY}"),
        "type": "capture_stall",
        "title": "microphone offline â€” recording paused",
        "body": body,
        "actions": [],
        "autoDismissMs": 60000
    });
...
fn show_recovered(app: AppHandle, data: Value) {
    ...
    if !take_warned(key) {
        return; // no prior warning from this process â€” nothing to confirm
    }

    let body = if key == NO_INPUT_KEY {
        "a microphone is available again â€” MEMORA resumed recording.".to_string()
    } else {
        format!(
            "\"{}\" is back â€” MEMORA is recording from your selected mic again.",
            display_name(pinned_raw)
        )
    };
    let payload = serde_json::json!({
