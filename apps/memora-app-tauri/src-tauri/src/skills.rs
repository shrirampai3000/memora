...
//! device, copy chosen ones into the MEMORA skills store
//! (`<data_dir>/skills/<name>/`), and list / remove what's been imported.
//!
//! The store is the source of truth. `memora-core`'s pi executor mirrors it
...
fn background_ai_tools_home() -> Option<PathBuf> {
    #[cfg(feature = "e2e")]
    {
        ...
        return std::env::var_os("MEMORA_E2E_AI_TOOLS_HOME").map(PathBuf::from);
    }
    #[cfg(not(feature = "e2e"))]
    {
        // A dev build gets a fresh data dir, so onboarding reads as incomplete
        // and this background setup runs â€” rewriting the developer's real
        // ~/.claude, ~/.codex/config.toml, ~/.cursor/mcp.json â€¦ to point at the
...
fn background_bun_path() -> Option<PathBuf> {
    #[cfg(feature = "e2e")]
    if let Some(path) = std::env::var_os("MEMORA_E2E_BUN_PATH") {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        for attempt in 1..=3 {
            ...
            match tokio::task::spawn_blocking(move || {
                memora_engine::cli::agent::setup_all_detected_desktop_in(
                    &home,
                    &bun_path,
                    api_key.as_deref(),
                    &api_url,
...
/// Baseline skills MEMORA writes itself on every session. Importing a skill
/// under one of these names would clobber them, so we reject it.
const RESERVED_SKILL_NAMES: [&str; 3] = ["MEMORA-api", "MEMORA-cli", "MEMORA-team"];
...
/// A skill currently sitting in the MEMORA store.
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
...
/// Install the two built-in MEMORA skills into a supported external agent.
...
pub fn install_external_agent_skills(target: String) -> Result<Vec<String>, String> {
    ...
    memora_engine::cli::agent::install_skills(cli_target, "http://localhost:3030")
        .map(|paths| {
            paths
                .into_iter()
...
/// Remove the two built-in MEMORA skills from a supported external agent.
...
pub fn remove_external_agent_skills(target: String) -> Result<Vec<String>, String> {
    ...
    memora_engine::cli::agent::remove_skills(cli_target)
        .map(|paths| {
            paths
                .into_iter()
                .map(|path| path.to_string_lossy().to_string())
...
pub struct RegistrySkill {
    ...
    pub path: String,
    /// Provenance for the badge: `anthropic` | `openai` | `MEMORA` | `community`.
    #[serde(default)]
    pub source: String,
...
/// Curated catalog shipped with the app â€” the offline source of truth, and the
...
const REMOTE_REGISTRY_URL: &str = "https://raw.githubusercontent.com/MEMORA/MEMORA/main/apps/memora-app-tauri/src-tauri/skills-registry.json";
...
fn skills_store_dir() -> PathBuf {
    memora_core::paths::default_MEMORA_data_dir().join("skills")
}
...
/// Deliberately tiny â€” we only need two scalar fields for display, not a full
...
fn parse_skill_frontmatter(skill_md: &Path) -> (Option<String>, Option<String>) {
    ...
    for line in raw.lines() {
        ...
        if !in_frontmatter {
            // No frontmatter fence before real content â€” give up.
            if !trimmed.is_empty() {
                break;
            }
            continue;
...
pub fn scan_device_skills() -> Result<Vec<DeviceSkill>, String> {
    ...
    for (root, label) in scan_roots() {
        let entries = match std::fs::read_dir(&root) {
            Ok(e) => e,
            Err(_) => continue, // root may not exist â€” that's fine
        };
        for entry in entries.flatten() {
            let dir = entry.path();
...
/// List skills currently in the MEMORA store.
#[tauri::command]
#[specta::specta]
...
/// Copy a skill folder into the MEMORA store. `source_path` is the folder
...
pub fn import_skill(source_path: String) -> Result<ImportedSkill, String> {
    ...
    if RESERVED_SKILL_NAMES.contains(&key.as_str()) {
        return Err(format!("\"{key}\" is a reserved MEMORA skill name"));
    }
    ...
    memora_core::paths::copy_dir_all(&src, &dest)
...
pub fn remove_imported_skill(name: String) -> Result<(), String> {
    ...
    let chat_copy = memora_core::paths::default_MEMORA_data_dir()
        .join("pi-chat")
        .join(".pi")
        .join("skills")
        .join(&key);
...
// ---------------------------------------------------------------------------
// Skills registry â€” a curated catalog of installable skills, browsed in the
...
fn http_client() -> Result<reqwest::Client, String> {
    reqwest::Client::builder()
        ...
        .user_agent("MEMORA-app (+https://github.com/shrirampai3000/memora)")
        .build()
        .map_err(|e| format!("failed to build http client: {e}"))
}

/// Parse a registry JSON document, tolerating malformed entries â€” one bad row
...
fn parse_registry(raw: &str) -> Vec<RegistrySkill> {
    ...
    for entry in entries {
        match serde_json::from_value::<RegistrySkill>(entry) {
            ...
            Ok(_) => {} // missing a required field â€” skip quietly
...
pub async fn fetch_skills_registry() -> Result<Vec<RegistrySkill>, String> {
    ...
    if skills.is_empty() {
        // Remote was reachable but unusable (empty / all-malformed) â€” bundle wins.
...
/// `owner/repo` â€” exactly two non-empty, charset-safe segments.
...
async fn download_skill_dir(
    ...
    if status == reqwest::StatusCode::FORBIDDEN || status == reqwest::StatusCode::TOO_MANY_REQUESTS
    {
        let exhausted = res
            ...
            == Some("0");
        if exhausted {
            return Err("GitHub API rate limit reached â€” try again in a little while, or import the skill folder manually.".to_string());
        }
    ...
    if tree.truncated {
        return Err("that repository is too large to fetch a subfolder from automatically â€” import the skill folder manually instead.".to_string());
    }
...
mod tests {
    ...
    fn parse_registry_tolerates_bad_entries() {
        ...
        // Total garbage â†’ empty, never panics.
