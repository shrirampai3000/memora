...
async fn open_secret_store() -> Option<memora_secrets::SecretStore> {
...
/// Activate MEMORA after an external OAuth flow completes.
///
/// macOS can activate the application as a whole and let AppKit restore the
/// user's last active MEMORA window. Windows and Linux require a concrete
...
pub(crate) fn activate_app_after_oauth_impl(app_handle: &AppHandle, window: &WebviewWindow) {
    #[cfg(target_os = "macos")]
    {
        ...
        if let Err(e) = app_handle.run_on_main_thread(|| {
            ...
            warn!("OAuth connected but could not activate MEMORA: {e}");
        } else {
            info!("OAuth connected; activated MEMORA");
        }
    }
...
pub async fn oauth_connect(
    ...
    // id is validated against the server-side whitelist â€” the UI never supplies
    // raw scope strings.
    ...
    if config.redirect_uri_override.is_none() {
        let effective_port = std::env::var("MEMORA_PORT")
            .ok()
            .and_then(|v| v.parse::<u16>().ok())
            .unwrap_or(settings.recording.port);
        if effective_port != 3030 {
            return Err(format!(
                "OAuth callbacks require the MEMORA API on port 3030, but it is configured for port {}. Reset the port in settings to use connections.",
                effective_port
            ));
        }
    }
    ...
    // and is templated in here. Validate up front â€” before registering the
    // pending callback â€” so a bad subdomain fails fast, and so we never build a
    ...
    let auth_url_str = if needs_subdomain {
        ...
        if sub.is_empty() {
            return Err(format!(
                "{} needs your subdomain â€” enter it before connecting",
                integration_id
            ));
        }
        if !sub.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
            return Err(format!(
                "invalid subdomain '{}' â€” use only letters, numbers and hyphens (e.g. 'acme' for acme.zendesk.com)",
                sub
            ));
        }
    ...
    let (tx, rx) = oneshot::channel::<OAuthCallbackResult>();
    {
        ...
        // Reclaim those tombstones here â€” this is the only insert point.
    ...
    let callback_timeout = std::env::var("MEMORA_OAUTH_TIMEOUT_SECS")
        .ok()
    ...
    let result = tokio::time::timeout(callback_timeout, rx)
        ...
        .map_err(|_| {
            ...
            format!(
                "{} sign-in expired before the browser steps finished â€” click connect to try again",
                integration_id
            )
        })?
    ...
    let (code, callback_extras): (String, Option<serde_json::Value>) = match result {
        ...
        OAuthCallbackResult::ProviderError {
            ...
            return Err(match error.as_str() {
                "access_denied" => {
                    "authorization was denied or cancelled in the browser â€” try connecting again"
                        .to_string()
                }
    ...
    // for the token exchange â€” its token endpoint lives on that subdomain.
    ...
    // Channel.* / ChannelMessage.* scopes on consent â€” leaving `scope: "openid
    ...
    //
    // Diagnosed from Louis signing in with louis@github.com/shrirampai3000/memora (a personal MSA)
    // on 2026-04-24 â€” tenant id `9188040d-6c67-4c5b-b112-36a304b66dad` is
    ...
    if integration_id == "teams" {
        if let Some(id_token) = token_data["id_token"].as_str() {
            if extract_tid_from_jwt(id_token).as_deref() == Some(MSA_PERSONAL_TENANT_ID) {
                // No token gets written â€” we bail out before the write below.
    ...
    // stored OAuth JSON â€” same pattern as QuickBooks' {realmId}.
    ...
    // and every subsequent call says "not connected" â€” which is the exact
    ...
    // provider coexist â€” connecting a second Google account must not clobber
    // the first.
    ...
    // blindly deleted that slot after every instanced save â€” which is exactly
    ...
    // Google Calendar is disconnected â€” wake it so prewarm/meeting-detection
...
pub async fn oauth_status(
    ...
    // likely cause is a keychain ACL mismatch (e.g. devâ†”prod bundle split) â€”
...
mod tests {
    ...
    use memora_connect::oauth::{OAuthCallbackResult, PendingOAuth, PENDING_OAUTH};
    use serde_json::json;
    ...
    async fn oauth_cancel_drops_pending_flow_and_closes_channel() {
        ...
        // Dropped sender closes the channel â€” the waiter unblocks immediately.
    ...
    /// on the next insert: entries older than 2Ã— the callback timeout go,
    /// fresh ones stay.
    #[tokio::test]
    async fn stale_pending_entries_are_swept_fresh_ones_kept() {
        use memora_connect::oauth::OAUTH_CALLBACK_TIMEOUT;
...
/// subset of Microsoft Graph that personal accounts are allowed to â€” Teams
...
///
/// Atlassian OAuth tokens are tenant-agnostic â€” you must resolve the cloud_id
