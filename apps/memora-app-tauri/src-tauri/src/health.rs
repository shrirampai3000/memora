...
/// crashed and respawns it. Reuses the Stopped threshold (30 â‰ˆ 30s at 1Hz),
...
/// (bad config, revoked permission, corrupt DB) can't restart-storm â€” it falls
...
// Boot phase â€” tracks where we are inside ServerCore::start.
//
...
// "server is dead" via HTTP alone â€” so they both time out and retry, and the
...
pub fn set_boot_error(err: &str) {
    ...
    guard.since_epoch_secs = now_epoch();
    tracing::error!("boot phase â†’ error: {}", err);
}
...
/// still-initializing native sessions â€” see #3622 (onnxruntime SIGSEGV during
...
pub enum BootReadiness {
    ...
    /// happen â€” a process restart is safe. Kept distinct from `Ready`
...
/// Mirror of `routes::capture::HighFpsState` â€” what the tray needs to
...
struct HealthCheckResponse {
    ...
    /// DRM streaming content detected â€” capture should be fully stopped
    #[serde(default)]
    ...
    /// stale start flag render a stuck "Startingâ€¦".
    #[serde(default)]
...
fn capture_failure_signals(
    ...
    // engine saw neither a unique frame nor a capture attempt for 60s â€” which
...
/// Vision is off because MEMORA turned it off, not because anything
...
/// Bare `frame_status == "stale"` â€” no unique frame AND no capture attempt for
...
/// attributed â€” the pill then keeps its generic wording rather than guessing.
...
/// are treated as Recording â€” the server IS running, it's just behind on
...
fn decide_status(
    ...
    match health_result {
        Ok(health) if health.status == "unhealthy" || health.status == "error" => {
            ...
            // /health is a soft signal â€” DB pool pressure, OCR queue backpressure,
        ...
        Ok(_) => {
            // Server is responding (healthy, stale, or degraded â€” with or without
            ...
            // â€” real permission/capture failures are detected by permission_monitor
            // (see line 498-504 below). Don't surface Error in the tray for this.
            RecordingStatus::Recording
        }
        Err(_) => {
            // Connection error â€” is the server still starting up?
...
struct EngineRespawnCheck {
    ...
    /// Active subscription / dev build â€” don't respawn a lapsed install.
    entitled: bool,
    /// The server was up at some point â†’ this is a crash, not a boot failure.
    ever_connected: bool,
...
impl EngineRespawnCheck {
    ...
    /// brought a crashed embedded engine back â€” recording just sat `Stopped`.
    /// This restores parity. True only when recording is supposed to be ON but
    /// the server has been unreachable long enough to be a genuine crash â€” not a
    /// sleep/wake blip, a deliberate stop, a still-booting or merely-degraded
    /// server â€” and the per-window respawn budget isn't spent.
...
fn respawn_engine_if_crashed(
    ...
    // Server reachable again â†’ reset the budget so a later, unrelated crash gets
    ...
    // Port conflict â€” restarting into the same occupied port is futile. But
        ...
        if boot.phase == "error" {
            if let Some(ref err) = boot.error {
                if err.contains("in use") {
                    ...
                    if port_free {
                        // Only auto-restart if recording was intended â€” don't
                        ...
                        if !wants_recording {
                            info!(
                                "port {} is free but recording not intended â€” clearing error only",
                                api.port
                            );
                        ...
                        info!("port {} is now free â€” restarting engine", api.port);
                        set_boot_phase("idle", None);
                        *last_port_conflict_notified = None;
                        // Spawn directly instead of falling through to
                        // EngineRespawnCheck â€” that check requires ever_connected,
                        // which is false if the server never started successfully.
                        *last_restart_triggered = Some(now);
                        let app_clone = app.clone();
                        tokio::spawn(async move {
                            match crate::recording::spawn_MEMORA(
                        ...
                        warn!("skipping auto-respawn: port conflict detected â€” {}", err);
    ...
    let check = EngineRespawnCheck {
        ...
        in_restart_grace: last_restart_triggered
            ...
            .unwrap_or(false),
        recently_woke: memora_engine::sleep_monitor::recently_woke_from_sleep(),
        start_in_progress,
    ...
    tokio::spawn(async move {
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                // by a leaked connection â€” unrecoverable in-process. Surface
...
/// "Startingâ€¦" while the server is RESPONDING. The flags are AtomicBools
/// cleared across many exit paths in recording.rs, and `capture_running`
/// comes from a `try_lock` that can fail under contention â€” a leaked flag or
/// permanently contended lock pinned a Windows enterprise machine on
/// "Startingâ€¦" for hours while /health showed capture flowing (2026-06-11
...
fn apply_capture_session_status(
    ...
    // otherwise pin the tray on a misleading "Startingâ€¦" forever â€” the exact
    ...
    match capture_running {
        ...
        // crash:
        //   - intent OFF â†’ the user stopped capture (tray/shortcut "stop
        //     recording", which keeps the server up) â†’ honest Paused.
        //   - intent ON  â†’ the local handle is stale while the engine is
        ...
        //     /health instead of pinning the tray on Paused/"Startingâ€¦" forever
...
fn status_to_icon_key(status: RecordingStatus) -> &'static str {
    match status {
        ...
        // Outside work hours is a neutral, intentional state â€” show the calm
...
pub async fn start_health_check(app: tauri::AppHandle) -> Result<()> {
    ...
    // Bare-"stale" frame flow, debounced on its own user-presence tier â€”
    ...
    // continuously true â€” feeds clamp_start_in_progress so a leaked flag
    // can't pin the tray on "Startingâ€¦" forever (see START_PIN_CEILING).
        ...
        loop {
            ...
            match &health_result {
                ...
                Err(_) => {
                    ...
                    // Don't reset consecutive_unhealthy on connection error â€” if the server
                ...
                app.try_state::<crate::recording::RecordingState>()
            {
                ...
                // Source of truth for "the user wants capture on" â€” lets the
                // tray tell a deliberate pause (intent OFF â†’ Paused) from a
                // handle desync while the engine is healthy (intent ON â†’
                // trust /health, don't stick on "Startingâ€¦"/Paused).
                (
                    capture_running,
                    start_in_progress,
            ...
            // pin the tray on "Startingâ€¦" forever while capture is actually
            ...
            if start_in_progress_raw && !start_in_progress {
                if !start_pin_warned {
                    ...
                    warn!(
                        "start-in-progress flag stuck for >{}s while server responding â€” \
            ...
            // so the tray doesn't show a stuck "Startingâ€¦".
            ...
            // `memora-engine::permission_monitor` + capture-module emissions.
            // The old health-based degradedâ†’permission-lost heuristic was removed:
            ...
            // Per-monitor vision status â€” replaces health-derived monitor rows when
            ...
            // 503 is normal (vision disabled / older engine) â€” keep last known.
            if let Ok(res) = api
            ...
            if current_status != last_status || theme != last_theme {
                ...
                let icon_path = if is_unhealthy_icon(current_status) {
                    if theme == Mode::Light {
                        "assets/MEMORA-logo-tray-black-failed.png"
                    } else {
                        "assets/MEMORA-logo-tray-white-failed.png"
                    }
                } else {
                    if theme == Mode::Light {
                        "assets/MEMORA-logo-tray-black.png"
                    } else {
                        "assets/MEMORA-logo-tray-white.png"
                    }
                };
                ...
                let _ = app.run_on_main_thread(move || {
                    crate::window::with_autorelease_pool(|| {
                        if let Some(main_tray) = app_clone.tray_by_id("MEMORA_main") {
                            if let Err(e) =
            ...
            // â”€â”€ Detect any restart (manual, notification, settings-triggered) â”€â”€
            // When spawn_MEMORA completes, it updates last_spawn_epoch.
            ...
            if let Some(rec_state) = app.try_state::<crate::recording::RecordingState>() {
                ...
                if current_epoch > 0 && current_epoch != last_known_spawn_epoch {
                    if last_known_spawn_epoch > 0 {
                        // A restart happened â€” activate grace period
                        info!("detected restart (spawn epoch {} â†’ {}), activating {}s stall detection grace",
            ...
            // â”€â”€ DRM content pause / resume â”€â”€
            ...
            // â”€â”€ Capture stall detection â”€â”€
            ...
            if should_track_capture_stalls_after(
                ...
                if let Ok(ref health) = health_result {
                    ...
                    // incident to near zero. Content-free â€” stage name, ages and a
                    ...
                    let woke = memora_engine::sleep_monitor::recently_woke_from_sleep();
            ...
            // â”€â”€ Recording-health overlay (issue #5127) â”€â”€
            ...
            // after wake from sleep, or â€” for plain connection loss â€” before
            ...
            let recently_woke = memora_engine::sleep_monitor::recently_woke_from_sleep();
            let intentionally_paused = !capture_intended
                || schedule_paused
                || matches!(&health_result, Ok(health) if health.drm_content_paused)
                || memora_config::should_pause_audio_for_lock()
...
async fn show_port_conflict_notification(app: &tauri::AppHandle, error_msg: &str) -> Result<()> {
    #[cfg(target_os = "macos")]
    {
        if crate::window::MAIN_PANEL_SHOWN.load(std::sync::atomic::Ordering::SeqCst) {
            tracing::debug!("skipping port conflict notification â€” overlay is visible");
            return Ok(());
        }
    }
    let body = format!(
    ...
    let payload = serde_json::json!({
        "id": "port_conflict",
        "type": "port_conflict",
        "title": "port conflict â€” recording stopped",
        "body": body,
        "actions": [
...
/// Skips showing if the main overlay panel is visible â€” the notification panel
...
async fn show_capture_stall_notification(app: &tauri::AppHandle, system: &str) -> Result<()> {
    ...
    {
        if crate::window::MAIN_PANEL_SHOWN.load(std::sync::atomic::Ordering::SeqCst) {
            tracing::debug!("skipping capture stall notification â€” overlay is visible");
            return Ok(());
        }
    }
    let payload = serde_json::json!({
        ...
        "body": format!("MEMORA has not received recent {} updates. this may recover on its own, but restarting usually fixes it.", system),
        "actions": [
            { "label": "RESTART", "action": "restart_recording", "primary": true }
        ],
        "autoDismissMs": 30000
...
async fn check_health(
        ...
        .await
    {
        ...
        Ok(response) => {
            // Server returned non-2xx (e.g. 503 for "degraded") â€” still try to
...
mod tests {
    ...
    fn cpu_compat_mode_round_trips_through_snapshot() {
        ...
        // convention) â€” an unguarded set_boot_phase here would race the
    ...
    /// The pause must not mask a genuine fault either â€” including the case
    ...
    fn test_unhealthy_below_threshold_holds_recording() {
        ...
        let status = decide_status(
            ...
            0,
            CONSECUTIVE_FAILURES_THRESHOLD,
            1, // only 1 unhealthy â€” below threshold of 10
            CONSECUTIVE_UNHEALTHY_THRESHOLD,
            RecordingStatus::Recording,
        );
        assert_eq!(
    ...
    fn test_capture_absent_with_live_server_is_paused() {
        // Genuine user pause: handle absent AND intent OFF â†’ honest Paused.
        let status = apply_capture_session_status(
            RecordingStatus::Recording,
            true,
            Some(false),
    ...
    fn test_capture_absent_while_starting_stays_starting() {
        // Start flag asserted (intent ON while starting) â†’ Starting wins over
    ...
    // â”€â”€ Capture-intent desync (the "stuck on Startingâ€¦" tray bug) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    //
    ...
    // `capture_running == Some(false)` â†’ Paused â†’ the "starting" icon and sat
    ...
    fn test_handle_absent_intent_on_is_recording_not_paused() {
        ...
        // None, user still wants recording â†’ must show Recording, never Paused.
        assert_eq!(
    ...
    fn test_handle_present_is_recording_regardless_of_intent() {
        // A live handle is authoritative: Some(true) â†’ Recording either way.
    ...
    fn test_lock_contended_is_recording_regardless_of_intent() {
        // None = `capture.try_lock()` failed (contention). Never Paused â€” falls
    ...
    fn test_start_flag_beats_desync_trust() {
        ...
        // intent ON and a None handle â€” we don't want to prematurely claim
    ...
    fn test_schedule_pause_beats_desync_trust() {
        ...
        // handle is None â€” never leak a misleading Recording outside work hours.
        let status = apply_capture_session_status(
            RecordingStatus::Recording,
            true,
            Some(false),
    ...
    fn test_paused_and_starting_share_starting_icon() {
        ...
        // Paused desync was indistinguishable from a real "Startingâ€¦" and the
    ...
    fn test_audio_toggle_restart_scenario_never_sticks_on_starting() {
        ...
        // to Recording â€” never latch Paused/Starting.
        ...
        for tick in 0..10 {
            ...
            let status = apply_capture_session_status(
                ...
                false,       // no schedule
                true,        // intent ON â€” user never stopped recording
            );
            assert_eq!(
                status,
                RecordingStatus::Recording,
    ...
    // â”€â”€ Work-hours schedule pause â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    //
    // Repro of a field report: a user with a work-hours schedule booted
    // before their window, the engine started then immediately
    // stopped capture ("outside work-hours schedule â€” stopping all capture"),
    // and the tray sat on a stuck "Startingâ€¦". The inputs below are identical
    // to `test_capture_absent_while_starting_stays_starting` (server up, no
    // capture session, start flag still asserted) â€” only `schedule_paused` is
    ...
    // our back must NOT keep reading as Recording â€” that's the "overlay says
    ...
    // Outside work hours is intentional, not a failure â€” calm icon, not red.
    #[test]
    ...
    fn test_flicker_scenario_simulation() {
        ...
        // 10 consecutive failures â€” still below threshold of 30
        let status = decide_status(
    ...
    fn test_real_crash_still_detected() {
        // Server truly crashes â€” 30 consecutive seconds of failures
        let grace = Duration::from_secs(30);
        let threshold = CONSECUTIVE_FAILURES_THRESHOLD;

        // At threshold (30 failures = 30s) â€” transitions to Stopped
        let status = decide_status(
            &make_connection_error(),
    ...
    fn test_server_crash_after_boot_shows_error() {
        ...
        // Server was healthy, now crashes â€” after threshold failures (30s)
        let status = decide_status(
            &make_connection_error(),
            Duration::from_secs(60),
            grace,
    ...
    fn test_server_never_starts_shows_error_after_grace() {
        ...
        // Server never starts â€” after grace period, show the error
        let status = decide_no_debounce(
            &make_connection_error(),
    ...
    // runner â€” otherwise one test's `set_boot_phase("ready")` would race
    ...
    fn clamp_start_in_progress_passes_within_ceiling_and_resets() {
        ...
        // raw=true within ceiling â†’ true, timer starts
        assert!(clamp_start_in_progress(
            true,
            &mut since,
            Duration::from_secs(60)
        ...
        // raw drops â†’ false + timer resets (a fresh start later gets a fresh window)
        assert!(!clamp_start_in_progress(
            false,
            &mut since,
            Duration::from_secs(60)
    ...
    fn clamp_start_in_progress_stops_trusting_leaked_flag_past_ceiling() {
        ...
        // exceeds it â€” models the leaked-flag case that pinned the Windows
        // enterprise tray on "Startingâ€¦" for hours.
        let mut since = Some(Instant::now() - Duration::from_secs(1));
        assert!(!clamp_start_in_progress(true, &mut since, Duration::ZERO));
        // Timer must NOT reset while raw stays true â€” the episode is one pin.
        assert!(since.is_some());
    }
    ...
    fn never_respawns_when_user_stopped() {
        // wants_recording = false â†’ deliberate stop (incl. the tray "stop").
    ...
    fn never_respawns_a_never_started_server() {
        // ever_connected = false â†’ boot failure, not a crash; don't fight it.
        assert!(!EngineRespawnCheck {
    ...
    fn never_respawns_right_after_wake() {
        // Sleep/wake transiently kills the HTTP server â€” let it recover itself.
        assert!(!EngineRespawnCheck {
            recently_woke: true,
            ..crash_baseline()
        }
