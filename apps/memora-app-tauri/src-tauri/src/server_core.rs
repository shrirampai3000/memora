...
pub struct ServerCore {
    ...
    pub vision_metrics: Arc<memora_screen::PipelineMetrics>,
    pub power_manager: Arc<PowerManagerHandle>,
    pub pipe_manager: Arc<tokio::sync::Mutex<memora_core::pipes::PipeManager>>,
    ...
    pub high_fps_controller: Arc<memora_engine::high_fps_controller::HighFpsController>,
    ...
    pub port: u16,
    /// Local API auth key â€” exposed to the frontend via Tauri command so
    ...
    /// tears down â€” otherwise their in-flight sqlx queries (which use
    ...
    /// `shutdown()` â€” before this they survived engine restarts, kept SQLite
    ...
    oauth_refresher:
        Option<Arc<memora_connect::oauth_refresh_scheduler::OAuthRefreshScheduler>>,
    external_memory_sync:
        Option<Arc<memora_engine::external_memory_sync::ExternalMemorySyncScheduler>>,
    ...
    /// closing the pools â€” otherwise surviving keep-alive/WebSocket
...
/// Best-effort and time-bounded â€” only called once after all bind retries are exhausted.
async fn identify_port_holder(port: u16) -> Option<String> {
    ...
    {
        // netstat -ano â†’ parse lines matching our exact port in LISTENING state.
        // We filter in Rust rather than piping through findstr because
        // `findstr :<port>` matches substrings (e.g. :3030 matches :30300).
        let mut netstat = memora_core::no_window_command_async("netstat");
        ...
        let mut tasklist = memora_core::no_window_command_async("tasklist");
        tasklist
...
/// retried â€” anything else (permission denied, bad address) fails fast on
/// the first attempt.
        ...
        match bind_listener(addr).await {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::AddrInUse && attempt < attempts => {
                if attempt == 1 || attempt % 4 == 0 {
                    warn!(
                        "port {} in use (attempt {}/{}), retrying in {:?} â€” \
...
impl ServerCore {
    ...
    pub async fn start(
        ...
        on_pipe_output: Option<memora_core::pipes::OnPipeOutputLine>,
        owned_browser: Option<
            std::sync::Arc<memora_connect::connections::browser::OwnedBrowser>,
        >,
        ...
        let ai_gateway_url = crate::config::MEMORA_ai_gateway_url()?;

        // --- Environment setup ---
        std::env::set_var("MEMORA_FD_LIMIT", "8192");
        if !config.analytics_id.is_empty() {
            std::env::set_var("MEMORA_ANALYTICS_ID", &config.analytics_id);
        }
        ...
        if std::env::var("MEMORA_DISTRIBUTION").is_err() {
            std::env::set_var("MEMORA_DISTRIBUTION", "desktop-app");
        }
        ...
        crate::health::set_boot_phase(
            "migrating_database",
            Some("updating database â€” this may take several minutes on large installs"),
        );
        ...
        // sync, or a stale advisory lock from a crashed prior MEMORA
        ...
        // immediately â€” retrying would just delay the user-visible error.
        ...
        let db = {
                ...
                match DatabaseManager::new(&db_path, config.db_config.clone()).await {
                    ...
                    Err(e) => {
                        ...
                        if is_lock && attempt_idx + 1 < DB_LOCK_RETRY_DELAYS_SECS.len() {
                            // warn, not error â€” expected transient condition
                            warn!(
                        ...
                        // Final failure â€” either non-lock error or exhausted retries
                        let msg = if is_lock {
                            format!(
            ...
            match built {
                ...
                None => {
                    ...
                    let msg = format!(
                        "Database is locked â€” exhausted all {} retry attempts. {}",
                        DB_LOCK_RETRY_DELAYS_SECS.len(),
                        e
                    );
                    crate::health::set_boot_error(&msg);
        ...
        let initial_power_pref = config
            ...
            .as_deref()
            .and_then(|s| {
                serde_json::from_value::<memora_engine::power::PowerMode>(
        ...
        // the Clerk JWT (despite the name â€” see line 96 where the same value
        ...
        // restart â€” paying users who signed in after the sidecar started got
        ...
        // â€” if `set_cloud_token` has already pushed a fresher value (e.g. the
        ...
        {
            match memora_secrets::SecretStore::open_for_data_dir_with_vault_key(
                &config.data_dir,
            )
            .await
            {
                Ok(store) => {
                    let fixed = memora_secrets::fix_secret_file_permissions(&config.data_dir);
                    if fixed > 0 {
                        info!("fixed permissions on {} credential files", fixed);
                    }
                    match memora_secrets::migrate_legacy_secrets(&store, &config.data_dir).await
                    {
                        Ok(report) => {
                    ...
                    match memora_connect::oauth::sweep_shadowed_default_slots(&store).await {
                        Ok(n) if n > 0 => {
                    ...
                    // inactivity expiry) â€” without this, a token can rot
                    ...
                    let refresher = Arc::new(
                        memora_connect::oauth_refresh_scheduler::OAuthRefreshScheduler::new(),
                    );
                    refresher.start(store_arc.clone());
                    oauth_refresher_handle = Some(refresher.clone());
                    server.oauth_refresher = Some(refresher);

                    // Background sync of memories â†’ Claude Code's CLAUDE.md
                    ...
                    let memory_sync = Arc::new(
                        memora_engine::external_memory_sync::ExternalMemorySyncScheduler::new(),
                    );
        ...
        let pi_executor = Arc::new(
            memora_core::agents::pi::PiExecutor::with_shared_user_token(
        ...
        let mut agent_executors: std::collections::HashMap<
            String,
            Arc<dyn memora_core::agents::AgentExecutor>,
        > = std::collections::HashMap::new();
        agent_executors.insert("pi".to_string(), pi_executor.clone());

        let pipe_store: Option<Arc<dyn memora_core::pipes::PipeStore>> = Some(Arc::new(
        ...
        let mcp_session_access =
            memora_core::pipes::mcp_access::McpSessionAccessRegistry::new();
        ...
        // manual click.
        {
            let MEMORA_dir_for_live_views = config.data_dir.clone();
        ...
        }
        {
            ...
            let MEMORA_dir_for_check = config.data_dir.clone();
            pipe_manager.set_connection_check(Arc::new(move |required| {
                ...
                Box::pin(async move {
                    memora_connect::missing_pipe_connections(ss.as_deref(), &dir, &required)
                        .await
                })
            }));
        }
        ...
        let shared_pipe_manager = Arc::new(tokio::sync::Mutex::new(pipe_manager));
        {
            ...
            let MEMORA_dir_for_cb = config.data_dir.clone();
            ...
            shared_pipe_manager.lock().await.set_on_run_complete(Arc::new(
                move |pipe_name, execution_id, trigger_type, success, duration_secs, error_type| {
                    ...
                    // Auto-register pipe artifacts to ~/.memora/outputs/
                    if success {
                        let db = db_for_cb.clone();
                        let dir = MEMORA_dir_for_cb.clone();
                        ...
                        tokio::spawn(async move {
                            ...
                            let items = {
                                ...
                                let all = mgr
                                    .list_artifact_declarations(
                                        memora_engine::routes::artifacts::ARTIFACT_FALLBACK_CAP,
                                    )
                                    .await;
                                all.into_iter()
                                    .find(|(n, _)| n == &name)
                            ...
                            if !items.is_empty() {
                                memora_engine::routes::artifacts::auto_register_pipe_artifacts(
                                    &db,
                                    items,
                                    &name,
                                    execution_id,
        ...
        let high_fps_controller = Arc::new(
            memora_engine::high_fps_controller::HighFpsController::new(
                config.hd_recording_default,
                config.hd_recording_interval_ms,
            ),
        );

        // meeting_ended â†’ auto-stop a meeting-bound session when the call
        // ends. Without this the only safety net is the 4-hour hard cap.
        {
            ...
            owned_tasks.push(tokio::spawn(async move {
                ...
                while let Some(event) = sub.next().await {
                    ...
                    // flag is the only meeting signal it has â€” see set_in_meeting.)
        ...
        // meeting_started â†’ (1) upgrade any prewarm-pending session to a
            ...
            Err(e) => {
                let msg = if e.kind() == std::io::ErrorKind::AddrInUse {
                    match identify_port_holder(config.port).await {
                        Some(proc) => format!(
                            "port {} is already in use by {}. close that process or set MEMORA_PORT to a different value",
                            config.port, proc
                        ),
                        None => format!(
                            "port {} is already in use by another process. close that process or set MEMORA_PORT to a different value",
                            config.port
                        ),
                    }
                } else {
        ...
        // WebSocket handler still reaches the pools once they close â€” the
        ...
        let http_task = tokio::spawn(async move {
            ...
            let outcome = AssertUnwindSafe(SCServer::serve_router_with_listener_graceful(
                ...
                async move {
                    ...
                    // the whole ServerCore is gone â€” treat as shutdown.
        ...
        // wants multi-device discovery. Enable with MEMORA_ENABLE_MDNS=1.
        let mdns_enabled = std::env::var("MEMORA_ENABLE_MDNS")
            .map(|v| matches!(v.trim(), "1" | "true" | "TRUE"))
            .unwrap_or(false);
        memora_connect::mdns::set_enabled(mdns_enabled);
        if let Err(e) = memora_connect::mdns::advertise(config.port) {
            warn!("mdns advertisement failed (non-fatal): {}", e);
        }

        // â”€â”€ Async PII reconciliation workers (issue #3185 / PR #3188) â”€â”€â”€â”€â”€
        // Two independent workers â€” text and image â€” each gated by its
        // own toggle. Both off by default; users opt in through
        // Settings â†’ Privacy â†’ "AI PII removal".
        //
        // The single `pii_backend` config flag selects the inner
        // adapter for BOTH modalities:
        //   - "local"   â†’ on-device ONNX models for both text and image
        //   - "tinfoil" â†’ confidential-compute enclave (H200) for both
        ...
        // regardless (see memora_redact::parse_allow_list).
        let pii_labels = config.pii_redaction_labels.clone();

        // Cloud Clerk JWT â€” same token used for the cloud transcription
        // bearer (see line 96). Tinfoil's enclave is on the MEMORA
        // cloud auth boundary, so the user's signed-in token is what
        // authenticates redactor requests. Without this the worker logs
        // "no api key â€” requests will be un-authenticated" on every
        ...
        // Opt-in (Settings â†’ Privacy â†’ "redact secrets in agent logs", default
        ...
        if config.redact_agent_session_secrets {
            if let Ok(pi_dir) = memora_core::agents::pi::pi_config_dir() {
                ...
                let cfg = WorkerConfig {
                    ...
                    // sweep every 5 min â€” agent logs aren't latency-sensitive, and
        ...
        if config.async_pii_redaction {
            ...
            use memora_redact::adapters::opf::{OpfAdapter, OpfConfig};
            use memora_redact::adapters::tinfoil::{TinfoilConfig, TinfoilRedactor};
            use memora_redact::pipeline::{Pipeline, PipelineConfig};
            use memora_redact::worker::{Worker, WorkerConfig, ALL_TARGET_TABLES};
            use memora_redact::Pseudonymizer;
            use memora_redact::Redactor;
            use memora_redact::TextRedactionPolicy;
            ...
            //   - "local"   â†’ on-device candle OPF v3 (opf-rs). First
            //                 run downloads ~2.8 GB from
            //                 huggingface.co/MEMORA/pii-text-redactor
            //                 in the background; until the download
            //                 finishes the worker runs regex-only.
            //   - "tinfoil" â†’ Tinfoil confidential-compute enclave.
            //
                                ...
                                Err(e) => {
                                    warn!(
                                        "couldn't load OPF redactor either ({e}); running \
                                         ...
                                         to 'tinfoil' in Settings â†’ Privacy â†’ AI PII removal \
        ...
        if config.async_image_pii_redaction {
            ...
            use memora_redact::adapters::tinfoil_image::{
                TinfoilImageConfig, TinfoilImageRedactor,
            };
            use memora_redact::image::worker::{ImageWorker, ImageWorkerConfig};
            use memora_redact::ImageRedactionPolicy;
            use memora_redact::ImageRedactor;
            ...
            if use_tinfoil {
                ...
                // from huggingface.co/MEMORA/pii-image-redactor and
                // verifies SHA-256 before landing in ~/.memora/models/.
    ...
    pub async fn shutdown(mut self) {
        ...
        memora_connect::mdns::shutdown();

        // Tell redaction workers to exit BEFORE the tokio runtime tears
        // down â€” otherwise their in-flight sqlx queries panic with
        ...
        // connections keep answering on the port with closed pools â€” the
        // Windows "MEMORA isn't reachable" zombie (2026-08-04 report).
        ...
        self.db.close().await;
        memora_secrets::close_all_secret_pools().await;
...
mod tests {
    ...
    async fn bind_retry_succeeds_once_previous_listener_releases() {
        // Grab an ephemeral port, keep it held, then release it mid-retry â€”
    ...
    fn process_lookup_parsers_fall_back_cleanly() {
        ...
        assert_eq!(
            parse_tasklist_process_name("\"memora.exe\",\"42\""),
            Some("memora.exe")
        );
