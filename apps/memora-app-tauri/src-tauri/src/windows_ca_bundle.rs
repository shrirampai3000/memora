...
//!
//! Problem: bun and node ship their own CA bundle â€” they do NOT read the
...
//! every HTTPS request â€” breaking PortableGit download, Pi's npm install,
...
//! to `%LOCALAPPDATA%\MEMORA\system-roots.pem`, and set
...
//! automatically. One export â†’ every bun/node path works.
//!
...
fn install_inner() -> Result<std::path::PathBuf, String> {
    ...
    // Null crypto provider handle â€” we're not decrypting anything, just
    ...
    // but some AV products use CA â€” include both defensively.
    ...
    let dir = std::path::PathBuf::from(&local_app_data).join("MEMORA");
