...
fn emit_disk_space_low(available_bytes: u64) -> Result<(), String> {
    let event = memora_events::DiskSpaceLowEvent::new(available_bytes, ".e2e".to_string());
    memora_events::send_event(event.event_name(), event).map_err(|error| error.to_string())
}
...
fn emit_disk_space_recovered(available_bytes: u64) -> Result<(), String> {
    let event =
        memora_events::DiskSpaceRecoveredEvent::new(available_bytes, ".e2e".to_string());
    memora_events::send_event(event.event_name(), event).map_err(|error| error.to_string())
}
...
/// stranded-live-transcription bug therefore needs a genuine auto meeting â€”
...
fn emit_meeting_overlay_transcript(
    ...
    memora_events::send_event(event_name, data).map_err(|error| error.to_string())
}
...
/// envelopes into the WebView. Tests avoid the extra WebViewâ†’Rustâ†’WebView
...
async fn arm_capture_loop_silent_fault() -> Result<bool, String> {
    let armed = memora_engine::event_driven_capture::e2e_arm_capture_loop_silent_fault();
    if !armed {
...
async fn arm_sck_lookup_hang_fault() -> Result<bool, String> {
    #[cfg(target_os = "macos")]
    {
        let armed = memora_screen::monitor::e2e_arm_sck_lookup_hang_fault();
        if !armed {
            return Err(
...
async fn screen_is_locked() -> Result<bool, String> {
    Ok(memora_engine::sleep_monitor::screen_is_locked())
}
...
async fn recording_health_return_race(
    ...
    let seed_enabled = std::env::var("MEMORA_E2E_SEED")
        .ok()
        .map(|flags| {
            flags
                .split(',')
...
/// default â€” otherwise every window show swaps them out of their fullscreen
