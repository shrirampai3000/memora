...
/// Apply deterministic settings mutations requested by `MEMORA_E2E_SEED`.
pub(crate) fn apply_settings(app: &AppHandle, store: &mut SettingsStore) {
    // E2E seed: when MEMORA_E2E_SEED contains "no-recording", flip
    ...
    if e2e_flags.iter().any(|f| f == "cloud-audio-fallback") {
        ...
        info!("E2E seed: MEMORA cloud audio fallback");
    }
    ...
    if e2e_flags.iter().any(|f| f == "capture-restart-devices") {
        ...
        // shape as `meetings-only-audio` â€” real OS devices, no OCR/STT models
        // â€” but CONTINUOUS capture, so devices are open at rest and a restart
...
///
/// - 12 frames whose OCR text contains "vector" â€” enough that the results grid
...
/// `frames_ai` trigger â†’ searchable) and `text_json` (per-word boxes â†’
/// served by `/frames/{id}/text` for highlight rendering). Bounds are
/// normalized 0â€“1, matching `parse_all_text_positions`. Passing `None` for
...
async fn seed_search_fixture(db: &DatabaseManager) {
    let now = Utc::now();
    let fixture_dir = std::env::var_os("MEMORA_DATA_DIR")
    ...
    let fixture_path = fixture_dir.join("MEMORA-e2e-search-fixture.jpg");
    ...
    // Single "vector" word box per frame (normalized 0â€“1, area well under the
    ...
    // (see #4645). Harmless for the reflow test â€” it's just another result.
    ...
    // only ever prove the negative case (nothing readable â†’ dropped). This one
    ...
    let visible_text_path = fixture_dir.join("MEMORA-e2e-visible-text.jpg");
    ...
    // That is the point â€” the frame carries the query solely in hidden
    ...
    let missing_thumbnail_path = fixture_dir.join("MEMORA-e2e-missing-thumbnail.jpg");
...
fn require_isolated_db_hard_fault_seed() -> Result<PathBuf, String> {
    if !seed_requested(DB_HARD_FAULT_FLAG) {
        return Err(format!(
            "database hard-fault command requires MEMORA_E2E_SEED={DB_HARD_FAULT_FLAG}"
        ));
    }
    let home = dirs::home_dir().ok_or_else(|| "home directory is unavailable".to_string())?;
    let expected = home.join(".memora").join(".e2e");
    let configured = memora_core::paths::default_MEMORA_data_dir();
