...
/// Generation counter â€” incremented on every analyze call.
...
/// VisionKit is slow â€” only the latest request is processed, older ones are
...
pub async fn livetext_init(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if window_label != window.label() {
            warn!(
                "livetext_init: requested window '{}' but invoked from '{}' â€” attaching to the invoking window",
                window_label,
                window.label()
            );
        }
...
pub async fn livetext_analyze(
    ...
    {
        let window_label = window.label().to_string();
        // Bump generation â€” the worker checks this before doing expensive work.
