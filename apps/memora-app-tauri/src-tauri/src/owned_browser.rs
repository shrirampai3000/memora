...
//! (e.g. wikipedia.org) do not have access to `window.__TAURI__` â€” the
...
use memora_connect::connections::browser::{EvalResult, OwnedWebviewHandle};
use serde::Serialize;
...
/// Embedded webview label â€” also used by the frontend Tauri commands.
...
///   requested (agent/sidebar), shown while loading â€” like a browser omnibox
///   during navigation.
...
// The `document.title` result transport â€” marker prefix, bridge init script,
// chunked-result codec, and title-restore helper â€” lives in
...
/// navigation â€” `sid` for a chat agent (equals the frontend `conversationId`),
...
struct BrowserSessionAccessRequestPayload {
    ...
    navigation_id: Option<String>,
    /// Owner of the navigation that triggered this prompt â€” see
...
struct V20CookieBlockPayload {
    ...
    reason: String,
    navigation_id: Option<String>,
    /// Owner of the navigation that triggered this block â€” see
    /// [`OwnedBrowserNavigateEvent::owner`].
    owner: Option<String>,
}
...
/// already persisted, show MEMORA's own warning before the first Keychain
...
fn session_prompt_timeout_error(host: &str) -> String {
    format!(
        "Timed out waiting for browser-session permission for {host}. Approve the MEMORA browser prompt or log in inside the owned browser, then retry."
    )
}
...
struct OwnedBrowserState {
    ...
    /// concurrent navigations from two sources can race this â€” the
...
/// WebView2 `Source` â€” not subframe navigation targets.
...
fn child_webview_builder(
    ...
    let builder = tauri::webview::WebviewBuilder::new(label.to_string(), url)
        ...
        .on_navigation(move |_url| {
            ...
            // so never copy it into the sidebar â€” only reflect load activity.
...
impl TauriOwnedHandle {
    ...
    async fn eval_inner(
        ...
        // Hold the mutex for the entire eval â€” see eval_lock comment.
        ...
        if let Some(parsed) = &target_url {
            ...
            // eval), which is fine â€” those don't pass a url, so this branch and
            // its navigate event don't fire.
            prepare_navigation(&self.app, &self.state, parsed, owner, true).await;
        }

        // If no child webview is attached â€” e.g. a background/scheduled pipe
        // whose browser sidebar panel was never opened â€” create a hidden,
        ...
        // into the panel â€” a seamless handoff.
        ...
        // native webview over whatever section the user is on â€” a pipe working
        ...
        // reassembled â€” see [`transport`]. The whole read honours `timeout`.
        ...
        let payload = match self.read_eval_payload(&active, start, timeout, &id).await {
            ...
            Err(e) => {
                ...
                // navigate-and-scrape URL path too â€” otherwise a headless pipe's
        ...
        // The id is informational â€” with eval_lock serialising calls, there's
    ...
    /// reassembled into the full payload â€” so results larger than the browser's
    ...
    /// that seq satisfies the wait â€” so we don't latch the header or a previous
...
impl OwnedWebviewHandle for TauriOwnedHandle {
    async fn eval(
        ...
        // Plain eval / snapshot â€” never owner-tagged (no navigation event).
        self.eval_inner(code, url, timeout, None).await
    }

    /// Owner-aware eval â€” the navigate-and-scrape path a background pipe uses
    ...
    /// exists â€” so the owned browser is never advertised as ready while an eval
    /// would still fail.
    ...
    async fn navigate(&self, url: &str, owner: Option<&str>) -> Result<(), String> {
        ...
        // (POST /connections/browsers/owned-default/eval â€” see the
        // OwnedBrowser trait impl in memora-connect); without this
        ...
        // panel is on screen is a frontend concern â€” the chat layer that hosts
        ...
        // return â€” `webview.navigate()` only schedules the load. If we
        ...
        // platform webview â€” that belongs in the bidirectional-bridge
        // work, not here.
...
// ---------------------------------------------------------------------------
// Install â€” return ready-to-attach handle
// ---------------------------------------------------------------------------
...
pub fn spawn_install_when_ready(
    app: AppHandle,
    MEMORA_dir: PathBuf,
    owned_browser: std::sync::Arc<memora_connect::connections::browser::OwnedBrowser>,
) {
    tauri::async_runtime::spawn(async move {
        ...
        for attempt in 1..=MAX_ATTEMPTS {
            match install(&app, MEMORA_dir.clone()).await {
                Ok(handle) => {
...
async fn prepare_navigation(
    ...
    // below â€” and the native page-load/title callbacks that follow â€” carry the
...
/// down the WebDriver context of whatever window it parents to â€” parenting the
...
/// during headless background use. Idempotent â€” reused across background calls.
...
/// background host window, so a background/scheduled pipe â€” or any agent call
/// that arrives before the sidebar mounts â€” can drive the browser headlessly
...
// ---------------------------------------------------------------------------
// Tauri commands â€” sidebar controls (frontend â†’ child webview)
// ---------------------------------------------------------------------------
...
pub async fn owned_browser_navigate(
    ...
    if let Some(active) = state.active().await {
        // Visibility is owned by the frontend sidebar â€” never force-show here
...
async fn inject_cookies_for_url(app: &AppHandle, url: &url::Url) -> Result<(), String> {
    let Some(host) = url.host_str() else {
        info!("owned-browser cookies: skipping inject â€” url has no host");
        return Ok(());
    };
    ...
    if cookies.is_empty() {
        #[cfg(target_os = "windows")]
        {
            ...
            {
                // App-bound encrypted (v20) cookies â€” try extension first.
                match extension_cookies_for_host(app, host).await {
                    ...
                    Err(e) => {
                        info!(
                            host,
                            "owned-browser cookies: extension fallback unavailable for v20 â€” {e}"
                        );
                    }
                }
                if cookies.is_empty() {
                    let context = browser_state().context_for_url(url.as_str());
                    // Extension couldn't supply cookies â€” show the v20 card.
                ...
                .is_some()
            {
                // DB locked (browser running) â€” browser_session_decision_for_url already
                ...
                match extension_cookies_for_host(app, host).await {
                    ...
                    Err(e) => {
                        info!(
                            host,
                            "owned-browser cookies: extension unavailable for locked browser â€” {e}"
                        );
                        return Ok(());
                    }
                }
        ...
        if cookies.is_empty() {
            info!(
                host,
                "owned-browser cookies: 0 cookies available â€” navigating without inject \
...
async fn browser_session_decision_for_url(
    ...
    if !crate::owned_browser_cookies::has_cookies_for_host(&host_key).await {
        ...
        {
            // If the MEMORA Browser Bridge extension is already connected,
            ...
            if is_extension_connected(app).await {
                info!(
                    ...
                    "owned-browser: browser DB locked but extension is connected â€” using extension cookies"
                );
    ...
    if GLOBAL_SESSION_ACCESS_DISABLED.load(Ordering::SeqCst) {
        info!(
            host = host_key.as_str(),
            "owned-browser cookies: disabled by user â€” navigating without real-browser session"
        );
        return BrowserSessionDecision::ContinueLoggedOut;
    }
    ...
    match &decision {
        BrowserSessionDecision::UseBrowserSession => {
            // Set the global runtime flag â€” frontend is responsible for
        ...
        BrowserSessionDecision::CancelNavigation(_) => {
            // No user choice happened â€” keep the setting unchanged so retrying
...
/// build error is logged and ignored â€” the navigate proceeds without
    ...
    let _ = app.run_on_main_thread(move || {
        ...
        unsafe {
            ...
            for c in &cookies {
                ...
                // pass the documented string equivalents â€” Apple has
                ...
                // as Chromium stored it â€” that's what controls scope.
                ...
                // NSHTTPCookieVersion = 0 â†’ classic Netscape semantics.
                ...
                // Fire-and-forget â€” completion fires async, but
...
async fn inject_cookies_windows(
    ...
    let Some(webview) = browser_state().active().await else {
        warn!("owned-browser cookies: WebView2 inject skipped â€” no active owned browser");
        return 0;
    };
