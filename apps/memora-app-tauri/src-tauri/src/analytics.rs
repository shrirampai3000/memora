...
impl AnalyticsManager {
    ...
    pub async fn fetch_attribution(&self) {
        ...
        match self
            .client
            .get("https://github.com/shrirampai3000/memora/api/attribution")
    ...
    pub async fn send_event(
        ...
        let mut payload = json!({
            ...
            "properties": {
                ...
                "release": format!("MEMORA-app@{}", app_version),
                "$set": {
                    "app_version": app_version,
                    "os_name": os_name,
                    "os_version": os_version,
        ...
        // off a full traversal â€” 61 walks in one 13-hour session, ~26% of the
        ...
        if !status.is_success() {
            ...
            return Err(format!("PostHog API error: {} â€” {}", status, body).into());
        }
    ...
    pub async fn start_periodic_event(&self) {
        ...
        // sleep) fire back-to-back in a burst â€” coalesce them into one.
    ...
    fn read_feature_config(&self) -> serde_json::Value {
        let store_path = self.MEMORA_dir_path.join("store.bin");
        ...
        // Extract settings object â€” store has top-level keys like "settings", "onboarding"
    ...
    async fn check_recording_health(
        ...
        // Extract pipeline quality metrics (no private data â€” only counts/rates/latencies)
        ...
        Ok(json!({
            ...
            // NOTE: dropped pipeline_ocr_cache_hit_rate â€” no OCR cache exists
            ...
            // DB write-queue wedge signals â€” the recording-stall root cause.
...
pub fn start_analytics(
    ...
    // CI / automation (GitHub Actions, etc. â€” see
    ...
    tokio::spawn({
        ...
        async move {
            ...
            // github.com/shrirampai3000/memora regardless â€” so an opt-out user still leaked their
            ...
            // so PostHog can merge the person and build a real downloadâ†’open funnel
