...
//! in the `memora-redact` crate â€” the frontend ships raw text and does no
//! redaction of its own.
//!
//! Two distinct concerns:
//!   1. **Free-text PII** (logs + chat) â†’ the crate's `Pipeline` (regex pre-pass
...
//!      text redaction.
//!   2. **Config secrets by key name** (the settings JSON) â†’ a deterministic
//!      `serde_json` walk. Arbitrary, prefix-less credentials (e.g. a Deepgram
//!      key) have no detectable shape, so only the field name identifies them â€”
...
/// Does this settings field name hold a credential? Broad on purpose â€” better
...
/// Never panics â€” returns a diagnostic line if the input isn't valid JSON.
...
mod tests {
    ...
    use memora_redact::{RedactError, RedactionOutput, Redactor};
    use serde_json::json;
    use std::sync::Arc;
    use std::time::Duration;
    ...
    async fn unattended_diagnostics_use_fail_closed_local_redaction() {
        let raw = concat!(
            ...
            "MEMORA://auth?api_key=eyJhbGciOiJIUzI1NiJ9.fake-signature&source=email"
        )
        .to_string();
