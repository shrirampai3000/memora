...
//!
//! Opens a system-managed browser sheet, intercepts the `MEMORA://` redirect,
...
// Placeholder ivars â€” no per-instance state needed; anchor is resolved at call time.
...
// Must be MainThreadOnly â€” AppKit window APIs are not thread-safe.
define_class!(
    #[unsafe(super(NSObject))]
    #[thread_kind = MainThreadOnly]
    #[name = "MEMORAAuthPresentationProvider"]
    #[ivars = ProviderIvars]
    ...
    unsafe impl ASWebAuthenticationPresentationContextProviding for AuthPresentationProvider {
        ...
        fn presentation_anchor(&self, _session: &ASWebAuthenticationSession) -> Retained<NSObject> {
            ...
            get_key_window_as_anchor().expect(
                "presentationAnchor called without a window â€” should have been caught by pre-check",
            )
        }
    }
);
...
// NSWindow â†’ NSResponder â†’ NSObject, so two `into_super` calls walk up the chain.
...
// completion block would be released without this â€” dropping the block cancels
...
// One slot â€” starting a second session while one is running replaces it,
...
/// All ObjC work is dispatched to the main thread â€” ASWebAuthenticationSession
...
pub async fn start_session(
    ...
    dispatch2::DispatchQueue::main().exec_async(move || {
        ...
        let completion_handler =
            RcBlock::new(move |callback_url: *mut NSURL, error: *mut NSError| {
                let result = if !error.is_null() {
                    ...
                    let code = error.code();
                    // canceledLogin (code 1) means the user tapped Cancel â€” not a hard error.
                ...
                // After the sheet closes, focus stays with Safari â€” pull it back.
        ...
        // None â€” the ObjC protocol requires a non-null anchor, so we must reject early.
        if get_key_window_as_anchor().is_none() {
            if let Some(tx) = tx.lock().unwrap().take() {
                let _ = tx.send(Err(
                    "No window available to present login sheet â€” open MEMORA first"
                        .to_string(),
                ));
            }
            return;
