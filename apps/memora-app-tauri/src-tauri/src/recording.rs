...
//! Tauri commands for managing the MEMORA server and capture session.
//!
...
use memora_engine::RecordingConfig;
...
pub async fn test_openai_compatible_transcription(
    ...
    return Err("Audio transcription is disabled in Memora.".to_string());
}
...
fn require_recording_access(store: &SettingsStore) -> Result<(), String> {
    ...
    Err("account_required: sign in to start MEMORA recording".to_string())
}
...
/// Minimum seconds between consecutive stopâ†’spawn cycles.
...
pub struct RecordingState {
    ...
    /// Self-contained â€” `CaptureSession::stop()` needs no external references.
    ...
    /// emits `shortcut-start-recording` app-wide â€” every listening window
    /// fires `commands.startCapture()` simultaneously. Without this guard,
    /// concurrent calls both pass the is_some() check, both build a
    /// CaptureSession, and the second clobbers the first â€” dropping the
    ...
    /// Epoch seconds of last successful spawn â€” enforces cooldown between restarts
    pub last_spawn_epoch: Arc<AtomicU64>,
    /// Capture intent: true while capture is supposed to be running. Tracked at
    /// every on/off point â€” `spawn_MEMORA`/`start_capture` set it,
    /// `stop_MEMORA`/`stop_capture` clear it â€” because capture has two
    /// on-paths (full spawn vs the tray toggle) and two off-paths. Lets the
    /// health watchdog tell a crash (intent still ON â†’ respawn) from a
    /// deliberate stop (intent OFF â†’ leave it down), including the tray "stop
    /// recording" that keeps the server up. `last_spawn_epoch` can't carry this
    /// â€” it's reset to 0 on a failed spawn too, and never sees the tray toggle.
    ...
    /// `set_cloud_token` Tauri command â€” pushed by the frontend on every
    /// sign-in / sign-out â€” survive capture toggles. The Server's own
...
impl RecordingState {
    ...
    /// stop: `start_capture` / `spawn_MEMORA` set it on; `stop_capture` /
    /// `stop_MEMORA` clear it. (Capture has two on-paths and two off-paths;
...
pub struct AudioDeviceInfo {
    ...
    /// (MEMORA-audio's `device_detection::bluetooth_mic_allowed` +
...
pub async fn get_available_monitors() -> Result<Vec<MonitorDevice>, String> {
    ...
    let monitors = memora_screen::monitor::list_monitors().await;
...
pub async fn stop_capture(
    ...
    // â€” which would resurrect capture the user deliberately stopped.
...
/// same call. Read-only â€” the restore path is what consumes the entry.
...
async fn restore_interrupted_meeting_for_capture_restart(
    ...
    // the row on shutdown and again on start â€” so that probe reliably finds
    // nothing and the coordinator idles while the meeting is still running.
    if let Err(e) = memora_events::send_event(
        "meeting_streaming_reattach",
        serde_json::json!({
...
/// `BEGIN IMMEDIATE` contention pushes the handler past the deadline) â€” and a
...
pub async fn start_capture(
    ...
    // Capture is now intended to run (tray/shortcut start, mic-grant reinit, â€¦)
    // â€” record it so the health watchdog will respawn a crashed engine instead
    ...
    // tray emits `shortcut-start-recording` app-wide â€” every listening window
    ...
    // with the second â€” surfacing as a PoolClosed cascade and silently lost
    // audio chunks.
    ...
    // concurrent `start_capture_internal` (called from spawn_MEMORA's
    ...
    // "connection error" forever â€” escalate to a full restart instead.
    let (port, api_key) = {
        let server_guard = state.server.lock().await;
        let Some(ref core) = *server_guard else {
            return Err("Server not running â€” cannot start capture".to_string());
        };
    ...
    if !healthy {
        warn!(
            "Server unresponsive on port {} â€” requesting full restart",
            port
        );
        let _ = app.emit("request-server-restart", ());
        return Err(format!(
            "Server not responding on port {} â€” full restart requested",
            port
        ));
    }
    ...
    let server = server_guard
        .as_ref()
        .ok_or_else(|| "Server not running â€” cannot start capture".to_string())?;
...
/// Stop capture AND server so the next spawn_MEMORA does a full restart.
...
#[tauri::command]
#[specta::specta]
pub async fn stop_MEMORA(
    state: State<'_, RecordingState>,
    _app: tauri::AppHandle,
) -> Result<(), String> {
    // Deliberate stop â†’ clear the intent so the health watchdog leaves the
    // server down instead of auto-respawning it.
    state.set_capture_intent(false);

    let _lifecycle_guard = state.server_lifecycle.lock().await;
    stop_MEMORA_inner(&state).await
}

async fn stop_MEMORA_inner(state: &RecordingState) -> Result<(), String> {
    info!("stop_MEMORA: stopping capture and server");
    ...
    // Shut down the server so the next spawn_MEMORA does a full restart
    // with fresh settings (auth key, port, etc.). Without this, spawn_MEMORA
    ...
    // Reset flags so the next spawn_MEMORA takes the full-start path
...
/// teardown can stall right after sleep/wake â€” so without an outer bound the
/// relaunch is held until those internal timeouts fire one after another.
///
/// See the 2026-06-26 MacBook Air incident: a 2.5.57 â†’ 2.5.73 update froze for
/// ~57s before relaunching â€” the `VisionManager` 10s abort plus impatient Quit
...
pub async fn spawn_MEMORA(
    ...
    let _lifecycle_guard = state.server_lifecycle.lock().await;
    spawn_MEMORA_inner(&state, app).await
}

async fn spawn_MEMORA_inner(
    state: &RecordingState,
    app: tauri::AppHandle,
) -> Result<(), String> {
    info!(
        "spawn_MEMORA: starting server (capture intended: {})",
        state.capture_intended()
    );
    ...
    // `to_recording_config` applies MEMORA_PORT for isolated dev/E2E
    ...
    // used to time out after 15s and retry â€” which was fine for small
    ...
    //   - "ready" â†’ server is up, we're done
    //   - "error" â†’ initial start failed, safe to take over and retry
    //   - "migrating_database" / "building_audio" / "starting_pipes" / "starting"
    //     â†’ another thread is making progress, keep waiting no matter how long
    //
    ...
    if state.is_starting.swap(true, Ordering::SeqCst) {
        ...
        loop {
            // Fast path: HTTP health up â†’ done.
            if let Ok(resp) = reqwest::Client::new()
                .get(&health_url)
            ...
            match phase.phase.as_str() {
                "ready" => {
                    // Phase says ready â€” HTTP may be binding right now. Loop
                    ...
                    {
                        warn!(
                            "In-flight server start still unreachable after {}s despite boot phase=ready â€” taking over stale start",
                ...
                "idle" => {
                    // is_starting was true but phase never updated â€” the
                    ...
                    if start_wait.elapsed() > std::time::Duration::from_secs(30) {
                        warn!("is_starting set but boot phase still idle after 30s â€” taking over");
                        state.is_starting.store(false, Ordering::SeqCst);
                        if state.is_starting.swap(true, Ordering::SeqCst) {
                            return Ok(());
                        }
                ...
                _ => {
                    ...
                    // â€” keep waiting, progress is being made.
                }
            }
    ...
    // --- Check existing server ---
    {
        ...
        if server_guard.is_some() {
            ...
            if probe_server_health(&health_url, api_key.as_deref()).await {
                ...
                // Server is fine â€” just ensure capture is running
                drop(server_guard);
    ...
    if state.capture_intended() && !permissions_check.screen_recording.permitted() {
        ...
        // recording status indicator stops showing "Startingâ€¦" forever
    ...
    // one is still running, so sweeping it is not cleanup â€” it is the bug: the
    ...
    // helper in `apps/memora-app-tauri/lib/events/types.ts`).
    ...
    let owned_browser = memora_connect::connections::browser::OwnedBrowser::default_instance();
                ...
                let capture = if capture_intended_now(&wants_recording) {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            // Server started but capture failed â€” store server anyway
    ...
    match result_rx.await {
        Ok(Ok(())) => {
            info!("MEMORA started successfully");
        ...
        Ok(Err(e)) => {
            ...
            if e.contains("no monitors matched")
                || e.contains(memora_engine::vision_manager::ZERO_DISPLAYS_ENUMERATED)
            {
                crate::health::set_recording_status(crate::health::RecordingStatus::Error);
            }
            Err(e)
...
mod teardown_tests {
    ...
    /// at the timeout so the pending restart/exit proceeds â€” it must NOT block
