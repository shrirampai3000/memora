...
//! land â€” recording genuinely being lost) staleness confirms after the same
...
//! primarily from OS input idle time â€” no engine cooperation needed â€” with the
...
/// one-second checks = 15 minutes â€” long enough to outlast the engine's
...
pub(crate) fn capture_loop_silent_e2e_enabled() -> bool {
    ...
    {
        return std::env::var("MEMORA_E2E_SEED")
            .ok()
            .is_some_and(|seeds| {
                seeds
                    .split(',')
...
pub(crate) fn capture_loop_silent_e2e_started() -> bool {
    capture_loop_silent_e2e_enabled()
        && memora_engine::event_driven_capture::e2e_capture_loop_silent_fault_started()
}
...
/// on FFI failure â€” callers treat unknown as idle, the conservative direction
...
///   loss happening right now â€” alert after 90 attended stale checks, the same
...
/// (latching), and [`StaleTier::reset`] â€” so ticks with no observation
...
impl StaleTier {
    ...
    /// the shared cooldown allows it â€” the caller shows the notification and
    ...
    /// state â€” the next run starts from scratch.
...
mod tests {
    ...
    fn user_presence_prefers_os_input_idle_and_fails_toward_idle() {
        ...
        // Stale OS input with no engine signal â†’ idle tier.
        assert!(!user_present_from(Some(window + 1.0), None, now));
        // No OS source (Linux / FFI failure) + no engine signal â†’ idle tier.
    ...
    fn os_input_idle_source_returns_sane_values() {
        ...
        // on failure) â€” never panic, never a negative/NaN that could flip the
        // tier unpredictably.
    ...
    fn idle_stale_run_does_not_alert_on_the_input_that_wakes_capture() {
        ...
        // 160 seconds of idle staleness â€” the observed false-positive window.
        for _ in 0..160 {
    ...
    fn notification_latch_fires_once_per_run_and_survives_cooldown_blocking() {
        ...
        // Still stale, still confirmed â€” but latched.
        tier.observe(true, false);
        assert!(tier.confirmed());
        assert!(!tier.take_notification(true, true));
    ...
    fn confirmation_holds_across_observation_free_ticks() {
        ...
        assert!(tier.confirmed());
        // (no observe calls â€” e.g. three failed /health requests)
        assert!(tier.confirmed());
    ...
    /// (u32::MAX seconds â‰ˆ 136 years, but saturating math is free).
    #[test]
