...
use memora_core::paths::{
    default_MEMORA_data_dir, ensure_spotlight_excluded, ensure_spotlight_excluded_best_effort,
};
use std::{fs, net::IpAddr, path::PathBuf};
use tracing::warn;

pub const MEMORA_AI_GATEWAY_URL: &str =
    memora_core::agents::pi::MEMORA_API_URL;
const E2E_AI_GATEWAY_URL_ENV: &str = "MEMORA_E2E_AI_GATEWAY_URL";
...
fn resolve_ai_gateway_url(e2e_override: Option<&str>, e2e_mode: bool) -> Result<String, String> {
    if !e2e_mode {
        return Ok(MEMORA_AI_GATEWAY_URL.to_string());
    }

    match e2e_override {
        Some(raw_url) => validate_e2e_ai_gateway_url(raw_url),
        None => Ok(MEMORA_AI_GATEWAY_URL.to_string()),
    }
}
...
pub fn MEMORA_ai_gateway_url() -> Result<String, String> {
    #[cfg(feature = "e2e")]
...
#[tauri::command]
#[specta::specta]
pub async fn get_MEMORA_ai_gateway_url() -> Result<String, String> {
    MEMORA_ai_gateway_url()
}
...
pub fn get_base_dir(
    ...
    let default_path = default_MEMORA_data_dir();
...
/// Tauri command: absolute path of the MEMORA base dir (where store.bin
/// lives). Honors MEMORA_DATA_DIR; the webview must use this instead of
/// hardcoding ~/.memora, or it reads/writes a different settings file
/// than the Rust side whenever the override is set.
#[tauri::command]
#[specta::specta]
pub async fn get_MEMORA_base_dir(app: tauri::AppHandle) -> Result<String, String> {
...
/// custom path was unusable and we silently fell back to default (~/.memora or MEMORA_DATA_DIR).
pub fn resolve_data_dir(data_dir_setting: &str) -> anyhow::Result<(PathBuf, bool)> {
    let default_path = default_MEMORA_data_dir();

    // "default" or empty â†’ use ~/.memora
...
mod tests {
    ...
    fn test_existing_absolute_path_gets_spotlight_migration() {
        ...
        let tmp = root.path().join("existing-MEMORA-data");
        fs::create_dir_all(tmp.join("data")).unwrap();
        fs::write(tmp.join("db.sqlite"), b"existing install").unwrap();

        #[cfg(target_os = "macos")]
        {
            memora_core::paths::set_spotlight_excluded(&tmp, false).unwrap();
            assert!(!memora_core::paths::is_spotlight_excluded(&tmp).unwrap());
        }
    ...
    fn e2e_build_reads_configured_ai_gateway_override() {
        ...
        assert_eq!(MEMORA_ai_gateway_url().unwrap(), configured_url);
    }
}
