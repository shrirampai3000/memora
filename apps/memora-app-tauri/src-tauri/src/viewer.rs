...
//! In-app file viewer â€” opens local files (markdown, json, text, images)
...
///
/// `~\â€¦` is also expanded on Windows, where `\` is the native separator. On
...
/// directory* â€” e.g. `.pi/skills/MEMORA-api/SKILL.md` â€” so a naive read from
...
///
/// `.pi/skills/<name>/â€¦` is a mirror of the canonical skills store at
/// `<data_dir>/skills/<name>/â€¦`, and `.pi/â€¦` more generally is mirrored into the
/// chat session dir and every pipe dir â€” so we remap those explicitly.
///
...
pub fn resolve_local_path(path: &str) -> PathBuf {
    resolve_local_path_in(path, &memora_core::paths::default_MEMORA_data_dir())
}
...
fn resolve_local_path_in(path: &str, data_dir: &Path) -> PathBuf {
    ...
    // Every pipe project dir mirrors `.pi/â€¦`; only worth probing for
...
fn friendly_io_error(e: &std::io::Error) -> String {
    ...
    match e.kind() {
        ErrorKind::NotFound => {
            "file not found â€” it may have been moved, deleted, or not created yet".to_string()
        }
        ErrorKind::PermissionDenied => {
            "permission denied â€” MEMORA can't read this file".to_string()
        }
        _ => e.to_string(),
    }
}
...
mod tests {
    ...
    fn resolve_pi_skills_maps_to_store() {
        ...
        let skill = data.join("skills").join("MEMORA-api");
        std::fs::create_dir_all(&skill).unwrap();
        let file = skill.join("SKILL.md");
        std::fs::write(&file, b"# skill").unwrap();

        let got = resolve_local_path_in(".pi/skills/MEMORA-api/SKILL.md", &data);
        assert_eq!(got, file);

        // Same with a leading `./`.
        let got2 = resolve_local_path_in("./.pi/skills/MEMORA-api/SKILL.md", &data);
        assert_eq!(got2, file);
    ...
    fn resolve_pi_relative_falls_back_to_chat_dir() {
        // A non-skill `.pi/â€¦` path resolves against the chat session dir.
    ...
    fn resolve_missing_relative_falls_back_to_expanded() {
        // Nothing exists anywhere â†’ return the expanded (still-relative) path so
    ...
    fn resolve_never_probes_user_folders() {
        ...
        // be resolved to a real file outside the data dir â€” even if such a file
    ...
    fn resolve_normalizes_windows_separators() {
        // A Windows-style `.pi\skills\â€¦` citation still remaps to the store.
    ...
    fn looks_binary_detects_high_nonprintable() {
        // ~50% control bytes â€” clearly binary (ELF / executable territory).
