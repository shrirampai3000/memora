...
pub async fn get_log_files(app: AppHandle) -> Result<Vec<LogFile>, String> {
    let data_dir = get_MEMORA_data_dir(&app).map_err(|e| e.to_string())?;
    let MEMORA_data_dir = get_data_dir(&app).map_err(|e| e.to_string())?;
    Ok(collect_log_files(&[data_dir, MEMORA_data_dir]).await)
}
...
/// `dataDir`, and it also honors `MEMORA_DATA_DIR` in isolated E2E runs.
#[tauri::command]
#[specta::specta]
...
/// (missing, unmounted, permission denied â€” all common on Windows with a custom
...
pub fn get_data_dir(app: &AppHandle) -> anyhow::Result<PathBuf> {
    let default_path = memora_core::paths::default_MEMORA_data_dir();
...
pub fn get_MEMORA_data_dir(_app: &AppHandle) -> anyhow::Result<PathBuf> {
...
mod tests {
    ...
    async fn collects_only_log_files() {
        ...
        write_file(dir.path(), "MEMORA.log", "hello").await;
        ...
        assert_eq!(names(&files), HashSet::from(["MEMORA.log".to_string()]));
    }
    ...
    async fn missing_dir_is_skipped_not_fatal() {
        ...
        // A missing directory must not sink the whole call â€” this is the exact
