...
//      happened to hold, and `overlay.analysis = â€¦` wipes the selection â€” so a
...
mod imp {
    ...
    /// Longest purely-alphabetic token in the transcript â€” a term VisionKit
    ...
    pub fn run() {
        ...
        // Warm the cache off-main while the main runloop pumps â€” VisionKit hops
        ...
        // â”€â”€ 1. A pending analysis is never applied to a different frame â”€â”€â”€â”€â”€â”€
        unsafe {
            lt_hide(win());
            hs_pump(0.3);
            ...
            // â€¦and the analysis survives, so the correct update still applies it.
            position(&frame_a);
            hs_pump(0.3);
            let applied = applied_frame();
            check(
        ...
        // â”€â”€ 2. Applied analysis retires while the next frame loads â”€â”€â”€â”€â”€â”€â”€
        unsafe {
        ...
        // â”€â”€ 3. Highlight requested before its analysis lands still paints â”€â”€â”€â”€
        unsafe {
        ...
        // â”€â”€ 4. Scrolling off the match clears the hit â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        unsafe {
            ...
            // Scrolling back to the match restores it â€” the request survives.
            analyze(&img, &frame_a).expect("cached analyze should succeed");
            position(&frame_a);
            hs_pump(0.5);
            let selected = selected_len();
