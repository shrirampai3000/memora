...
// Interactive check for the Live Text overlay fix â€” drag-select text with the
// mouse and watch what you actually got.
//
// Opens its own window, uses the real Swift bridge and real VisionKit, and
// touches nothing the running app owns (no database, no ~/.memora writes).
// LiveTextManager is a per-process singleton, so an installed MEMORA is
// unaffected.
//
//   cargo run --bin livetext-interactive                 # generated frames
//   cargo run --bin livetext-interactive -- a.png b.png  # real frames
//
// Press âŽ in the terminal to swap the displayed frame. Any text you select is
...
mod imp {
    ...
    /// it while the main runloop keeps pumping â€” calling it on main deadlocks
    ...
    /// overlay â€” the exact order the timeline uses.
    ...
    pub fn run() {
        ...
        println!("  press âŽ to swap frames, ctrl-c to quit.\n");
