...
; Kills all MEMORA processes by name and by install-directory path.
...
!macro _SP_KillProcesses
  ...
  DetailPrint "Stopping MEMORA processes..."
  nsExec::ExecToLog 'taskkill /F /T /IM MEMORA.exe'
  Pop $0
  DetailPrint "taskkill MEMORA.exe: $0"
  nsExec::ExecToLog 'taskkill /F /T /IM MEMORA-app.exe'
  Pop $0
  DetailPrint "taskkill MEMORA-app.exe: $0"
  ...
  ClearErrors
  FileOpen $1 "$PLUGINSDIR\MEMORA-kill.ps1" w
  IfErrors ${_SP_SKIP}
    ...
    ; MEMORA targets 64-bit Windows 10/11 only. RemoteSigned is sufficient --
    ...
    nsExec::ExecToLog /TIMEOUT=30000 '"$WINDIR\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -NonInteractive -inputformat none -ExecutionPolicy RemoteSigned -File "$PLUGINSDIR\MEMORA-kill.ps1"'
    Pop $0
...
!macro NSIS_HOOK_PREINSTALL
  ...
  !insertmacro _SP_ClearLockedFile "MEMORA.exe"
  !insertmacro _SP_ClearLockedFile "MEMORA-app.exe"
...
!macro NSIS_HOOK_POSTUNINSTALL
  ...
  RMDir /r "$LOCALAPPDATA\MEMORA\git-portable"
  ; Clean up runtime-downloaded baseline bun (non-AVX2 CPU fallback)
  RMDir /r "$LOCALAPPDATA\MEMORA\bun-baseline"
  ; Remove parent dir only if empty (preserves other MEMORA data)
  RMDir "$LOCALAPPDATA\MEMORA"
!macroend
