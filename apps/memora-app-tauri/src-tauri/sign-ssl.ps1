...
# Used by BOTH consumer and enterprise Windows builds â€” same SSL.com EV cert.
...
# filename â€” observed concretely on Windows ARM64 for the bun sidecar:
...
#   - v2.4.258 (2026-05-22, build 26291170628) â€” both Windows x64 + ARM64
...
# runs succeed with no code change â€” purely SSL-side outages.
#
...
# 25-min Windows builds. If SSL is down longer than that, fail loudly â€”
...
while (-not $signed -and $attempt -lt $maxAttempts) {
    $attempt += 1
    if ($attempt -gt 1) {
        # Exponential backoff: 30s, 60s, 120s, 240s â€” plus 0-15s jitter so
    ...
    # merge turns it into a TERMINATING NativeCommandError under 'Stop' â€” which
    ...
    # v2.5.80, build 28389355122 â€” no "Sign attempt 2/5" ever logged). Drop to
    ...
    # and fail fast â€” burning 5 retries Ã— exponential backoff (~7.5 min)
