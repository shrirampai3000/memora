...
struct ShortcutReminderPreview {
    static func main() {
        ...
        if let iconPath = ProcessInfo.processInfo.environment["MEMORA_OVERLAY_PREVIEW_ICON"],
        ...
        if arguments.contains("--notification") {
            ...
            let notification: [String: Any] = [
                ...
                "body": "MEMORA is saving this meeting for transcription: weekly sync",
                "type": "meeting",
            ...
            if let data = try? JSONSerialization.data(withJSONObject: notification),
               let json = String(data: data, encoding: .utf8) {
                ...
                // drained yet â€” post behind it so the pill exists first, exactly
        ...
        print("native shortcut overlay preview running at size '\(size)', anchored '\(anchor)' â€” press Ctrl-C to quit")
        app.run()
    }
