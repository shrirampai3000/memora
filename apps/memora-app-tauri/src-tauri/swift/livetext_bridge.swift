...
/// its own distinct key rather than aliasing onto a shared instance â€” sharing
...
private final class LiveTextShared: @unchecked Sendable {
    ...
    let urlSession: URLSession = {
        ...
        // No caching â€” frames are unique, caching just wastes RAM
        config.urlCache = nil
...
private class LiveTextInstance {
    ...
    /// Analysis waiting to be applied â€” only set on the overlay when
    ...
    /// Frame ID associated with pendingAnalysis â€” used to validate that the
    ...
    /// to. Kept here because `overlay.analysis = â€¦` resets the overlay's
    ...
    /// bind A's text boxes to B's pixels â€” the overlay looks fine but dragging
    ...
    /// Highlight *requests* deliberately survive â€” the overlay is rebuilt from
...
public func ltInit(_ window: UnsafePointer<CChar>?, _ windowPtr: UInt64) -> Int32 {
    ...
    if #available(macOS 13.0, *) {
        ...
        // Create overlay â€” caller (lt_init) is invoked from main thread via
        ...
        MainActor.assumeIsolated {
            ...
            // Start with text selection enabled â€” this is the whole point of the
...
public func ltAnalyzeImage(
    ...
    if #available(macOS 13.0, *) {
        ...
        // Check analysis cache first â€” revisited or prefetched frames are instant
        ...
        // Don't apply to overlay yet â€” store as pending. The analysis will be
...
/// or update any overlay â€” only populates the shared cache for future instant
...
public func ltUpdatePosition(
    ...
    if #available(macOS 13.0, *) {
        ...
        // computes hit regions against the correct geometry â€” and only when
...
/// Different String instances have incompatible index storage â€” using
/// analysis.transcript indices causes "String index is out of bounds" crash
/// when VisionKit converts Range<String.Index> â†’ NSRange internally.
...
public func ltHighlightRanges(
    ...
    if #available(macOS 14.0, *) {
        ...
        // main.sync from the main thread deadlocks â€” callers today are tokio
