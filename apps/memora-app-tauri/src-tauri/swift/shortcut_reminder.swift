...
private var nonIntrusiveE2E: Bool {
    guard let raw = getenv("MEMORA_E2E_NON_INTRUSIVE") else { return false }
...
/// Swift only renders it â€” all detection/debounce/recovery logic lives in Rust.
...
final class OverlayMetrics: ObservableObject {
    ...
    /// "normal" | "failure" | "fixing" | "recovered" â€” set only via
    ...
    /// "audio" | "screen" | "" â€” which subsystem failed, when the engine could
    ...
    /// `failureHeadline` in app/shortcut-reminder/page.tsx â€” both render the
    ...
    /// True when the cursor is inside the panel area â€” drives expand/collapse
...
// Base sizes â€” multiplied by gOverlayScale at panel creation
...
/// Settings historically stored both `Super+Control+â€¦` and
/// `Control+Super+â€¦`; the overlay should always read `Cmd+Ctrl+â€¦`.
func prettifyShortcut(_ raw: String) -> String {
    let normalized = raw
        .replacingOccurrences(of: "âŒ˜", with: "Cmd+")
        .replacingOccurrences(of: "âŒƒ", with: "Ctrl+")
        .replacingOccurrences(of: "âŒ¥", with: "Opt+")
...
// bright window â€” a browser, a document, most of the day â€” it only reaches
...
/// toast covers the card's header row â€” which is precisely the case that
...
struct ShortcutReminderView: View {
    ...
    /// Derived from metrics â€” replaces the former @Binding which relied on
    ...
    private var failureView: some View {
        HStack(spacing: 0) {
            // The message zone is itself a Button â€” and clicking it RESTARTS.
            ...
            Button(action: {
                ...
                HStack(spacing: 0) {
                    ...
                    // fits the fixed 200pt panel â€” the panel frame is never
            ...
            if isExpanded {
                ...
                Button(action: {
                    // Optimistic â€” Rust pushes the authoritative "fixing"
    ...
    // overlay â€” the dock owns timeline, search, chat and settings, but nothing
        ...
        ZStack(alignment: .topTrailing) {
            ...
            if metrics.meetingActive {
                Circle()
                    ...
                    .offset(x: c(2), y: c(-2))
                    .help("meeting live â€” hover for transcript")
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: s(5)) {
                ...
                .help(
                    metrics.meetingPinned
                        ? "unpin â€” the card hides again when the pointer leaves"
                        : "pin â€” keep this card open after the pointer leaves"
                )
...
class ShortcutReminderController: NSObject, NSWindowDelegate {
        ...
        DispatchQueue.main.async { [self] in
            ...
            case "status":
                ...
                if !active || previousId != meetingId {
                    ...
                    // replaced), so drop it â€” the card must not outlive it.
                    metrics.meetingPinned = false
                    meetingStopTimeoutWorkItem?.cancel()
                    meetingStopTimeoutWorkItem = nil
                }
    ...
    func setMeetingStopResult(_ succeeded: Bool) {
        DispatchQueue.main.async { [self] in
            if succeeded {
                ...
                meetingStopTimeoutWorkItem = nil
                metrics.meetingStopping = false
                metrics.meetingStopError = "meeting did not stop â€” try again"
            }
            refreshTranscriptPanelVisibility()
        }
    }
    ...
    /// This is the state the drag glue and the attachment stack are about â€” the
    ...
    /// each one lands in `refreshTranscriptPanelVisibility` â€” including while
    ...
    private func beginStopMeeting() {
        ...
        let timeout = DispatchWorkItem { [weak self] in
            ...
            self.metrics.meetingStopError = "still active â€” try stop again"
            self.refreshTranscriptPanelVisibility()
        }
    ...
    /// frame is deliberately NOT resized â€” all health content is sized to fit
    ...
    func setHealthState(_ state: String, detail: String = "", subsystem: String = "") {
        DispatchQueue.main.async { [self] in
            ...
            if self.metrics.healthState != state {
                ...
                // Normal states are trailing-anchored, health states centred â€”
    ...
    /// geometry mid-gesture â€” a toast arriving, a hover exit landing. Without
...
// mouseUp arrives first, let it through â€” SwiftUI completes the tap normally.
//
...
// almost immediately â€” measured at 3-7ms â€” whenever the app is not the active
...
// actually was. Owning the loop makes it one begin, N moves, one end â€” the same
...
private class DraggableHostingView<Content: View>: NSHostingView<Content> {
    /// Called when a drag begins â€” lets the controller collapse the pill.
    ...
    override func mouseDown(with event: NSEvent) {
        // Let SwiftUI handle the press while the mouse is still down â€”
        ...
        dragMonitor = NSEvent.addLocalMonitorForEvents(matching: [.leftMouseDragged, .leftMouseUp]) { [weak self] event in
            ...
            case .leftMouseUp:
                ...
                if self.isDragging {
                    // Drag just ended â€” swallow so SwiftUI's button doesn't see
                    // mouseUp and fire its action.
                    self.isDragging = false
                    self.onDragEnded?()
                    return nil
                }
                // Normal click â€” let the event reach SwiftUI.
                return event
            case .leftMouseDragged:
                ...
                if hypot(dx, dy) > dragThreshold {
                    // Drag â€” collapse pill, then move the window ourselves for
