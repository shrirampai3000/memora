...
struct NotificationAction: Codable {
    ...
    // The field is never read by the Swift side â€” only `id`, `type`, `primary`,
    // `url`, `label` are â€” so making it optional restores native rendering
...
// MEMORA brand: black & white geometric minimalism
...
private enum Brand {
    ...
    static func swiftUIMonoFont(size: CGFloat, weight: Font.Weight = .regular) -> Font {
        ...
        // dynamic weight via font descriptor â€” must use the specific face name.
        let name: String
        switch weight {
...
struct NotificationContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            // Body â€” render basic markdown inline, scrollable when long
            ...
            if !payload.actions.isEmpty {
                HStack(spacing: 8) {
                    ...
                    Spacer()
                    BrandTextButton(label: "DISMISS â†’", fontSize: 10) {
                        onDismiss()
                    }
                }
                .padding(.horizontal, 14)
            ...
            // Progress bar â€” 2px, sharp, no radius
        ...
        // Override link handling â€” SwiftUI's default openURL doesn't work
        // in non-activating panels. MEMORA:// URLs go through the
        // action callback so they stay in-process (no macOS app activation
        // bounce); everything else opens via NSWorkspace.
        .environment(\.openURL, OpenURLAction { url in
            if url.scheme == "MEMORA" {
...
struct MarkdownText: View {
    ...
    /// `viewerOverridePath` is set when the link is a `MEMORA://view?path=â€¦`
    /// deeplink (rewritten from a local file path by the /notify route). It
    /// carries the original file path so the panel can render an â†— button
    /// next to the link to open the file in the OS default app â€” escape
    ...
    /// If `url` is `MEMORA://view?path=â€¦`, return the decoded path. Else nil.
    fileprivate static func viewerOverridePath(for url: URL) -> String? {
        guard url.scheme == "MEMORA" else { return nil }
    ...
    private func renderLine(_ line: String) -> some View {
        ...
        if !hasLinks {
            // No links â€” pure Text concatenation (wraps naturally)
            textView(for: segments)
                .lineSpacing(2)
                .lineLimit(nil)
        } else {
            // Has links â€” render text segments as Text, links as clickable Buttons
            VStack(alignment: .leading, spacing: 2) {
...
/// Tiny â†— button rendered next to a `MEMORA://view?path=â€¦` link so the
...
private struct ViewerOverrideButton: View {
    ...
    var body: some View {
        Button(action: {
            // Try Obsidian first for markdown â€” same logic as Rust's
...
/// Open a URL with the right transport: in-app for MEMORA:// (no
...
public func notifShow(_ jsonPtr: UnsafePointer<CChar>) -> Int32 {
    ...
    guard let data = json.data(using: .utf8) else {
        NSLog("[MEMORA-notif] failed to convert json to utf8 data")
        return -1
    }
    let payload: NotificationPayload
    do {
        ...
        NSLog("[MEMORA-notif] json decode failed: \(error) â€” json: \(json.prefix(500))")
        return -1
    }
...
// A native list of the notification history â€” same data as the pipes-store
...
extension View {
    /// Kill the blue macOS focus ring inside the inbox â€” the panel becomes
...
/// Actions worth rendering as row buttons â€” mirrors the webview bell's
/// filter: dismiss (the row âœ• covers it), copy and source (dedicated
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .top, spacing: 0) {
                ...
                // Sibling of the expand button, not nested inside it â€” nested
                // buttons make the âœ• also toggle the row.
    ...
    var body: some View {
        VStack(spacing: 0) {
            VStack(spacing: 7) {
                HStack {
                    Text("inbox")
                        ...
                        .foregroundColor(.primary)
                    Spacer()
                    // Clears both tabs, so it stays reachable from either one â€”
            ...
            if entries.isEmpty || (viewMode == .priority && priorityEntries.isEmpty) {
                Spacer()
                VStack(spacing: 4) {
                    Text(entries.isEmpty ? "no notifications yet" : "youâ€™re caught up")
                        .font(Brand.swiftUIMonoFont(size: 10, weight: .medium))
                        .foregroundColor(.primary.opacity(0.8))
                    if !entries.isEmpty {
                        Button(action: { onViewModeChange(.all) }) {
...
class InboxPanelController: NSObject {
    ...
    /// toggle() â€” without this, clicking the bell to close would immediately
    /// reopen.
    ...
    func toggle(json: String?) -> Bool {
        ...
        let work = { [self] in
            if visible {
                ...
                // click-outside monitor â€” the user meant "close".
            } else {
    ...
    private func updateContent() {
        ...
        let view = InboxListView(
            ...
            onRunAction: { [weak self] entry, action in
                ...
                // real action â€” same JSON the toast sends, so Rust + the JS
        ...
        // stay readable in light AND dark mode â€” a hardcoded dark card made
    ...
    private func installMonitors() {
        removeMonitors()
        // Click anywhere outside the panel dismisses it â€” other apps arrive
