...
//! (`apps/memora-app-tauri/src-tauri/src/enterprise/sync.rs`'s `#[cfg(test)] mod tests`). This
...
//! `cargo test --bin MEMORA-app`. An integration test target compiles
//! independently and avoids that bin tree entirely â€” keeping these tests
...
// tree, so pull in the real source rather than shimming it â€” otherwise this
