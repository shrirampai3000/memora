...
test('native backend changes do not invalidate the frontend input hash', async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), 'MEMORA-frontend-hash-'))
	try {
		...
		await fs.writeFile(path.join(cargoTarget, 'MEMORA-app.exe'), 'first build')
		expect(await computeInputHash(root)).toBe(initial)

		await fs.writeFile(path.join(cargoTarget, 'MEMORA-app.exe'), 'second build')
		expect(await computeInputHash(root)).toBe(initial)
