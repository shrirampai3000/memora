...
preview_cache="${TMPDIR:-/tmp}/MEMORA-shortcut-overlay-preview"
...
preview_icon="$preview_bundle/Contents/Resources/MEMORA.icns"
watch=1
preview_pid=""
preview_args=()
...
launch_preview() {
    MEMORA_OVERLAY_PREVIEW_ICON="$preview_icon" \
...
if ((watch == 0)); then
    MEMORA_OVERLAY_PREVIEW_ICON="$preview_icon" exec "$preview_binary" "${preview_args[@]}"
fi
...
echo "watching shortcut_reminder.swift â€” save to rebuild and relaunch; press Ctrl-C to quit"
launch_preview
