...
// even when no frontend source had changed â€” the slowest, most redundant step
// of a rebuild.
//
// How it works:
//   1. Hash the frontend inputs â€” every file in the app package EXCEPT the
...
//      (~/.cache/MEMORA/frontend-out/<hash>) -> copy it into out/. No build.
...
// frontendDist rather than silently embedding an old UI â€” the #4645 guarantee.
...
// this machine. Override with MEMORA_FRONTEND_CACHE_DIR (set to '' or
// 'off' to disable caching entirely).
const cacheEnv = process.env.MEMORA_FRONTEND_CACHE_DIR
const cacheRoot =
	cacheEnv === '' || cacheEnv === 'off'
		? null
		: cacheEnv ||
			path.join(process.env.XDG_CACHE_HOME || path.join(os.homedir(), '.cache'), 'MEMORA', 'frontend-out')
...
// time, so each one is a bundle input â€” enumerated dynamically because a
...
// changed: rebaking `NEXT_PUBLIC_MEMORA_WEB_URL` with untouched sources
...
async function main() {
	const forced = ['1', 'true'].includes(String(process.env.MEMORA_FORCE_FRONTEND_BUILD).toLowerCase())
	...
	if (!forced && (await readOutKey()) === key && (await hasIndex(outDir))) {
		console.log('[build-frontend] out/ already current â€” nothing to build')
		return
	}

	// 2) A matching build is in the local cache â€” restore it, no `next build`.
	if (!forced && entry && (await hasIndex(entry))) {
		...
		console.log('[build-frontend] restored â€” skipped next build')
		return
	}

	// 3) Cache miss â€” build, then save the result into the cache.
	console.log(
		forced
			? '[build-frontend] MEMORA_FORCE_FRONTEND_BUILD set â€” building unconditionally'
			: '[build-frontend] no cached build for current inputs â€” building',
	)
