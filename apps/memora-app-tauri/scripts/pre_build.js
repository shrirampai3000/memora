...
// Windows arch: 'x64' (amd64) or 'arm64' (aarch64) â€” used for bun binary name and ffmpeg/CRT paths
...
// the rebuild path â€” so an unchanged frontend can reuse out/ and skip
...
const config = {
	...
	windows: {
		...
		// Windows ARM64 (aarch64-pc-windows-msvc) â€” tordona/ffmpeg-win-arm64
...
async function copyBunBinary() {
	...
	if (platform === 'windows') {
		...
		// Tauri externalBin looks for bun-{target_triple}; on Windows arm64 â†’ aarch64-pc-windows-msvc, x64 â†’ x86_64-pc-windows-msvc
		...
		// the Intel app â€” surfaced as Pi-install "Bad CPU type in executable
		// (os error 86)" on Intel Macs).
		const bunVersion = '1.3.10';
		const releaseTarget = process.env.MEMORA_RELEASE_TARGET;
		...
		// In CI we set MEMORA_RELEASE_TARGET per-matrix-entry and only need
		...
		if (wanted.length === 0) {
			throw new Error(`unknown MEMORA_RELEASE_TARGET for macOS: ${releaseTarget}`);
		}
...
async function downloadStaticLinuxFfmpeg() {
	...
	// 5xx). downloadFile() retries on any curl failure â€” including HTTP error
	// codes via curl -f â€” so those transient responses fall inside the retry
...
// Regression guard for 9a68ae9de â€” static layer. Covers every macOS sidecar,
...
// would wedge mid-iteration after the second sidecar â€” no output, no
...
async function runWithTimeout(cmd, { timeoutMs = 30_000, label } = {}) {
	...
	if (timedOut) {
		throw new Error(
			`${label || cmd.join(' ')} timed out after ${timeoutMs}ms â€” likely a bun shell / system-tool hang.`
		);
	}
	if (exitCode !== 0) {
		throw new Error(
	...
	for (const bin of sidecars) {
		...
		if (!new RegExp(`\\b${expectedArch}\\b`).test(fileOut)) {
			throw new Error(
				...
				`  ${fileOut}\n` +
				`filename promises ${expectedArch} â€” Tauri ships it under the matching target.`
			);
		}
...
// Regression guard for 9a68ae9de â€” runtime layer.
...
//
// Only checks the host-arch sidecar â€” the other arch gets exercised on its
...
async function verifyMacosSidecarsRun() {
	...
	for (const bin of sidecars) {
		...
		// not the v2.4.243 sidecar crash this guard is looking for â€” warn and
		...
		clearTimeout(timer);
		if (timedOut) {
			console.warn(`  WARN: ${bin} sandbox verify timed out after 30s â€” skipping (likely a tooling issue, not a sidecar regression)`);
			continue;
		}
		if (exitCode !== 0) {
			...
			throw new Error(
				...
				`this is the v2.4.243 crash class â€” see commit 9a68ae9de.`
			);
		}
		console.log(`  ok: ${bin}`);
	}
...
if (platform == 'linux') {
	...
	if (inCI) {
		console.log('CI detected: apt packages handled by workflow cache-apt-pkgs-action âœ…\n');
	} else {
		...
		try {
			...
			if (aptPackagesNotInstalled.length > 0) {
				...
				console.log('Package installation completed successfully âœ…\n');
			} else {
				console.log('all required packages are already installed âœ…\n');
			}
		} catch (error) {
...
/** Find Microsoft.VC14*.CRT dir (143/144/145): VCToolsRedistDir â†’ vswhere â†’ standard paths. arch: 'x64' or 'arm64' (Windows ARM64). */
...
if (platform == 'macos') {
	...
	// runtime (`8.1`, `8.1_2`, â€¦), so dyld fails with
	//   Library not loaded: â€¦/Cellar/ffmpeg/8.1_1/lib/libavdevice.62.dylib
	// and ffmpeg SIGABRTs on every invocation. osxexperts.net binaries
	// are statically linked and have zero external deps â€” safe to copy
	// into any .app.
  ...
  console.log('FFMPEG and FFPROBE checks completed');
	...
	// Runs unconditionally â€” cache hits get verified too.
...
if (!process.env.GITHUB_ENV) {
	...
	// Get relative path to MEMORA-app-tauri folder
