...
#   .\exe-to-intunewin.ps1 -SetupExe "C:\path\to\MEMORA-2.50.3-setup.exe"
#   .\exe-to-intunewin.ps1 -SetupExe "C:\path\to\MEMORA-2.50.3-setup.exe" -OutDir "C:\intunewin\out"
#
# Run from: apps/MEMORA-app-tauri (or pass full path to -SetupExe).
...
$installScript = Join-Path $packageDir "install-MEMORA-enterprise.ps1"
@"
`$ErrorActionPreference = "Stop"
`$setup = Join-Path `$PSScriptRoot "$setupName"
`$proc = Start-Process -FilePath `$setup -ArgumentList "/S" -Wait -PassThru
if (`$proc.ExitCode -ne 0) {
    throw "MEMORA installer failed with exit code `$(`$proc.ExitCode)"
}
`$base = [Microsoft.Win32.RegistryKey]::OpenBaseKey(
    [Microsoft.Win32.RegistryHive]::LocalMachine,
    [Microsoft.Win32.RegistryView]::Registry64
)
`$key = `$base.CreateSubKey("SOFTWARE\MEMORA")
...
Write-Host "Package folder: $packageDir (installer + install-MEMORA-enterprise.ps1)"
