...
export async function setupOpenBlas({ cwd, winArch }) {
	...
	await ensureCachedDirectory({
		...
		validate: async (directory) => {
			...
			// ARM64 exe imports the PE internal name "openblas.dll" â€” the
...
async function flattenOpenBlas(openblasPath, winArch) {
	// Flatten package wrapper (win64/OpenBLAS-0331-dll) â€” only when freshly extracted, not bin/include/lib
	...
	// Also rename openblas.dll/.lib â†’ libopenblas.dll/.lib (runtime + MSVC import library).
	if (winArch === 'arm64') {
		...
		// internal name is also "openblas.dll" â€” the exe's import table resolves
		...
		// Rename openblas.lib â†’ libopenblas.lib (MSVC import library for linking)
