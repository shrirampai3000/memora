...
function resolveLocalApiKey(): string {
	const configured = process.env.MEMORA_LOCAL_API_KEY?.trim();
	if (configured) return configured;
	const result = spawnSync("MEMORA", ["auth", "token"], {
		encoding: "utf8",
...
function resolveSkillPath(): string {
	const configured =
		argument("skill") || process.env.MEMORA_EVAL_SKILL_PATH?.trim();
	...
	throw new Error(
		"MEMORA's Pi skill is unavailable. Start the desktop AI once or pass --skill=/path/to/SKILL.md.",
	);
}
...
const outputDir = configuredOutputDir
	? configuredOutputDir
	: mkdtempSync(join(tmpdir(), "MEMORA-daily-summary-eval-"));
mkdirSync(outputDir, { recursive: true, mode: 0o700 });
chmodSync(outputDir, 0o700);

const localApiKey = resolveLocalApiKey();
if (!localApiKey) {
	console.error("No local MEMORA API key is available.");
	process.exit(2);
}
...
for (let dayIndex = 0; dayIndex < requestedDays; dayIndex += 1) {
	...
	const pi = spawnSync(
		...
		],
		{
			...
			env: {
				...process.env,
				PI_CODING_AGENT_DIR: join(homedir(), ".MEMORA", "pi-config"),
				MEMORA_LOCAL_API_KEY: localApiKey,
				MEMORA_LOCAL_API_URL: "http://localhost:3030",
			},
		},
	);
