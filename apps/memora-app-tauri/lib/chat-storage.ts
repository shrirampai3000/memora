...
async function getChatsDir(): Promise<string> {
  ...
  // Resolve under the active data dir (honors MEMORA_DATA_DIR / a relocated
...
async function saveConversationFileInner(
  ...
  // treat as "missing" via its try/catch â†’ return null path. rename()
...
export async function migrateFromStoreBin(): Promise<void> {
  try {
    ...
    console.log(
      `[chat-storage] migrating ${history.conversations.length} conversations from store.bin to ~/.MEMORA/chats/`
    );
