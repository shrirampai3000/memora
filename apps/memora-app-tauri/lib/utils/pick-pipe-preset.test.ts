...
describe("pickPipePreset", () => {
  ...
  it("falls back to the default preset when no 'pipes' preset exists", () => {
    // Non-pro user: single "memora" preset on auto, marked default.
    const presets = [
      { id: "memora", model: "auto", defaultPreset: true },
    ];
    expect(pickPipePreset(presets)?.id).toBe("memora");
  });
  ...
  it("never assigns an ACP chat harness to a raw Pi pipe", () => {
    ...
    expect(pickPipePreset([
      ...
      { id: "raw", provider: "MEMORA-cloud", model: "auto", defaultPreset: true },
    ])?.id).toBe("raw");
  });

  it("coerces a frontier-model preset to 'auto' (pipes must not run frontier)", () => {
    // No "pipes" preset; default is pinned to Opus â†’ pipe would inherit a cost bomb.
