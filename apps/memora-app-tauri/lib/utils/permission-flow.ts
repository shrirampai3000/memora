...
export interface WatchAuthorizationStatusOptions {
  ...
  /**
   * Continue polling even when `document.hidden` is true (e.g. the memora
...
/**
 * MEMORA-facing alias: every OS permission we surface in onboarding,
...
function isMemoraAppBundle(
  appPath: string | null | undefined,
): appPath is string {
  if (!appPath || !appPath.endsWith(".app")) return false;
  return /MEMORA/i.test(appPath);
}
...
async function getMemoraAppPath(): Promise<string | null> {
  if (state.appPath !== undefined) return state.appPath;
  const appPath = await suggestedHostAppPath();
  state.appPath = isMemoraAppBundle(appPath) ? appPath : null;
...
async function restoreAlwaysOnTopWhenMemoraRefocuses(): Promise<void> {
...
async function letSystemSettingsComeForward(): Promise<void> {
  try {
    ...
    await restoreAlwaysOnTopWhenMemoraRefocuses();
  } catch {
...
async function reclaimMemoraWindow(): Promise<void> {
  if (state.pendingFocusUnlisten) {
    try { state.pendingFocusUnlisten(); } catch {}
    state.pendingFocusUnlisten = null;
  }
  ...
  try {
    ...
    console.error(
      "failed to refocus memora window after permission flow:",
      error,
    );
  }
}
...
export async function requestPermissionWithFlow(
  ...
  const appPath = dragPermission ? await getMemoraAppPath() : null;
