...
describe("presetImageSrc", () => {
  ...
  it("maps providers and falls back to the custom icon", () => {
    ...
    expect(presetImageSrc("MEMORA-cloud")).toBe("/images/MEMORA.png");
    expect(presetImageSrc("acp")).not.toBe("/images/MEMORA.png");
...
describe("generatePresetName", () => {
  ...
  it("falls back to the provider when no meaningful model is picked", () => {
    expect(generatePresetName({ provider: "MEMORA-cloud", model: "auto" }, [])).toBe(
      "MEMORA-cloud",
    );
