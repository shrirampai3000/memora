...
describe("hasHumanRenderMarker", () => {
  it("accepts the human and html variants, whitespace-tolerant", () => {
    expect(hasHumanRenderMarker("<!-- memora:render=human -->")).toBe(true);
    expect(hasHumanRenderMarker("<!--MEMORA:render=html-->")).toBe(true);
    expect(
      hasHumanRenderMarker("<!--   memora:render = human   -->"),
    ).toBe(true);
    expect(
      hasHumanRenderMarker("<html><!-- memora:render=human --><body>x"),
    ).toBe(true);
  });
  ...
  it("rejects unmarked or look-alike content", () => {
    ...
    expect(hasHumanRenderMarker("memora:render=human")).toBe(false); // not a comment
    expect(hasHumanRenderMarker("<!-- memora:render=evil -->")).toBe(false);
    expect(hasHumanRenderMarker("")).toBe(false);
  });

  it("HUMAN_RENDER_MARKER is exported and matches", () => {
    expect(HUMAN_RENDER_MARKER.test("<!-- memora:render=human -->")).toBe(
      true,
    );
  });
  ...
  it("also accepts the meta form (survives comment-stripping minifiers)", () => {
    expect(
      hasHumanRenderMarker(
        '<meta name="memora:render" content="human">',
      ),
    ).toBe(true);
    // attribute order is not significant
    expect(
      hasHumanRenderMarker(
        "<meta content='html' name='memora:render'>",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a meta with the wrong content value", () => {
    expect(
      hasHumanRenderMarker('<meta name="memora:render" content="nope">'),
    ).toBe(false);
  });
});
...
describe("shouldRenderHtmlByDefault", () => {
  it("renders full documents, marked artifacts, and bare fragments first", () => {
    ...
    expect(
      shouldRenderHtmlByDefault("<!-- memora:render=human --><h1>hi</h1>"),
    ).toBe(true);
    expect(
      shouldRenderHtmlByDefault(
        '<meta name="memora:render" content="human"><h1>hi</h1>',
      ),
    ).toBe(true);
...
describe("wrapHtmlForSandbox", () => {
  ...
  it("includes the frame->host bridge (resize + openLink only)", () => {
    ...
    expect(out).toContain("MEMORA-viewer");
