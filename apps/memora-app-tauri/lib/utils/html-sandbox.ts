...
export const HUMAN_RENDER_MARKER =
  /<!--\s*memora:render\s*=\s*(?:human|html)\s*-->/i;

/**
 * Meta form: `<meta name="memora:render" content="human">`. Survives HTML
 * minifiers that strip comments; attribute order is not significant.
 */
function hasMetaRenderMarker(text: string): boolean {
  const tag = text.match(/<meta\b[^>]*\bmemora:render\b[^>]*>/i);
 ...
 * channel from frame â†’ host, and it is deliberately tiny:
 ...
 *  - full doc with `<head>`  â†’ inject right after `<head>`
 *  - has `<html>` but no head â†’ insert a `<head>` with our payload
 *  - bare fragment            â†’ wrap in a minimal document
