...
export const DISMISSED_STORAGE_KEY = "MEMORA-announcements-dismissed-v1";
...
export const PREVIEW_STORAGE_KEY = "MEMORA-announcement-preview";
...
export function areRemoteAnnouncementsDisabled(
  ...
  return (
    ...
    env.NEXT_PUBLIC_MEMORA_E2E === "true" ||
    env.NEXT_PUBLIC_MEMORA_DISABLE_REMOTE_ANNOUNCEMENTS === "true" ||
...
export function isDevOrE2EAppIdentity(
  ...
  return (
    normalizedIdentifier === "github.com/shrirampai3000/memora.dev" ||
    normalizedIdentifier.endsWith(".dev") ||
    normalizedIdentifier.includes("e2e") ||
    normalizedName === "MEMORA-app" ||
...
export function safeAnnouncementExternalUrl(raw: unknown): string | null {
  ...
  try {
    ...
    const memoraGithub =
      host === "github.com" &&
      (url.pathname === "/MEMORA" ||
        url.pathname.startsWith("/MEMORA/"));
    return firstParty || memoraGithub ? url.toString() : null;
  } catch {
    return null;
  }
}
...
// â”€â”€ dismissal persistence (localStorage, SSR/Tauri-static-export safe) â”€â”€â”€â”€â”€â”€
