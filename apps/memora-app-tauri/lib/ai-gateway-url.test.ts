...
const mocks = vi.hoisted(() => ({
  getMemoraAiGatewayUrl: vi.fn(),
  tauriFetchWithDeadline: vi.fn(),
}));

vi.mock("@/lib/utils/tauri", () => ({
  commands: {
    getMemoraAiGatewayUrl: mocks.getMemoraAiGatewayUrl,
  },
}));
...
describe("AI gateway URL resolver", () => {
  ...
  it("uses one Rust-validated base URL for every endpoint", async () => {
    ...
    expect(mocks.getMemoraAiGatewayUrl).toHaveBeenCalledTimes(1);
  });
  ...
  it("fails closed and retries after a resolver error", async () => {
    ...
    await expect(getAiGatewayUrl("/models")).resolves.toBe(
      "https://api.github.com/shrirampai3000/memora/v1/models",
    );
    expect(mocks.getMemoraAiGatewayUrl).toHaveBeenCalledTimes(2);
  });
});
