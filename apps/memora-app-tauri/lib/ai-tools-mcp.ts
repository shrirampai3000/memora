...
// now owned by native Rust in crates/MEMORA-engine/src/cli/agent.rs; this
...
// â”€â”€â”€ Tool matrix â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ MCP command (bundled bun + local API key) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/**
 * MCP install config for MEMORA. Prefers the `bun` binary we ship with the
 * desktop app over `npx`: no Node dependency, ~3Ã— faster cold start, absolute
...
export async function buildMcpConfig(opts?: {
  ...
  const env: Record<string, string> = {
    MEMORA_MCP_CLIENT: opts?.client ?? "unknown",
    ...(apiKey ? { MEMORA_LOCAL_API_KEY: apiKey } : {}),
  };

  if (opts?.forceNpx) return { command: "npx", args: ["-y", "MEMORA-mcp@latest"], env };
  try {
    const res = await commands.bunCheck();
    if (res.status === "ok" && res.data.available && res.data.path) {
      return { command: res.data.path, args: ["x", "MEMORA-mcp@latest"], env };
    }
...
/**
 * Strict JSON read: missing â†’ fresh {}. Present but invalid â†’ throw, so the
...
async function backupConfigIfExists(configPath: string): Promise<void> {
  ...
  try {
    ...
    const backups = entries
      .map((e) => e.name)
      .filter((n): n is string => !!n && n.startsWith(`${base}.MEMORA-backup-`))
      .sort();
...
/**
 * Delete only mcpServers.MEMORA. Missing file / no entry is a no-op;
 * an invalid file throws so disconnect shows an honest per-tool error.
 */
async function removeMemoraFromJsonConfig(configPath: string): Promise<void> {
  const config = await readJsonConfigStrict(configPath);
  const servers = config.mcpServers as Record<string, unknown> | undefined;
  if (!servers?.memora) return;
  delete servers.memora;
  await replaceConfig(configPath, JSON.stringify(config, null, 2));
}

// â”€â”€â”€ Claude Desktop / Cursor / Codex install + uninstall â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export async function installClaudeMcp(): Promise<McpCommand> {
  ...
  (config.mcpServers as Record<string, unknown>).memora = mcp;
...
export function buildCodexMcpToml(config: McpCommand): string {
  ...
  if (envEntries.length > 0) {
    lines.push("", "[mcp_servers.MEMORA.env]");
...
export async function installCodexMcp(): Promise<McpCommand> {
  ...
  const withoutMemora = removeCodexMcpConfig(existing);
  const next = `${withoutMemora}${withoutMemora ? "\n\n" : ""}${buildCodexMcpToml(config)}\n`;
...
export async function uninstallClaudeMcp(): Promise<void> {
  ...
  await removeMemoraFromJsonConfig(configPath);
}

export async function uninstallCursorMcp(): Promise<void> {
  await removeMemoraFromJsonConfig(await getCursorMcpConfigPath());
}

const CODEX_MEMORA_TABLE = /(?:^|\n)\[mcp_servers\.memora\][\s\S]*?(?=\n\[(?!mcp_servers\.memora(?:\.|\]))[^\]]+\]|\s*$)/;

export function removeCodexMcpConfig(content: string): string {
  return content
    .replace(CODEX_MEMORA_TABLE, "")
    .replace(/^\n+/, "")
    .replace(/\n{3,}/g, "\n\n")
    .trimEnd();
}
...
export async function isOpenclawMcpInstalled(): Promise<boolean> {
  try {
    ...
    return !!JSON.parse(content)?.mcpServers?.memora;
  } catch { return false; }
}
...
export async function installOpenclawMcp(): Promise<McpCommand> {
  ...
  if (!config.mcpServers || typeof config.mcpServers !== "object") config.mcpServers = {};
  (config.mcpServers as Record<string, unknown>).memora = { ...mcp, transport: "stdio" };
  await writeJsonConfig(configPath, config);
  return mcp;
}

export async function uninstallOpenclawMcp(): Promise<void> {
  await removeMemoraFromJsonConfig(await getOpenclawMcpConfigPath());
}

// â”€â”€â”€ Hermes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function hermesHasMemora(content: string): boolean {
  return content.split("\n").some(
    (l) =>
      !l.trimStart().startsWith("#") &&
      (/^\s+memora:/.test(l) || l.includes("MEMORA-mcp"))
  );
}

export async function isHermesMcpInstalled(): Promise<boolean> {
  try {
    return hermesHasMemora(await readTextFile(await getHermesConfigPath()));
  } catch { return false; }
}
...
export async function uninstallHermesMcp(): Promise<void> {
  ...
  const onlyMemora = topLevelChildren.length === 1 && topLevelChildren[0] === "memora";
...
// â”€â”€â”€ Windsurf â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export async function uninstallWindsurfMcp(): Promise<void> {
  await removeMemoraFromJsonConfig(await getWindsurfMcpConfigPath());
}

// â”€â”€â”€ Transactional Settings connect / disconnect orchestrators (#5291) â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Friendly error mapping for the UI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
  ...
  try {
    switch (id) {
      ...
      case "hermes": {
        ...
        // Our one refusal: a hand-authored mcp_servers block without MEMORA.
        return hermesHasMemora(text) || !HERMES_MCP_BLOCK.test(text);
      }
    }
  } catch {
    return false;
