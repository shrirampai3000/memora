...
// Route a memora:// deeplink to the window that can handle it. Meeting
// deeplinks belong to the Home window's meetings page; everything else to Main.
export function isMeetingDeeplink(url: string) {
  return url.startsWith("memora://meeting/") ||
    url.startsWith("memora://meeting?");
}
...
export async function executeNotificationAction(
  ...
  switch (action.type) {
    ...
    case "api": {
      if (action.url) {
        // An `api` action calls back into the local memora API with the
    ...
    case "source": {
      const sourceUrl =
        ...
        ctx.sourceUrl;
      if (sourceUrl) {
        if (sourceUrl.startsWith("memora://")) {
