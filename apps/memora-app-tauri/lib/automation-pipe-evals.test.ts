...
describe("Automate My Work evaluations", () => {
  ...
  it("injects the existing inventory and requires one evidence-backed proposal before writes", () => {
    ...
    expect(prompt).toContain("Authorization: Bearer $MEMORA_LOCAL_API_KEY");
    expect(prompt).toContain("MEMORA_API_KEY");
  ...
  it("keeps the fresh-install template on the same evidence-first contract", () => {
    const bundledTemplate = readFileSync(
      resolve(__dirname, "../../../crates/MEMORA-core/assets/pipes/automate-my-work/pipe.md"),
      "utf8",
    );
    ...
    expect(bundledTemplate).toContain("Authorization: Bearer $MEMORA_LOCAL_API_KEY");
