...
vi.mock("@/lib/web-url", () => ({
  ...
  memoraWebBase: (fallbackHost: string) => mocks.webBaseOverride.value ?? fallbackHost,
  memoraWebUrl: (path: string, fallbackHost: string) =>
...
describe("tauriFetchWithDeadline", () => {
  ...
  it("sends a deadline signal and a connect timeout the plugin never sets itself", async () => {
    await tauriFetchWithDeadline("https://github.com/shrirampai3000/memora/api/enterprise/policy", {
      method: "GET",
  ...
  it("clones the caller's init, which the plugin mutates", async () => {
    ...
    await tauriFetchWithDeadline("https://github.com/shrirampai3000/memora/1", init);
    await tauriFetchWithDeadline("https://github.com/shrirampai3000/memora/2", init);
  ...
  it("preserves the response the plugin produced", async () => {
    mocks.fetch.mockImplementation(async () => {
      ...
      Object.defineProperty(res, "url", { value: "https://github.com/shrirampai3000/memora/final" });
      return res;
    });

    const res = await tauriFetchWithDeadline("https://github.com/shrirampai3000/memora/api/enterprise/policy");
    expect(res.status).toBe(201);
    expect(res.statusText).toBe("Created");
    expect(res.headers.get("x-trace")).toBe("abc");
    expect(res.url).toBe("https://github.com/shrirampai3000/memora/final");
  ...
  it("never lets the connect timeout outlive the overall deadline", async () => {
    await tauriFetchWithDeadline("https://github.com/shrirampai3000/memora/x", undefined, { timeoutMs: 2_000 });
  ...
  it("honors a connect timeout the caller passed through init", async () => {
    ...
    await tauriFetchWithDeadline("https://github.com/shrirampai3000/memora/x", { connectTimeout: 2_000 });
    expect(mocks.fetch.mock.calls[0][1].connectTimeout).toBe(2_000);
  });

  it("clamps a connect timeout passed through init, not just through options", async () => {
    await tauriFetchWithDeadline(
      "https://github.com/shrirampai3000/memora/x",
  ...
  it("releases the deadline immediately for a bodyless response", async () => {
    ...
    const res = await tauriFetchWithDeadline("https://github.com/shrirampai3000/memora/x", { method: "DELETE" });
  ...
  it("releases the deadline when the request itself fails", async () => {
    ...
    await expect(tauriFetchWithDeadline("https://github.com/shrirampai3000/memora/x")).rejects.toThrow(
