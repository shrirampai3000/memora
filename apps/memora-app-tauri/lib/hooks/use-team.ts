...
const API = memoraWebUrl("/api/team", "https://github.com/shrirampai3000/memora");
...
export function useTeam() {
  ...
  const createInviteLink = useCallback(
    async (teamName: string, key: CryptoKey, forceNew = false) => {
      ...
      return memoraWebUrl(`/join/${tokenData.invite_token}#key=${encodeURIComponent(base64Key)}`, "https://github.com/shrirampai3000/memora");
    },
    [headers]
  );
  ...
  const joinTeam = useCallback(
    async (
      ...
      if (opts.base64Key) {
        ...
        throw new Error(
          "this looks like a notification-only invite link (no encryption key). ask your admin to open their desktop app â†’ settings â†’ team â†’ copy invite link, and share that link directly with you (slack/imessage as plain text)."
        );
      }
