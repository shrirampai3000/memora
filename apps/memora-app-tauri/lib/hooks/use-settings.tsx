...
export type Settings = SettingsStore & {
	 ...
	 * video frames, and images from memora data. Default true. When
	 * false, the "Cloud audio + video + image analysis" section is
	 * stripped from `~/.claude/skills/MEMORA-api/SKILL.md` so agents
	 ...
	 *   a "+ HD" action. Click â†’ starts a meeting-bound session that
	 ...
	 * Default 100 â‰ˆ 10 fps. Clamped to >= 33 ms (30 fps ceiling). */
	...
	/** Global blanket permission: allow memora to copy browser cookies
...
const applyProCloudAudioDefaults = (settings: Settings): Settings => {
	...
	const userChoseCustomEngine =
		...
		settings.audioTranscriptionEngine !== "MEMORA-cloud";
	...
	settings.meetingLiveTranscriptionProvider = "MEMORA-cloud";
...
export function createDefaultSettingsObject(): Settings {
	try {
		...
		DEFAULT_SETTINGS.showMemoraShortcut = p === "windows" ? "Alt+S" : "Control+Super+S";
...
async function waitForE2eSettingsWriteDelay(): Promise<void> {
	if (
		process.env.NEXT_PUBLIC_MEMORA_E2E !== "true" ||
...
export const getStore = async () => {
	if (!_store) {
		_store = (async () => {
			...
			// store.bin as Rust (get_base_dir honors MEMORA_DATA_DIR); a
			// hardcoded ~/.memora here splits the settings store in two
			// whenever that override is set.
			let baseDir: string | null = null;
			try {
				const res = await commands.getMemoraBaseDir();
				if (res.status === "ok") {
					baseDir = res.data;
				} else {
					console.warn("get_MEMORA_base_dir failed, using ~/.memora:", res.error);
				}
			} catch (e) {
				console.warn("get_MEMORA_base_dir unavailable, using ~/.memora:", e);
			}
			if (!baseDir) {
				baseDir = `${await homeDir()}/.memora`;
			}
...
function createSettingsStore() {
	const get = async (): Promise<Settings> => {
		...
		if (!(settings as any)._presetsSeededForUser && settings.user?.token) {
			...
			const isAnonymousPlaceholder =
				presets.length === 1 &&
				(presets[0] as any)?.id === MEMORA_PRESET_ID &&
				(presets[0] as any)?.provider === "MEMORA-cloud";
		...
		// Migration: Rename "pi" provider to "MEMORA-cloud" for clarity
		if (settings.aiPresets?.some((p: any) => p.provider === "pi")) {
			settings.aiPresets = settings.aiPresets.map((p: any) =>
				p.provider === "pi" ? { ...p, provider: "MEMORA-cloud" } : p
			);
			needsUpdate = true;
		}

		// Migration: Rename "pi-agent" preset id to "MEMORA-cloud"
		if (settings.aiPresets?.some((p: any) => p.id === "pi-agent")) {
			settings.aiPresets = settings.aiPresets.map((p: any) =>
				p.id === "pi-agent" ? { ...p, id: "MEMORA-cloud" } : p
			);
			needsUpdate = true;
		}
		...
		// - macOS â†’ whisper-large-v3-turbo-quantized
		// - Windows/Linux â†’ parakeet
		// Does NOT set MEMORA-cloud here because user may not be logged in yet.
		...
		if (!(settings as any)._parakeetDefaultMigrationDone) {
			...
			if (isWhisperVariant || engine === "MEMORA-cloud" || engine === "parakeet") {
		...
		// both background and live transcription to Memora Cloud. The marker
		...
		//   - only if provider is still MEMORA-cloud and model is exactly the seeded
	...
	const set = (value: Partial<Settings>) =>
		enqueueSettingsStoreWrite(async () => {
			...
			if ("user" in value) {
				// On logout / Proâ†’non-Pro transition, clear the V2 marker so a future
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	...
	useEffect(() => {
		const loadSettings = async () => {
			try {
				...
				// such as MEMORA_PORT. Refresh after applying the persisted
	...
	// Install global fetch interceptor to catch 401s from github.com/shrirampai3000/memora
	...
	useEffect(() => {
		const unlistenPromise = listen("MEMORA-auth-signout", () => {
	...
	useEffect(() => {
		...
		//  - the environment guard (CI / MEMORA_DISABLE_TELEMETRY), which is
	...
	// Upgrade the seeded "chat" preset Sonnet â†’ Opus 5 the moment the user
	...
	const updateSettings = async (updates: Partial<Settings>) => {
		...
		if (clearsAccount) {
			...
			emit("MEMORA-auth-signout").catch(() => {});
		}
	...
	const loadUser = async (token: string, verify = false) => {
		...
		try {
			const response = await fetch(
				memoraWebUrl("/api/user", "https://github.com/shrirampai3000/memora"),
				{
					method: "POST",
					headers: {
			...
			if (!settings.user?.id) {
				...
				// Bridge app identity â†’ website identity via email alias
