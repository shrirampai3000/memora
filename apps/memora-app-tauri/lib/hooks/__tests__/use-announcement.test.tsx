...
const {
  ...
  appName: { current: "memora" },
  appIdentifier: { current: "github.com/shrirampai3000/memora" },
}));
...
describe("useAnnouncement", () => {
  ...
  it("suppresses remote flag announcements in the development app identity", async () => {
    appName.current = "memora - Development";
    appIdentifier.current = "github.com/shrirampai3000/memora.dev";
    flagPayload.current = FLAG;
  ...
  it("a /notify push (announcement event) overrides and bypasses dismissal", async () => {
    // flag is present AND already dismissed â€” the push must still win
    flagPayload.current = FLAG;
    window.localStorage.setItem(
      "MEMORA-announcements-dismissed-v1",
  ...
  it("activateCta opens external urls in the system browser", async () => {
    flagPayload.current = {
      ...FLAG,
      cta: { label: "read more", url: "https://github.com/shrirampai3000/memora/blog" },
    };
    const { result } = renderHook(() => useAnnouncement());
    await flushAnnouncementEffects();
    ...
    expect(openMock).toHaveBeenCalledWith("https://github.com/shrirampai3000/memora/blog");
  ...
  it("activateCta opens the install dialog for a pipe cta instead of installing", async () => {
    ...
    // the same event a `memora://install-pipe` deep link emits, so the
  ...
  it("prefers the pipe destination over route and url on the same cta", async () => {
    flagPayload.current = {
      ...
      cta: {
        ...
        route: "/settings",
        url: "https://github.com/shrirampai3000/memora/blog",
      },
    };
    const { result } = renderHook(() => useAnnouncement());
    await flushAnnouncementEffects();
