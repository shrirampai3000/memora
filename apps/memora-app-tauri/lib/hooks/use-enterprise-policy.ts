...
const ACCOUNT_LOGIN_REQUIRED_ERROR =
  "your organization requires signing in with your memora account";
...
/**
 * Same check when NEXT_PUBLIC_MEMORA_WEB_URL points at a control plane on
...
export const E2E_ENTERPRISE_POLICY_KEY = "MEMORA_e2e_enterprise_policy";
export const E2E_ENTERPRISE_HEARTBEAT_STATUS_KEY = "MEMORA_e2e_enterprise_heartbeat_status";
export const E2E_ENTERPRISE_SKIP_SAVED_LICENSE_KEY =
  "MEMORA_e2e_enterprise_skip_saved_license";

function enterpriseE2eMocksEnabled(): boolean {
  return process.env.NEXT_PUBLIC_MEMORA_E2E === "true" && typeof window !== "undefined";
}
...
function toLocalAiPreset(
  ...
  const provider = preset.provider === "MEMORA-cloud" ? "MEMORA-cloud" : preset.provider;
  return {
    id: preset.id,
...
async function applyManagedDeviceSettings(lockedSettings: Record<string, unknown>): Promise<void> {
  ...
  try {
    await withTimeout(
      "enterprise input capture stopMemora",
      commands.stopMemora(),
      ENGINE_RESTART_COMMAND_TIMEOUT_MS
    );
    await new Promise((resolve) => setTimeout(resolve, 1000));
    await withTimeout(
      "enterprise input capture spawnMemora",
      commands.spawnMemora(null),
      ENGINE_RESTART_COMMAND_TIMEOUT_MS
    );
  } finally {
...
async function sendHeartbeat(
  ...
  try {
    ...
    const res = await tauriFetchWithDeadline(
      memoraWebUrl("/api/enterprise/heartbeat", "https://github.com/shrirampai3000/memora"),
      {
        method: "POST",
        headers: {
          ...credentialHeaders,
...
export function useEnterprisePolicyRuntime() {
  ...
  const fetchPolicy = useCallback(async (
    ...
    try {
      ...
      // Fallback: read directly from ~/.MEMORA/auth.json when the
      ...
      if (e2ePolicy.present) {
        ...
        const res = await tauriFetchWithDeadline(
          memoraWebUrl("/api/enterprise/policy", "https://github.com/shrirampai3000/memora"),
          {
            method: "GET",
            headers,
      ...
      if (result.aiPresetPolicy) {
        try {
          ...
          console.log(
            `[enterprise] applied AI preset policy: cloud=${result.aiPresetPolicy.allow_MEMORA_cloud}, employee=${result.aiPresetPolicy.allow_employee_custom_presets}, managed=${result.aiPresetPolicy.managed_presets.length}`
          );
        } catch (e) {
      ...
      // Persist admin status into ~/.MEMORA/enterprise.json so the
  ...
  const authenticateCredential = useCallback(async (
    ...
    if (result.ok) {
      ...
      // employee to sign in with their memora account. Device policy was
   ...
   * API, saves it to ~/.MEMORA/enterprise.json, and starts fetching policy.
   */
  ...
  useEffect(() => {
    ...
    (async () => {
      // Read license key from enterprise.json (MDM or ~/.MEMORA/)
