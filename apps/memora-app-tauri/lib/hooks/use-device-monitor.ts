...
export function useDeviceMonitor() {
    ...
    (async () => {
      ...
      for (const d of withoutLocal) {
        try {
          ...
          // unreachable â€” keep it, user may want to retry later
        }
        deduped.push(d);
      }
  ...
  // Auto-discover memora instances on the network
  ...
  const discoverDevices = useCallback(async () => {
    ...
    try {
      ...
      // Probe each host on port 3030 to find memora instances
      const PROBE_TIMEOUT_MS = 3_000;
      const seenHostnames = new Set<string>();
      if (localHostname) seenHostnames.add(localHostname);

      // Try common memora ports
