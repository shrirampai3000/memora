...
async function enterprisePipesDir(): Promise<string> {
  try {
    ...
    console.warn(
      "[enterprise-pipes] failed to resolve active data dir, using ~/.memora:",
      result.error,
    );
  } catch (error) {
    console.warn(
      "[enterprise-pipes] active data dir command unavailable, using ~/.memora:",
      error,
    );
  }

  return await join(await homeDir(), ".memora", "pipes");
}
...
/**
 * Gather pipe execution statuses from the local memora API.
