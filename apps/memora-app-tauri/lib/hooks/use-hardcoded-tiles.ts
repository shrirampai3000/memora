...
export async function getInstalledMcpVersion(): Promise<string | null> {
  try {
    ...
    return config?.mcpServers?.memora ? "installed" : null;
  } catch { return null; }
}
...
const KNOWN_DEFAULT_MCP_ARGS: readonly string[][] = [
  ...
  ["x", "MEMORA-mcp@latest"],  // bundled bun / bunx (current)
  ["x", "MEMORA-mcp"],         // bundled bun / bunx (older)
];
...
export function isStaleClaudeMemoraEntry(entry: unknown): boolean {
  ...
  const hasKey = !!e.env?.MEMORA_LOCAL_API_KEY;
  const hasClaudeClient = e.env?.MEMORA_MCP_CLIENT === "claude";
  return !hasKey || !hasClaudeClient;
}

// Read the currently-installed Claude memora entry (or null). Used to decide
// whether an auto-repair rewrite is warranted.
export async function getInstalledClaudeMemoraEntry(): Promise<unknown | null> {
  try {
    const configPath = await getClaudeConfigPath();
    if (!configPath) return null;
    const config = JSON.parse(await readTextFile(configPath));
    return config?.mcpServers?.memora ?? null;
  } catch { return null; }
}
...
export function hasEnabledCodexMcp(content: string): boolean {
  const table = content.match(CODEX_MEMORA_TABLE)?.[0] ?? "";
...
export function hasEnabledGrokMcp(content: string): boolean {
  try {
    ...
    return servers.some((s) => s?.id === "memora" && s?.enabled !== false);
  } catch { return false; }
}
