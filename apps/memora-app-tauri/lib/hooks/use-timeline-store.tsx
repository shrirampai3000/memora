...
// â†’ negligible CPU; no DB polling. See timeline-liveness.ts.
let lastMessageAt = 0;
let livenessTimer: ReturnType<typeof setInterval> | null = null;
const LIVENESS_CHECK_INTERVAL_MS = 10000; // how often the watchdog ticks
// 1.5Ã— the 30s server keep-alive: tolerates one dropped keep-alive plus event-loop
	...
	connectWebSocket: () => {
		...
		ws.onerror = () => {
			...
			if (connectionAttempts < MAX_SILENT_RETRIES) {
				...
				set({ 
					isLoading: currentFrames.length === 0, 
					message: currentFrames.length === 0 ? "connecting to MEMORA..." : null,
					isConnected: false,
				});
