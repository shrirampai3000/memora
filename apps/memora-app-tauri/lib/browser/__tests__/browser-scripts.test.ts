...
function readBrowserScript(name: string): string {
  let dir = process.cwd();
  for (let i = 0; i < 8; i++) {
    const cand = path.join(dir, "crates/MEMORA-engine/src/browser_scripts", name);
...
describe("act.js â€” deterministic actions resolve by ref and mutate the DOM", () => {
  ...
  it("fill sets the value via the native setter and fires input + change", async () => {
    ...
    const res = await runAct(refOf(email), "fill", "ada@github.com/shrirampai3000/memora");
    expect(res.ok).toBe(true);
    expect(email.value).toBe("ada@github.com/shrirampai3000/memora");
