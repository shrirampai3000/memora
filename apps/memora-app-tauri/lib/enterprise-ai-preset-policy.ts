...
function normalizeManagedPreset(value: unknown, index: number): EnterpriseManagedAiPreset | null {
  ...
  const model =
    provider === "MEMORA-cloud"
      ? stringValue(value.model, "MEMORA-cloud")
  ...
  return {
    ...
    provider,
    url: provider === "MEMORA-cloud" ? "" : stringValue(value.url),
    model,
    api_key:
      provider === "MEMORA-cloud" || provider === "native-ollama" || provider === "openai-chatgpt"
        ? ""
...
export function normalizeEnterpriseAiPresetPolicy(value: unknown): EnterpriseAiPresetPolicy {
  ...
  const allowMemoraCloud = value.allow_MEMORA_cloud !== false;
  ...
  const validDefaults = new Set([
    ...(allowMemoraCloud ? ["MEMORA-cloud"] : []),
    ...managedPresets.map((preset) => preset.id),
  ]);

  return {
    version: 2,
    allow_MEMORA_cloud: allowMemoraCloud,
    ...
    default_preset_id: validDefaults.has(defaultPresetId)
      ? defaultPresetId
      : managedPresets[0]?.id ?? (allowMemoraCloud ? "MEMORA-cloud" : null),
...
export function isPresetAllowedByEnterprisePolicy(
  ...
  if (preset.provider === "MEMORA-cloud") return policy.allow_MEMORA_cloud;
