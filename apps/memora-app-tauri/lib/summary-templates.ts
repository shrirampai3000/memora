...
export function buildAutomateMyWorkPrompt(existingPipes: AutomationPipeInventory[] = []) {
  return `<role>
You are a memora automation expert. Find one repeated, costly workflow that could become a useful LOW-RISK automation ("pipe"). Your first job is discovery, not creation. A repair recommendation or no recommendation is better than manufacturing a generic pipe.
</role>

Read the memora skill first so you know the API and how pipes work. During discovery, use progressive disclosure and the memora API only. Authenticate every localhost:3030 request with \`Authorization: Bearer $MEMORA_LOCAL_API_KEY\`; \`MEMORA_API_KEY\` is for the hosted API and is not proof that the local API is misconfigured. Never estimate time from frame counts and never use /raw_sql for this task.
...
The recommendation must be read-only: it may query memora and write one declared result inside its own output directory, but it must not send messages, call outbound services, modify user files, or take destructive action.
...
## Repeated workflow
**Trigger â†’ current manual steps â†’ desired result**
...
**Inputs:** [specific local memora data]
 ...
 * pipe.md files in crates/MEMORA-core/assets/pipes/.
 */
export const FALLBACK_TEMPLATES: TemplatePipe[] = [
  ...
  },
  {
    ...
    icon: "\u{1F4CB}",
    featured: true,
    prompt: `Analyze my screen and audio recordings from today (last 16 hours). Read the memora skill first. Use limit=10 per search, max 5 searches total. For app-usage totals, aggregate by app over the time range using whatever memora query tool you have (a COUNT/GROUP BY query or the activity summary). Use only memora's recorded data, not this project's files or other apps' source.
