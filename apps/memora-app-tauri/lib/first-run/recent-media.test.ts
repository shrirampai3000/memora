...
// Real naming, verified against ~/.MEMORA/data/data/<date>/: compacted
...
const screenChunk =
  "/Users/x/.MEMORA/data/data/2026-08-08/compact_monitor_1_1786151334622.mp4";
const audioChunk =
  "/Users/x/.MEMORA/data/MacBook Pro Microphone (input)_2026-08-07_18-00-00.mp4";
...
describe("classifyMediaPath", () => {
  it("treats a screen chunk as video and a still frame as image", () => {
    ...
    expect(
      classifyMediaPath("/Users/x/.MEMORA/data/data/2026-08-08/1786151574972_m1.jpg"),
    ).toBe("image");
    expect(classifyMediaPath("/Users/x/.MEMORA/data/frame_12.png")).toBe("image");
  });

  // memora writes microphone and system audio as mp4 as well. Playing the
  ...
  it("never treats an audio capture as screen media", () => {
    ...
    expect(
      classifyMediaPath("/Users/x/.MEMORA/data/System Audio (output)_2026-08-07_18-00-00.mp4"),
    ).toBeNull();
  });
...
describe("preserveFirstRunMedia", () => {
  const still = {
    path: "/Users/x/.MEMORA/data/data/2026-08-08/1786151574972_m1.jpg",
    kind: "image" as const,
    appName: "Arc",
  };
  ...
  it("copies a still out of the capture dir, keeping its extension", async () => {
    ...
    expect(preserved.path).toMatch(
      /^\/Users\/x\/\.memora\/first-run\/summary-\d+\.jpg$/,
    );
