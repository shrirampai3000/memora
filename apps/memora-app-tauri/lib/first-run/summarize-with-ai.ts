...
function buildSummaryPrompt(facts: string): string {
  return `You are MEMORA. You have been recording this person's screen for a few minutes, right after they finished setup. Below is EVERYTHING you observed. Write the first thing you ever say to them about their own work.
...
export async function summarizeFirstRunWithAi(
  ...
  try {
    ...
    const dir = await join(home, ".memora", SUMMARY_PROJECT_DIR);
