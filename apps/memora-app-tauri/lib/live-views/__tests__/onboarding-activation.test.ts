...
describe("onboarding Live View activation", () => {
  ...
  it("reads and then clears browser-only goal keys after settings migration", () => {
    window.localStorage.setItem("MEMORA.home.focus.v1", "work_patterns");

    expect(readLegacyUserGoalCategory()).toBe("work_patterns");
    expect(window.localStorage.getItem("MEMORA.home.focus.v1")).toBe(
      "work_patterns",
    );

    clearLegacyUserGoalCategory();

    expect(window.localStorage.getItem("MEMORA.home.focus.v1")).toBeNull();
  });
