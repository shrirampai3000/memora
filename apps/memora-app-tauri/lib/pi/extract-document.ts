...
// sidecar RPC only carries text + image content â€” there is no document channel
...
// drag-drop accept the same set â€” previously yaml/xml/html/ini/toml were
...
// DOCX, XLSX, and XLS-as-OOXML are all ZIP archives â€” the magic bytes
...
// ("Can't find end of central directory â€” is this a zip file?"). That
...
// developer to read the docs â€” not something a chat-toast should
...
async function extractRaw(bytes: Uint8Array, ext: string): Promise<string> {
  ...
  if (e === "pdf") {
    ...
    if (bytes.byteLength < 5 ||
        ...
        bytes[4] !== 0x2d) {
      throw new Error("not a valid .pdf (file is missing the PDF signature â€” it may be corrupted or a placeholder)");
    }
  ...
  if (e === "docx") {
    if (!looksLikeZip(bytes)) {
      ...
      throw new Error("not a valid .docx (file is not a Word OOXML archive â€” it may be a legacy .doc, corrupted, or a placeholder)");
    }
  ...
  if (e === "xlsx") {
    if (!looksLikeZip(bytes)) {
      throw new Error("not a valid .xlsx (file is not an Excel OOXML archive â€” it may be a legacy .xls, corrupted, or a placeholder)");
    }
  ...
  if (e === "xls") {
    // Legacy .xls is OLE binary, NOT a zip â€” SheetJS handles it.
    const mod: any = await import("xlsx");
    const XLSX = mod.default ?? mod;
    const wb = XLSX.read(bytes, { type: "array" });
    const parts: string[] = [];
...
export async function extractDocument(
  ...
  // utf-8 text â€” a literal NUL flips git/grep into binary-file mode.
...
export function docToPromptText(doc: ExtractedDoc): string {
  const notice = doc.truncated
    ? `\n\n[â€¦truncated â€” original was ${doc.charCount.toLocaleString()} characters]`
    : "";
