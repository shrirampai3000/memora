...
describe("normalizeComposerMentionsForModel", () => {
  ...
  const skills = [
    { name: "deep-research", path: "/Users/u/.MEMORA/skills/deep-research" },
  ];
  ...
  it("resolves content type, app and tags into one context block", () => {
    ...
    expect(result.modelInput).toContain("<MEMORA_query_context>");
  ...
  it("strips the resolved tokens from the sentence the model reads", () => {
    ...
    const sentence = result.modelInput.split("</MEMORA_query_context>")[1]?.trim();
  ...
  it("turns a known skill token into a loadable path", () => {
    ...
    expect(result.modelInput).toContain("path: /Users/u/.MEMORA/skills/deep-research");
