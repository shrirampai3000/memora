...
export interface SourceCitation {
  ...
  // local (memora) citation, the footer makes the row jump into the
...
function sourceCitationsFromToolCall(toolCall: ToolCallLike | undefined): SourceCitation[] {
  ...
  // Memora timeline just because its bare name collides with a memora
  ...
  const isForeignMcp = mcpServer !== null && mcpServer !== "memora";
  ...
  if (toolName === "MEMORA_search") {
    return [memoraToolCitation(args)];
  }

  // memora MCP tools (ACP agents surface them as mcp__memora__<name>,
...
function memoraToolCitation(args: Record<string, unknown>): SourceCitation {
  ...
  return {
    id: stableId(["MEMORA-search-tool", contentType, appName, query, range]),
    kind: "memora",
    title: "Memora search",
...
function bashCitations(command: string, resultText: string): SourceCitation[] {
  ...
  for (const call of extractMemoraApiCalls(command)) {
    citations.push(memoraApiCitation(call));
  }

  for (const link of extractWebLinks(command)) {
    if (isLocalMemoraUrl(link.url)) continue;
    citations.push({
      id: stableId(["web", link.url]),
      kind: "web",
...
function memoraApiCitation(call: string): SourceCitation {
  const path = extractPath(call);
  const query = extractQuery(call);
  const title = memoraTitle(path);
  const kind = memoraKind(path);
  ...
  const timestamp = kind === "memora" ? navTimestamp(params.start_time) : undefined;
  const searchText = path === "/search" ? params.q : undefined;

  return {
    id: stableId(["memora", path, query]),
    kind,
    title,
    subtitle: memoraSubtitle(path, query),
    timestamp,
    query: searchText,
  };
}

function memoraKind(path: string): SourceCitationKind {
...
function memoraTitle(path: string): string {
  if (path === "/search") return "Memora search";
  if (path === "/activity-summary") return "Activity summary";
  if (path === "/raw_sql") return "Local database query";
  if (path === "/memories") return "Memora memories";
  ...
  if (path === "/health") return "Memora health";
  if (path.startsWith("/connections/")) return `${titleCase(path.split("/")[2] ?? "connector")} connection`;
  return "Memora API";
}

function memoraSubtitle(path: string, query: string): string | undefined {
  const params = queryParams(query);
  if (path === "/search") {
    const parts = [
  ...
  if (path.startsWith("/connections/perplexity/")) {
    return "external web context via Memora connection";
  }
...
function fileKind(path: string): SourceCitationKind {
  ...
  if (path.includes("/.MEMORA/pipes/")) return "pipe";
  if (path.includes("/.MEMORA/chats/")) return "memora";
  return "file";
}
...
function normalizeToolName(toolName: string): string {
  return toolName
    ...
    // mcp__memora__search-content) or a human title ("Grep"); strip the MCP
...
function extractMemoraApiCalls(command: string): string[] {
...
function isSourceKind(kind: string): kind is SourceCitationKind {
  return ["memora", "database", "connector", "web", "file", "memory", "pipe", "command"].includes(kind);
}
...
function isLocalMemoraUrl(url: string): boolean {
  try {
