...
declare global {
  interface Window {
    __MEMORA_BROWSER_RUNTIME_INSTALLED__?: boolean;
  }
}

export function installBrowserRuntime() {
  const mode = parseMode(process.env.NEXT_PUBLIC_MEMORA_WEB_DEV);
  if (!mode || typeof window === "undefined") return;
  if (window.__MEMORA_BROWSER_RUNTIME_INSTALLED__) return;
  window.__MEMORA_BROWSER_RUNTIME_INSTALLED__ = true;

  const apiPort = Number(process.env.NEXT_PUBLIC_MEMORA_WEB_API_PORT) || 3030;
  const scenario = parseScenario(
    process.env.NEXT_PUBLIC_MEMORA_WEB_SCENARIO,
  );
  ...
  mockIPC(
    createBrowserIpcMock({
      mode,
      apiPort,
      apiKey: process.env.NEXT_PUBLIC_MEMORA_WEB_API_KEY,
  ...
  document.documentElement.dataset.memoraWebDev = mode;
  console.info(
