...
describe("team-crypto", () => {
  ...
  describe("invite link key flow (legacy)", () => {
    it("should simulate full invite link flow: generate â†’ export â†’ URL â†’ import â†’ decrypt", async () => {
      ...
      // simulate invite URL
      const inviteUrl = `memora://join-team?team_id=abc-123&key=${encodeURIComponent(exportedKey)}`;
  ...
  describe("passphrase-based key wrapping", () => {
    ...
    it("should round-trip wrap â†’ unwrap with correct passphrase", async () => {
    ...
    it("should simulate full secure invite flow: wrap â†’ claim â†’ unwrap â†’ decrypt", async () => {
