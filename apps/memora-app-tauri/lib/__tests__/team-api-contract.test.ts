...
/**
 * Tests that validate the app â†” backend API contract.
...
describe("team API contract", () => {
  describe("POST /api/team/create", () => {
    it("should send correct request shape", async () => {
      ...
      await mockFetch("https://github.com/shrirampai3000/memora/api/team/create", {
        method: "POST",
        headers: {
  ...
  describe("secure invite E2E flow", () => {
    it("admin wraps key â†’ uploads claim â†’ member fetches claim â†’ unwraps â†’ decrypts configs", async () => {
      ...
      const inviteLink = `memora://join-team?team_id=${MOCK_TEAM_ID}&invite_token=tok-xyz&claim=${mockClaimToken}`;
  ...
  describe("legacy invite link E2E flow (backwards compat)", () => {
    it("old invite links with raw key still work", async () => {
      ...
      const inviteLink = `memora://join-team?team_id=${MOCK_TEAM_ID}&key=${encodeURIComponent(keyBase64)}`;
      const url = new URL(inviteLink);
