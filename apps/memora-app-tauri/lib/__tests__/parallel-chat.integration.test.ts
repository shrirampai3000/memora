 ...
 *   2. switch to a new chat B mid-stream (snapshot A â†’ store)
 *   3. send a message in B, B's Pi streams
 *   4. click back to A â†’ A must show A's accumulated content,
 *      not be empty / reset
 *
 * The bugs this catches:
 *   - "switch wipes A": handler fell through to startNewConversation
 *     because disk had nothing for A â†’ reset.
 *   - "snapshot wrote []": stale-closure listener captured initial
 *     render's empty messages â†’ wrote them to store.
 *   - "router never accumulated": payload.session / payload.type was
 *     undefined â†’ router silently no-op'd, so by the time the user
...
describe("parallel chat: Louis's repro at the data layer", () => {
  it("preserves chat A's messages after switching to B and back", async () => {
    // â”€â”€ Step 1: user opens chat A (sidebar upserts), sends "hello" â”€â”€
    seed("A");
    ...
    // â”€â”€ Step 2: user clicks "+ new chat" (B) mid-stream â”€â”€
    seed("B");
    ...
    // â”€â”€ Pi for A keeps streaming. Router accumulates. â”€â”€
    await handlePiEvent({
      source: "pi",
      sessionId: "A",
    ...
    // â”€â”€ Pi for A finishes. Router fires endTurn + persists. â”€â”€
    ...
    // â”€â”€ Step 3: user types in B, Pi for B streams. â”€â”€
    const panelB = {
  ...
  it("does NOT reorder the sidebar when a backgrounded session emits text", async () => {
    ...
    // newer was created last â†’ stays at top regardless of older's activity.
