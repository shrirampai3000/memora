...
describe("source citations", () => {
  ...
  it("derives a memora search citation from bash curl calls", () => {
    const citations = sourceCitationsFromMessage({
      contentBlocks: [
        {
          type: "tool",
    ...
    expect(citations[0].kind).toBe("memora");
    expect(citations[0].title).toBe("Memora search");
    expect(citations[0].subtitle).toContain("audio");
    expect(citations[0].subtitle).toContain("query: pricing");
  });

  it("names connector calls instead of collapsing them to generic memora api", () => {
    const citations = sourceCitationsFromMessage({
      contentBlocks: [
        {
          type: "tool",
    ...
    expect(citations[0]).toMatchObject({
      ...
      subtitle: "external web context via Memora connection",
    });
  });

  it("labels memora memory endpoint citations as memory", () => {
    const citations = sourceCitationsFromMessage({
      contentBlocks: [
        {
          type: "tool",
  ...
  it("pulls structured result links out of bash tool output", () => {
      ...
      contentBlocks: [
        {
          ...
          toolCall: {
            ...
            result: JSON.stringify({
              search_results: [
                { title: "Memora docs", url: "https://docs.github.com/shrirampai3000/memora/chat" },
              ],
            }),
            isRunning: false,
          },
    ...
    expect(citations[1]).toMatchObject({
      kind: "web",
      title: "Memora docs",
      href: "https://docs.github.com/shrirampai3000/memora/chat",
    });
  });
    ...
    const citations = sourceCitationsFromMessage({
      contentBlocks: [
        ...
        },
        {
          ...
          toolCall: {
            toolName: "read",
            args: { path: "/tmp/MEMORA-source-citations/apps/MEMORA-app-tauri/components/standalone-chat.tsx" },
            result: "code",
            isRunning: false,
          },
        },
      ...
      contentBlocks: [
        {
          ...
          toolCall: {
            ...
            args: { path: "/Users/louisbeaumont/.MEMORA/pipes/lead-radar/pipe.ts" },
            result: "code",
            isRunning: false,
          },
        },
      ...
      contentBlocks: [
        {
          ...
          toolCall: {
            toolName: "bash",
            args: { command: "sed -n '1,20p' apps/MEMORA-app-tauri/components/standalone-chat.tsx" },
            result: "code",
            isRunning: false,
          },
        },
      ...
      contentBlocks: [
        {
          ...
          toolCall: {
            toolName: "web_search",
            args: { query: "memora docs" },
            result:
              "Sources:\n- [Docs](https://docs.github.com/shrirampai3000/memora/chat)\n- https://docs.github.com/shrirampai3000/memora/chat",
            isRunning: false,
          },
        },
      ],
      ...
      contentBlocks: [
        {
          ...
          toolCall: {
            ...
            result: {
              ...
              details: {
                sources: [
                  { title: "Docs", url: "https://docs.github.com/shrirampai3000/memora/chat" },
                  { title: "Duplicate", url: "https://docs.github.com/shrirampai3000/memora/chat" },
                ],
              },
            },
            isRunning: false,
  ...
  it("derives direct MEMORA_search tool citations", () => {
    const citations = sourceCitationsFromMessage({
      contentBlocks: [
        {
          type: "tool",
          toolCall: {
            toolName: "MEMORA_search",
            args: {
              content_type: "ocr",
              app_name: "Slack",
              query: "roadmap",
  ...
  it("anchors MEMORA_search tool citations to the capture moment", () => {
    const citations = sourceCitationsFromMessage({
      contentBlocks: [
        {
          type: "tool",
  ...
  it("anchors bash memora /search citations to the url-encoded start_time", () => {
    const citations = sourceCitationsFromMessage({
      contentBlocks: [
        {
          type: "tool",
  ...
  it("leaves MEMORA_search citations without a start_time unanchored", () => {
    const citations = sourceCitationsFromMessage({
      contentBlocks: [
        {
          type: "tool",
      ...
      contentBlocks: [
        {
          ...
          toolCall: {
            toolName: "read",
            args: { path: "/Users/me/.MEMORA/pipes/sync/state.json" },
            result: "{}",
            isRunning: false,
          },
        },
  ...
  describe("computeChatCitationPlan", () => {
    ...
    it("aggregates an agentic loop turn (â‰¥2 citation-bearing assistants)", () => {
      const plan = computeChatCitationPlan([
        userMsg("u1"),
    ...
    const markdown = formatSourceCitationsMarkdown([
      {
        ...
        title: "Docs",
        subtitle: "web search",
        href: "https://docs.github.com/shrirampai3000/memora/",
      },
    ]);

    expect(markdown).toContain("**Source:**");
    expect(markdown).toContain("[Docs](https://docs.github.com/shrirampai3000/memora/)");
    expect(markdown).toContain("web search");
  });
});

describe("ACP / mcp__ tool citations", () => {
  it("derives a memora citation from an mcp__memora__ search tool", () => {
    const citations = sourceCitationsFromMessage({
      contentBlocks: [
        {
          type: "tool",
          toolCall: {
            toolName: "mcp__memora__search-content",
  ...
  it("derives a memora citation from mcp__memora__activity-summary", () => {
    const citations = sourceCitationsFromMessage({
      contentBlocks: [
        {
          type: "tool",
          toolCall: {
            toolName: "mcp__memora__activity-summary",
  ...
  it("does NOT attribute a third-party MCP 'search' tool to the local timeline", () => {
    ...
    expect(
      citations.some((c: { kind?: string }) => c.kind === "memora"),
    ).toBe(false);
  });
