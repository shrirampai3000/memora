...
describe("areRemoteAnnouncementsDisabled", () => {
  ...
  it("allows remote announcements in production by default", () => {
    ...
    vi.stubEnv("NEXT_PUBLIC_MEMORA_E2E", "false");
    vi.stubEnv("NEXT_PUBLIC_MEMORA_DISABLE_REMOTE_ANNOUNCEMENTS", "false");
  ...
  it("disables remote announcements in e2e builds", () => {
    vi.stubEnv("NEXT_PUBLIC_MEMORA_E2E", "true");
    expect(areRemoteAnnouncementsDisabled()).toBe(true);
  });

  it("supports an explicit remote-announcement kill switch", () => {
    vi.stubEnv("NEXT_PUBLIC_MEMORA_DISABLE_REMOTE_ANNOUNCEMENTS", "true");
    expect(areRemoteAnnouncementsDisabled()).toBe(true);
  });
      ...
      areRemoteAnnouncementsDisabled(
        {
          ...
          NEXT_PUBLIC_MEMORA_E2E: "false",
          NEXT_PUBLIC_MEMORA_DISABLE_REMOTE_ANNOUNCEMENTS: "false",
        },
        "localhost",
      ),
    ).toBe(true);
...
describe("isDevOrE2EAppIdentity", () => {
  it.each([
    ["memora - Development", "github.com/shrirampai3000/memora.dev"],
    ["MEMORA-app", "github.com/shrirampai3000/memora.dev"],
    ["memora", "github.com/shrirampai3000/memora.e2e"],
    ["memora e2e", "github.com/shrirampai3000/memora"],
  ])("detects dev/e2e app identity: %s / %s", (name, identifier) => {
    expect(isDevOrE2EAppIdentity(name, identifier)).toBe(true);
  });

  it.each([
    ["memora", "github.com/shrirampai3000/memora"],
    ["memora beta", "github.com/shrirampai3000/memora.beta"],
    ["memora enterprise", "github.com/shrirampai3000/memora.enterprise"],
...
describe("parseAnnouncement", () => {
  ...
  it("prefers route over url but keeps url when only url is set", () => {
    const both = parseAnnouncement({ ...VALID, cta: { label: "go", url: "https://github.com/shrirampai3000/memora/blog", route: "/r" } });
    expect(both!.cta).toEqual({ label: "go", url: "https://github.com/shrirampai3000/memora/blog", route: "/r" });
    const urlOnly = parseAnnouncement({ id: "x", title: "t", body: "b", cta: { label: "go", url: "https://github.com/shrirampai3000/memora/blog" } });
    expect(urlOnly!.cta).toEqual({ label: "go", url: "https://github.com/shrirampai3000/memora/blog" });
  });
...
describe("survey payloads", () => {
  const surveyPayload = {
    ...
    survey: {
      submitLabel: "send",
      questions: [
        { id: "discovery_source", type: "single-choice", prompt: "where did you first hear about memora?", choices: [
