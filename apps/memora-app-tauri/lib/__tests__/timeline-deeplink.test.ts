...
describe("timelineTimestampFromDeepLink", () => {
  it("preserves the raw positive offset used by captured timestamps", () => {
    ...
    expect(
      timelineTimestampFromDeepLink(
        `memora://timeline?timestamp=${timestamp}`,
      ),
    ).toBe(timestamp);
  });
  ...
  it("decodes a percent-encoded positive offset", () => {
    expect(
      timelineTimestampFromDeepLink(
        "memora://timeline?timestamp=2026-07-21T18%3A18%3A46.214586%2B00%3A00",
      ),
  ...
  it("supports start_time links and prefers timestamp when both are present", () => {
    expect(
      timelineTimestampFromDeepLink(
        "memora://timeline?start_time=2026-07-20T23:58:04Z",
      ),
    ).toBe("2026-07-20T23:58:04Z");
    expect(
      timelineTimestampFromDeepLink(
        "memora://timeline?start_time=2026-07-20T23:58:04Z&timestamp=2026-07-21T18:18:46Z",
      ),
  ...
  it("rejects invalid timestamps and non-timeline links", () => {
    expect(
      timelineTimestampFromDeepLink(
        "memora://timeline?timestamp=not-a-date",
      ),
    ).toBeNull();
    expect(
      timelineTimestampFromDeepLink(
        "memora://frame/42?timestamp=2026-07-21T18:18:46Z",
      ),
    ).toBeNull();
  });
});
