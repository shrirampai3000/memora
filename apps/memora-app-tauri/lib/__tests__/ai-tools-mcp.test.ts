...
const backupsOf = (path: string) =>
  Array.from(fsMock.files.keys()).filter((p) => p.startsWith(`${path}.MEMORA-backup-`));
...
describe("safe config IO", () => {
  it("preserves unrelated servers and settings, and takes a backup", async () => {
    ...
    expect(config.mcpServers.github.com/shrirampai3000/memoramand).toBe("/app/bun");
    expect(config.mcpServers.MEMORA.env.MEMORA_LOCAL_API_KEY).toBe("sp-test");
    expect(config.mcpServers.MEMORA.env.MEMORA_MCP_CLIENT).toBe("cursor");
  ...
  it("starts fresh on a missing config without creating a backup", async () => {
    ...
    const config = JSON.parse(fsMock.files.get(CURSOR)!);
    expect(Object.keys(config.mcpServers)).toEqual(["memora"]);
    expect(backupsOf(CURSOR)).toHaveLength(0);
  });
  ...
  it("uninstall is idempotent and refuses invalid files", async () => {
    await expect(uninstallCursorMcp()).resolves.toBeUndefined(); // missing â†’ no-op

    fsMock.files.set(CURSOR, JSON.stringify({ mcpServers: { memora: {}, other: { command: "x" } } }));
    await uninstallCursorMcp();
    const config = JSON.parse(fsMock.files.get(CURSOR)!);
    expect(config.mcpServers.other.command).toBe("x");
    expect(config.mcpServers.memora).toBeUndefined();

    await expect(uninstallCursorMcp()).resolves.toBeUndefined(); // no entry â†’ no-op
...
describe("hermes yaml handling", () => {
  it("appends a real block below the shipped commented example, and removes it cleanly", async () => {
    ...
    expect(withBlock).toContain("MEMORA_LOCAL_API_KEY");
    ...
    expect(after).not.toContain("MEMORA_LOCAL_API_KEY");
  });

  it("fails visibly on a hand-authored mcp_servers block", async () => {
    fsMock.files.set(HERMES, "mcp_servers:\n  other:\n    command: x\n");

    await expect(installHermesMcp()).rejects.toThrow(/add the memora server manually/);
...
describe("transactional connect / disconnect", () => {
  ...
  it("returns the written MCP command on success", async () => {
    ...
    expect(JSON.parse(fsMock.files.get(CURSOR)!).mcpServers.memora).toBeTruthy();
  });
