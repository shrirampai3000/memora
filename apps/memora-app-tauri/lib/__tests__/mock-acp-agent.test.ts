...
const nodeExecutable = process.env.MEMORA_NODE_PATH || process.execPath;
...
class MockAcpClient {
  ...
  constructor(
    ...
    const command = process.env.MEMORA_NODE_PATH || (process.platform === "win32" ? "npx.cmd" : "npx");
    const args = process.env.MEMORA_NODE_PATH
      ? [fixturePath, `--scenario=${scenario}`]
      : ["tsx", fixturePath, `--scenario=${scenario}`];
    ...
    this.child.once("exit", (code, signal) => {
      this.exitError = new Error(`mock ACP agent exited (code=${code}, signal=${signal}): ${this.stderr.join("\n")}`);
  ...
  request(method: string, params: Record<string, unknown>, timeoutMs = 15_000): Promise<any> {
...
async function initialize(client: MockAcpClient): Promise<any> {
  return client.request("initialize", {
    ...
    clientInfo: { name: "memora ACP e2e client", version: "1.0.0" },
  });
}
...
async function newSession(client: MockAcpClient): Promise<string> {
  const result = await client.request("session/new", {
    cwd: path.resolve("."),
    mcpServers: [
      { name: "memora", command: "memora", args: ["mcp"], env: [] },
    ],
  });
...
describe("mock ACP agent protocol fixture", () => {
  ...
  it("advertises agent-managed auth and creates a session after authentication", async () => {
    ...
    try {
      const initialized = await initialize(client);
      expect(initialized.authMethods[0].id).toBe("mock-agent-auth");
      await expect(newSession(client)).rejects.toThrow("èªè¨¼ãŒå¿…è¦ã§ã™");
      await expect(
