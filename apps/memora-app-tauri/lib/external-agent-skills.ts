...
// Paths mirror the CLI's layout() in crates/MEMORA-engine/src/cli/agent.rs.
...
export async function areExternalAgentSkillsInstalled(
  ...
  try {
    await Promise.all([
      readTextFile(await join(skillsRoot, "MEMORA-api", "SKILL.md")),
      readTextFile(await join(skillsRoot, "MEMORA-cli", "SKILL.md")),
    ]);
    return true;
  } catch {
    return false;
