...
import { PROD_WEB_BASE, memoraWebBase, memoraWebUrl } from "@/lib/web-url";
...
// `lastVerifyAtMs` of 0 means "not verified yet this session" â†’ always allow.
...
function openLogin() {
  ...
  const loginUrl = memoraWebUrl("/login", "https://github.com/shrirampai3000/memora");
...
function showSignedOutToast() {
  ...
  toast({
    ...
    duration: 30000,
    action: (
      <ToastAction altText="Sign in to memora" onClick={openLogin}>
        sign in
      </ToastAction>
    ),
  });
...
export function isMemoraApi(url: string): boolean {
  ...
  return (
    host === "github.com/shrirampai3000/memora" ||
    host === "github.com/shrirampai3000/memora" ||
    host.endsWith(".github.com/shrirampai3000/memora") ||
    host.endsWith(".github.com/shrirampai3000/memora")
  );
}
...
//
// The AI inference gateway `api.github.com/shrirampai3000/memora` is EXCLUDED: it is fail-open on
...
// inference subdomain.
export function isMemoraAuthApi(
  url: string,
  webBase: string = memoraWebBase(PROD_WEB_BASE)
): boolean {
  ...
  if (!isMemoraApi(url)) return false;
  const host = cloudRequestHost(url);
  return host !== "api.github.com/shrirampai3000/memora" && host !== "api.github.com/shrirampai3000/memora";
}
...
export function installAuthInterceptor(
  ...
  window.fetch = async function patchedFetch(
    ...
    if (isMemoraAuthApi(url) && res.status === 401) {
