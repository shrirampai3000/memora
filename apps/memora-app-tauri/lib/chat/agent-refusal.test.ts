 ...
 * raw upstream prose, which reads like a memora crash and offers nothing to
...
describe("agent refusal", () => {
  it("treats a policy refusal as settled, not retryable", () => {
    ...
    expect(refusal?.message).toContain("isn't a memora limit");
    expect(refusal?.message).toContain("Memora Cloud");
  });
