...
const SAFETY_REFUSAL_MESSAGE =
  "The selected model declined this request because of its safety policy. This is not a memora outage. Start a new chat and revise the request with clear authorized context, or ask for high-level guidance.";
...
export function isHostedMemoraProvider(provider?: string | null): boolean {
  // memora's own hosted gateway (default chat preset + the Pi agent both
  // route through api.github.com/shrirampai3000/memora). A connection failure here is on us,
  // not the user's machine.
  return provider === "MEMORA-cloud" || provider === "pi";
}
...
export function buildContextOverflowMessage(errorStr = ""): string {
  ...
  if (requested && available) {
    if (available < MIN_AGENT_CONTEXT_TOKENS) {
      return `This provider exposes ${tokenFormatter.format(available)} context tokens, but Memora's agent request already needs ${tokenFormatter.format(requested)}. Increase the provider's context window (for example, Ollama num_ctx) to at least ${tokenFormatter.format(MIN_AGENT_CONTEXT_TOKENS)}, or choose a larger-context model, then retry in a new chat.`;
    }
    return `This provider accepts ${tokenFormatter.format(available)} context tokens, but the request used ${tokenFormatter.format(requested)}. Set Settings â†’ AI â†’ Advanced â†’ model context tokens to ${tokenFormatter.format(available)}, then start a new chat and retry.`;
  }
  return "This chat is too long for the selected model. Match Settings â†’ AI â†’ Advanced â†’ model context tokens to the provider's context window. Start a new chat or remove large attachments/screenshots, then retry.";
}
 ...
 * organization policy to be enabled"), which reads like a memora crash and
...
export function buildChatGptAccountIdMessage(): string {
  return "Your ChatGPT sign-in doesn't include chat access: the login token has no ChatGPT account id. This usually means an Enterprise/Business workspace where the admin hasn't enabled Codex local app access. Reconnect ChatGPT in Settings â†’ AI with a personal account, or ask your workspace admin to enable access.";
}
...
function buildGenericProviderErrorMessage(
  ...
  if (isHostedMemoraProvider(provider)) {
    ...
    if (normalized.includes("free_plan_hosted_background_disabled")) {
      return "memora cloud AI for background scheduled tasks requires a paid plan. You can still run this scheduled task with Ollama or your own provider key.";
    }
    if (normalized.includes("free_chat_client_update_required")) {
      return "Update memora to use your 2 daily free AI messages.";
    }
  ...
  if (
    ...
    return "The custom AI provider rejected the request. Check the API key and Custom URL in Settings â†’ AI, including any required API path such as /v1, then run Test Connection.";
  }
...
export function buildProviderErrorPresentation(
  ...
  // text: the raw upstream prose reads like a memora crash and says nothing
