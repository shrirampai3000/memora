...
- "upcoming meetings / calendar events / what's on my calendar / schedule" â†’ if a calendar integration is connected (google-calendar, apple-calendar), call its events endpoint first; only fall back to audio search if no calendar is connected
- "meeting / call / conversation / what did I/they say" â†’ search with content_type: "audio", no q param (for past meetings/calls captured by memora)
- "how long / time spent / which apps / most used" â†’ activity-summary (not raw frame counts or SQL)
- "what was on screen / what was I reading" â†’ search with content_type: "all" or "accessibility"
- "what was I doing / recent activity / summarize my day" â†’ activity-summary first. Check its data_status before claiming "no data". /search only for verbatim quotes or frame_ids.
...
export function buildConnectionsContext(
  ...
  const entries = withDesc
    .map((c) => `## ${c.name} (${c.id})\n${c.description}`)
    .join("\n\n");
  return `\n\n# Connected integrations\n\nThe user has connected the following external services. Use the endpoints listed under each to fetch live data when relevant. All endpoints are on http://localhost:3030 and require \`-H "Authorization: Bearer $MEMORA_LOCAL_API_KEY"\`.\n\n${entries}`;
}
