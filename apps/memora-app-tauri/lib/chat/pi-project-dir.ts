...
export async function piProjectDirForSession(
  ...
  const defaultBaseDir = await join(home, ".memora");
  ...
  try {
    const result = await commands.getMemoraBaseDir();
    if (result.status === "ok" && result.data) baseDir = result.data;
  } catch {
    // Production defaults to ~/.MEMORA. Isolated app/E2E data roots are
    // returned by getMemoraBaseDir, so a command failure safely falls back.
  }
