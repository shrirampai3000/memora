...
export function isLocalMemoraUrl(url: URL): boolean {
...
export function webTargetFromUrl(url: URL, kind: WebTargetKind): WebTargetPresentation | null {
  if (isLocalMemoraUrl(url)) return null;
  return {
    url: url.toString(),
...
export function classifyCurl(cmd: string): CurlPresentation | null {
  ...
  const url = urls.find(isLocalMemoraUrl);
  if (!url) {
...
export function endpointFamily(path: string): string {
  ...
  if (path.startsWith("/connections/")) {
    ...
    // user-visible card matches the verb in the title (Sent email â†’ EMAIL).
...
function curlActivity(command: string): ToolActivityPresentation | null {
  ...
  const localUrl = urls.find(isLocalMemoraUrl);
  ...
  if (
    ...
    return activity("Checking Memora", "Checked Memora");
  }
  if (path === "/export") {
    return activity("Preparing an export", "Prepared an export");
  }

  return activity("Working in Memora", "Completed work in Memora");
}
...
// Arg-key â†’ REST query-param name. Search and activity share this shape.
...
/**
 * If `toolName` is a memora MCP tool, synthesize the equivalent local curl
 ...
 */
export function mcpMemoraCommand(
  toolName: string,
  args: Record<string, unknown> = {},
): string | null {
  let name: string;
  if (/^mcp__memora__/i.test(toolName)) {
    name = toolName.replace(/^mcp__memora__/i, "");
  } else if (!MCP_PREFIX_RE.test(toolName) && MCP_MEMORA_ENDPOINTS[toolName]) {
    // Some agents drop the mcp__ prefix; still recognize a known memora tool.
    name = toolName;
  } else {
    return null;
  }
  ...
  const endpoint = MCP_MEMORA_ENDPOINTS[name];
  if (!endpoint) {
    // A memora MCP tool we haven't mapped: a bare local URL still reads as
    // "Working in Memora" rather than a generic background step.
...
// de-slugify (activity-summary â†’ "Activity summary"). Beats a generic step.
