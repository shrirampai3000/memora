...
export function validateQuotaUpgradeAction({
  ...
  try {
    ...
    if (
      url.protocol !== "https:" ||
      !["github.com/shrirampai3000/memora", "github.com/shrirampai3000/memora"].includes(url.hostname) ||
 ...
 * arbitrary nested provider errors. The URL is accepted only for Memora's
...
export function buildModelNotAllowedMessage(errorStr: string): string {
  ...
  const named = modelNotAllowedName(errorStr)
    ...
    : "This model isn't";
  return `${named} available on Memora Cloud. Switch to Auto, or use your own provider key. If you picked a coding agent, re-select it in Settings â†’ AI presets.`;
}
