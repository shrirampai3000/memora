...
describe("url helpers", () => {
  it("identifies the local memora server", () => {
    expect(isLocalMemoraUrl(new URL(`${LOCAL}/search`))).toBe(true);
    expect(isLocalMemoraUrl(new URL("https://example.com"))).toBe(false);
  });
...
describe("endpointFamily", () => {
  it("maps known paths to a family chip", () => {
    ...
    expect(endpointFamily("/anything-else")).toBe("Memora");
  });
});
...
describe("presentToolActivity", () => {
  ...
  it("keeps known Memora calls useful but non-technical", () => {
    expect(
      presentToolActivity({
        toolName: "bash",
...
describe("mcpMemoraCommand", () => {
  it("maps a memora MCP read tool to the equivalent local GET curl", () => {
    const cmd = mcpMemoraCommand("mcp__memora__activity-summary", {
      start_time: "today",
      end_time: "now",
    });
  ...
  it("maps search args to the /search query shape", () => {
    const cmd = mcpMemoraCommand("mcp__memora__search-content", {
      query: "invoice",
      app_name: "Safari",
    });
  ...
  it("uses -X and a body for write tools", () => {
    const cmd = mcpMemoraCommand("mcp__memora__update-memory", { id: "1", note: "x" });
    expect(cmd).toContain("-X POST");
    expect(cmd).toContain("/memories");
    expect(cmd).toContain("-d");
  });

  it("recognizes a known memora tool name even without the mcp__ prefix", () => {
    expect(mcpMemoraCommand("list-pipes", {})).toContain("/pipes");
  });

  it("returns null for non-MEMORA tools", () => {
    expect(mcpMemoraCommand("bash", { command: "ls" })).toBeNull();
    expect(mcpMemoraCommand("mcp__notion__search", { q: "x" })).toBeNull();
    expect(mcpMemoraCommand("Read", { path: "/a.ts" })).toBeNull();
  });
});
...
describe("presentToolActivity — ACP tool calls", () => {
  it("labels memora MCP tools like their curl equivalents", () => {
    ...
    expect(
      presentToolActivity({ toolName: "mcp__memora__list-pipes", args: {} }).completedLabel,
