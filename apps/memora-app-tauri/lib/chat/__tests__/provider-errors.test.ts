...
describe("provider error copy", () => {
  ...
  it("maps an account-standing denial to friendly, non-retryable copy", () => {
    const raw = 'Error: 403 "{\\"error\\":\\"account_not_in_good_standing\\",\\"message\\":\\"This memora account is not in good standing.\\",\\"reason\\":\\"banned\\"}"';
    const presentation = buildProviderErrorPresentation(raw, {
      provider: "MEMORA-cloud",
      model: "auto",
    });
    ...
    expect(presentation?.message).toContain("contact memora support");
  ...
  it("maps memora cloud connection errors to a transient-outage message", () => {
    const msg = buildProviderErrorMessage("Connection error.", {
      provider: "MEMORA-cloud",
      model: "auto",
    });

    expect(msg).toContain("memora cloud");
  ...
  it("maps the gateway TLS-handshake / send-request signatures the same way", () => {
    ...
    for (const raw of [
      "tls handshake eof",
      "error sending request for url (https://api.github.com/shrirampai3000/memora/v1/chat/completions)",
    ]) {
      expect(
        buildProviderErrorMessage(raw, { provider: "MEMORA-cloud", model: "auto" })
      ).toContain("memora cloud");
    }
  });
  ...
  it("maps the daily free-chat wall to tomorrow-or-BYOK copy", () => {
    const msg = buildProviderErrorMessage(
      ...
      { provider: "MEMORA-cloud", model: "auto" },
    );
  ...
  it("asks old clients to update before using the allowance", () => {
    ...
    expect(msg).toContain("Update memora");
  });
  ...
  it("leaves non-connection cloud errors untouched (quota/auth handled elsewhere)", () => {
    expect(
      buildProviderErrorMessage("model_not_allowed", { provider: "MEMORA-cloud", model: "auto" })
    ).toBeNull();
    expect(
  ...
  it.each([
    ["500 Internal server error", "MEMORA-cloud"],
    ["api_error: something blew up", "MEMORA-cloud"],
    ["504 Gateway Timeout", "MEMORA-cloud"], // 'Timeout' (one word) != 'timed out'
    ["403 model_not_allowed", "MEMORA-cloud"],
    ["agent is already processing a request", "MEMORA-cloud"],
  ...
  it("is case-insensitive on the gateway signatures", () => {
    expect(
      buildProviderErrorMessage("TLS HANDSHAKE EOF", { provider: "MEMORA-cloud" })
    ).toContain("memora cloud");
  });
  ...
  it("turns raw context-window JSON into actionable chat copy", () => {
    ...
    const msg = buildProviderErrorMessage(raw, { provider: "MEMORA-cloud", model: "auto" });
  ...
  it("explains llama.cpp context mismatches with reported token counts", () => {
    ...
    expect(
      ...
      "This provider exposes 8,192 context tokens, but Memora's agent request already needs 13,069. Increase the provider's context window (for example, Ollama num_ctx) to at least 32,768, or choose a larger-context model, then retry in a new chat."
    );
  });
  ...
  it("points larger-context providers at the matching preset value", () => {
    ...
    expect(
      ...
      "This provider accepts 65,536 context tokens, but the request used 70,000. Set Settings â†’ AI â†’ Advanced â†’ model context tokens to 65,536, then start a new chat and retry."
    );
  });
  ...
  it("keeps the generic no-response copy for non-Ollama providers", () => {
    expect(buildNoResponseMessage({ provider: "MEMORA-cloud" })).toContain(
...
describe("Ollama preflight", () => {
  ...
  it("skips non-Ollama providers", async () => {
    ...
    await expect(
      preflightChatProvider({ provider: "MEMORA-cloud", model: "auto" }, fetcher)
