...
describe("buildDailyLimitMessage", () => {
  ...
  it("offers the validated upgrade without exposing the exhausted lane", () => {
    const message = buildDailyLimitMessage(
      '{"error":{"code":"hosted_ai_allowance_exceeded"},"allowance":{"lane":"explicit","plan":"business","managed_by":"cloudflare"},"required_plan":"business_max","upgrade_url":"https://github.com/shrirampai3000/memora/account/billing?target_plan=pro_max&interval=month"}',
    );
    expect(message).toBe(
  ...
  it("defers structured Business recovery to the persistent action panel", () => {
    const error = JSON.stringify({
      error: JSON.stringify({
        ...
        upgrade_url: "https://github.com/shrirampai3000/memora/account/billing",
    ...
    expect(parseQuotaUpgradeAction(error)).toEqual({
      ...
      upgradeUrl: "https://github.com/shrirampai3000/memora/account/billing",
  ...
  it("accepts allow-listed next plans but rejects missing or off-domain upgrade actions", () => {
    ...
    for (const [requiredPlan, targetPlan] of [
      ...
      const upgradeUrl = `https://github.com/shrirampai3000/memora/account/billing?target_plan=${targetPlan}&interval=month`;
      expect(
        parseQuotaUpgradeAction(
          JSON.stringify({
            error: "daily_cost_limit_exceeded",
...
describe("parseRateLimitWaitSeconds", () => {
  ...
  it("clamps to [1, 60]", () => {
    ...
    expect(parseRateLimitWaitSeconds('{"reset_in": 0}')).toBe(10); // 0 â†’ invalid â†’ default
  });
});
...
describe("presentQuotaError", () => {
  const costLimitError =
    'HTTP 429 {"error":"daily_cost_limit_exceeded","required_plan":"business","upgrade_url":"https://github.com/shrirampai3000/memora/account/billing","resets_at":"2026-08-06T00:00:00Z"}';
...
describe("buildModelNotAllowedMessage", () => {
  ...
  const GATEWAY_403 = JSON.stringify({
    ...
    message:
      'Model "codex-acp" is not available through memora cloud. Choose another model or use your own provider key.',
    plan: "business",
    required_plan: null,
    upgrade_url: null,
    byok_supported: true,
  ...
  it("names the rejected model and the ways out", () => {
    ...
    expect(msg).toContain("Settings â†’ AI presets");
  });
  ...
  it("offers the upgrade only when the gateway supplies a validated one", () => {
    const msg = buildModelNotAllowedMessage(
      JSON.stringify({
        error: "model_not_allowed",
        message: 'Model "claude-opus-5" is not available through memora cloud.',
        required_plan: "business",
        upgrade_url: "https://github.com/shrirampai3000/memora/account/billing",
      }),
    );
  ...
  it("ignores an upgrade url that is not memora billing", () => {
    const msg = buildModelNotAllowedMessage(
      JSON.stringify({
        error: "model_not_allowed",
        required_plan: "business",
