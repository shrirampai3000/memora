...
const WALL_ERROR =
  'HTTP 429 {"error":"free_chat_limit_exceeded","message":"...","limit":2,"upgrade_url":"https://github.com/shrirampai3000/memora/onboarding","resets_at":"2026-08-06T00:00:00Z"}';

describe("parseFreeWall", () => {
  it("parses the wall rejection with plans url and reset time", () => {
    expect(parseFreeWall(WALL_ERROR)).toEqual({
      plansUrl: "https://github.com/shrirampai3000/memora/onboarding",
  ...
  it("handles escaped-quote bodies (double-encoded gateway errors)", () => {
    ...
    expect(parseFreeWall(escaped)?.plansUrl).toBe(
      "https://github.com/shrirampai3000/memora/onboarding",
    );
  });
  ...
  it("falls back to the default plans url when the field is missing or hostile", () => {
    ...
    ).toBe("https://github.com/shrirampai3000/memora/onboarding");
    expect(
      parseFreeWall(
...
describe("validateFreeWallPlansUrl", () => {
  it("allows only memora https pricing/billing pages", () => {
    expect(
      validateFreeWallPlansUrl("https://github.com/shrirampai3000/memora/onboarding"),
    ).toBeTruthy();
    expect(
      validateFreeWallPlansUrl("https://github.com/shrirampai3000/memora/account/billing"),
    ).toBeTruthy();
    expect(validateFreeWallPlansUrl("http://github.com/shrirampai3000/memora/onboarding")).toBeNull();
    expect(validateFreeWallPlansUrl("https://github.com/shrirampai3000/memora/pricing")).toBeNull();
