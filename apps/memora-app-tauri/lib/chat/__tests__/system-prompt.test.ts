...
describe("buildSystemPrompt", () => {
  ...
  it("opens by establishing the Memora assistant role", () => {
    expect(prompt.startsWith("You are the user's memora assistant.")).toBe(true);
  });
...
describe("buildConnectionsContext", () => {
  ...
  it("renders only the connections that have a description", () => {
    const out = buildConnectionsContext([
      ...
      { id: "slack", name: "Slack" }, // no description â†’ omitted
    ]);
  ...
  it("includes the localhost bearer-token reminder", () => {
    ...
    expect(out).toContain("MEMORA_LOCAL_API_KEY");
  });
