...
/**
 * Single source of truth for local memora API access.
 *
 * All API calls to the memora backend MUST go through this module.
 * It handles:
...
function applyApiConfig(config: LocalApiConfig): void {
  ...
  if (_authEnabled && _apiKey && typeof document !== "undefined") {
    document.cookie = `MEMORA_auth=${_apiKey}; path=/; SameSite=Strict`;
  } else if (!_authEnabled && typeof document !== "undefined") {
    document.cookie = "MEMORA_auth=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
  }
}
...
/**
 * Get the base URL for the local memora API.
 */
...
/**
 * Fetch wrapper for the local memora API.
 *
