...
export function getCliPath() {
  ...
  switch (os) {
    case "windows":
      return "%LOCALAPPDATA%\\memora\\MEMORA.exe";
    case "macos":
      return "/Applications/MEMORA.app/Contents/MacOS/MEMORA";
    case "linux":
      return "/usr/local/bin/MEMORA";
    default:
      return "memora";
  }
}
    ...
    return Array.from(uniqueKeys)
      .map((key) => {
        if (key === "super" || key === "meta" || key === "command" || key === "cmd") {
          return os === "macos" ? "âŒ˜" : "âŠž";
        }
        if (key === "ctrl" || key === "control") return "âŒƒ";
        if (key === "alt") return os === "macos" ? "âŒ¥" : "Alt";
