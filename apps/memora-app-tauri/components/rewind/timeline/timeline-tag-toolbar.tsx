...
function pipeInitials(name: string): string {
	// "meeting-summary" â†’ "MS", "day-recap" â†’ "DR", "ai-habits" â†’ "AH"
	...
	const handleRetranscribe = useCallback(async () => {
		...
		try {
			...
			await showChatWithPrefill({
				context: "",
				prompt: `please read your MEMORA-api skill first, then retranscribe the audio from ${start} to ${end}. after retranscribing, ask me if i want to assign speaker names to the different speakers in the transcription.`,
				autoSend: true,
				...
				<div className="flex items-center justify-between gap-2 pr-4">
					<div className="flex items-center gap-2 text-xs text-muted-foreground min-w-0">
						...
						<span className="truncate">
							{startTime} â†’ {endTime} ({durationStr})
						</span>
					</div>
				...
				{showDeleteConfirm ? (
					...
					<div className="flex flex-col gap-2">
						<p className="text-xs text-muted-foreground leading-relaxed">
							...
							<span className="font-medium text-foreground">{startTime} â†’ {endTime}</span> ({durationStr})? this cannot be undone.
						</p>
