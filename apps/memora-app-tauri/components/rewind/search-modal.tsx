...
const FrameThumbnail = ({
  ...
  // host (localhost:3030) so the MEMORA_auth cookie isn't sent.
...
export function SearchModal({ isOpen, onClose, onNavigateToTimestamp, embedded = false, standalone = false }: SearchModalProps) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // movement steps a whole row of them while â†â†’ walks them one at a time.
      ...
      // When the input has focus the caret comes first: â†/â†’ only reach the frame
