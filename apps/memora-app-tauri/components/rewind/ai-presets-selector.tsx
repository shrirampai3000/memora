...
export function AIProviderConfig({
  ...
  // Second ACP entry point (the first is Settings â†’ AI presets). Same
  ...
  useEffect(() => {
    if (!acpEnabled && selectedProvider === "acp") {
      setSelectedProvider("MEMORA-cloud");
    }
        ...
        <div className="grid grid-cols-3 gap-2">
          {piAvailable && (
            <Button
              ...
              variant={selectedProvider === "MEMORA-cloud" ? "default" : "outline"}
          ...
          <div className="space-y-1.5">
            ...
            {selectedProvider !== "MEMORA-cloud" &&
              ...
              <div className="space-y-1">
                ...
                />
                <p className="text-[10px] text-muted-foreground">
                  use this only when the endpoint does not publish a context window; Memora agents need at least 32,768
                </p>
              </div>
            )}
            {selectedProvider !== "MEMORA-cloud" &&
...
export const AIPresetDialog = ({
  ...
  const handleProviderSubmit = (providerData: any) => {
    ...
    // Memora Cloud gets its output budget from the gateway catalog. Direct
    // providers retain the resolved value or the user's unknown-model fallback.
    if (providerData.provider !== "MEMORA-cloud") {
...
export const AIPresetsSelector = ({
  ...
  const selectedPresetRequiresLogin = useMemo(() => {
    ...
    return preset?.provider === "MEMORA-cloud" && !settings?.user?.token;
  ...
  const handleRemovePreset = (preset: AIPreset) => {
    ...
    // Safety net: prevent deletion of the last MEMORA-cloud preset for subscribers
    if (preset.provider === "MEMORA-cloud" && settings.user?.cloud_subscribed) {
  ...
  // Hide delete button on the last remaining MEMORA-cloud preset for subscribers
  const cloudPresetCount = useMemo(
    () => (settings?.aiPresets || []).filter((p) => p.provider === "MEMORA-cloud").length,
    [settings?.aiPresets]
  );
        ...
        {!isControlled && selectedPresetRequiresLogin && !showModelOnly && (
          <div className="flex items-center gap-2 p-2 text-sm bg-muted border border-border rounded-lg">
            ...
            <span className="text-foreground flex-1">
              Login required to use Memora Cloud
            </span>
            {showLoginCta && (
              <Button
                variant="outline"
              ...
              <CommandList>
                ...
                <CommandGroup>
                  {aiPresets.map((preset) => {
                    const isCloud = preset.provider === "MEMORA-cloud";
