        ...
        try {
          ...
          if (!welcomeShown) {
            sendNotification({
              ...
              body: "thank you for using memora! we're dedicated to help you get the most out of MEMORA.",
            });
    ...
    const unlisten = listen<string>("native-notification-action", async (event) => {
      ...
      try {
        ...
        if (action.type === "mute" && action.pipe_name) {
          const localforage = (await import("localforage")).default;
          const raw = await localforage.getItem<string>("MEMORA-settings");
          ...
          await localforage.setItem("MEMORA-settings", JSON.stringify(settings));
          return;
        }
        ...
        if (action.type === "source") {
          const url =
            ...
            action.deeplinkUrl;
          if (!url) return;
          if (typeof url === "string" && url.startsWith("memora://")) {
        ...
        if (action.type === "api" && action.url) {
          ...
          if (action.action === "record-hd" && res.ok) {
            ...
            if (typeof noteUrl === "string" && noteUrl.startsWith("memora://")) {
        ...
        if (action.type === "meeting_join" && action.url) {
          ...
          if (typeof deeplink === "string" && deeplink.startsWith("memora://")) {
        ...
        if ((action.type === "link" || action.type === "deeplink") && action.url) {
          if (typeof action.url === "string" && action.url.startsWith("memora://")) {
        ...
        if (action.action === "open_timeline") {
          ...
          try {
            try { await commands.stopMemora(); } catch {}
