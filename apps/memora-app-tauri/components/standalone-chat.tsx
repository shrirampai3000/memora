...
export function StandaloneChat({
  ...
  // see if the agent connects (acp_ready â†’ close) or asks to sign in again
  // (â†’ error). The dialog never toggles closed in between, so it can't flicker.
  ...
  const handleSetActivePreset = useCallback((preset: AIPreset | undefined | ((prev: AIPreset | undefined) => AIPreset | undefined)) => {
    if (typeof preset === "function") {
      ...
      // (settings â†’ home). Only for direct user selection, not the
  ...
  const toggleBrowserPanel = useCallback(() => {
    ...
    if (activeSideView === "side" && browserPanelState.hasUrl) {
      window.dispatchEvent(
        new CustomEvent("memora:browser-sidebar-toggle", {
  ...
  useEffect(() => {
    ...
    const unlisten = listen<AgentEventEnvelope>(AGENT_TOPICS.event, (event) => {
      ...
      if (!title.startsWith("memora:connect:")) return;
  ...
  // ACP connect broker: an agent's MEMORA_connect_app tool blocks on
  ...
  const handleAgentActionEvent = useCallback((value: unknown, sessionId: string): boolean => {
    const trace = (stage: string, details?: Record<string, unknown>) => {
      if (process.env.NEXT_PUBLIC_MEMORA_E2E !== "true" || typeof window === "undefined") return;
  ...
  // chat with an agentic loop (â‰¥2 assistant messages with citations between
  ...
  return (
    <div ref={dropRootRef} className={cn("flex flex-col bg-background", className ?? "h-screen")} data-testid="section-home">
      ...
      <ChatComposer
        ...
        connectBanner={{
          ...
          onDismiss: () => {
            ...
            try { localStorage.setItem("MEMORA_connect_banner_dismissed", "true"); } catch {}
          },
        }}
        onStop={handleStop}
      />
      ...
      <BrowserSidebar
        ...
        // navigation `owner` via x-MEMORA-session). Lets the sidebar reveal
