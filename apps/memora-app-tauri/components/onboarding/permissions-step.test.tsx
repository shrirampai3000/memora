...
describe("onboarding permission wheel", () => {
  ...
  it("only the focused (first ungranted) row is interactive", async () => {
    ...
    expect(accessibilityRow()).toBeEnabled();
  ...
  it("advances focus and refocuses the window when the poller detects a grant", async () => {
    // granting accessibility flips the next status check to granted
    mocks.requestPermissionWithFlow.mockImplementation(async () => {
      mocks.checkAccessibilityPermissionCmd.mockResolvedValue("granted");
      mocks.checkAccessibilityPermissionLiveCmd.mockResolvedValue("granted");
      return undefined;
    });
    ...
    // the post-request recheck detects the grant
    await waitFor(() => expect(mocks.requestPermissionWithFlow).toHaveBeenCalledWith("accessibility"));
  });
  ...
  it("keeps the path collapsed until asked, then opens it", async () => {
    ...
    expect(path).toHaveTextContent("/Users/test/.memora");
  ...
  // ~/.memora when settings.dataDir is unset, so it disagrees with the
  // engine whenever the directory came from MEMORA_DATA_DIR or a flag.
  it("shows a relocated data dir rather than a hardcoded ~/.memora", async () => {
    mocks.getActiveDataDir.mockResolvedValue({
      status: "ok",
      data: "/Volumes/Vault/MEMORA-data",
    });
    ...
    const path = await screen.findByTestId("onboarding-data-dir-path");
    expect(path).toHaveTextContent("/Volumes/Vault/MEMORA-data");
    expect(path.textContent ?? "").not.toContain(".MEMORA/");
  });
  ...
  it("advertises the pause control in the collapsed summary", async () => {
    ...
    ).toHaveTextContent("pause recording anytime from the memora icon");
  });
