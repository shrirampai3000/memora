  ...
  const permissions: PermissionDef[] = [
    {
      ...
      subtitle: "Lets Memora understand app content without OCR",
    ...
    },
    {
      ...
      subtitle: (
        <>
          Lets Memora index what&apos;s on your screen: windows, docs,
          chats, code. {" "}
  ...
  const pollPermissions = useCallback(async () => {
    ...
    try {
      do {
        ...
        // Refocus only on a confirmed false â†’ true transition, so permissions
    ...
    >
      ...
      <div className="flex flex-col items-center mb-6">
        ...
        <p className="font-mono text-[10px] text-muted-foreground mt-1 text-center max-w-xs">
          Grant permissions to start screen capture.
        </p>
      </div>
      ...
      {screenRestartRequired ? (
        ...
        >
          ...
          >
            ...
            />
            {restarting ? "restarting..." : "restart memora"}
          </button>
        </div>
      ) : (
        <>
          ...
          {showSkip && !allRequiredGranted && (
            ...
            >
              continue without all permissions â†’
            </motion.button>
          )}
        </>
      )}
