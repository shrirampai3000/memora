...
const mocks = vi.hoisted(() => ({
  ...
  capture: vi.fn(),
  localFetch: vi.fn(),
  stopMemora: vi.fn().mockResolvedValue(undefined),
  spawnMemora: vi.fn().mockResolvedValue(undefined),
}));
...
vi.mock("@/lib/utils/tauri", () => ({
  commands: {
    stopMemora: mocks.stopMemora,
    spawnMemora: mocks.spawnMemora,
  },
}));
...
describe("TimelineChoice", () => {
  ...
  it("does not restart memora during first-run onboarding", async () => {
    mocks.settings.deviceTier = "high";
    const handleNextSlide = vi.fn();
    render(<TimelineChoice handleNextSlide={handleNextSlide} />);
    ...
    expect(mocks.stopMemora).not.toHaveBeenCalled();
    expect(mocks.spawnMemora).not.toHaveBeenCalled();
    expect(handleNextSlide).toHaveBeenCalledTimes(1);
  });
  ...
  it("restarts a running engine so a replayed choice takes effect", async () => {
    ...
    await waitFor(() => expect(mocks.spawnMemora).toHaveBeenCalledTimes(1));
    expect(mocks.stopMemora).toHaveBeenCalledTimes(1);
    expect(handleNextSlide).toHaveBeenCalledTimes(1);
  });
  ...
  it("advances without waiting for the restart to finish", async () => {
    ...
    mocks.stopMemora.mockImplementation(() => new Promise<void>(() => {}));
    const handleNextSlide = vi.fn();
    render(<TimelineChoice handleNextSlide={handleNextSlide} />);
  ...
  it("still advances when the restart itself fails", async () => {
    mocks.settings.deviceTier = "low";
    mocks.localFetch.mockResolvedValue({ ok: true, status: 200 });
    mocks.spawnMemora.mockRejectedValue(new Error("spawn failed"));
  ...
  // already ship in Settings â†’ Privacy.
