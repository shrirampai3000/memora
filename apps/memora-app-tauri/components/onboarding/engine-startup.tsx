...
export default function EngineStartup({ handleNextSlide }: EngineStartupProps) {
  ...
  // When spawn_memora rejects (e.g. TCC permission denied) we used to
  ...
  useEffect(() => {
    const start = async () => {
      try {
        ...
        const result = await commands.spawnMemora(null);
        if (result.status === "error") {
          throw new Error(result.error);
        }

        // spawn_memora resolves only after ServerCore and CaptureSession
        ...
        console.error("failed to start memora:", message);
  ...
  const handleContinueWithoutRecording = async () => {
    try {
      // spawnMemora marks capture as intended before startup. Clear that
      // intent so port recovery cannot start capture after this explicit choice.
      const result = await commands.stopMemora();
      if (result.status === "error") {
        throw new Error(result.error);
      }
    } catch (err) {
  ...
  const sendLogs = async () => {
    ...
    try {
      const BASE_URL = memoraWebBase("https://github.com/shrirampai3000/memora");
      const machineId =
  ...
  const openLogsFolder = async () => {
    try {
      ...
      const memoraDir = await join(home, ".memora");
      await revealItemInDir(memoraDir);
    } catch (err) {
  ...
  // â”€â”€ Engine startup phase (starting / stuck) â”€â”€
  return (
            ...
            >
              ...
              {spawnErrorKind === "port_conflict" ? (
                <>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        openUrl("https://cal.com/team/MEMORA/chat")
                      }
                      className="font-mono text-[10px] h-7 px-2"
                    >
                ...
                <>
                  ...
                  <button
                    onClick={() =>
                      openUrl("https://docs.github.com/shrirampai3000/memora/permissions")
                    }
