...
describe("file viewer â€” html render", () => {
  ...
  it("renders a marked snippet by default", () => {
    const content = htmlContent("<!-- memora:render=human --><h1>hi</h1>");
    const { container } = render(
      <ViewerFileContent path={content.path} content={content} />,
    );
    expect(container.querySelector("iframe")).not.toBeNull();
  ...
  it("toggles between rendered and source", () => {
    ...
    // starts rendered
    expect(container.querySelector("iframe")).not.toBeNull();
    fireEvent.click(screen.getByTestId("html-render-toggle")); // â†’ source
    expect(container.querySelector("iframe")).toBeNull();
    fireEvent.click(screen.getByTestId("html-render-toggle")); // â†’ rendered
    expect(container.querySelector("iframe")).not.toBeNull();
  });
