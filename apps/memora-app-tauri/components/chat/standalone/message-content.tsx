...
// (a native ACP Read/Edit, a non-MEMORA MCP call) has no curl and no card.
function effectiveCommand(toolCall: ToolCall): string | null {
  if (toolCall.toolName === "bash") {
    return String(toolCall.args?.command ?? "") || null;
  }
  return mcpMemoraCommand(toolCall.toolName, (toolCall.args ?? {}) as Record<string, unknown>);
}
...
function bashToolDetailsPresentation(toolCall: ToolCall): BashToolDetailsPresentation | null {
  ...
  const localUrl = urlsInCommand(command).find(isLocalMemoraUrl);
...
function FriendlyToolDetails({ toolCall }: { toolCall: ToolCall }) {
  ...
  // Raw pi's bash curl AND memora MCP tool calls both resolve to a local
      ...
      <div className="flex flex-col items-center flex-shrink-0 w-5">
        ...
        <div className="relative flex items-center justify-center w-5 h-5">
          {connectionIconName && !toolCall.isRunning && !toolCall.isError ? (
            ...
            // X mark for error
            <span className="text-[10px] font-mono font-bold text-foreground leading-none">âœ—</span>
          ) : (
...
// existing assets in apps/MEMORA-app-tauri/public/images/.
...
function mergeWorkAndIntermediateText(groups: GroupedBlock[]): GroupedBlock[] {
  ...
  // No tool calls at all â†’ nothing to merge, show text as-is.
