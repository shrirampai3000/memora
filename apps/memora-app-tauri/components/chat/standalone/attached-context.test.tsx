...
const PROMPT =
  "Help me share the reviewed, frozen Memora snapshot attached as context to Notion.";
...
describe("attached context", () => {
  ...
  it("keeps the payload out of the visible bubble but not out of reach", () => {
    ...
    expect(screen.getByText(/MEMORA_share_context/)).toBeVisible();
  ...
  it("still cards a malformed payload", () => {
    const parsed = parseAttachedContext(
      '[Context from search: {"kind":"MEMORA_share_context", broken]\n\ndo the thing',
    );
  ...
  it("handles an envelope with no prompt after it", () => {
    ...
    expect(screen.getByText("frozen Memora snapshot")).toBeVisible();
  });
});
