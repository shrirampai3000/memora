...
export function usePiSessionLifecycle({
  ...
  useEffect(() => {
    ...
    setActivePreset((prev) => {
      if (!prev) {
        // On fresh mount after a navigation (e.g. settings â†’ home), restore
  ...
  const buildProviderConfig = useCallback((preset?: AIPreset | null): ResolvedPiProviderConfig | null => {
    ...
    // agents read live screen context through the memora MCP server
  ...
  useEffect(() => {
    ...
    // prompt into an initialize failure. ACP sessions get memora context
