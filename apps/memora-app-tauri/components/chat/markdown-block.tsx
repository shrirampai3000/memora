    ...
    <MemoizedReactMarkdown
      ...
      components={{
        ...
        a({ href, children, ...props }) {
          if (
            href?.startsWith("memora://timeline") ||
            href?.startsWith("memora://frame") ||
            href?.startsWith("memora://view")
          ) {
            const handleMemoraLinkClick = async (
              e: React.MouseEvent<HTMLAnchorElement>,
            ) => {
              e.preventDefault();
              try {
                if (href.startsWith("memora://view")) {
                  const viewerPath = memoraViewerPathFromHref(href);
                  if (viewerPath && onOpenViewerPath) {
                    onOpenViewerPath(viewerPath);
                    return;
                  }
                  if (await openMemoraViewerLink(href)) return;
                }

                if (href.startsWith("memora://frame")) {
                ...
                console.error("Failed to open memora link:", error);
              }
            };
