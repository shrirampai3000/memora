 ...
 * Data flow:
 *
 *   â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”  emit chat-load-conversation  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
 *   â”‚ ChatSidebar (this file)â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–¶â”‚ Standaloneâ”‚
 *   â”‚                        â”‚                                â”‚ Chat      â”‚
 *   â”‚  reads chat-store      â”‚  emit chat-current-session     â”‚           â”‚
 *   â”‚  emits user actions    â”‚â—€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤ (mounts   â”‚
 *   â”‚                        â”‚                                â”‚  Pi via   â”‚
 *   â”‚                        â”‚                                â”‚  piStart) â”‚
 *   â””â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜                                â””â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”˜
 *            â”‚ writes pinned/hidden                                â”‚ writes deltas
 *            â–¼                                                     â–¼ to Pi stdout
 ...
 *   â”‚ chat-storage.ts        â”‚         â”‚ pi-event-router.ts             â”‚
 *   â”‚ (~/.MEMORA/chats/) â”‚         â”‚ listens app-wide for pi_event  â”‚
 *   â”‚                        â”‚         â”‚ writes status/preview to store â”‚
...
const DELETED_PIPE_EXECUTIONS_KEY = "memora:deleted-pipe-executions";
...
export function ChatSidebar({ className, onViewAll }: ChatSidebarProps) {
  ...
  const [collapsedRecentsSections, setCollapsedRecentsSections] = useState<Set<string>>(() => {
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (k?.startsWith("memora:recents-subsection-collapsed:")) {
          if (localStorage.getItem(k) === "true") {
            set.add(k.slice("memora:recents-subsection-collapsed:".length));
          }
        }
      }
    } catch { /* ignore */ }
    ...
    setCollapsedRecentsSections((prev) => {
      ...
      try {
        localStorage.setItem(
          `memora:recents-subsection-collapsed:${key}`,
          String(!collapsed),
        );
      } catch { /* ignore */ }
      return next;
  ...
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(() => {
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k?.startsWith("memora:group-expanded:")) {
          if (localStorage.getItem(k) === "true") {
            set.add(k.slice("memora:group-expanded:".length));
          }
        }
      }
    } catch { /* ignore */ }
  ...
  useEffect(() => {
    ...
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k?.startsWith("memora:group-expanded:")) {
          const groupKey = k.slice("memora:group-expanded:".length);
  ...
  useEffect(() => {
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k?.startsWith("memora:recents-subsection-collapsed:")) {
          const sectionKey = k.slice("memora:recents-subsection-collapsed:".length);
  ...
  const [pinnedCollapsed, setPinnedCollapsed] = useCollapsedPref(
    "memora:pinned-collapsed",
    true
  );
  const [recentsCollapsed, setRecentsCollapsed] = useCollapsedPref("memora:recents-collapsed");
  const [archivedCollapsed, setArchivedCollapsed] = useCollapsedPref(
    "memora:closed-collapsed",
    true
  );
...
export function CollapsedChatSidebarButton({
  ...
  useEffect(() => {
    ...
    // effect â†’ React #185 ("Maximum update depth exceeded").
 ...
 *   loading dot â†’ streaming/thinking/tool/queued
 *   unread dot  â†’ new content
 *   compact age â†’ idle/read
 *   âš            â†’ error
 *
