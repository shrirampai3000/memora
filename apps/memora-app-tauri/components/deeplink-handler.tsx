...
export function DeeplinkHandler() {
  ...
  useEffect(() => {
    ...
    const processDeepLinkUrl = async (url: string) => {
      ...
      // memora" button opens this link. Refresh the authenticated account
      ...
      //   memora://subscription-success?purchase_token=<token>
      ...
      if (
        ...
        if (purchaseToken) {
          try {
            ...
            toast({
              title: "welcome to memora business!",
      ...
      // Handle OAuth callbacks relayed from the HTTPS page on github.com/shrirampai3000/memora.
      ...
      //   memora[-enterprise]://oauth/connections/callback?code=...&state=...
      //   memora[-enterprise]://oauth/mcp/<serverId>/callback?code=...&state=...
      ...
      if (parsedUrl.host === "oauth") {
        ...
        try {
          ...
          toast({
            ...
            description: msg || "couldn't reach the local memora engine",
            variant: "destructive",
          });
        }
      }
      ...
      //   memora://timeline?timestamp=ISO8601
      //   memora://timeline?start_time=ISO8601&end_time=ISO8601
      ...
      // Handle pipe install deep links: memora://install-pipe?url=<encoded-url>
      if (
      ...
      //   memora://chat/<conversationId>?message=<messageId>
      //   memora://chat?conversation=<conversationId>&message=<messageId>
      ...
      // Handle in-app file viewer: memora://view?path=<encoded-path>
      ...
      if (parsedUrl.host === "view" || parsedUrl.pathname === "view") {
        const filePath = memoraViewerPathFromHref(url);
        if (filePath) {
          try {
            await openMemoraViewerLink(url);
          } catch (error) {
      ...
      // Handle frame deep links: memora://frame/12345
      ...
      //   memora://meeting/123?live=1
      //   memora://meeting?id=123
    ...
    const unlisten = Promise.all([
      ...
      listen("shortcut-start-audio", async () => {
        await commands.stopMemora();
        await commands.spawnMemora(null);
