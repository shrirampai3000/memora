...
export function buildPaletteEntries(
  ...
  // so the hint is a plain platform ternary like settings-search's âŒ˜K badge.
  const inApp = (key: string) => (isMac ? `âŒ˜${key}` : `Ctrl+${key}`);
      ...
      : [
          {
            ...
            hint: globalShortcutHint(settings, "showMemoraShortcut", isMac),
            icon: MonitorPlay,
...
export function CommandPalette({ deps }: { deps: CommandPaletteDeps }) {
  ...
  useEventListener("keydown", (e) => {
    ...
    // nothing, and users expect âŒ˜K to work from the composer. The settings
    // page binds its own âŒ˜K while mounted; this component only lives on
