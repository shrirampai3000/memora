...
// gated on user.token) AND a "Memora Business Â· active" card (gated on
...
// object; swap `mocks.state.user` per case. Keep `@/lib/app-entitlement` REAL â€”
...
// Tauri plugins the effect wires up on mount â€” keep them inert.
...
describe("AccountSection subscription/login gating", () => {
  ...
  it("shows the active plan card for a real signed-in cloud subscriber", () => {
    ...
    expect(loginStatus()).toContain("logged in as pro@MEMORA.test");
    ...
    expect(mocks.openUrl).toHaveBeenCalledWith("https://github.com/shrirampai3000/memora/account");
  });
  ...
  it("shows Max and Ultra as plans and opens the exact Max billing target", () => {
    ...
    expect(mocks.openUrl).toHaveBeenCalledWith(
      "https://github.com/shrirampai3000/memora/account/billing?target_plan=pro_max&interval=month",
    );
  ...
  it("marks the capacity level a Max account actually pays for", () => {
    ...
    // allowance" â€” the gateway grants Max the same 400 credits as Business.
    mocks.state.user = {
      id: "u1",
      email: "max@MEMORA.test",
      token: "tok",
      cloud_subscribed: true,
  ...
  it("routes Max to Ultra and recommends nothing above Ultra or off-ladder", () => {
    ...
    expect(mocks.openUrl).toHaveBeenLastCalledWith(
      "https://github.com/shrirampai3000/memora/account/billing?target_plan=pro_ultra&interval=month",
    );
  ...
  it("opens website billing before a profile-granted Business plan expires", () => {
    ...
    expect(mocks.openUrl).toHaveBeenCalledWith(
      "https://github.com/shrirampai3000/memora/account/billing",
    );
  });
  ...
  it("does not regress the logged-in Basic plan badge (token, no cloud)", () => {
    ...
    expect(loginStatus()).toContain("logged in as basic@MEMORA.test");
    expect(screen.queryByTestId(ACTIVE_CARD)).not.toBeInTheDocument();
  ...
  it("sends existing Basic subscribers to billing even with a stale cloud flag", () => {
    ...
    expect(mocks.openUrl).toHaveBeenCalledWith(
      "https://github.com/shrirampai3000/memora/account/billing?target_plan=pro&interval=month",
    );
  });
  ...
  it("offers Lifetime users Business and starts a separate subscription checkout", async () => {
    ...
    expect(screen.queryByTestId(ACTIVE_CARD)).not.toBeInTheDocument();
    expect(screen.getByText("Memora Lifetime")).toBeInTheDocument();
