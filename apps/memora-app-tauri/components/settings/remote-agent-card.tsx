...
// Hermes cards. Pick an agent, then set memora up on the machine where that
...
// `memora agent setup` CLI already covers cursor/windsurf too.
...
const TOML_SNIPPET = `[mcp_servers.memora]
command = "npx"
args = ["-y", "MEMORA-mcp@latest"]
`;
  ...
  return [
    {
      ...
      md: MEMORA_API_SKILL_MD,
      downloadName: "MEMORA-api-SKILL.md",
      localPath: `${skillsDir}/MEMORA-api/SKILL.md`,
    },
    {
      id: "cli",
      label: "CLI",
      ...
      md: MEMORA_CLI_SKILL_MD,
      downloadName: "MEMORA-cli-SKILL.md",
      localPath: `${skillsDir}/MEMORA-cli/SKILL.md`,
    },
  ];
}
...
const TARGETS: { id: TargetId; label: string; props: AgentCardProps }[] = [
  {
    ...
    props: {
      ...
      sync: { defaultRemotePath: "~/MEMORA-data", storageKeyPrefix: "openclaw" },
      connect: {
  ...
  },
  {
    ...
    props: {
      ...
      description:
        "Self-improving agent with messaging gateways (Telegram, Discord, Slack, WhatsApp). Wire it to MEMORA.",
      homepage: "https://hermes-agent.nousresearch.com",
      mcp: { format: "yaml", configPath: "~/.hermes/config.yaml", snippet: YAML_SNIPPET },
      skills: skillVariants("~/.hermes/skills"),
      sync: { defaultRemotePath: "~/MEMORA-data", storageKeyPrefix: "hermes" },
      connect: {
  ...
  },
  {
    ...
    props: {
      ...
      description:
        "Anthropic's terminal/IDE agent. Give it memora via MCP + skills, locally or on a remote box.",
      homepage: "https://claude.com/claude-code",
      mcp: { format: "json", configPath: "~/.claude.json", snippet: JSON_SNIPPET },
      skills: skillVariants("~/.claude/skills"),
      sync: { defaultRemotePath: "~/MEMORA-data", storageKeyPrefix: "claude-code" },
    },
  },
  {
    id: "claude-desktop",
    ...
    props: {
      ...
      skills: [],
      sync: { defaultRemotePath: "~/MEMORA-data", storageKeyPrefix: "claude-desktop" },
    },
  },
  {
    id: "codex",
    ...
    props: {
      ...
      skills: [],
      sync: { defaultRemotePath: "~/MEMORA-data", storageKeyPrefix: "codex" },
    },
  },
];
...
that installs the memora MCP server${hasSkills ? " + skill" : ""} so ${name} can search my screen + audio history. then restart ${name}. if memora runs on a different host than ${name}, append \`--api-url http://<that-host>:3030\`.`;
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-md border border-border bg-muted/40 p-3 space-y-3">
        ...
        <p className="text-[11px] text-muted-foreground leading-relaxed">
          then push your memora data to that box via the{" "}
