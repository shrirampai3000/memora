...
/** Search fields for the Audio & meetings destination (Removed in Memora). */
...
const getAudioEngineResolution = (
  ...
  if (requested === "MEMORA-cloud" && !hasCloudAuth) {
    return {
      requested,
      active: fallback,
...
// â”€â”€â”€ Transcription Dictionary â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return (
    <Card className="border-border bg-card">
      <CardContent className="px-3 py-2.5">
        ...
        >
          <Input name="vocab-word" placeholder="e.g. memora" className="h-7 text-xs flex-1" spellCheck={false} autoCorrect="off" autoCapitalize="off" />
...
export function RecordingSettings({ section }: { section: RecordingSettingsSection }) {
  ...
  // the audio engine (file at ~/.MEMORA/audio-exclusions.json); we just
  ...
  useEffect(() => {
    ...
    const needsAecSync =
      ...
      Boolean(settings.memoraAecEnabled) !== expectedSettings.memoraAecEnabled ||
  ...
  useTauriEvent("data-dir-fallback", () => {
    toast({
      ...
      description:
        "the configured data directory could not be accessed. recordings are using the default directory (~/.memora).",
      variant: "destructive",
      duration: 10000,
    });
  });
  ...
  const handleUpdate = async () => {
    ...
    if (resWriteExcl.status === "error") throw new Error(resWriteExcl.error);
      ...
      await (needsServerRestart ? commands.stopMemora() : commands.stopCapture());
      await new Promise((resolve) => setTimeout(resolve, 500));
      await (needsServerRestart ? commands.spawnMemora(null) : commands.startCapture());
      ...
      toast({
        ...
        description: needsServerRestart
          ? "Memora server restarted with new settings"
...
Your screen is a pipe. Everything you see, hear, and type flows through it. Memora just makes sure nothing valuable leaks away.`;
      ...
      <div className="space-y-2 pt-2">
              ...
              <div className="flex min-w-0 items-center space-x-2.5">
                ...
                <div className="min-w-0">
                  <h3 className="text-sm font-medium text-foreground">
                    Hide memora from screen capture
                  </h3>
                  <p className="text-xs text-muted-foreground">
                    You&apos;ll still see memora, but it stays hidden from screenshots and anyone watching your shared screen.
                  </p>
                </div>
              </div>
              <Switch
                ...
                >
                  ...
                  <SelectContent>
                    <SelectGroup>
                      ...
                      <SelectItem value="MEMORA-cloud">
                        Memora Cloud{hwCapability?.recommendedEngine === "MEMORA-cloud" && " â˜…"}
                      </SelectItem>
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => {
                      const mode: AecMode = checked
                        ? (isMacOS ? "macos" : isWindows ? "windows" : "memora")
                        : "off";
        ...
        <Card className="border-border bg-card">
          <CardContent className="px-3 py-2.5">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center space-x-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5">
                    Live meeting notes
                    <HelpTooltip text="Streams only the active meeting into the live note. This is separate from background 24/7 recording and can use your selected transcription engine, memora cloud, or a direct provider." />
                  </h3>
                ...
                {(settings.meetingLiveTranscriptionEnabled ?? true) && (
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="MEMORA-cloud">memora cloud live</SelectItem>
            ...
            {(settings.meetingLiveTranscriptionEnabled ?? true) &&
              (settings.meetingLiveTranscriptionProvider ?? "selected-engine") === "MEMORA-cloud" &&
              !settings.user?.token && (
              <p className="mt-2 ml-[26px] text-xs text-muted-foreground">
                Log in to memora cloud to use the cloud live provider.
              </p>
            )}
            {(settings.meetingLiveTranscriptionEnabled ?? true) &&
            ...
            <div className="mt-2 pt-2 border-t border-border/50 flex items-center justify-between">
              <span className="text-xs text-muted-foreground flex items-center gap-1.5">
                ...
                <HelpTooltip text="When the meeting stops, memora appends what you typed (and the files you edited) during the meeting to the meeting note. Turn off to keep notes clean." />
              </span>
              <Switch
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center space-x-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Turn what Memora already captures into a clearer view for AI. Nothing is recorded twice.
                      </p>
                    </div>
                  </div>
