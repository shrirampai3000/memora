...
// ---------------------------------------------------------------------------
// Canonical memora SKILL.md content. Re-exported from a generated module so
// the "copy / save SKILL.md" text the cards serve is always the source of truth
// from the repo: .claude/skills/{MEMORA-api,MEMORA-cli}/SKILL.md
...
if you have the memora MCP tools (search-content, activity-summary, list-meetings,
update-memory), use them. otherwise query the local REST API at http://localhost:3030
(or http://MEMORA_IP:3030 if i run memora on another machine):
...
so you don't pull too much. if memora skills are available, load them first.
...
- if you have the memora update-memory tool: write each durable fact as a memory
  with namespaced tags (person:, project:, topic:) and importance 0-1. retrieve later
  with search-content content_type='memory'. this is the same memory the memora
...
export type AgentCardProps = {
  ...
  /** One or more installable memora skills (e.g. API + CLI). First is shown by default. */
  skills: SkillVariant[];
  sync: {
    /** Default remote path on the VPS where ~/.memora gets pushed. */
    defaultRemotePath: string;
    /** Prefix used for localStorage keys + posthog event names. */
    storageKeyPrefix: string;
  };
  /** If set, renders a "Connect" tab for entering credentials that memora pipes use to call this agent. */
  connect?: {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground leading-relaxed">
        Register memora as an MCP server in {name}. Best when {name} runs on the
        same machine as MEMORA.
      </p>
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5 pt-2 border-t border-border">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          ...
          host. To let {name} reach the memora data on <em>this</em> machine, use the{" "}
...
function RemoteSyncSection({
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground leading-relaxed">
        Push your <code className="bg-muted px-1 rounded">~/.memora</code> over
...
export function ConnectSection({ integrationId, fields }: { integrationId: string; fields: ConnectField[] }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground leading-relaxed">
        Let memora scheduled tasks call back to this agent. Enter the gateway credentials so scheduled tasks can send events and messages directly to it.
      </p>
      {fields.map((field) => (
...
function SecondBrainCallout({ name }: { name: string }) {
  ...
  const saveMd = useCallback(async () => {
    setIsSaving(true);
    try {
      await writeTextFile("MEMORA-second-brain.md", SECOND_BRAIN_PROMPT, {
        baseDir: BaseDirectory.Download,
      });
      const dir = await downloadDir();
      setSavedPath(await join(dir, "MEMORA-second-brain.md"));
      toast({ title: "saved to Downloads", description: "MEMORA-second-brain.md" });
  ...
  return (
    <div className="rounded-lg border border-border bg-muted/40 p-3 space-y-2">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <a
          href="#"
          onClick={(e) => { e.preventDefault(); openUrl("https://docs.github.com/shrirampai3000/memora/second-brain"); }}
