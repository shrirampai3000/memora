...
export function StorageSection() {
  ...
  const handleApplyRestart = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "restarted",
        description: "memora restarted with the new data directory",
      });
    } catch (error) {
      console.error("failed to restart memora:", error);
      toast({
        title: "restart failed",
        description: "please restart memora manually for the change to apply",
        variant: "destructive",
        duration: 5000,
      });
    } finally {
