...
const GITHUB_RELEASES_API = "https://api.github.com/repos/MEMORA/MEMORA/releases";
...
async function installGrokMcp(): Promise<void> {
  ...
  const next = servers.filter((s) => s?.id !== "memora");
...
async function uninstallGrokMcp(): Promise<void> {
  ...
  mcp.servers = (mcp.servers as Record<string, unknown>[]).filter((s) => s?.id !== "memora");
  await writeFile(configPath, new TextEncoder().encode(JSON.stringify(config, null, 2)));
}
...
const INTEGRATION_ICONS: Record<string, React.ReactNode> = {
    ...
    // user-browser: your real Chrome/Arc/Edge via the memora extension.
...
function ClaudePanel({ onConnected, onDisconnected }: { onConnected?: () => void; onDisconnected?: () => void }) {
  ...
  useEffect(() => {
    getInstalledClaudeMemoraEntry().then(async (entry) => {
      if (!entry) return;
      ...
      if (isStaleClaudeMemoraEntry(entry)) {
        try {
          const next = await buildMcpConfig({ client: "claude" });
          if (next.env?.MEMORA_LOCAL_API_KEY) {
  ...
  const handleConnect = async () => {
    try {
      ...
      if (mcp.command === "npx") {
        await message(
          "connected, but memora couldn't find its bundled runtime, so it wrote a config that needs Node.js installed.\n\nif Claude can't start memora, install Node (https://nodejs.org) or reinstall the memora app, then reconnect.",
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Install the memora MCP plus API and CLI skills for Claude in one click.
      </p>
...
function CodexPanel({ onConnected, onDisconnected }: { onConnected?: () => void; onDisconnected?: () => void }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">Install the memora MCP plus API and CLI skills for Codex in one click.</p>
      <div className="flex flex-wrap gap-2">
        {state === "installed" ? (
          <Button onClick={handleDisconnect} variant="outline" size="sm" className="gap-1.5 h-7 text-xs normal-case font-sans tracking-normal">
            <LogOut className="h-3 w-3" />disconnect
...
function ClaudeCodePanel() {
  ...
  const cmd = "claude mcp add memora -- npx -y MEMORA-mcp@latest";
...
// memory-sync feature backed by the MEMORA-connect Integrations of
  ...
  return (
    <div className="border-t border-border pt-3 mt-3 space-y-2">
      <div className="space-y-0.5">
        ...
        <p className="text-xs text-muted-foreground">
          writes your memora memories into {targetFilename} so {assistantName} sees them
...
const OBSIDIAN_DEFAULT_FOLDER = "memora";
...
function ObsidianMemorySyncSubsection() {
  ...
  const notePath = `${vaultPath.replace(/[\\/]+$/, "")}/${folderClean}/MEMORA-memories.md`;
  ...
  return (
    <div className="border-t border-border pt-3 mt-1 space-y-2">
      <div className="space-y-0.5">
        <p className="text-xs font-medium text-foreground">memory sync (beta)</p>
        <p className="text-xs text-muted-foreground">
          writes your memora memories into a note in this vault so they show up
...
function MstyPanel() {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        3. Give the tool a name (e.g. <strong>memora</strong>) and click <strong>Add</strong>
      </p>
...
function OllamaPanel() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Use Ollama as a local AI provider for MEMORA.
      </p>
...
function LMStudioPanel() {
  const [status, setStatus] = useState<"idle" | "checking" | "connected" | "error">("idle");
  const [models, setModels] = useState<string[]>([]);
  const deeplink = "lmstudio://add_mcp?name=memora&config=eyJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsInNjcmVlbnBpcGUtbWNwIl19";
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Connect LM Studio to memora&apos;s screen &amp; audio data, or use it as a local AI provider.
      </p>
      <div className="flex flex-wrap gap-2">
        <Button onClick={() => openUrl(deeplink)} size="sm" className="gap-1.5 h-7 text-xs normal-case font-sans tracking-normal">
          <Download className="h-3 w-3" /> add memora MCP to LM Studio
        </Button>
...
> = {
  slack: [
    { id: "send", label: "Send only", description: "Post messages as you. Memora can't read your Slack." },
...
function OAuthPanel({
  ...
  const handleCancel = async () => {
    ...
    // cancelâ†’connect sequence can race: a late-arriving oauth_cancel would
...
function ObsidianPanel({ onConnected, onDisconnected }: { onConnected?: () => void; onDisconnected?: () => void }) {
  ...
  const handleConnect = async (rawPath: string) => {
    ...
    try {
      ...
      // First vault â†’ default connection (backward compatible). Additional
      // vaults â†’ a named instance keyed by a unique slug of the folder name.
...
// OAuth flow in one click. A provider may register memora as a *confidential*
// client (client_secret_basic) during dynamic client registration; that is
// handled generically in MEMORA-connect's mcp_servers.rs, so a new provider
...
export function ConnectionsSection({
  ...
  // Composio-backed connections (managed auth through github.com/shrirampai3000/memora; see
  ...
  useEffect(() => {
    ...
    fetch(memoraWebUrl("/api/composio/status", "https://github.com/shrirampai3000/memora"), {
  ...
  const renderPanel = () => {
    ...
    switch (selected) {
      ...
      case "krisp": return <OAuthMcpPanel
        ...
        description={<>Connect Krisp so your AI can search your meeting transcripts, notes, and action items. Sign-in is handled by Krisp&apos;s OAuth, so memora never sees your password.</>}
      ...
      case "plaud": return <OAuthMcpPanel
        ...
        description={<>Connect Plaud so your AI can search your Plaud recordings, transcripts, summaries, and notes. Sign-in is handled by Plaud&apos;s OAuth, so memora never sees your password.</>}
      ...
      default:
        ...
        return (
          <div className="space-y-2">
            <p className="text-xs text-muted-foreground">
              couldn&apos;t load connection metadata. the local memora server may be starting up.
            </p>
            <Button
              size="sm"
              variant="outline"
