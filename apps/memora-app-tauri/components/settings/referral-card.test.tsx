...
const referral = {
  code: "REF-A1B2C3D4",
  link: "https://github.com/shrirampai3000/memora/?ref=REF-A1B2C3D4",
  redemptions: 2,
  rewardsEarned: 1,
  maxRedemptions: 12,
};
...
describe("ReferralCard", () => {
  ...
  it("uses the backend-issued referral link instead of inventing one from the user id", async () => {
    mocks.user = { email: "paid+test@github.com/shrirampai3000/memora", token: "token-1" };
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://github.com/shrirampai3000/memora/api/referral?email=paid%2Btest%40github.com/shrirampai3000/memora",
  ...
  it("explains paid-plan eligibility instead of showing a broken trial link", async () => {
    mocks.user = { email: "trial@github.com/shrirampai3000/memora", token: "token-1" };
    vi.stubGlobal(
      "fetch",
  ...
  it("shows a retry path when referral loading fails", async () => {
    mocks.user = { email: "paid@github.com/shrirampai3000/memora", token: "token-1" };
  ...
  it("sends invitations with the authenticated backend-issued link", async () => {
    ...
    expect(fetchMock).toHaveBeenLastCalledWith(
      "https://github.com/shrirampai3000/memora/api/referral/invite",
      expect.objectContaining({
        ...
        body: JSON.stringify({
          ...
          senderName: "paid@github.com/shrirampai3000/memora",
        }),
      }),
    );
  });
