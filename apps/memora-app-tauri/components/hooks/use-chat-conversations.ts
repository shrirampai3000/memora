...
interface UseChatConversationsOpts {
  ...
  // â”€â”€ Per-conversation composer draft snapshot â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export function useChatConversations(opts: UseChatConversationsOpts) {
  ...
  const [showHistory, setShowHistoryRaw] = useState(() => {
    try { return localStorage.getItem("memora:chat-history-open") === "true"; } catch { return false; }
  });
  const setShowHistory = useCallback((v: boolean | ((prev: boolean) => boolean)) => {
    setShowHistoryRaw((prev) => {
      const next = typeof v === "function" ? v(prev) : v;
      try { localStorage.setItem("memora:chat-history-open", String(next)); } catch {}
      return next;
    });
  }, []);
  ...
  const saveConversation = async (
    ...
    // plumbing wrappers (<attached file: ...>, <MEMORA-large-context>, etc.)
        ...
        try {
          ...
          if (aiTitle) {
            ...
            if (existingConv.titleSource === "fallback" || !existingConv.titleSource) {
              ...
              // so the sidebar transitions directly from partial â†’ final
  ...
  useEffect(() => {
    if (prevIsLoadingRef.current && !isLoading && messages.length > 0) {
      ...
      // If the user has typed follow-up messages, some won't have pipe- IDs â†’ save.
  ...
  // isLoading: true â†’ false edge above. Quitting the app mid-stream lost
