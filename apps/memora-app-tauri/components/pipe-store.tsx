...
function DiscoverView({ onInstalled }: { onInstalled?: () => void }) {
  ...
  useEffect(() => {
    if (!localStorage.getItem("memora:pipes-welcome-dismissed")) {
      setShowWelcome(true);
    }
  }, []);
  ...
  const dismissWelcome = () => {
    ...
    localStorage.setItem("memora:pipes-welcome-dismissed", "1");
  };
  ...
  const requestInstall = (pipe: StorePipe | PipeDetail) => {
    // Already installed with an update available â†’ run the in-place UPDATE flow
  ...
  const handleStoreUpdate = async (slug: string, pipeName: string) => {
    ...
    try {
      ...
      // Clear just this card's update state â†’ re-renders one card, no reload.
...
function PublisherIdentity({
  ...
  return (
    <div
      ...
      aria-label={publisher.isMemoraTeam ? "official Memora publisher" : undefined}
      title={compact && publisher.isMemoraTeam ? "built by memora team" : undefined}
      className={cn(
    ...
    >
      ...
      >
        {compact ? (
          ...
          <>
            {publisher.isMemoraTeam ? "built by " : "by "}
            <span className="font-medium text-foreground">{publisher.name}</span>
          </>
        )}
      </span>
      {publisher.verified && !publisher.isMemoraTeam && (
        <BadgeCheck
