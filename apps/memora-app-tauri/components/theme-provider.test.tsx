...
describe("ThemeProvider", () => {
  it("replaces a stale local cache with the shared settings theme", async () => {
    localStorage.setItem("MEMORA-ui-theme", "light");

    renderThemeProvider();

    await waitFor(() => {
      expect(localStorage.getItem("MEMORA-ui-theme")).toBe("system");
  ...
  it("uses an explicit shared setting even when localStorage disagrees", async () => {
    ...
    await waitFor(() => {
      ...
      expect(localStorage.getItem("MEMORA-ui-theme")).toBe("dark");
      expect(document.documentElement).toHaveClass("dark");
    });
  ...
  it("follows shared setting changes from another window", async () => {
    ...
    await waitFor(() => {
      ...
      expect(localStorage.getItem("MEMORA-ui-theme")).toBe("light");
