...
async function getWindowsUpdateOptions() {
  ...
  return {
    checkOptions: {
      endpoints: [
        memoraWebUrl(`/api/app-update/${channel}/windows-${cpuArch}/{{current_version}}`, "https://github.com/shrirampai3000/memora"),
      ],
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (os === "windows") {
        ...
        // Stop memora before update on Windows
        try {
          await commands.stopMemora();
        } catch (e) {
          console.warn("failed to stop memora:", e);
        }
        ...
        toast({
          ...
          description: "memora will restart automatically",
          duration: 10000,
        });
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-2 flex-1">
        <Sparkles className="h-4 w-4 text-primary" />
        <span>
          memora <span className="font-medium">v{updateInfo.version}</span> is ready
        </span>
      </div>
      <div className="flex items-center gap-2">
        <Button
