...
const processMarkerPrefix = process.env.MEMORA_MOCK_PROCESS_MARKER_PREFIX;
const processMarkerToken = process.env.MEMORA_MOCK_PROCESS_MARKER_TOKEN;
const forbiddenCloudToken = process.env.MEMORA_API_KEY;
if (scenario === "terminal" && forbiddenCloudToken) {
  throw new Error("ACP adapter inherited forbidden MEMORA_API_KEY");
}
const descendant = (() => {
  ...
  const script = [
    ...
    "const marker = process.env.MEMORA_MOCK_DESCENDANT_MARKER;",
    "const token = process.env.MEMORA_MOCK_PROCESS_MARKER_TOKEN;",
    "fs.writeFileSync(marker, JSON.stringify({ token, pid: process.pid }) + '\\n', { flag: 'wx' });",
    // Staying alive after stdin/parent shutdown forces MEMORA to reap the
  ...
  const child = spawn(process.execPath, ["-e", script], {
    env: {
      ...process.env,
      MEMORA_MOCK_DESCENDANT_MARKER: descendantMarker,
      MEMORA_MOCK_PROCESS_MARKER_TOKEN: processMarkerToken,
    },
    stdio: "ignore",
    windowsHide: true,
  });
...
async function verifyTerminalLifecycle(cwd: string): Promise<void> {
  const created = await requestClient("terminal/create", {
    ...
    args: [
      "-e",
      "if (process.env.MEMORA_API_KEY) process.exit(41); process.stdout.write('terminal-head-'); process.stderr.write('terminal-tail')",
    ],
    cwd,
    env: [{ name: "MEMORA_API_KEY", value: "forbidden-terminal-sentinel" }],
...
async function verifyMEMORAMcpRuntime(server: Record<string, unknown>): Promise<void> {
  ...
  if (typeof command !== "string" || !command || args.length === 0) {
    throw new Error("MEMORA MCP stdio command is incomplete");
  }
  ...
  child.on("exit", (code, signal) => {
    rejectPending(new Error(
      `MEMORA MCP exited before verification (code=${code}, signal=${signal}): ${stderr}`,
    ));
  });
  ...
  const request = (method: string, params: Record<string, unknown>) => {
    ...
    return new Promise<Record<string, unknown>>((resolve, reject) => {
      const timer = setTimeout(() => {
        pending.delete(id);
        reject(new Error(`MEMORA MCP ${method} timed out: ${stderr}`));
      }, 60_000);
  ...
  try {
    await request("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "MEMORA-acp-e2e", version: "1.0.0" },
    });
    ...
    if (!toolRows.some((tool) =>
      ...
      throw new Error("MEMORA MCP did not expose health-check");
    }
    const health = await request("tools/call", { name: "health-check", arguments: {} });
    if (health.isError === true) {
      throw new Error(`MEMORA MCP health-check failed: ${JSON.stringify(health.content)}`);
    }
    ...
    if (typeof healthText?.text !== "string" || !healthText.text.includes("status")) {
      throw new Error(`MEMORA MCP returned invalid health payload: ${JSON.stringify(health)}`);
    }
  } finally {
    child.stdin.end();
    child.kill();
    rejectPending(new Error("MEMORA MCP verification finished"));
  }
}
  ...
  switch (message.method) {
    ...
    case "session/new": {
      ...
      if (scenario === "auth" && !authenticated) {
        ...
        fail(message.id, -32000, "èªè¨¼ãŒå¿…è¦ã§ã™ã€‚");
        return;
      }
      if (sessionOpen) {
      ...
      if (scenario === "mcp") {
        const expectedUrl = process.env.MEMORA_MOCK_EXPECT_MCP_URL;
        const expectedKey = process.env.MEMORA_MOCK_EXPECT_MCP_KEY;
        const expectedHasKey = process.env.MEMORA_MOCK_EXPECT_MCP_HAS_KEY === "true";
        const server = mcpServers.find((candidate) => {
          return candidate && typeof candidate === "object" &&
            (candidate as Record<string, unknown>).name === "MEMORA";
        }) as Record<string, unknown> | undefined;
        ...
        const hasExpectedUrl = Boolean(expectedUrl) &&
          args.includes("--MEMORA-url") &&
          args.includes(expectedUrl) &&
          serverEnv.MEMORA_API_URL === expectedUrl;
        const hasExpectedKey = expectedHasKey
          ? Boolean(expectedKey) && serverEnv.MEMORA_LOCAL_API_KEY === expectedKey
          : !("MEMORA_LOCAL_API_KEY" in serverEnv);
        if (!hasExpectedUrl || !hasExpectedKey) {
          fail(message.id, -32602, "MEMORA MCP URL/key registration mismatch");
          return;
        }
        // The bundled companion tools server must be registered alongside the
        // core MEMORA server, launched from a staged local file.
        const toolsServer = mcpServers.find(
          (candidate) =>
            candidate && typeof candidate === "object" &&
            (candidate as Record<string, unknown>).name === "MEMORA-tools",
        ) as Record<string, unknown> | undefined;
        const toolsArgs = Array.isArray(toolsServer?.args) ? toolsServer.args : [];
        if (!toolsServer || !toolsArgs.some((a) => String(a).endsWith("MEMORA-tools.mjs"))) {
          fail(message.id, -32602, "bundled MEMORA-tools MCP server was not registered");
          return;
        }
        try {
          await verifyMEMORAMcpRuntime(server ?? {});
        } catch (error) {
          fail(
            message.id,
            -32603,
        ...
        update({
          sessionUpdate: "agent_message_chunk",
          content: { type: "text", text: "Mock MEMORA MCP registration verified" },
