...
const isDestructiveDbFaultRun = (process.env.MEMORA_E2E_SEED ?? '')
  .split(',')
...
let previousMEMORAApiKey: string | undefined;
...
async function startLocalGatewayIfRequested(): Promise<void> {
  if (process.env.MEMORA_E2E_LOCAL_AI_GATEWAY !== 'true') return;
  ...
  process.env.MEMORA_E2E_AI_GATEWAY_URL = gatewayRequestProxy.baseUrl;
  process.env.MEMORA_E2E_LOCAL_AI_GATEWAY_TOKEN =
    gatewayModule.LOCAL_GATEWAY_SERVICE_TOKEN;
  previousMEMORAApiKey = process.env.MEMORA_API_KEY;
  process.env.MEMORA_API_KEY = gatewayModule.LOCAL_GATEWAY_SERVICE_TOKEN;
  process.env.MEMORA_E2E_GATEWAY_REQUESTS_FILE = GATEWAY_REQUESTS_FILE;
  process.env.MEMORA_E2E_GATEWAY_INBOUND_REQUESTS_FILE =
...
async function stopLocalGateway(): Promise<void> {
  ...
  if (previousMEMORAApiKey === undefined) {
    delete process.env.MEMORA_API_KEY;
  } else {
    process.env.MEMORA_API_KEY = previousMEMORAApiKey;
  }
  previousMEMORAApiKey = undefined;
...
export const config: TestrunnerConfig = {
  ...
  onPrepare: async () => {
    console.log('Starting MEMORA app (WebDriver on port %s)...', WEBDRIVER_PORT);
