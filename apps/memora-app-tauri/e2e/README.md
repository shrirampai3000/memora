...
Cross-platform E2E for MEMORA using [tauri-plugin-webdriver](https://crates.io/crates/tauri-plugin-webdriver). macOS, Windows, Linux.
...
```bash
NEXT_PUBLIC_MEMORA_E2E=true bun tauri build --no-sign --debug --verbose --no-bundle -- --features e2e
```

- `NEXT_PUBLIC_MEMORA_E2E=true` â€” compiles in the frontend E2E hooks. CI sets
...
- `--no-sign` â€” skip code signing (dev)
- `--debug` â€” debug build, faster than release
- `--verbose` â€” show build output
- `--no-bundle` â€” binary only, no installer
- `-- --features e2e` â€” enable WebDriver plugin
...
This uses `MEMORA_E2E_SEED=onboarding,no-recording,cloud-audio-fallback`
to keep vision capture off while leaving the audio settings visible with
MEMORA Cloud saved and no logged-in user. It asserts the Recording fallback
...
app names), and that the model's text â€” never the deterministic fallback â€” is
...
`MEMORA_E2E_SEED=onboarding` (vision ON) so it needs a host with **Screen
...
window â€” i.e. high-fps capture and normal indexing run concurrently (#3699 /
...
`MEMORA_FOCUS_PORT` (default `11436`) so tests can exercise `/notify` and
...
- **Bun** â‰¥ 1.3.10 â€” `winget install oven-sh.bun` or from [bun.sh](https://bun.sh)
- **Rust** stable (x86_64-pc-windows-msvc) â€” `rustup target add x86_64-pc-windows-msvc`
- **MSVC build tools** â€” Visual Studio 2022 Build Tools with C++ workload
- **ONNX Runtime** â€” the pre_build script downloads this automatically during `bun tauri build`
- No Scream audio driver needed for local runs (only required in CI for audio capture tests)

### Step-by-step (PowerShell)

```powershell
# 1. Install frontend dependencies (from repo root or apps/MEMORA-app-tauri)
...
$env:MEMORA_E2E_SEED="onboarding,no-audio"; bun run wdio run e2e/wdio.conf.ts --spec e2e/specs/windows-core-recording.spec.ts
...
$env:MEMORA_E2E_SEED="onboarding,no-audio,event-trigger-capture"; bun run wdio run e2e/wdio.conf.ts --spec e2e/specs/windows-core-recording.spec.ts

# 7. Run the same trigger lane with raw key DB rows opted in; verifies key rows
#    get linked frame_id too.
$env:MEMORA_E2E_SEED="onboarding,no-audio,event-trigger-capture,keyboard-db-capture"; bun run wdio run e2e/wdio.conf.ts --spec e2e/specs/windows-core-recording.spec.ts
```
...
| `.e2e/` | Isolated MEMORA data dir used during tests (deleted on each run) |
...
**Binary not found**
```
Error: MEMORA debug binary not found at â€¦\src-tauri\target\debug\MEMORA-app.exe
```
...
Check `apps/MEMORA-app-tauri/.e2e/` for log files after a run. The app launcher pipes stdout/stderr with an `[app]` prefix to the test runner console.
...
| `settings-sections.spec.ts` | Navigates General â†’ Recording â†’ AI Presets â†’ AI Settings â†’ Speakers; verifies moved controls, enabled/disabled analysis flows, content, and no crash |
| `audio-fallback.spec.ts` | macOS opt-in spec for the MEMORA Cloud â†’ local Whisper fallback alert and `/notify` history |
...
| `parallel-chat.spec.ts` | Drives chat-load-conversation + fake `pi_event` envelopes from the webview to walk Louis's repro: chat A â†’ chat B â†’ back to A. Asserts A's messages are still in the DOM (catches the "switch wipes A" regression) and that backgrounded streaming does NOT reorder sidebar rows. |
| `chat-settings-background-stream.spec.ts` | Starts a long synthetic chat stream, navigates into the standalone `/settings` route mid-stream (unmounting the home page), then returns. Asserts the running chat remains live in Recents and, after clicking it, the full response â€” early tokens (snapshotted on unmount) plus the final token (streamed in the background) â€” is present. Catches the "opening Settings stops the current chat" regression. |
