...
export class PiConversationHarness {
  ...
  /**
   * Start Pi with the MEMORA provider. The E2E app build resolves the
      ...
      request.on("end", () => {
        if (request.method === "GET" && request.url?.endsWith("/models")) {
          ...
          response.end(
            JSON.stringify({
              object: "list",
              data: [{ id: "MEMORA-e2e", object: "model" }],
            }),
          );
          return;
        }
        ...
        setTimeout(() => {
          ...
          if (scriptedToolCall) {
            const callId = `call_MEMORA_e2e_${this.scriptedToolCallIndex}`;
            response.end(
