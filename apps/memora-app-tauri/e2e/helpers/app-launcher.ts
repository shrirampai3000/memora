...
process.env.MEMORA_PORT ??= String(LOCAL_API_PORT);
...
// e2e/helpers/ â†’ ../.. = app root (Bun runs from source, no dist)
const APP_ROOT = resolve(__dirname, '../..');
export const E2E_DATA_DIR = resolve(
  process.env.MEMORA_E2E_DATA_DIR ??
    resolve(homedir(), '.MEMORA', '.e2e'),
);
...
//
// Override with `MEMORA_E2E_SEED=onboarding` (or any custom value) when
...
// leaves audio UI enabled, disables vision, and saves MEMORA Cloud while
...
export const E2E_SEED_FLAGS =
  process.env.MEMORA_E2E_SEED ?? 'onboarding,no-recording,search-fixture';
...
export function getAppPath(): string {
  ...
  const name = process.platform === 'win32' ? 'MEMORA-app.exe' : 'MEMORA-app';
...
export async function startApp(port = WEBDRIVER_PORT): Promise<ReturnType<typeof spawn> | null> {
  ...
  if (!existsSync(appPath)) {
    throw new Error(
      `MEMORA debug binary not found at ${appPath}. Build with e2e feature: cd apps/MEMORA-app-tauri && bun tauri build --no-sign --debug --verbose --no-bundle -- --features e2e`
    );
  }
  ...
  const launchedProcess = spawn(appPath, [], {
    env: {
      ...process.env,
      MEMORA_DATA_DIR: E2E_DATA_DIR,
      MEMORA_E2E_SEED: E2E_SEED_FLAGS,
      MEMORA_FOCUS_PORT: String(FOCUS_PORT),
      MEMORA_PORT: String(LOCAL_API_PORT),
      TAURI_WEBDRIVER_PORT: String(port),
      ...(backgroundAiToolsEnabled
        ? {
            MEMORA_E2E_AI_TOOLS_HOME: E2E_AI_TOOLS_HOME,
            MEMORA_E2E_BUN_PATH: E2E_BUN_PATH,
          }
        : {}),
  ...
  if (process.env.MEMORA_E2E_QUIET_APP_LOGS !== 'true') {
