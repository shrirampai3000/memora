...
guard let path = ProcessInfo.processInfo.environment["MEMORA_SPOTLIGHT_CANARY_DIR"] else {
    exit(2)
}
...
export function removeSpotlightExclusion(directory: string): void {
  ...
  execFileSync("/usr/bin/swift", ["-e", REMOVE_EXCLUSION_SWIFT], {
    env: {
      ...process.env,
      MEMORA_SPOTLIGHT_CANARY_DIR: canonicalDirectory,
    },
...
export async function assertSpotlightExclusionCanary(
  ...
  const controlDir = mkdtempSync(join(tmpdir(), "MEMORA-spotlight-control-"));
  const filename = `MEMORA-spotlight-canary-${randomUUID()}.txt`;
