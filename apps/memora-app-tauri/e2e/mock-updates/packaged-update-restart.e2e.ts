 ...
 *     â†’ real download â†’ signature verify â†’ stage + PRE-EXTRACT
 *     â†’ tray click (exact production handler, via the e2e driver route)
 *     â†’ restart gate (boot phase = idle: the signed-out MacBook Air repro)
 *     â†’ teardown â†’ rename fast-path install â†’ relaunch
...
// â”€â”€ Small utils â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function todayLog(): Promise<string> {
  // Rolling appender: MEMORA-app.YYYY-MM-DD.log in the data dir.
  const day = new Date().toISOString().slice(0, 10);
  const p = path.join(DATA_DIR, `MEMORA-app.${day}.log`);
  try {
...
function launchApp(): ReturnType<typeof Bun.spawn> {
  const bin = path.join(installedApp(), 'Contents', 'MacOS', 'MEMORA-app');
  must('installed app binary exists', existsSync(bin), bin);
  return Bun.spawn([bin], {
    cwd: WORKDIR,
    env: {
      ...process.env,
      MEMORA_DATA_DIR: DATA_DIR,
...
// â”€â”€ Phases â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function buildArtifacts(oldVersion: string): void {
  ...
  //    the build config (e2e/official â‡’ "MEMORA - Development.app"), so
  //    discover it rather than assume "MEMORA.app".
...
async function main(): Promise<void> {
  ...
  log(`old app version: ${oldVersion} â†’ update version: ${NEW_VERSION}`);
  ...
  try {
    ...
    // Real check â†’ download â†’ verify â†’ stage â†’ PRE-EXTRACT.
    ...
    // Rust Duration debug formatting: "123.456Âµs" | "12.345ms" | "1.234s".
    const fastMs = logAfter.match(/installed via pre-extracted fast path in ([\d.]+)(Âµs|ms|s)/);
    ...
    check(
      ...
      logAfter.includes(`update attempt recorded: ${oldVersion} â†’ ${NEW_VERSION}`),
    );
    check(
      'log: marker classified as applied on new boot',
      logAfter.includes(`previous update install applied: ${oldVersion} â†’ ${NEW_VERSION}`),
    );
    check('log: no deferred-restart warnings', !logAfter.includes('deferring restart to avoid'));

    // â”€â”€ Scenario 4: post-update state + click-with-nothing-staged safety â”€â”€â”€â”€â”€

    const settled = await waitFor('post-update updater settled', 60_000, async () => {
      const s = await fetchState();
      // manifest serves 9.9.9 == current â‡’ updater returns None â‡’ no staged update
    ...
    const day = new Date().toISOString().slice(0, 10);
    const src = path.join(DATA_DIR, `MEMORA-app.${day}.log`);
    if (existsSync(src)) {
  ...
  console.log('\nâ•â•â•â•â•â•â•â• packaged updater e2e â•â•â•â•â•â•â•â•');
...
try {
  ...
  console.error('\nâ•â•â•â•â•â•â•â• packaged updater e2e ABORTED â•â•â•â•â•â•â•â•');
