 ...
 * `MEMORA_E2E_SEED=onboarding,no-audio` in CI and verifies the real core
...
const WINDOWS_CPU_SAMPLE_DURATION_MS = Number(
  process.env.MEMORA_E2E_WINDOWS_CPU_SAMPLE_DURATION_MS ?? "30000",
);
const WINDOWS_CPU_USER_AVG_MAX_PERCENT = Number(
  process.env.MEMORA_E2E_WINDOWS_CPU_USER_AVG_MAX_PERCENT ?? "25",
);
const WINDOWS_CPU_USER_P95_MAX_PERCENT = Number(
  process.env.MEMORA_E2E_WINDOWS_CPU_USER_P95_MAX_PERCENT ?? "50",
);
const WINDOWS_CPU_USER_MAX_MAX_PERCENT = Number(
  process.env.MEMORA_E2E_WINDOWS_CPU_USER_MAX_MAX_PERCENT ?? "80",
);
...
public static class MEMORAE2ECpuWindow {
...
try {
  ...
  while (-not $notepad.HasExited -and (Get-Date) -lt $deadline) {
    $activated = [MEMORAE2ECpuWindow]::SetForegroundWindow($handle)
    Start-Sleep -Milliseconds 100
    if (-not $activated -or [MEMORAE2ECpuWindow]::GetForegroundWindow() -ne $handle) {
  ...
  [void][MEMORAE2ECpuWindow]::SetForegroundWindow($handle)
  Start-Sleep -Milliseconds 100
...
$form.Text = 'MEMORA E2E Capture Marker'
...
using System;
using System.Runtime.InteropServices;
public static class MEMORAE2EForeground {
  [DllImport("user32.dll")]
  public static extern bool SetForegroundWindow(IntPtr hWnd);
}
"@
...
$timer.Add_Tick({
  if ($script:step -eq 0) {
    [void][MEMORAE2EForeground]::SetForegroundWindow($formA.Handle)
    $formA.Activate()
  } elseif ($script:step -eq 1) {
    [void][MEMORAE2EForeground]::SetForegroundWindow($formB.Handle)
    $formB.Activate()
...
$timer.Interval = 750
$timer.Add_Tick({
  [void][MEMORAE2EForeground]::SetForegroundWindow($form.Handle)
  $form.Activate()
  $textbox.Focus()
  if ($script:step -lt 0) {
...
describe("Windows core recording pipeline", function () {
  ...
  async function probeMarkerIndexing(): Promise<MarkerProbe> {
    ...
    const marker = `MEMORA CORE CAPTURE MARKER ${Date.now()}`;
  ...
  it("keeps capture responsive and process-tree CPU bounded while typing", async function () {
    ...
    if (!rootPid) throw new Error("MEMORA app PID is unavailable for CPU sampling");

    const marker = `MEMORA CPU CAPTURE ${Date.now()}`;
  ...
  it("links window focus events to captured frames", async function () {
    ...
    const marker = `MEMORA FOCUS TRIGGER ${Date.now()}`;
  ...
  it("uses keystrokes as capture triggers without storing raw key rows by default", async function () {
    ...
    const marker = `MEMORA KEY TRIGGER ${Date.now()}`;
    const sinceIso = new Date(Date.now() - 1_000).toISOString();
  ...
  it("uses clipboard operations as capture triggers without storing clipboard rows by default", async function () {
    ...
    const marker = `MEMORA CLIPBOARD TRIGGER ${Date.now()}`;
    const sinceIso = new Date(Date.now() - 1_000).toISOString();
  ...
  it("links opt-in raw key rows to captured frames", async function () {
    ...
    const marker = `MEMORA KEY DB TRIGGER ${Date.now()}`;
    const sinceIso = new Date(Date.now() - 1_000).toISOString();
    cleanupMarkerWindow = spawnWindowsKeyProbe(marker);
