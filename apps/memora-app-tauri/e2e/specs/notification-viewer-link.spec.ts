...
// End-to-end coverage for the notification â†’ viewer link path:
...
// through `openNotificationLink` â†’ `openMEMORAViewerLink` â†’
...
describe("Notification â†’ viewer link rewrite + render", function () {
  ...
  before(async () => {
    tmpDir = mkdtempSync(resolvePath(tmpdir(), "MEMORA-e2e-notif-"));
  ...
  it("POST /notify rewrites a file-path markdown link to MEMORA://view", async () => {
    ...
    expect(body).toContain("MEMORA://view?path=");
  ...
  it("external https:// URLs are NOT rewritten (rewrite is scoped to local paths)", async () => {
    ...
    await postNotification({
      id: externalId,
      title: "external url",
      body: "[Docs](https://github.com/shrirampai3000/memora/docs)",
      notification_type: "pipe",
    });
    ...
    const body = ours?.body ?? "";
    expect(body).toContain("https://github.com/shrirampai3000/memora/docs");
    expect(body).not.toContain("MEMORA://view");
  });
