...
/**
 * E2E reproducer for the duplicate "Current MEMORA connected integrations
 ...
 *   - Bug present:  title/first-message = the raw blob            â†’ FAIL
 *   - Fixed:        title/first-message = the original user text   â†’ PASS
 *
...
// Fresh id the foreground panel never opens â†’ its events flow through the
// background router (registerDefault â†’ handlePiEvent), which is the path that
    ...
    try {
      ...
      return (
        ...
        (conv.title.includes("connections_context") ||
          conv.title.startsWith("Current MEMORA connected integrations context"))
      );
    } catch {
      return false;
    }
...
describe.skip("Connections-context duplicate chat (#4689)", function () {
  ...
  it("materializes the echoed user turn WITHOUT the connections-context blob", async () => {
    // (1) First event on an unknown id â†’ router lazy-creates the session
    ...
    // (3) A short assistant reply + agent_end â†’ triggers the background persist
    //     (deriveFallbackConversationTitle â†’ title from first user message).
    await emitAgentEvent(CHAT_ID, {
      type: "message_start",
    ...
    expect(conv.title ?? "").not.toContain(
      "Current MEMORA connected integrations context",
    );
