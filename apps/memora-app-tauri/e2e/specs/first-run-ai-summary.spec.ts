...
/**
 * The first thing MEMORA ever says about your work, written by a model.
 *
...
const LEARNING_STORAGE_KEY = "MEMORA.first-run.learning-window.v1";
...
function forwardedToProvider(): string {
  const path = process.env.MEMORA_E2E_GATEWAY_REQUESTS_FILE;
    ...
    (
      ...
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          email: "e2e-first-run-ai@MEMORA.test",
...
describe("First-run summary is written by the model", function () {
  ...
  before(async function () {
    if (
      ...
      process.env.MEMORA_E2E_LOCAL_AI_GATEWAY !== "true" ||
      !seedFlags.includes("onboarding")
    ) {
      this.skip();
    }
    const token = process.env.MEMORA_E2E_LOCAL_AI_GATEWAY_TOKEN;
    const gatewayUrl = process.env.MEMORA_E2E_AI_GATEWAY_URL;
    if (!token || !gatewayUrl) {
