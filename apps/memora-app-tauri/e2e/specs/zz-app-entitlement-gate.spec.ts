...
//
// The e2e build bypasses the gate by default (NEXT_PUBLIC_MEMORA_E2E) so the
...
const FORCE_KEY = "MEMORA_e2e_force_billing_gate";
const E2E_ACCOUNT_USER_KEY = "MEMORA_e2e_account_user";
const E2E_ACCOUNT_USER_EVENT = "MEMORA-e2e-seed-account-user";
const FAKE_DENIED_TOKEN = "e2e-fake-token-cloud-sub-app-denied";
const FAKE_DENIED_EMAIL = "e2e-cloud-sub-app-denied@MEMORA.test";
 ...
 *  (components/app-entitlement-gate.tsx calls stopMEMORA for an unentitled
...
async function restartEngineForTrailingSpecs(): Promise<void> {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    await invoke("spawn_MEMORA", { overrideArgs: null }).catch(() => {});
    try {
...
describe("App entitlement gate", () => {
  ...
  it("requires plan verification for conflicting cloud subscription evidence", async () => {
    try {
      ...
      await emitDeepLink(`MEMORA://login?api_key=${FAKE_DENIED_TOKEN}`);
