...
describe("Local API search and stability", function () {
  ...
  before(async () => {
    ...
    const expectedPort = Number(process.env.MEMORA_PORT ?? "3030");
  ...
  const queryCases = [
    ...
    ["spaces", "hello world"],
    ["Chinese", "ä¸­æ–‡"],
    ["accented text", "cafÃ© dÃ©jÃ  vu"],
    ["Russian", "Ð¿Ñ€Ð¸Ð²ÐµÑ‚"],
    ["emoji", "MEMORA ðŸ˜€"],
    ["HTML script tag", "<script>alert(1)</script>"],
    ["URL text", "https://github.com/shrirampai3000/memora/docs?x=1&y=2"],
    ["SQL-ish text", "'; DROP TABLE frames; --"],
    ["Windows path", "C:\\Users\\louis\\Documents\\MEMORA"],
    ["tabs and newlines", "line one\nline two\tend"],
    ["double quotes", '"quoted phrase"'],
    ["single wildcard", "*"],
    ["FTS-ish operators", "MEMORA OR test NEAR app"],
    ["punctuation", "!@#$%^&*()_+-=[]{}|;:,.<>/?"],
    ...
    ["markdown link", "[MEMORA](https://github.com/shrirampai3000/memora)"],
    ["markdown code", "`bun run test:e2e`"],
    ["email address", "test@example.com"],
    ["IPv6 address", "[::1]:3030"],
    ["percent sign", "100% match"],
    ["repeated terms", "MEMORA ".repeat(80)],
  ...
  const filterParamCases = [
    ...
    ["/search?limit=2&app_name=MEMORA.exe", "Windows executable app filter"],
    ["/search?limit=2&app_name=C%3A%5CProgram%20Files%5CMEMORA.exe", "path-like app filter"],
    ["/search?limit=2&window_name=", "empty window filter"],
    ["/search?limit=2&window_name=MEMORA", "window title filter"],
    ...
    ["/search?limit=2&q=test&window_name=MEMORA", "query plus window"],
    ...
    ["/search?limit=2&content_type=all&app_name=Google%20Chrome&window_name=MEMORA", "combined all filters"],
  ...
  const invalidAuthCases = [
    ...
    ["random auth scheme", { Authorization: "MEMORA bad" }, "/search?limit=1"],
