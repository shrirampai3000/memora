...
`);
  ...
  it("runs against the isolated E2E data directory instead of user app data", async function () {
    ...
    expect(E2E_DATA_DIR.toLowerCase()).not.toContain("\\appdata\\roaming\\.MEMORA");
...
`);
  ...
  it("does not show recent Windows Defender detections for MEMORA when Defender data is available", async function () {
    if (!isWindows) this.skip();
...
$hits = Get-MpThreatDetection -ErrorAction SilentlyContinue |
  Where-Object {
    ...
    (($_.ProcessName -like '*MEMORA*') -or ($_.Resources -join ' ' -like '*MEMORA*'))
  }
