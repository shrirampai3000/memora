    ...
    child.stdin.write(
      JSON.stringify({
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: "MEMORA-desktop-e2e", version: "1.0.0" },
        },
      }) + "\n",
    );
  });
  ...
  function () {
    ...
    it("connects detected Codex and Cursor configs in the Rust background task", async () => {
      ...
      expect(codex).toContain(
        `MEMORA_LOCAL_API_KEY = ${JSON.stringify(api.key)}`,
      );
      expect(codex).toContain(
        `MEMORA_API_URL = "http://localhost:${api.port}"`,
      );
      expect(codex).toContain('MEMORA_MCP_CLIENT = "codex"');
      ...
      expect(cursor.mcpServers.MEMORA.env).toEqual({
        MEMORA_API_URL: `http://localhost:${api.port}`,
        MEMORA_LOCAL_API_KEY: api.key,
        MEMORA_MCP_CLIENT: "cursor",
      });
