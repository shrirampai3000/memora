...
describe("Owned browser — headless background drive", function () {
  ...
  (canDriveOwnedBrowser ? it : it.skip)(
    ...
    async () => {
      ...
      expect(r.n).toBe(M);
      expect(r.marker).toBe("æ—¥æœ¬èªž");
      expect(r.tree.length).toBe(M);
      expect([...r.tree].every((c) => c === "â†’")).toBe(true);
  ...
  (canDriveOwnedBrowser ? it : it.skip)(
    ...
    async () => {
      ...
      const fill = await postAct(port, key, {
        ...
        action: "fill",
        value: "ada@github.com/shrirampai3000/memora",
      });
      ...
      expect(filled.body?.result).toBe("ada@github.com/shrirampai3000/memora");
