 ...
 * WebDriver window list of the window it's parented to (handles â†’ []), so we
...
async function readBrowserStateCacheUrl(
  ...
  }, `MEMORA:browser-state:${chatId}`)) as string | null;
}
...
describe.skip("Owned browser — per-chat navigation ownership", function () {
  ...
  (canHideBackgroundDrive ? it : it.skip)(
    ...
    async () => {
      ...
      //    path ignored the `x-MEMORA-session` header and emitted the
...
describe.skip("Owned browser — background navigation visibility", function () {
  ...
  (canDriveOwnedBrowser ? it : it.skip)(
    ...
    async () => {
      // 1. Put the home window on Meeting notes (chat layer â†’ display:none) while
