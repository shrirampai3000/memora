...
function resetRunIdentifiers(): void {
  ...
  treeMarkerPrefix = path.join(os.tmpdir(), `MEMORA-acp-process-${treeSession}`);
...
async function startAcp(
  ...
  const projectDir = path.join(os.tmpdir(), `MEMORA-acp-e2e-${sessionId}`);
  ...
  expect(existsSync(path.join(projectDir, ".MEMORA", "agent", "acp-bridge.ts"))).toBe(false);
...
async function beginAcpStart(sessionId: string, scenario: "auth"): Promise<void> {
  await browser.execute(
    ...
    {
      sessionId,
      projectDir: path.join(os.tmpdir(), `MEMORA-acp-e2e-${sessionId}`),
...
describe("ACP backend", function () {
  ...
  it("uses the official Rust ACP SDK for stream, plan, tool, permission, and cancel", async () => {
    ...
    // MEMORA gives each UI request its own id instead of suppressing it as
  ...
  it("reaps adapter descendants when the Rust ACP runtime stops", async () => {
    ...
    await startAcp(treeSession, "tree", {
      MEMORA_MOCK_PROCESS_MARKER_PREFIX: treeMarkerPrefix,
      MEMORA_MOCK_PROCESS_MARKER_TOKEN: treeMarkerToken,
    });
    await browser.waitUntil(
      async () =>
  ...
  it("registers MEMORA MCP with the live local API URL and optional key", async () => {
    ...
    const fixtureEnv: Record<string, string> = {
      MEMORA_MOCK_EXPECT_MCP_URL: expectedUrl,
      MEMORA_MOCK_EXPECT_MCP_HAS_KEY: String(Boolean(api.key)),
    };
    if (api.key) fixtureEnv.MEMORA_MOCK_EXPECT_MCP_KEY = api.key;
    ...
    expect(events.some(
      (envelope) =>
        ...
        envelope.event?.update?.content?.text ===
          "Mock MEMORA MCP registration verified",
    )).toBe(true);
  ...
  it("scrubs cloud tokens while running ACP terminal callbacks", async () => {
    await startAcp(terminalSession, "terminal", {
      MEMORA_API_KEY: "forbidden-adapter-sentinel",
  ...
  it("fails startup promptly and reaps the runtime when the adapter exits", async () => {
    ...
    const startupError = await invokeOrThrow<string>("plugin:e2e|capture_pi_start_error", {
      ...
      projectDir: path.join(os.tmpdir(), `MEMORA-acp-e2e-${exitSession}`),
  ...
  it("refuses a coding-agent preset that lost its ACP backend instead of charging the cloud", async () => {
    ...
    // provider with "MEMORA", which sent the agent id to the gateway as a
    // model name; the resulting 403 was rendered as "upgrade to MEMORA
    // Business" on accounts that already held the plan.
    const startupError = await invokeOrThrow<string>("plugin:e2e|capture_pi_start_error", {
      sessionId: orphanedPresetSession,
      projectDir: path.join(os.tmpdir(), `MEMORA-acp-e2e-${orphanedPresetSession}`),
      providerConfig: {
        provider: "acp",
        url: "",
        model: "codex-acp",
