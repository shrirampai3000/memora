 ...
 *   2. Log in via the synthetic `deep-link-received` channel â†’ the account card
 ...
 * state). after() fully signs out (re-login â†’ click logout) so the residual
...
const FAKE_EMAIL = "e2e-stale-sub@MEMORA.test";
 ...
 *  subscription. Matches by path so it survives the github.com/shrirampai3000/memora â†’ github.com/shrirampai3000/memora
...
/** Open Home â†’ Settings â†’ Account so the login status + plan card render. */
...
describe.skip("Account never shows an active plan card under a not-logged-in header", function () {
  ...
  it("hides the cloud plan card once the token is gone but cloud_subscribed lingers", async () => {
    // â”€â”€ Phase A: log in as a cloud subscriber; the active card must show â”€â”€â”€â”€â”€â”€
    ...
    const signIn = await $("h3*=Sign in to MEMORA");
