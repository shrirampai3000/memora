...
describe("Within-chat context loss (issue #3636 — user's actual bug)", function () {
  ...
  it("keeps the bounded recovery snapshot available on every frontend send", async () => {
    // â”€â”€ Turn 1: seed prior history into React state and stream a fake
    //          assistant reply. This does NOT call sendPiMessage, so the
    //          newly-started Pi process still has no conversation state. â”€â”€
    ...
    // â”€â”€ Turn 2: the frontend supplies history as a recovery payload. â”€â”€
    ...
    // â”€â”€ Turn 3: the frontend remains stateless and supplies the snapshot
    //          again; the Rust test below owns the cold/warm decision. â”€â”€
