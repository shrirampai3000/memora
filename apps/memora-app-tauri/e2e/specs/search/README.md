...
```
MEMORA_E2E_SEED=onboarding,no-recording,search-fixture
```
...
- 12 frames whose OCR text contains **"vector"** â€” enough to exercise the
  results grid layout.
...
The script sets `MEMORA_PORT` to a non-default port so the harness app
boots its own server even on a dev machine already running MEMORA on
`:3030`. Add `RECORD_VIDEO=1` to capture an mp4 under `e2e/videos/`.

## Specs

- `search-bugs-4645.spec.ts` â€” reproduces the grid-reflow bug from #4645: on a
...
- **highlight over-match** (`ThumbnailHighlightOverlay` uses
  `text.includes(term)`) â€” wasn't reproducible deterministically via the
  rendered overlay; likely better as a unit test of the matcher, or needs
  root-cause work.
- **occurrence arrows, frame-preview text selection, URL/frame off-by-one** â€”
