 ...
 *
 * Covers the HD feature set that shipped across v2.4.292 â†’ v2.4.300:
 ...
 *
 * Build + run from apps/MEMORA-app-tauri:
 *   NEXT_PUBLIC_MEMORA_E2E=true bun tauri build --no-sign --debug \
 *     --no-bundle -- --features e2e
 *   bun run test:e2e:hd:macos
 *   # which is: MEMORA_E2E_SEED=onboarding bun run wdio run e2e/wdio.conf.ts \
...
describe("HD recording pipeline (high-fps capture)", function () {
  ...
  it("starts an HD session, writes a CFR mp4 chunk, and keeps indexing frames", async function () {
    ...
    // Foreground a non-MEMORA window so OCR has real, non-filtered content
    // to index during the HD window (timeline/search drop "MEMORA" frames).
