...
async function installFetchRecorder(): Promise<void> {
  await browser.execute(() => {
    const w = window as typeof window & {
      __MEMORASearchFetchLog?: SearchFetchLog[];
      __MEMORAOriginalFetch?: typeof fetch;
    };

    w.__MEMORASearchFetchLog = [];

    if (w.__MEMORAOriginalFetch) return;

    w.__MEMORAOriginalFetch = window.fetch.bind(window);
    window.fetch = ((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return w.__MEMORAOriginalFetch!(input, init);
    }) as typeof fetch;
  });
}
...
async function getFetchLog(): Promise<SearchFetchLog[]> {
  return (await browser.execute(() => {
    ...
    return w.__MEMORASearchFetchLog ?? [];
  })) as SearchFetchLog[];
}
...
describe("Search request priority", function () {
  ...
  it("sends keyword search before secondary search, facet, and speaker requests", async () => {
    ...
    await input.setValue(`MEMORA-priority-${Date.now()}`);
