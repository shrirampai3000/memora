 ...
 * sign-out (updateSettings with user â†’ null) and broadcast across windows via a
 * "MEMORA-auth-signout" event. loadUser snapshots the generation at entry
...
const FAKE_EMAIL = "e2e-logout@MEMORA.test";
 ...
 *  get a 401, and the auth interceptor broadcasts "MEMORA-auth-signout",
 ...
 *  specific host so it survives the github.com/shrirampai3000/memora â†’ github.com/shrirampai3000/memora domain switch. */
...
/** Open Home â†’ Settings â†’ Account so the login status + logout button render. */
...
describe("Logout is not resurrected by an in-flight loadUser", function () {
  ...
  it("stays logged out after one click even when a slow loadUser resolves afterwards", async () => {
    // â”€â”€ Phase A: log in (fast mock) so the logout button is present â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Phase B: slow loadUser in flight, then logout before it resolves â”€â”€â”€â”€â”€
    const SLOW = t(2_500);
