 ...
 *   MEMORA_E2E_SEED=onboarding,no-recording MEMORA_PORT=3048 \
...
const FORCE_BILLING_GATE_KEY = "MEMORA_e2e_force_billing_gate";
...
describe("meeting workspace tabs and responsive layout", function () {
  ...
  before(async () => {
    ...
    try {
      ...
      await browser
        .saveScreenshot("/tmp/MEMORA-meeting-workspace-home-failure.png")
        .catch(() => undefined);
      throw error;
    }
  ...
  it("keeps tab state and content stable through a narrow-window resize", async () => {
    ...
    const screenshotPath = resolve(
      "/tmp",
      "MEMORA-meeting-workspace-responsive.png",
    );
