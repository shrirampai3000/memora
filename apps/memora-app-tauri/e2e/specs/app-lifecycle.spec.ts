...
describe("App lifecycle and UI stability", function () {
  ...
  it("round-trips localStorage", async () => {
    const ok = (await browser.execute(() => {
      localStorage.setItem("MEMORA-e2e-localstorage", "ok");
      const value = localStorage.getItem("MEMORA-e2e-localstorage");
      localStorage.removeItem("MEMORA-e2e-localstorage");
      return value === "ok";
    })) as boolean;
    expect(ok).toBe(true);
  });
  ...
  it("round-trips sessionStorage", async () => {
    const ok = (await browser.execute(() => {
      sessionStorage.setItem("MEMORA-e2e-sessionstorage", "ok");
      const value = sessionStorage.getItem("MEMORA-e2e-sessionstorage");
      sessionStorage.removeItem("MEMORA-e2e-sessionstorage");
      return value === "ok";
    })) as boolean;
    expect(ok).toBe(true);
  });
