...
// Regression for 2005157ec ("fix: centralize MEMORA viewer link
...
// `MEMORA://view?path=...` parsing + `open_viewer_window` invocation.
...
describe("Viewer deeplink: openMEMORAViewerLink â†’ open_viewer_window", function () {
  ...
  before(async () => {
    tmpDir = mkdtempSync(resolvePath(tmpdir(), "MEMORA-e2e-viewer-"));
