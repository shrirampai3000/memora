 ...
 *    freshly logged-in user kept the "MEMORA Cloud is not active" fallback
 *    (and local Whisper) for up to ten minutes. See MEMORA/MEMORA#4339.
 ...
 *   1. Seed `cloud-audio-fallback` â†’ audio engine = MEMORA-cloud, logged out.
...
const FAKE_EMAIL = "e2e-reverify@MEMORA.test";
 ...
 *  the github.com/shrirampai3000/memora â†’ github.com/shrirampai3000/memora host switch. */
...
(canRun ? describe : describe.skip)("audio fallback clears on login (free tier gets cloud)", function () {
  ...
  it("flips from 'not active' to active when a free (unsubscribed) user logs in", async () => {
    // â”€â”€ Phase A: logged-out seed â†’ recording settings shows the fallback alert â”€
