...
const commitmentsPayloads: Record<string, object> = {
  ...
  "inbox-context": {
    content:
      "MEMORA could verify these from the recorded call and the later log handoff. The video-control follow-up has no explicit deadline, so it remains **needs review** rather than overdue.",
  },
};
...
const accountingPayloads: Record<string, object> = {
  ...
  "accounting-boundary": {
    content:
      "Compared the connected QuickBooks vendor ledger with Gmail attachments. MEMORA found candidates but **did not change the books, pay anything, or contact a vendor**.",
  },
};
...
describe("Live View reusable item actions", function () {
  ...
  it("snoozes, corrects, resolves, removes, reopens, and rehydrates the same generic list", async () => {
    ...
    await correctionInput.setValue(
      "The OS prompt came from the connected Pipe, not MEMORA capture",
    );
    await browser.pause(t(300));
