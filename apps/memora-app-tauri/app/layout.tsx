...
export default function RootLayout({
  ...
  useEffect(() => {
    ...
    // Write immediately so the stack lands in ~/.MEMORA/MEMORA-app.
          ...
          dangerouslySetInnerHTML={{
            __html: `
              (function() {
                ...
                try {
                  var theme = localStorage.getItem('MEMORA-ui-theme');
                ...
                try {
                  var savedFs = localStorage.getItem('MEMORA-font-size');
                  if (savedFs) {
      ...
      <body className={`${inter.className} antialiased scrollbar-hide ${isSearch ? "bg-transparent" : ""}`}>
        ...
        <Providers>
          {/* DeeplinkHandler is mounted in Providers (outside the entitlement
              gate) so the memora:// login callback is always caught, even
