...
type MainSection = "home" | "timeline" | "brain" | "pipes" | "connections" | "help";
...
const ALL_SECTIONS = [
  "home", "timeline", "pipes", "help", "brain", "connections", "history",
  "feedback", // backwards compat â†’ maps to "help"
  "memories", // backwards compat â†’ maps to "brain"
  "artifacts", // backwards compat â†’ maps to "brain"
];
...
function HomeContent() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if ((e.metaKey || e.ctrlKey) && !e.shiftKey && !e.altKey && e.key.toLowerCase() === "n") {
        ...
        setActiveSection("home");
        startNewChat();
        // Focus the chat input. When standalone-chat is already mounted (homeâ†’home)
  ...
  const refreshRecordingDevices = useCallback(async () => {
    try {
      const [health, visionStatus, capturePausedResult]: [
      ...
      if (Array.isArray(visionStatus) && visionStatus.length > 0) {
        ...
        // display-only (no id â†’ no pause control).
  ...
  return (
    <>
      ...
      />
          ...
          <div
            className={cn(
              // top-0.5 + items-center puts each icon's center at yâ‰ˆ15px,
          ...
          >
            <Tooltip>
              ...
              <TooltipContent side="bottom" className="text-xs">
                {sidebarCollapsed ? "expand sidebar" : "collapse sidebar"} <kbd className="ml-1 px-1 py-0.5 bg-muted rounded text-[10px]" suppressHydrationWarning>{isMac ? "âŒ˜B" : "Ctrl+B"}</kbd>
              </TooltipContent>
            </Tooltip>
