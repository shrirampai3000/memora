...
MEMORA is most useful when it's running 24/7 â€” which is exactly what makes it
awkward to hack on. The installed prod app already holds port `3030` and
`~/.MEMORA`, so naively running a dev build alongside it is the silent-capture
collision in [#3466](https://github.com/MEMORA/MEMORA/issues/3466).
...
| script | what it does |
|--------|--------------|
| `sp-dev-app` | quit prod app â†’ `bun tauri dev` â†’ **restore prod on exit** (even on crash/Ctrl-C) |
...
They're optional accelerators, not a required toolchain â€” each one wraps patterns
...
- **Hacking on the desktop app (UI, tray, Tauri commands)** â†’ `sp-dev-app`. It uses
  the real `~/.MEMORA`, so you get realistic data. The catch: a dev DB migration
  can permanently alter your prod DB. For risky migrations, use the CLI mode below.
- **Hacking on the CLI/core/engine, or testing a migration** â†’ `sp-dev-cli`. It runs
  against a throwaway data dir (`$TMPDIR/MEMORA-dev`) on port `3031`, so your prod
...
3. **`bun tauri dev`/`bun tauri build` auto-run `pre_build.js`; a raw `cargo build`
   ...
   ```bash
   cd apps/MEMORA-app-tauri && bun scripts/pre_build.js
   ```

   (Invoke the `.js` directly, or `export PATH="$HOME/.bun/bin:$PATH"` first â€” the
...
toolchain. The scripts are machine-agnostic â€” `MEMORA_SRC_DIR` points each at its
local clone â€” so nothing here changes; it's purely a hardware choice. Skip it if you
...
a fork release asset) â€” don't commit it to the repo. Unlike the dev scripts above,
this one isn't macOS-specific.

## Scope

macOS on Apple Silicon, which is MEMORA's primary dev target. The `sp-*` dev
scripts use `osascript`/`pgrep`/`open` semantics that are macOS-specific; they aren't
written or tested for Linux or Windows (`pr-evidence` is portable). Build MEMORA
