...
# complex (~1600 node) DOM while the MEMORA engine records. The orphaned
# 2s/focus-change tree walker freezes such apps >100ms roughly once per 1.2s;
# after the fix those freezes must be gone.
#
# Usage:
#   .\verify-walker-fix.ps1 -EngineExe <path\to\MEMORA.exe> -Label before
#   .\verify-walker-fix.ps1 -EngineExe <path\to\MEMORA.exe> -Label after
...
# refuse to measure with a foreign MEMORA instance running
$existing = Get-Process | Where-Object { $_.ProcessName -eq 'MEMORA' }
if ($existing -and -not $Force) {
    Write-Output 'FAIL: a MEMORA process is already running; stop it first (or pass -Force to kill it).'
    exit 2
}
...
Get-Process | Where-Object { $_.ProcessName -match '^MEMORA$|ffmpeg' } | ForEach-Object { try { Stop-Process -Id $_.Id -Force -Confirm:$false -ErrorAction Stop } catch {} }
