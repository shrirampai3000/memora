...
> check details against `crates/MEMORA-core/src/pipes`. Run `bun scripts/check-doc-freshness.ts` for current drift.

## Current Architecture

All pipe state lives in-memory (`Arc<Mutex<HashMap>>`). Logs written as JSON files to `~/.MEMORA/pipes/{name}/logs/`. A global `Semaphore::new(1)` serializes all pipe execution. The Pi agent subprocess uses `wait_with_output()` with **no timeout**. PID is captured from `child.id()` at spawn (line 198 of pi.rs) but stored as `ExecutionHandle { pid: 0 }` in the running map (lines 371, 756 of mod.rs) â€” the real PID from spawn is never written back.

### State Dependencies (what a single pipe run touches)

```
pipe.md (config + prompt)
    â”œâ”€â”€ store.bin (AI preset â†’ model + provider)
    â”‚     â””â”€â”€ may be written by Tauri app concurrently
    â”œâ”€â”€ ~/.pi/agent/models.json (provider config, merged on every run)
    â”œâ”€â”€ ~/.pi/agent/auth.json (API keys, merged on every run)
    â”œâ”€â”€ pi binary (found via PATH/known locations)
    â”œâ”€â”€ MEMORA API at localhost:3030 (queried by pipe during execution)
    â”‚     â””â”€â”€ SQLite DB (OCR, audio, UI data the pipe reads)
    â”œâ”€â”€ LLM provider API (external network call)
    â”‚     â””â”€â”€ API key (from auth.json or MEMORA_API_KEY env)
    â”œâ”€â”€ ./output/ directory (pipe writes results here)
    â””â”€â”€ ./logs/ directory (execution logs written after completion)
```
...
**But:** `resolve_preset()` reads `store.bin` with no locking. If Tauri writes store.bin at the exact moment the scheduler reads it, you get a partial read â†’ JSON parse failure â†’ `None` â†’ silent fallback to pipe.md defaults.
...
**Current:** There IS no queue â€” preset resolved right before `executor.run()`. But with the proposed DB queue, a pipe could sit queued for minutes while user changes preset.
...
**Current:** `resolve_preset()` returns `None` â†’ falls back to pipe.md `model:` field. Silent. User thinks they're using Claude Opus but pipe runs with Haiku (the pipe.md default).
...
**Current:** `ensure_pi_config()` writes `MEMORA_API_KEY` env var and merges auth.json. But if MEMORA cloud token expires, pi gets a 401. Stderr says "unauthorized" but user sees generic failure.
...
**Current:** Provider URL comes from preset â†’ models.json merge. If URL becomes unreachable, pi hangs on HTTP timeout (could be 30s-2min depending on the HTTP client pi uses internally).
...
**Current:** `serde_json::from_str()` returns `Err` â†’ `resolve_preset()` returns `None` â†’ silent fallback.
...
**Current:** Only maps `pi`, `native-ollama`, `openai`, `custom`. Any other provider string â†’ `None` â†’ no provider passed to pi â†’ pi uses its own default.
...
**Current:** Process killed by OS. In-memory state lost entirely. No record the execution ever happened. Pipe might have been mid-write to output file â†’ partial/corrupt output.
**Required:**
...
#### B3. macOS App Nap throttles MEMORA
**Current:** App Nap can suspend the server process â†’ scheduler stops ticking â†’ pipes don't run on schedule. When un-napped, scheduler resumes but missed windows are gone.
**Required:**
- Already mitigated by `NSProcessInfo` activity assertion in Tauri app
- But CLI (`MEMORA` binary) doesn't have this protection
- For CLI: document that App Nap can affect scheduling reliability

#### B4. Computer clock jumps (NTP sync, timezone change, DST)
**Current:** Scheduler uses `Utc::now()` and `Local::now()`. `last_run` is in-memory HashMap. Clock jump forward â†’ multiple pipes suddenly "due". Clock jump backward â†’ pipe that just ran appears to not have run yet (re-runs).
**Required:**
...
#### B5. Laptop lid close â†’ WiFi drops â†’ LLM API mid-response
...
**Required:** At minimum, log to stderr (tracing). For DB: SQLite will return SQLITE_FULL â€” handle gracefully, don't crash.

#### C3. Logs accumulate indefinitely
**Current:** No rotation. Every run creates a new JSON file in `~/.MEMORA/pipes/{name}/logs/`. Over months, thousands of small files.
**Required:**
...
- DB: same file permissions as main MEMORA DB
- Consider: option to redact logs (strip API keys, PII) â€” but complex, phase 3+
...
- Don't auto-delete â€” user may want partial results
...
#### C8. Cannot correlate pipe output with MEMORA data
**Current:** Pipe queries MEMORA API with a time range. Output written to files. No link between "which frames/audio the pipe saw" and "what it produced".
...
- Or: split into `spawn() â†’ ExecutionHandle` and `wait(handle) â†’ AgentOutput`
...
**Required:** Already handled â€” `spawn()` error propagates to `PipeRunLog { success: false }`. But error message should be clear: "pi binary not found at {path}".
...
#### D6. Multiple MEMORA instances running pipes
...
**But:** There's a race â€” `last_run.insert()` at line 751 happens before `running.insert()` at line 756. A narrow window exists where the running check passes for both.
**Required:** Atomic operation: insert into running map AND update last_run in same lock scope.

#### E4. Scheduler task panics
**Current:** `tokio::spawn()` â€” if the closure panics, the task dies silently. Pipes stop being scheduled. No error visible to user.
...
**Current:** `should_run()` compares `now >= next_occurrence(last_run)`. If clock jumps backward, `now` might be before the next occurrence â†’ pipe doesn't run.
...
**But:** If user edits pipe.md, `load_pipes()` isn't called automatically â€” config is stale until next scheduler tick re-reads? Actually no â€” pipes are loaded once at startup (line 227-275) and only modified by explicit API calls (enable/disable/update). Scheduler uses the in-memory snapshot.
...
**Current:** YAML parse failure â†’ pipe skipped during `load_pipes()`. No error surfaced.
...
**Required (future):** Content validation â€” pipe.md must have valid frontmatter. Warn about unknown agents. Don't auto-enable installed pipes.

---

### G. Network & API Dependencies

#### G1. MEMORA API not ready when pipe runs
**Current:** Pipe prompt says "MEMORA API: http://localhost:3030". Pi queries this. If server still booting, pi gets connection refused. Fails.
**Required:** Pipe scheduler should only start after server is listening. Currently scheduler starts at line 912 of MEMORA-server.rs, server starts at line ~950. There's a potential race.

#### G2. MEMORA API port is not 3030
**Current:** Hardcoded in `render_prompt()` (line 992): `MEMORA API: http://localhost:3030`. If user runs on port 3031, pipe queries wrong port.
...
**Required:**
- Chat sessions are different from pipes (interactive vs batch) â€” keep separate
...
**Required:** Not directly related to pipes (pipes query API, not hardware). But if MEMORA server is blocked on a permission dialog, API requests from pi hang â†’ pi hangs â†’ timeout kills it. Acceptable with timeout in place.
...
CREATE TABLE pipe_executions (
    ...
    status            TEXT NOT NULL DEFAULT 'queued',
        -- queued â†’ running â†’ completed | failed | cancelled | timed_out
    ...
    -- execution context (snapshot at queue time â€” immutable after creation)
    ...
    api_port          INTEGER,       -- MEMORA port used in prompt
    ...
    -- data context (what MEMORA data window this execution covered)
...
```
          ...
          â”€â”€â”€â”€â”€â”€â”€â”€â–º â”‚         â”‚
                    â””â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”˜
                         â”‚ semaphore acquired, spawn process
                         â–¼
                    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”
                    â”‚ running â”‚â”€â”€â”€â”€ PID set in DB
                    â”‚         â”‚â”€â”€â”€â”€ stdout/stderr streaming to DB
                    â””â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”˜
                    â•±    â”‚    â•²
                   â•±     â”‚     â•²
                  â–¼      â–¼      â–¼
           â”Œâ”€â”€â”€â”€â”€â”€â” â”Œâ”€â”€â”€â”€â”€â”€â” â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
           â”‚compl.â”‚ â”‚failedâ”‚ â”‚timed_out â”‚
           â””â”€â”€â”€â”€â”€â”€â”˜ â””â”€â”€â”¬â”€â”€â”€â”˜ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                       â”‚
                  retry_count < max?
                  yes â†’ new row (trigger='retry', retry_of=id)
                  no  â†’ stay failed

    At any point from queued or running:
        user cancel â†’ cancelled
        server shutdown â†’ failed (error_type='shutdown')
        sleep/crash â†’ failed (error_type='interrupted', detected on startup)
```
...
| Phase | What | Blocks |
|-------|------|--------|
| **P0** | Execution timeout (5min default) + process group kill | Everything â€” without this, one hung pipe = system dead |
