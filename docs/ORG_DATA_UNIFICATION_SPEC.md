...
<!-- doc-covers: crates/MEMORA-team-memory -->
<!-- doc-verified: 8c0d11e2f -->
> **Current.** Last verified against 8c0d11e2f (2026-06-12).

Status: proposal. Spans two repos: `MEMORA` (desktop/engine, this repo) and
`website-MEMORA` (cloud control plane + runner).
...
Do **not** merge the engines â€” sqlite does not federate (30 employees = 30 dbs, not one
queryable org brain). Unify the three layers *above* storage instead:

1. **Artifact shape** â€” one SAF envelope, local and cloud.
2. **Query interface** â€” one contract, so a pipe runs unchanged local or cloud.
3. **On-demand frame fetch** â€” ship only the ~30 frames a SOP cites, not the firehose.

Sequencing: **P3 â†’ P1 â†’ P2** (hot ask first, then the shared envelope, then the big rock).

---

## P1 â€” Unified artifact shape (SAF everywhere)

### Current state
- Cloud has a real envelope: `Artifact` in
  `website-MEMORA/lib/enterprise/artifacts/types.ts:128` (`saf_version`, `artifact_id`,
...
- Local has a *different* shape: `OutputRecord` in
  `MEMORA/crates/MEMORA-db/src/types.rs:305` (`source`, `source_type`, `title`,
  `kind`, `output_path`, `preview`, `metadata`) served by
  `crates/MEMORA-engine/src/routes/outputs.rs`. It is a file registry, not a typed
...
1. **Shared SAF contract.** Lift the SAF types into one source of truth both repos consume.
   Cheapest: a published `@MEMORA/saf` types package (or vendored `saf.ts` in each repo
...
2. **Local can emit SAF.** Add SAF as an *optional richer layer* over the existing outputs
   registry â€” do not remove plain files. Migration on `outputs`:
   ```sql
...
3. **One renderer.** The desktop artifacts view learns to render a SAF body (it already
   ...
   `website-MEMORA/app/account/workspace/enterprise-workflows-dashboard.tsx`). One
...
- SAF validation must be lenient on unknown `kind` (forward-compat) â€” the envelope rails are
...
## P2 â€” One query interface (portable pipes)

### Current state â€” three different access patterns
- **Local pipes**: hit `localhost:3030` â€” `/search?content_type=â€¦`, `/frames/{id}` (JPEG,
...
- **Cloud HTTP**: a *third* surface already exists at
  `website-MEMORA/app/api/enterprise/v1/*` â€” `records`, `search`, `rollups`, `devices`,
...
### Proposed change
1. **Define the MEMORA query contract** (one doc, versioned): the read surface every
   runtime must serve.
   - `GET /search` â€” `q`, `content_type` (ocr|audio|ui|all), `start_time`/`end_time`,
     `app_name`, `limit`, `offset`.
   - `GET /frames/{id}` â€” `image/jpeg` (see P3 for the cloud impl).
...
2. **Runner serves the contract over the lake.** Instead of pipes `cat`-ing JSONL, the runner
   ...
   `$MEMORA_API` (default `http://localhost:3030`) and stop caring where they are.
3. **Rewrite seed pipes to query, not cat.** `sop-generator` / `workflow-discovery` ask
   ...
   "some orgs have GBs, never cat blindly" hazard the prompts currently warn about by hand â€”
...
- The `v1` API stays; we tighten param parity and add `/frames/{id}`. Existing callers keep
  working.
- Runner shim is new surface area on the VM â€” keep it loopback-only, no external port.
...
## P3 â€” On-demand frame fetch (cheap images) â€” DO FIRST

### Current state
- Only `kind:"snapshot"` records carry an image to the cloud, shipped ~1 per 5-min sync as a
  288Ã—180 JPEG of "the latest frame at sync time" (desktop `enterprise_sync.rs`
...
### Proposed change â€” lazy push of cited frames
...
2. **Frame-request channel.** When a runner pipe cites frame_ids it wants as images, it writes
   ...
   `{ frame_ids: [12200, 12431, â€¦], requested_at }`. Capped (e.g. â‰¤200 ids).
...
4. **Resolution at render.** `EvidenceRef` already has `frame_id` (`types.ts:68`). The
   dashboard already resolves `snapshot:N` â†’ data URI (`enterprise-workflows-dashboard.tsx`
   `resolveSnapshotPlaceholders`); generalize it to `frame_id` â†’ signed org-storage URL for
   `frames/{license}/{device}/{id}.jpg`. **No new SAF body field** â€” the citation *is* the
   image pointer.
...
- Pull (runner â†’ device tunnel) breaks on offline laptops.
- Bulk upload of all frames is the wrong target (gigabytes uploaded to use kilobytes; raw
  ...
  cites: ~30 frames/SOP/day/device â†’ MB/mo, ~$0.01/mo storage at R2.
...
- Redaction is load-bearing â€” never upload an unredacted frame. Reuse the existing redact
